 ...
 * Like {@link org.opennms.alec.mcp.McpMetrics} it is published as an OSGi
 * service, folded by the ALEC driver into the engine's JMX-reported registry
 * and therefore charted by OpenNMS's stock JMX collection ("ALEC Graph
 * Stats") with no OpenNMS-side change. Gauges only, because that collection's
...
public class DefaultLlmUsageMetrics implements LlmUsageMetrics, MetricSet {
    ...
    private final Map<ToolConsumer, LongAdder> tokensByConsumer = new EnumMap<>(ToolConsumer.class);
    private final Map<ToolConsumer, LongAdder> callsByConsumer = new EnumMap<>(ToolConsumer.class);
    ...
    public void recordRound(ToolConsumer consumer, TokenUsage usage) {
        ...
        tokensByConsumer.get(consumer == null ? ToolConsumer.RCA : consumer).add(total);
    }
    ...
    public void recordCall(ToolConsumer consumer, boolean success) {
        ...
        callsByConsumer.get(consumer == null ? ToolConsumer.RCA : consumer).increment();
    }
    ...
    public long getTokens(ToolConsumer consumer) {
    ...
    public Map<String, Metric> getMetrics() {
        ...
        m.put(TOKENS_RCA, (Gauge<Long>) () -> getTokens(ToolConsumer.RCA));
        m.put(TOKENS_CLUSTERING, (Gauge<Long>) () -> getTokens(ToolConsumer.CLUSTERING));
        m.put(TOKENS_VALIDATION, (Gauge<Long>) () -> getTokens(ToolConsumer.VALIDATION));
        m.put(CALLS_RCA, (Gauge<Long>) () -> getCalls(ToolConsumer.RCA));
        m.put(CALLS_CLUSTERING, (Gauge<Long>) () -> getCalls(ToolConsumer.CLUSTERING));
