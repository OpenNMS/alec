...
public class Overlay {
    ...
    public static Path createTmpOverlayInClasspath(String overlayDirToCopy) throws Exception {
        URL resourceDirURL = Thread.currentThread()
                .getContextClassLoader()
                .getResource(Objects.requireNonNull(OVERLAY_DIR));
        ...
        if (overlayDirToCopy != null) {
            File toCopy = new File(Paths.get(resourceDirURL.getPath(), overlayDirToCopy).toUri());
            if (toCopy.exists()) {
                FileUtils.copyDirectory(toCopy, tmpResourceDir.toFile());
            }
        }
    ...
    public static String getOverlayDir(String dir) {
        return Paths.get(OVERLAY_DIR,dir).toString();
    }
}
