const Qe = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
}, _w = {}, Lw = window.Vue.resolveComponent, qw = window.Vue.createVNode, Kw = window.Vue.openBlock, $w = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const eU = { class: "main" };
function tU(n, a) {
  const r = Lw("router-view");
  return Kw(), $w("div", eU, [
    qw(r)
  ]);
}
const nU = /* @__PURE__ */ Qe(_w, [["render", tU], ["__scopeId", "data-v-5d32d140"]]), rU = window.Vue.defineComponent, iU = window.Vue.openBlock, aU = window.Vue.createBlock, oU = /* @__PURE__ */ rU({
  setup(n) {
    return (a, r) => (iU(), aU(nU));
  }
});
var Qr = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function lU(n) {
  return n && n.__esModule && Object.prototype.hasOwnProperty.call(n, "default") ? n.default : n;
}
var eh = { exports: {} }, rs = { exports: {} }, th = function(a, r) {
  return function() {
    for (var s = new Array(arguments.length), f = 0; f < s.length; f++)
      s[f] = arguments[f];
    return a.apply(r, s);
  };
}, sU = th, is = Object.prototype.toString, as = function(n) {
  return function(a) {
    var r = is.call(a);
    return n[r] || (n[r] = r.slice(8, -1).toLowerCase());
  };
}(/* @__PURE__ */ Object.create(null));
function Sn(n) {
  return n = n.toLowerCase(), function(r) {
    return as(r) === n;
  };
}
function os(n) {
  return Array.isArray(n);
}
function Ua(n) {
  return typeof n > "u";
}
function cU(n) {
  return n !== null && !Ua(n) && n.constructor !== null && !Ua(n.constructor) && typeof n.constructor.isBuffer == "function" && n.constructor.isBuffer(n);
}
var nh = Sn("ArrayBuffer");
function uU(n) {
  var a;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? a = ArrayBuffer.isView(n) : a = n && n.buffer && nh(n.buffer), a;
}
function dU(n) {
  return typeof n == "string";
}
function fU(n) {
  return typeof n == "number";
}
function rh(n) {
  return n !== null && typeof n == "object";
}
function ha(n) {
  if (as(n) !== "object")
    return !1;
  var a = Object.getPrototypeOf(n);
  return a === null || a === Object.prototype;
}
var hU = Sn("Date"), pU = Sn("File"), mU = Sn("Blob"), VU = Sn("FileList");
function ls(n) {
  return is.call(n) === "[object Function]";
}
function TU(n) {
  return rh(n) && ls(n.pipe);
}
function vU(n) {
  var a = "[object FormData]";
  return n && (typeof FormData == "function" && n instanceof FormData || is.call(n) === a || ls(n.toString) && n.toString() === a);
}
var wU = Sn("URLSearchParams");
function UU(n) {
  return n.trim ? n.trim() : n.replace(/^\s+|\s+$/g, "");
}
function RU() {
  return typeof navigator < "u" && (navigator.product === "ReactNative" || navigator.product === "NativeScript" || navigator.product === "NS") ? !1 : typeof window < "u" && typeof document < "u";
}
function ss(n, a) {
  if (!(n === null || typeof n > "u"))
    if (typeof n != "object" && (n = [n]), os(n))
      for (var r = 0, l = n.length; r < l; r++)
        a.call(null, n[r], r, n);
    else
      for (var s in n)
        Object.prototype.hasOwnProperty.call(n, s) && a.call(null, n[s], s, n);
}
function Il() {
  var n = {};
  function a(s, f) {
    ha(n[f]) && ha(s) ? n[f] = Il(n[f], s) : ha(s) ? n[f] = Il({}, s) : os(s) ? n[f] = s.slice() : n[f] = s;
  }
  for (var r = 0, l = arguments.length; r < l; r++)
    ss(arguments[r], a);
  return n;
}
function NU(n, a, r) {
  return ss(a, function(s, f) {
    r && typeof s == "function" ? n[f] = sU(s, r) : n[f] = s;
  }), n;
}
function gU(n) {
  return n.charCodeAt(0) === 65279 && (n = n.slice(1)), n;
}
function MU(n, a, r, l) {
  n.prototype = Object.create(a.prototype, l), n.prototype.constructor = n, r && Object.assign(n.prototype, r);
}
function ZU(n, a, r) {
  var l, s, f, h = {};
  a = a || {};
  do {
    for (l = Object.getOwnPropertyNames(n), s = l.length; s-- > 0; )
      f = l[s], h[f] || (a[f] = n[f], h[f] = !0);
    n = Object.getPrototypeOf(n);
  } while (n && (!r || r(n, a)) && n !== Object.prototype);
  return a;
}
function JU(n, a, r) {
  n = String(n), (r === void 0 || r > n.length) && (r = n.length), r -= a.length;
  var l = n.indexOf(a, r);
  return l !== -1 && l === r;
}
function kU(n) {
  if (!n)
    return null;
  var a = n.length;
  if (Ua(a))
    return null;
  for (var r = new Array(a); a-- > 0; )
    r[a] = n[a];
  return r;
}
var yU = function(n) {
  return function(a) {
    return n && a instanceof n;
  };
}(typeof Uint8Array < "u" && Object.getPrototypeOf(Uint8Array)), We = {
  isArray: os,
  isArrayBuffer: nh,
  isBuffer: cU,
  isFormData: vU,
  isArrayBufferView: uU,
  isString: dU,
  isNumber: fU,
  isObject: rh,
  isPlainObject: ha,
  isUndefined: Ua,
  isDate: hU,
  isFile: pU,
  isBlob: mU,
  isFunction: ls,
  isStream: TU,
  isURLSearchParams: wU,
  isStandardBrowserEnv: RU,
  forEach: ss,
  merge: Il,
  extend: NU,
  trim: UU,
  stripBOM: gU,
  inherits: MU,
  toFlatObject: ZU,
  kindOf: as,
  kindOfTest: Sn,
  endsWith: JU,
  toArray: kU,
  isTypedArray: yU,
  isFileList: VU
}, qn = We;
function ad(n) {
  return encodeURIComponent(n).replace(/%3A/gi, ":").replace(/%24/g, "$").replace(/%2C/gi, ",").replace(/%20/g, "+").replace(/%5B/gi, "[").replace(/%5D/gi, "]");
}
var ih = function(a, r, l) {
  if (!r)
    return a;
  var s;
  if (l)
    s = l(r);
  else if (qn.isURLSearchParams(r))
    s = r.toString();
  else {
    var f = [];
    qn.forEach(r, function(T, N) {
      T === null || typeof T > "u" || (qn.isArray(T) ? N = N + "[]" : T = [T], qn.forEach(T, function(k) {
        qn.isDate(k) ? k = k.toISOString() : qn.isObject(k) && (k = JSON.stringify(k)), f.push(ad(N) + "=" + ad(k));
      }));
    }), s = f.join("&");
  }
  if (s) {
    var h = a.indexOf("#");
    h !== -1 && (a = a.slice(0, h)), a += (a.indexOf("?") === -1 ? "?" : "&") + s;
  }
  return a;
}, bU = We;
function Sa() {
  this.handlers = [];
}
Sa.prototype.use = function(a, r, l) {
  return this.handlers.push({
    fulfilled: a,
    rejected: r,
    synchronous: l ? l.synchronous : !1,
    runWhen: l ? l.runWhen : null
  }), this.handlers.length - 1;
};
Sa.prototype.eject = function(a) {
  this.handlers[a] && (this.handlers[a] = null);
};
Sa.prototype.forEach = function(a) {
  bU.forEach(this.handlers, function(l) {
    l !== null && a(l);
  });
};
var EU = Sa, FU = We, WU = function(a, r) {
  FU.forEach(a, function(s, f) {
    f !== r && f.toUpperCase() === r.toUpperCase() && (a[r] = s, delete a[f]);
  });
}, ah = We;
function ur(n, a, r, l, s) {
  Error.call(this), this.message = n, this.name = "AxiosError", a && (this.code = a), r && (this.config = r), l && (this.request = l), s && (this.response = s);
}
ah.inherits(ur, Error, {
  toJSON: function() {
    return {
      message: this.message,
      name: this.name,
      description: this.description,
      number: this.number,
      fileName: this.fileName,
      lineNumber: this.lineNumber,
      columnNumber: this.columnNumber,
      stack: this.stack,
      config: this.config,
      code: this.code,
      status: this.response && this.response.status ? this.response.status : null
    };
  }
});
var oh = ur.prototype, lh = {};
[
  "ERR_BAD_OPTION_VALUE",
  "ERR_BAD_OPTION",
  "ECONNABORTED",
  "ETIMEDOUT",
  "ERR_NETWORK",
  "ERR_FR_TOO_MANY_REDIRECTS",
  "ERR_DEPRECATED",
  "ERR_BAD_RESPONSE",
  "ERR_BAD_REQUEST",
  "ERR_CANCELED"
].forEach(function(n) {
  lh[n] = { value: n };
});
Object.defineProperties(ur, lh);
Object.defineProperty(oh, "isAxiosError", { value: !0 });
ur.from = function(n, a, r, l, s, f) {
  var h = Object.create(oh);
  return ah.toFlatObject(n, h, function(T) {
    return T !== Error.prototype;
  }), ur.call(h, n.message, a, r, l, s), h.name = n.name, f && Object.assign(h, f), h;
};
var hr = ur, sh = {
  silentJSONParsing: !0,
  forcedJSONParsing: !0,
  clarifyTimeoutError: !1
}, Nt = We;
function SU(n, a) {
  a = a || new FormData();
  var r = [];
  function l(f) {
    return f === null ? "" : Nt.isDate(f) ? f.toISOString() : Nt.isArrayBuffer(f) || Nt.isTypedArray(f) ? typeof Blob == "function" ? new Blob([f]) : Buffer.from(f) : f;
  }
  function s(f, h) {
    if (Nt.isPlainObject(f) || Nt.isArray(f)) {
      if (r.indexOf(f) !== -1)
        throw Error("Circular reference detected in " + h);
      r.push(f), Nt.forEach(f, function(T, N) {
        if (!Nt.isUndefined(T)) {
          var g = h ? h + "." + N : N, k;
          if (T && !h && typeof T == "object") {
            if (Nt.endsWith(N, "{}"))
              T = JSON.stringify(T);
            else if (Nt.endsWith(N, "[]") && (k = Nt.toArray(T))) {
              k.forEach(function(b) {
                !Nt.isUndefined(b) && a.append(g, l(b));
              });
              return;
            }
          }
          s(T, g);
        }
      }), r.pop();
    } else
      a.append(h, l(f));
  }
  return s(n), a;
}
var ch = SU, ol, od;
function AU() {
  if (od)
    return ol;
  od = 1;
  var n = hr;
  return ol = function(r, l, s) {
    var f = s.config.validateStatus;
    !s.status || !f || f(s.status) ? r(s) : l(new n(
      "Request failed with status code " + s.status,
      [n.ERR_BAD_REQUEST, n.ERR_BAD_RESPONSE][Math.floor(s.status / 100) - 4],
      s.config,
      s.request,
      s
    ));
  }, ol;
}
var ll, ld;
function xU() {
  if (ld)
    return ll;
  ld = 1;
  var n = We;
  return ll = n.isStandardBrowserEnv() ? function() {
    return {
      write: function(l, s, f, h, p, T) {
        var N = [];
        N.push(l + "=" + encodeURIComponent(s)), n.isNumber(f) && N.push("expires=" + new Date(f).toGMTString()), n.isString(h) && N.push("path=" + h), n.isString(p) && N.push("domain=" + p), T === !0 && N.push("secure"), document.cookie = N.join("; ");
      },
      read: function(l) {
        var s = document.cookie.match(new RegExp("(^|;\\s*)(" + l + ")=([^;]*)"));
        return s ? decodeURIComponent(s[3]) : null;
      },
      remove: function(l) {
        this.write(l, "", Date.now() - 864e5);
      }
    };
  }() : function() {
    return {
      write: function() {
      },
      read: function() {
        return null;
      },
      remove: function() {
      }
    };
  }(), ll;
}
var BU = function(a) {
  return /^([a-z][a-z\d+\-.]*:)?\/\//i.test(a);
}, CU = function(a, r) {
  return r ? a.replace(/\/+$/, "") + "/" + r.replace(/^\/+/, "") : a;
}, DU = BU, QU = CU, uh = function(a, r) {
  return a && !DU(r) ? QU(a, r) : r;
}, sl, sd;
function OU() {
  if (sd)
    return sl;
  sd = 1;
  var n = We, a = [
    "age",
    "authorization",
    "content-length",
    "content-type",
    "etag",
    "expires",
    "from",
    "host",
    "if-modified-since",
    "if-unmodified-since",
    "last-modified",
    "location",
    "max-forwards",
    "proxy-authorization",
    "referer",
    "retry-after",
    "user-agent"
  ];
  return sl = function(l) {
    var s = {}, f, h, p;
    return l && n.forEach(l.split(`
`), function(N) {
      if (p = N.indexOf(":"), f = n.trim(N.substr(0, p)).toLowerCase(), h = n.trim(N.substr(p + 1)), f) {
        if (s[f] && a.indexOf(f) >= 0)
          return;
        f === "set-cookie" ? s[f] = (s[f] ? s[f] : []).concat([h]) : s[f] = s[f] ? s[f] + ", " + h : h;
      }
    }), s;
  }, sl;
}
var cl, cd;
function zU() {
  if (cd)
    return cl;
  cd = 1;
  var n = We;
  return cl = n.isStandardBrowserEnv() ? function() {
    var r = /(msie|trident)/i.test(navigator.userAgent), l = document.createElement("a"), s;
    function f(h) {
      var p = h;
      return r && (l.setAttribute("href", p), p = l.href), l.setAttribute("href", p), {
        href: l.href,
        protocol: l.protocol ? l.protocol.replace(/:$/, "") : "",
        host: l.host,
        search: l.search ? l.search.replace(/^\?/, "") : "",
        hash: l.hash ? l.hash.replace(/^#/, "") : "",
        hostname: l.hostname,
        port: l.port,
        pathname: l.pathname.charAt(0) === "/" ? l.pathname : "/" + l.pathname
      };
    }
    return s = f(window.location.href), function(p) {
      var T = n.isString(p) ? f(p) : p;
      return T.protocol === s.protocol && T.host === s.host;
    };
  }() : function() {
    return function() {
      return !0;
    };
  }(), cl;
}
var ul, ud;
function Aa() {
  if (ud)
    return ul;
  ud = 1;
  var n = hr, a = We;
  function r(l) {
    n.call(this, l == null ? "canceled" : l, n.ERR_CANCELED), this.name = "CanceledError";
  }
  return a.inherits(r, n, {
    __CANCEL__: !0
  }), ul = r, ul;
}
var dl, dd;
function GU() {
  return dd || (dd = 1, dl = function(a) {
    var r = /^([-+\w]{1,25})(:?\/\/|:)/.exec(a);
    return r && r[1] || "";
  }), dl;
}
var fl, fd;
function hd() {
  if (fd)
    return fl;
  fd = 1;
  var n = We, a = AU(), r = xU(), l = ih, s = uh, f = OU(), h = zU(), p = sh, T = hr, N = Aa(), g = GU();
  return fl = function(b) {
    return new Promise(function(F, C) {
      var D = b.data, A = b.headers, Q = b.responseType, S;
      function H() {
        b.cancelToken && b.cancelToken.unsubscribe(S), b.signal && b.signal.removeEventListener("abort", S);
      }
      n.isFormData(D) && n.isStandardBrowserEnv() && delete A["Content-Type"];
      var y = new XMLHttpRequest();
      if (b.auth) {
        var _ = b.auth.username || "", $ = b.auth.password ? unescape(encodeURIComponent(b.auth.password)) : "";
        A.Authorization = "Basic " + btoa(_ + ":" + $);
      }
      var L = s(b.baseURL, b.url);
      y.open(b.method.toUpperCase(), l(L, b.params, b.paramsSerializer), !0), y.timeout = b.timeout;
      function Ae() {
        if (!!y) {
          var fe = "getAllResponseHeaders" in y ? f(y.getAllResponseHeaders()) : null, pe = !Q || Q === "text" || Q === "json" ? y.responseText : y.response, we = {
            data: pe,
            status: y.status,
            statusText: y.statusText,
            headers: fe,
            config: b,
            request: y
          };
          a(function(ke) {
            F(ke), H();
          }, function(ke) {
            C(ke), H();
          }, we), y = null;
        }
      }
      if ("onloadend" in y ? y.onloadend = Ae : y.onreadystatechange = function() {
        !y || y.readyState !== 4 || y.status === 0 && !(y.responseURL && y.responseURL.indexOf("file:") === 0) || setTimeout(Ae);
      }, y.onabort = function() {
        !y || (C(new T("Request aborted", T.ECONNABORTED, b, y)), y = null);
      }, y.onerror = function() {
        C(new T("Network Error", T.ERR_NETWORK, b, y, y)), y = null;
      }, y.ontimeout = function() {
        var pe = b.timeout ? "timeout of " + b.timeout + "ms exceeded" : "timeout exceeded", we = b.transitional || p;
        b.timeoutErrorMessage && (pe = b.timeoutErrorMessage), C(new T(
          pe,
          we.clarifyTimeoutError ? T.ETIMEDOUT : T.ECONNABORTED,
          b,
          y
        )), y = null;
      }, n.isStandardBrowserEnv()) {
        var Pe = (b.withCredentials || h(L)) && b.xsrfCookieName ? r.read(b.xsrfCookieName) : void 0;
        Pe && (A[b.xsrfHeaderName] = Pe);
      }
      "setRequestHeader" in y && n.forEach(A, function(pe, we) {
        typeof D > "u" && we.toLowerCase() === "content-type" ? delete A[we] : y.setRequestHeader(we, pe);
      }), n.isUndefined(b.withCredentials) || (y.withCredentials = !!b.withCredentials), Q && Q !== "json" && (y.responseType = b.responseType), typeof b.onDownloadProgress == "function" && y.addEventListener("progress", b.onDownloadProgress), typeof b.onUploadProgress == "function" && y.upload && y.upload.addEventListener("progress", b.onUploadProgress), (b.cancelToken || b.signal) && (S = function(fe) {
        !y || (C(!fe || fe && fe.type ? new N() : fe), y.abort(), y = null);
      }, b.cancelToken && b.cancelToken.subscribe(S), b.signal && (b.signal.aborted ? S() : b.signal.addEventListener("abort", S))), D || (D = null);
      var de = g(L);
      if (de && ["http", "https", "file"].indexOf(de) === -1) {
        C(new T("Unsupported protocol " + de + ":", T.ERR_BAD_REQUEST, b));
        return;
      }
      y.send(D);
    });
  }, fl;
}
var hl, pd;
function YU() {
  return pd || (pd = 1, hl = null), hl;
}
var Ze = We, md = WU, Vd = hr, IU = sh, HU = ch, PU = {
  "Content-Type": "application/x-www-form-urlencoded"
};
function Td(n, a) {
  !Ze.isUndefined(n) && Ze.isUndefined(n["Content-Type"]) && (n["Content-Type"] = a);
}
function jU() {
  var n;
  return (typeof XMLHttpRequest < "u" || typeof process < "u" && Object.prototype.toString.call(process) === "[object process]") && (n = hd()), n;
}
function XU(n, a, r) {
  if (Ze.isString(n))
    try {
      return (a || JSON.parse)(n), Ze.trim(n);
    } catch (l) {
      if (l.name !== "SyntaxError")
        throw l;
    }
  return (r || JSON.stringify)(n);
}
var xa = {
  transitional: IU,
  adapter: jU(),
  transformRequest: [function(a, r) {
    if (md(r, "Accept"), md(r, "Content-Type"), Ze.isFormData(a) || Ze.isArrayBuffer(a) || Ze.isBuffer(a) || Ze.isStream(a) || Ze.isFile(a) || Ze.isBlob(a))
      return a;
    if (Ze.isArrayBufferView(a))
      return a.buffer;
    if (Ze.isURLSearchParams(a))
      return Td(r, "application/x-www-form-urlencoded;charset=utf-8"), a.toString();
    var l = Ze.isObject(a), s = r && r["Content-Type"], f;
    if ((f = Ze.isFileList(a)) || l && s === "multipart/form-data") {
      var h = this.env && this.env.FormData;
      return HU(f ? { "files[]": a } : a, h && new h());
    } else if (l || s === "application/json")
      return Td(r, "application/json"), XU(a);
    return a;
  }],
  transformResponse: [function(a) {
    var r = this.transitional || xa.transitional, l = r && r.silentJSONParsing, s = r && r.forcedJSONParsing, f = !l && this.responseType === "json";
    if (f || s && Ze.isString(a) && a.length)
      try {
        return JSON.parse(a);
      } catch (h) {
        if (f)
          throw h.name === "SyntaxError" ? Vd.from(h, Vd.ERR_BAD_RESPONSE, this, null, this.response) : h;
      }
    return a;
  }],
  timeout: 0,
  xsrfCookieName: "XSRF-TOKEN",
  xsrfHeaderName: "X-XSRF-TOKEN",
  maxContentLength: -1,
  maxBodyLength: -1,
  env: {
    FormData: YU()
  },
  validateStatus: function(a) {
    return a >= 200 && a < 300;
  },
  headers: {
    common: {
      Accept: "application/json, text/plain, */*"
    }
  }
};
Ze.forEach(["delete", "get", "head"], function(a) {
  xa.headers[a] = {};
});
Ze.forEach(["post", "put", "patch"], function(a) {
  xa.headers[a] = Ze.merge(PU);
});
var cs = xa, _U = We, LU = cs, qU = function(a, r, l) {
  var s = this || LU;
  return _U.forEach(l, function(h) {
    a = h.call(s, a, r);
  }), a;
}, pl, vd;
function dh() {
  return vd || (vd = 1, pl = function(a) {
    return !!(a && a.__CANCEL__);
  }), pl;
}
var wd = We, ml = qU, KU = dh(), $U = cs, eR = Aa();
function Vl(n) {
  if (n.cancelToken && n.cancelToken.throwIfRequested(), n.signal && n.signal.aborted)
    throw new eR();
}
var tR = function(a) {
  Vl(a), a.headers = a.headers || {}, a.data = ml.call(
    a,
    a.data,
    a.headers,
    a.transformRequest
  ), a.headers = wd.merge(
    a.headers.common || {},
    a.headers[a.method] || {},
    a.headers
  ), wd.forEach(
    ["delete", "get", "head", "post", "put", "patch", "common"],
    function(s) {
      delete a.headers[s];
    }
  );
  var r = a.adapter || $U.adapter;
  return r(a).then(function(s) {
    return Vl(a), s.data = ml.call(
      a,
      s.data,
      s.headers,
      a.transformResponse
    ), s;
  }, function(s) {
    return KU(s) || (Vl(a), s && s.response && (s.response.data = ml.call(
      a,
      s.response.data,
      s.response.headers,
      a.transformResponse
    ))), Promise.reject(s);
  });
}, at = We, fh = function(a, r) {
  r = r || {};
  var l = {};
  function s(g, k) {
    return at.isPlainObject(g) && at.isPlainObject(k) ? at.merge(g, k) : at.isPlainObject(k) ? at.merge({}, k) : at.isArray(k) ? k.slice() : k;
  }
  function f(g) {
    if (at.isUndefined(r[g])) {
      if (!at.isUndefined(a[g]))
        return s(void 0, a[g]);
    } else
      return s(a[g], r[g]);
  }
  function h(g) {
    if (!at.isUndefined(r[g]))
      return s(void 0, r[g]);
  }
  function p(g) {
    if (at.isUndefined(r[g])) {
      if (!at.isUndefined(a[g]))
        return s(void 0, a[g]);
    } else
      return s(void 0, r[g]);
  }
  function T(g) {
    if (g in r)
      return s(a[g], r[g]);
    if (g in a)
      return s(void 0, a[g]);
  }
  var N = {
    url: h,
    method: h,
    data: h,
    baseURL: p,
    transformRequest: p,
    transformResponse: p,
    paramsSerializer: p,
    timeout: p,
    timeoutMessage: p,
    withCredentials: p,
    adapter: p,
    responseType: p,
    xsrfCookieName: p,
    xsrfHeaderName: p,
    onUploadProgress: p,
    onDownloadProgress: p,
    decompress: p,
    maxContentLength: p,
    maxBodyLength: p,
    beforeRedirect: p,
    transport: p,
    httpAgent: p,
    httpsAgent: p,
    cancelToken: p,
    socketPath: p,
    responseEncoding: p,
    validateStatus: T
  };
  return at.forEach(Object.keys(a).concat(Object.keys(r)), function(k) {
    var b = N[k] || f, B = b(k);
    at.isUndefined(B) && b !== T || (l[k] = B);
  }), l;
}, Tl, Ud;
function hh() {
  return Ud || (Ud = 1, Tl = {
    version: "0.27.2"
  }), Tl;
}
var nR = hh().version, sn = hr, us = {};
["object", "boolean", "number", "function", "string", "symbol"].forEach(function(n, a) {
  us[n] = function(l) {
    return typeof l === n || "a" + (a < 1 ? "n " : " ") + n;
  };
});
var Rd = {};
us.transitional = function(a, r, l) {
  function s(f, h) {
    return "[Axios v" + nR + "] Transitional option '" + f + "'" + h + (l ? ". " + l : "");
  }
  return function(f, h, p) {
    if (a === !1)
      throw new sn(
        s(h, " has been removed" + (r ? " in " + r : "")),
        sn.ERR_DEPRECATED
      );
    return r && !Rd[h] && (Rd[h] = !0, console.warn(
      s(
        h,
        " has been deprecated since v" + r + " and will be removed in the near future"
      )
    )), a ? a(f, h, p) : !0;
  };
};
function rR(n, a, r) {
  if (typeof n != "object")
    throw new sn("options must be an object", sn.ERR_BAD_OPTION_VALUE);
  for (var l = Object.keys(n), s = l.length; s-- > 0; ) {
    var f = l[s], h = a[f];
    if (h) {
      var p = n[f], T = p === void 0 || h(p, f, n);
      if (T !== !0)
        throw new sn("option " + f + " must be " + T, sn.ERR_BAD_OPTION_VALUE);
      continue;
    }
    if (r !== !0)
      throw new sn("Unknown option " + f, sn.ERR_BAD_OPTION);
  }
}
var iR = {
  assertOptions: rR,
  validators: us
}, ph = We, aR = ih, Nd = EU, gd = tR, Ba = fh, oR = uh, mh = iR, Kn = mh.validators;
function dr(n) {
  this.defaults = n, this.interceptors = {
    request: new Nd(),
    response: new Nd()
  };
}
dr.prototype.request = function(a, r) {
  typeof a == "string" ? (r = r || {}, r.url = a) : r = a || {}, r = Ba(this.defaults, r), r.method ? r.method = r.method.toLowerCase() : this.defaults.method ? r.method = this.defaults.method.toLowerCase() : r.method = "get";
  var l = r.transitional;
  l !== void 0 && mh.assertOptions(l, {
    silentJSONParsing: Kn.transitional(Kn.boolean),
    forcedJSONParsing: Kn.transitional(Kn.boolean),
    clarifyTimeoutError: Kn.transitional(Kn.boolean)
  }, !1);
  var s = [], f = !0;
  this.interceptors.request.forEach(function(B) {
    typeof B.runWhen == "function" && B.runWhen(r) === !1 || (f = f && B.synchronous, s.unshift(B.fulfilled, B.rejected));
  });
  var h = [];
  this.interceptors.response.forEach(function(B) {
    h.push(B.fulfilled, B.rejected);
  });
  var p;
  if (!f) {
    var T = [gd, void 0];
    for (Array.prototype.unshift.apply(T, s), T = T.concat(h), p = Promise.resolve(r); T.length; )
      p = p.then(T.shift(), T.shift());
    return p;
  }
  for (var N = r; s.length; ) {
    var g = s.shift(), k = s.shift();
    try {
      N = g(N);
    } catch (b) {
      k(b);
      break;
    }
  }
  try {
    p = gd(N);
  } catch (b) {
    return Promise.reject(b);
  }
  for (; h.length; )
    p = p.then(h.shift(), h.shift());
  return p;
};
dr.prototype.getUri = function(a) {
  a = Ba(this.defaults, a);
  var r = oR(a.baseURL, a.url);
  return aR(r, a.params, a.paramsSerializer);
};
ph.forEach(["delete", "get", "head", "options"], function(a) {
  dr.prototype[a] = function(r, l) {
    return this.request(Ba(l || {}, {
      method: a,
      url: r,
      data: (l || {}).data
    }));
  };
});
ph.forEach(["post", "put", "patch"], function(a) {
  function r(l) {
    return function(f, h, p) {
      return this.request(Ba(p || {}, {
        method: a,
        headers: l ? {
          "Content-Type": "multipart/form-data"
        } : {},
        url: f,
        data: h
      }));
    };
  }
  dr.prototype[a] = r(), dr.prototype[a + "Form"] = r(!0);
});
var lR = dr, vl, Md;
function sR() {
  if (Md)
    return vl;
  Md = 1;
  var n = Aa();
  function a(r) {
    if (typeof r != "function")
      throw new TypeError("executor must be a function.");
    var l;
    this.promise = new Promise(function(h) {
      l = h;
    });
    var s = this;
    this.promise.then(function(f) {
      if (!!s._listeners) {
        var h, p = s._listeners.length;
        for (h = 0; h < p; h++)
          s._listeners[h](f);
        s._listeners = null;
      }
    }), this.promise.then = function(f) {
      var h, p = new Promise(function(T) {
        s.subscribe(T), h = T;
      }).then(f);
      return p.cancel = function() {
        s.unsubscribe(h);
      }, p;
    }, r(function(h) {
      s.reason || (s.reason = new n(h), l(s.reason));
    });
  }
  return a.prototype.throwIfRequested = function() {
    if (this.reason)
      throw this.reason;
  }, a.prototype.subscribe = function(l) {
    if (this.reason) {
      l(this.reason);
      return;
    }
    this._listeners ? this._listeners.push(l) : this._listeners = [l];
  }, a.prototype.unsubscribe = function(l) {
    if (!!this._listeners) {
      var s = this._listeners.indexOf(l);
      s !== -1 && this._listeners.splice(s, 1);
    }
  }, a.source = function() {
    var l, s = new a(function(h) {
      l = h;
    });
    return {
      token: s,
      cancel: l
    };
  }, vl = a, vl;
}
var wl, Zd;
function cR() {
  return Zd || (Zd = 1, wl = function(a) {
    return function(l) {
      return a.apply(null, l);
    };
  }), wl;
}
var Ul, Jd;
function uR() {
  if (Jd)
    return Ul;
  Jd = 1;
  var n = We;
  return Ul = function(r) {
    return n.isObject(r) && r.isAxiosError === !0;
  }, Ul;
}
var kd = We, dR = th, pa = lR, fR = fh, hR = cs;
function Vh(n) {
  var a = new pa(n), r = dR(pa.prototype.request, a);
  return kd.extend(r, pa.prototype, a), kd.extend(r, a), r.create = function(s) {
    return Vh(fR(n, s));
  }, r;
}
var Ke = Vh(hR);
Ke.Axios = pa;
Ke.CanceledError = Aa();
Ke.CancelToken = sR();
Ke.isCancel = dh();
Ke.VERSION = hh().version;
Ke.toFormData = ch;
Ke.AxiosError = hr;
Ke.Cancel = Ke.CanceledError;
Ke.all = function(a) {
  return Promise.all(a);
};
Ke.spread = cR();
Ke.isAxiosError = uR();
rs.exports = Ke;
rs.exports.default = Ke;
(function(n) {
  n.exports = rs.exports;
})(eh);
const Th = /* @__PURE__ */ lU(eh.exports);
Th.create({
  baseURL: "/opennms/api/v2".toString() || "/opennms/api/v2",
  withCredentials: !0
});
const pr = Th.create({
  baseURL: "/opennms/rest".toString() || "/opennms/rest",
  withCredentials: !0
}), gt = {
  ENGINE_DBSCAN: "dbscan",
  ENGINE_DEEP_LEARNING: "cluster",
  HELLINGER_OPTION: "hellinger",
  SPACE_DISTANCE_OPTION: "alarminspaceandtimedistance",
  DATE_FORMAT: "d/M HH:mm:ss",
  ACCEPTED: "ACCEPTED",
  REJECTED: "REJECTED"
}, pR = "/alec", mR = "/alec/engine/configuration", vh = "/alec/agreement/configuration", VR = "/alec/situation", wh = async (n) => {
  try {
    return (await pr.post(`${vh}`, {
      agreed: n
    })).status === 201;
  } catch {
    return !1;
  }
}, TR = async () => {
  try {
    const n = await pr.get(`${vh}`);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, vR = async (n, a) => {
  try {
    return (await pr.post(mR, {
      distanceMeasureName: a ? gt.HELLINGER_OPTION : gt.SPACE_DISTANCE_OPTION,
      engineName: n,
      alpha: 144.47117699,
      beta: 0.55257784,
      epsilon: 100
    })).status === 200;
  } catch {
    return !1;
  }
}, wR = async (n, a) => {
  try {
    return (await pr.post(`${pR}/situation/${a}/${n}`, {
      headers: {
        "Access-Control-Allow-Origin": "*"
      }
    })).status === 200;
  } catch {
    return !1;
  }
}, UR = async () => {
  try {
    const n = await pr.get(VR);
    return n.status === 200, n.data;
  } catch {
    return !1;
  }
};
var qe = { exports: {} };
/**
 * @license
 * Lodash <https://lodash.com/>
 * Copyright OpenJS Foundation and other contributors <https://openjsf.org/>
 * Released under MIT license <https://lodash.com/license>
 * Based on Underscore.js 1.8.3 <http://underscorejs.org/LICENSE>
 * Copyright Jeremy Ashkenas, DocumentCloud and Investigative Reporters & Editors
 */
(function(n, a) {
  (function() {
    var r, l = "4.17.21", s = 200, f = "Unsupported core-js use. Try https://npms.io/search?q=ponyfill.", h = "Expected a function", p = "Invalid `variable` option passed into `_.template`", T = "__lodash_hash_undefined__", N = 500, g = "__lodash_placeholder__", k = 1, b = 2, B = 4, F = 1, C = 2, D = 1, A = 2, Q = 4, S = 8, H = 16, y = 32, _ = 64, $ = 128, L = 256, Ae = 512, Pe = 30, de = "...", fe = 800, pe = 16, we = 1, ae = 2, ke = 3, ye = 1 / 0, Oe = 9007199254740991, Vr = 17976931348623157e292, si = 0 / 0, Zt = 4294967295, lp = Zt - 1, sp = Zt >>> 1, cp = [
      ["ary", $],
      ["bind", D],
      ["bindKey", A],
      ["curry", S],
      ["curryRight", H],
      ["flip", Ae],
      ["partial", y],
      ["partialRight", _],
      ["rearg", L]
    ], xn = "[object Arguments]", ci = "[object Array]", up = "[object AsyncFunction]", Tr = "[object Boolean]", vr = "[object Date]", dp = "[object DOMException]", ui = "[object Error]", di = "[object Function]", ys = "[object GeneratorFunction]", Vt = "[object Map]", wr = "[object Number]", fp = "[object Null]", Ft = "[object Object]", bs = "[object Promise]", hp = "[object Proxy]", Ur = "[object RegExp]", Tt = "[object Set]", Rr = "[object String]", fi = "[object Symbol]", pp = "[object Undefined]", Nr = "[object WeakMap]", mp = "[object WeakSet]", gr = "[object ArrayBuffer]", Bn = "[object DataView]", Da = "[object Float32Array]", Qa = "[object Float64Array]", Oa = "[object Int8Array]", za = "[object Int16Array]", Ga = "[object Int32Array]", Ya = "[object Uint8Array]", Ia = "[object Uint8ClampedArray]", Ha = "[object Uint16Array]", Pa = "[object Uint32Array]", Vp = /\b__p \+= '';/g, Tp = /\b(__p \+=) '' \+/g, vp = /(__e\(.*?\)|\b__t\)) \+\n'';/g, Es = /&(?:amp|lt|gt|quot|#39);/g, Fs = /[&<>"']/g, wp = RegExp(Es.source), Up = RegExp(Fs.source), Rp = /<%-([\s\S]+?)%>/g, Np = /<%([\s\S]+?)%>/g, Ws = /<%=([\s\S]+?)%>/g, gp = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, Mp = /^\w*$/, Zp = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g, ja = /[\\^$.*+?()[\]{}|]/g, Jp = RegExp(ja.source), Xa = /^\s+/, kp = /\s/, yp = /\{(?:\n\/\* \[wrapped with .+\] \*\/)?\n?/, bp = /\{\n\/\* \[wrapped with (.+)\] \*/, Ep = /,? & /, Fp = /[^\x00-\x2f\x3a-\x40\x5b-\x60\x7b-\x7f]+/g, Wp = /[()=,{}\[\]\/\s]/, Sp = /\\(\\)?/g, Ap = /\$\{([^\\}]*(?:\\.[^\\}]*)*)\}/g, Ss = /\w*$/, xp = /^[-+]0x[0-9a-f]+$/i, Bp = /^0b[01]+$/i, Cp = /^\[object .+?Constructor\]$/, Dp = /^0o[0-7]+$/i, Qp = /^(?:0|[1-9]\d*)$/, Op = /[\xc0-\xd6\xd8-\xf6\xf8-\xff\u0100-\u017f]/g, hi = /($^)/, zp = /['\n\r\u2028\u2029\\]/g, pi = "\\ud800-\\udfff", Gp = "\\u0300-\\u036f", Yp = "\\ufe20-\\ufe2f", Ip = "\\u20d0-\\u20ff", As = Gp + Yp + Ip, xs = "\\u2700-\\u27bf", Bs = "a-z\\xdf-\\xf6\\xf8-\\xff", Hp = "\\xac\\xb1\\xd7\\xf7", Pp = "\\x00-\\x2f\\x3a-\\x40\\x5b-\\x60\\x7b-\\xbf", jp = "\\u2000-\\u206f", Xp = " \\t\\x0b\\f\\xa0\\ufeff\\n\\r\\u2028\\u2029\\u1680\\u180e\\u2000\\u2001\\u2002\\u2003\\u2004\\u2005\\u2006\\u2007\\u2008\\u2009\\u200a\\u202f\\u205f\\u3000", Cs = "A-Z\\xc0-\\xd6\\xd8-\\xde", Ds = "\\ufe0e\\ufe0f", Qs = Hp + Pp + jp + Xp, _a = "['\u2019]", _p = "[" + pi + "]", Os = "[" + Qs + "]", mi = "[" + As + "]", zs = "\\d+", Lp = "[" + xs + "]", Gs = "[" + Bs + "]", Ys = "[^" + pi + Qs + zs + xs + Bs + Cs + "]", La = "\\ud83c[\\udffb-\\udfff]", qp = "(?:" + mi + "|" + La + ")", Is = "[^" + pi + "]", qa = "(?:\\ud83c[\\udde6-\\uddff]){2}", Ka = "[\\ud800-\\udbff][\\udc00-\\udfff]", Cn = "[" + Cs + "]", Hs = "\\u200d", Ps = "(?:" + Gs + "|" + Ys + ")", Kp = "(?:" + Cn + "|" + Ys + ")", js = "(?:" + _a + "(?:d|ll|m|re|s|t|ve))?", Xs = "(?:" + _a + "(?:D|LL|M|RE|S|T|VE))?", _s = qp + "?", Ls = "[" + Ds + "]?", $p = "(?:" + Hs + "(?:" + [Is, qa, Ka].join("|") + ")" + Ls + _s + ")*", em = "\\d*(?:1st|2nd|3rd|(?![123])\\dth)(?=\\b|[A-Z_])", tm = "\\d*(?:1ST|2ND|3RD|(?![123])\\dTH)(?=\\b|[a-z_])", qs = Ls + _s + $p, nm = "(?:" + [Lp, qa, Ka].join("|") + ")" + qs, rm = "(?:" + [Is + mi + "?", mi, qa, Ka, _p].join("|") + ")", im = RegExp(_a, "g"), am = RegExp(mi, "g"), $a = RegExp(La + "(?=" + La + ")|" + rm + qs, "g"), om = RegExp([
      Cn + "?" + Gs + "+" + js + "(?=" + [Os, Cn, "$"].join("|") + ")",
      Kp + "+" + Xs + "(?=" + [Os, Cn + Ps, "$"].join("|") + ")",
      Cn + "?" + Ps + "+" + js,
      Cn + "+" + Xs,
      tm,
      em,
      zs,
      nm
    ].join("|"), "g"), lm = RegExp("[" + Hs + pi + As + Ds + "]"), sm = /[a-z][A-Z]|[A-Z]{2}[a-z]|[0-9][a-zA-Z]|[a-zA-Z][0-9]|[^a-zA-Z0-9 ]/, cm = [
      "Array",
      "Buffer",
      "DataView",
      "Date",
      "Error",
      "Float32Array",
      "Float64Array",
      "Function",
      "Int8Array",
      "Int16Array",
      "Int32Array",
      "Map",
      "Math",
      "Object",
      "Promise",
      "RegExp",
      "Set",
      "String",
      "Symbol",
      "TypeError",
      "Uint8Array",
      "Uint8ClampedArray",
      "Uint16Array",
      "Uint32Array",
      "WeakMap",
      "_",
      "clearTimeout",
      "isFinite",
      "parseInt",
      "setTimeout"
    ], um = -1, se = {};
    se[Da] = se[Qa] = se[Oa] = se[za] = se[Ga] = se[Ya] = se[Ia] = se[Ha] = se[Pa] = !0, se[xn] = se[ci] = se[gr] = se[Tr] = se[Bn] = se[vr] = se[ui] = se[di] = se[Vt] = se[wr] = se[Ft] = se[Ur] = se[Tt] = se[Rr] = se[Nr] = !1;
    var le = {};
    le[xn] = le[ci] = le[gr] = le[Bn] = le[Tr] = le[vr] = le[Da] = le[Qa] = le[Oa] = le[za] = le[Ga] = le[Vt] = le[wr] = le[Ft] = le[Ur] = le[Tt] = le[Rr] = le[fi] = le[Ya] = le[Ia] = le[Ha] = le[Pa] = !0, le[ui] = le[di] = le[Nr] = !1;
    var dm = {
      \u00C0: "A",
      \u00C1: "A",
      \u00C2: "A",
      \u00C3: "A",
      \u00C4: "A",
      \u00C5: "A",
      \u00E0: "a",
      \u00E1: "a",
      \u00E2: "a",
      \u00E3: "a",
      \u00E4: "a",
      \u00E5: "a",
      \u00C7: "C",
      \u00E7: "c",
      \u00D0: "D",
      \u00F0: "d",
      \u00C8: "E",
      \u00C9: "E",
      \u00CA: "E",
      \u00CB: "E",
      \u00E8: "e",
      \u00E9: "e",
      \u00EA: "e",
      \u00EB: "e",
      \u00CC: "I",
      \u00CD: "I",
      \u00CE: "I",
      \u00CF: "I",
      \u00EC: "i",
      \u00ED: "i",
      \u00EE: "i",
      \u00EF: "i",
      \u00D1: "N",
      \u00F1: "n",
      \u00D2: "O",
      \u00D3: "O",
      \u00D4: "O",
      \u00D5: "O",
      \u00D6: "O",
      \u00D8: "O",
      \u00F2: "o",
      \u00F3: "o",
      \u00F4: "o",
      \u00F5: "o",
      \u00F6: "o",
      \u00F8: "o",
      \u00D9: "U",
      \u00DA: "U",
      \u00DB: "U",
      \u00DC: "U",
      \u00F9: "u",
      \u00FA: "u",
      \u00FB: "u",
      \u00FC: "u",
      \u00DD: "Y",
      \u00FD: "y",
      \u00FF: "y",
      \u00C6: "Ae",
      \u00E6: "ae",
      \u00DE: "Th",
      \u00FE: "th",
      \u00DF: "ss",
      \u0100: "A",
      \u0102: "A",
      \u0104: "A",
      \u0101: "a",
      \u0103: "a",
      \u0105: "a",
      \u0106: "C",
      \u0108: "C",
      \u010A: "C",
      \u010C: "C",
      \u0107: "c",
      \u0109: "c",
      \u010B: "c",
      \u010D: "c",
      \u010E: "D",
      \u0110: "D",
      \u010F: "d",
      \u0111: "d",
      \u0112: "E",
      \u0114: "E",
      \u0116: "E",
      \u0118: "E",
      \u011A: "E",
      \u0113: "e",
      \u0115: "e",
      \u0117: "e",
      \u0119: "e",
      \u011B: "e",
      \u011C: "G",
      \u011E: "G",
      \u0120: "G",
      \u0122: "G",
      \u011D: "g",
      \u011F: "g",
      \u0121: "g",
      \u0123: "g",
      \u0124: "H",
      \u0126: "H",
      \u0125: "h",
      \u0127: "h",
      \u0128: "I",
      \u012A: "I",
      \u012C: "I",
      \u012E: "I",
      \u0130: "I",
      \u0129: "i",
      \u012B: "i",
      \u012D: "i",
      \u012F: "i",
      \u0131: "i",
      \u0134: "J",
      \u0135: "j",
      \u0136: "K",
      \u0137: "k",
      \u0138: "k",
      \u0139: "L",
      \u013B: "L",
      \u013D: "L",
      \u013F: "L",
      \u0141: "L",
      \u013A: "l",
      \u013C: "l",
      \u013E: "l",
      \u0140: "l",
      \u0142: "l",
      \u0143: "N",
      \u0145: "N",
      \u0147: "N",
      \u014A: "N",
      \u0144: "n",
      \u0146: "n",
      \u0148: "n",
      \u014B: "n",
      \u014C: "O",
      \u014E: "O",
      \u0150: "O",
      \u014D: "o",
      \u014F: "o",
      \u0151: "o",
      \u0154: "R",
      \u0156: "R",
      \u0158: "R",
      \u0155: "r",
      \u0157: "r",
      \u0159: "r",
      \u015A: "S",
      \u015C: "S",
      \u015E: "S",
      \u0160: "S",
      \u015B: "s",
      \u015D: "s",
      \u015F: "s",
      \u0161: "s",
      \u0162: "T",
      \u0164: "T",
      \u0166: "T",
      \u0163: "t",
      \u0165: "t",
      \u0167: "t",
      \u0168: "U",
      \u016A: "U",
      \u016C: "U",
      \u016E: "U",
      \u0170: "U",
      \u0172: "U",
      \u0169: "u",
      \u016B: "u",
      \u016D: "u",
      \u016F: "u",
      \u0171: "u",
      \u0173: "u",
      \u0174: "W",
      \u0175: "w",
      \u0176: "Y",
      \u0177: "y",
      \u0178: "Y",
      \u0179: "Z",
      \u017B: "Z",
      \u017D: "Z",
      \u017A: "z",
      \u017C: "z",
      \u017E: "z",
      \u0132: "IJ",
      \u0133: "ij",
      \u0152: "Oe",
      \u0153: "oe",
      \u0149: "'n",
      \u017F: "s"
    }, fm = {
      "&": "&amp;",
      "<": "&lt;",
      ">": "&gt;",
      '"': "&quot;",
      "'": "&#39;"
    }, hm = {
      "&amp;": "&",
      "&lt;": "<",
      "&gt;": ">",
      "&quot;": '"',
      "&#39;": "'"
    }, pm = {
      "\\": "\\",
      "'": "'",
      "\n": "n",
      "\r": "r",
      "\u2028": "u2028",
      "\u2029": "u2029"
    }, mm = parseFloat, Vm = parseInt, Ks = typeof Qr == "object" && Qr && Qr.Object === Object && Qr, Tm = typeof self == "object" && self && self.Object === Object && self, be = Ks || Tm || Function("return this")(), eo = a && !a.nodeType && a, pn = eo && !0 && n && !n.nodeType && n, $s = pn && pn.exports === eo, to = $s && Ks.process, lt = function() {
      try {
        var v = pn && pn.require && pn.require("util").types;
        return v || to && to.binding && to.binding("util");
      } catch {
      }
    }(), ec = lt && lt.isArrayBuffer, tc = lt && lt.isDate, nc = lt && lt.isMap, rc = lt && lt.isRegExp, ic = lt && lt.isSet, ac = lt && lt.isTypedArray;
    function $e(v, R, U) {
      switch (U.length) {
        case 0:
          return v.call(R);
        case 1:
          return v.call(R, U[0]);
        case 2:
          return v.call(R, U[0], U[1]);
        case 3:
          return v.call(R, U[0], U[1], U[2]);
      }
      return v.apply(R, U);
    }
    function vm(v, R, U, W) {
      for (var Y = -1, te = v == null ? 0 : v.length; ++Y < te; ) {
        var Re = v[Y];
        R(W, Re, U(Re), v);
      }
      return W;
    }
    function st(v, R) {
      for (var U = -1, W = v == null ? 0 : v.length; ++U < W && R(v[U], U, v) !== !1; )
        ;
      return v;
    }
    function wm(v, R) {
      for (var U = v == null ? 0 : v.length; U-- && R(v[U], U, v) !== !1; )
        ;
      return v;
    }
    function oc(v, R) {
      for (var U = -1, W = v == null ? 0 : v.length; ++U < W; )
        if (!R(v[U], U, v))
          return !1;
      return !0;
    }
    function Xt(v, R) {
      for (var U = -1, W = v == null ? 0 : v.length, Y = 0, te = []; ++U < W; ) {
        var Re = v[U];
        R(Re, U, v) && (te[Y++] = Re);
      }
      return te;
    }
    function Vi(v, R) {
      var U = v == null ? 0 : v.length;
      return !!U && Dn(v, R, 0) > -1;
    }
    function no(v, R, U) {
      for (var W = -1, Y = v == null ? 0 : v.length; ++W < Y; )
        if (U(R, v[W]))
          return !0;
      return !1;
    }
    function ce(v, R) {
      for (var U = -1, W = v == null ? 0 : v.length, Y = Array(W); ++U < W; )
        Y[U] = R(v[U], U, v);
      return Y;
    }
    function _t(v, R) {
      for (var U = -1, W = R.length, Y = v.length; ++U < W; )
        v[Y + U] = R[U];
      return v;
    }
    function ro(v, R, U, W) {
      var Y = -1, te = v == null ? 0 : v.length;
      for (W && te && (U = v[++Y]); ++Y < te; )
        U = R(U, v[Y], Y, v);
      return U;
    }
    function Um(v, R, U, W) {
      var Y = v == null ? 0 : v.length;
      for (W && Y && (U = v[--Y]); Y--; )
        U = R(U, v[Y], Y, v);
      return U;
    }
    function io(v, R) {
      for (var U = -1, W = v == null ? 0 : v.length; ++U < W; )
        if (R(v[U], U, v))
          return !0;
      return !1;
    }
    var Rm = ao("length");
    function Nm(v) {
      return v.split("");
    }
    function gm(v) {
      return v.match(Fp) || [];
    }
    function lc(v, R, U) {
      var W;
      return U(v, function(Y, te, Re) {
        if (R(Y, te, Re))
          return W = te, !1;
      }), W;
    }
    function Ti(v, R, U, W) {
      for (var Y = v.length, te = U + (W ? 1 : -1); W ? te-- : ++te < Y; )
        if (R(v[te], te, v))
          return te;
      return -1;
    }
    function Dn(v, R, U) {
      return R === R ? xm(v, R, U) : Ti(v, sc, U);
    }
    function Mm(v, R, U, W) {
      for (var Y = U - 1, te = v.length; ++Y < te; )
        if (W(v[Y], R))
          return Y;
      return -1;
    }
    function sc(v) {
      return v !== v;
    }
    function cc(v, R) {
      var U = v == null ? 0 : v.length;
      return U ? lo(v, R) / U : si;
    }
    function ao(v) {
      return function(R) {
        return R == null ? r : R[v];
      };
    }
    function oo(v) {
      return function(R) {
        return v == null ? r : v[R];
      };
    }
    function uc(v, R, U, W, Y) {
      return Y(v, function(te, Re, oe) {
        U = W ? (W = !1, te) : R(U, te, Re, oe);
      }), U;
    }
    function Zm(v, R) {
      var U = v.length;
      for (v.sort(R); U--; )
        v[U] = v[U].value;
      return v;
    }
    function lo(v, R) {
      for (var U, W = -1, Y = v.length; ++W < Y; ) {
        var te = R(v[W]);
        te !== r && (U = U === r ? te : U + te);
      }
      return U;
    }
    function so(v, R) {
      for (var U = -1, W = Array(v); ++U < v; )
        W[U] = R(U);
      return W;
    }
    function Jm(v, R) {
      return ce(R, function(U) {
        return [U, v[U]];
      });
    }
    function dc(v) {
      return v && v.slice(0, mc(v) + 1).replace(Xa, "");
    }
    function et(v) {
      return function(R) {
        return v(R);
      };
    }
    function co(v, R) {
      return ce(R, function(U) {
        return v[U];
      });
    }
    function Mr(v, R) {
      return v.has(R);
    }
    function fc(v, R) {
      for (var U = -1, W = v.length; ++U < W && Dn(R, v[U], 0) > -1; )
        ;
      return U;
    }
    function hc(v, R) {
      for (var U = v.length; U-- && Dn(R, v[U], 0) > -1; )
        ;
      return U;
    }
    function km(v, R) {
      for (var U = v.length, W = 0; U--; )
        v[U] === R && ++W;
      return W;
    }
    var ym = oo(dm), bm = oo(fm);
    function Em(v) {
      return "\\" + pm[v];
    }
    function Fm(v, R) {
      return v == null ? r : v[R];
    }
    function Qn(v) {
      return lm.test(v);
    }
    function Wm(v) {
      return sm.test(v);
    }
    function Sm(v) {
      for (var R, U = []; !(R = v.next()).done; )
        U.push(R.value);
      return U;
    }
    function uo(v) {
      var R = -1, U = Array(v.size);
      return v.forEach(function(W, Y) {
        U[++R] = [Y, W];
      }), U;
    }
    function pc(v, R) {
      return function(U) {
        return v(R(U));
      };
    }
    function Lt(v, R) {
      for (var U = -1, W = v.length, Y = 0, te = []; ++U < W; ) {
        var Re = v[U];
        (Re === R || Re === g) && (v[U] = g, te[Y++] = U);
      }
      return te;
    }
    function vi(v) {
      var R = -1, U = Array(v.size);
      return v.forEach(function(W) {
        U[++R] = W;
      }), U;
    }
    function Am(v) {
      var R = -1, U = Array(v.size);
      return v.forEach(function(W) {
        U[++R] = [W, W];
      }), U;
    }
    function xm(v, R, U) {
      for (var W = U - 1, Y = v.length; ++W < Y; )
        if (v[W] === R)
          return W;
      return -1;
    }
    function Bm(v, R, U) {
      for (var W = U + 1; W--; )
        if (v[W] === R)
          return W;
      return W;
    }
    function On(v) {
      return Qn(v) ? Dm(v) : Rm(v);
    }
    function vt(v) {
      return Qn(v) ? Qm(v) : Nm(v);
    }
    function mc(v) {
      for (var R = v.length; R-- && kp.test(v.charAt(R)); )
        ;
      return R;
    }
    var Cm = oo(hm);
    function Dm(v) {
      for (var R = $a.lastIndex = 0; $a.test(v); )
        ++R;
      return R;
    }
    function Qm(v) {
      return v.match($a) || [];
    }
    function Om(v) {
      return v.match(om) || [];
    }
    var zm = function v(R) {
      R = R == null ? be : zn.defaults(be.Object(), R, zn.pick(be, cm));
      var U = R.Array, W = R.Date, Y = R.Error, te = R.Function, Re = R.Math, oe = R.Object, fo = R.RegExp, Gm = R.String, ct = R.TypeError, wi = U.prototype, Ym = te.prototype, Gn = oe.prototype, Ui = R["__core-js_shared__"], Ri = Ym.toString, re = Gn.hasOwnProperty, Im = 0, Vc = function() {
        var e = /[^.]+$/.exec(Ui && Ui.keys && Ui.keys.IE_PROTO || "");
        return e ? "Symbol(src)_1." + e : "";
      }(), Ni = Gn.toString, Hm = Ri.call(oe), Pm = be._, jm = fo(
        "^" + Ri.call(re).replace(ja, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"
      ), gi = $s ? R.Buffer : r, qt = R.Symbol, Mi = R.Uint8Array, Tc = gi ? gi.allocUnsafe : r, Zi = pc(oe.getPrototypeOf, oe), vc = oe.create, wc = Gn.propertyIsEnumerable, Ji = wi.splice, Uc = qt ? qt.isConcatSpreadable : r, Zr = qt ? qt.iterator : r, mn = qt ? qt.toStringTag : r, ki = function() {
        try {
          var e = Un(oe, "defineProperty");
          return e({}, "", {}), e;
        } catch {
        }
      }(), Xm = R.clearTimeout !== be.clearTimeout && R.clearTimeout, _m = W && W.now !== be.Date.now && W.now, Lm = R.setTimeout !== be.setTimeout && R.setTimeout, yi = Re.ceil, bi = Re.floor, ho = oe.getOwnPropertySymbols, qm = gi ? gi.isBuffer : r, Rc = R.isFinite, Km = wi.join, $m = pc(oe.keys, oe), Ne = Re.max, xe = Re.min, eV = W.now, tV = R.parseInt, Nc = Re.random, nV = wi.reverse, po = Un(R, "DataView"), Jr = Un(R, "Map"), mo = Un(R, "Promise"), Yn = Un(R, "Set"), kr = Un(R, "WeakMap"), yr = Un(oe, "create"), Ei = kr && new kr(), In = {}, rV = Rn(po), iV = Rn(Jr), aV = Rn(mo), oV = Rn(Yn), lV = Rn(kr), Fi = qt ? qt.prototype : r, br = Fi ? Fi.valueOf : r, gc = Fi ? Fi.toString : r;
      function u(e) {
        if (he(e) && !I(e) && !(e instanceof K)) {
          if (e instanceof ut)
            return e;
          if (re.call(e, "__wrapped__"))
            return Mu(e);
        }
        return new ut(e);
      }
      var Hn = function() {
        function e() {
        }
        return function(t) {
          if (!ue(t))
            return {};
          if (vc)
            return vc(t);
          e.prototype = t;
          var i = new e();
          return e.prototype = r, i;
        };
      }();
      function Wi() {
      }
      function ut(e, t) {
        this.__wrapped__ = e, this.__actions__ = [], this.__chain__ = !!t, this.__index__ = 0, this.__values__ = r;
      }
      u.templateSettings = {
        escape: Rp,
        evaluate: Np,
        interpolate: Ws,
        variable: "",
        imports: {
          _: u
        }
      }, u.prototype = Wi.prototype, u.prototype.constructor = u, ut.prototype = Hn(Wi.prototype), ut.prototype.constructor = ut;
      function K(e) {
        this.__wrapped__ = e, this.__actions__ = [], this.__dir__ = 1, this.__filtered__ = !1, this.__iteratees__ = [], this.__takeCount__ = Zt, this.__views__ = [];
      }
      function sV() {
        var e = new K(this.__wrapped__);
        return e.__actions__ = je(this.__actions__), e.__dir__ = this.__dir__, e.__filtered__ = this.__filtered__, e.__iteratees__ = je(this.__iteratees__), e.__takeCount__ = this.__takeCount__, e.__views__ = je(this.__views__), e;
      }
      function cV() {
        if (this.__filtered__) {
          var e = new K(this);
          e.__dir__ = -1, e.__filtered__ = !0;
        } else
          e = this.clone(), e.__dir__ *= -1;
        return e;
      }
      function uV() {
        var e = this.__wrapped__.value(), t = this.__dir__, i = I(e), o = t < 0, c = i ? e.length : 0, d = N1(0, c, this.__views__), m = d.start, V = d.end, w = V - m, M = o ? V : m - 1, Z = this.__iteratees__, J = Z.length, E = 0, x = xe(w, this.__takeCount__);
        if (!i || !o && c == w && x == w)
          return jc(e, this.__actions__);
        var z = [];
        e:
          for (; w-- && E < x; ) {
            M += t;
            for (var j = -1, G = e[M]; ++j < J; ) {
              var q = Z[j], ee = q.iteratee, rt = q.type, Ye = ee(G);
              if (rt == ae)
                G = Ye;
              else if (!Ye) {
                if (rt == we)
                  continue e;
                break e;
              }
            }
            z[E++] = G;
          }
        return z;
      }
      K.prototype = Hn(Wi.prototype), K.prototype.constructor = K;
      function Vn(e) {
        var t = -1, i = e == null ? 0 : e.length;
        for (this.clear(); ++t < i; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function dV() {
        this.__data__ = yr ? yr(null) : {}, this.size = 0;
      }
      function fV(e) {
        var t = this.has(e) && delete this.__data__[e];
        return this.size -= t ? 1 : 0, t;
      }
      function hV(e) {
        var t = this.__data__;
        if (yr) {
          var i = t[e];
          return i === T ? r : i;
        }
        return re.call(t, e) ? t[e] : r;
      }
      function pV(e) {
        var t = this.__data__;
        return yr ? t[e] !== r : re.call(t, e);
      }
      function mV(e, t) {
        var i = this.__data__;
        return this.size += this.has(e) ? 0 : 1, i[e] = yr && t === r ? T : t, this;
      }
      Vn.prototype.clear = dV, Vn.prototype.delete = fV, Vn.prototype.get = hV, Vn.prototype.has = pV, Vn.prototype.set = mV;
      function Wt(e) {
        var t = -1, i = e == null ? 0 : e.length;
        for (this.clear(); ++t < i; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function VV() {
        this.__data__ = [], this.size = 0;
      }
      function TV(e) {
        var t = this.__data__, i = Si(t, e);
        if (i < 0)
          return !1;
        var o = t.length - 1;
        return i == o ? t.pop() : Ji.call(t, i, 1), --this.size, !0;
      }
      function vV(e) {
        var t = this.__data__, i = Si(t, e);
        return i < 0 ? r : t[i][1];
      }
      function wV(e) {
        return Si(this.__data__, e) > -1;
      }
      function UV(e, t) {
        var i = this.__data__, o = Si(i, e);
        return o < 0 ? (++this.size, i.push([e, t])) : i[o][1] = t, this;
      }
      Wt.prototype.clear = VV, Wt.prototype.delete = TV, Wt.prototype.get = vV, Wt.prototype.has = wV, Wt.prototype.set = UV;
      function St(e) {
        var t = -1, i = e == null ? 0 : e.length;
        for (this.clear(); ++t < i; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function RV() {
        this.size = 0, this.__data__ = {
          hash: new Vn(),
          map: new (Jr || Wt)(),
          string: new Vn()
        };
      }
      function NV(e) {
        var t = Hi(this, e).delete(e);
        return this.size -= t ? 1 : 0, t;
      }
      function gV(e) {
        return Hi(this, e).get(e);
      }
      function MV(e) {
        return Hi(this, e).has(e);
      }
      function ZV(e, t) {
        var i = Hi(this, e), o = i.size;
        return i.set(e, t), this.size += i.size == o ? 0 : 1, this;
      }
      St.prototype.clear = RV, St.prototype.delete = NV, St.prototype.get = gV, St.prototype.has = MV, St.prototype.set = ZV;
      function Tn(e) {
        var t = -1, i = e == null ? 0 : e.length;
        for (this.__data__ = new St(); ++t < i; )
          this.add(e[t]);
      }
      function JV(e) {
        return this.__data__.set(e, T), this;
      }
      function kV(e) {
        return this.__data__.has(e);
      }
      Tn.prototype.add = Tn.prototype.push = JV, Tn.prototype.has = kV;
      function wt(e) {
        var t = this.__data__ = new Wt(e);
        this.size = t.size;
      }
      function yV() {
        this.__data__ = new Wt(), this.size = 0;
      }
      function bV(e) {
        var t = this.__data__, i = t.delete(e);
        return this.size = t.size, i;
      }
      function EV(e) {
        return this.__data__.get(e);
      }
      function FV(e) {
        return this.__data__.has(e);
      }
      function WV(e, t) {
        var i = this.__data__;
        if (i instanceof Wt) {
          var o = i.__data__;
          if (!Jr || o.length < s - 1)
            return o.push([e, t]), this.size = ++i.size, this;
          i = this.__data__ = new St(o);
        }
        return i.set(e, t), this.size = i.size, this;
      }
      wt.prototype.clear = yV, wt.prototype.delete = bV, wt.prototype.get = EV, wt.prototype.has = FV, wt.prototype.set = WV;
      function Mc(e, t) {
        var i = I(e), o = !i && Nn(e), c = !i && !o && nn(e), d = !i && !o && !c && _n(e), m = i || o || c || d, V = m ? so(e.length, Gm) : [], w = V.length;
        for (var M in e)
          (t || re.call(e, M)) && !(m && (M == "length" || c && (M == "offset" || M == "parent") || d && (M == "buffer" || M == "byteLength" || M == "byteOffset") || Ct(M, w))) && V.push(M);
        return V;
      }
      function Zc(e) {
        var t = e.length;
        return t ? e[Jo(0, t - 1)] : r;
      }
      function SV(e, t) {
        return Pi(je(e), vn(t, 0, e.length));
      }
      function AV(e) {
        return Pi(je(e));
      }
      function Vo(e, t, i) {
        (i !== r && !Ut(e[t], i) || i === r && !(t in e)) && At(e, t, i);
      }
      function Er(e, t, i) {
        var o = e[t];
        (!(re.call(e, t) && Ut(o, i)) || i === r && !(t in e)) && At(e, t, i);
      }
      function Si(e, t) {
        for (var i = e.length; i--; )
          if (Ut(e[i][0], t))
            return i;
        return -1;
      }
      function xV(e, t, i, o) {
        return Kt(e, function(c, d, m) {
          t(o, c, i(c), m);
        }), o;
      }
      function Jc(e, t) {
        return e && kt(t, Me(t), e);
      }
      function BV(e, t) {
        return e && kt(t, _e(t), e);
      }
      function At(e, t, i) {
        t == "__proto__" && ki ? ki(e, t, {
          configurable: !0,
          enumerable: !0,
          value: i,
          writable: !0
        }) : e[t] = i;
      }
      function To(e, t) {
        for (var i = -1, o = t.length, c = U(o), d = e == null; ++i < o; )
          c[i] = d ? r : qo(e, t[i]);
        return c;
      }
      function vn(e, t, i) {
        return e === e && (i !== r && (e = e <= i ? e : i), t !== r && (e = e >= t ? e : t)), e;
      }
      function dt(e, t, i, o, c, d) {
        var m, V = t & k, w = t & b, M = t & B;
        if (i && (m = c ? i(e, o, c, d) : i(e)), m !== r)
          return m;
        if (!ue(e))
          return e;
        var Z = I(e);
        if (Z) {
          if (m = M1(e), !V)
            return je(e, m);
        } else {
          var J = Be(e), E = J == di || J == ys;
          if (nn(e))
            return Lc(e, V);
          if (J == Ft || J == xn || E && !c) {
            if (m = w || E ? {} : mu(e), !V)
              return w ? h1(e, BV(m, e)) : f1(e, Jc(m, e));
          } else {
            if (!le[J])
              return c ? e : {};
            m = Z1(e, J, V);
          }
        }
        d || (d = new wt());
        var x = d.get(e);
        if (x)
          return x;
        d.set(e, m), Iu(e) ? e.forEach(function(G) {
          m.add(dt(G, t, i, G, e, d));
        }) : Gu(e) && e.forEach(function(G, q) {
          m.set(q, dt(G, t, i, q, e, d));
        });
        var z = M ? w ? Co : Bo : w ? _e : Me, j = Z ? r : z(e);
        return st(j || e, function(G, q) {
          j && (q = G, G = e[q]), Er(m, q, dt(G, t, i, q, e, d));
        }), m;
      }
      function CV(e) {
        var t = Me(e);
        return function(i) {
          return kc(i, e, t);
        };
      }
      function kc(e, t, i) {
        var o = i.length;
        if (e == null)
          return !o;
        for (e = oe(e); o--; ) {
          var c = i[o], d = t[c], m = e[c];
          if (m === r && !(c in e) || !d(m))
            return !1;
        }
        return !0;
      }
      function yc(e, t, i) {
        if (typeof e != "function")
          throw new ct(h);
        return Cr(function() {
          e.apply(r, i);
        }, t);
      }
      function Fr(e, t, i, o) {
        var c = -1, d = Vi, m = !0, V = e.length, w = [], M = t.length;
        if (!V)
          return w;
        i && (t = ce(t, et(i))), o ? (d = no, m = !1) : t.length >= s && (d = Mr, m = !1, t = new Tn(t));
        e:
          for (; ++c < V; ) {
            var Z = e[c], J = i == null ? Z : i(Z);
            if (Z = o || Z !== 0 ? Z : 0, m && J === J) {
              for (var E = M; E--; )
                if (t[E] === J)
                  continue e;
              w.push(Z);
            } else
              d(t, J, o) || w.push(Z);
          }
        return w;
      }
      var Kt = tu(Jt), bc = tu(wo, !0);
      function DV(e, t) {
        var i = !0;
        return Kt(e, function(o, c, d) {
          return i = !!t(o, c, d), i;
        }), i;
      }
      function Ai(e, t, i) {
        for (var o = -1, c = e.length; ++o < c; ) {
          var d = e[o], m = t(d);
          if (m != null && (V === r ? m === m && !nt(m) : i(m, V)))
            var V = m, w = d;
        }
        return w;
      }
      function QV(e, t, i, o) {
        var c = e.length;
        for (i = P(i), i < 0 && (i = -i > c ? 0 : c + i), o = o === r || o > c ? c : P(o), o < 0 && (o += c), o = i > o ? 0 : Pu(o); i < o; )
          e[i++] = t;
        return e;
      }
      function Ec(e, t) {
        var i = [];
        return Kt(e, function(o, c, d) {
          t(o, c, d) && i.push(o);
        }), i;
      }
      function Ee(e, t, i, o, c) {
        var d = -1, m = e.length;
        for (i || (i = k1), c || (c = []); ++d < m; ) {
          var V = e[d];
          t > 0 && i(V) ? t > 1 ? Ee(V, t - 1, i, o, c) : _t(c, V) : o || (c[c.length] = V);
        }
        return c;
      }
      var vo = nu(), Fc = nu(!0);
      function Jt(e, t) {
        return e && vo(e, t, Me);
      }
      function wo(e, t) {
        return e && Fc(e, t, Me);
      }
      function xi(e, t) {
        return Xt(t, function(i) {
          return Dt(e[i]);
        });
      }
      function wn(e, t) {
        t = en(t, e);
        for (var i = 0, o = t.length; e != null && i < o; )
          e = e[yt(t[i++])];
        return i && i == o ? e : r;
      }
      function Wc(e, t, i) {
        var o = t(e);
        return I(e) ? o : _t(o, i(e));
      }
      function ze(e) {
        return e == null ? e === r ? pp : fp : mn && mn in oe(e) ? R1(e) : A1(e);
      }
      function Uo(e, t) {
        return e > t;
      }
      function OV(e, t) {
        return e != null && re.call(e, t);
      }
      function zV(e, t) {
        return e != null && t in oe(e);
      }
      function GV(e, t, i) {
        return e >= xe(t, i) && e < Ne(t, i);
      }
      function Ro(e, t, i) {
        for (var o = i ? no : Vi, c = e[0].length, d = e.length, m = d, V = U(d), w = 1 / 0, M = []; m--; ) {
          var Z = e[m];
          m && t && (Z = ce(Z, et(t))), w = xe(Z.length, w), V[m] = !i && (t || c >= 120 && Z.length >= 120) ? new Tn(m && Z) : r;
        }
        Z = e[0];
        var J = -1, E = V[0];
        e:
          for (; ++J < c && M.length < w; ) {
            var x = Z[J], z = t ? t(x) : x;
            if (x = i || x !== 0 ? x : 0, !(E ? Mr(E, z) : o(M, z, i))) {
              for (m = d; --m; ) {
                var j = V[m];
                if (!(j ? Mr(j, z) : o(e[m], z, i)))
                  continue e;
              }
              E && E.push(z), M.push(x);
            }
          }
        return M;
      }
      function YV(e, t, i, o) {
        return Jt(e, function(c, d, m) {
          t(o, i(c), d, m);
        }), o;
      }
      function Wr(e, t, i) {
        t = en(t, e), e = wu(e, t);
        var o = e == null ? e : e[yt(ht(t))];
        return o == null ? r : $e(o, e, i);
      }
      function Sc(e) {
        return he(e) && ze(e) == xn;
      }
      function IV(e) {
        return he(e) && ze(e) == gr;
      }
      function HV(e) {
        return he(e) && ze(e) == vr;
      }
      function Sr(e, t, i, o, c) {
        return e === t ? !0 : e == null || t == null || !he(e) && !he(t) ? e !== e && t !== t : PV(e, t, i, o, Sr, c);
      }
      function PV(e, t, i, o, c, d) {
        var m = I(e), V = I(t), w = m ? ci : Be(e), M = V ? ci : Be(t);
        w = w == xn ? Ft : w, M = M == xn ? Ft : M;
        var Z = w == Ft, J = M == Ft, E = w == M;
        if (E && nn(e)) {
          if (!nn(t))
            return !1;
          m = !0, Z = !1;
        }
        if (E && !Z)
          return d || (d = new wt()), m || _n(e) ? fu(e, t, i, o, c, d) : w1(e, t, w, i, o, c, d);
        if (!(i & F)) {
          var x = Z && re.call(e, "__wrapped__"), z = J && re.call(t, "__wrapped__");
          if (x || z) {
            var j = x ? e.value() : e, G = z ? t.value() : t;
            return d || (d = new wt()), c(j, G, i, o, d);
          }
        }
        return E ? (d || (d = new wt()), U1(e, t, i, o, c, d)) : !1;
      }
      function jV(e) {
        return he(e) && Be(e) == Vt;
      }
      function No(e, t, i, o) {
        var c = i.length, d = c, m = !o;
        if (e == null)
          return !d;
        for (e = oe(e); c--; ) {
          var V = i[c];
          if (m && V[2] ? V[1] !== e[V[0]] : !(V[0] in e))
            return !1;
        }
        for (; ++c < d; ) {
          V = i[c];
          var w = V[0], M = e[w], Z = V[1];
          if (m && V[2]) {
            if (M === r && !(w in e))
              return !1;
          } else {
            var J = new wt();
            if (o)
              var E = o(M, Z, w, e, t, J);
            if (!(E === r ? Sr(Z, M, F | C, o, J) : E))
              return !1;
          }
        }
        return !0;
      }
      function Ac(e) {
        if (!ue(e) || b1(e))
          return !1;
        var t = Dt(e) ? jm : Cp;
        return t.test(Rn(e));
      }
      function XV(e) {
        return he(e) && ze(e) == Ur;
      }
      function _V(e) {
        return he(e) && Be(e) == Tt;
      }
      function LV(e) {
        return he(e) && Ki(e.length) && !!se[ze(e)];
      }
      function xc(e) {
        return typeof e == "function" ? e : e == null ? Le : typeof e == "object" ? I(e) ? Dc(e[0], e[1]) : Cc(e) : rd(e);
      }
      function go(e) {
        if (!Br(e))
          return $m(e);
        var t = [];
        for (var i in oe(e))
          re.call(e, i) && i != "constructor" && t.push(i);
        return t;
      }
      function qV(e) {
        if (!ue(e))
          return S1(e);
        var t = Br(e), i = [];
        for (var o in e)
          o == "constructor" && (t || !re.call(e, o)) || i.push(o);
        return i;
      }
      function Mo(e, t) {
        return e < t;
      }
      function Bc(e, t) {
        var i = -1, o = Xe(e) ? U(e.length) : [];
        return Kt(e, function(c, d, m) {
          o[++i] = t(c, d, m);
        }), o;
      }
      function Cc(e) {
        var t = Qo(e);
        return t.length == 1 && t[0][2] ? Tu(t[0][0], t[0][1]) : function(i) {
          return i === e || No(i, e, t);
        };
      }
      function Dc(e, t) {
        return zo(e) && Vu(t) ? Tu(yt(e), t) : function(i) {
          var o = qo(i, e);
          return o === r && o === t ? Ko(i, e) : Sr(t, o, F | C);
        };
      }
      function Bi(e, t, i, o, c) {
        e !== t && vo(t, function(d, m) {
          if (c || (c = new wt()), ue(d))
            KV(e, t, m, i, Bi, o, c);
          else {
            var V = o ? o(Yo(e, m), d, m + "", e, t, c) : r;
            V === r && (V = d), Vo(e, m, V);
          }
        }, _e);
      }
      function KV(e, t, i, o, c, d, m) {
        var V = Yo(e, i), w = Yo(t, i), M = m.get(w);
        if (M) {
          Vo(e, i, M);
          return;
        }
        var Z = d ? d(V, w, i + "", e, t, m) : r, J = Z === r;
        if (J) {
          var E = I(w), x = !E && nn(w), z = !E && !x && _n(w);
          Z = w, E || x || z ? I(V) ? Z = V : me(V) ? Z = je(V) : x ? (J = !1, Z = Lc(w, !0)) : z ? (J = !1, Z = qc(w, !0)) : Z = [] : Dr(w) || Nn(w) ? (Z = V, Nn(V) ? Z = ju(V) : (!ue(V) || Dt(V)) && (Z = mu(w))) : J = !1;
        }
        J && (m.set(w, Z), c(Z, w, o, d, m), m.delete(w)), Vo(e, i, Z);
      }
      function Qc(e, t) {
        var i = e.length;
        if (!!i)
          return t += t < 0 ? i : 0, Ct(t, i) ? e[t] : r;
      }
      function Oc(e, t, i) {
        t.length ? t = ce(t, function(d) {
          return I(d) ? function(m) {
            return wn(m, d.length === 1 ? d[0] : d);
          } : d;
        }) : t = [Le];
        var o = -1;
        t = ce(t, et(O()));
        var c = Bc(e, function(d, m, V) {
          var w = ce(t, function(M) {
            return M(d);
          });
          return { criteria: w, index: ++o, value: d };
        });
        return Zm(c, function(d, m) {
          return d1(d, m, i);
        });
      }
      function $V(e, t) {
        return zc(e, t, function(i, o) {
          return Ko(e, o);
        });
      }
      function zc(e, t, i) {
        for (var o = -1, c = t.length, d = {}; ++o < c; ) {
          var m = t[o], V = wn(e, m);
          i(V, m) && Ar(d, en(m, e), V);
        }
        return d;
      }
      function e1(e) {
        return function(t) {
          return wn(t, e);
        };
      }
      function Zo(e, t, i, o) {
        var c = o ? Mm : Dn, d = -1, m = t.length, V = e;
        for (e === t && (t = je(t)), i && (V = ce(e, et(i))); ++d < m; )
          for (var w = 0, M = t[d], Z = i ? i(M) : M; (w = c(V, Z, w, o)) > -1; )
            V !== e && Ji.call(V, w, 1), Ji.call(e, w, 1);
        return e;
      }
      function Gc(e, t) {
        for (var i = e ? t.length : 0, o = i - 1; i--; ) {
          var c = t[i];
          if (i == o || c !== d) {
            var d = c;
            Ct(c) ? Ji.call(e, c, 1) : bo(e, c);
          }
        }
        return e;
      }
      function Jo(e, t) {
        return e + bi(Nc() * (t - e + 1));
      }
      function t1(e, t, i, o) {
        for (var c = -1, d = Ne(yi((t - e) / (i || 1)), 0), m = U(d); d--; )
          m[o ? d : ++c] = e, e += i;
        return m;
      }
      function ko(e, t) {
        var i = "";
        if (!e || t < 1 || t > Oe)
          return i;
        do
          t % 2 && (i += e), t = bi(t / 2), t && (e += e);
        while (t);
        return i;
      }
      function X(e, t) {
        return Io(vu(e, t, Le), e + "");
      }
      function n1(e) {
        return Zc(Ln(e));
      }
      function r1(e, t) {
        var i = Ln(e);
        return Pi(i, vn(t, 0, i.length));
      }
      function Ar(e, t, i, o) {
        if (!ue(e))
          return e;
        t = en(t, e);
        for (var c = -1, d = t.length, m = d - 1, V = e; V != null && ++c < d; ) {
          var w = yt(t[c]), M = i;
          if (w === "__proto__" || w === "constructor" || w === "prototype")
            return e;
          if (c != m) {
            var Z = V[w];
            M = o ? o(Z, w, V) : r, M === r && (M = ue(Z) ? Z : Ct(t[c + 1]) ? [] : {});
          }
          Er(V, w, M), V = V[w];
        }
        return e;
      }
      var Yc = Ei ? function(e, t) {
        return Ei.set(e, t), e;
      } : Le, i1 = ki ? function(e, t) {
        return ki(e, "toString", {
          configurable: !0,
          enumerable: !1,
          value: el(t),
          writable: !0
        });
      } : Le;
      function a1(e) {
        return Pi(Ln(e));
      }
      function ft(e, t, i) {
        var o = -1, c = e.length;
        t < 0 && (t = -t > c ? 0 : c + t), i = i > c ? c : i, i < 0 && (i += c), c = t > i ? 0 : i - t >>> 0, t >>>= 0;
        for (var d = U(c); ++o < c; )
          d[o] = e[o + t];
        return d;
      }
      function o1(e, t) {
        var i;
        return Kt(e, function(o, c, d) {
          return i = t(o, c, d), !i;
        }), !!i;
      }
      function Ci(e, t, i) {
        var o = 0, c = e == null ? o : e.length;
        if (typeof t == "number" && t === t && c <= sp) {
          for (; o < c; ) {
            var d = o + c >>> 1, m = e[d];
            m !== null && !nt(m) && (i ? m <= t : m < t) ? o = d + 1 : c = d;
          }
          return c;
        }
        return yo(e, t, Le, i);
      }
      function yo(e, t, i, o) {
        var c = 0, d = e == null ? 0 : e.length;
        if (d === 0)
          return 0;
        t = i(t);
        for (var m = t !== t, V = t === null, w = nt(t), M = t === r; c < d; ) {
          var Z = bi((c + d) / 2), J = i(e[Z]), E = J !== r, x = J === null, z = J === J, j = nt(J);
          if (m)
            var G = o || z;
          else
            M ? G = z && (o || E) : V ? G = z && E && (o || !x) : w ? G = z && E && !x && (o || !j) : x || j ? G = !1 : G = o ? J <= t : J < t;
          G ? c = Z + 1 : d = Z;
        }
        return xe(d, lp);
      }
      function Ic(e, t) {
        for (var i = -1, o = e.length, c = 0, d = []; ++i < o; ) {
          var m = e[i], V = t ? t(m) : m;
          if (!i || !Ut(V, w)) {
            var w = V;
            d[c++] = m === 0 ? 0 : m;
          }
        }
        return d;
      }
      function Hc(e) {
        return typeof e == "number" ? e : nt(e) ? si : +e;
      }
      function tt(e) {
        if (typeof e == "string")
          return e;
        if (I(e))
          return ce(e, tt) + "";
        if (nt(e))
          return gc ? gc.call(e) : "";
        var t = e + "";
        return t == "0" && 1 / e == -ye ? "-0" : t;
      }
      function $t(e, t, i) {
        var o = -1, c = Vi, d = e.length, m = !0, V = [], w = V;
        if (i)
          m = !1, c = no;
        else if (d >= s) {
          var M = t ? null : T1(e);
          if (M)
            return vi(M);
          m = !1, c = Mr, w = new Tn();
        } else
          w = t ? [] : V;
        e:
          for (; ++o < d; ) {
            var Z = e[o], J = t ? t(Z) : Z;
            if (Z = i || Z !== 0 ? Z : 0, m && J === J) {
              for (var E = w.length; E--; )
                if (w[E] === J)
                  continue e;
              t && w.push(J), V.push(Z);
            } else
              c(w, J, i) || (w !== V && w.push(J), V.push(Z));
          }
        return V;
      }
      function bo(e, t) {
        return t = en(t, e), e = wu(e, t), e == null || delete e[yt(ht(t))];
      }
      function Pc(e, t, i, o) {
        return Ar(e, t, i(wn(e, t)), o);
      }
      function Di(e, t, i, o) {
        for (var c = e.length, d = o ? c : -1; (o ? d-- : ++d < c) && t(e[d], d, e); )
          ;
        return i ? ft(e, o ? 0 : d, o ? d + 1 : c) : ft(e, o ? d + 1 : 0, o ? c : d);
      }
      function jc(e, t) {
        var i = e;
        return i instanceof K && (i = i.value()), ro(t, function(o, c) {
          return c.func.apply(c.thisArg, _t([o], c.args));
        }, i);
      }
      function Eo(e, t, i) {
        var o = e.length;
        if (o < 2)
          return o ? $t(e[0]) : [];
        for (var c = -1, d = U(o); ++c < o; )
          for (var m = e[c], V = -1; ++V < o; )
            V != c && (d[c] = Fr(d[c] || m, e[V], t, i));
        return $t(Ee(d, 1), t, i);
      }
      function Xc(e, t, i) {
        for (var o = -1, c = e.length, d = t.length, m = {}; ++o < c; ) {
          var V = o < d ? t[o] : r;
          i(m, e[o], V);
        }
        return m;
      }
      function Fo(e) {
        return me(e) ? e : [];
      }
      function Wo(e) {
        return typeof e == "function" ? e : Le;
      }
      function en(e, t) {
        return I(e) ? e : zo(e, t) ? [e] : gu(ne(e));
      }
      var l1 = X;
      function tn(e, t, i) {
        var o = e.length;
        return i = i === r ? o : i, !t && i >= o ? e : ft(e, t, i);
      }
      var _c = Xm || function(e) {
        return be.clearTimeout(e);
      };
      function Lc(e, t) {
        if (t)
          return e.slice();
        var i = e.length, o = Tc ? Tc(i) : new e.constructor(i);
        return e.copy(o), o;
      }
      function So(e) {
        var t = new e.constructor(e.byteLength);
        return new Mi(t).set(new Mi(e)), t;
      }
      function s1(e, t) {
        var i = t ? So(e.buffer) : e.buffer;
        return new e.constructor(i, e.byteOffset, e.byteLength);
      }
      function c1(e) {
        var t = new e.constructor(e.source, Ss.exec(e));
        return t.lastIndex = e.lastIndex, t;
      }
      function u1(e) {
        return br ? oe(br.call(e)) : {};
      }
      function qc(e, t) {
        var i = t ? So(e.buffer) : e.buffer;
        return new e.constructor(i, e.byteOffset, e.length);
      }
      function Kc(e, t) {
        if (e !== t) {
          var i = e !== r, o = e === null, c = e === e, d = nt(e), m = t !== r, V = t === null, w = t === t, M = nt(t);
          if (!V && !M && !d && e > t || d && m && w && !V && !M || o && m && w || !i && w || !c)
            return 1;
          if (!o && !d && !M && e < t || M && i && c && !o && !d || V && i && c || !m && c || !w)
            return -1;
        }
        return 0;
      }
      function d1(e, t, i) {
        for (var o = -1, c = e.criteria, d = t.criteria, m = c.length, V = i.length; ++o < m; ) {
          var w = Kc(c[o], d[o]);
          if (w) {
            if (o >= V)
              return w;
            var M = i[o];
            return w * (M == "desc" ? -1 : 1);
          }
        }
        return e.index - t.index;
      }
      function $c(e, t, i, o) {
        for (var c = -1, d = e.length, m = i.length, V = -1, w = t.length, M = Ne(d - m, 0), Z = U(w + M), J = !o; ++V < w; )
          Z[V] = t[V];
        for (; ++c < m; )
          (J || c < d) && (Z[i[c]] = e[c]);
        for (; M--; )
          Z[V++] = e[c++];
        return Z;
      }
      function eu(e, t, i, o) {
        for (var c = -1, d = e.length, m = -1, V = i.length, w = -1, M = t.length, Z = Ne(d - V, 0), J = U(Z + M), E = !o; ++c < Z; )
          J[c] = e[c];
        for (var x = c; ++w < M; )
          J[x + w] = t[w];
        for (; ++m < V; )
          (E || c < d) && (J[x + i[m]] = e[c++]);
        return J;
      }
      function je(e, t) {
        var i = -1, o = e.length;
        for (t || (t = U(o)); ++i < o; )
          t[i] = e[i];
        return t;
      }
      function kt(e, t, i, o) {
        var c = !i;
        i || (i = {});
        for (var d = -1, m = t.length; ++d < m; ) {
          var V = t[d], w = o ? o(i[V], e[V], V, i, e) : r;
          w === r && (w = e[V]), c ? At(i, V, w) : Er(i, V, w);
        }
        return i;
      }
      function f1(e, t) {
        return kt(e, Oo(e), t);
      }
      function h1(e, t) {
        return kt(e, hu(e), t);
      }
      function Qi(e, t) {
        return function(i, o) {
          var c = I(i) ? vm : xV, d = t ? t() : {};
          return c(i, e, O(o, 2), d);
        };
      }
      function Pn(e) {
        return X(function(t, i) {
          var o = -1, c = i.length, d = c > 1 ? i[c - 1] : r, m = c > 2 ? i[2] : r;
          for (d = e.length > 3 && typeof d == "function" ? (c--, d) : r, m && Ge(i[0], i[1], m) && (d = c < 3 ? r : d, c = 1), t = oe(t); ++o < c; ) {
            var V = i[o];
            V && e(t, V, o, d);
          }
          return t;
        });
      }
      function tu(e, t) {
        return function(i, o) {
          if (i == null)
            return i;
          if (!Xe(i))
            return e(i, o);
          for (var c = i.length, d = t ? c : -1, m = oe(i); (t ? d-- : ++d < c) && o(m[d], d, m) !== !1; )
            ;
          return i;
        };
      }
      function nu(e) {
        return function(t, i, o) {
          for (var c = -1, d = oe(t), m = o(t), V = m.length; V--; ) {
            var w = m[e ? V : ++c];
            if (i(d[w], w, d) === !1)
              break;
          }
          return t;
        };
      }
      function p1(e, t, i) {
        var o = t & D, c = xr(e);
        function d() {
          var m = this && this !== be && this instanceof d ? c : e;
          return m.apply(o ? i : this, arguments);
        }
        return d;
      }
      function ru(e) {
        return function(t) {
          t = ne(t);
          var i = Qn(t) ? vt(t) : r, o = i ? i[0] : t.charAt(0), c = i ? tn(i, 1).join("") : t.slice(1);
          return o[e]() + c;
        };
      }
      function jn(e) {
        return function(t) {
          return ro(td(ed(t).replace(im, "")), e, "");
        };
      }
      function xr(e) {
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return new e();
            case 1:
              return new e(t[0]);
            case 2:
              return new e(t[0], t[1]);
            case 3:
              return new e(t[0], t[1], t[2]);
            case 4:
              return new e(t[0], t[1], t[2], t[3]);
            case 5:
              return new e(t[0], t[1], t[2], t[3], t[4]);
            case 6:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5]);
            case 7:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5], t[6]);
          }
          var i = Hn(e.prototype), o = e.apply(i, t);
          return ue(o) ? o : i;
        };
      }
      function m1(e, t, i) {
        var o = xr(e);
        function c() {
          for (var d = arguments.length, m = U(d), V = d, w = Xn(c); V--; )
            m[V] = arguments[V];
          var M = d < 3 && m[0] !== w && m[d - 1] !== w ? [] : Lt(m, w);
          if (d -= M.length, d < i)
            return su(
              e,
              t,
              Oi,
              c.placeholder,
              r,
              m,
              M,
              r,
              r,
              i - d
            );
          var Z = this && this !== be && this instanceof c ? o : e;
          return $e(Z, this, m);
        }
        return c;
      }
      function iu(e) {
        return function(t, i, o) {
          var c = oe(t);
          if (!Xe(t)) {
            var d = O(i, 3);
            t = Me(t), i = function(V) {
              return d(c[V], V, c);
            };
          }
          var m = e(t, i, o);
          return m > -1 ? c[d ? t[m] : m] : r;
        };
      }
      function au(e) {
        return Bt(function(t) {
          var i = t.length, o = i, c = ut.prototype.thru;
          for (e && t.reverse(); o--; ) {
            var d = t[o];
            if (typeof d != "function")
              throw new ct(h);
            if (c && !m && Ii(d) == "wrapper")
              var m = new ut([], !0);
          }
          for (o = m ? o : i; ++o < i; ) {
            d = t[o];
            var V = Ii(d), w = V == "wrapper" ? Do(d) : r;
            w && Go(w[0]) && w[1] == ($ | S | y | L) && !w[4].length && w[9] == 1 ? m = m[Ii(w[0])].apply(m, w[3]) : m = d.length == 1 && Go(d) ? m[V]() : m.thru(d);
          }
          return function() {
            var M = arguments, Z = M[0];
            if (m && M.length == 1 && I(Z))
              return m.plant(Z).value();
            for (var J = 0, E = i ? t[J].apply(this, M) : Z; ++J < i; )
              E = t[J].call(this, E);
            return E;
          };
        });
      }
      function Oi(e, t, i, o, c, d, m, V, w, M) {
        var Z = t & $, J = t & D, E = t & A, x = t & (S | H), z = t & Ae, j = E ? r : xr(e);
        function G() {
          for (var q = arguments.length, ee = U(q), rt = q; rt--; )
            ee[rt] = arguments[rt];
          if (x)
            var Ye = Xn(G), it = km(ee, Ye);
          if (o && (ee = $c(ee, o, c, x)), d && (ee = eu(ee, d, m, x)), q -= it, x && q < M) {
            var Ve = Lt(ee, Ye);
            return su(
              e,
              t,
              Oi,
              G.placeholder,
              i,
              ee,
              Ve,
              V,
              w,
              M - q
            );
          }
          var Rt = J ? i : this, Ot = E ? Rt[e] : e;
          return q = ee.length, V ? ee = x1(ee, V) : z && q > 1 && ee.reverse(), Z && w < q && (ee.length = w), this && this !== be && this instanceof G && (Ot = j || xr(Ot)), Ot.apply(Rt, ee);
        }
        return G;
      }
      function ou(e, t) {
        return function(i, o) {
          return YV(i, e, t(o), {});
        };
      }
      function zi(e, t) {
        return function(i, o) {
          var c;
          if (i === r && o === r)
            return t;
          if (i !== r && (c = i), o !== r) {
            if (c === r)
              return o;
            typeof i == "string" || typeof o == "string" ? (i = tt(i), o = tt(o)) : (i = Hc(i), o = Hc(o)), c = e(i, o);
          }
          return c;
        };
      }
      function Ao(e) {
        return Bt(function(t) {
          return t = ce(t, et(O())), X(function(i) {
            var o = this;
            return e(t, function(c) {
              return $e(c, o, i);
            });
          });
        });
      }
      function Gi(e, t) {
        t = t === r ? " " : tt(t);
        var i = t.length;
        if (i < 2)
          return i ? ko(t, e) : t;
        var o = ko(t, yi(e / On(t)));
        return Qn(t) ? tn(vt(o), 0, e).join("") : o.slice(0, e);
      }
      function V1(e, t, i, o) {
        var c = t & D, d = xr(e);
        function m() {
          for (var V = -1, w = arguments.length, M = -1, Z = o.length, J = U(Z + w), E = this && this !== be && this instanceof m ? d : e; ++M < Z; )
            J[M] = o[M];
          for (; w--; )
            J[M++] = arguments[++V];
          return $e(E, c ? i : this, J);
        }
        return m;
      }
      function lu(e) {
        return function(t, i, o) {
          return o && typeof o != "number" && Ge(t, i, o) && (i = o = r), t = Qt(t), i === r ? (i = t, t = 0) : i = Qt(i), o = o === r ? t < i ? 1 : -1 : Qt(o), t1(t, i, o, e);
        };
      }
      function Yi(e) {
        return function(t, i) {
          return typeof t == "string" && typeof i == "string" || (t = pt(t), i = pt(i)), e(t, i);
        };
      }
      function su(e, t, i, o, c, d, m, V, w, M) {
        var Z = t & S, J = Z ? m : r, E = Z ? r : m, x = Z ? d : r, z = Z ? r : d;
        t |= Z ? y : _, t &= ~(Z ? _ : y), t & Q || (t &= ~(D | A));
        var j = [
          e,
          t,
          c,
          x,
          J,
          z,
          E,
          V,
          w,
          M
        ], G = i.apply(r, j);
        return Go(e) && Uu(G, j), G.placeholder = o, Ru(G, e, t);
      }
      function xo(e) {
        var t = Re[e];
        return function(i, o) {
          if (i = pt(i), o = o == null ? 0 : xe(P(o), 292), o && Rc(i)) {
            var c = (ne(i) + "e").split("e"), d = t(c[0] + "e" + (+c[1] + o));
            return c = (ne(d) + "e").split("e"), +(c[0] + "e" + (+c[1] - o));
          }
          return t(i);
        };
      }
      var T1 = Yn && 1 / vi(new Yn([, -0]))[1] == ye ? function(e) {
        return new Yn(e);
      } : rl;
      function cu(e) {
        return function(t) {
          var i = Be(t);
          return i == Vt ? uo(t) : i == Tt ? Am(t) : Jm(t, e(t));
        };
      }
      function xt(e, t, i, o, c, d, m, V) {
        var w = t & A;
        if (!w && typeof e != "function")
          throw new ct(h);
        var M = o ? o.length : 0;
        if (M || (t &= ~(y | _), o = c = r), m = m === r ? m : Ne(P(m), 0), V = V === r ? V : P(V), M -= c ? c.length : 0, t & _) {
          var Z = o, J = c;
          o = c = r;
        }
        var E = w ? r : Do(e), x = [
          e,
          t,
          i,
          o,
          c,
          Z,
          J,
          d,
          m,
          V
        ];
        if (E && W1(x, E), e = x[0], t = x[1], i = x[2], o = x[3], c = x[4], V = x[9] = x[9] === r ? w ? 0 : e.length : Ne(x[9] - M, 0), !V && t & (S | H) && (t &= ~(S | H)), !t || t == D)
          var z = p1(e, t, i);
        else
          t == S || t == H ? z = m1(e, t, V) : (t == y || t == (D | y)) && !c.length ? z = V1(e, t, i, o) : z = Oi.apply(r, x);
        var j = E ? Yc : Uu;
        return Ru(j(z, x), e, t);
      }
      function uu(e, t, i, o) {
        return e === r || Ut(e, Gn[i]) && !re.call(o, i) ? t : e;
      }
      function du(e, t, i, o, c, d) {
        return ue(e) && ue(t) && (d.set(t, e), Bi(e, t, r, du, d), d.delete(t)), e;
      }
      function v1(e) {
        return Dr(e) ? r : e;
      }
      function fu(e, t, i, o, c, d) {
        var m = i & F, V = e.length, w = t.length;
        if (V != w && !(m && w > V))
          return !1;
        var M = d.get(e), Z = d.get(t);
        if (M && Z)
          return M == t && Z == e;
        var J = -1, E = !0, x = i & C ? new Tn() : r;
        for (d.set(e, t), d.set(t, e); ++J < V; ) {
          var z = e[J], j = t[J];
          if (o)
            var G = m ? o(j, z, J, t, e, d) : o(z, j, J, e, t, d);
          if (G !== r) {
            if (G)
              continue;
            E = !1;
            break;
          }
          if (x) {
            if (!io(t, function(q, ee) {
              if (!Mr(x, ee) && (z === q || c(z, q, i, o, d)))
                return x.push(ee);
            })) {
              E = !1;
              break;
            }
          } else if (!(z === j || c(z, j, i, o, d))) {
            E = !1;
            break;
          }
        }
        return d.delete(e), d.delete(t), E;
      }
      function w1(e, t, i, o, c, d, m) {
        switch (i) {
          case Bn:
            if (e.byteLength != t.byteLength || e.byteOffset != t.byteOffset)
              return !1;
            e = e.buffer, t = t.buffer;
          case gr:
            return !(e.byteLength != t.byteLength || !d(new Mi(e), new Mi(t)));
          case Tr:
          case vr:
          case wr:
            return Ut(+e, +t);
          case ui:
            return e.name == t.name && e.message == t.message;
          case Ur:
          case Rr:
            return e == t + "";
          case Vt:
            var V = uo;
          case Tt:
            var w = o & F;
            if (V || (V = vi), e.size != t.size && !w)
              return !1;
            var M = m.get(e);
            if (M)
              return M == t;
            o |= C, m.set(e, t);
            var Z = fu(V(e), V(t), o, c, d, m);
            return m.delete(e), Z;
          case fi:
            if (br)
              return br.call(e) == br.call(t);
        }
        return !1;
      }
      function U1(e, t, i, o, c, d) {
        var m = i & F, V = Bo(e), w = V.length, M = Bo(t), Z = M.length;
        if (w != Z && !m)
          return !1;
        for (var J = w; J--; ) {
          var E = V[J];
          if (!(m ? E in t : re.call(t, E)))
            return !1;
        }
        var x = d.get(e), z = d.get(t);
        if (x && z)
          return x == t && z == e;
        var j = !0;
        d.set(e, t), d.set(t, e);
        for (var G = m; ++J < w; ) {
          E = V[J];
          var q = e[E], ee = t[E];
          if (o)
            var rt = m ? o(ee, q, E, t, e, d) : o(q, ee, E, e, t, d);
          if (!(rt === r ? q === ee || c(q, ee, i, o, d) : rt)) {
            j = !1;
            break;
          }
          G || (G = E == "constructor");
        }
        if (j && !G) {
          var Ye = e.constructor, it = t.constructor;
          Ye != it && "constructor" in e && "constructor" in t && !(typeof Ye == "function" && Ye instanceof Ye && typeof it == "function" && it instanceof it) && (j = !1);
        }
        return d.delete(e), d.delete(t), j;
      }
      function Bt(e) {
        return Io(vu(e, r, ku), e + "");
      }
      function Bo(e) {
        return Wc(e, Me, Oo);
      }
      function Co(e) {
        return Wc(e, _e, hu);
      }
      var Do = Ei ? function(e) {
        return Ei.get(e);
      } : rl;
      function Ii(e) {
        for (var t = e.name + "", i = In[t], o = re.call(In, t) ? i.length : 0; o--; ) {
          var c = i[o], d = c.func;
          if (d == null || d == e)
            return c.name;
        }
        return t;
      }
      function Xn(e) {
        var t = re.call(u, "placeholder") ? u : e;
        return t.placeholder;
      }
      function O() {
        var e = u.iteratee || tl;
        return e = e === tl ? xc : e, arguments.length ? e(arguments[0], arguments[1]) : e;
      }
      function Hi(e, t) {
        var i = e.__data__;
        return y1(t) ? i[typeof t == "string" ? "string" : "hash"] : i.map;
      }
      function Qo(e) {
        for (var t = Me(e), i = t.length; i--; ) {
          var o = t[i], c = e[o];
          t[i] = [o, c, Vu(c)];
        }
        return t;
      }
      function Un(e, t) {
        var i = Fm(e, t);
        return Ac(i) ? i : r;
      }
      function R1(e) {
        var t = re.call(e, mn), i = e[mn];
        try {
          e[mn] = r;
          var o = !0;
        } catch {
        }
        var c = Ni.call(e);
        return o && (t ? e[mn] = i : delete e[mn]), c;
      }
      var Oo = ho ? function(e) {
        return e == null ? [] : (e = oe(e), Xt(ho(e), function(t) {
          return wc.call(e, t);
        }));
      } : il, hu = ho ? function(e) {
        for (var t = []; e; )
          _t(t, Oo(e)), e = Zi(e);
        return t;
      } : il, Be = ze;
      (po && Be(new po(new ArrayBuffer(1))) != Bn || Jr && Be(new Jr()) != Vt || mo && Be(mo.resolve()) != bs || Yn && Be(new Yn()) != Tt || kr && Be(new kr()) != Nr) && (Be = function(e) {
        var t = ze(e), i = t == Ft ? e.constructor : r, o = i ? Rn(i) : "";
        if (o)
          switch (o) {
            case rV:
              return Bn;
            case iV:
              return Vt;
            case aV:
              return bs;
            case oV:
              return Tt;
            case lV:
              return Nr;
          }
        return t;
      });
      function N1(e, t, i) {
        for (var o = -1, c = i.length; ++o < c; ) {
          var d = i[o], m = d.size;
          switch (d.type) {
            case "drop":
              e += m;
              break;
            case "dropRight":
              t -= m;
              break;
            case "take":
              t = xe(t, e + m);
              break;
            case "takeRight":
              e = Ne(e, t - m);
              break;
          }
        }
        return { start: e, end: t };
      }
      function g1(e) {
        var t = e.match(bp);
        return t ? t[1].split(Ep) : [];
      }
      function pu(e, t, i) {
        t = en(t, e);
        for (var o = -1, c = t.length, d = !1; ++o < c; ) {
          var m = yt(t[o]);
          if (!(d = e != null && i(e, m)))
            break;
          e = e[m];
        }
        return d || ++o != c ? d : (c = e == null ? 0 : e.length, !!c && Ki(c) && Ct(m, c) && (I(e) || Nn(e)));
      }
      function M1(e) {
        var t = e.length, i = new e.constructor(t);
        return t && typeof e[0] == "string" && re.call(e, "index") && (i.index = e.index, i.input = e.input), i;
      }
      function mu(e) {
        return typeof e.constructor == "function" && !Br(e) ? Hn(Zi(e)) : {};
      }
      function Z1(e, t, i) {
        var o = e.constructor;
        switch (t) {
          case gr:
            return So(e);
          case Tr:
          case vr:
            return new o(+e);
          case Bn:
            return s1(e, i);
          case Da:
          case Qa:
          case Oa:
          case za:
          case Ga:
          case Ya:
          case Ia:
          case Ha:
          case Pa:
            return qc(e, i);
          case Vt:
            return new o();
          case wr:
          case Rr:
            return new o(e);
          case Ur:
            return c1(e);
          case Tt:
            return new o();
          case fi:
            return u1(e);
        }
      }
      function J1(e, t) {
        var i = t.length;
        if (!i)
          return e;
        var o = i - 1;
        return t[o] = (i > 1 ? "& " : "") + t[o], t = t.join(i > 2 ? ", " : " "), e.replace(yp, `{
/* [wrapped with ` + t + `] */
`);
      }
      function k1(e) {
        return I(e) || Nn(e) || !!(Uc && e && e[Uc]);
      }
      function Ct(e, t) {
        var i = typeof e;
        return t = t == null ? Oe : t, !!t && (i == "number" || i != "symbol" && Qp.test(e)) && e > -1 && e % 1 == 0 && e < t;
      }
      function Ge(e, t, i) {
        if (!ue(i))
          return !1;
        var o = typeof t;
        return (o == "number" ? Xe(i) && Ct(t, i.length) : o == "string" && t in i) ? Ut(i[t], e) : !1;
      }
      function zo(e, t) {
        if (I(e))
          return !1;
        var i = typeof e;
        return i == "number" || i == "symbol" || i == "boolean" || e == null || nt(e) ? !0 : Mp.test(e) || !gp.test(e) || t != null && e in oe(t);
      }
      function y1(e) {
        var t = typeof e;
        return t == "string" || t == "number" || t == "symbol" || t == "boolean" ? e !== "__proto__" : e === null;
      }
      function Go(e) {
        var t = Ii(e), i = u[t];
        if (typeof i != "function" || !(t in K.prototype))
          return !1;
        if (e === i)
          return !0;
        var o = Do(i);
        return !!o && e === o[0];
      }
      function b1(e) {
        return !!Vc && Vc in e;
      }
      var E1 = Ui ? Dt : al;
      function Br(e) {
        var t = e && e.constructor, i = typeof t == "function" && t.prototype || Gn;
        return e === i;
      }
      function Vu(e) {
        return e === e && !ue(e);
      }
      function Tu(e, t) {
        return function(i) {
          return i == null ? !1 : i[e] === t && (t !== r || e in oe(i));
        };
      }
      function F1(e) {
        var t = Li(e, function(o) {
          return i.size === N && i.clear(), o;
        }), i = t.cache;
        return t;
      }
      function W1(e, t) {
        var i = e[1], o = t[1], c = i | o, d = c < (D | A | $), m = o == $ && i == S || o == $ && i == L && e[7].length <= t[8] || o == ($ | L) && t[7].length <= t[8] && i == S;
        if (!(d || m))
          return e;
        o & D && (e[2] = t[2], c |= i & D ? 0 : Q);
        var V = t[3];
        if (V) {
          var w = e[3];
          e[3] = w ? $c(w, V, t[4]) : V, e[4] = w ? Lt(e[3], g) : t[4];
        }
        return V = t[5], V && (w = e[5], e[5] = w ? eu(w, V, t[6]) : V, e[6] = w ? Lt(e[5], g) : t[6]), V = t[7], V && (e[7] = V), o & $ && (e[8] = e[8] == null ? t[8] : xe(e[8], t[8])), e[9] == null && (e[9] = t[9]), e[0] = t[0], e[1] = c, e;
      }
      function S1(e) {
        var t = [];
        if (e != null)
          for (var i in oe(e))
            t.push(i);
        return t;
      }
      function A1(e) {
        return Ni.call(e);
      }
      function vu(e, t, i) {
        return t = Ne(t === r ? e.length - 1 : t, 0), function() {
          for (var o = arguments, c = -1, d = Ne(o.length - t, 0), m = U(d); ++c < d; )
            m[c] = o[t + c];
          c = -1;
          for (var V = U(t + 1); ++c < t; )
            V[c] = o[c];
          return V[t] = i(m), $e(e, this, V);
        };
      }
      function wu(e, t) {
        return t.length < 2 ? e : wn(e, ft(t, 0, -1));
      }
      function x1(e, t) {
        for (var i = e.length, o = xe(t.length, i), c = je(e); o--; ) {
          var d = t[o];
          e[o] = Ct(d, i) ? c[d] : r;
        }
        return e;
      }
      function Yo(e, t) {
        if (!(t === "constructor" && typeof e[t] == "function") && t != "__proto__")
          return e[t];
      }
      var Uu = Nu(Yc), Cr = Lm || function(e, t) {
        return be.setTimeout(e, t);
      }, Io = Nu(i1);
      function Ru(e, t, i) {
        var o = t + "";
        return Io(e, J1(o, B1(g1(o), i)));
      }
      function Nu(e) {
        var t = 0, i = 0;
        return function() {
          var o = eV(), c = pe - (o - i);
          if (i = o, c > 0) {
            if (++t >= fe)
              return arguments[0];
          } else
            t = 0;
          return e.apply(r, arguments);
        };
      }
      function Pi(e, t) {
        var i = -1, o = e.length, c = o - 1;
        for (t = t === r ? o : t; ++i < t; ) {
          var d = Jo(i, c), m = e[d];
          e[d] = e[i], e[i] = m;
        }
        return e.length = t, e;
      }
      var gu = F1(function(e) {
        var t = [];
        return e.charCodeAt(0) === 46 && t.push(""), e.replace(Zp, function(i, o, c, d) {
          t.push(c ? d.replace(Sp, "$1") : o || i);
        }), t;
      });
      function yt(e) {
        if (typeof e == "string" || nt(e))
          return e;
        var t = e + "";
        return t == "0" && 1 / e == -ye ? "-0" : t;
      }
      function Rn(e) {
        if (e != null) {
          try {
            return Ri.call(e);
          } catch {
          }
          try {
            return e + "";
          } catch {
          }
        }
        return "";
      }
      function B1(e, t) {
        return st(cp, function(i) {
          var o = "_." + i[0];
          t & i[1] && !Vi(e, o) && e.push(o);
        }), e.sort();
      }
      function Mu(e) {
        if (e instanceof K)
          return e.clone();
        var t = new ut(e.__wrapped__, e.__chain__);
        return t.__actions__ = je(e.__actions__), t.__index__ = e.__index__, t.__values__ = e.__values__, t;
      }
      function C1(e, t, i) {
        (i ? Ge(e, t, i) : t === r) ? t = 1 : t = Ne(P(t), 0);
        var o = e == null ? 0 : e.length;
        if (!o || t < 1)
          return [];
        for (var c = 0, d = 0, m = U(yi(o / t)); c < o; )
          m[d++] = ft(e, c, c += t);
        return m;
      }
      function D1(e) {
        for (var t = -1, i = e == null ? 0 : e.length, o = 0, c = []; ++t < i; ) {
          var d = e[t];
          d && (c[o++] = d);
        }
        return c;
      }
      function Q1() {
        var e = arguments.length;
        if (!e)
          return [];
        for (var t = U(e - 1), i = arguments[0], o = e; o--; )
          t[o - 1] = arguments[o];
        return _t(I(i) ? je(i) : [i], Ee(t, 1));
      }
      var O1 = X(function(e, t) {
        return me(e) ? Fr(e, Ee(t, 1, me, !0)) : [];
      }), z1 = X(function(e, t) {
        var i = ht(t);
        return me(i) && (i = r), me(e) ? Fr(e, Ee(t, 1, me, !0), O(i, 2)) : [];
      }), G1 = X(function(e, t) {
        var i = ht(t);
        return me(i) && (i = r), me(e) ? Fr(e, Ee(t, 1, me, !0), r, i) : [];
      });
      function Y1(e, t, i) {
        var o = e == null ? 0 : e.length;
        return o ? (t = i || t === r ? 1 : P(t), ft(e, t < 0 ? 0 : t, o)) : [];
      }
      function I1(e, t, i) {
        var o = e == null ? 0 : e.length;
        return o ? (t = i || t === r ? 1 : P(t), t = o - t, ft(e, 0, t < 0 ? 0 : t)) : [];
      }
      function H1(e, t) {
        return e && e.length ? Di(e, O(t, 3), !0, !0) : [];
      }
      function P1(e, t) {
        return e && e.length ? Di(e, O(t, 3), !0) : [];
      }
      function j1(e, t, i, o) {
        var c = e == null ? 0 : e.length;
        return c ? (i && typeof i != "number" && Ge(e, t, i) && (i = 0, o = c), QV(e, t, i, o)) : [];
      }
      function Zu(e, t, i) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var c = i == null ? 0 : P(i);
        return c < 0 && (c = Ne(o + c, 0)), Ti(e, O(t, 3), c);
      }
      function Ju(e, t, i) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var c = o - 1;
        return i !== r && (c = P(i), c = i < 0 ? Ne(o + c, 0) : xe(c, o - 1)), Ti(e, O(t, 3), c, !0);
      }
      function ku(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ee(e, 1) : [];
      }
      function X1(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ee(e, ye) : [];
      }
      function _1(e, t) {
        var i = e == null ? 0 : e.length;
        return i ? (t = t === r ? 1 : P(t), Ee(e, t)) : [];
      }
      function L1(e) {
        for (var t = -1, i = e == null ? 0 : e.length, o = {}; ++t < i; ) {
          var c = e[t];
          o[c[0]] = c[1];
        }
        return o;
      }
      function yu(e) {
        return e && e.length ? e[0] : r;
      }
      function q1(e, t, i) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var c = i == null ? 0 : P(i);
        return c < 0 && (c = Ne(o + c, 0)), Dn(e, t, c);
      }
      function K1(e) {
        var t = e == null ? 0 : e.length;
        return t ? ft(e, 0, -1) : [];
      }
      var $1 = X(function(e) {
        var t = ce(e, Fo);
        return t.length && t[0] === e[0] ? Ro(t) : [];
      }), e0 = X(function(e) {
        var t = ht(e), i = ce(e, Fo);
        return t === ht(i) ? t = r : i.pop(), i.length && i[0] === e[0] ? Ro(i, O(t, 2)) : [];
      }), t0 = X(function(e) {
        var t = ht(e), i = ce(e, Fo);
        return t = typeof t == "function" ? t : r, t && i.pop(), i.length && i[0] === e[0] ? Ro(i, r, t) : [];
      });
      function n0(e, t) {
        return e == null ? "" : Km.call(e, t);
      }
      function ht(e) {
        var t = e == null ? 0 : e.length;
        return t ? e[t - 1] : r;
      }
      function r0(e, t, i) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var c = o;
        return i !== r && (c = P(i), c = c < 0 ? Ne(o + c, 0) : xe(c, o - 1)), t === t ? Bm(e, t, c) : Ti(e, sc, c, !0);
      }
      function i0(e, t) {
        return e && e.length ? Qc(e, P(t)) : r;
      }
      var a0 = X(bu);
      function bu(e, t) {
        return e && e.length && t && t.length ? Zo(e, t) : e;
      }
      function o0(e, t, i) {
        return e && e.length && t && t.length ? Zo(e, t, O(i, 2)) : e;
      }
      function l0(e, t, i) {
        return e && e.length && t && t.length ? Zo(e, t, r, i) : e;
      }
      var s0 = Bt(function(e, t) {
        var i = e == null ? 0 : e.length, o = To(e, t);
        return Gc(e, ce(t, function(c) {
          return Ct(c, i) ? +c : c;
        }).sort(Kc)), o;
      });
      function c0(e, t) {
        var i = [];
        if (!(e && e.length))
          return i;
        var o = -1, c = [], d = e.length;
        for (t = O(t, 3); ++o < d; ) {
          var m = e[o];
          t(m, o, e) && (i.push(m), c.push(o));
        }
        return Gc(e, c), i;
      }
      function Ho(e) {
        return e == null ? e : nV.call(e);
      }
      function u0(e, t, i) {
        var o = e == null ? 0 : e.length;
        return o ? (i && typeof i != "number" && Ge(e, t, i) ? (t = 0, i = o) : (t = t == null ? 0 : P(t), i = i === r ? o : P(i)), ft(e, t, i)) : [];
      }
      function d0(e, t) {
        return Ci(e, t);
      }
      function f0(e, t, i) {
        return yo(e, t, O(i, 2));
      }
      function h0(e, t) {
        var i = e == null ? 0 : e.length;
        if (i) {
          var o = Ci(e, t);
          if (o < i && Ut(e[o], t))
            return o;
        }
        return -1;
      }
      function p0(e, t) {
        return Ci(e, t, !0);
      }
      function m0(e, t, i) {
        return yo(e, t, O(i, 2), !0);
      }
      function V0(e, t) {
        var i = e == null ? 0 : e.length;
        if (i) {
          var o = Ci(e, t, !0) - 1;
          if (Ut(e[o], t))
            return o;
        }
        return -1;
      }
      function T0(e) {
        return e && e.length ? Ic(e) : [];
      }
      function v0(e, t) {
        return e && e.length ? Ic(e, O(t, 2)) : [];
      }
      function w0(e) {
        var t = e == null ? 0 : e.length;
        return t ? ft(e, 1, t) : [];
      }
      function U0(e, t, i) {
        return e && e.length ? (t = i || t === r ? 1 : P(t), ft(e, 0, t < 0 ? 0 : t)) : [];
      }
      function R0(e, t, i) {
        var o = e == null ? 0 : e.length;
        return o ? (t = i || t === r ? 1 : P(t), t = o - t, ft(e, t < 0 ? 0 : t, o)) : [];
      }
      function N0(e, t) {
        return e && e.length ? Di(e, O(t, 3), !1, !0) : [];
      }
      function g0(e, t) {
        return e && e.length ? Di(e, O(t, 3)) : [];
      }
      var M0 = X(function(e) {
        return $t(Ee(e, 1, me, !0));
      }), Z0 = X(function(e) {
        var t = ht(e);
        return me(t) && (t = r), $t(Ee(e, 1, me, !0), O(t, 2));
      }), J0 = X(function(e) {
        var t = ht(e);
        return t = typeof t == "function" ? t : r, $t(Ee(e, 1, me, !0), r, t);
      });
      function k0(e) {
        return e && e.length ? $t(e) : [];
      }
      function y0(e, t) {
        return e && e.length ? $t(e, O(t, 2)) : [];
      }
      function b0(e, t) {
        return t = typeof t == "function" ? t : r, e && e.length ? $t(e, r, t) : [];
      }
      function Po(e) {
        if (!(e && e.length))
          return [];
        var t = 0;
        return e = Xt(e, function(i) {
          if (me(i))
            return t = Ne(i.length, t), !0;
        }), so(t, function(i) {
          return ce(e, ao(i));
        });
      }
      function Eu(e, t) {
        if (!(e && e.length))
          return [];
        var i = Po(e);
        return t == null ? i : ce(i, function(o) {
          return $e(t, r, o);
        });
      }
      var E0 = X(function(e, t) {
        return me(e) ? Fr(e, t) : [];
      }), F0 = X(function(e) {
        return Eo(Xt(e, me));
      }), W0 = X(function(e) {
        var t = ht(e);
        return me(t) && (t = r), Eo(Xt(e, me), O(t, 2));
      }), S0 = X(function(e) {
        var t = ht(e);
        return t = typeof t == "function" ? t : r, Eo(Xt(e, me), r, t);
      }), A0 = X(Po);
      function x0(e, t) {
        return Xc(e || [], t || [], Er);
      }
      function B0(e, t) {
        return Xc(e || [], t || [], Ar);
      }
      var C0 = X(function(e) {
        var t = e.length, i = t > 1 ? e[t - 1] : r;
        return i = typeof i == "function" ? (e.pop(), i) : r, Eu(e, i);
      });
      function Fu(e) {
        var t = u(e);
        return t.__chain__ = !0, t;
      }
      function D0(e, t) {
        return t(e), e;
      }
      function ji(e, t) {
        return t(e);
      }
      var Q0 = Bt(function(e) {
        var t = e.length, i = t ? e[0] : 0, o = this.__wrapped__, c = function(d) {
          return To(d, e);
        };
        return t > 1 || this.__actions__.length || !(o instanceof K) || !Ct(i) ? this.thru(c) : (o = o.slice(i, +i + (t ? 1 : 0)), o.__actions__.push({
          func: ji,
          args: [c],
          thisArg: r
        }), new ut(o, this.__chain__).thru(function(d) {
          return t && !d.length && d.push(r), d;
        }));
      });
      function O0() {
        return Fu(this);
      }
      function z0() {
        return new ut(this.value(), this.__chain__);
      }
      function G0() {
        this.__values__ === r && (this.__values__ = Hu(this.value()));
        var e = this.__index__ >= this.__values__.length, t = e ? r : this.__values__[this.__index__++];
        return { done: e, value: t };
      }
      function Y0() {
        return this;
      }
      function I0(e) {
        for (var t, i = this; i instanceof Wi; ) {
          var o = Mu(i);
          o.__index__ = 0, o.__values__ = r, t ? c.__wrapped__ = o : t = o;
          var c = o;
          i = i.__wrapped__;
        }
        return c.__wrapped__ = e, t;
      }
      function H0() {
        var e = this.__wrapped__;
        if (e instanceof K) {
          var t = e;
          return this.__actions__.length && (t = new K(this)), t = t.reverse(), t.__actions__.push({
            func: ji,
            args: [Ho],
            thisArg: r
          }), new ut(t, this.__chain__);
        }
        return this.thru(Ho);
      }
      function P0() {
        return jc(this.__wrapped__, this.__actions__);
      }
      var j0 = Qi(function(e, t, i) {
        re.call(e, i) ? ++e[i] : At(e, i, 1);
      });
      function X0(e, t, i) {
        var o = I(e) ? oc : DV;
        return i && Ge(e, t, i) && (t = r), o(e, O(t, 3));
      }
      function _0(e, t) {
        var i = I(e) ? Xt : Ec;
        return i(e, O(t, 3));
      }
      var L0 = iu(Zu), q0 = iu(Ju);
      function K0(e, t) {
        return Ee(Xi(e, t), 1);
      }
      function $0(e, t) {
        return Ee(Xi(e, t), ye);
      }
      function eT(e, t, i) {
        return i = i === r ? 1 : P(i), Ee(Xi(e, t), i);
      }
      function Wu(e, t) {
        var i = I(e) ? st : Kt;
        return i(e, O(t, 3));
      }
      function Su(e, t) {
        var i = I(e) ? wm : bc;
        return i(e, O(t, 3));
      }
      var tT = Qi(function(e, t, i) {
        re.call(e, i) ? e[i].push(t) : At(e, i, [t]);
      });
      function nT(e, t, i, o) {
        e = Xe(e) ? e : Ln(e), i = i && !o ? P(i) : 0;
        var c = e.length;
        return i < 0 && (i = Ne(c + i, 0)), $i(e) ? i <= c && e.indexOf(t, i) > -1 : !!c && Dn(e, t, i) > -1;
      }
      var rT = X(function(e, t, i) {
        var o = -1, c = typeof t == "function", d = Xe(e) ? U(e.length) : [];
        return Kt(e, function(m) {
          d[++o] = c ? $e(t, m, i) : Wr(m, t, i);
        }), d;
      }), iT = Qi(function(e, t, i) {
        At(e, i, t);
      });
      function Xi(e, t) {
        var i = I(e) ? ce : Bc;
        return i(e, O(t, 3));
      }
      function aT(e, t, i, o) {
        return e == null ? [] : (I(t) || (t = t == null ? [] : [t]), i = o ? r : i, I(i) || (i = i == null ? [] : [i]), Oc(e, t, i));
      }
      var oT = Qi(function(e, t, i) {
        e[i ? 0 : 1].push(t);
      }, function() {
        return [[], []];
      });
      function lT(e, t, i) {
        var o = I(e) ? ro : uc, c = arguments.length < 3;
        return o(e, O(t, 4), i, c, Kt);
      }
      function sT(e, t, i) {
        var o = I(e) ? Um : uc, c = arguments.length < 3;
        return o(e, O(t, 4), i, c, bc);
      }
      function cT(e, t) {
        var i = I(e) ? Xt : Ec;
        return i(e, qi(O(t, 3)));
      }
      function uT(e) {
        var t = I(e) ? Zc : n1;
        return t(e);
      }
      function dT(e, t, i) {
        (i ? Ge(e, t, i) : t === r) ? t = 1 : t = P(t);
        var o = I(e) ? SV : r1;
        return o(e, t);
      }
      function fT(e) {
        var t = I(e) ? AV : a1;
        return t(e);
      }
      function hT(e) {
        if (e == null)
          return 0;
        if (Xe(e))
          return $i(e) ? On(e) : e.length;
        var t = Be(e);
        return t == Vt || t == Tt ? e.size : go(e).length;
      }
      function pT(e, t, i) {
        var o = I(e) ? io : o1;
        return i && Ge(e, t, i) && (t = r), o(e, O(t, 3));
      }
      var mT = X(function(e, t) {
        if (e == null)
          return [];
        var i = t.length;
        return i > 1 && Ge(e, t[0], t[1]) ? t = [] : i > 2 && Ge(t[0], t[1], t[2]) && (t = [t[0]]), Oc(e, Ee(t, 1), []);
      }), _i = _m || function() {
        return be.Date.now();
      };
      function VT(e, t) {
        if (typeof t != "function")
          throw new ct(h);
        return e = P(e), function() {
          if (--e < 1)
            return t.apply(this, arguments);
        };
      }
      function Au(e, t, i) {
        return t = i ? r : t, t = e && t == null ? e.length : t, xt(e, $, r, r, r, r, t);
      }
      function xu(e, t) {
        var i;
        if (typeof t != "function")
          throw new ct(h);
        return e = P(e), function() {
          return --e > 0 && (i = t.apply(this, arguments)), e <= 1 && (t = r), i;
        };
      }
      var jo = X(function(e, t, i) {
        var o = D;
        if (i.length) {
          var c = Lt(i, Xn(jo));
          o |= y;
        }
        return xt(e, o, t, i, c);
      }), Bu = X(function(e, t, i) {
        var o = D | A;
        if (i.length) {
          var c = Lt(i, Xn(Bu));
          o |= y;
        }
        return xt(t, o, e, i, c);
      });
      function Cu(e, t, i) {
        t = i ? r : t;
        var o = xt(e, S, r, r, r, r, r, t);
        return o.placeholder = Cu.placeholder, o;
      }
      function Du(e, t, i) {
        t = i ? r : t;
        var o = xt(e, H, r, r, r, r, r, t);
        return o.placeholder = Du.placeholder, o;
      }
      function Qu(e, t, i) {
        var o, c, d, m, V, w, M = 0, Z = !1, J = !1, E = !0;
        if (typeof e != "function")
          throw new ct(h);
        t = pt(t) || 0, ue(i) && (Z = !!i.leading, J = "maxWait" in i, d = J ? Ne(pt(i.maxWait) || 0, t) : d, E = "trailing" in i ? !!i.trailing : E);
        function x(Ve) {
          var Rt = o, Ot = c;
          return o = c = r, M = Ve, m = e.apply(Ot, Rt), m;
        }
        function z(Ve) {
          return M = Ve, V = Cr(q, t), Z ? x(Ve) : m;
        }
        function j(Ve) {
          var Rt = Ve - w, Ot = Ve - M, id = t - Rt;
          return J ? xe(id, d - Ot) : id;
        }
        function G(Ve) {
          var Rt = Ve - w, Ot = Ve - M;
          return w === r || Rt >= t || Rt < 0 || J && Ot >= d;
        }
        function q() {
          var Ve = _i();
          if (G(Ve))
            return ee(Ve);
          V = Cr(q, j(Ve));
        }
        function ee(Ve) {
          return V = r, E && o ? x(Ve) : (o = c = r, m);
        }
        function rt() {
          V !== r && _c(V), M = 0, o = w = c = V = r;
        }
        function Ye() {
          return V === r ? m : ee(_i());
        }
        function it() {
          var Ve = _i(), Rt = G(Ve);
          if (o = arguments, c = this, w = Ve, Rt) {
            if (V === r)
              return z(w);
            if (J)
              return _c(V), V = Cr(q, t), x(w);
          }
          return V === r && (V = Cr(q, t)), m;
        }
        return it.cancel = rt, it.flush = Ye, it;
      }
      var TT = X(function(e, t) {
        return yc(e, 1, t);
      }), vT = X(function(e, t, i) {
        return yc(e, pt(t) || 0, i);
      });
      function wT(e) {
        return xt(e, Ae);
      }
      function Li(e, t) {
        if (typeof e != "function" || t != null && typeof t != "function")
          throw new ct(h);
        var i = function() {
          var o = arguments, c = t ? t.apply(this, o) : o[0], d = i.cache;
          if (d.has(c))
            return d.get(c);
          var m = e.apply(this, o);
          return i.cache = d.set(c, m) || d, m;
        };
        return i.cache = new (Li.Cache || St)(), i;
      }
      Li.Cache = St;
      function qi(e) {
        if (typeof e != "function")
          throw new ct(h);
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return !e.call(this);
            case 1:
              return !e.call(this, t[0]);
            case 2:
              return !e.call(this, t[0], t[1]);
            case 3:
              return !e.call(this, t[0], t[1], t[2]);
          }
          return !e.apply(this, t);
        };
      }
      function UT(e) {
        return xu(2, e);
      }
      var RT = l1(function(e, t) {
        t = t.length == 1 && I(t[0]) ? ce(t[0], et(O())) : ce(Ee(t, 1), et(O()));
        var i = t.length;
        return X(function(o) {
          for (var c = -1, d = xe(o.length, i); ++c < d; )
            o[c] = t[c].call(this, o[c]);
          return $e(e, this, o);
        });
      }), Xo = X(function(e, t) {
        var i = Lt(t, Xn(Xo));
        return xt(e, y, r, t, i);
      }), Ou = X(function(e, t) {
        var i = Lt(t, Xn(Ou));
        return xt(e, _, r, t, i);
      }), NT = Bt(function(e, t) {
        return xt(e, L, r, r, r, t);
      });
      function gT(e, t) {
        if (typeof e != "function")
          throw new ct(h);
        return t = t === r ? t : P(t), X(e, t);
      }
      function MT(e, t) {
        if (typeof e != "function")
          throw new ct(h);
        return t = t == null ? 0 : Ne(P(t), 0), X(function(i) {
          var o = i[t], c = tn(i, 0, t);
          return o && _t(c, o), $e(e, this, c);
        });
      }
      function ZT(e, t, i) {
        var o = !0, c = !0;
        if (typeof e != "function")
          throw new ct(h);
        return ue(i) && (o = "leading" in i ? !!i.leading : o, c = "trailing" in i ? !!i.trailing : c), Qu(e, t, {
          leading: o,
          maxWait: t,
          trailing: c
        });
      }
      function JT(e) {
        return Au(e, 1);
      }
      function kT(e, t) {
        return Xo(Wo(t), e);
      }
      function yT() {
        if (!arguments.length)
          return [];
        var e = arguments[0];
        return I(e) ? e : [e];
      }
      function bT(e) {
        return dt(e, B);
      }
      function ET(e, t) {
        return t = typeof t == "function" ? t : r, dt(e, B, t);
      }
      function FT(e) {
        return dt(e, k | B);
      }
      function WT(e, t) {
        return t = typeof t == "function" ? t : r, dt(e, k | B, t);
      }
      function ST(e, t) {
        return t == null || kc(e, t, Me(t));
      }
      function Ut(e, t) {
        return e === t || e !== e && t !== t;
      }
      var AT = Yi(Uo), xT = Yi(function(e, t) {
        return e >= t;
      }), Nn = Sc(function() {
        return arguments;
      }()) ? Sc : function(e) {
        return he(e) && re.call(e, "callee") && !wc.call(e, "callee");
      }, I = U.isArray, BT = ec ? et(ec) : IV;
      function Xe(e) {
        return e != null && Ki(e.length) && !Dt(e);
      }
      function me(e) {
        return he(e) && Xe(e);
      }
      function CT(e) {
        return e === !0 || e === !1 || he(e) && ze(e) == Tr;
      }
      var nn = qm || al, DT = tc ? et(tc) : HV;
      function QT(e) {
        return he(e) && e.nodeType === 1 && !Dr(e);
      }
      function OT(e) {
        if (e == null)
          return !0;
        if (Xe(e) && (I(e) || typeof e == "string" || typeof e.splice == "function" || nn(e) || _n(e) || Nn(e)))
          return !e.length;
        var t = Be(e);
        if (t == Vt || t == Tt)
          return !e.size;
        if (Br(e))
          return !go(e).length;
        for (var i in e)
          if (re.call(e, i))
            return !1;
        return !0;
      }
      function zT(e, t) {
        return Sr(e, t);
      }
      function GT(e, t, i) {
        i = typeof i == "function" ? i : r;
        var o = i ? i(e, t) : r;
        return o === r ? Sr(e, t, r, i) : !!o;
      }
      function _o(e) {
        if (!he(e))
          return !1;
        var t = ze(e);
        return t == ui || t == dp || typeof e.message == "string" && typeof e.name == "string" && !Dr(e);
      }
      function YT(e) {
        return typeof e == "number" && Rc(e);
      }
      function Dt(e) {
        if (!ue(e))
          return !1;
        var t = ze(e);
        return t == di || t == ys || t == up || t == hp;
      }
      function zu(e) {
        return typeof e == "number" && e == P(e);
      }
      function Ki(e) {
        return typeof e == "number" && e > -1 && e % 1 == 0 && e <= Oe;
      }
      function ue(e) {
        var t = typeof e;
        return e != null && (t == "object" || t == "function");
      }
      function he(e) {
        return e != null && typeof e == "object";
      }
      var Gu = nc ? et(nc) : jV;
      function IT(e, t) {
        return e === t || No(e, t, Qo(t));
      }
      function HT(e, t, i) {
        return i = typeof i == "function" ? i : r, No(e, t, Qo(t), i);
      }
      function PT(e) {
        return Yu(e) && e != +e;
      }
      function jT(e) {
        if (E1(e))
          throw new Y(f);
        return Ac(e);
      }
      function XT(e) {
        return e === null;
      }
      function _T(e) {
        return e == null;
      }
      function Yu(e) {
        return typeof e == "number" || he(e) && ze(e) == wr;
      }
      function Dr(e) {
        if (!he(e) || ze(e) != Ft)
          return !1;
        var t = Zi(e);
        if (t === null)
          return !0;
        var i = re.call(t, "constructor") && t.constructor;
        return typeof i == "function" && i instanceof i && Ri.call(i) == Hm;
      }
      var Lo = rc ? et(rc) : XV;
      function LT(e) {
        return zu(e) && e >= -Oe && e <= Oe;
      }
      var Iu = ic ? et(ic) : _V;
      function $i(e) {
        return typeof e == "string" || !I(e) && he(e) && ze(e) == Rr;
      }
      function nt(e) {
        return typeof e == "symbol" || he(e) && ze(e) == fi;
      }
      var _n = ac ? et(ac) : LV;
      function qT(e) {
        return e === r;
      }
      function KT(e) {
        return he(e) && Be(e) == Nr;
      }
      function $T(e) {
        return he(e) && ze(e) == mp;
      }
      var ev = Yi(Mo), tv = Yi(function(e, t) {
        return e <= t;
      });
      function Hu(e) {
        if (!e)
          return [];
        if (Xe(e))
          return $i(e) ? vt(e) : je(e);
        if (Zr && e[Zr])
          return Sm(e[Zr]());
        var t = Be(e), i = t == Vt ? uo : t == Tt ? vi : Ln;
        return i(e);
      }
      function Qt(e) {
        if (!e)
          return e === 0 ? e : 0;
        if (e = pt(e), e === ye || e === -ye) {
          var t = e < 0 ? -1 : 1;
          return t * Vr;
        }
        return e === e ? e : 0;
      }
      function P(e) {
        var t = Qt(e), i = t % 1;
        return t === t ? i ? t - i : t : 0;
      }
      function Pu(e) {
        return e ? vn(P(e), 0, Zt) : 0;
      }
      function pt(e) {
        if (typeof e == "number")
          return e;
        if (nt(e))
          return si;
        if (ue(e)) {
          var t = typeof e.valueOf == "function" ? e.valueOf() : e;
          e = ue(t) ? t + "" : t;
        }
        if (typeof e != "string")
          return e === 0 ? e : +e;
        e = dc(e);
        var i = Bp.test(e);
        return i || Dp.test(e) ? Vm(e.slice(2), i ? 2 : 8) : xp.test(e) ? si : +e;
      }
      function ju(e) {
        return kt(e, _e(e));
      }
      function nv(e) {
        return e ? vn(P(e), -Oe, Oe) : e === 0 ? e : 0;
      }
      function ne(e) {
        return e == null ? "" : tt(e);
      }
      var rv = Pn(function(e, t) {
        if (Br(t) || Xe(t)) {
          kt(t, Me(t), e);
          return;
        }
        for (var i in t)
          re.call(t, i) && Er(e, i, t[i]);
      }), Xu = Pn(function(e, t) {
        kt(t, _e(t), e);
      }), ea = Pn(function(e, t, i, o) {
        kt(t, _e(t), e, o);
      }), iv = Pn(function(e, t, i, o) {
        kt(t, Me(t), e, o);
      }), av = Bt(To);
      function ov(e, t) {
        var i = Hn(e);
        return t == null ? i : Jc(i, t);
      }
      var lv = X(function(e, t) {
        e = oe(e);
        var i = -1, o = t.length, c = o > 2 ? t[2] : r;
        for (c && Ge(t[0], t[1], c) && (o = 1); ++i < o; )
          for (var d = t[i], m = _e(d), V = -1, w = m.length; ++V < w; ) {
            var M = m[V], Z = e[M];
            (Z === r || Ut(Z, Gn[M]) && !re.call(e, M)) && (e[M] = d[M]);
          }
        return e;
      }), sv = X(function(e) {
        return e.push(r, du), $e(_u, r, e);
      });
      function cv(e, t) {
        return lc(e, O(t, 3), Jt);
      }
      function uv(e, t) {
        return lc(e, O(t, 3), wo);
      }
      function dv(e, t) {
        return e == null ? e : vo(e, O(t, 3), _e);
      }
      function fv(e, t) {
        return e == null ? e : Fc(e, O(t, 3), _e);
      }
      function hv(e, t) {
        return e && Jt(e, O(t, 3));
      }
      function pv(e, t) {
        return e && wo(e, O(t, 3));
      }
      function mv(e) {
        return e == null ? [] : xi(e, Me(e));
      }
      function Vv(e) {
        return e == null ? [] : xi(e, _e(e));
      }
      function qo(e, t, i) {
        var o = e == null ? r : wn(e, t);
        return o === r ? i : o;
      }
      function Tv(e, t) {
        return e != null && pu(e, t, OV);
      }
      function Ko(e, t) {
        return e != null && pu(e, t, zV);
      }
      var vv = ou(function(e, t, i) {
        t != null && typeof t.toString != "function" && (t = Ni.call(t)), e[t] = i;
      }, el(Le)), wv = ou(function(e, t, i) {
        t != null && typeof t.toString != "function" && (t = Ni.call(t)), re.call(e, t) ? e[t].push(i) : e[t] = [i];
      }, O), Uv = X(Wr);
      function Me(e) {
        return Xe(e) ? Mc(e) : go(e);
      }
      function _e(e) {
        return Xe(e) ? Mc(e, !0) : qV(e);
      }
      function Rv(e, t) {
        var i = {};
        return t = O(t, 3), Jt(e, function(o, c, d) {
          At(i, t(o, c, d), o);
        }), i;
      }
      function Nv(e, t) {
        var i = {};
        return t = O(t, 3), Jt(e, function(o, c, d) {
          At(i, c, t(o, c, d));
        }), i;
      }
      var gv = Pn(function(e, t, i) {
        Bi(e, t, i);
      }), _u = Pn(function(e, t, i, o) {
        Bi(e, t, i, o);
      }), Mv = Bt(function(e, t) {
        var i = {};
        if (e == null)
          return i;
        var o = !1;
        t = ce(t, function(d) {
          return d = en(d, e), o || (o = d.length > 1), d;
        }), kt(e, Co(e), i), o && (i = dt(i, k | b | B, v1));
        for (var c = t.length; c--; )
          bo(i, t[c]);
        return i;
      });
      function Zv(e, t) {
        return Lu(e, qi(O(t)));
      }
      var Jv = Bt(function(e, t) {
        return e == null ? {} : $V(e, t);
      });
      function Lu(e, t) {
        if (e == null)
          return {};
        var i = ce(Co(e), function(o) {
          return [o];
        });
        return t = O(t), zc(e, i, function(o, c) {
          return t(o, c[0]);
        });
      }
      function kv(e, t, i) {
        t = en(t, e);
        var o = -1, c = t.length;
        for (c || (c = 1, e = r); ++o < c; ) {
          var d = e == null ? r : e[yt(t[o])];
          d === r && (o = c, d = i), e = Dt(d) ? d.call(e) : d;
        }
        return e;
      }
      function yv(e, t, i) {
        return e == null ? e : Ar(e, t, i);
      }
      function bv(e, t, i, o) {
        return o = typeof o == "function" ? o : r, e == null ? e : Ar(e, t, i, o);
      }
      var qu = cu(Me), Ku = cu(_e);
      function Ev(e, t, i) {
        var o = I(e), c = o || nn(e) || _n(e);
        if (t = O(t, 4), i == null) {
          var d = e && e.constructor;
          c ? i = o ? new d() : [] : ue(e) ? i = Dt(d) ? Hn(Zi(e)) : {} : i = {};
        }
        return (c ? st : Jt)(e, function(m, V, w) {
          return t(i, m, V, w);
        }), i;
      }
      function Fv(e, t) {
        return e == null ? !0 : bo(e, t);
      }
      function Wv(e, t, i) {
        return e == null ? e : Pc(e, t, Wo(i));
      }
      function Sv(e, t, i, o) {
        return o = typeof o == "function" ? o : r, e == null ? e : Pc(e, t, Wo(i), o);
      }
      function Ln(e) {
        return e == null ? [] : co(e, Me(e));
      }
      function Av(e) {
        return e == null ? [] : co(e, _e(e));
      }
      function xv(e, t, i) {
        return i === r && (i = t, t = r), i !== r && (i = pt(i), i = i === i ? i : 0), t !== r && (t = pt(t), t = t === t ? t : 0), vn(pt(e), t, i);
      }
      function Bv(e, t, i) {
        return t = Qt(t), i === r ? (i = t, t = 0) : i = Qt(i), e = pt(e), GV(e, t, i);
      }
      function Cv(e, t, i) {
        if (i && typeof i != "boolean" && Ge(e, t, i) && (t = i = r), i === r && (typeof t == "boolean" ? (i = t, t = r) : typeof e == "boolean" && (i = e, e = r)), e === r && t === r ? (e = 0, t = 1) : (e = Qt(e), t === r ? (t = e, e = 0) : t = Qt(t)), e > t) {
          var o = e;
          e = t, t = o;
        }
        if (i || e % 1 || t % 1) {
          var c = Nc();
          return xe(e + c * (t - e + mm("1e-" + ((c + "").length - 1))), t);
        }
        return Jo(e, t);
      }
      var Dv = jn(function(e, t, i) {
        return t = t.toLowerCase(), e + (i ? $u(t) : t);
      });
      function $u(e) {
        return $o(ne(e).toLowerCase());
      }
      function ed(e) {
        return e = ne(e), e && e.replace(Op, ym).replace(am, "");
      }
      function Qv(e, t, i) {
        e = ne(e), t = tt(t);
        var o = e.length;
        i = i === r ? o : vn(P(i), 0, o);
        var c = i;
        return i -= t.length, i >= 0 && e.slice(i, c) == t;
      }
      function Ov(e) {
        return e = ne(e), e && Up.test(e) ? e.replace(Fs, bm) : e;
      }
      function zv(e) {
        return e = ne(e), e && Jp.test(e) ? e.replace(ja, "\\$&") : e;
      }
      var Gv = jn(function(e, t, i) {
        return e + (i ? "-" : "") + t.toLowerCase();
      }), Yv = jn(function(e, t, i) {
        return e + (i ? " " : "") + t.toLowerCase();
      }), Iv = ru("toLowerCase");
      function Hv(e, t, i) {
        e = ne(e), t = P(t);
        var o = t ? On(e) : 0;
        if (!t || o >= t)
          return e;
        var c = (t - o) / 2;
        return Gi(bi(c), i) + e + Gi(yi(c), i);
      }
      function Pv(e, t, i) {
        e = ne(e), t = P(t);
        var o = t ? On(e) : 0;
        return t && o < t ? e + Gi(t - o, i) : e;
      }
      function jv(e, t, i) {
        e = ne(e), t = P(t);
        var o = t ? On(e) : 0;
        return t && o < t ? Gi(t - o, i) + e : e;
      }
      function Xv(e, t, i) {
        return i || t == null ? t = 0 : t && (t = +t), tV(ne(e).replace(Xa, ""), t || 0);
      }
      function _v(e, t, i) {
        return (i ? Ge(e, t, i) : t === r) ? t = 1 : t = P(t), ko(ne(e), t);
      }
      function Lv() {
        var e = arguments, t = ne(e[0]);
        return e.length < 3 ? t : t.replace(e[1], e[2]);
      }
      var qv = jn(function(e, t, i) {
        return e + (i ? "_" : "") + t.toLowerCase();
      });
      function Kv(e, t, i) {
        return i && typeof i != "number" && Ge(e, t, i) && (t = i = r), i = i === r ? Zt : i >>> 0, i ? (e = ne(e), e && (typeof t == "string" || t != null && !Lo(t)) && (t = tt(t), !t && Qn(e)) ? tn(vt(e), 0, i) : e.split(t, i)) : [];
      }
      var $v = jn(function(e, t, i) {
        return e + (i ? " " : "") + $o(t);
      });
      function ew(e, t, i) {
        return e = ne(e), i = i == null ? 0 : vn(P(i), 0, e.length), t = tt(t), e.slice(i, i + t.length) == t;
      }
      function tw(e, t, i) {
        var o = u.templateSettings;
        i && Ge(e, t, i) && (t = r), e = ne(e), t = ea({}, t, o, uu);
        var c = ea({}, t.imports, o.imports, uu), d = Me(c), m = co(c, d), V, w, M = 0, Z = t.interpolate || hi, J = "__p += '", E = fo(
          (t.escape || hi).source + "|" + Z.source + "|" + (Z === Ws ? Ap : hi).source + "|" + (t.evaluate || hi).source + "|$",
          "g"
        ), x = "//# sourceURL=" + (re.call(t, "sourceURL") ? (t.sourceURL + "").replace(/\s/g, " ") : "lodash.templateSources[" + ++um + "]") + `
`;
        e.replace(E, function(G, q, ee, rt, Ye, it) {
          return ee || (ee = rt), J += e.slice(M, it).replace(zp, Em), q && (V = !0, J += `' +
__e(` + q + `) +
'`), Ye && (w = !0, J += `';
` + Ye + `;
__p += '`), ee && (J += `' +
((__t = (` + ee + `)) == null ? '' : __t) +
'`), M = it + G.length, G;
        }), J += `';
`;
        var z = re.call(t, "variable") && t.variable;
        if (!z)
          J = `with (obj) {
` + J + `
}
`;
        else if (Wp.test(z))
          throw new Y(p);
        J = (w ? J.replace(Vp, "") : J).replace(Tp, "$1").replace(vp, "$1;"), J = "function(" + (z || "obj") + `) {
` + (z ? "" : `obj || (obj = {});
`) + "var __t, __p = ''" + (V ? ", __e = _.escape" : "") + (w ? `, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
` : `;
`) + J + `return __p
}`;
        var j = nd(function() {
          return te(d, x + "return " + J).apply(r, m);
        });
        if (j.source = J, _o(j))
          throw j;
        return j;
      }
      function nw(e) {
        return ne(e).toLowerCase();
      }
      function rw(e) {
        return ne(e).toUpperCase();
      }
      function iw(e, t, i) {
        if (e = ne(e), e && (i || t === r))
          return dc(e);
        if (!e || !(t = tt(t)))
          return e;
        var o = vt(e), c = vt(t), d = fc(o, c), m = hc(o, c) + 1;
        return tn(o, d, m).join("");
      }
      function aw(e, t, i) {
        if (e = ne(e), e && (i || t === r))
          return e.slice(0, mc(e) + 1);
        if (!e || !(t = tt(t)))
          return e;
        var o = vt(e), c = hc(o, vt(t)) + 1;
        return tn(o, 0, c).join("");
      }
      function ow(e, t, i) {
        if (e = ne(e), e && (i || t === r))
          return e.replace(Xa, "");
        if (!e || !(t = tt(t)))
          return e;
        var o = vt(e), c = fc(o, vt(t));
        return tn(o, c).join("");
      }
      function lw(e, t) {
        var i = Pe, o = de;
        if (ue(t)) {
          var c = "separator" in t ? t.separator : c;
          i = "length" in t ? P(t.length) : i, o = "omission" in t ? tt(t.omission) : o;
        }
        e = ne(e);
        var d = e.length;
        if (Qn(e)) {
          var m = vt(e);
          d = m.length;
        }
        if (i >= d)
          return e;
        var V = i - On(o);
        if (V < 1)
          return o;
        var w = m ? tn(m, 0, V).join("") : e.slice(0, V);
        if (c === r)
          return w + o;
        if (m && (V += w.length - V), Lo(c)) {
          if (e.slice(V).search(c)) {
            var M, Z = w;
            for (c.global || (c = fo(c.source, ne(Ss.exec(c)) + "g")), c.lastIndex = 0; M = c.exec(Z); )
              var J = M.index;
            w = w.slice(0, J === r ? V : J);
          }
        } else if (e.indexOf(tt(c), V) != V) {
          var E = w.lastIndexOf(c);
          E > -1 && (w = w.slice(0, E));
        }
        return w + o;
      }
      function sw(e) {
        return e = ne(e), e && wp.test(e) ? e.replace(Es, Cm) : e;
      }
      var cw = jn(function(e, t, i) {
        return e + (i ? " " : "") + t.toUpperCase();
      }), $o = ru("toUpperCase");
      function td(e, t, i) {
        return e = ne(e), t = i ? r : t, t === r ? Wm(e) ? Om(e) : gm(e) : e.match(t) || [];
      }
      var nd = X(function(e, t) {
        try {
          return $e(e, r, t);
        } catch (i) {
          return _o(i) ? i : new Y(i);
        }
      }), uw = Bt(function(e, t) {
        return st(t, function(i) {
          i = yt(i), At(e, i, jo(e[i], e));
        }), e;
      });
      function dw(e) {
        var t = e == null ? 0 : e.length, i = O();
        return e = t ? ce(e, function(o) {
          if (typeof o[1] != "function")
            throw new ct(h);
          return [i(o[0]), o[1]];
        }) : [], X(function(o) {
          for (var c = -1; ++c < t; ) {
            var d = e[c];
            if ($e(d[0], this, o))
              return $e(d[1], this, o);
          }
        });
      }
      function fw(e) {
        return CV(dt(e, k));
      }
      function el(e) {
        return function() {
          return e;
        };
      }
      function hw(e, t) {
        return e == null || e !== e ? t : e;
      }
      var pw = au(), mw = au(!0);
      function Le(e) {
        return e;
      }
      function tl(e) {
        return xc(typeof e == "function" ? e : dt(e, k));
      }
      function Vw(e) {
        return Cc(dt(e, k));
      }
      function Tw(e, t) {
        return Dc(e, dt(t, k));
      }
      var vw = X(function(e, t) {
        return function(i) {
          return Wr(i, e, t);
        };
      }), ww = X(function(e, t) {
        return function(i) {
          return Wr(e, i, t);
        };
      });
      function nl(e, t, i) {
        var o = Me(t), c = xi(t, o);
        i == null && !(ue(t) && (c.length || !o.length)) && (i = t, t = e, e = this, c = xi(t, Me(t)));
        var d = !(ue(i) && "chain" in i) || !!i.chain, m = Dt(e);
        return st(c, function(V) {
          var w = t[V];
          e[V] = w, m && (e.prototype[V] = function() {
            var M = this.__chain__;
            if (d || M) {
              var Z = e(this.__wrapped__), J = Z.__actions__ = je(this.__actions__);
              return J.push({ func: w, args: arguments, thisArg: e }), Z.__chain__ = M, Z;
            }
            return w.apply(e, _t([this.value()], arguments));
          });
        }), e;
      }
      function Uw() {
        return be._ === this && (be._ = Pm), this;
      }
      function rl() {
      }
      function Rw(e) {
        return e = P(e), X(function(t) {
          return Qc(t, e);
        });
      }
      var Nw = Ao(ce), gw = Ao(oc), Mw = Ao(io);
      function rd(e) {
        return zo(e) ? ao(yt(e)) : e1(e);
      }
      function Zw(e) {
        return function(t) {
          return e == null ? r : wn(e, t);
        };
      }
      var Jw = lu(), kw = lu(!0);
      function il() {
        return [];
      }
      function al() {
        return !1;
      }
      function yw() {
        return {};
      }
      function bw() {
        return "";
      }
      function Ew() {
        return !0;
      }
      function Fw(e, t) {
        if (e = P(e), e < 1 || e > Oe)
          return [];
        var i = Zt, o = xe(e, Zt);
        t = O(t), e -= Zt;
        for (var c = so(o, t); ++i < e; )
          t(i);
        return c;
      }
      function Ww(e) {
        return I(e) ? ce(e, yt) : nt(e) ? [e] : je(gu(ne(e)));
      }
      function Sw(e) {
        var t = ++Im;
        return ne(e) + t;
      }
      var Aw = zi(function(e, t) {
        return e + t;
      }, 0), xw = xo("ceil"), Bw = zi(function(e, t) {
        return e / t;
      }, 1), Cw = xo("floor");
      function Dw(e) {
        return e && e.length ? Ai(e, Le, Uo) : r;
      }
      function Qw(e, t) {
        return e && e.length ? Ai(e, O(t, 2), Uo) : r;
      }
      function Ow(e) {
        return cc(e, Le);
      }
      function zw(e, t) {
        return cc(e, O(t, 2));
      }
      function Gw(e) {
        return e && e.length ? Ai(e, Le, Mo) : r;
      }
      function Yw(e, t) {
        return e && e.length ? Ai(e, O(t, 2), Mo) : r;
      }
      var Iw = zi(function(e, t) {
        return e * t;
      }, 1), Hw = xo("round"), Pw = zi(function(e, t) {
        return e - t;
      }, 0);
      function jw(e) {
        return e && e.length ? lo(e, Le) : 0;
      }
      function Xw(e, t) {
        return e && e.length ? lo(e, O(t, 2)) : 0;
      }
      return u.after = VT, u.ary = Au, u.assign = rv, u.assignIn = Xu, u.assignInWith = ea, u.assignWith = iv, u.at = av, u.before = xu, u.bind = jo, u.bindAll = uw, u.bindKey = Bu, u.castArray = yT, u.chain = Fu, u.chunk = C1, u.compact = D1, u.concat = Q1, u.cond = dw, u.conforms = fw, u.constant = el, u.countBy = j0, u.create = ov, u.curry = Cu, u.curryRight = Du, u.debounce = Qu, u.defaults = lv, u.defaultsDeep = sv, u.defer = TT, u.delay = vT, u.difference = O1, u.differenceBy = z1, u.differenceWith = G1, u.drop = Y1, u.dropRight = I1, u.dropRightWhile = H1, u.dropWhile = P1, u.fill = j1, u.filter = _0, u.flatMap = K0, u.flatMapDeep = $0, u.flatMapDepth = eT, u.flatten = ku, u.flattenDeep = X1, u.flattenDepth = _1, u.flip = wT, u.flow = pw, u.flowRight = mw, u.fromPairs = L1, u.functions = mv, u.functionsIn = Vv, u.groupBy = tT, u.initial = K1, u.intersection = $1, u.intersectionBy = e0, u.intersectionWith = t0, u.invert = vv, u.invertBy = wv, u.invokeMap = rT, u.iteratee = tl, u.keyBy = iT, u.keys = Me, u.keysIn = _e, u.map = Xi, u.mapKeys = Rv, u.mapValues = Nv, u.matches = Vw, u.matchesProperty = Tw, u.memoize = Li, u.merge = gv, u.mergeWith = _u, u.method = vw, u.methodOf = ww, u.mixin = nl, u.negate = qi, u.nthArg = Rw, u.omit = Mv, u.omitBy = Zv, u.once = UT, u.orderBy = aT, u.over = Nw, u.overArgs = RT, u.overEvery = gw, u.overSome = Mw, u.partial = Xo, u.partialRight = Ou, u.partition = oT, u.pick = Jv, u.pickBy = Lu, u.property = rd, u.propertyOf = Zw, u.pull = a0, u.pullAll = bu, u.pullAllBy = o0, u.pullAllWith = l0, u.pullAt = s0, u.range = Jw, u.rangeRight = kw, u.rearg = NT, u.reject = cT, u.remove = c0, u.rest = gT, u.reverse = Ho, u.sampleSize = dT, u.set = yv, u.setWith = bv, u.shuffle = fT, u.slice = u0, u.sortBy = mT, u.sortedUniq = T0, u.sortedUniqBy = v0, u.split = Kv, u.spread = MT, u.tail = w0, u.take = U0, u.takeRight = R0, u.takeRightWhile = N0, u.takeWhile = g0, u.tap = D0, u.throttle = ZT, u.thru = ji, u.toArray = Hu, u.toPairs = qu, u.toPairsIn = Ku, u.toPath = Ww, u.toPlainObject = ju, u.transform = Ev, u.unary = JT, u.union = M0, u.unionBy = Z0, u.unionWith = J0, u.uniq = k0, u.uniqBy = y0, u.uniqWith = b0, u.unset = Fv, u.unzip = Po, u.unzipWith = Eu, u.update = Wv, u.updateWith = Sv, u.values = Ln, u.valuesIn = Av, u.without = E0, u.words = td, u.wrap = kT, u.xor = F0, u.xorBy = W0, u.xorWith = S0, u.zip = A0, u.zipObject = x0, u.zipObjectDeep = B0, u.zipWith = C0, u.entries = qu, u.entriesIn = Ku, u.extend = Xu, u.extendWith = ea, nl(u, u), u.add = Aw, u.attempt = nd, u.camelCase = Dv, u.capitalize = $u, u.ceil = xw, u.clamp = xv, u.clone = bT, u.cloneDeep = FT, u.cloneDeepWith = WT, u.cloneWith = ET, u.conformsTo = ST, u.deburr = ed, u.defaultTo = hw, u.divide = Bw, u.endsWith = Qv, u.eq = Ut, u.escape = Ov, u.escapeRegExp = zv, u.every = X0, u.find = L0, u.findIndex = Zu, u.findKey = cv, u.findLast = q0, u.findLastIndex = Ju, u.findLastKey = uv, u.floor = Cw, u.forEach = Wu, u.forEachRight = Su, u.forIn = dv, u.forInRight = fv, u.forOwn = hv, u.forOwnRight = pv, u.get = qo, u.gt = AT, u.gte = xT, u.has = Tv, u.hasIn = Ko, u.head = yu, u.identity = Le, u.includes = nT, u.indexOf = q1, u.inRange = Bv, u.invoke = Uv, u.isArguments = Nn, u.isArray = I, u.isArrayBuffer = BT, u.isArrayLike = Xe, u.isArrayLikeObject = me, u.isBoolean = CT, u.isBuffer = nn, u.isDate = DT, u.isElement = QT, u.isEmpty = OT, u.isEqual = zT, u.isEqualWith = GT, u.isError = _o, u.isFinite = YT, u.isFunction = Dt, u.isInteger = zu, u.isLength = Ki, u.isMap = Gu, u.isMatch = IT, u.isMatchWith = HT, u.isNaN = PT, u.isNative = jT, u.isNil = _T, u.isNull = XT, u.isNumber = Yu, u.isObject = ue, u.isObjectLike = he, u.isPlainObject = Dr, u.isRegExp = Lo, u.isSafeInteger = LT, u.isSet = Iu, u.isString = $i, u.isSymbol = nt, u.isTypedArray = _n, u.isUndefined = qT, u.isWeakMap = KT, u.isWeakSet = $T, u.join = n0, u.kebabCase = Gv, u.last = ht, u.lastIndexOf = r0, u.lowerCase = Yv, u.lowerFirst = Iv, u.lt = ev, u.lte = tv, u.max = Dw, u.maxBy = Qw, u.mean = Ow, u.meanBy = zw, u.min = Gw, u.minBy = Yw, u.stubArray = il, u.stubFalse = al, u.stubObject = yw, u.stubString = bw, u.stubTrue = Ew, u.multiply = Iw, u.nth = i0, u.noConflict = Uw, u.noop = rl, u.now = _i, u.pad = Hv, u.padEnd = Pv, u.padStart = jv, u.parseInt = Xv, u.random = Cv, u.reduce = lT, u.reduceRight = sT, u.repeat = _v, u.replace = Lv, u.result = kv, u.round = Hw, u.runInContext = v, u.sample = uT, u.size = hT, u.snakeCase = qv, u.some = pT, u.sortedIndex = d0, u.sortedIndexBy = f0, u.sortedIndexOf = h0, u.sortedLastIndex = p0, u.sortedLastIndexBy = m0, u.sortedLastIndexOf = V0, u.startCase = $v, u.startsWith = ew, u.subtract = Pw, u.sum = jw, u.sumBy = Xw, u.template = tw, u.times = Fw, u.toFinite = Qt, u.toInteger = P, u.toLength = Pu, u.toLower = nw, u.toNumber = pt, u.toSafeInteger = nv, u.toString = ne, u.toUpper = rw, u.trim = iw, u.trimEnd = aw, u.trimStart = ow, u.truncate = lw, u.unescape = sw, u.uniqueId = Sw, u.upperCase = cw, u.upperFirst = $o, u.each = Wu, u.eachRight = Su, u.first = yu, nl(u, function() {
        var e = {};
        return Jt(u, function(t, i) {
          re.call(u.prototype, i) || (e[i] = t);
        }), e;
      }(), { chain: !1 }), u.VERSION = l, st(["bind", "bindKey", "curry", "curryRight", "partial", "partialRight"], function(e) {
        u[e].placeholder = u;
      }), st(["drop", "take"], function(e, t) {
        K.prototype[e] = function(i) {
          i = i === r ? 1 : Ne(P(i), 0);
          var o = this.__filtered__ && !t ? new K(this) : this.clone();
          return o.__filtered__ ? o.__takeCount__ = xe(i, o.__takeCount__) : o.__views__.push({
            size: xe(i, Zt),
            type: e + (o.__dir__ < 0 ? "Right" : "")
          }), o;
        }, K.prototype[e + "Right"] = function(i) {
          return this.reverse()[e](i).reverse();
        };
      }), st(["filter", "map", "takeWhile"], function(e, t) {
        var i = t + 1, o = i == we || i == ke;
        K.prototype[e] = function(c) {
          var d = this.clone();
          return d.__iteratees__.push({
            iteratee: O(c, 3),
            type: i
          }), d.__filtered__ = d.__filtered__ || o, d;
        };
      }), st(["head", "last"], function(e, t) {
        var i = "take" + (t ? "Right" : "");
        K.prototype[e] = function() {
          return this[i](1).value()[0];
        };
      }), st(["initial", "tail"], function(e, t) {
        var i = "drop" + (t ? "" : "Right");
        K.prototype[e] = function() {
          return this.__filtered__ ? new K(this) : this[i](1);
        };
      }), K.prototype.compact = function() {
        return this.filter(Le);
      }, K.prototype.find = function(e) {
        return this.filter(e).head();
      }, K.prototype.findLast = function(e) {
        return this.reverse().find(e);
      }, K.prototype.invokeMap = X(function(e, t) {
        return typeof e == "function" ? new K(this) : this.map(function(i) {
          return Wr(i, e, t);
        });
      }), K.prototype.reject = function(e) {
        return this.filter(qi(O(e)));
      }, K.prototype.slice = function(e, t) {
        e = P(e);
        var i = this;
        return i.__filtered__ && (e > 0 || t < 0) ? new K(i) : (e < 0 ? i = i.takeRight(-e) : e && (i = i.drop(e)), t !== r && (t = P(t), i = t < 0 ? i.dropRight(-t) : i.take(t - e)), i);
      }, K.prototype.takeRightWhile = function(e) {
        return this.reverse().takeWhile(e).reverse();
      }, K.prototype.toArray = function() {
        return this.take(Zt);
      }, Jt(K.prototype, function(e, t) {
        var i = /^(?:filter|find|map|reject)|While$/.test(t), o = /^(?:head|last)$/.test(t), c = u[o ? "take" + (t == "last" ? "Right" : "") : t], d = o || /^find/.test(t);
        !c || (u.prototype[t] = function() {
          var m = this.__wrapped__, V = o ? [1] : arguments, w = m instanceof K, M = V[0], Z = w || I(m), J = function(q) {
            var ee = c.apply(u, _t([q], V));
            return o && E ? ee[0] : ee;
          };
          Z && i && typeof M == "function" && M.length != 1 && (w = Z = !1);
          var E = this.__chain__, x = !!this.__actions__.length, z = d && !E, j = w && !x;
          if (!d && Z) {
            m = j ? m : new K(this);
            var G = e.apply(m, V);
            return G.__actions__.push({ func: ji, args: [J], thisArg: r }), new ut(G, E);
          }
          return z && j ? e.apply(this, V) : (G = this.thru(J), z ? o ? G.value()[0] : G.value() : G);
        });
      }), st(["pop", "push", "shift", "sort", "splice", "unshift"], function(e) {
        var t = wi[e], i = /^(?:push|sort|unshift)$/.test(e) ? "tap" : "thru", o = /^(?:pop|shift)$/.test(e);
        u.prototype[e] = function() {
          var c = arguments;
          if (o && !this.__chain__) {
            var d = this.value();
            return t.apply(I(d) ? d : [], c);
          }
          return this[i](function(m) {
            return t.apply(I(m) ? m : [], c);
          });
        };
      }), Jt(K.prototype, function(e, t) {
        var i = u[t];
        if (i) {
          var o = i.name + "";
          re.call(In, o) || (In[o] = []), In[o].push({ name: t, func: i });
        }
      }), In[Oi(r, A).name] = [{
        name: "wrapper",
        func: r
      }], K.prototype.clone = sV, K.prototype.reverse = cV, K.prototype.value = uV, u.prototype.at = Q0, u.prototype.chain = O0, u.prototype.commit = z0, u.prototype.next = G0, u.prototype.plant = I0, u.prototype.reverse = H0, u.prototype.toJSON = u.prototype.valueOf = u.prototype.value = P0, u.prototype.first = u.prototype.head, Zr && (u.prototype[Zr] = Y0), u;
    }, zn = zm();
    pn ? ((pn.exports = zn)._ = zn, eo._ = zn) : be._ = zn;
  }).call(Qr);
})(qe, qe.exports);
const RR = window.Pinia.defineStore, NR = RR("situationsStore", {
  state: () => ({
    situations: []
  }),
  actions: {
    async getSituations() {
      this.situations = [];
      const n = await UR();
      if (n) {
        const a = qe.exports.groupBy(n, "status"), r = [
          ...a.CREATED || [],
          ...a.ACCEPTED || [],
          ...a.REJECTED || []
        ];
        this.situations = r;
      }
    }
  }
}), gR = window.Vue.defineComponent, MR = window.Vue.unref, ZR = window.Vue.renderList, JR = window.Vue.Fragment, Rl = window.Vue.openBlock, Nl = window.Vue.createElementBlock, kR = window.Vue.toDisplayString, yR = window.Vue.normalizeClass;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const bR = { class: "alarms-list" }, ER = /* @__PURE__ */ gR({
  props: {
    alarms: null,
    size: null
  },
  setup(n) {
    const a = n;
    return (r, l) => (Rl(), Nl("div", bR, [
      (Rl(!0), Nl(JR, null, ZR(MR(qe.exports.groupBy)(a == null ? void 0 : a.alarms, "severity"), (s, f) => (Rl(), Nl("div", {
        class: yR(["alarm-count", [`${f.toString().toLowerCase()}-color`, a.size]]),
        key: f
      }, kR(s.length), 3))), 128))
    ]));
  }
});
const Uh = /* @__PURE__ */ Qe(ER, [["__scopeId", "data-v-950fe87b"]]), FR = window.Vue.openBlock, WR = window.Vue.createElementBlock, SR = window.Vue.createElementVNode;
var AR = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const xR = {}, BR = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, CR = /* @__PURE__ */ SR("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM15.36,8.64l-4.95,4.95-2.3-2.3a1,1,0,0,0-1.41,0h0a1,1,0,0,0,0,1.41l3,3a1,1,0,0,0,1.41,0l5.66-5.66a1,1,0,0,0,0-1.41h0A1,1,0,0,0,15.36,8.64Z" }, null, -1), DR = [
  CR
];
function QR(n, a) {
  return FR(), WR("svg", BR, DR);
}
var Rh = /* @__PURE__ */ AR(xR, [["render", QR]]);
var OR = Object.defineProperty, yd = Object.getOwnPropertySymbols, zR = Object.prototype.hasOwnProperty, GR = Object.prototype.propertyIsEnumerable, bd = (n, a, r) => a in n ? OR(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, Ed = (n, a) => {
  for (var r in a || (a = {}))
    zR.call(a, r) && bd(n, r, a[r]);
  if (yd)
    for (var r of yd(a))
      GR.call(a, r) && bd(n, r, a[r]);
  return n;
};
const YR = window.Vue.defineComponent, IR = window.Vue.toRaw, gl = window.Vue.h;
var HR = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const PR = {
  icon: {
    type: Object,
    required: !1
  },
  flex: {
    type: Boolean,
    default: !1
  },
  title: {
    type: String,
    required: !1
  }
}, jR = YR({
  props: PR,
  render() {
    const n = this.$attrs, a = n.class ? n.class.split(" ").reduce((s, f) => (s[f] = !0, s), {}) : {}, r = {};
    a["feather-icon"] = !0, this.flex && (a["feather-icon-flex"] = !0), this.title ? (r["aria-label"] = this.title, r["aria-hidden"] = "false") : r["aria-hidden"] = "true", r.focusable = "false", r.role = "img";
    let l = IR(this.icon);
    return this.$slots.default ? gl("span", { class: "feather-icon-container" }, [
      gl(this.$slots.default()[0], Ed({
        class: a
      }, r))
    ]) : gl(l, Ed({
      class: a
    }, r));
  }
});
var He = /* @__PURE__ */ HR(jR, [["__scopeId", "data-v-52cbf270"]]);
const XR = window.Vue.openBlock, _R = window.Vue.createElementBlock, LR = window.Vue.createElementVNode;
var qR = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const KR = {}, $R = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, eN = /* @__PURE__ */ LR("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM14.59,8,12,10.59,9.41,8A1,1,0,0,0,8,8H8A1,1,0,0,0,8,9.41L10.59,12,8,14.59A1,1,0,0,0,8,16H8a1,1,0,0,0,1.41,0L12,13.41,14.59,16A1,1,0,0,0,16,16h0a1,1,0,0,0,0-1.41L13.41,12,16,9.41A1,1,0,0,0,16,8h0A1,1,0,0,0,14.59,8Z" }, null, -1), tN = [
  eN
];
function nN(n, a) {
  return XR(), _R("svg", $R, tN);
}
var Nh = /* @__PURE__ */ qR(KR, [["render", nN]]);
const rN = window.Vue.defineComponent, gn = window.Vue.unref, Fd = window.Vue.normalizeClass, ta = window.Vue.createElementVNode, iN = window.Vue.toDisplayString, Ml = window.Vue.createVNode, Zl = window.Vue.openBlock, Jl = window.Vue.createElementBlock, Wd = window.Vue.createCommentVNode;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const aN = { class: "content" }, oN = { class: "title-row" }, lN = { class: "title" }, sN = {
  key: 0,
  class: "accepted"
}, cN = {
  key: 1,
  class: "rejected"
}, uN = /* @__PURE__ */ rN({
  props: {
    situationInfo: null,
    selected: { type: Boolean }
  },
  emits: ["situation-selected"],
  setup(n, { emit: a }) {
    const r = n, l = gt.ACCEPTED, s = gt.REJECTED, f = () => {
      var h;
      a("situation-selected", (h = r.situationInfo) == null ? void 0 : h.id);
    };
    return (h, p) => {
      var T, N, g, k;
      return Zl(), Jl("div", {
        onClick: f,
        class: Fd(["card", {
          selected: r.selected,
          rejected: r.situationInfo.status == gn(s)
        }])
      }, [
        ta("div", {
          class: Fd(["severity-line", [`${(N = (T = r.situationInfo) == null ? void 0 : T.severity) == null ? void 0 : N.toLowerCase()}-bg dark`]])
        }, null, 2),
        ta("div", aN, [
          ta("div", oN, [
            ta("div", lN, "[ " + iN((g = r.situationInfo) == null ? void 0 : g.id) + " ]", 1),
            r.situationInfo.status == gn(l) ? (Zl(), Jl("div", sN, [
              Ml(gn(He), {
                icon: gn(Rh),
                "aria-hidden": "true",
                class: "icon accepted"
              }, null, 8, ["icon"])
            ])) : Wd("", !0),
            r.situationInfo.status == gn(s) ? (Zl(), Jl("div", cN, [
              Ml(gn(He), {
                icon: gn(Nh),
                "aria-hidden": "true",
                class: "icon rejected"
              }, null, 8, ["icon"])
            ])) : Wd("", !0)
          ]),
          Ml(Uh, {
            alarms: (k = r.situationInfo) == null ? void 0 : k.alarms,
            size: "normal"
          }, null, 8, ["alarms"])
        ])
      ], 2);
    };
  }
});
const dN = /* @__PURE__ */ Qe(uN, [["__scopeId", "data-v-e39a68f4"]]);
const na = window.Vue.ref, fN = window.Vue.inject, hN = window.Vue.computed, pN = window.Vue.onMounted, mN = {
  id: {
    type: String
  },
  controls: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, VN = (n) => {
  const a = na(!1), r = na(), l = na(n.controls), s = na(n.id), f = () => {
    r.value && r.value.focus();
  }, h = fN("registerTab");
  pN(() => {
    if (r.value && h) {
      const T = r.value.parentElement, N = T && T.parentElement ? T.parentElement : void 0, g = Array.from(N ? N.children : []).filter((b) => b.querySelectorAll("[role=tab]").length), k = T ? g.indexOf(T) : -1;
      h({
        el: r.value,
        focus: f,
        disabled: n.disabled,
        selected: a,
        id: s,
        controls: l,
        index: k
      });
    }
  });
  const p = hN(() => ({
    role: "tab",
    ref: "tab",
    tabindex: a.value ? 0 : -1,
    id: s.value,
    "aria-selected": a.value,
    "aria-controls": l.value,
    "aria-disabled": n.disabled,
    "data-ref-id": "feather-tab"
  }));
  return {
    selected: a,
    attrs: p,
    tab: r
  };
};
const Ra = function(n, a) {
  return window ? window.setTimeout(n, a) : setTimeout(n, a);
}, Na = function(n) {
  return window ? window.clearTimeout(n) : clearTimeout(n);
};
var TN = Object.defineProperty, vN = Object.defineProperties, wN = Object.getOwnPropertyDescriptors, Sd = Object.getOwnPropertySymbols, UN = Object.prototype.hasOwnProperty, RN = Object.prototype.propertyIsEnumerable, Ad = (n, a, r) => a in n ? TN(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, xd = (n, a) => {
  for (var r in a || (a = {}))
    UN.call(a, r) && Ad(n, r, a[r]);
  if (Sd)
    for (var r of Sd(a))
      RN.call(a, r) && Ad(n, r, a[r]);
  return n;
}, NN = (n, a) => vN(n, wN(a));
const gN = window.Vue.defineComponent, MN = window.Vue.h;
var ZN = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const JN = {
  center: {
    type: Boolean,
    default: !1
  }
}, kN = gN({
  props: JN,
  data() {
    return {
      pressed: !1,
      active: !1,
      styles: {},
      failsafe: -1
    };
  },
  computed: {
    parent() {
      return this.$el.parentNode;
    }
  },
  methods: {
    onClick(n) {
      this.pressed = !1, this.active = !1, requestAnimationFrame(() => {
        const { clientWidth: a, clientHeight: r } = this.parent, l = Math.round(Math.max(a, r));
        let s = {
          top: "0px",
          left: "0px"
        };
        if (!this.center) {
          const f = this.parent.getBoundingClientRect(), h = n.pageY, p = n.pageX;
          s = {
            top: `${h - f.top - l / 2 - document.documentElement.scrollTop}px`,
            left: `${p - f.left - l / 2 - document.documentElement.scrollLeft}px`
          };
        }
        this.styles = NN(xd({}, s), {
          height: `${l}px`,
          width: `${l}px`
        }), this.pressed = !0, requestAnimationFrame(() => {
          this.active = !0, Na(this.failsafe), this.failsafe = Ra(() => {
            this.pressed = !1, this.active = !1;
          }, 380 + 100 + 20);
        });
      });
    }
  },
  render() {
    if (this.pressed !== !1)
      return MN("div", {
        style: xd({}, this.styles),
        class: ["ripple", { active: this.active, center: this.center }],
        onTransitionEnd: () => {
          this.pressed = !1, this.active = !1;
        },
        onTransitionCancel: () => {
          this.pressed = !1, this.active = !1;
        }
      });
  },
  mounted() {
    this.parent.addEventListener("click", this.onClick);
    const n = window.getComputedStyle(this.parent);
    this.parent.style.overflow = "hidden", !(n.position === "relative" || n.position === "absolute" || n.position === "fixed") && (this.parent.style.position = "relative");
  },
  unmounted() {
    this.parent.removeEventListener("click", this.onClick);
  }
});
var Fn = /* @__PURE__ */ ZN(kN, [["__scopeId", "data-v-18e2a5db"]]);
const ve = function(n) {
  n = n || "feather";
  const a = Math.floor(Math.random() * 1e9);
  return [n.replace(/\s+/g, "-"), Date.now(), a].join("-");
}, ge = {
  PAGEUP: 33,
  PAGEDOWN: 34,
  UP: 38,
  DOWN: 40,
  LEFT: 37,
  RIGHT: 39,
  HOME: 36,
  END: 35,
  ENTER: 13,
  SPACE: 32,
  ESCAPE: 27,
  BACKSPACE: 8,
  DELETE: 46
}, Bd = window.Vue.ref, yN = window.Vue.toRef, bN = window.Vue.watch, Cd = window.Vue.provide, EN = {
  prop: "modelValue",
  event: "update:modelValue"
}, FN = {
  "update:modelValue": (n) => !0
}, WN = {
  modelValue: {
    type: Number,
    default: 0
  },
  vertical: {
    type: Boolean,
    default: !0
  }
}, SN = (n, a) => {
  const r = yN(n, "modelValue"), l = Bd(n.modelValue), s = Bd([]);
  bN(r, (F) => {
    T(F);
  });
  const f = (F) => {
    F.preventDefault(), s.value.some((C, D) => C.tab && C.tab.el.contains(F.target) ? (p(D), T(D), !0) : !1);
  }, h = (F) => {
    if (((L) => L.shiftKey || L.ctrlKey || L.metaKey || L.altKey)(F))
      return;
    const D = F.keyCode, A = (L) => {
      L.stopPropagation(), L.preventDefault();
    }, Q = s.value.filter((L) => L.tab && !L.tab.disabled), S = s.value.findIndex((L) => L.tab && L.tab.el.contains(document.activeElement));
    let H = S !== -1 ? S : l.value;
    const y = [ge.RIGHT], _ = [ge.LEFT], $ = [ge.ENTER, ge.SPACE];
    n.vertical && (y.push(ge.DOWN), _.push(ge.UP)), y.indexOf(D) > -1 ? (H++, H >= Q.length && (H = 0), A(F), p(s.value.indexOf(Q[H]))) : _.indexOf(D) > -1 && (H--, H < 0 && (H = Q.length - 1), A(F), p(s.value.indexOf(Q[H]))), $.indexOf(D) > -1 && T(H);
  }, p = (F) => {
    s.value.forEach(function(C, D) {
      F === D && C.tab && C.tab.focus();
    });
  }, T = (F) => {
    const C = s.value[F];
    !C || C.tab && C.tab.disabled || (s.value.forEach((D, A) => {
      D.tab && (D.tab.selected = F === A), D.panel && (D.panel.selected = F === A);
    }), l.value = F, a.emit("update:modelValue", F));
  };
  Cd("registerTab", (F) => {
    const C = F.index;
    C > -1 && (s.value[C] = { ...s.value[C], tab: F }, s.value = [...s.value], k());
  }), Cd("registerPanel", (F) => {
    const C = F.index;
    C > -1 && (s.value[C] = {
      ...s.value[C],
      panel: F
    }, s.value = [...s.value], k());
  });
  const k = () => {
    s.value.forEach(({ tab: F, panel: C }, D) => {
      if (C && F) {
        const A = F.id || ve("tab"), Q = F.controls || ve("panel");
        F.controls = Q, F.id = A, C.tab = A, C.id = Q;
      }
      D === l.value && (C && (C.selected = !0), F && (F.selected = !0));
    });
  };
  return {
    listeners: {
      click: f,
      keydown: h
    },
    attrs: {
      role: "tablist"
    },
    selected: l,
    pairs: s
  };
}, ra = window.Vue.ref, AN = window.Vue.inject, xN = window.Vue.computed, BN = window.Vue.onMounted, CN = {
  id: {
    type: String
  },
  tab: {
    type: String
  }
}, DN = (n) => {
  const a = ra(!1), r = ra(), l = ra(n.tab), s = ra(n.id), f = AN("registerPanel");
  BN(() => {
    if (f) {
      const p = r.value, T = p && p.parentElement ? p.parentElement : void 0, N = p ? Array.from(T ? T.children : []).indexOf(p) : -1;
      f({
        selected: a,
        id: s,
        tab: l,
        el: r.value,
        index: N
      });
    }
  });
  const h = xN(() => ({
    role: "tabpanel",
    id: s.value,
    ref: "panel",
    tabindex: "0",
    "aria-expanded": a.value,
    "aria-labelledby": l.value,
    "data-ref-id": "feather-tab-panel"
  }));
  return {
    selected: a,
    attrs: h,
    panel: r
  };
}, ds = window.Vue.defineComponent, QN = window.Vue.resolveComponent, fs = window.Vue.openBlock, hs = window.Vue.createElementBlock, _r = window.Vue.createElementVNode, gh = window.Vue.mergeProps, ga = window.Vue.renderSlot, ON = window.Vue.createVNode, zN = window.Vue.normalizeStyle, GN = window.Vue.toHandlers, YN = window.Vue.withDirectives, IN = window.Vue.normalizeProps, HN = window.Vue.guardReactiveProps, PN = window.Vue.vShow;
var ps = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const jN = mN, XN = ds({
  props: jN,
  setup(n) {
    return VN(n);
  },
  components: {
    FeatherRipple: Fn
  }
}), _N = { role: "presentation" }, LN = { class: "tab-text" };
function qN(n, a, r, l, s, f) {
  const h = QN("FeatherRipple");
  return fs(), hs("li", _N, [
    _r("button", gh(n.attrs, {
      class: ["tab hover focus", { disabled: n.disabled, selected: n.selected }]
    }), [
      _r("span", LN, [
        ga(n.$slots, "default", {}, void 0, !0)
      ]),
      ON(h)
    ], 16)
  ]);
}
var Dd = /* @__PURE__ */ ps(XN, [["render", qN], ["__scopeId", "data-v-e6bb52b6"]]);
const KN = WN, $N = FN, eg = ds({
  model: EN,
  emits: $N,
  props: KN,
  setup(n, a) {
    return SN(n, a);
  },
  data: () => ({
    transform: "",
    durationNumber: 250,
    width: "1px",
    ro: void 0
  }),
  watch: {
    selected() {
      this.updateSlider();
    },
    pairs: {
      handler(n) {
        n && n.length && this.ro && n.forEach((a) => {
          a.tab && this.ro.observe(a.tab.el);
        });
      },
      immediate: !0
    }
  },
  computed: {
    duration() {
      return `${this.durationNumber}ms`;
    }
  },
  methods: {
    updateSlider() {
      this.$refs.slider.getBoundingClientRect().width < 5 ? this.durationNumber = 0 : this.durationNumber = 250, this.$nextTick(() => {
        const n = this.$el.getBoundingClientRect(), a = this.$el.querySelector("[aria-selected='true']").getBoundingClientRect(), r = a.left - n.left, l = a.height - 2;
        this.width = `${a.width}px`, this.transform = `translateX(${r}px) translateY(${l}px)`;
      });
    }
  },
  mounted() {
    this.updateSlider(), this.ro = new ResizeObserver(() => {
      this.updateSlider();
    });
  },
  unmounted() {
    this.ro.disconnect();
  }
}), tg = { class: "feather-tab-container" }, ng = { class: "tab-panels" };
function rg(n, a, r, l, s, f) {
  return fs(), hs("div", tg, [
    _r("div", {
      ref: "slider",
      class: "feather-tab-slider",
      style: zN({
        transform: n.transform,
        "transition-duration": n.duration,
        width: n.width
      })
    }, null, 4),
    _r("ul", gh(n.attrs, GN(n.listeners)), [
      ga(n.$slots, "tabs", {}, void 0, !0)
    ], 16),
    _r("div", ng, [
      ga(n.$slots, "default", {}, void 0, !0)
    ])
  ]);
}
var ig = /* @__PURE__ */ ps(eg, [["render", rg], ["__scopeId", "data-v-27adffb9"]]);
const ag = CN, og = ds({
  props: ag,
  setup(n) {
    return DN(n);
  }
});
function lg(n, a, r, l, s, f) {
  return YN((fs(), hs("div", IN(HN(n.attrs)), [
    ga(n.$slots, "default")
  ], 16)), [
    [PN, n.selected]
  ]);
}
var Qd = /* @__PURE__ */ ps(og, [["render", lg]]);
const sg = window.Vue.defineComponent, cg = window.Vue.toDisplayString, ug = window.Vue.normalizeClass, dg = window.Vue.openBlock, fg = window.Vue.createElementBlock, hg = window.Vue.createCommentVNode, pg = /* @__PURE__ */ sg({
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const a = n;
    return (r, l) => a != null && a.severity ? (dg(), fg("span", {
      key: 0,
      class: ug(["severity-status", [`${a.severity.toLowerCase()}-color`]])
    }, cg(a.severity), 3)) : hg("", !0);
  }
});
const Mh = /* @__PURE__ */ Qe(pg, [["__scopeId", "data-v-83c2cdce"]]), mg = window.Vue.defineComponent, Od = window.Vue.toDisplayString, zd = window.Vue.createElementVNode, Vg = window.Vue.openBlock, Tg = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const vg = { class: "box" }, wg = { class: "label" }, Ug = { class: "date" }, Rg = /* @__PURE__ */ mg({
  props: {
    label: null,
    info: null
  },
  setup(n) {
    const a = n;
    return (r, l) => (Vg(), Tg("div", vg, [
      zd("div", wg, Od(a.label), 1),
      zd("div", Ug, Od(a.info), 1)
    ]));
  }
});
const kl = /* @__PURE__ */ Qe(Rg, [["__scopeId", "data-v-b4afa751"]]);
const Ng = window.Vue.computed, gg = (n, a) => {
  const r = {};
  return Object.keys(a).forEach((l) => {
    r[`${l}Label`] = Ng(() => n.value[l] ? n.value[l] : a[l]);
  }), r;
}, Mg = window.Vue.openBlock, Zg = window.Vue.createElementBlock, Jg = window.Vue.createElementVNode;
var kg = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const yg = {}, bg = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Eg = /* @__PURE__ */ Jg("path", { d: "M19,17.55,13.43,12,19,6.4A1,1,0,1,0,17.58,5L12,10.58,6.42,5A1,1,0,0,0,5,6.42L10.6,12,5,17.6A1,1,0,0,0,5,19a1,1,0,0,0,.71.29,1,1,0,0,0,.71-.3L12,13.41,17.6,19a1,1,0,0,0,.71.29A1,1,0,0,0,19,19,1,1,0,0,0,19,17.55Z" }, null, -1), Fg = [
  Eg
];
function Wg(n, a) {
  return Mg(), Zg("svg", bg, Fg);
}
var Zh = /* @__PURE__ */ kg(yg, [["render", Wg]]);
const Gd = window.Vue.computed, Sg = (n, a, r) => {
  const l = Gd(() => a.value.filter((f) => !f.disabled)), s = Gd(() => n.value ? l.value.indexOf(n.value) : -1);
  return {
    selectPrevious() {
      n.value && n.value.disabled || (s.value === 0 ? r(l.value[l.value.length - 1]) : r(l.value[s.value - 1]));
    },
    selectNext() {
      n.value && n.value.disabled || (s.value === l.value.length - 1 ? r(l.value[0]) : r(l.value[s.value + 1]));
    }
  };
};
const Ag = window.Vue.openBlock, xg = window.Vue.createElementBlock, ms = window.Vue.createElementVNode;
var Bg = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const Cg = {}, Dg = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Qg = /* @__PURE__ */ ms("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" }, null, -1), Og = /* @__PURE__ */ ms("rect", {
  x: "11",
  y: "7",
  width: "2",
  height: "7",
  rx: "1"
}, null, -1), zg = /* @__PURE__ */ ms("rect", {
  x: "11",
  y: "15",
  width: "2",
  height: "2",
  rx: "0.65"
}, null, -1), Gg = [
  Qg,
  Og,
  zg
];
function Yg(n, a) {
  return Ag(), xg("svg", Dg, Gg);
}
var Ig = /* @__PURE__ */ Bg(Cg, [["render", Yg]]), Hg = Object.defineProperty, Pg = Object.defineProperties, jg = Object.getOwnPropertyDescriptors, Yd = Object.getOwnPropertySymbols, Xg = Object.prototype.hasOwnProperty, _g = Object.prototype.propertyIsEnumerable, Id = (n, a, r) => a in n ? Hg(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, Jh = (n, a) => {
  for (var r in a || (a = {}))
    Xg.call(a, r) && Id(n, r, a[r]);
  if (Yd)
    for (var r of Yd(a))
      _g.call(a, r) && Id(n, r, a[r]);
  return n;
}, kh = (n, a) => Pg(n, jg(a));
const mr = window.Vue.defineComponent, ei = window.Vue.resolveComponent, bt = window.Vue.openBlock, lr = window.Vue.createElementBlock, Lg = window.Vue.createVNode, Ma = window.Vue.createBlock, qg = window.Vue.withModifiers, fr = window.Vue.inject, ti = window.Vue.computed, Kg = window.Vue.normalizeClass, $n = window.Vue.createElementVNode, Za = window.Vue.toDisplayString, ma = window.Vue.renderSlot, Lr = window.Vue.createCommentVNode, $g = window.Vue.withDirectives, eM = window.Vue.vShow, Hl = window.Vue.ref, Hd = window.Vue.toRef, Pd = window.Vue.nextTick, Pl = window.Vue.watch;
window.Vue.createTextVNode;
window.Vue.Fragment;
window.Vue.renderList;
const Vs = window.Vue.provide, jd = window.Vue.isRef, tM = window.Vue.onBeforeUnmount;
var ii = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const nM = {
  title: {
    type: String,
    default: ""
  },
  icon: {
    type: Object,
    required: !0
  }
}, rM = mr({
  props: nM,
  components: {
    FeatherIcon: He
  }
}), iM = ["title"];
function aM(n, a, r, l, s, f) {
  const h = ei("FeatherIcon");
  return bt(), lr("a", {
    title: n.title,
    class: "action-icon hide-when-disabled",
    href: "#"
  }, [
    Lg(h, { icon: n.icon }, null, 8, ["icon"])
  ], 8, iM);
}
var oM = /* @__PURE__ */ ii(rM, [["render", aM], ["__scopeId", "data-v-4265058e"]]);
const lM = mr({
  emits: ["clear"],
  props: {
    clear: {
      type: String,
      default: ""
    }
  },
  computed: {
    clearIcon() {
      return Zh;
    }
  },
  components: {
    ActionIcon: oM
  }
});
function sM(n, a, r, l, s, f) {
  const h = ei("ActionIcon");
  return bt(), Ma(h, {
    onClick: a[0] || (a[0] = qg((p) => n.$emit("clear"), ["stop", "prevent"])),
    title: n.clear,
    icon: n.clearIcon,
    "data-ref-id": "feather-form-element-clear"
  }, null, 8, ["title", "icon"]);
}
var cM = /* @__PURE__ */ ii(lM, [["render", sM]]);
const uM = mr({
  computed: {
    errorIcon() {
      return Ig;
    }
  },
  components: {
    FeatherIcon: He
  }
});
function dM(n, a, r, l, s, f) {
  const h = ei("FeatherIcon");
  return bt(), Ma(h, {
    icon: n.errorIcon,
    class: "error-icon hide-when-disabled",
    "data-ref-id": "error-icon"
  }, null, 8, ["icon"]);
}
var fM = /* @__PURE__ */ ii(uM, [["render", dM], ["__scopeId", "data-v-0b8faef3"]]);
const hM = {
  for: {
    type: String,
    required: !0
  },
  focused: {
    type: Boolean,
    default: !1
  },
  showClear: {
    type: Boolean,
    default: !1
  },
  clearText: {
    type: String
  },
  raised: {
    type: Boolean,
    default: !1
  }
}, pM = {
  clear: () => !0,
  "wrapper-click": (n) => !0
}, mM = mr({
  emits: pM,
  props: hM,
  data() {
    return {
      prefixWidth: 0,
      prefixObserver: void 0
    };
  },
  setup() {
    const n = fr("wrapperOptions", {}), a = fr("validationErrorMessage", !1), r = ti(() => n.error ? n.error : a && a.value ? a.value : !1);
    return kh(Jh({}, n), { error: r });
  },
  computed: {
    computedClearText() {
      return this.clearText ? this.clearText : this.clear ? this.clear : "";
    },
    inputId() {
      return this.for;
    },
    hasPre() {
      return this.$slots.pre && this.$slots.pre().findIndex((a) => a.children && a.children.length !== 0) !== -1;
    },
    containerCls() {
      const n = [];
      return this.hideLabel && n.push("hide-label"), this.raised && n.push("raised"), this.focused && n.push("focused"), this.error && n.push("error"), this.background && n.push("background"), this.disabled && n.push("disabled"), this.inline && n.push("inline"), this.hasPre && n.push("has-prefix"), n;
    }
  },
  methods: {
    handleWrapperClick(n) {
      this.disabled || this.$emit("wrapper-click", n);
    }
  },
  mounted() {
    const n = this.$el.querySelector(".prefix");
    if (n) {
      const a = { childList: !0, subtree: !0 }, r = () => {
        const l = this.$el.querySelector(".prefix");
        this.prefixWidth = l ? l.offsetWidth : 0;
      };
      this.prefixObserver = new MutationObserver(r), this.prefixObserver.observe(n, a), r();
    }
  },
  unmounted() {
    this.prefixObserver && this.prefixObserver.disconnect();
  },
  components: {
    ClearIcon: cM,
    ErrorIcon: fM
  }
}), VM = {
  "aria-hidden": "true",
  class: "feather-input-border"
}, TM = ["for"], vM = { class: "prefix" }, wM = { class: "post" };
function UM(n, a, r, l, s, f) {
  const h = ei("ClearIcon"), p = ei("ErrorIcon");
  return bt(), lr("div", {
    class: Kg(["feather-input-wrapper-container", n.containerCls])
  }, [
    $n("fieldset", VM, [
      $n("legend", null, Za(n.label), 1)
    ]),
    $n("label", {
      class: "feather-input-label",
      for: n.inputId,
      "data-ref-id": "feather-form-element-label"
    }, Za(n.label), 9, TM),
    $n("div", {
      class: "feather-input-wrapper",
      onClick: a[1] || (a[1] = (...T) => n.handleWrapperClick && n.handleWrapperClick(...T))
    }, [
      $n("div", vM, [
        ma(n.$slots, "pre", {}, void 0, !0)
      ]),
      ma(n.$slots, "default", {}, void 0, !0),
      $n("div", wM, [
        n.showClear && n.computedClearText ? (bt(), Ma(h, {
          key: 0,
          clear: n.computedClearText,
          onClear: a[0] || (a[0] = (T) => n.$emit("clear"))
        }, null, 8, ["clear"])) : Lr("", !0),
        n.error ? (bt(), Ma(p, { key: 1 })) : Lr("", !0),
        ma(n.$slots, "post", {}, void 0, !0)
      ])
    ])
  ], 2);
}
var RM = /* @__PURE__ */ ii(mM, [["render", UM], ["__scopeId", "data-v-4db296db"]]);
const NM = mr({
  setup() {
    const n = fr("subTextOptions", {}), a = fr("validationErrorMessage", !1), r = ti(() => n.error ? n.error : a && a.value ? a.value : "");
    return kh(Jh({}, n), { error: r });
  }
}), gM = { class: "feather-input-sub-text" }, MM = {
  key: 0,
  class: "feather-input-spacer"
}, ZM = {
  key: 1,
  class: "feather-input-hint",
  "data-ref-id": "feather-form-element-hint"
}, JM = {
  key: 2,
  class: "feather-input-error",
  "data-ref-id": "feather-form-element-error",
  "aria-live": "assertive"
};
function kM(n, a, r, l, s, f) {
  return $g((bt(), lr("div", gM, [
    !n.hint && !n.error.length ? (bt(), lr("div", MM, "\xA0")) : Lr("", !0),
    n.hint && !n.error.length ? (bt(), lr("div", ZM, Za(n.hint), 1)) : Lr("", !0),
    n.error.length > 0 ? (bt(), lr("div", JM, Za(n.error), 1)) : Lr("", !0),
    ma(n.$slots, "right", {}, void 0, !0)
  ], 512)), [
    [eM, !n.inline || n.hint || n.error.length]
  ]);
}
var Ts = /* @__PURE__ */ ii(NM, [["render", kM], ["__scopeId", "data-v-8e0ac99e"]]);
const yM = {
  headingText: {
    type: Function,
    default: (n) => n.length && n.length === 1 ? "1 error" : `${n.length} errors`
  },
  errorList: {
    type: Array,
    default: () => []
  },
  generalError: {
    type: String,
    default: ""
  }
};
mr({
  props: yM,
  setup(n) {
    const a = fr("featherFormErrors", Hl([])), r = Hd(n, "errorList"), l = ti(() => {
      var g;
      return (g = r.value) != null && g.length ? r.value : a.value;
    }), s = Hd(n, "generalError"), f = (g) => {
      document.getElementById(g).focus();
    }, h = (g) => g.replace(/ \*$/, ""), p = Hl(), T = (g) => `${h(g.label)} - ${g.message}`, N = ti(() => (l.value.length && Pd(() => p.value.focus()), n.headingText(l.value)));
    return Pl(s, (g) => {
      g.length && Pd(() => p.value.focus());
    }), {
      errors: l,
      errorsHeading: N,
      heading: p,
      focusElement: f,
      mainError: s,
      getFullMessage: T
    };
  }
});
const vs = (n, a, r, l, s) => {
  const f = fr("featherForm", !1);
  if (l && f && n.value) {
    const h = Hl("");
    Vs("validationErrorMessage", h);
    const p = () => {
      if (s && jd(s) && s.value)
        return h.value = s.value, {
          success: !1,
          message: s.value,
          inputId: n.value,
          label: r
        };
      try {
        return l.validateSync(a.value), h.value = "", { success: !0 };
      } catch (g) {
        const k = g;
        return h.value = k.errors[0], {
          success: !1,
          message: k.errors[0],
          inputId: n.value,
          label: r
        };
      }
    }, N = {
      clear: () => {
        h.value = "";
      },
      validate: p
    };
    return s && jd(s) && Pl(s, () => {
      f.runValidation();
    }), Pl(n, (g, k) => {
      g && f && f.register(g, N), k && f && f.deregister(k);
    }, { immediate: !0 }), tM(() => {
      f.deregister(n.value, !0);
    }), { validate: p };
  }
  return { validate: () => !0 };
}, ws = (n) => ({
  inherittedAttrs: ti(() => ({
    class: n.class,
    "data-ref-id": n["data-ref-id"]
  }))
}), Us = {
  hint: {
    type: String
  },
  error: {
    type: String
  },
  inline: {
    type: Boolean,
    default: !1
  }
}, Rs = (n) => {
  Vs("subTextOptions", n);
}, bM = {
  label: {
    type: String,
    required: !0
  },
  error: {
    type: String
  },
  clear: {
    type: String,
    default: ""
  },
  background: {
    type: Boolean,
    default: !1
  },
  disabled: {
    type: Boolean,
    default: !1
  },
  inline: {
    type: Boolean,
    default: !1
  },
  hideLabel: {
    type: Boolean,
    default: !1
  }
}, EM = (n) => {
  Vs("wrapperOptions", n);
}, ia = window.Vue.ref, FM = window.Vue.watch, WM = window.Vue.watchEffect, Xd = window.Vue.computed, yl = window.Vue.provide, yh = (n, a, r, l, s) => {
  const f = ia([]), h = ia(), p = ia(), T = ia();
  WM(() => {
    if (!f.value.length)
      return;
    const A = f.value.map((Q) => Q.value);
    if (n.value !== void 0 && n.value !== null && (h.value = f.value[A.indexOf(n.value)]), !h.value && f.value.length) {
      let Q = f.value.filter((S) => !S.disabled);
      Q = Q.length ? Q : f.value, p.value = Q[0], p.value.first = !0;
    }
  }), FM(h, (A, Q) => {
    Q && (Q.checked = !1), A && (A.checked = !0);
  });
  const N = (A) => {
    A && A.disabled || (p.value && (p.value.first = !1), h.value !== A && (a("update:modelValue", A.value), h.value = A, A.focus()));
  }, g = Xd(() => h.value || p.value), k = Sg(g, f, N), b = Xd(() => ve("feather-radio-group"));
  T.value = b.value;
  const { validate: B } = vs(T, n, r, l, s);
  return yl("register", (A) => {
    f.value = [...f.value, A], T.value === b.value && (T.value = A.id);
  }), yl("select", N), yl("blur", (A) => {
    a("blur", A);
  }), {
    keydown: (A) => {
      switch (A.keyCode) {
        case 13:
        case 32:
          h.value ? N(h.value) : p.value && N(p.value);
          break;
        case 40:
        case 39:
          k.selectNext();
          break;
        case 37:
        case 38:
          k.selectPrevious();
          break;
      }
    },
    ...k,
    focus: () => {
      h.value && h.value.focus();
    },
    validate: B,
    firstElementId: T,
    groupId: b
  };
};
var SM = Object.defineProperty, AM = Object.defineProperties, xM = Object.getOwnPropertyDescriptors, _d = Object.getOwnPropertySymbols, BM = Object.prototype.hasOwnProperty, CM = Object.prototype.propertyIsEnumerable, Ld = (n, a, r) => a in n ? SM(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, cr = (n, a) => {
  for (var r in a || (a = {}))
    BM.call(a, r) && Ld(n, r, a[r]);
  if (_d)
    for (var r of _d(a))
      CM.call(a, r) && Ld(n, r, a[r]);
  return n;
}, bh = (n, a) => AM(n, xM(a));
const fn = window.Vue.defineComponent, qr = window.Vue.inject, Ja = window.Vue.computed, Kr = window.Vue.ref, mt = window.Vue.resolveComponent, ot = window.Vue.openBlock, ai = window.Vue.createElementBlock, Eh = window.Vue.normalizeClass, Et = window.Vue.renderSlot, cn = window.Vue.createBlock, ni = window.Vue.createCommentVNode, ka = window.Vue.createElementVNode, DM = window.Vue.withModifiers, Ca = window.Vue.createVNode, Fh = window.Vue.toRef, jl = window.Vue.mergeProps, jt = window.Vue.withCtx, QM = window.Vue.h, OM = window.Vue.provide;
var hn = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const zM = {
  disabled: {
    type: Boolean,
    default: !1
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  clickable: {
    type: Boolean,
    default: !1
  }
}, GM = fn({
  props: zM,
  setup(n) {
    const a = qr("isCondensed", null), r = Ja(() => a || n.condensed), l = Kr(!1);
    return {
      focused: l,
      handleFocus: () => {
        l.value = !0;
      },
      handleBlur: () => {
        l.value = !1;
      },
      isCondensed: r
    };
  },
  components: {
    FeatherRipple: Fn
  }
}), YM = ["aria-disabled"];
function IM(n, a, r, l, s, f) {
  const h = mt("FeatherRipple");
  return ot(), ai("div", {
    class: Eh(["chip", {
      condensed: n.isCondensed,
      disabled: n.disabled,
      focused: n.focused
    }]),
    onFocusin: a[0] || (a[0] = (p) => n.clickable ? n.handleFocus : null),
    onFocusout: a[1] || (a[1] = (p) => n.clickable ? n.handleBlur : null),
    "aria-disabled": n.disabled
  }, [
    Et(n.$slots, "default", {}, void 0, !0),
    n.clickable ? (ot(), cn(h, { key: 0 })) : ni("", !0)
  ], 42, YM);
}
var Ns = /* @__PURE__ */ hn(GM, [["render", IM], ["__scopeId", "data-v-44d413dc"]]);
const HM = {
  disabled: {
    type: Boolean,
    default: !1
  },
  textId: {
    type: String,
    required: !0
  },
  label: {
    type: String,
    required: !0
  }
}, PM = fn({
  emits: ["delete"],
  props: HM,
  setup(n, a) {
    return {
      handleDelete: () => {
        n.disabled || a.emit("delete");
      },
      icon: Zh
    };
  },
  components: {
    FeatherIcon: He
  }
}), jM = { class: "chip-delete" }, XM = ["aria-label", "aria-describedby"];
function _M(n, a, r, l, s, f) {
  const h = mt("FeatherIcon");
  return ot(), ai("span", jM, [
    ka("a", {
      href: "#",
      class: "delete-icon",
      role: "button",
      onClick: a[0] || (a[0] = DM((...p) => n.handleDelete && n.handleDelete(...p), ["stop", "prevent"])),
      "aria-label": n.label,
      "aria-describedby": n.textId
    }, [
      Ca(h, {
        icon: n.icon,
        flex: ""
      }, null, 8, ["icon"])
    ], 8, XM)
  ]);
}
var LM = /* @__PURE__ */ hn(PM, [["render", _M], ["__scopeId", "data-v-4bae6cb4"]]);
const qM = fn({
  data() {
    return {
      titleText: void 0
    };
  },
  methods: {
    ellipsisActive() {
      return this.$refs.container.offsetWidth < this.$refs.container.scrollWidth;
    },
    updateTitle() {
      this.titleText = this.ellipsisActive() ? this.$refs.container.textContent : void 0;
    }
  },
  mounted() {
    this.updateTitle();
  },
  beforeUpdate() {
    this.updateTitle();
  }
}), KM = ["title"];
function $M(n, a, r, l, s, f) {
  return ot(), ai("span", {
    class: "label",
    title: n.titleText,
    ref: "container"
  }, [
    Et(n.$slots, "default", {}, void 0, !0)
  ], 8, KM);
}
var gs = /* @__PURE__ */ hn(qM, [["render", $M], ["__scopeId", "data-v-1a0445b2"]]);
const eZ = {}, tZ = {
  class: "chip-icon",
  role: "presentation"
};
function nZ(n, a) {
  return ot(), ai("span", tZ, [
    Et(n.$slots, "default", {}, void 0, !0)
  ]);
}
var Ms = /* @__PURE__ */ hn(eZ, [["render", nZ], ["__scopeId", "data-v-2230176f"]]);
const qd = {
  delete: "Remove"
}, rZ = fn({
  inheritAttrs: !1,
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    labels: {
      type: Object,
      default: () => qd
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n, a) {
    const r = gg(Fh(n, "labels"), qd), l = Ja(() => ve("chip-text")), s = () => {
      n.disabled || a.emit("click");
    }, f = cr({}, a.attrs);
    return n.disabled && delete f.onClick, bh(cr({}, r), {
      chipTextId: l,
      handleClick: s,
      attrs: f
    });
  },
  computed: {
    chipTextAttrs() {
      return this.canClick ? {
        role: "button",
        tabindex: "0"
      } : {};
    },
    canDelete() {
      return !!this.$attrs.onDelete && !this.disabled;
    },
    canClick() {
      return !!this.$attrs.onClick && !this.disabled;
    },
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((a) => a.children && a.children.length !== 0 || typeof a.type == "object") !== -1;
    }
  },
  components: {
    Chip: Ns,
    DeleteIcon: LM,
    Label: gs,
    PreIcon: Ms
  }
}), iZ = ["aria-disabled"];
function aZ(n, a, r, l, s, f) {
  const h = mt("PreIcon"), p = mt("Label"), T = mt("DeleteIcon"), N = mt("Chip");
  return ot(), cn(N, jl(n.attrs, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: { hover: n.canClick, focus: n.canClick || n.canDelete },
    role: "row",
    clickable: n.canClick
  }), {
    default: jt(() => [
      ka("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        ka("span", jl(n.chipTextAttrs, { class: "chip-label-button" }), [
          n.hasIcon ? (ot(), cn(h, { key: 0 }, {
            default: jt(() => [
              Et(n.$slots, "icon", {}, void 0, !0)
            ]),
            _: 3
          })) : ni("", !0),
          Ca(p, { id: n.chipTextId }, {
            default: jt(() => [
              Et(n.$slots, "default", {}, void 0, !0)
            ]),
            _: 3
          }, 8, ["id"])
        ], 16)
      ], 8, iZ),
      n.canDelete ? (ot(), cn(T, {
        key: 0,
        disabled: n.disabled,
        "text-id": n.chipTextId,
        label: n.deleteLabel,
        role: "gridcell",
        onDelete: a[0] || (a[0] = (g) => n.$emit("delete"))
      }, null, 8, ["disabled", "text-id", "label"])) : ni("", !0)
    ]),
    _: 3
  }, 16, ["disabled", "condensed", "class", "clickable"]);
}
var oZ = /* @__PURE__ */ hn(rZ, [["render", aZ], ["__scopeId", "data-v-48b2704a"]]);
const lZ = fn({
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((a) => a.children && a.children.length !== 0 || typeof a.type == "object") !== -1;
    }
  },
  components: {
    Chip: Ns,
    Label: gs,
    PreIcon: Ms
  }
}), sZ = ["aria-disabled"];
function cZ(n, a, r, l, s, f) {
  const h = mt("PreIcon"), p = mt("Label"), T = mt("Chip");
  return ot(), cn(T, {
    role: "row",
    disabled: n.disabled,
    condensed: n.condensed,
    clickable: !1
  }, {
    default: jt(() => [
      ka("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        n.hasIcon ? (ot(), cn(h, { key: 0 }, {
          default: jt(() => [
            Et(n.$slots, "icon", {}, void 0, !0)
          ]),
          _: 3
        })) : ni("", !0),
        Ca(p, null, {
          default: jt(() => [
            Et(n.$slots, "default", {}, void 0, !0)
          ]),
          _: 3
        })
      ], 8, sZ)
    ]),
    _: 3
  }, 8, ["disabled", "condensed"]);
}
var uZ = /* @__PURE__ */ hn(lZ, [["render", cZ], ["__scopeId", "data-v-3e0c4eba"]]);
const dZ = fn({
  props: {
    value: {
      type: [String, Object, Number, Array],
      required: !0
    },
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n) {
    const a = Kr(!1), r = Kr(!1), l = Ja(() => ve("chip-label-id")), s = Ja(() => a.value || r.value ? 0 : -1), f = Kr(), h = () => {
      f.value.$el.focus();
    }, p = qr("register", (b) => {
    }), T = qr("blur", (b) => {
    }), N = qr("select", (b) => {
    }), g = {
      first: a,
      focus: h,
      disabled: n.disabled,
      value: n.value,
      checked: r
    };
    return p(g), {
      labelId: l,
      tabindex: s,
      first: a,
      blur: T,
      click: () => {
        N(g);
      },
      input: f,
      checked: r
    };
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((a) => a.children && a.children.length !== 0 || typeof a.type == "object") !== -1;
    }
  },
  components: {
    Chip: Ns,
    Label: gs,
    PreIcon: Ms
  }
});
function fZ(n, a, r, l, s, f) {
  const h = mt("PreIcon"), p = mt("Label"), T = mt("Chip");
  return ot(), cn(T, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: Eh(["focus hover", { selected: n.checked }]),
    role: "radio",
    ref: "input",
    "aria-checked": n.checked ? "true" : "false",
    "aria-disabled": n.disabled,
    "aria-labelledby": n.labelId,
    tabindex: n.tabindex,
    onClick: n.click,
    "allow-click": !n.disabled,
    onBlur: n.blur
  }, {
    default: jt(() => [
      n.hasIcon ? (ot(), cn(h, { key: 0 }, {
        default: jt(() => [
          Et(n.$slots, "icon", {}, void 0, !0)
        ]),
        _: 3
      })) : ni("", !0),
      Ca(p, { id: n.labelId }, {
        default: jt(() => [
          Et(n.$slots, "default", {}, void 0, !0)
        ]),
        _: 3
      }, 8, ["id"])
    ]),
    _: 3
  }, 8, ["disabled", "condensed", "class", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex", "onClick", "allow-click", "onBlur"]);
}
var hZ = /* @__PURE__ */ hn(dZ, [["render", fZ], ["__scopeId", "data-v-bbcc2f70"]]);
const pZ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  labels: {
    type: Object
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  value: {
    type: [String, Object, Number, Array, Boolean]
  }
}, Kd = fn({
  props: pZ,
  setup() {
    return { format: qr("chipListFormat", "") };
  },
  render() {
    const n = (a) => QM(a, cr(cr({}, this.$props), this.$attrs), cr({
      default: this.$slots.default
    }, this.$slots.icon && { icon: this.$slots.icon }));
    return this.format === "grid" ? n(oZ) : this.format === "radio" ? n(hZ) : n(uZ);
  }
}), mZ = {
  label: {
    type: String,
    required: !0
  },
  mode: {
    type: String,
    default: "list",
    validator: (n) => ["list", "radio", "single"].indexOf(n) !== -1
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  condensed: {
    type: Boolean,
    default: !1
  }
}, VZ = fn({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: mZ,
  setup(n, a) {
    const r = n.mode === "list" ? "grid" : n.mode;
    OM("chipListFormat", r);
    const l = r === "single";
    if (r === "radio") {
      const h = Fh(n, "modelValue");
      return bh(cr({
        attrs: {
          role: "radiogroup"
        }
      }, yh(h, a.emit, n.label, {}, Kr(""))), {
        single: l
      });
    }
    return { attrs: {
      role: "grid"
    }, keydown: () => {
    }, single: l };
  }
}), TZ = ["aria-label"];
function vZ(n, a, r, l, s, f) {
  return ot(), ai("div", jl(n.attrs, {
    "data-ref-id": "feather-chip-list",
    "aria-label": n.label,
    class: ["chip-list", { condensed: n.condensed, single: n.single }],
    onKeydown: a[0] || (a[0] = (...h) => n.keydown && n.keydown(...h))
  }), [
    Et(n.$slots, "default", {}, void 0, !0)
  ], 16, TZ);
}
var wZ = /* @__PURE__ */ hn(VZ, [["render", vZ], ["__scopeId", "data-v-1e06f41d"]]);
function Wn(n) {
  if (n === null || n === !0 || n === !1)
    return NaN;
  var a = Number(n);
  return isNaN(a) ? a : a < 0 ? Math.ceil(a) : Math.floor(a);
}
function Se(n, a) {
  if (a.length < n)
    throw new TypeError(n + " argument" + (n > 1 ? "s" : "") + " required, but only " + a.length + " present");
}
function De(n) {
  Se(1, arguments);
  var a = Object.prototype.toString.call(n);
  return n instanceof Date || typeof n == "object" && a === "[object Date]" ? new Date(n.getTime()) : typeof n == "number" || a === "[object Number]" ? new Date(n) : ((typeof n == "string" || a === "[object String]") && typeof console < "u" && (console.warn("Starting with v2.0.0-beta.1 date-fns doesn't accept strings as date arguments. Please use `parseISO` to parse strings. See: https://github.com/date-fns/date-fns/blob/master/docs/upgradeGuide.md#string-arguments"), console.warn(new Error().stack)), new Date(NaN));
}
function UZ(n, a) {
  Se(2, arguments);
  var r = De(n).getTime(), l = Wn(a);
  return new Date(r + l);
}
var RZ = {};
function oi() {
  return RZ;
}
function Xl(n) {
  var a = new Date(Date.UTC(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds(), n.getMilliseconds()));
  return a.setUTCFullYear(n.getFullYear()), n.getTime() - a.getTime();
}
function NZ(n, a) {
  Se(2, arguments);
  var r = De(n), l = De(a), s = r.getTime() - l.getTime();
  return s < 0 ? -1 : s > 0 ? 1 : s;
}
function gZ(n) {
  return Se(1, arguments), n instanceof Date || typeof n == "object" && Object.prototype.toString.call(n) === "[object Date]";
}
function MZ(n) {
  if (Se(1, arguments), !gZ(n) && typeof n != "number")
    return !1;
  var a = De(n);
  return !isNaN(Number(a));
}
function ZZ(n, a) {
  Se(2, arguments);
  var r = Wn(a);
  return UZ(n, -r);
}
var JZ = 864e5;
function kZ(n) {
  Se(1, arguments);
  var a = De(n), r = a.getTime();
  a.setUTCMonth(0, 1), a.setUTCHours(0, 0, 0, 0);
  var l = a.getTime(), s = r - l;
  return Math.floor(s / JZ) + 1;
}
function ya(n) {
  Se(1, arguments);
  var a = 1, r = De(n), l = r.getUTCDay(), s = (l < a ? 7 : 0) + l - a;
  return r.setUTCDate(r.getUTCDate() - s), r.setUTCHours(0, 0, 0, 0), r;
}
function Wh(n) {
  Se(1, arguments);
  var a = De(n), r = a.getUTCFullYear(), l = new Date(0);
  l.setUTCFullYear(r + 1, 0, 4), l.setUTCHours(0, 0, 0, 0);
  var s = ya(l), f = new Date(0);
  f.setUTCFullYear(r, 0, 4), f.setUTCHours(0, 0, 0, 0);
  var h = ya(f);
  return a.getTime() >= s.getTime() ? r + 1 : a.getTime() >= h.getTime() ? r : r - 1;
}
function yZ(n) {
  Se(1, arguments);
  var a = Wh(n), r = new Date(0);
  r.setUTCFullYear(a, 0, 4), r.setUTCHours(0, 0, 0, 0);
  var l = ya(r);
  return l;
}
var bZ = 6048e5;
function EZ(n) {
  Se(1, arguments);
  var a = De(n), r = ya(a).getTime() - yZ(a).getTime();
  return Math.round(r / bZ) + 1;
}
function ba(n, a) {
  var r, l, s, f, h, p, T, N;
  Se(1, arguments);
  var g = oi(), k = Wn((r = (l = (s = (f = a == null ? void 0 : a.weekStartsOn) !== null && f !== void 0 ? f : a == null || (h = a.locale) === null || h === void 0 || (p = h.options) === null || p === void 0 ? void 0 : p.weekStartsOn) !== null && s !== void 0 ? s : g.weekStartsOn) !== null && l !== void 0 ? l : (T = g.locale) === null || T === void 0 || (N = T.options) === null || N === void 0 ? void 0 : N.weekStartsOn) !== null && r !== void 0 ? r : 0);
  if (!(k >= 0 && k <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  var b = De(n), B = b.getUTCDay(), F = (B < k ? 7 : 0) + B - k;
  return b.setUTCDate(b.getUTCDate() - F), b.setUTCHours(0, 0, 0, 0), b;
}
function Sh(n, a) {
  var r, l, s, f, h, p, T, N;
  Se(1, arguments);
  var g = De(n), k = g.getUTCFullYear(), b = oi(), B = Wn((r = (l = (s = (f = a == null ? void 0 : a.firstWeekContainsDate) !== null && f !== void 0 ? f : a == null || (h = a.locale) === null || h === void 0 || (p = h.options) === null || p === void 0 ? void 0 : p.firstWeekContainsDate) !== null && s !== void 0 ? s : b.firstWeekContainsDate) !== null && l !== void 0 ? l : (T = b.locale) === null || T === void 0 || (N = T.options) === null || N === void 0 ? void 0 : N.firstWeekContainsDate) !== null && r !== void 0 ? r : 1);
  if (!(B >= 1 && B <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var F = new Date(0);
  F.setUTCFullYear(k + 1, 0, B), F.setUTCHours(0, 0, 0, 0);
  var C = ba(F, a), D = new Date(0);
  D.setUTCFullYear(k, 0, B), D.setUTCHours(0, 0, 0, 0);
  var A = ba(D, a);
  return g.getTime() >= C.getTime() ? k + 1 : g.getTime() >= A.getTime() ? k : k - 1;
}
function FZ(n, a) {
  var r, l, s, f, h, p, T, N;
  Se(1, arguments);
  var g = oi(), k = Wn((r = (l = (s = (f = a == null ? void 0 : a.firstWeekContainsDate) !== null && f !== void 0 ? f : a == null || (h = a.locale) === null || h === void 0 || (p = h.options) === null || p === void 0 ? void 0 : p.firstWeekContainsDate) !== null && s !== void 0 ? s : g.firstWeekContainsDate) !== null && l !== void 0 ? l : (T = g.locale) === null || T === void 0 || (N = T.options) === null || N === void 0 ? void 0 : N.firstWeekContainsDate) !== null && r !== void 0 ? r : 1), b = Sh(n, a), B = new Date(0);
  B.setUTCFullYear(b, 0, k), B.setUTCHours(0, 0, 0, 0);
  var F = ba(B, a);
  return F;
}
var WZ = 6048e5;
function SZ(n, a) {
  Se(1, arguments);
  var r = De(n), l = ba(r, a).getTime() - FZ(r, a).getTime();
  return Math.round(l / WZ) + 1;
}
function ie(n, a) {
  for (var r = n < 0 ? "-" : "", l = Math.abs(n).toString(); l.length < a; )
    l = "0" + l;
  return r + l;
}
var AZ = {
  y: function(n, a) {
    var r = n.getUTCFullYear(), l = r > 0 ? r : 1 - r;
    return ie(a === "yy" ? l % 100 : l, a.length);
  },
  M: function(n, a) {
    var r = n.getUTCMonth();
    return a === "M" ? String(r + 1) : ie(r + 1, 2);
  },
  d: function(n, a) {
    return ie(n.getUTCDate(), a.length);
  },
  a: function(n, a) {
    var r = n.getUTCHours() / 12 >= 1 ? "pm" : "am";
    switch (a) {
      case "a":
      case "aa":
        return r.toUpperCase();
      case "aaa":
        return r;
      case "aaaaa":
        return r[0];
      case "aaaa":
      default:
        return r === "am" ? "a.m." : "p.m.";
    }
  },
  h: function(n, a) {
    return ie(n.getUTCHours() % 12 || 12, a.length);
  },
  H: function(n, a) {
    return ie(n.getUTCHours(), a.length);
  },
  m: function(n, a) {
    return ie(n.getUTCMinutes(), a.length);
  },
  s: function(n, a) {
    return ie(n.getUTCSeconds(), a.length);
  },
  S: function(n, a) {
    var r = a.length, l = n.getUTCMilliseconds(), s = Math.floor(l * Math.pow(10, r - 3));
    return ie(s, a.length);
  }
};
const rn = AZ;
var er = {
  am: "am",
  pm: "pm",
  midnight: "midnight",
  noon: "noon",
  morning: "morning",
  afternoon: "afternoon",
  evening: "evening",
  night: "night"
}, xZ = {
  G: function(n, a, r) {
    var l = n.getUTCFullYear() > 0 ? 1 : 0;
    switch (a) {
      case "G":
      case "GG":
      case "GGG":
        return r.era(l, {
          width: "abbreviated"
        });
      case "GGGGG":
        return r.era(l, {
          width: "narrow"
        });
      case "GGGG":
      default:
        return r.era(l, {
          width: "wide"
        });
    }
  },
  y: function(n, a, r) {
    if (a === "yo") {
      var l = n.getUTCFullYear(), s = l > 0 ? l : 1 - l;
      return r.ordinalNumber(s, {
        unit: "year"
      });
    }
    return rn.y(n, a);
  },
  Y: function(n, a, r, l) {
    var s = Sh(n, l), f = s > 0 ? s : 1 - s;
    if (a === "YY") {
      var h = f % 100;
      return ie(h, 2);
    }
    return a === "Yo" ? r.ordinalNumber(f, {
      unit: "year"
    }) : ie(f, a.length);
  },
  R: function(n, a) {
    var r = Wh(n);
    return ie(r, a.length);
  },
  u: function(n, a) {
    var r = n.getUTCFullYear();
    return ie(r, a.length);
  },
  Q: function(n, a, r) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (a) {
      case "Q":
        return String(l);
      case "QQ":
        return ie(l, 2);
      case "Qo":
        return r.ordinalNumber(l, {
          unit: "quarter"
        });
      case "QQQ":
        return r.quarter(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "QQQQQ":
        return r.quarter(l, {
          width: "narrow",
          context: "formatting"
        });
      case "QQQQ":
      default:
        return r.quarter(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  q: function(n, a, r) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (a) {
      case "q":
        return String(l);
      case "qq":
        return ie(l, 2);
      case "qo":
        return r.ordinalNumber(l, {
          unit: "quarter"
        });
      case "qqq":
        return r.quarter(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "qqqqq":
        return r.quarter(l, {
          width: "narrow",
          context: "standalone"
        });
      case "qqqq":
      default:
        return r.quarter(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  M: function(n, a, r) {
    var l = n.getUTCMonth();
    switch (a) {
      case "M":
      case "MM":
        return rn.M(n, a);
      case "Mo":
        return r.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "MMM":
        return r.month(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "MMMMM":
        return r.month(l, {
          width: "narrow",
          context: "formatting"
        });
      case "MMMM":
      default:
        return r.month(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  L: function(n, a, r) {
    var l = n.getUTCMonth();
    switch (a) {
      case "L":
        return String(l + 1);
      case "LL":
        return ie(l + 1, 2);
      case "Lo":
        return r.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "LLL":
        return r.month(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "LLLLL":
        return r.month(l, {
          width: "narrow",
          context: "standalone"
        });
      case "LLLL":
      default:
        return r.month(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  w: function(n, a, r, l) {
    var s = SZ(n, l);
    return a === "wo" ? r.ordinalNumber(s, {
      unit: "week"
    }) : ie(s, a.length);
  },
  I: function(n, a, r) {
    var l = EZ(n);
    return a === "Io" ? r.ordinalNumber(l, {
      unit: "week"
    }) : ie(l, a.length);
  },
  d: function(n, a, r) {
    return a === "do" ? r.ordinalNumber(n.getUTCDate(), {
      unit: "date"
    }) : rn.d(n, a);
  },
  D: function(n, a, r) {
    var l = kZ(n);
    return a === "Do" ? r.ordinalNumber(l, {
      unit: "dayOfYear"
    }) : ie(l, a.length);
  },
  E: function(n, a, r) {
    var l = n.getUTCDay();
    switch (a) {
      case "E":
      case "EE":
      case "EEE":
        return r.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "EEEEE":
        return r.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "EEEEEE":
        return r.day(l, {
          width: "short",
          context: "formatting"
        });
      case "EEEE":
      default:
        return r.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  e: function(n, a, r, l) {
    var s = n.getUTCDay(), f = (s - l.weekStartsOn + 8) % 7 || 7;
    switch (a) {
      case "e":
        return String(f);
      case "ee":
        return ie(f, 2);
      case "eo":
        return r.ordinalNumber(f, {
          unit: "day"
        });
      case "eee":
        return r.day(s, {
          width: "abbreviated",
          context: "formatting"
        });
      case "eeeee":
        return r.day(s, {
          width: "narrow",
          context: "formatting"
        });
      case "eeeeee":
        return r.day(s, {
          width: "short",
          context: "formatting"
        });
      case "eeee":
      default:
        return r.day(s, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  c: function(n, a, r, l) {
    var s = n.getUTCDay(), f = (s - l.weekStartsOn + 8) % 7 || 7;
    switch (a) {
      case "c":
        return String(f);
      case "cc":
        return ie(f, a.length);
      case "co":
        return r.ordinalNumber(f, {
          unit: "day"
        });
      case "ccc":
        return r.day(s, {
          width: "abbreviated",
          context: "standalone"
        });
      case "ccccc":
        return r.day(s, {
          width: "narrow",
          context: "standalone"
        });
      case "cccccc":
        return r.day(s, {
          width: "short",
          context: "standalone"
        });
      case "cccc":
      default:
        return r.day(s, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  i: function(n, a, r) {
    var l = n.getUTCDay(), s = l === 0 ? 7 : l;
    switch (a) {
      case "i":
        return String(s);
      case "ii":
        return ie(s, a.length);
      case "io":
        return r.ordinalNumber(s, {
          unit: "day"
        });
      case "iii":
        return r.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "iiiii":
        return r.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "iiiiii":
        return r.day(l, {
          width: "short",
          context: "formatting"
        });
      case "iiii":
      default:
        return r.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  a: function(n, a, r) {
    var l = n.getUTCHours(), s = l / 12 >= 1 ? "pm" : "am";
    switch (a) {
      case "a":
      case "aa":
        return r.dayPeriod(s, {
          width: "abbreviated",
          context: "formatting"
        });
      case "aaa":
        return r.dayPeriod(s, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "aaaaa":
        return r.dayPeriod(s, {
          width: "narrow",
          context: "formatting"
        });
      case "aaaa":
      default:
        return r.dayPeriod(s, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  b: function(n, a, r) {
    var l = n.getUTCHours(), s;
    switch (l === 12 ? s = er.noon : l === 0 ? s = er.midnight : s = l / 12 >= 1 ? "pm" : "am", a) {
      case "b":
      case "bb":
        return r.dayPeriod(s, {
          width: "abbreviated",
          context: "formatting"
        });
      case "bbb":
        return r.dayPeriod(s, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "bbbbb":
        return r.dayPeriod(s, {
          width: "narrow",
          context: "formatting"
        });
      case "bbbb":
      default:
        return r.dayPeriod(s, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  B: function(n, a, r) {
    var l = n.getUTCHours(), s;
    switch (l >= 17 ? s = er.evening : l >= 12 ? s = er.afternoon : l >= 4 ? s = er.morning : s = er.night, a) {
      case "B":
      case "BB":
      case "BBB":
        return r.dayPeriod(s, {
          width: "abbreviated",
          context: "formatting"
        });
      case "BBBBB":
        return r.dayPeriod(s, {
          width: "narrow",
          context: "formatting"
        });
      case "BBBB":
      default:
        return r.dayPeriod(s, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  h: function(n, a, r) {
    if (a === "ho") {
      var l = n.getUTCHours() % 12;
      return l === 0 && (l = 12), r.ordinalNumber(l, {
        unit: "hour"
      });
    }
    return rn.h(n, a);
  },
  H: function(n, a, r) {
    return a === "Ho" ? r.ordinalNumber(n.getUTCHours(), {
      unit: "hour"
    }) : rn.H(n, a);
  },
  K: function(n, a, r) {
    var l = n.getUTCHours() % 12;
    return a === "Ko" ? r.ordinalNumber(l, {
      unit: "hour"
    }) : ie(l, a.length);
  },
  k: function(n, a, r) {
    var l = n.getUTCHours();
    return l === 0 && (l = 24), a === "ko" ? r.ordinalNumber(l, {
      unit: "hour"
    }) : ie(l, a.length);
  },
  m: function(n, a, r) {
    return a === "mo" ? r.ordinalNumber(n.getUTCMinutes(), {
      unit: "minute"
    }) : rn.m(n, a);
  },
  s: function(n, a, r) {
    return a === "so" ? r.ordinalNumber(n.getUTCSeconds(), {
      unit: "second"
    }) : rn.s(n, a);
  },
  S: function(n, a) {
    return rn.S(n, a);
  },
  X: function(n, a, r, l) {
    var s = l._originalDate || n, f = s.getTimezoneOffset();
    if (f === 0)
      return "Z";
    switch (a) {
      case "X":
        return ef(f);
      case "XXXX":
      case "XX":
        return kn(f);
      case "XXXXX":
      case "XXX":
      default:
        return kn(f, ":");
    }
  },
  x: function(n, a, r, l) {
    var s = l._originalDate || n, f = s.getTimezoneOffset();
    switch (a) {
      case "x":
        return ef(f);
      case "xxxx":
      case "xx":
        return kn(f);
      case "xxxxx":
      case "xxx":
      default:
        return kn(f, ":");
    }
  },
  O: function(n, a, r, l) {
    var s = l._originalDate || n, f = s.getTimezoneOffset();
    switch (a) {
      case "O":
      case "OO":
      case "OOO":
        return "GMT" + $d(f, ":");
      case "OOOO":
      default:
        return "GMT" + kn(f, ":");
    }
  },
  z: function(n, a, r, l) {
    var s = l._originalDate || n, f = s.getTimezoneOffset();
    switch (a) {
      case "z":
      case "zz":
      case "zzz":
        return "GMT" + $d(f, ":");
      case "zzzz":
      default:
        return "GMT" + kn(f, ":");
    }
  },
  t: function(n, a, r, l) {
    var s = l._originalDate || n, f = Math.floor(s.getTime() / 1e3);
    return ie(f, a.length);
  },
  T: function(n, a, r, l) {
    var s = l._originalDate || n, f = s.getTime();
    return ie(f, a.length);
  }
};
function $d(n, a) {
  var r = n > 0 ? "-" : "+", l = Math.abs(n), s = Math.floor(l / 60), f = l % 60;
  if (f === 0)
    return r + String(s);
  var h = a || "";
  return r + String(s) + h + ie(f, 2);
}
function ef(n, a) {
  if (n % 60 === 0) {
    var r = n > 0 ? "-" : "+";
    return r + ie(Math.abs(n) / 60, 2);
  }
  return kn(n, a);
}
function kn(n, a) {
  var r = a || "", l = n > 0 ? "-" : "+", s = Math.abs(n), f = ie(Math.floor(s / 60), 2), h = ie(s % 60, 2);
  return l + f + r + h;
}
const BZ = xZ;
var tf = function(n, a) {
  switch (n) {
    case "P":
      return a.date({
        width: "short"
      });
    case "PP":
      return a.date({
        width: "medium"
      });
    case "PPP":
      return a.date({
        width: "long"
      });
    case "PPPP":
    default:
      return a.date({
        width: "full"
      });
  }
}, Ah = function(n, a) {
  switch (n) {
    case "p":
      return a.time({
        width: "short"
      });
    case "pp":
      return a.time({
        width: "medium"
      });
    case "ppp":
      return a.time({
        width: "long"
      });
    case "pppp":
    default:
      return a.time({
        width: "full"
      });
  }
}, CZ = function(n, a) {
  var r = n.match(/(P+)(p+)?/) || [], l = r[1], s = r[2];
  if (!s)
    return tf(n, a);
  var f;
  switch (l) {
    case "P":
      f = a.dateTime({
        width: "short"
      });
      break;
    case "PP":
      f = a.dateTime({
        width: "medium"
      });
      break;
    case "PPP":
      f = a.dateTime({
        width: "long"
      });
      break;
    case "PPPP":
    default:
      f = a.dateTime({
        width: "full"
      });
      break;
  }
  return f.replace("{{date}}", tf(l, a)).replace("{{time}}", Ah(s, a));
}, DZ = {
  p: Ah,
  P: CZ
};
const QZ = DZ;
var OZ = ["D", "DD"], zZ = ["YY", "YYYY"];
function GZ(n) {
  return OZ.indexOf(n) !== -1;
}
function YZ(n) {
  return zZ.indexOf(n) !== -1;
}
function nf(n, a, r) {
  if (n === "YYYY")
    throw new RangeError("Use `yyyy` instead of `YYYY` (in `".concat(a, "`) for formatting years to the input `").concat(r, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "YY")
    throw new RangeError("Use `yy` instead of `YY` (in `".concat(a, "`) for formatting years to the input `").concat(r, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "D")
    throw new RangeError("Use `d` instead of `D` (in `".concat(a, "`) for formatting days of the month to the input `").concat(r, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "DD")
    throw new RangeError("Use `dd` instead of `DD` (in `".concat(a, "`) for formatting days of the month to the input `").concat(r, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
}
var IZ = {
  lessThanXSeconds: {
    one: "less than a second",
    other: "less than {{count}} seconds"
  },
  xSeconds: {
    one: "1 second",
    other: "{{count}} seconds"
  },
  halfAMinute: "half a minute",
  lessThanXMinutes: {
    one: "less than a minute",
    other: "less than {{count}} minutes"
  },
  xMinutes: {
    one: "1 minute",
    other: "{{count}} minutes"
  },
  aboutXHours: {
    one: "about 1 hour",
    other: "about {{count}} hours"
  },
  xHours: {
    one: "1 hour",
    other: "{{count}} hours"
  },
  xDays: {
    one: "1 day",
    other: "{{count}} days"
  },
  aboutXWeeks: {
    one: "about 1 week",
    other: "about {{count}} weeks"
  },
  xWeeks: {
    one: "1 week",
    other: "{{count}} weeks"
  },
  aboutXMonths: {
    one: "about 1 month",
    other: "about {{count}} months"
  },
  xMonths: {
    one: "1 month",
    other: "{{count}} months"
  },
  aboutXYears: {
    one: "about 1 year",
    other: "about {{count}} years"
  },
  xYears: {
    one: "1 year",
    other: "{{count}} years"
  },
  overXYears: {
    one: "over 1 year",
    other: "over {{count}} years"
  },
  almostXYears: {
    one: "almost 1 year",
    other: "almost {{count}} years"
  }
}, HZ = function(n, a, r) {
  var l, s = IZ[n];
  return typeof s == "string" ? l = s : a === 1 ? l = s.one : l = s.other.replace("{{count}}", a.toString()), r != null && r.addSuffix ? r.comparison && r.comparison > 0 ? "in " + l : l + " ago" : l;
};
const PZ = HZ;
function bl(n) {
  return function() {
    var a = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {}, r = a.width ? String(a.width) : n.defaultWidth, l = n.formats[r] || n.formats[n.defaultWidth];
    return l;
  };
}
var jZ = {
  full: "EEEE, MMMM do, y",
  long: "MMMM do, y",
  medium: "MMM d, y",
  short: "MM/dd/yyyy"
}, XZ = {
  full: "h:mm:ss a zzzz",
  long: "h:mm:ss a z",
  medium: "h:mm:ss a",
  short: "h:mm a"
}, _Z = {
  full: "{{date}} 'at' {{time}}",
  long: "{{date}} 'at' {{time}}",
  medium: "{{date}}, {{time}}",
  short: "{{date}}, {{time}}"
}, LZ = {
  date: bl({
    formats: jZ,
    defaultWidth: "full"
  }),
  time: bl({
    formats: XZ,
    defaultWidth: "full"
  }),
  dateTime: bl({
    formats: _Z,
    defaultWidth: "full"
  })
};
const qZ = LZ;
var KZ = {
  lastWeek: "'last' eeee 'at' p",
  yesterday: "'yesterday at' p",
  today: "'today at' p",
  tomorrow: "'tomorrow at' p",
  nextWeek: "eeee 'at' p",
  other: "P"
}, $Z = function(n, a, r, l) {
  return KZ[n];
};
const eJ = $Z;
function Or(n) {
  return function(a, r) {
    var l = r != null && r.context ? String(r.context) : "standalone", s;
    if (l === "formatting" && n.formattingValues) {
      var f = n.defaultFormattingWidth || n.defaultWidth, h = r != null && r.width ? String(r.width) : f;
      s = n.formattingValues[h] || n.formattingValues[f];
    } else {
      var p = n.defaultWidth, T = r != null && r.width ? String(r.width) : n.defaultWidth;
      s = n.values[T] || n.values[p];
    }
    var N = n.argumentCallback ? n.argumentCallback(a) : a;
    return s[N];
  };
}
var tJ = {
  narrow: ["B", "A"],
  abbreviated: ["BC", "AD"],
  wide: ["Before Christ", "Anno Domini"]
}, nJ = {
  narrow: ["1", "2", "3", "4"],
  abbreviated: ["Q1", "Q2", "Q3", "Q4"],
  wide: ["1st quarter", "2nd quarter", "3rd quarter", "4th quarter"]
}, rJ = {
  narrow: ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"],
  abbreviated: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  wide: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
}, iJ = {
  narrow: ["S", "M", "T", "W", "T", "F", "S"],
  short: ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"],
  abbreviated: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
  wide: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
}, aJ = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  }
}, oJ = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  }
}, lJ = function(n, a) {
  var r = Number(n), l = r % 100;
  if (l > 20 || l < 10)
    switch (l % 10) {
      case 1:
        return r + "st";
      case 2:
        return r + "nd";
      case 3:
        return r + "rd";
    }
  return r + "th";
}, sJ = {
  ordinalNumber: lJ,
  era: Or({
    values: tJ,
    defaultWidth: "wide"
  }),
  quarter: Or({
    values: nJ,
    defaultWidth: "wide",
    argumentCallback: function(n) {
      return n - 1;
    }
  }),
  month: Or({
    values: rJ,
    defaultWidth: "wide"
  }),
  day: Or({
    values: iJ,
    defaultWidth: "wide"
  }),
  dayPeriod: Or({
    values: aJ,
    defaultWidth: "wide",
    formattingValues: oJ,
    defaultFormattingWidth: "wide"
  })
};
const cJ = sJ;
function zr(n) {
  return function(a) {
    var r = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = r.width, s = l && n.matchPatterns[l] || n.matchPatterns[n.defaultMatchWidth], f = a.match(s);
    if (!f)
      return null;
    var h = f[0], p = l && n.parsePatterns[l] || n.parsePatterns[n.defaultParseWidth], T = Array.isArray(p) ? dJ(p, function(k) {
      return k.test(h);
    }) : uJ(p, function(k) {
      return k.test(h);
    }), N;
    N = n.valueCallback ? n.valueCallback(T) : T, N = r.valueCallback ? r.valueCallback(N) : N;
    var g = a.slice(h.length);
    return {
      value: N,
      rest: g
    };
  };
}
function uJ(n, a) {
  for (var r in n)
    if (n.hasOwnProperty(r) && a(n[r]))
      return r;
}
function dJ(n, a) {
  for (var r = 0; r < n.length; r++)
    if (a(n[r]))
      return r;
}
function fJ(n) {
  return function(a) {
    var r = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = a.match(n.matchPattern);
    if (!l)
      return null;
    var s = l[0], f = a.match(n.parsePattern);
    if (!f)
      return null;
    var h = n.valueCallback ? n.valueCallback(f[0]) : f[0];
    h = r.valueCallback ? r.valueCallback(h) : h;
    var p = a.slice(s.length);
    return {
      value: h,
      rest: p
    };
  };
}
var hJ = /^(\d+)(th|st|nd|rd)?/i, pJ = /\d+/i, mJ = {
  narrow: /^(b|a)/i,
  abbreviated: /^(b\.?\s?c\.?|b\.?\s?c\.?\s?e\.?|a\.?\s?d\.?|c\.?\s?e\.?)/i,
  wide: /^(before christ|before common era|anno domini|common era)/i
}, VJ = {
  any: [/^b/i, /^(a|c)/i]
}, TJ = {
  narrow: /^[1234]/i,
  abbreviated: /^q[1234]/i,
  wide: /^[1234](th|st|nd|rd)? quarter/i
}, vJ = {
  any: [/1/i, /2/i, /3/i, /4/i]
}, wJ = {
  narrow: /^[jfmasond]/i,
  abbreviated: /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i,
  wide: /^(january|february|march|april|may|june|july|august|september|october|november|december)/i
}, UJ = {
  narrow: [/^j/i, /^f/i, /^m/i, /^a/i, /^m/i, /^j/i, /^j/i, /^a/i, /^s/i, /^o/i, /^n/i, /^d/i],
  any: [/^ja/i, /^f/i, /^mar/i, /^ap/i, /^may/i, /^jun/i, /^jul/i, /^au/i, /^s/i, /^o/i, /^n/i, /^d/i]
}, RJ = {
  narrow: /^[smtwf]/i,
  short: /^(su|mo|tu|we|th|fr|sa)/i,
  abbreviated: /^(sun|mon|tue|wed|thu|fri|sat)/i,
  wide: /^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)/i
}, NJ = {
  narrow: [/^s/i, /^m/i, /^t/i, /^w/i, /^t/i, /^f/i, /^s/i],
  any: [/^su/i, /^m/i, /^tu/i, /^w/i, /^th/i, /^f/i, /^sa/i]
}, gJ = {
  narrow: /^(a|p|mi|n|(in the|at) (morning|afternoon|evening|night))/i,
  any: /^([ap]\.?\s?m\.?|midnight|noon|(in the|at) (morning|afternoon|evening|night))/i
}, MJ = {
  any: {
    am: /^a/i,
    pm: /^p/i,
    midnight: /^mi/i,
    noon: /^no/i,
    morning: /morning/i,
    afternoon: /afternoon/i,
    evening: /evening/i,
    night: /night/i
  }
}, ZJ = {
  ordinalNumber: fJ({
    matchPattern: hJ,
    parsePattern: pJ,
    valueCallback: function(n) {
      return parseInt(n, 10);
    }
  }),
  era: zr({
    matchPatterns: mJ,
    defaultMatchWidth: "wide",
    parsePatterns: VJ,
    defaultParseWidth: "any"
  }),
  quarter: zr({
    matchPatterns: TJ,
    defaultMatchWidth: "wide",
    parsePatterns: vJ,
    defaultParseWidth: "any",
    valueCallback: function(n) {
      return n + 1;
    }
  }),
  month: zr({
    matchPatterns: wJ,
    defaultMatchWidth: "wide",
    parsePatterns: UJ,
    defaultParseWidth: "any"
  }),
  day: zr({
    matchPatterns: RJ,
    defaultMatchWidth: "wide",
    parsePatterns: NJ,
    defaultParseWidth: "any"
  }),
  dayPeriod: zr({
    matchPatterns: gJ,
    defaultMatchWidth: "any",
    parsePatterns: MJ,
    defaultParseWidth: "any"
  })
};
const JJ = ZJ;
var kJ = {
  code: "en-US",
  formatDistance: PZ,
  formatLong: qZ,
  formatRelative: eJ,
  localize: cJ,
  match: JJ,
  options: {
    weekStartsOn: 0,
    firstWeekContainsDate: 1
  }
};
const xh = kJ;
var yJ = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, bJ = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, EJ = /^'([^]*?)'?$/, FJ = /''/g, WJ = /[a-zA-Z]/;
function SJ(n, a, r) {
  var l, s, f, h, p, T, N, g, k, b, B, F, C, D, A, Q, S, H;
  Se(2, arguments);
  var y = String(a), _ = oi(), $ = (l = (s = r == null ? void 0 : r.locale) !== null && s !== void 0 ? s : _.locale) !== null && l !== void 0 ? l : xh, L = Wn((f = (h = (p = (T = r == null ? void 0 : r.firstWeekContainsDate) !== null && T !== void 0 ? T : r == null || (N = r.locale) === null || N === void 0 || (g = N.options) === null || g === void 0 ? void 0 : g.firstWeekContainsDate) !== null && p !== void 0 ? p : _.firstWeekContainsDate) !== null && h !== void 0 ? h : (k = _.locale) === null || k === void 0 || (b = k.options) === null || b === void 0 ? void 0 : b.firstWeekContainsDate) !== null && f !== void 0 ? f : 1);
  if (!(L >= 1 && L <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var Ae = Wn((B = (F = (C = (D = r == null ? void 0 : r.weekStartsOn) !== null && D !== void 0 ? D : r == null || (A = r.locale) === null || A === void 0 || (Q = A.options) === null || Q === void 0 ? void 0 : Q.weekStartsOn) !== null && C !== void 0 ? C : _.weekStartsOn) !== null && F !== void 0 ? F : (S = _.locale) === null || S === void 0 || (H = S.options) === null || H === void 0 ? void 0 : H.weekStartsOn) !== null && B !== void 0 ? B : 0);
  if (!(Ae >= 0 && Ae <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  if (!$.localize)
    throw new RangeError("locale must contain localize property");
  if (!$.formatLong)
    throw new RangeError("locale must contain formatLong property");
  var Pe = De(n);
  if (!MZ(Pe))
    throw new RangeError("Invalid time value");
  var de = Xl(Pe), fe = ZZ(Pe, de), pe = {
    firstWeekContainsDate: L,
    weekStartsOn: Ae,
    locale: $,
    _originalDate: Pe
  }, we = y.match(bJ).map(function(ae) {
    var ke = ae[0];
    if (ke === "p" || ke === "P") {
      var ye = QZ[ke];
      return ye(ae, $.formatLong);
    }
    return ae;
  }).join("").match(yJ).map(function(ae) {
    if (ae === "''")
      return "'";
    var ke = ae[0];
    if (ke === "'")
      return AJ(ae);
    var ye = BZ[ke];
    if (ye)
      return !(r != null && r.useAdditionalWeekYearTokens) && YZ(ae) && nf(ae, a, String(n)), !(r != null && r.useAdditionalDayOfYearTokens) && GZ(ae) && nf(ae, a, String(n)), ye(fe, ae, $.localize, pe);
    if (ke.match(WJ))
      throw new RangeError("Format string contains an unescaped latin alphabet character `" + ke + "`");
    return ae;
  }).join("");
  return we;
}
function AJ(n) {
  var a = n.match(EJ);
  return a ? a[1].replace(FJ, "'") : n;
}
function Bh(n, a) {
  if (n == null)
    throw new TypeError("assign requires that input parameter not be null or undefined");
  for (var r in a)
    Object.prototype.hasOwnProperty.call(a, r) && (n[r] = a[r]);
  return n;
}
function xJ(n) {
  return Bh({}, n);
}
var rf = 1e3 * 60, Ea = 60 * 24, af = Ea * 30, of = Ea * 365;
function BJ(n, a, r) {
  var l, s, f;
  Se(2, arguments);
  var h = oi(), p = (l = (s = r == null ? void 0 : r.locale) !== null && s !== void 0 ? s : h.locale) !== null && l !== void 0 ? l : xh;
  if (!p.formatDistance)
    throw new RangeError("locale must contain localize.formatDistance property");
  var T = NZ(n, a);
  if (isNaN(T))
    throw new RangeError("Invalid time value");
  var N = Bh(xJ(r), {
    addSuffix: Boolean(r == null ? void 0 : r.addSuffix),
    comparison: T
  }), g, k;
  T > 0 ? (g = De(a), k = De(n)) : (g = De(n), k = De(a));
  var b = String((f = r == null ? void 0 : r.roundingMethod) !== null && f !== void 0 ? f : "round"), B;
  if (b === "floor")
    B = Math.floor;
  else if (b === "ceil")
    B = Math.ceil;
  else if (b === "round")
    B = Math.round;
  else
    throw new RangeError("roundingMethod must be 'floor', 'ceil' or 'round'");
  var F = k.getTime() - g.getTime(), C = F / rf, D = Xl(k) - Xl(g), A = (F - D) / rf, Q = r == null ? void 0 : r.unit, S;
  if (Q ? S = String(Q) : C < 1 ? S = "second" : C < 60 ? S = "minute" : C < Ea ? S = "hour" : A < af ? S = "day" : A < of ? S = "month" : S = "year", S === "second") {
    var H = B(F / 1e3);
    return p.formatDistance("xSeconds", H, N);
  } else if (S === "minute") {
    var y = B(C);
    return p.formatDistance("xMinutes", y, N);
  } else if (S === "hour") {
    var _ = B(C / 60);
    return p.formatDistance("xHours", _, N);
  } else if (S === "day") {
    var $ = B(A / Ea);
    return p.formatDistance("xDays", $, N);
  } else if (S === "month") {
    var L = B(A / af);
    return L === 12 && Q !== "month" ? p.formatDistance("xYears", 1, N) : p.formatDistance("xMonths", L, N);
  } else if (S === "year") {
    var Ae = B(A / of);
    return p.formatDistance("xYears", Ae, N);
  }
  throw new RangeError("unit must be 'second', 'minute', 'hour', 'day', 'month' or 'year'");
}
const dn = (n) => {
  let a = "";
  if (n)
    try {
      a = SJ(new Date(n), gt.DATE_FORMAT);
    } catch {
      console.log("error date", n);
    }
  return a;
}, CJ = window.Vue.defineComponent, aa = window.Vue.toDisplayString, yn = window.Vue.createElementVNode, DJ = window.Vue.createVNode, lf = window.Vue.unref, sf = window.Vue.createTextVNode, QJ = window.Vue.openBlock, OJ = window.Vue.createElementBlock, zJ = window.Vue.createCommentVNode, GJ = window.Vue.pushScopeId, YJ = window.Vue.popScopeId, Ch = (n) => (GJ("data-v-cc91403f"), n = n(), YJ(), n), IJ = {
  key: 0,
  class: "card"
}, HJ = { class: "row" }, PJ = { class: "title" }, jJ = ["innerHTML"], XJ = /* @__PURE__ */ Ch(() => /* @__PURE__ */ yn("strong", null, "First Event", -1)), _J = /* @__PURE__ */ Ch(() => /* @__PURE__ */ yn("strong", null, "Last Event", -1)), LJ = /* @__PURE__ */ CJ({
  props: {
    alarm: null
  },
  setup(n) {
    const a = n;
    return (r, l) => {
      var s;
      return a.alarm ? (QJ(), OJ("div", IJ, [
        yn("div", HJ, [
          yn("div", PJ, aa(n.alarm.nodeLabel) + " [" + aa(n.alarm.id) + "]", 1),
          DJ(Mh, {
            severity: (s = n.alarm) == null ? void 0 : s.severity
          }, null, 8, ["severity"])
        ]),
        yn("div", {
          innerHTML: n.alarm.description
        }, null, 8, jJ),
        yn("div", null, [
          XJ,
          sf(" - " + aa(lf(dn)(n.alarm.firstTime)), 1)
        ]),
        yn("div", null, [
          _J,
          sf(" - " + aa(lf(dn)(n.alarm.time)), 1)
        ])
      ])) : zJ("", !0);
    };
  }
});
const qJ = /* @__PURE__ */ Qe(LJ, [["__scopeId", "data-v-cc91403f"]]), KJ = window.Vue.defineComponent, $J = window.Vue.normalizeClass, ek = window.Vue.openBlock, tk = window.Vue.createElementBlock, nk = window.Vue.createCommentVNode, rk = /* @__PURE__ */ KJ({
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const a = n;
    return (r, l) => a != null && a.severity ? (ek(), tk("span", {
      key: 0,
      class: $J(["circle", [`${a.severity.toLowerCase()}-bg dark`]])
    }, null, 2)) : nk("", !0);
  }
});
const ik = /* @__PURE__ */ Qe(rk, [["__scopeId", "data-v-e08880d6"]]), ak = window.Vue.defineComponent, _l = window.Vue.createElementVNode, Dh = window.Vue.createTextVNode, oa = window.Vue.unref, cf = window.Vue.normalizeClass, El = window.Vue.withCtx, Fl = window.Vue.createVNode, uf = window.Vue.renderList, df = window.Vue.Fragment, tr = window.Vue.openBlock, la = window.Vue.createElementBlock, ok = window.Vue.toDisplayString, ff = window.Vue.createBlock, lk = window.Vue.pushScopeId, sk = window.Vue.popScopeId, ck = (n) => (lk("data-v-1575a51a"), n = n(), sk(), n), uk = { class: "container" }, dk = /* @__PURE__ */ ck(() => /* @__PURE__ */ _l("div", { class: "title" }, "Alarms", -1)), fk = /* @__PURE__ */ Dh(" ALL "), hk = { class: "section" }, pk = { class: "alarm-list" }, hf = window.Vue.ref, mk = window.Vue.watch, Vk = window.Vue.computed, Tk = /* @__PURE__ */ ak({
  props: {
    alarms: null
  },
  setup(n) {
    const a = n, r = Vk(() => qe.exports.keys(qe.exports.groupBy(a.alarms, "severity"))), l = hf(["all"]), s = hf(a.alarms), f = (h) => {
      l.value = l.value.filter((p) => p !== "all"), l.value.includes(h) ? l.value = l.value.filter((p) => p !== h) : l.value.push(h), h === "all" || l.value.length === 0 ? (l.value = ["all"], s.value = a.alarms) : s.value = a.alarms.filter(
        (p) => l.value.includes(p.severity)
      );
    };
    return mk(a, () => {
      l.value = ["all"], s.value = a.alarms;
    }), (h, p) => (tr(), la("div", uk, [
      dk,
      (tr(), ff(oa(wZ), {
        key: l.value.toString(),
        condensed: "",
        class: "alarm-filters",
        label: "Random list for condensed visual testing"
      }, {
        default: El(() => [
          Fl(oa(Kd), {
            class: cf({ clicked: l.value.includes("all") }),
            onClick: p[0] || (p[0] = (T) => f("all"))
          }, {
            default: El(() => [
              fk
            ]),
            _: 1
          }, 8, ["class"]),
          (tr(!0), la(df, null, uf(oa(r), (T) => (tr(), ff(oa(Kd), {
            class: cf({ clicked: l.value.includes(T) }),
            key: T,
            onClick: (N) => f(T)
          }, {
            default: El(() => [
              Fl(ik, { severity: T }, null, 8, ["severity"]),
              Dh(ok(T), 1)
            ]),
            _: 2
          }, 1032, ["class", "onClick"]))), 128))
        ]),
        _: 1
      })),
      _l("div", hk, [
        _l("div", pk, [
          (tr(!0), la(df, null, uf(s.value, (T) => (tr(), la("div", {
            key: T.id
          }, [
            Fl(qJ, { alarm: T }, null, 8, ["alarm"])
          ]))), 128))
        ])
      ])
    ]));
  }
});
const vk = /* @__PURE__ */ Qe(Tk, [["__scopeId", "data-v-1575a51a"]]);
var wk = Object.defineProperty, Uk = Object.defineProperties, Rk = Object.getOwnPropertyDescriptors, pf = Object.getOwnPropertySymbols, Nk = Object.prototype.hasOwnProperty, gk = Object.prototype.propertyIsEnumerable, mf = (n, a, r) => a in n ? wk(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, Gr = (n, a) => {
  for (var r in a || (a = {}))
    Nk.call(a, r) && mf(n, r, a[r]);
  if (pf)
    for (var r of pf(a))
      gk.call(a, r) && mf(n, r, a[r]);
  return n;
}, Vf = (n, a) => Uk(n, Rk(a));
const Mk = window.Vue.defineComponent, Zk = window.Vue.inject, Yr = window.Vue.h;
var Jk = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const kk = {
  primary: {
    type: Boolean,
    default: !1
  },
  text: {
    type: Boolean,
    default: !1
  },
  secondary: {
    type: Boolean,
    default: !1
  },
  icon: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  },
  asAnchor: {
    type: Boolean,
    default: !1
  },
  onColor: {
    type: Boolean,
    default: !1
  }
}, yk = Mk({
  inheritAttrs: !1,
  props: kk,
  setup() {
    return { hasTooltip: Zk("feather-has-tooltip", !1) };
  },
  render() {
    const n = () => {
      let p = "";
      this.primary && (p = "btn-primary"), this.secondary && (p = "btn-secondary"), (this.text || this.icon) && (p = "btn-text");
      const T = ["btn", "hover", "focus", p];
      return this.icon && (T.push("btn-icon"), T.push("btn-icon-table")), this.onColor && T.push("on-color"), T;
    }, a = this.asAnchor ? "a" : "button", r = {}, l = Gr({}, this.$attrs);
    r.attrs = l || {}, this.asAnchor ? r.attrs.role = "button" : r.attrs.type = r.attrs.type || "button", this.disabled && (r.attrs["aria-disabled"] = "true"), r.on = {
      onClick: (p) => {
        this.disabled ? (this.asAnchor && p.preventDefault(), this.$emit("disabled-click", p)) : this.$emit("click", p);
      }
    };
    const s = n();
    r.class = [this.$attrs.class].concat(s), this.$slots.icon && r.class.push("has-icon");
    let f = Yr(Fn);
    if (this.disabled && (f = void 0), this.icon && this.$slots.default) {
      const p = this.icon;
      return r.attrs["aria-label"] = p, this.hasTooltip || (r.attrs.title = p), Yr(a, Vf(Gr(Gr({}, r.attrs), r.on), { class: r.class }), [
        this.$slots.default(),
        this.disabled ? void 0 : Yr(Fn, { center: !0 })
      ]);
    }
    const h = Yr("span", { class: ["btn-content"] }, [
      this.$slots.default ? this.$slots.default() : ""
    ]);
    return Yr(a, Vf(Gr(Gr({}, r.attrs), r.on), { class: r.class }), [
      this.$slots.icon ? this.$slots.icon() : void 0,
      h,
      f
    ]);
  }
});
var Fa = /* @__PURE__ */ Jk(yk, [["__scopeId", "data-v-702d1074"]]);
const bk = "/whoami", Ek = async () => {
  try {
    const n = await pr.get(bk);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, Fk = window.Pinia.defineStore, li = Fk("userStore", {
  state: () => ({
    isAdmin: !1,
    userId: null,
    firstTime: !0,
    allowSave: !0
  }),
  actions: {
    async getUserRole() {
      const n = await Ek();
      return n && (this.isAdmin = n.roles.includes("ROLE_ADMIN"), this.userId = n.id), n;
    },
    async getAlecInfo() {
      const n = await TR();
      n && (this.firstTime = !1, this.allowSave = n.agreed);
    },
    async savePermission(n) {
      if (this.allowSave = n, !n) {
        const a = await wh(n);
        this.allowSave = a;
      }
    }
  }
}), Wk = window.Vue.defineComponent, Ue = window.Vue.unref, Ir = window.Vue.normalizeClass, zt = window.Vue.createVNode, Wl = window.Vue.toDisplayString, Gt = window.Vue.openBlock, an = window.Vue.createElementBlock, sa = window.Vue.createCommentVNode, Tf = window.Vue.withCtx, Sk = window.Vue.createBlock, Ht = window.Vue.createElementVNode, Ak = window.Vue.Fragment, xk = window.Vue.pushScopeId, Bk = window.Vue.popScopeId, Ck = (n) => (xk("data-v-b1da26ca"), n = n(), Bk(), n), Dk = { class: "section" }, Qk = {
  key: 0,
  class: "btn-row"
}, Ok = { key: 0 }, zk = { key: 1 }, Gk = { key: 0 }, Yk = { key: 1 }, Ik = {
  key: 1,
  class: "situation-detail"
}, Hk = { class: "situation-info" }, Pk = { class: "id" }, jk = ["innerHTML"], Xk = /* @__PURE__ */ Ck(() => /* @__PURE__ */ Ht("p", null, null, -1)), _k = { class: "boxes" }, Lk = { class: "parameters" }, qk = {
  key: 0,
  class: "section"
}, Kk = window.Vue.ref, $k = window.Vue.watch, ey = /* @__PURE__ */ Wk({
  props: {
    situationInfo: null
  },
  emits: ["situation-status-changed"],
  setup(n, { emit: a }) {
    const r = n, l = gt.ACCEPTED, s = gt.REJECTED, f = li(), h = Kk(r.situationInfo.status), p = (T) => {
      var N, g;
      wR((N = r.situationInfo) == null ? void 0 : N.id, T.toLowerCase()), h.value = T, a("situation-status-changed", T, (g = r.situationInfo) == null ? void 0 : g.id);
    };
    return $k(r, () => {
      h.value = r.situationInfo.status || "";
    }), (T, N) => {
      var g, k, b, B, F;
      return Gt(), an(Ak, null, [
        Ht("div", Dk, [
          Ue(f).allowSave ? (Gt(), an("div", Qk, [
            h.value !== Ue(s) ? (Gt(), Sk(Ue(Fa), {
              key: 0,
              class: Ir(["btn", { accepted: h.value == Ue(l) }]),
              "data-test": "btn-accept",
              onClick: N[0] || (N[0] = () => p(Ue(l)))
            }, {
              default: Tf(() => [
                zt(Ue(He), {
                  icon: Ue(Rh),
                  "aria-hidden": "true",
                  class: Ir(["icon accept", { accepted: h.value == Ue(l) }])
                }, null, 8, ["icon", "class"]),
                h.value == Ue(l) ? (Gt(), an("span", Ok, Wl(Ue(l)), 1)) : (Gt(), an("span", zk, " ACCEPT"))
              ]),
              _: 1
            }, 8, ["class"])) : sa("", !0),
            zt(Ue(Fa), {
              class: Ir(["btn", { rejected: h.value == Ue(s) }]),
              onClick: N[1] || (N[1] = () => p(Ue(s)))
            }, {
              default: Tf(() => [
                zt(Ue(He), {
                  icon: Ue(Nh),
                  "aria-hidden": "true",
                  class: Ir(["icon reject", { rejected: h.value == Ue(s) }])
                }, null, 8, ["icon", "class"]),
                h.value == Ue(s) ? (Gt(), an("span", Gk, Wl(Ue(s)), 1)) : (Gt(), an("span", Yk, " REJECT"))
              ]),
              _: 1
            }, 8, ["class"])
          ])) : sa("", !0),
          r.situationInfo ? (Gt(), an("div", Ik, [
            Ht("div", {
              class: Ir(["severity-line", [`${(k = (g = r.situationInfo) == null ? void 0 : g.severity) == null ? void 0 : k.toLowerCase()}-bg dark`]])
            }, null, 2),
            Ht("div", Hk, [
              Ht("div", Pk, [
                Ht("div", null, "Situation " + Wl((b = r.situationInfo) == null ? void 0 : b.id), 1),
                zt(Mh, {
                  severity: (B = r.situationInfo) == null ? void 0 : B.severity
                }, null, 8, ["severity"])
              ]),
              Ht("span", {
                innerHTML: r.situationInfo.description
              }, null, 8, jk),
              Xk,
              Ht("div", _k, [
                zt(kl, {
                  label: "First Event",
                  info: Ue(dn)(r.situationInfo.creationTime)
                }, null, 8, ["info"]),
                zt(kl, {
                  label: "Last Event",
                  info: Ue(dn)(r.situationInfo.lastTime)
                }, null, 8, ["info"]),
                zt(kl, {
                  label: "Reduction key",
                  info: r.situationInfo.reductionKey
                }, null, 8, ["info"])
              ])
            ]),
            Ht("div", Lk, [
              zt(Uh, {
                alarms: (F = r.situationInfo) == null ? void 0 : F.alarms,
                size: "large"
              }, null, 8, ["alarms"])
            ])
          ])) : sa("", !0)
        ]),
        r.situationInfo.alarms.length > 0 ? (Gt(), an("div", qk, [
          zt(vk, {
            alarms: r.situationInfo.alarms
          }, null, 8, ["alarms"])
        ])) : sa("", !0)
      ], 64);
    };
  }
});
const ty = /* @__PURE__ */ Qe(ey, [["__scopeId", "data-v-b1da26ca"]]);
const ny = window.Vue.openBlock, ry = window.Vue.createElementBlock, iy = window.Vue.createElementVNode;
var ay = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const oy = {}, ly = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, sy = /* @__PURE__ */ iy("path", { d: "M6.7,9.29a1,1,0,0,0,0,1.41L12,16l5.3-5.3a1,1,0,0,0-1.41-1.41L12,13.17,8.11,9.29A1,1,0,0,0,6.7,9.29Z" }, null, -1), cy = [
  sy
];
function uy(n, a) {
  return ny(), ry("svg", ly, cy);
}
var dy = /* @__PURE__ */ ay(oy, [["render", uy]]);
const fy = window.Vue.watch, hy = window.Vue.onBeforeUnmount, py = window.Vue.ref, my = window.Vue.onMounted, Vy = (n) => {
  const a = py(!1);
  let r = !1;
  const l = (h) => {
    n(h), r = !1;
  };
  function s(h) {
    r || (requestAnimationFrame(() => l(h)), r = !0);
  }
  const f = () => {
    window && window.removeEventListener("resize", s);
  };
  return my(() => {
    const h = fy(a, (p) => {
      window && p ? window.addEventListener("resize", s) : f();
    }, {
      immediate: !0
    });
    hy(() => {
      h(), f();
    });
  }), a;
}, Ty = window.Vue.watch, vy = window.Vue.onBeforeUnmount, wy = window.Vue.ref, Uy = window.Vue.onMounted, Ry = (n, a) => {
  const r = wy(!1), l = (h) => {
    h.target === window && a(h);
  }, s = (h) => {
    let p = [];
    Array.isArray(n.value) ? p = n.value : p = [n.value], p.some((N) => N && N.contains(h.target)) || a(h);
  }, f = () => {
    document && window && (document.removeEventListener("click", s, !0), document.removeEventListener("focus", s, !0), window.removeEventListener("blur", l));
  };
  return Uy(() => {
    const h = Ty(r, (p) => {
      document && window && p ? (document.addEventListener("click", s, !0), document.addEventListener("focus", s, !0), window.addEventListener("blur", l)) : f();
    }, {
      immediate: !0
    });
    vy(() => {
      h(), f();
    });
  }), r;
}, Ny = window.Vue.watch, gy = window.Vue.onBeforeUnmount, My = window.Vue.ref, Qh = (n, a) => {
  const r = My(!1);
  let l = !1;
  const s = (T) => {
    a(T), l = !1;
  };
  function f(T) {
    l || (requestAnimationFrame(() => s(T)), l = !0);
  }
  const h = () => {
    n.value && n.value.removeEventListener("scroll", f, !0);
  }, p = Ny([n, r], ([T, N], g) => {
    g && g.length && g[0] && g[0].removeEventListener("scroll", f, !0), N && T ? T.addEventListener("scroll", f, !0) : h();
  }, {
    immediate: !0
  });
  return gy(() => {
    p(), h();
  }), r;
}, Zy = window.Vue.defineComponent, Yt = window.Vue.ref, vf = window.Vue.toRef, Jy = window.Vue.onMounted, ky = window.Vue.watch, wf = window.Vue.computed, yy = window.Vue.nextTick, Uf = window.Vue.openBlock, Rf = window.Vue.createElementBlock, Nf = window.Vue.renderSlot, by = window.Vue.normalizeClass, Ey = window.Vue.normalizeStyle, Fy = window.Vue.createCommentVNode;
var Wy = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const Sy = {
  open: {
    type: Boolean,
    default: !1
  },
  noExpand: {
    type: Boolean,
    default: !1
  },
  cover: {
    type: Boolean,
    default: !1
  },
  right: {
    type: Boolean,
    default: !1
  },
  hasFocus: {
    type: Boolean,
    default: !1
  },
  dataRefId: {
    type: String,
    default: "feather-menu"
  },
  fill: {
    type: Boolean,
    default: !1
  },
  triggerId: {
    type: String
  }
}, Ay = {
  "trigger-click": (n) => !0,
  "outside-click": (n) => !0
}, xy = Zy({
  emits: Ay,
  props: Sy,
  setup(n, a) {
    const r = Yt(), l = Yt(), s = vf(n, "open"), f = vf(n, "noExpand"), h = Yt("auto"), p = Yt(), T = Yt(n.triggerId || ve("feather-menu-trigger")), N = Yt(ve("feather-menu-dropdown")), g = Yt(""), k = Yt("");
    Jy(() => {
      p.value = window;
    });
    const b = Yt(!1), B = () => ({
      height: p.value.innerHeight,
      width: p.value.innerWidth,
      left: 0,
      top: 0
    }), F = () => {
      if (!l.value)
        return;
      const y = r.value.getBoundingClientRect();
      b.value = !0, h.value = "auto", yy(() => {
        let { height: _, width: $ } = l.value.getBoundingClientRect();
        const L = B(), Ae = L.height, Pe = L.width;
        n.fill && $ < y.width ? (h.value = y.width + "px", $ = y.width) : h.value = $ + "px";
        let de = 0;
        Ae - y.bottom < _ && y.top >= _ ? (de = y.top - _, n.cover && (de += y.height)) : (de = y.bottom, n.cover && (de -= y.height));
        let fe = n.right ? y.right - $ : y.left;
        !n.right && y.right >= $ && Pe - y.left < $ && (fe = y.right - $), n.right && y.right <= $ && Pe - y.left > $ && (fe = y.left), k.value = `${fe}px`, g.value = `${de}px`, b.value = !1;
      });
    }, D = Ry(r, (y) => {
      a.emit("outside-click", y);
    }), A = Vy(F), Q = Qh(p, F);
    ky([s, l], ([y, _]) => {
      y && _ && F(), D.value = y, A.value = y, Q.value = y;
    });
    const S = wf(() => {
      const y = {
        id: T.value,
        "aria-haspopup": "true"
      };
      return s.value && (y["aria-controls"] = N.value), f.value || (y["aria-expanded"] = s.value ? "true" : "false"), y;
    }), H = wf(() => ({
      click: (y) => {
        a.emit("trigger-click", y);
      }
    }));
    return {
      positionTop: g,
      positionLeft: k,
      triggerId: T,
      triggerAttrs: S,
      triggerListeners: H,
      menuId: N,
      menu: l,
      menuWidth: h,
      root: r,
      calculatePosition: F,
      calculating: b
    };
  }
}), By = ["data-ref-id"], Cy = ["data-ref-id", "id"];
function Dy(n, a, r, l, s, f) {
  return Uf(), Rf("div", {
    class: "feather-menu",
    "data-ref-id": n.dataRefId,
    ref: "root"
  }, [
    Nf(n.$slots, "trigger", {
      attrs: n.triggerAttrs,
      on: n.triggerListeners
    }, void 0, !0),
    n.open ? (Uf(), Rf("div", {
      key: 0,
      class: by(["feather-menu-dropdown", { hidden: n.calculating }]),
      "data-ref-id": n.dataRefId + "-dropdown",
      ref: "menu",
      id: n.menuId,
      style: Ey({ left: n.positionLeft, top: n.positionTop, width: n.menuWidth })
    }, [
      Nf(n.$slots, "default", { labelId: n.triggerId }, void 0, !0)
    ], 14, Cy)) : Fy("", !0)
  ], 8, By);
}
var Qy = /* @__PURE__ */ Wy(xy, [["render", Dy], ["__scopeId", "data-v-f75af406"]]);
const Oy = window.Vue.defineComponent, zy = window.Vue.openBlock, Gy = window.Vue.createElementBlock, Yy = window.Vue.normalizeClass, Iy = window.Vue.pushScopeId, Hy = window.Vue.popScopeId, Ll = window.Vue.createElementVNode;
var Py = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const jy = {
  disabled: {
    type: Boolean,
    default: !1
  },
  checked: {
    type: Boolean,
    default: !1
  }
}, Xy = {
  click: (n) => !0
}, _y = Oy({
  emits: Xy,
  props: jy,
  methods: {
    handleClick(n) {
      this.disabled || this.$emit("click", n);
    }
  }
}), Oh = (n) => (Iy("data-v-07e020f5"), n = n(), Hy(), n), Ly = /* @__PURE__ */ Oh(() => /* @__PURE__ */ Ll("div", { class: "track" }, null, -1)), qy = /* @__PURE__ */ Oh(() => /* @__PURE__ */ Ll("div", { class: "switcher" }, [
  /* @__PURE__ */ Ll("div", { class: "switch-circle" })
], -1)), Ky = [
  Ly,
  qy
];
function $y(n, a, r, l, s, f) {
  return zy(), Gy("div", {
    class: Yy(["switch-container", { checked: n.checked, disabled: n.disabled }]),
    onClick: a[0] || (a[0] = (...h) => n.handleClick && n.handleClick(...h))
  }, Ky, 2);
}
var e2 = /* @__PURE__ */ Py(_y, [["render", $y], ["__scopeId", "data-v-07e020f5"]]), t2 = Object.defineProperty, n2 = Object.defineProperties, r2 = Object.getOwnPropertyDescriptors, gf = Object.getOwnPropertySymbols, i2 = Object.prototype.hasOwnProperty, a2 = Object.prototype.propertyIsEnumerable, Mf = (n, a, r) => a in n ? t2(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, Zf = (n, a) => {
  for (var r in a || (a = {}))
    i2.call(a, r) && Mf(n, r, a[r]);
  if (gf)
    for (var r of gf(a))
      a2.call(a, r) && Mf(n, r, a[r]);
  return n;
}, Jf = (n, a) => n2(n, r2(a));
const Zs = window.Vue.defineComponent, Mn = window.Vue.h, o2 = window.Vue.openBlock, l2 = window.Vue.createElementBlock, s2 = window.Vue.renderSlot;
window.Vue.resolveComponent;
window.Vue.createBlock;
window.Vue.withCtx;
window.Vue.pushScopeId;
window.Vue.popScopeId;
window.Vue.createElementVNode;
window.Vue.createVNode;
var zh = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const c2 = {
  asLi: {
    type: Boolean,
    default: !1
  },
  disabled: {
    type: Boolean,
    default: !1
  },
  selected: {
    type: Boolean,
    default: !1
  },
  highlighted: {
    type: Boolean,
    default: !1
  }
}, u2 = Zs({
  inheritAttrs: !1,
  props: c2,
  render() {
    let n;
    this.$slots.icon && this.$slots.icon().findIndex((h) => h.children && h.children.length !== 0 || h.type && h.type.render) !== -1 && (n = Mn("span", { class: ["feather-list-item-icon"] }, {
      default: this.$slots.icon
    }));
    const r = Mn("span", { class: ["feather-list-item-text"] }, { default: this.$slots.default });
    let l;
    this.$slots.post && (l = Mn("span", { class: "feather-list-item-post" }, this.$slots.post()));
    const s = this.disabled ? void 0 : Mn(Fn);
    if (this.asLi)
      return Mn("li", Jf(Zf({}, this.$attrs), {
        class: [
          "feather-list-item hover focus",
          {
            selected: this.selected,
            highlighted: this.highlighted,
            disabled: this.disabled
          },
          this.$attrs.class || ""
        ]
      }), [n, r, l, s]);
    const f = Mn("a", Jf(Zf({}, this.$attrs), {
      class: [
        "feather-list-item focus hover",
        {
          selected: this.selected,
          disabled: this.disabled
        },
        this.$attrs.class || ""
      ]
    }), [n, r, l, s]);
    return Mn("li", {}, [f]);
  }
});
var Js = /* @__PURE__ */ zh(u2, [["__scopeId", "data-v-7c46b2b3"]]);
Zs({
  components: {
    FeatherListItem: Js
  }
});
const d2 = {}, f2 = { class: "feather-list" };
function h2(n, a) {
  return o2(), l2("ul", f2, [
    s2(n.$slots, "default", {}, void 0, !0)
  ]);
}
var p2 = /* @__PURE__ */ zh(d2, [["render", h2], ["__scopeId", "data-v-941a1d50"]]);
const m2 = {
  "update:modelValue": (n) => !0,
  click: (n) => !0,
  keydown: (n) => !0
}, V2 = {
  disabled: {
    type: Boolean,
    default: !1
  },
  modelValue: {
    type: Boolean,
    default: !1
  }
};
Zs({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  emits: m2,
  props: V2,
  computed: {
    labelId() {
      return ve("switch-label");
    }
  },
  methods: {
    focus() {
      this.$refs.input.$el.focus();
    },
    updateValue() {
      this.disabled || this.$emit("update:modelValue", !this.modelValue);
    },
    click(n) {
      this.focus(), this.updateValue(), this.$emit("click", n);
    },
    keydown(n) {
      (n.keyCode === ge.SPACE || n.keyCode === ge.ENTER) && this.updateValue(), n.keyCode === ge.SPACE && n.preventDefault(), this.$emit("keydown", n);
    }
  },
  components: { SwitchRender: e2, FeatherListItem: Js }
});
const T2 = function(n, a) {
  if (!n || !a)
    return;
  let r = n.getBoundingClientRect().height;
  const l = getComputedStyle(n);
  r += parseInt(l.getPropertyValue("margin-top"), 10), r += parseInt(l.getPropertyValue("margin-bottom"), 10), a.scrollTop = n.offsetTop - a.getBoundingClientRect().height + r;
};
var v2 = Object.defineProperty, w2 = Object.defineProperties, U2 = Object.getOwnPropertyDescriptors, kf = Object.getOwnPropertySymbols, R2 = Object.prototype.hasOwnProperty, N2 = Object.prototype.propertyIsEnumerable, yf = (n, a, r) => a in n ? v2(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, ql = (n, a) => {
  for (var r in a || (a = {}))
    R2.call(a, r) && yf(n, r, a[r]);
  if (kf)
    for (var r of kf(a))
      N2.call(a, r) && yf(n, r, a[r]);
  return n;
}, g2 = (n, a) => w2(n, U2(a));
const Gh = window.Vue.defineComponent, bn = window.Vue.resolveComponent, Va = window.Vue.openBlock, bf = window.Vue.createBlock, Ta = window.Vue.mergeProps, En = window.Vue.withCtx, Yh = window.Vue.createElementBlock, M2 = window.Vue.Fragment, Z2 = window.Vue.renderList, J2 = window.Vue.createTextVNode, k2 = window.Vue.toDisplayString, y2 = window.Vue.computed, Ef = window.Vue.toRef, Hr = window.Vue.createVNode, Ff = window.Vue.toHandlers, b2 = window.Vue.renderSlot, E2 = window.Vue.normalizeClass, F2 = window.Vue.createElementVNode;
var Ih = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const W2 = Gh({
  inheritAttrs: !0,
  emits: ["select"],
  props: {
    label: {
      type: String,
      required: !0
    },
    options: {
      type: Array,
      default: () => []
    },
    textProp: {
      type: String,
      default: "_text"
    },
    activeIndex: {
      type: Number,
      required: !0
    }
  },
  computed: {
    activeId() {
      return ve("feather-select-active");
    },
    listAttrs() {
      return {
        tabindex: -1,
        role: "listbox",
        "aria-label": this.label,
        "aria-activedescendant": this.activeId
      };
    }
  },
  watch: {
    activeIndex(n) {
      n > -1 && this.$nextTick(() => {
        const a = Array.prototype.slice.call(this.$el.querySelectorAll("li"))[n];
        T2(a, this.$refs.list.$el);
      });
    }
  },
  methods: {
    isSelected(n) {
      return this.activeIndex === n;
    },
    getId(n) {
      return n === this.activeIndex ? this.activeId : null;
    },
    select(n) {
      this.$emit("select", n);
    }
  },
  components: {
    FeatherList: p2,
    FeatherListItem: Js
  }
});
function S2(n, a, r, l, s, f) {
  const h = bn("FeatherListItem"), p = bn("FeatherList");
  return Va(), bf(p, Ta(n.listAttrs, {
    ref: "list",
    class: "feather-select-options-list"
  }), {
    default: En(() => [
      (Va(!0), Yh(M2, null, Z2(n.options, (T, N) => (Va(), bf(h, {
        key: T[n.textProp],
        "as-li": "",
        id: n.getId(N),
        role: "option",
        tabindex: "-1",
        class: "result-item",
        "aria-selected": n.isSelected(N),
        selected: n.isSelected(N),
        onClick: (g) => n.select(T)
      }, {
        default: En(() => [
          J2(k2(T[n.textProp]), 1)
        ]),
        _: 2
      }, 1032, ["id", "aria-selected", "selected", "onClick"]))), 128))
    ]),
    _: 1
  }, 16);
}
var A2 = /* @__PURE__ */ Ih(W2, [["render", S2], ["__scopeId", "data-v-eae820da"]]);
const x2 = g2(ql(ql({}, bM), Us), {
  modelValue: {
    type: Object,
    required: !1
  },
  textProp: {
    type: String,
    default: "_text"
  },
  options: {
    type: Array,
    default: () => []
  },
  schema: {
    type: Object,
    required: !1
  }
}), B2 = {
  "update:modelValue": (n) => !0
}, C2 = Gh({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  emits: B2,
  props: x2,
  setup(n, a) {
    Rs(n), EM(n);
    const r = y2(() => ve("feather-select-input")), { validate: l } = vs(r, Ef(n, "modelValue"), n.label, n.schema, Ef(n, "error"));
    return ql({
      inputId: r,
      validate: l
    }, ws(a.attrs));
  },
  data() {
    return {
      hasFocus: !1,
      showMenu: !1,
      charsSoFar: "",
      internalValue: this.modelValue,
      delayTimeout: -1
    };
  },
  computed: {
    showClear() {
      return !!this.modelValue;
    },
    subTextId() {
      return ve("feather-select-description");
    },
    inputAttrs() {
      return {
        id: this.inputId,
        "aria-haspopup": "listbox",
        "aria-invalid": this.$attrs["aria-invalid"] || !!this.error,
        value: this.valueText,
        readonly: !0,
        disabled: this.disabled,
        "aria-disabled": this.disabled,
        "aria-describedby": (this.$attrs["aria-describedby"] || "").split(" ").concat([this.subTextId]).filter(Boolean).join(" ")
      };
    },
    inputListeners() {
      return {
        focus: this.handleInputFocus,
        blur: this.handleInputBlur,
        keydown: this.handleKeyDown
      };
    },
    raised() {
      return !!this.internalValue || this.hasFocus;
    },
    valueText() {
      return this.internalValue && this.internalValue[this.textProp] ? this.internalValue[this.textProp] : "";
    },
    activeIndex() {
      if (this.internalValue && this.internalValue[this.textProp]) {
        const n = this.internalValue, a = this.options.filter((r) => r[this.textProp] === n[this.textProp]);
        if (a && a.length)
          return this.options.indexOf(a[0]);
      }
      return -1;
    },
    icon: () => dy
  },
  watch: {
    showMenu(n) {
      n ? (this.internalValue || this.select(this.options[0]), this.$nextTick(() => {
        this.$refs.input.focus();
      })) : this.emitSelection();
    },
    modelValue(n) {
      this.internalValue = n;
    }
  },
  methods: {
    closeMenu() {
      this.showMenu = !1, this.$nextTick(() => {
        this.$refs.input.focus();
      });
    },
    handleClear() {
      this.handleSelect(void 0), this.emitSelection();
    },
    handleInputFocus() {
      this.disabled || this.hasFocus || (this.hasFocus = !0);
    },
    handleInputBlur() {
      this.hasFocus && !this.showMenu && (this.hasFocus = !1, this.validate());
    },
    handleTriggerClick() {
      this.disabled || (this.showMenu = !0, this.hasFocus = !0);
    },
    handleOutsideClick() {
      this.showMenu = !1, this.hasFocus = !1;
    },
    handleSelect(n) {
      this.select(n), this.showMenu = !1, this.$refs.input.focus();
    },
    select(n) {
      this.internalValue = n;
    },
    emitSelection() {
      this.$emit("update:modelValue", this.internalValue);
    },
    handleKeyDown(n) {
      if (n.keyCode === ge.ENTER)
        n.preventDefault(), this.showMenu = !this.showMenu, this.showMenu || this.$nextTick(() => {
          this.$refs.input.focus();
        });
      else if (n.keyCode === ge.ESCAPE)
        this.closeMenu(), n.stopPropagation();
      else if (n.keyCode === ge.DOWN)
        n.preventDefault(), this.activeIndex + 1 < this.options.length && this.select(this.options[this.activeIndex + 1]), this.showMenu = !0;
      else if (n.keyCode === ge.UP)
        n.preventDefault(), this.activeIndex - 1 >= 0 && this.select(this.options[this.activeIndex - 1]), this.showMenu = !0;
      else if (n.keyCode === ge.HOME)
        n.preventDefault(), this.select(this.options[0]), this.showMenu = !0;
      else if (n.keyCode === ge.END)
        n.preventDefault(), this.select(this.options[this.options.length - 1]), this.showMenu = !0;
      else if (this.showMenu) {
        const a = String.fromCharCode(n.keyCode);
        this.charsSoFar += a, this.searchAfterDelay();
      }
    },
    searchAfterDelay() {
      Na(this.delayTimeout), this.delayTimeout = Ra(() => {
        const n = this.options.filter((a) => a[this.textProp] && a[this.textProp].toLowerCase().indexOf(this.charsSoFar.toLowerCase()) === 0);
        n && n.length && this.select(n[0]), this.charsSoFar = "";
      }, 250);
    }
  },
  components: {
    InputWrapper: RM,
    InputSubText: Ts,
    FeatherMenu: Qy,
    List: A2,
    FeatherIcon: He
  }
});
function D2(n, a, r, l, s, f) {
  const h = bn("FeatherIcon"), p = bn("InputWrapper"), T = bn("List"), N = bn("FeatherMenu"), g = bn("InputSubText");
  return Va(), Yh("div", Ta(n.inherittedAttrs, { class: "feather-select-container" }), [
    Hr(N, {
      "no-expand": "",
      fill: "",
      open: n.showMenu,
      onOutsideClick: n.handleOutsideClick,
      onTriggerClick: n.handleTriggerClick,
      class: "feather-select-menu-container",
      "data-ref-id": "feather-select-menu-container"
    }, {
      trigger: En((k) => [
        Hr(p, Ta({
          for: n.inputId,
          raised: n.raised,
          focused: n.hasFocus,
          "show-clear": n.showClear,
          onClear: n.handleClear
        }, k.attrs, Ff(k.on), {
          class: ["feather-select-wrapper", { focused: n.hasFocus }]
        }), {
          pre: En(() => [
            b2(n.$slots, "pre", {}, void 0, !0)
          ]),
          post: En(() => [
            Hr(h, {
              class: E2(["feather-select-icon", { rotate: n.showMenu }]),
              icon: n.icon
            }, null, 8, ["class", "icon"])
          ]),
          default: En(() => [
            F2("input", Ta(n.inputAttrs, {
              class: "feather-select-input",
              "data-ref-id": "feather-select-input"
            }, Ff(n.inputListeners), { ref: "input" }), null, 16)
          ]),
          _: 2
        }, 1040, ["for", "raised", "focused", "show-clear", "onClear", "class"])
      ]),
      default: En(() => [
        Hr(T, {
          ref: "list",
          "data-ref-id": "feather-select-list",
          label: n.label,
          options: n.options,
          "text-prop": n.textProp,
          "active-index": n.activeIndex,
          onSelect: n.handleSelect,
          onKeydown: n.handleKeyDown
        }, null, 8, ["label", "options", "text-prop", "active-index", "onSelect", "onKeydown"])
      ]),
      _: 3
    }, 8, ["open", "onOutsideClick", "onTriggerClick"]),
    Hr(g, { id: n.subTextId }, null, 8, ["id"])
  ], 16);
}
var Q2 = /* @__PURE__ */ Ih(C2, [["render", D2], ["__scopeId", "data-v-ecb32d90"]]);
const O2 = window.Vue.openBlock, z2 = window.Vue.createElementBlock, Hh = window.Vue.createElementVNode;
var G2 = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const Y2 = {}, I2 = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, H2 = /* @__PURE__ */ Hh("path", { d: "M16,11H13V8a1,1,0,0,0-2,0v3H8a1,1,0,0,0,0,2h3v3a1,1,0,0,0,2,0V13h3a1,1,0,0,0,0-2Z" }, null, -1), P2 = /* @__PURE__ */ Hh("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" }, null, -1), j2 = [
  H2,
  P2
];
function X2(n, a) {
  return O2(), z2("svg", I2, j2);
}
var _2 = /* @__PURE__ */ G2(Y2, [["render", X2]]);
const va = window.Vue.openBlock, Kl = window.Vue.createElementBlock, Ph = window.Vue.createElementVNode, L2 = window.Vue.defineComponent, Zn = window.Vue.ref, q2 = window.Vue.provide, Wf = window.Vue.computed, K2 = window.Vue.onUnmounted, Sf = window.Vue.toRef, $2 = window.Vue.resolveComponent, eb = window.Vue.Fragment, tb = window.Vue.createBlock, nb = window.Vue.Teleport, Af = window.Vue.createVNode, rb = window.Vue.Transition, ib = window.Vue.withCtx, ab = window.Vue.normalizeClass, ob = window.Vue.normalizeStyle, lb = window.Vue.toDisplayString, sb = window.Vue.createCommentVNode, cb = window.Vue.renderSlot, Sl = window.Vue.nextTick;
var jh = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const ub = {}, db = {
  width: "32",
  height: "9",
  viewBox: "0 0 32 9",
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, fb = /* @__PURE__ */ Ph("path", {
  "fill-rule": "evenodd",
  "clip-rule": "evenodd",
  d: "M8.01471 2.17157C7.26457 1.42143 6.24715 1 5.18629 1H0.5C0.223858 1 0 0.776142 0 0.5V0.5C0 0.223858 0.223858 0 0.5 0H31.5C31.7761 0 32 0.223858 32 0.5V0.5C32 0.776142 31.7761 1 31.5 1H27.1274C26.0666 1 25.0492 1.42143 24.299 2.17157L18.9853 7.4853C17.4232 9.04739 14.8905 9.04739 13.3284 7.48529L8.01471 2.17157Z",
  fill: "var(--feather-high-visibility-on-surface)"
}, null, -1), hb = [
  fb
];
function pb(n, a) {
  return va(), Kl("svg", db, hb);
}
var mb = /* @__PURE__ */ jh(ub, [["render", pb]]), Fe = /* @__PURE__ */ ((n) => (n.top = "top", n.bottom = "bottom", n.left = "left", n.right = "right", n))(Fe || {}), Pt = /* @__PURE__ */ ((n) => (n.center = "center", n.left = "left", n.right = "right", n))(Pt || {});
const Vb = (n, a, r, l = 9) => {
  const s = window.innerHeight - n.bottom, f = window.innerWidth - n.right, h = [];
  n.top >= a.height + l && h.push(Fe.top), s >= a.height + l && h.push(Fe.bottom);
  const p = [];
  f >= a.width + l && p.push(Fe.right), n.left >= a.width + l && p.push(Fe.left);
  let T = [...p, ...h];
  return (r === Fe.top || r === Fe.bottom) && (T = [...h, ...p]), T.indexOf(r) > -1 ? r : T.length ? T[0] : r;
}, Tb = (n, a, r, l, s = 28) => {
  if (n === Fe.left || n === Fe.right)
    return Pt.center;
  const f = a.left + a.width / 2, h = window.innerWidth - a.right, p = [], T = f, N = h + a.width / 2, g = r.width - s, k = r.width / 2;
  return T >= k && N >= k && p.push(Pt.center), N >= g && p.push(Pt.left), T >= g && p.push(Pt.right), p.indexOf(l) > -1 ? l : p.length ? p[0] : l;
}, vb = {
  title: {
    type: String,
    required: !0
  },
  placement: {
    type: String,
    default: () => Fe.top
  },
  pointerAlignment: {
    type: String,
    default: () => Pt.center
  },
  enterDelay: {
    type: Number,
    default: 400
  },
  exitDelay: {
    type: Number,
    default: 400
  }
}, wb = L2({
  props: vb,
  setup(n) {
    const a = Zn(!1), r = Zn(!1), l = ve("feather-tooltip-trigger"), s = ve("feather-tooltip"), f = "data-feather-tooltip";
    q2("feather-has-tooltip", !0);
    let h = 0;
    const p = () => {
      Na(h), a.value || (h = Ra(C, n.enterDelay));
    }, T = () => {
      Na(h), h = Ra(D, n.exitDelay);
    }, N = (de) => {
      de.keyCode === ge.ESCAPE && (de.preventDefault(), D(!0));
    }, g = Wf(() => ({
      [f]: l,
      "aria-describedby": s
    })), k = {
      mouseenter: p,
      mouseleave: T,
      focus: p,
      blur: T,
      keydown: N
    }, b = Zn(document), B = Qh(b, () => D(!0));
    K2(() => D(!0));
    const F = () => document.getElementById(s), C = () => {
      r.value = !1, a.value = !0, Sl(() => {
        const de = F();
        Pe(de), a.value = !1, Sl(() => {
          r.value = !0, a.value = !0, B.value = !0;
        });
      });
    }, D = (de = !1) => {
      _.value = "", y.value = "", $.value = "", L.value = "", a.value = !1, de && (r.value = !1), B.value = !1;
    }, A = Sf(n, "placement"), Q = Sf(n, "pointerAlignment"), S = 8, H = 24, y = Zn(""), _ = Zn(""), $ = Zn(""), L = Zn(""), Ae = Wf(() => L.value ? "p-" + L.value : !1), Pe = (de) => {
      const fe = document.querySelector(`[${f}=${l}]`);
      if (!fe) {
        console.log("trigger not found");
        return;
      }
      Sl(() => {
        const pe = fe.getBoundingClientRect(), we = de.getBoundingClientRect(), ae = Vb(pe, we, A.value, S), ke = Tb(ae, pe, we, Q.value, H);
        $.value = ke.toString(), L.value = ae.toString();
        let ye = 0, Oe = 0;
        if ((ae === Fe.left || ae === Fe.right) && (ye = pe.top + pe.height / 2 - we.height / 2, ae === Fe.left && (Oe = pe.left - we.width - S), ae === Fe.right && (Oe = pe.right)), ae === Fe.top || ae === Fe.bottom) {
          ye = pe.top - we.height - S, ae === Fe.bottom && (ye = pe.bottom);
          const Vr = pe.left + pe.width / 2;
          switch (ke) {
            case Pt.center:
              Oe = Vr - we.width / 2;
              break;
            case Pt.left:
              Oe = Vr - H;
              break;
            case Pt.right:
              Oe = Vr - we.width + H;
              break;
          }
        }
        y.value = ye.toString() + "px", _.value = Oe.toString() + "px";
      });
    };
    return {
      attrs: g,
      listeners: k,
      show: a,
      animate: r,
      alignmentClass: $,
      placementClass: Ae,
      top: y,
      left: _,
      tooltipID: s
    };
  },
  components: {
    Pointer: mb
  }
}), Ub = ["id"];
function Rb(n, a, r, l, s, f) {
  const h = $2("Pointer");
  return va(), Kl(eb, null, [
    (va(), tb(nb, { to: "body" }, [
      Af(rb, { css: n.animate }, {
        default: ib(() => [
          n.show ? (va(), Kl("div", {
            key: 0,
            class: ab(["feather-tooltip-container", [n.alignmentClass, n.placementClass]]),
            ref: "tooltip",
            style: ob({ left: n.left, top: n.top })
          }, [
            Ph("div", {
              class: "tooltip",
              role: "tooltip",
              id: n.tooltipID
            }, lb(n.title), 9, Ub),
            Af(h, { class: "tooltip-pointer" })
          ], 6)) : sb("", !0)
        ]),
        _: 1
      }, 8, ["css"])
    ])),
    cb(n.$slots, "default", {
      attrs: n.attrs,
      on: n.listeners
    }, void 0, !0)
  ], 64);
}
var xf = /* @__PURE__ */ jh(wb, [["render", Rb], ["__scopeId", "data-v-3da6b22e"]]);
const Nb = window.Vue.defineComponent, ca = window.Vue.unref, Bf = window.Vue.toHandlers, Cf = window.Vue.mergeProps, Al = window.Vue.createElementVNode, Df = window.Vue.withCtx, Qf = window.Vue.createVNode, gb = window.Vue.normalizeClass, Mb = window.Vue.normalizeStyle, Zb = window.Vue.Fragment, Jb = window.Vue.openBlock, kb = window.Vue.createElementBlock, yb = /* @__PURE__ */ Nb({
  props: {
    alarm: null,
    proportion: null,
    minStart: null,
    maxEnd: null
  },
  setup(n) {
    const a = n, r = (s, f) => s && f ? (Number(f) - Number(s)) * a.proportion : 0, l = (s) => s ? (Number(s) - Number(a.minStart)) * a.proportion : 0;
    return (s, f) => (Jb(), kb(Zb, null, [
      Qf(ca(xf), {
        title: ca(dn)(n.alarm.firstTime)
      }, {
        default: Df(({ attrs: h, on: p }) => [
          Al("div", Cf({ class: "circle" }, h, Bf(p), {
            class: [`${n.alarm.severity.toLowerCase()}-bg dark`],
            style: {
              marginLeft: l(n.alarm.firstTime) + "px"
            }
          }), null, 16)
        ]),
        _: 1
      }, 8, ["title"]),
      Al("div", {
        class: gb(["line", [`${n.alarm.severity.toLowerCase()}-bg dark`]]),
        style: Mb({
          width: r(n.alarm.firstTime, n.alarm.time) + "px"
        })
      }, null, 6),
      Qf(ca(xf), {
        title: ca(dn)(n.alarm.time)
      }, {
        default: Df(({ attrs: h, on: p }) => [
          Al("div", Cf({ class: "circle" }, h, Bf(p), {
            class: [`${n.alarm.severity.toLowerCase()}-bg dark`]
          }), null, 16)
        ]),
        _: 1
      }, 8, ["title"])
    ], 64));
  }
});
const bb = /* @__PURE__ */ Qe(yb, [["__scopeId", "data-v-c620fbda"]]), Eb = window.Vue.openBlock, Fb = window.Vue.createElementBlock, Xh = window.Vue.createElementVNode;
var Wb = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const Sb = {}, Ab = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, xb = /* @__PURE__ */ Xh("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" }, null, -1), Bb = /* @__PURE__ */ Xh("path", { d: "M16,11H8a1,1,0,0,0,0,2h8a1,1,0,0,0,0-2Z" }, null, -1), Cb = [
  xb,
  Bb
];
function Db(n, a) {
  return Eb(), Fb("svg", Ab, Cb);
}
var Qb = /* @__PURE__ */ Wb(Sb, [["render", Db]]);
const Ob = window.Vue.defineComponent, zb = window.Vue.normalizeClass, Ce = window.Vue.createElementVNode, nr = window.Vue.toDisplayString, on = window.Vue.unref, ua = window.Vue.createVNode, Gb = window.Vue.createTextVNode, Of = window.Vue.renderList, xl = window.Vue.Fragment, rr = window.Vue.openBlock, ir = window.Vue.createElementBlock, Yb = window.Vue.createCommentVNode, Ib = window.Vue.pushScopeId, Hb = window.Vue.popScopeId, Pb = (n) => (Ib("data-v-c231ad04"), n = n(), Hb(), n), jb = { class: "section detail" }, Xb = { class: "id" }, _b = {
  key: 0,
  class: "section"
}, Lb = /* @__PURE__ */ Pb(() => /* @__PURE__ */ Ce("div", { class: "id" }, "Alarms", -1)), qb = { class: "action-btns" }, Kb = { class: "zoom" }, $b = /* @__PURE__ */ Gb(" Zoom "), eE = { class: "alarms" }, tE = { class: "times" }, nE = { class: "container" }, rE = { class: "ids" }, iE = { class: "timeline-container" }, ar = window.Vue.ref, aE = window.Vue.watch, oE = /* @__PURE__ */ Ob({
  props: {
    situation: null
  },
  setup(n) {
    var F, C, D, A;
    const a = n, r = 800;
    let l = ar(r);
    const s = [
      { id: 1, name: "Creation Time" },
      { id: 2, name: "Severity" },
      { id: 3, name: "Duration" }
    ], f = ar(s[0]), h = () => l.value / (Number(N.value) - Number(T.value)), p = ar(a.situation.alarms), T = ar(
      ((C = qe.exports.minBy((F = a.situation) == null ? void 0 : F.alarms, "firstTime")) == null ? void 0 : C.firstTime) || new Date()
    ), N = ar(((A = qe.exports.maxBy((D = a.situation) == null ? void 0 : D.alarms, "time")) == null ? void 0 : A.time) || new Date()), g = ar(h());
    aE(a, () => {
      var Q, S, H, y;
      T.value = ((S = qe.exports.minBy((Q = a.situation) == null ? void 0 : Q.alarms, "firstTime")) == null ? void 0 : S.firstTime) || new Date(), N.value = ((y = qe.exports.maxBy((H = a.situation) == null ? void 0 : H.alarms, "time")) == null ? void 0 : y.time) || new Date(), l.value = r, g.value = h(), p.value = a.situation.alarms, f.value = s[0];
    });
    const k = (Q) => {
      if ((Q == null ? void 0 : Q.id) === 1 && (p.value = a.situation.alarms), (Q == null ? void 0 : Q.id) === 2) {
        const S = qe.exports.groupBy(p.value, "severity"), H = [
          ...S.CRITICAL || [],
          ...S.MAJOR || [],
          ...S.MINOR || [],
          ...S.WARNING || [],
          ...S.NORMAL || [],
          ...S.CLEARED || [],
          ...S.INDETERMINATE || []
        ];
        p.value = H.filter((y) => y);
      }
      if ((Q == null ? void 0 : Q.id) === 3) {
        const S = qe.exports.reverse(
          qe.exports.sortBy(
            a.situation.alarms,
            (H) => Number(H.time) - Number(H.firstTime)
          )
        );
        p.value = S;
      }
    }, b = () => {
      l.value += 100, g.value = h();
    }, B = () => {
      l.value -= 100, g.value = h();
    };
    return (Q, S) => {
      var H, y;
      return rr(), ir(xl, null, [
        Ce("div", jb, [
          Ce("div", {
            class: zb(["severity-line", [`${(y = (H = a.situation) == null ? void 0 : H.severity) == null ? void 0 : y.toLowerCase()}-bg dark`]])
          }, null, 2),
          Ce("div", null, [
            Ce("div", Xb, "Situation " + nr(a.situation.id), 1),
            Ce("div", null, " Duration: " + nr(on(BJ)(new Date(N.value), new Date(T.value))), 1)
          ])
        ]),
        p.value.length > 0 ? (rr(), ir("div", _b, [
          Lb,
          Ce("div", qb, [
            ua(on(Q2), {
              class: "select",
              label: "Sort by:",
              options: s,
              modelValue: f.value,
              "onUpdate:modelValue": [
                S[0] || (S[0] = (_) => f.value = _),
                k
              ],
              "text-prop": "name",
              "data-test": "select-sort"
            }, null, 8, ["modelValue"]),
            Ce("div", Kb, [
              $b,
              Ce("div", null, [
                ua(on(He), {
                  icon: on(_2),
                  class: "zoom-icon",
                  onClick: b
                }, null, 8, ["icon"]),
                ua(on(He), {
                  icon: on(Qb),
                  class: "zoom-icon",
                  onClick: B
                }, null, 8, ["icon"])
              ])
            ])
          ]),
          Ce("div", eE, [
            Ce("div", tE, [
              Ce("div", null, nr(on(dn)(T.value)), 1),
              Ce("div", null, nr(on(dn)(N.value)), 1)
            ]),
            Ce("div", nE, [
              Ce("div", rE, [
                (rr(!0), ir(xl, null, Of(p.value, (_) => (rr(), ir("div", {
                  class: "alarm-id",
                  key: _.id
                }, nr(_.nodeLabel) + " [ " + nr(_.id) + " ] ", 1))), 128))
              ]),
              Ce("div", iE, [
                (rr(!0), ir(xl, null, Of(p.value, (_) => (rr(), ir("div", {
                  class: "timeline",
                  key: _.id
                }, [
                  ua(bb, {
                    alarm: _,
                    proportion: g.value,
                    "min-start": T.value,
                    "max-end": N.value
                  }, null, 8, ["alarm", "proportion", "min-start", "max-end"])
                ]))), 128))
              ])
            ])
          ])
        ])) : Yb("", !0)
      ], 64);
    };
  }
});
const lE = /* @__PURE__ */ Qe(oE, [["__scopeId", "data-v-c231ad04"]]), sE = window.Vue.defineComponent, _h = window.Vue.createTextVNode, Pr = window.Vue.unref, or = window.Vue.withCtx, Jn = window.Vue.createVNode, cE = window.Vue.openBlock, uE = window.Vue.createElementBlock, dE = window.Vue.createCommentVNode;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const fE = {
  key: 0,
  class: "detail"
}, hE = /* @__PURE__ */ _h("Details"), pE = /* @__PURE__ */ _h("Metrics"), mE = /* @__PURE__ */ sE({
  props: {
    alarmInfo: null
  },
  emits: ["situation-status-changed"],
  setup(n, { emit: a }) {
    const r = n, l = (s, f) => {
      a("situation-status-changed", s, f);
    };
    return (s, f) => r.alarmInfo ? (cE(), uE("div", fE, [
      Jn(Pr(ig), null, {
        tabs: or(() => [
          Jn(Pr(Dd), null, {
            default: or(() => [
              hE
            ]),
            _: 1
          }),
          Jn(Pr(Dd), null, {
            default: or(() => [
              pE
            ]),
            _: 1
          })
        ]),
        default: or(() => [
          Jn(Pr(Qd), { class: "panel" }, {
            default: or(() => [
              Jn(ty, {
                "situation-info": r.alarmInfo,
                onSituationStatusChanged: l
              }, null, 8, ["situation-info"])
            ]),
            _: 1
          }),
          Jn(Pr(Qd), { class: "panel" }, {
            default: or(() => [
              Jn(lE, {
                situation: r == null ? void 0 : r.alarmInfo
              }, null, 8, ["situation"])
            ]),
            _: 1
          })
        ]),
        _: 1
      })
    ])) : dE("", !0);
  }
});
const VE = /* @__PURE__ */ Qe(mE, [["__scopeId", "data-v-b34edfcf"]]), TE = window.Vue.defineComponent, $l = window.Vue.createElementVNode, vE = window.Vue.renderList, wE = window.Vue.Fragment, Bl = window.Vue.openBlock, Cl = window.Vue.createElementBlock, Dl = window.Vue.unref, zf = window.Vue.createVNode, UE = window.Vue.pushScopeId, RE = window.Vue.popScopeId, NE = (n) => (UE("data-v-0970f108"), n = n(), RE(), n), gE = { class: "list-main" }, ME = /* @__PURE__ */ NE(() => /* @__PURE__ */ $l("h2", null, "Situation List", -1)), ZE = { class: "container" }, JE = { class: "situation-list" }, kE = window.Vue.reactive, yE = window.Vue.ref, bE = /* @__PURE__ */ TE({
  setup(n) {
    const a = NR();
    a.getSituations();
    const r = kE({
      selectedSituationIndex: 0,
      situationSelected: ""
    }), l = yE(a.situations), s = (h) => {
      r.situationSelected = h, r.selectedSituationIndex = a.situations.findIndex(
        (p) => p.id === h
      );
    }, f = (h, p) => {
      const T = l.value;
      T.forEach((N) => {
        N.id === p && (N.status = h);
      }), l.value = qe.exports.cloneDeep(T);
    };
    return a.$subscribe((h, p) => {
      var T;
      l.value = a.situations, r.situationSelected = (T = p.situations[0]) == null ? void 0 : T.id;
    }), (h, p) => (Bl(), Cl("div", gE, [
      ME,
      $l("div", ZE, [
        $l("div", JE, [
          (Bl(!0), Cl(wE, null, vE(l.value, (T) => (Bl(), Cl("div", {
            key: T.id
          }, [
            zf(dN, {
              "situation-info": T,
              onSituationSelected: s,
              selected: Dl(r).situationSelected == T.id
            }, null, 8, ["situation-info", "selected"])
          ]))), 128))
        ]),
        zf(VE, {
          "alarm-info": Dl(a).situations[Dl(r).selectedSituationIndex],
          onSituationStatusChanged: f
        }, null, 8, ["alarm-info"])
      ])
    ]));
  }
});
const EE = /* @__PURE__ */ Qe(bE, [["__scopeId", "data-v-0970f108"]]);
var FE = Object.defineProperty, WE = Object.defineProperties, SE = Object.getOwnPropertyDescriptors, Gf = Object.getOwnPropertySymbols, AE = Object.prototype.hasOwnProperty, xE = Object.prototype.propertyIsEnumerable, Yf = (n, a, r) => a in n ? FE(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, wa = (n, a) => {
  for (var r in a || (a = {}))
    AE.call(a, r) && Yf(n, r, a[r]);
  if (Gf)
    for (var r of Gf(a))
      xE.call(a, r) && Yf(n, r, a[r]);
  return n;
}, Lh = (n, a) => WE(n, SE(a));
const qh = window.Vue.defineComponent, BE = window.Vue.ref, $r = window.Vue.computed, CE = window.Vue.reactive, If = window.Vue.watch, Ql = window.Vue.inject, Kh = window.Vue.resolveComponent, es = window.Vue.openBlock, $h = window.Vue.createElementBlock, un = window.Vue.createElementVNode, DE = window.Vue.createBlock, QE = window.Vue.createCommentVNode, ep = window.Vue.renderSlot, OE = window.Vue.pushScopeId, zE = window.Vue.popScopeId, Ol = window.Vue.toRef, Hf = window.Vue.mergeProps, GE = window.Vue.toDisplayString, YE = window.Vue.createVNode;
var tp = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const IE = {
  value: {
    type: [String, Number, Boolean, Array, Object, Date, Function],
    required: !0
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, HE = qh({
  props: IE,
  setup(n) {
    const a = BE(), r = () => {
      a.value.focus();
    }, l = $r(() => ve("feather-radio-button")), s = CE({
      first: !1,
      focus: r,
      disabled: n.disabled,
      value: n.value,
      checked: !1,
      id: l.value
    }), f = $r(() => ve("radio-label-id")), h = $r(() => s.first || s.checked ? 0 : -1);
    If(() => n.disabled, (k) => {
      s.disabled = k;
    }, { immediate: !0 }), If(() => n.value, (k) => {
      s.value = k;
    }, { immediate: !0 });
    const p = Ql("register", (k) => {
    }), T = Ql("blur", (k) => {
    }), N = Ql("select", (k) => {
    });
    return p(s), {
      labelId: f,
      tabindex: h,
      vm: s,
      blur: T,
      click: () => {
        N(s);
      },
      input: a,
      id: l
    };
  },
  components: {
    FeatherRipple: Fn
  }
}), PE = (n) => (OE("data-v-24790cf0"), n = n(), zE(), n), jE = { class: "layout-container" }, XE = ["id", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex"], _E = { class: "radio hover focus" }, LE = /* @__PURE__ */ PE(() => /* @__PURE__ */ un("div", {
  class: "icon",
  "aria-hidden": "true"
}, [
  /* @__PURE__ */ un("svg", { class: "dot" }, [
    /* @__PURE__ */ un("circle", {
      cx: "50%",
      cy: "50%",
      r: "0.3125rem",
      stroke: "black",
      "stroke-width": "0"
    })
  ])
], -1)), qE = ["id"];
function KE(n, a, r, l, s, f) {
  const h = Kh("feather-ripple");
  return es(), $h("div", jE, [
    un("div", {
      class: "feather-radio",
      role: "radio",
      ref: "input",
      id: n.id,
      "aria-checked": n.vm.checked ? "true" : "false",
      "aria-disabled": n.vm.disabled ? "true" : "false",
      "aria-labelledby": n.labelId,
      tabindex: n.tabindex,
      onClick: a[0] || (a[0] = (...p) => n.click && n.click(...p)),
      onBlur: a[1] || (a[1] = (...p) => n.blur && n.blur(...p)),
      "data-ref-id": "feather-radio"
    }, [
      un("div", _E, [
        LE,
        n.vm.disabled ? QE("", !0) : (es(), DE(h, {
          key: 0,
          center: ""
        }))
      ]),
      un("span", {
        class: "label",
        "data-ref-id": "feather-radio-label",
        id: n.labelId
      }, [
        ep(n.$slots, "default", {}, void 0, !0)
      ], 8, qE)
    ], 40, XE)
  ]);
}
var Wa = /* @__PURE__ */ tp(HE, [["render", KE], ["__scopeId", "data-v-24790cf0"]]);
const $E = Lh(wa({}, Us), {
  label: {
    type: String,
    required: !0
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
}), eF = {
  "update:modelValue": (n) => !0,
  blur: (n) => !0
}, tF = qh({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: $E,
  emits: eF,
  setup(n, a) {
    Rs(n);
    const r = Ol(n, "error"), l = Ol(n, "modelValue"), s = $r(() => ve("feather-input-description")), f = $r(() => {
      const h = Lh(wa({}, a.attrs), {
        class: "",
        "aria-describedby": s.value
      });
      return h["aria-invalid"] || (h["aria-invalid"] = !!r.value), h;
    });
    return wa(wa({
      descriptionId: s,
      attrs: f
    }, yh(l, a.emit, n.label, n.schema, Ol(n, "error"))), ws(a.attrs));
  },
  components: {
    InputSubText: Ts
  }
}), nF = ["for"], rF = ["id"];
function iF(n, a, r, l, s, f) {
  const h = Kh("InputSubText");
  return es(), $h("div", Hf(n.inherittedAttrs, {
    class: ["feather-radio-group-container", { vertical: n.vertical }]
  }), [
    un("label", {
      for: n.groupId,
      class: "group-label"
    }, GE(n.label), 9, nF),
    un("div", Hf(n.attrs, {
      class: "feather-radio-group",
      "data-ref-id": "feather-radio-group",
      role: "radiogroup",
      id: n.groupId,
      onKeydown: a[0] || (a[0] = (...p) => n.keydown && n.keydown(...p))
    }), [
      ep(n.$slots, "default", {}, void 0, !0)
    ], 16, rF),
    YE(h, { id: n.descriptionId }, null, 8, ["id"])
  ], 16);
}
var np = /* @__PURE__ */ tp(tF, [["render", iF], ["__scopeId", "data-v-6775aeb9"]]);
const aF = window.Vue.openBlock, oF = window.Vue.createElementBlock, lF = window.Vue.createStaticVNode;
var sF = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const cF = {}, uF = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, dF = /* @__PURE__ */ lF('<path d="M4,13H14V11H4a1,1,0,0,0,0,2Z"></path><path d="M21,6a1,1,0,0,0-1-1H14V7h6A1,1,0,0,0,21,6Z"></path><path d="M20,17H11v2h9a1,1,0,0,0,0-2Z"></path><path d="M11.5,3h-1a.5.5,0,0,0-.5.5V5H4A1,1,0,0,0,4,7h6V8.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,11.5,3Z"></path><path d="M20,11H18V9.5a.5.5,0,0,0-.5-.5h-1a.5.5,0,0,0-.5.5v5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5V13h2a1,1,0,0,0,0-2Z"></path><path d="M8.5,15h-1a.5.5,0,0,0-.5.5V17H4a1,1,0,0,0,0,2H7v1.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,8.5,15Z"></path>', 6), fF = [
  dF
];
function hF(n, a) {
  return aF(), oF("svg", uF, fF);
}
var pF = /* @__PURE__ */ sF(cF, [["render", hF]]);
const mF = window.Vue.openBlock, VF = window.Vue.createElementBlock, TF = window.Vue.createStaticVNode;
var vF = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const wF = {}, UF = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, RF = /* @__PURE__ */ TF('<rect x="8" y="7" width="8" height="2" rx="1"></rect><rect x="8" y="11" width="8" height="2" rx="1"></rect><rect x="8" y="15" width="8" height="2" rx="1"></rect><path d="M21,21l-3-3v2H6V9L4,7V20a2,2,0,0,0,2,2H18v2Z"></path><path d="M6,4H18V15l2,2V4a2,2,0,0,0-2-2H6V0L3,3,6,6Z"></path>', 5), NF = [
  RF
];
function gF(n, a) {
  return mF(), VF("svg", UF, NF);
}
var MF = /* @__PURE__ */ vF(wF, [["render", gF]]);
const ZF = window.Vue.openBlock, JF = window.Vue.createElementBlock, kF = window.Vue.createElementVNode;
var yF = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const bF = {}, EF = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, FF = /* @__PURE__ */ kF("path", { d: "M17,10h5a1,1,0,0,0,1-1V3a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1V5.59L10.59,11H8V9A1,1,0,0,0,7,8H2A1,1,0,0,0,1,9v6a1,1,0,0,0,1,1H7a1,1,0,0,0,1-1V13h2.58L16,18.49V21a1,1,0,0,0,1,1h5a1,1,0,0,0,1-1V15a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1v.64L12.41,12,16,8.42V9A1,1,0,0,0,17,10Zm1,6h3v4H18ZM18,4h3V8H18Z" }, null, -1), WF = [
  FF
];
function SF(n, a) {
  return ZF(), JF("svg", EF, WF);
}
var AF = /* @__PURE__ */ yF(bF, [["render", SF]]);
const xF = window.Vue.openBlock, BF = window.Vue.createElementBlock, CF = window.Vue.createStaticVNode;
var DF = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const QF = {}, OF = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, zF = /* @__PURE__ */ CF('<rect x="3" y="3" width="4" height="4"></rect><rect x="10" y="3" width="4" height="4"></rect><rect x="17" y="3" width="4" height="4"></rect><rect x="3" y="10" width="4" height="4"></rect><rect x="10" y="10" width="4" height="4"></rect><rect x="17" y="10" width="4" height="4"></rect><rect x="3" y="17" width="4" height="4"></rect><rect x="10" y="17" width="4" height="4"></rect><rect x="17" y="17" width="4" height="4"></rect>', 9), GF = [
  zF
];
function YF(n, a) {
  return xF(), BF("svg", OF, GF);
}
var IF = /* @__PURE__ */ DF(QF, [["render", YF]]);
const HF = window.Vue.openBlock, PF = window.Vue.createElementBlock, jF = window.Vue.createElementVNode;
var XF = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const _F = {}, LF = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, qF = /* @__PURE__ */ jF("path", { d: "M18,6a4,4,0,0,0-1,7.86V18a2,2,0,0,1-2,2H10a2,2,0,0,1-2-2V12.9a5.5,5.5,0,0,0,4.5-5.4V5a2,2,0,0,0-2-2h-1a1,1,0,0,0,0,2h1V7.5a3.5,3.5,0,0,1-7,0V5h1a1,1,0,0,0,0-2h-1a2,2,0,0,0-2,2V7.5A5.5,5.5,0,0,0,6,12.9V18a4,4,0,0,0,4,4h5a4,4,0,0,0,4-4V13.86A4,4,0,0,0,18,6Zm0,6a2,2,0,1,1,2-2A2,2,0,0,1,18,12Z" }, null, -1), KF = [
  qF
];
function $F(n, a) {
  return HF(), PF("svg", LF, KF);
}
var eW = /* @__PURE__ */ XF(_F, [["render", $F]]);
const rp = () => window.VRouter || ks, tW = window.Vue.defineComponent, Te = window.Vue.createElementVNode, ri = window.Vue.createTextVNode, Ie = window.Vue.unref, It = window.Vue.createVNode, da = window.Vue.withCtx, Pf = window.Vue.openBlock, jf = window.Vue.createElementBlock, nW = window.Vue.createCommentVNode, rW = window.Vue.pushScopeId, iW = window.Vue.popScopeId, Mt = (n) => (rW("data-v-0fd6b0cd"), n = n(), iW(), n), aW = { class: "container" }, oW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("div", { class: "title" }, "ALEC", -1)), lW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("div", { class: "description" }, [
  /* @__PURE__ */ ri(" The Architecture for Learning Enabled Correlation, or ALEC, provides a machine learning powered solution for alarm correlation. Complex networks produce significant, and potentially overwhelming, amount of events and alarms. "),
  /* @__PURE__ */ Te("br"),
  /* @__PURE__ */ Te("strong", null, "Our goal"),
  /* @__PURE__ */ ri(" with ALEC to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. ")
], -1)), sW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("h3", null, "Key Benefits:", -1)), cW = { class: "list" }, uW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("span", null, " Alleviate alarm load by clustering them into actionable situations. ", -1)), dW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("span", null, "Assist in root cause analysis.", -1)), fW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("span", null, " Avoid potential issues flagged by alarms with low visibility. ", -1)), hW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("span", null, "Easy configuration.", -1)), pW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("span", null, " Continuous improvement. Provided anonymized data helps us improve ALEC. ", -1)), mW = {
  key: 0,
  class: "optin"
}, VW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("h3", null, "Enable ALEC for more precise results:", -1)), TW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("div", null, " Our goal with ALEC, is to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. The information provided is anonymized and will be used for internal purposes only. ", -1)), vW = { class: "choices" }, wW = /* @__PURE__ */ Mt(() => /* @__PURE__ */ Te("div", { class: "radio-text" }, " By choosing \u201CYes\u201D you accept that OpenNMS can store the information that you provide, for educational and research purposes. ", -1)), UW = /* @__PURE__ */ ri("Yes"), RW = /* @__PURE__ */ ri("No"), NW = /* @__PURE__ */ ri(" Continue "), gW = window.Vue.ref, MW = /* @__PURE__ */ tW({
  setup(n) {
    const a = li(), r = rp(), l = gW(!0), s = () => {
      const f = Boolean(l.value);
      a.savePermission(f), f ? r.push({ name: "configuration" }) : r.push({ name: "situations" });
    };
    return (f, h) => (Pf(), jf("div", aW, [
      oW,
      lW,
      Te("div", null, [
        sW,
        Te("div", cW, [
          Te("div", null, [
            It(Ie(He), {
              icon: Ie(IF),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            uW
          ]),
          Te("div", null, [
            It(Ie(He), {
              icon: Ie(AF),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            dW
          ]),
          Te("div", null, [
            It(Ie(He), {
              icon: Ie(eW),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            fW
          ]),
          Te("div", null, [
            It(Ie(He), {
              icon: Ie(pF),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            hW
          ]),
          Te("div", null, [
            It(Ie(He), {
              icon: Ie(MF),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            pW
          ])
        ])
      ]),
      Ie(a).isAdmin ? (Pf(), jf("div", mW, [
        VW,
        TW,
        Te("div", vW, [
          wW,
          It(Ie(np), {
            horizontal: "",
            modelValue: l.value,
            "onUpdate:modelValue": h[0] || (h[0] = (p) => l.value = p),
            label: ""
          }, {
            default: da(() => [
              It(Ie(Wa), { value: !0 }, {
                default: da(() => [
                  UW
                ]),
                _: 1
              }),
              It(Ie(Wa), { value: !1 }, {
                default: da(() => [
                  RW
                ]),
                _: 1
              })
            ]),
            _: 1
          }, 8, ["modelValue"])
        ])
      ])) : nW("", !0),
      It(Ie(Fa), {
        "data-test": "continue-btn",
        primary: "",
        class: "continue-btn",
        onClick: s
      }, {
        default: da(() => [
          NW
        ]),
        _: 1
      })
    ]));
  }
});
const ZW = /* @__PURE__ */ Qe(MW, [["__scopeId", "data-v-0fd6b0cd"]]);
const JW = {}, kW = window.Vue.openBlock, yW = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const bW = { class: "main" };
function EW(n, a) {
  return kW(), yW("div", bW, "Something bad is happening...");
}
const FW = /* @__PURE__ */ Qe(JW, [["render", EW], ["__scopeId", "data-v-fefffdff"]]), WW = "data:image/png;base64,", SW = "data:image/png;base64,";
var AW = Object.defineProperty, xW = Object.defineProperties, BW = Object.getOwnPropertyDescriptors, Xf = Object.getOwnPropertySymbols, CW = Object.prototype.hasOwnProperty, DW = Object.prototype.propertyIsEnumerable, _f = (n, a, r) => a in n ? AW(n, a, { enumerable: !0, configurable: !0, writable: !0, value: r }) : n[a] = r, ts = (n, a) => {
  for (var r in a || (a = {}))
    CW.call(a, r) && _f(n, r, a[r]);
  if (Xf)
    for (var r of Xf(a))
      DW.call(a, r) && _f(n, r, a[r]);
  return n;
}, ip = (n, a) => xW(n, BW(a));
const ap = window.Vue.defineComponent, QW = window.Vue.inject, OW = window.Vue.resolveComponent, zl = window.Vue.openBlock, Lf = window.Vue.createElementBlock, sr = window.Vue.createElementVNode, zW = window.Vue.createBlock, qf = window.Vue.createCommentVNode, GW = window.Vue.renderSlot, YW = window.Vue.pushScopeId, IW = window.Vue.popScopeId, Gl = window.Vue.toRef, fa = window.Vue.computed, HW = window.Vue.ref;
window.Vue.mergeProps;
window.Vue.toDisplayString;
window.Vue.createVNode;
var PW = (n, a) => {
  const r = n.__vccOpts || n;
  for (const [l, s] of a)
    r[l] = s;
  return r;
};
const jW = {
  disabled: {
    type: Boolean,
    default: !1
  },
  modelValue: {
    type: Boolean,
    default: !1
  },
  indeterminate: {
    type: Boolean,
    default: !1
  },
  label: {
    type: String
  },
  noFocus: {
    type: Boolean,
    default: !1
  }
}, XW = {
  "update:modelValue": (n) => !0,
  click: (n) => !0,
  indeterminate: (n) => !0
}, _W = ap({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  emits: XW,
  props: jW,
  mounted() {
    this.registerCheckbox && this.registerCheckbox(this.inputId);
  },
  setup() {
    return {
      registerCheckbox: QW("registerCheckbox", (a) => {
      })
    };
  },
  computed: {
    inputId() {
      return this.label && this.label.length ? void 0 : ve("checkbox");
    },
    labelId() {
      return this.label && this.label.length ? void 0 : ve("checkbox-label");
    }
  },
  watch: {
    indeterminate: {
      handler(n) {
        this.$emit("update:modelValue", void 0), this.$emit("indeterminate", n);
      }
    }
  },
  methods: {
    focus() {
      this.$refs.input.focus();
    },
    updateValue() {
      this.disabled || (this.indeterminate ? (this.$emit("update:modelValue", !0), this.$emit("indeterminate", !1)) : (this.$emit("update:modelValue", !this.modelValue), this.$emit("indeterminate", !1)));
    },
    click(n) {
      this.focus(), this.updateValue(), this.$emit("click", n);
    },
    keydown(n) {
      (n.keyCode === ge.SPACE || n.keyCode === ge.ENTER) && this.updateValue(), n.keyCode === ge.SPACE && n.preventDefault();
    }
  },
  components: {
    FeatherRipple: Fn
  }
}), LW = (n) => (YW("data-v-a7af27e2"), n = n(), IW(), n), qW = { class: "layout-container" }, KW = ["aria-checked", "aria-disabled", "aria-labelledby", "aria-label", "id", "tabindex"], $W = { class: "checkbox hover focus" }, eS = /* @__PURE__ */ LW(() => /* @__PURE__ */ sr("div", { class: "box" }, [
  /* @__PURE__ */ sr("svg", {
    class: "checkmark",
    role: "presentation",
    focusable: "false",
    viewBox: "0 0 24 24"
  }, [
    /* @__PURE__ */ sr("path", {
      class: "path",
      fill: "none",
      d: "M1.73,12.91 8.1,19.28 22.79,4.59"
    })
  ]),
  /* @__PURE__ */ sr("div", { class: "indeterminate" })
], -1)), tS = ["id", "for"];
function nS(n, a, r, l, s, f) {
  const h = OW("feather-ripple");
  return zl(), Lf("div", qW, [
    sr("div", {
      class: "feather-checkbox feather-form-input feather-checkbox-table",
      onClick: a[0] || (a[0] = (...p) => n.click && n.click(...p)),
      onKeydown: a[1] || (a[1] = (...p) => n.keydown && n.keydown(...p)),
      "aria-checked": n.modelValue ? "true" : n.indeterminate ? "mixed" : "false",
      "aria-disabled": n.disabled,
      "aria-labelledby": n.labelId,
      "aria-label": n.label,
      id: n.inputId,
      tabindex: n.noFocus ? -1 : 0,
      ref: "input",
      role: "checkbox",
      "data-ref-id": "feather-checkbox"
    }, [
      sr("div", $W, [
        eS,
        n.disabled ? qf("", !0) : (zl(), zW(h, {
          key: 0,
          center: ""
        }))
      ]),
      n.label ? qf("", !0) : (zl(), Lf("label", {
        key: 0,
        "data-ref-id": "feather-checkbox-label",
        id: n.labelId,
        for: n.inputId
      }, [
        GW(n.$slots, "default", {}, void 0, !0)
      ], 8, tS))
    ], 40, KW)
  ]);
}
var rS = /* @__PURE__ */ PW(_W, [["render", nS], ["__scopeId", "data-v-a7af27e2"]]);
const iS = ip(ts({}, Us), {
  modelValue: {
    type: [Array, Object],
    required: !1
  },
  label: {
    type: String,
    required: !0
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
});
ap({
  props: iS,
  provide() {
    return {
      registerCheckbox: this.registerCheckbox
    };
  },
  setup(n, a) {
    Rs(n);
    const r = Gl(n, "error"), l = fa(() => ve("feather-checkbox-group")), s = fa(() => ve("feather-input-description")), f = fa(() => ve("feather-input-label")), h = fa(() => {
      const g = JSON.parse(JSON.stringify(a.attrs));
      return g["aria-invalid"] || (g["aria-invalid"] = !!r.value), ip(ts({}, g), {
        class: "",
        "aria-describedby": s.value
      });
    }), p = HW(l.value), { validate: T } = vs(p, Gl(n, "modelValue"), n.label, n.schema, Gl(n, "error"));
    return ts({
      groupId: l,
      inputId: p,
      descriptionId: s,
      labelId: f,
      attrs: h,
      validate: T,
      registerCheckbox: (g) => {
        g && p.value === l.value && (p.value = g);
      }
    }, ws(a.attrs));
  },
  components: {
    InputSubText: Ts
  }
});
const aS = window.Vue.defineComponent, Je = window.Vue.createElementVNode, ns = window.Vue.createTextVNode, ln = window.Vue.unref, jr = window.Vue.withCtx, Xr = window.Vue.createVNode, oS = window.Vue.openBlock, lS = window.Vue.createElementBlock, sS = window.Vue.pushScopeId, cS = window.Vue.popScopeId, An = (n) => (sS("data-v-9a5b5f4e"), n = n(), cS(), n), uS = { class: "container" }, dS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("h2", null, "Configuration", -1)), fS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("p", null, [
  /* @__PURE__ */ ns(" ALEC relies on correlation engines to identify related alarm groupings (situations). These engines are powered by machine learning techniques that leverage alarms data to make informed decisions. "),
  /* @__PURE__ */ Je("br"),
  /* @__PURE__ */ ns(" For detail information about proposed engines you can read "),
  /* @__PURE__ */ Je("strong", null, [
    /* @__PURE__ */ Je("a", {
      target: "_blank",
      href: "https://docs.opennms.com/alec/3.0.0-SNAPSHOT/engines/cluster.html"
    }, " here ")
  ])
], -1)), hS = { class: "radio-content" }, pS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("strong", { class: "title" }, "Clustering", -1)), mS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("div", null, " Groups data points (alarms) based on a distance measure. We calculate alarms difference in time and add it to their distance within their network topology ", -1)), VS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("img", {
  class: "img",
  src: WW
}, null, -1)), TS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("div", { class: "hellinger" }, [
  /* @__PURE__ */ Je("strong", null, "With hellinger distance"),
  /* @__PURE__ */ Je("br"),
  /* @__PURE__ */ Je("span", { class: "description" }, " (Uses the Hellinger Distance between alarms as a scaling variable. It pushes alarms further apart if its value is high and vice versa.) ")
], -1)), vS = /* @__PURE__ */ An(() => /* @__PURE__ */ Je("div", { class: "radio-content" }, [
  /* @__PURE__ */ Je("strong", { class: "title" }, "Deep Learning"),
  /* @__PURE__ */ Je("div", null, " A Neural Network network is consulted to assess if alarms are related. Based on its evaluation, situations are built by association. "),
  /* @__PURE__ */ Je("img", {
    class: "img img2",
    src: SW
  })
], -1)), wS = /* @__PURE__ */ ns(" Continue "), Kf = window.Vue.ref, US = /* @__PURE__ */ aS({
  setup(n) {
    const a = li(), r = rp(), l = Kf(!1), s = Kf(gt.ENGINE_DBSCAN), f = () => {
      wh(a.allowSave), vR(s.value, l.value), r.push({ name: "situations" });
    };
    return (h, p) => (oS(), lS("div", uS, [
      dS,
      fS,
      Je("div", null, [
        Xr(ln(np), {
          class: "radio-group",
          label: "Currently, ALEC provides two clustering based engines, please, select one (can be changed later):",
          modelValue: s.value,
          "onUpdate:modelValue": p[1] || (p[1] = (T) => s.value = T)
        }, {
          default: jr(() => [
            Xr(ln(Wa), {
              class: "radio",
              value: ln(gt).ENGINE_DBSCAN
            }, {
              default: jr(() => [
                Je("div", hS, [
                  pS,
                  mS,
                  VS,
                  Xr(ln(rS), {
                    modelValue: l.value,
                    "onUpdate:modelValue": p[0] || (p[0] = (T) => l.value = T),
                    disabled: s.value !== ln(gt).ENGINE_DBSCAN,
                    class: "checkbox"
                  }, {
                    default: jr(() => [
                      TS
                    ]),
                    _: 1
                  }, 8, ["modelValue", "disabled"])
                ])
              ]),
              _: 1
            }, 8, ["value"]),
            Xr(ln(Wa), {
              class: "radio",
              value: ln(gt).ENGINE_DEEP_LEARNING
            }, {
              default: jr(() => [
                vS
              ]),
              _: 1
            }, 8, ["value"])
          ]),
          _: 1
        }, 8, ["label", "modelValue"])
      ]),
      Xr(ln(Fa), {
        primary: "",
        class: "btn",
        onClick: p[2] || (p[2] = () => f())
      }, {
        default: jr(() => [
          wS
        ]),
        _: 1
      })
    ]));
  }
});
const RS = /* @__PURE__ */ Qe(US, [["__scopeId", "data-v-9a5b5f4e"]]), NS = window.VueRouter.createRouter, gS = window.VueRouter.createWebHistory, Yl = async () => {
  const n = window.VRouter || ks, a = li();
  if (!a.userId) {
    const r = await a.getUserRole();
    await a.getAlecInfo(), r ? n.push({ name: "home" }) : n.push({ name: "error" });
  }
}, op = [
  {
    path: "/",
    name: "home",
    beforeEnter: async (n) => {
      const a = window.VRouter || ks, r = li();
      await r.getUserRole(), await r.getAlecInfo(), r.firstTime ? a.push({ name: "welcome", params: n.params }) : a.push({ name: "situations", params: n.params });
    },
    component: {}
  },
  {
    path: "/welcome",
    name: "welcome",
    beforeEnter: () => Yl(),
    component: ZW
  },
  {
    path: "/setup",
    name: "configuration",
    beforeEnter: () => Yl(),
    component: RS
  },
  {
    path: "/situations",
    name: "situations",
    beforeEnter: () => Yl(),
    component: EE
  },
  {
    path: "/error",
    name: "error",
    component: FW
  }
], $f = window.VRouter;
if ($f)
  for (const n of op) {
    const { path: a, name: r, component: l, beforeEnter: s } = n;
    $f.addRoute("Plugin", {
      path: a.slice(1),
      name: r,
      component: l,
      beforeEnter: s
    });
  }
const ks = NS({
  history: gS(),
  routes: op
});
window.Vue.createApp;
window.Pinia.createPinia;
window.uiextension = oU;
