const Pe = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
}, b0 = {}, W0 = window.Vue.resolveComponent, E0 = window.Vue.createVNode, F0 = window.Vue.openBlock, S0 = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const A0 = { class: "main" };
function x0(n, i) {
  const a = W0("router-view");
  return F0(), S0("div", A0, [
    E0(a)
  ]);
}
const Q0 = /* @__PURE__ */ Pe(b0, [["render", x0], ["__scopeId", "data-v-5d32d140"]]), B0 = window.Vue.defineComponent, D0 = window.Vue.openBlock, O0 = window.Vue.createBlock, G0 = /* @__PURE__ */ B0({
  __name: "App",
  setup(n) {
    return (i, a) => (D0(), O0(Q0));
  }
});
var Rr = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function C0(n) {
  return n && n.__esModule && Object.prototype.hasOwnProperty.call(n, "default") ? n.default : n;
}
var Od = { exports: {} }, Vl = { exports: {} }, Gd = function(i, a) {
  return function() {
    for (var c = new Array(arguments.length), f = 0; f < c.length; f++)
      c[f] = arguments[f];
    return i.apply(a, c);
  };
}, z0 = Gd, Tl = Object.prototype.toString, Ul = function(n) {
  return function(i) {
    var a = Tl.call(i);
    return n[a] || (n[a] = a.slice(8, -1).toLowerCase());
  };
}(/* @__PURE__ */ Object.create(null));
function Tn(n) {
  return n = n.toLowerCase(), function(a) {
    return Ul(a) === n;
  };
}
function Rl(n) {
  return Array.isArray(n);
}
function Ha(n) {
  return typeof n > "u";
}
function Y0(n) {
  return n !== null && !Ha(n) && n.constructor !== null && !Ha(n.constructor) && typeof n.constructor.isBuffer == "function" && n.constructor.isBuffer(n);
}
var Cd = Tn("ArrayBuffer");
function I0(n) {
  var i;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? i = ArrayBuffer.isView(n) : i = n && n.buffer && Cd(n.buffer), i;
}
function H0(n) {
  return typeof n == "string";
}
function P0(n) {
  return typeof n == "number";
}
function zd(n) {
  return n !== null && typeof n == "object";
}
function Ca(n) {
  if (Ul(n) !== "object")
    return !1;
  var i = Object.getPrototypeOf(n);
  return i === null || i === Object.prototype;
}
var j0 = Tn("Date"), X0 = Tn("File"), L0 = Tn("Blob"), _0 = Tn("FileList");
function vl(n) {
  return Tl.call(n) === "[object Function]";
}
function q0(n) {
  return zd(n) && vl(n.pipe);
}
function K0(n) {
  var i = "[object FormData]";
  return n && (typeof FormData == "function" && n instanceof FormData || Tl.call(n) === i || vl(n.toString) && n.toString() === i);
}
var $0 = Tn("URLSearchParams");
function eR(n) {
  return n.trim ? n.trim() : n.replace(/^\s+|\s+$/g, "");
}
function tR() {
  return typeof navigator < "u" && (navigator.product === "ReactNative" || navigator.product === "NativeScript" || navigator.product === "NS") ? !1 : typeof window < "u" && typeof document < "u";
}
function Nl(n, i) {
  if (!(n === null || typeof n > "u"))
    if (typeof n != "object" && (n = [n]), Rl(n))
      for (var a = 0, l = n.length; a < l; a++)
        i.call(null, n[a], a, n);
    else
      for (var c in n)
        Object.prototype.hasOwnProperty.call(n, c) && i.call(null, n[c], c, n);
}
function ll() {
  var n = {};
  function i(c, f) {
    Ca(n[f]) && Ca(c) ? n[f] = ll(n[f], c) : Ca(c) ? n[f] = ll({}, c) : Rl(c) ? n[f] = c.slice() : n[f] = c;
  }
  for (var a = 0, l = arguments.length; a < l; a++)
    Nl(arguments[a], i);
  return n;
}
function nR(n, i, a) {
  return Nl(i, function(c, f) {
    a && typeof c == "function" ? n[f] = z0(c, a) : n[f] = c;
  }), n;
}
function rR(n) {
  return n.charCodeAt(0) === 65279 && (n = n.slice(1)), n;
}
function aR(n, i, a, l) {
  n.prototype = Object.create(i.prototype, l), n.prototype.constructor = n, a && Object.assign(n.prototype, a);
}
function iR(n, i, a) {
  var l, c, f, p = {};
  i = i || {};
  do {
    for (l = Object.getOwnPropertyNames(n), c = l.length; c-- > 0; )
      f = l[c], p[f] || (i[f] = n[f], p[f] = !0);
    n = Object.getPrototypeOf(n);
  } while (n && (!a || a(n, i)) && n !== Object.prototype);
  return i;
}
function oR(n, i, a) {
  n = String(n), (a === void 0 || a > n.length) && (a = n.length), a -= i.length;
  var l = n.indexOf(i, a);
  return l !== -1 && l === a;
}
function lR(n) {
  if (!n)
    return null;
  var i = n.length;
  if (Ha(i))
    return null;
  for (var a = new Array(i); i-- > 0; )
    a[i] = n[i];
  return a;
}
var sR = function(n) {
  return function(i) {
    return n && i instanceof n;
  };
}(typeof Uint8Array < "u" && Object.getPrototypeOf(Uint8Array)), we = {
  isArray: Rl,
  isArrayBuffer: Cd,
  isBuffer: Y0,
  isFormData: K0,
  isArrayBufferView: I0,
  isString: H0,
  isNumber: P0,
  isObject: zd,
  isPlainObject: Ca,
  isUndefined: Ha,
  isDate: j0,
  isFile: X0,
  isBlob: L0,
  isFunction: vl,
  isStream: q0,
  isURLSearchParams: $0,
  isStandardBrowserEnv: tR,
  forEach: Nl,
  merge: ll,
  extend: nR,
  trim: eR,
  stripBOM: rR,
  inherits: aR,
  toFlatObject: iR,
  kindOf: Ul,
  kindOfTest: Tn,
  endsWith: oR,
  toArray: lR,
  isTypedArray: sR,
  isFileList: _0
}, xn = we;
function fu(n) {
  return encodeURIComponent(n).replace(/%3A/gi, ":").replace(/%24/g, "$").replace(/%2C/gi, ",").replace(/%20/g, "+").replace(/%5B/gi, "[").replace(/%5D/gi, "]");
}
var Yd = function(i, a, l) {
  if (!a)
    return i;
  var c;
  if (l)
    c = l(a);
  else if (xn.isURLSearchParams(a))
    c = a.toString();
  else {
    var f = [];
    xn.forEach(a, function(v, k) {
      v === null || typeof v > "u" || (xn.isArray(v) ? k = k + "[]" : v = [v], xn.forEach(v, function(y) {
        xn.isDate(y) ? y = y.toISOString() : xn.isObject(y) && (y = JSON.stringify(y)), f.push(fu(k) + "=" + fu(y));
      }));
    }), c = f.join("&");
  }
  if (c) {
    var p = i.indexOf("#");
    p !== -1 && (i = i.slice(0, p)), i += (i.indexOf("?") === -1 ? "?" : "&") + c;
  }
  return i;
}, cR = we;
function $a() {
  this.handlers = [];
}
$a.prototype.use = function(i, a, l) {
  return this.handlers.push({
    fulfilled: i,
    rejected: a,
    synchronous: l ? l.synchronous : !1,
    runWhen: l ? l.runWhen : null
  }), this.handlers.length - 1;
};
$a.prototype.eject = function(i) {
  this.handlers[i] && (this.handlers[i] = null);
};
$a.prototype.forEach = function(i) {
  cR.forEach(this.handlers, function(l) {
    l !== null && i(l);
  });
};
var uR = $a, dR = we, fR = function(i, a) {
  dR.forEach(i, function(c, f) {
    f !== a && f.toUpperCase() === a.toUpperCase() && (i[a] = c, delete i[f]);
  });
}, Id = we;
function In(n, i, a, l, c) {
  Error.call(this), this.message = n, this.name = "AxiosError", i && (this.code = i), a && (this.config = a), l && (this.request = l), c && (this.response = c);
}
Id.inherits(In, Error, {
  toJSON: function() {
    return {
      message: this.message,
      name: this.name,
      description: this.description,
      number: this.number,
      fileName: this.fileName,
      lineNumber: this.lineNumber,
      columnNumber: this.columnNumber,
      stack: this.stack,
      config: this.config,
      code: this.code,
      status: this.response && this.response.status ? this.response.status : null
    };
  }
});
var Hd = In.prototype, Pd = {};
[
  "ERR_BAD_OPTION_VALUE",
  "ERR_BAD_OPTION",
  "ECONNABORTED",
  "ETIMEDOUT",
  "ERR_NETWORK",
  "ERR_FR_TOO_MANY_REDIRECTS",
  "ERR_DEPRECATED",
  "ERR_BAD_RESPONSE",
  "ERR_BAD_REQUEST",
  "ERR_CANCELED"
].forEach(function(n) {
  Pd[n] = { value: n };
});
Object.defineProperties(In, Pd);
Object.defineProperty(Hd, "isAxiosError", { value: !0 });
In.from = function(n, i, a, l, c, f) {
  var p = Object.create(Hd);
  return Id.toFlatObject(n, p, function(v) {
    return v !== Error.prototype;
  }), In.call(p, n.message, i, a, l, c), p.name = n.name, f && Object.assign(p, f), p;
};
var Xn = In, jd = {
  silentJSONParsing: !0,
  forcedJSONParsing: !0,
  clarifyTimeoutError: !1
}, dt = we;
function hR(n, i) {
  i = i || new FormData();
  var a = [];
  function l(f) {
    return f === null ? "" : dt.isDate(f) ? f.toISOString() : dt.isArrayBuffer(f) || dt.isTypedArray(f) ? typeof Blob == "function" ? new Blob([f]) : Buffer.from(f) : f;
  }
  function c(f, p) {
    if (dt.isPlainObject(f) || dt.isArray(f)) {
      if (a.indexOf(f) !== -1)
        throw Error("Circular reference detected in " + p);
      a.push(f), dt.forEach(f, function(v, k) {
        if (!dt.isUndefined(v)) {
          var J = p ? p + "." + k : k, y;
          if (v && !p && typeof v == "object") {
            if (dt.endsWith(k, "{}"))
              v = JSON.stringify(v);
            else if (dt.endsWith(k, "[]") && (y = dt.toArray(v))) {
              y.forEach(function(g) {
                !dt.isUndefined(g) && i.append(J, l(g));
              });
              return;
            }
          }
          c(v, J);
        }
      }), a.pop();
    } else
      i.append(p, l(f));
  }
  return c(n), i;
}
var Xd = hR, yo, hu;
function pR() {
  if (hu)
    return yo;
  hu = 1;
  var n = Xn;
  return yo = function(a, l, c) {
    var f = c.config.validateStatus;
    !c.status || !f || f(c.status) ? a(c) : l(new n(
      "Request failed with status code " + c.status,
      [n.ERR_BAD_REQUEST, n.ERR_BAD_RESPONSE][Math.floor(c.status / 100) - 4],
      c.config,
      c.request,
      c
    ));
  }, yo;
}
var bo, pu;
function mR() {
  if (pu)
    return bo;
  pu = 1;
  var n = we;
  return bo = n.isStandardBrowserEnv() ? function() {
    return {
      write: function(l, c, f, p, V, v) {
        var k = [];
        k.push(l + "=" + encodeURIComponent(c)), n.isNumber(f) && k.push("expires=" + new Date(f).toGMTString()), n.isString(p) && k.push("path=" + p), n.isString(V) && k.push("domain=" + V), v === !0 && k.push("secure"), document.cookie = k.join("; ");
      },
      read: function(l) {
        var c = document.cookie.match(new RegExp("(^|;\\s*)(" + l + ")=([^;]*)"));
        return c ? decodeURIComponent(c[3]) : null;
      },
      remove: function(l) {
        this.write(l, "", Date.now() - 864e5);
      }
    };
  }() : function() {
    return {
      write: function() {
      },
      read: function() {
        return null;
      },
      remove: function() {
      }
    };
  }(), bo;
}
var VR = function(i) {
  return /^([a-z][a-z\d+\-.]*:)?\/\//i.test(i);
}, TR = function(i, a) {
  return a ? i.replace(/\/+$/, "") + "/" + a.replace(/^\/+/, "") : i;
}, UR = VR, RR = TR, Ld = function(i, a) {
  return i && !UR(a) ? RR(i, a) : a;
}, Wo, mu;
function vR() {
  if (mu)
    return Wo;
  mu = 1;
  var n = we, i = [
    "age",
    "authorization",
    "content-length",
    "content-type",
    "etag",
    "expires",
    "from",
    "host",
    "if-modified-since",
    "if-unmodified-since",
    "last-modified",
    "location",
    "max-forwards",
    "proxy-authorization",
    "referer",
    "retry-after",
    "user-agent"
  ];
  return Wo = function(l) {
    var c = {}, f, p, V;
    return l && n.forEach(l.split(`
`), function(k) {
      if (V = k.indexOf(":"), f = n.trim(k.substr(0, V)).toLowerCase(), p = n.trim(k.substr(V + 1)), f) {
        if (c[f] && i.indexOf(f) >= 0)
          return;
        f === "set-cookie" ? c[f] = (c[f] ? c[f] : []).concat([p]) : c[f] = c[f] ? c[f] + ", " + p : p;
      }
    }), c;
  }, Wo;
}
var Eo, Vu;
function NR() {
  if (Vu)
    return Eo;
  Vu = 1;
  var n = we;
  return Eo = n.isStandardBrowserEnv() ? function() {
    var a = /(msie|trident)/i.test(navigator.userAgent), l = document.createElement("a"), c;
    function f(p) {
      var V = p;
      return a && (l.setAttribute("href", V), V = l.href), l.setAttribute("href", V), {
        href: l.href,
        protocol: l.protocol ? l.protocol.replace(/:$/, "") : "",
        host: l.host,
        search: l.search ? l.search.replace(/^\?/, "") : "",
        hash: l.hash ? l.hash.replace(/^#/, "") : "",
        hostname: l.hostname,
        port: l.port,
        pathname: l.pathname.charAt(0) === "/" ? l.pathname : "/" + l.pathname
      };
    }
    return c = f(window.location.href), function(V) {
      var v = n.isString(V) ? f(V) : V;
      return v.protocol === c.protocol && v.host === c.host;
    };
  }() : function() {
    return function() {
      return !0;
    };
  }(), Eo;
}
var Fo, Tu;
function ei() {
  if (Tu)
    return Fo;
  Tu = 1;
  var n = Xn, i = we;
  function a(l) {
    n.call(this, l == null ? "canceled" : l, n.ERR_CANCELED), this.name = "CanceledError";
  }
  return i.inherits(a, n, {
    __CANCEL__: !0
  }), Fo = a, Fo;
}
var So, Uu;
function wR() {
  return Uu || (Uu = 1, So = function(i) {
    var a = /^([-+\w]{1,25})(:?\/\/|:)/.exec(i);
    return a && a[1] || "";
  }), So;
}
var Ao, Ru;
function vu() {
  if (Ru)
    return Ao;
  Ru = 1;
  var n = we, i = pR(), a = mR(), l = Yd, c = Ld, f = vR(), p = NR(), V = jd, v = Xn, k = ei(), J = wR();
  return Ao = function(g) {
    return new Promise(function(S, O) {
      var G = g.data, B = g.headers, _ = g.responseType, ne;
      function oe() {
        g.cancelToken && g.cancelToken.unsubscribe(ne), g.signal && g.signal.removeEventListener("abort", ne);
      }
      n.isFormData(G) && n.isStandardBrowserEnv() && delete B["Content-Type"];
      var F = new XMLHttpRequest();
      if (g.auth) {
        var Ue = g.auth.username || "", fe = g.auth.password ? unescape(encodeURIComponent(g.auth.password)) : "";
        B.Authorization = "Basic " + btoa(Ue + ":" + fe);
      }
      var ee = c(g.baseURL, g.url);
      F.open(g.method.toUpperCase(), l(ee, g.params, g.paramsSerializer), !0), F.timeout = g.timeout;
      function pt() {
        if (!!F) {
          var Je = "getAllResponseHeaders" in F ? f(F.getAllResponseHeaders()) : null, mt = !_ || _ === "text" || _ === "json" ? F.responseText : F.response, je = {
            data: mt,
            status: F.status,
            statusText: F.statusText,
            headers: Je,
            config: g,
            request: F
          };
          i(function(Ee) {
            S(Ee), oe();
          }, function(Ee) {
            O(Ee), oe();
          }, je), F = null;
        }
      }
      if ("onloadend" in F ? F.onloadend = pt : F.onreadystatechange = function() {
        !F || F.readyState !== 4 || F.status === 0 && !(F.responseURL && F.responseURL.indexOf("file:") === 0) || setTimeout(pt);
      }, F.onabort = function() {
        !F || (O(new v("Request aborted", v.ECONNABORTED, g, F)), F = null);
      }, F.onerror = function() {
        O(new v("Network Error", v.ERR_NETWORK, g, F, F)), F = null;
      }, F.ontimeout = function() {
        var mt = g.timeout ? "timeout of " + g.timeout + "ms exceeded" : "timeout exceeded", je = g.transitional || V;
        g.timeoutErrorMessage && (mt = g.timeoutErrorMessage), O(new v(
          mt,
          je.clarifyTimeoutError ? v.ETIMEDOUT : v.ECONNABORTED,
          g,
          F
        )), F = null;
      }, n.isStandardBrowserEnv()) {
        var wt = (g.withCredentials || p(ee)) && g.xsrfCookieName ? a.read(g.xsrfCookieName) : void 0;
        wt && (B[g.xsrfHeaderName] = wt);
      }
      "setRequestHeader" in F && n.forEach(B, function(mt, je) {
        typeof G > "u" && je.toLowerCase() === "content-type" ? delete B[je] : F.setRequestHeader(je, mt);
      }), n.isUndefined(g.withCredentials) || (F.withCredentials = !!g.withCredentials), _ && _ !== "json" && (F.responseType = g.responseType), typeof g.onDownloadProgress == "function" && F.addEventListener("progress", g.onDownloadProgress), typeof g.onUploadProgress == "function" && F.upload && F.upload.addEventListener("progress", g.onUploadProgress), (g.cancelToken || g.signal) && (ne = function(Je) {
        !F || (O(!Je || Je && Je.type ? new k() : Je), F.abort(), F = null);
      }, g.cancelToken && g.cancelToken.subscribe(ne), g.signal && (g.signal.aborted ? ne() : g.signal.addEventListener("abort", ne))), G || (G = null);
      var nn = J(ee);
      if (nn && ["http", "https", "file"].indexOf(nn) === -1) {
        O(new v("Unsupported protocol " + nn + ":", v.ERR_BAD_REQUEST, g));
        return;
      }
      F.send(G);
    });
  }, Ao;
}
var xo, Nu;
function MR() {
  return Nu || (Nu = 1, xo = null), xo;
}
var Ve = we, wu = fR, Mu = Xn, ZR = jd, JR = Xd, gR = {
  "Content-Type": "application/x-www-form-urlencoded"
};
function Zu(n, i) {
  !Ve.isUndefined(n) && Ve.isUndefined(n["Content-Type"]) && (n["Content-Type"] = i);
}
function kR() {
  var n;
  return (typeof XMLHttpRequest < "u" || typeof process < "u" && Object.prototype.toString.call(process) === "[object process]") && (n = vu()), n;
}
function yR(n, i, a) {
  if (Ve.isString(n))
    try {
      return (i || JSON.parse)(n), Ve.trim(n);
    } catch (l) {
      if (l.name !== "SyntaxError")
        throw l;
    }
  return (a || JSON.stringify)(n);
}
var ti = {
  transitional: ZR,
  adapter: kR(),
  transformRequest: [function(i, a) {
    if (wu(a, "Accept"), wu(a, "Content-Type"), Ve.isFormData(i) || Ve.isArrayBuffer(i) || Ve.isBuffer(i) || Ve.isStream(i) || Ve.isFile(i) || Ve.isBlob(i))
      return i;
    if (Ve.isArrayBufferView(i))
      return i.buffer;
    if (Ve.isURLSearchParams(i))
      return Zu(a, "application/x-www-form-urlencoded;charset=utf-8"), i.toString();
    var l = Ve.isObject(i), c = a && a["Content-Type"], f;
    if ((f = Ve.isFileList(i)) || l && c === "multipart/form-data") {
      var p = this.env && this.env.FormData;
      return JR(f ? { "files[]": i } : i, p && new p());
    } else if (l || c === "application/json")
      return Zu(a, "application/json"), yR(i);
    return i;
  }],
  transformResponse: [function(i) {
    var a = this.transitional || ti.transitional, l = a && a.silentJSONParsing, c = a && a.forcedJSONParsing, f = !l && this.responseType === "json";
    if (f || c && Ve.isString(i) && i.length)
      try {
        return JSON.parse(i);
      } catch (p) {
        if (f)
          throw p.name === "SyntaxError" ? Mu.from(p, Mu.ERR_BAD_RESPONSE, this, null, this.response) : p;
      }
    return i;
  }],
  timeout: 0,
  xsrfCookieName: "XSRF-TOKEN",
  xsrfHeaderName: "X-XSRF-TOKEN",
  maxContentLength: -1,
  maxBodyLength: -1,
  env: {
    FormData: MR()
  },
  validateStatus: function(i) {
    return i >= 200 && i < 300;
  },
  headers: {
    common: {
      Accept: "application/json, text/plain, */*"
    }
  }
};
Ve.forEach(["delete", "get", "head"], function(i) {
  ti.headers[i] = {};
});
Ve.forEach(["post", "put", "patch"], function(i) {
  ti.headers[i] = Ve.merge(gR);
});
var wl = ti, bR = we, WR = wl, ER = function(i, a, l) {
  var c = this || WR;
  return bR.forEach(l, function(p) {
    i = p.call(c, i, a);
  }), i;
}, Qo, Ju;
function _d() {
  return Ju || (Ju = 1, Qo = function(i) {
    return !!(i && i.__CANCEL__);
  }), Qo;
}
var gu = we, Bo = ER, FR = _d(), SR = wl, AR = ei();
function Do(n) {
  if (n.cancelToken && n.cancelToken.throwIfRequested(), n.signal && n.signal.aborted)
    throw new AR();
}
var xR = function(i) {
  Do(i), i.headers = i.headers || {}, i.data = Bo.call(
    i,
    i.data,
    i.headers,
    i.transformRequest
  ), i.headers = gu.merge(
    i.headers.common || {},
    i.headers[i.method] || {},
    i.headers
  ), gu.forEach(
    ["delete", "get", "head", "post", "put", "patch", "common"],
    function(c) {
      delete i.headers[c];
    }
  );
  var a = i.adapter || SR.adapter;
  return a(i).then(function(c) {
    return Do(i), c.data = Bo.call(
      i,
      c.data,
      c.headers,
      i.transformResponse
    ), c;
  }, function(c) {
    return FR(c) || (Do(i), c && c.response && (c.response.data = Bo.call(
      i,
      c.response.data,
      c.response.headers,
      i.transformResponse
    ))), Promise.reject(c);
  });
}, Ye = we, qd = function(i, a) {
  a = a || {};
  var l = {};
  function c(J, y) {
    return Ye.isPlainObject(J) && Ye.isPlainObject(y) ? Ye.merge(J, y) : Ye.isPlainObject(y) ? Ye.merge({}, y) : Ye.isArray(y) ? y.slice() : y;
  }
  function f(J) {
    if (Ye.isUndefined(a[J])) {
      if (!Ye.isUndefined(i[J]))
        return c(void 0, i[J]);
    } else
      return c(i[J], a[J]);
  }
  function p(J) {
    if (!Ye.isUndefined(a[J]))
      return c(void 0, a[J]);
  }
  function V(J) {
    if (Ye.isUndefined(a[J])) {
      if (!Ye.isUndefined(i[J]))
        return c(void 0, i[J]);
    } else
      return c(void 0, a[J]);
  }
  function v(J) {
    if (J in a)
      return c(i[J], a[J]);
    if (J in i)
      return c(void 0, i[J]);
  }
  var k = {
    url: p,
    method: p,
    data: p,
    baseURL: V,
    transformRequest: V,
    transformResponse: V,
    paramsSerializer: V,
    timeout: V,
    timeoutMessage: V,
    withCredentials: V,
    adapter: V,
    responseType: V,
    xsrfCookieName: V,
    xsrfHeaderName: V,
    onUploadProgress: V,
    onDownloadProgress: V,
    decompress: V,
    maxContentLength: V,
    maxBodyLength: V,
    beforeRedirect: V,
    transport: V,
    httpAgent: V,
    httpsAgent: V,
    cancelToken: V,
    socketPath: V,
    responseEncoding: V,
    validateStatus: v
  };
  return Ye.forEach(Object.keys(i).concat(Object.keys(a)), function(y) {
    var g = k[y] || f, z = g(y);
    Ye.isUndefined(z) && g !== v || (l[y] = z);
  }), l;
}, Oo, ku;
function Kd() {
  return ku || (ku = 1, Oo = {
    version: "0.27.2"
  }), Oo;
}
var QR = Kd().version, Lt = Xn, Ml = {};
["object", "boolean", "number", "function", "string", "symbol"].forEach(function(n, i) {
  Ml[n] = function(l) {
    return typeof l === n || "a" + (i < 1 ? "n " : " ") + n;
  };
});
var yu = {};
Ml.transitional = function(i, a, l) {
  function c(f, p) {
    return "[Axios v" + QR + "] Transitional option '" + f + "'" + p + (l ? ". " + l : "");
  }
  return function(f, p, V) {
    if (i === !1)
      throw new Lt(
        c(p, " has been removed" + (a ? " in " + a : "")),
        Lt.ERR_DEPRECATED
      );
    return a && !yu[p] && (yu[p] = !0, console.warn(
      c(
        p,
        " has been deprecated since v" + a + " and will be removed in the near future"
      )
    )), i ? i(f, p, V) : !0;
  };
};
function BR(n, i, a) {
  if (typeof n != "object")
    throw new Lt("options must be an object", Lt.ERR_BAD_OPTION_VALUE);
  for (var l = Object.keys(n), c = l.length; c-- > 0; ) {
    var f = l[c], p = i[f];
    if (p) {
      var V = n[f], v = V === void 0 || p(V, f, n);
      if (v !== !0)
        throw new Lt("option " + f + " must be " + v, Lt.ERR_BAD_OPTION_VALUE);
      continue;
    }
    if (a !== !0)
      throw new Lt("Unknown option " + f, Lt.ERR_BAD_OPTION);
  }
}
var DR = {
  assertOptions: BR,
  validators: Ml
}, $d = we, OR = Yd, bu = uR, Wu = xR, ni = qd, GR = Ld, ef = DR, Qn = ef.validators;
function Hn(n) {
  this.defaults = n, this.interceptors = {
    request: new bu(),
    response: new bu()
  };
}
Hn.prototype.request = function(i, a) {
  typeof i == "string" ? (a = a || {}, a.url = i) : a = i || {}, a = ni(this.defaults, a), a.method ? a.method = a.method.toLowerCase() : this.defaults.method ? a.method = this.defaults.method.toLowerCase() : a.method = "get";
  var l = a.transitional;
  l !== void 0 && ef.assertOptions(l, {
    silentJSONParsing: Qn.transitional(Qn.boolean),
    forcedJSONParsing: Qn.transitional(Qn.boolean),
    clarifyTimeoutError: Qn.transitional(Qn.boolean)
  }, !1);
  var c = [], f = !0;
  this.interceptors.request.forEach(function(z) {
    typeof z.runWhen == "function" && z.runWhen(a) === !1 || (f = f && z.synchronous, c.unshift(z.fulfilled, z.rejected));
  });
  var p = [];
  this.interceptors.response.forEach(function(z) {
    p.push(z.fulfilled, z.rejected);
  });
  var V;
  if (!f) {
    var v = [Wu, void 0];
    for (Array.prototype.unshift.apply(v, c), v = v.concat(p), V = Promise.resolve(a); v.length; )
      V = V.then(v.shift(), v.shift());
    return V;
  }
  for (var k = a; c.length; ) {
    var J = c.shift(), y = c.shift();
    try {
      k = J(k);
    } catch (g) {
      y(g);
      break;
    }
  }
  try {
    V = Wu(k);
  } catch (g) {
    return Promise.reject(g);
  }
  for (; p.length; )
    V = V.then(p.shift(), p.shift());
  return V;
};
Hn.prototype.getUri = function(i) {
  i = ni(this.defaults, i);
  var a = GR(i.baseURL, i.url);
  return OR(a, i.params, i.paramsSerializer);
};
$d.forEach(["delete", "get", "head", "options"], function(i) {
  Hn.prototype[i] = function(a, l) {
    return this.request(ni(l || {}, {
      method: i,
      url: a,
      data: (l || {}).data
    }));
  };
});
$d.forEach(["post", "put", "patch"], function(i) {
  function a(l) {
    return function(f, p, V) {
      return this.request(ni(V || {}, {
        method: i,
        headers: l ? {
          "Content-Type": "multipart/form-data"
        } : {},
        url: f,
        data: p
      }));
    };
  }
  Hn.prototype[i] = a(), Hn.prototype[i + "Form"] = a(!0);
});
var CR = Hn, Go, Eu;
function zR() {
  if (Eu)
    return Go;
  Eu = 1;
  var n = ei();
  function i(a) {
    if (typeof a != "function")
      throw new TypeError("executor must be a function.");
    var l;
    this.promise = new Promise(function(p) {
      l = p;
    });
    var c = this;
    this.promise.then(function(f) {
      if (!!c._listeners) {
        var p, V = c._listeners.length;
        for (p = 0; p < V; p++)
          c._listeners[p](f);
        c._listeners = null;
      }
    }), this.promise.then = function(f) {
      var p, V = new Promise(function(v) {
        c.subscribe(v), p = v;
      }).then(f);
      return V.cancel = function() {
        c.unsubscribe(p);
      }, V;
    }, a(function(p) {
      c.reason || (c.reason = new n(p), l(c.reason));
    });
  }
  return i.prototype.throwIfRequested = function() {
    if (this.reason)
      throw this.reason;
  }, i.prototype.subscribe = function(l) {
    if (this.reason) {
      l(this.reason);
      return;
    }
    this._listeners ? this._listeners.push(l) : this._listeners = [l];
  }, i.prototype.unsubscribe = function(l) {
    if (!!this._listeners) {
      var c = this._listeners.indexOf(l);
      c !== -1 && this._listeners.splice(c, 1);
    }
  }, i.source = function() {
    var l, c = new i(function(p) {
      l = p;
    });
    return {
      token: c,
      cancel: l
    };
  }, Go = i, Go;
}
var Co, Fu;
function YR() {
  return Fu || (Fu = 1, Co = function(i) {
    return function(l) {
      return i.apply(null, l);
    };
  }), Co;
}
var zo, Su;
function IR() {
  if (Su)
    return zo;
  Su = 1;
  var n = we;
  return zo = function(a) {
    return n.isObject(a) && a.isAxiosError === !0;
  }, zo;
}
var Au = we, HR = Gd, za = CR, PR = qd, jR = wl;
function tf(n) {
  var i = new za(n), a = HR(za.prototype.request, i);
  return Au.extend(a, za.prototype, i), Au.extend(a, i), a.create = function(c) {
    return tf(PR(n, c));
  }, a;
}
var Qe = tf(jR);
Qe.Axios = za;
Qe.CanceledError = ei();
Qe.CancelToken = zR();
Qe.isCancel = _d();
Qe.VERSION = Kd().version;
Qe.toFormData = Xd;
Qe.AxiosError = Xn;
Qe.Cancel = Qe.CanceledError;
Qe.all = function(i) {
  return Promise.all(i);
};
Qe.spread = YR();
Qe.isAxiosError = IR();
Vl.exports = Qe;
Vl.exports.default = Qe;
(function(n) {
  n.exports = Vl.exports;
})(Od);
const nf = /* @__PURE__ */ C0(Od.exports), sl = nf.create({
  baseURL: "/opennms/api/v2".toString() || "/opennms/api/v2",
  withCredentials: !0
}), Ln = nf.create({
  baseURL: "/opennms/rest".toString() || "/opennms/rest",
  withCredentials: !0
});
var $t = { exports: {} };
/**
 * @license
 * Lodash <https://lodash.com/>
 * Copyright OpenJS Foundation and other contributors <https://openjsf.org/>
 * Released under MIT license <https://lodash.com/license>
 * Based on Underscore.js 1.8.3 <http://underscorejs.org/LICENSE>
 * Copyright Jeremy Ashkenas, DocumentCloud and Investigative Reporters & Editors
 */
(function(n, i) {
  (function() {
    var a, l = "4.17.21", c = 200, f = "Unsupported core-js use. Try https://npms.io/search?q=ponyfill.", p = "Expected a function", V = "Invalid `variable` option passed into `_.template`", v = "__lodash_hash_undefined__", k = 500, J = "__lodash_placeholder__", y = 1, g = 2, z = 4, S = 1, O = 2, G = 1, B = 2, _ = 4, ne = 8, oe = 16, F = 32, Ue = 64, fe = 128, ee = 256, pt = 512, wt = 30, nn = "...", Je = 800, mt = 16, je = 1, Re = 2, Ee = 3, Xe = 1 / 0, Bt = 9007199254740991, Gf = 17976931348623157e292, Br = 0 / 0, Vt = 4294967295, Cf = Vt - 1, zf = Vt >>> 1, Yf = [
      ["ary", fe],
      ["bind", G],
      ["bindKey", B],
      ["curry", ne],
      ["curryRight", oe],
      ["flip", pt],
      ["partial", F],
      ["partialRight", Ue],
      ["rearg", ee]
    ], Rn = "[object Arguments]", Dr = "[object Array]", If = "[object AsyncFunction]", qn = "[object Boolean]", Kn = "[object Date]", Hf = "[object DOMException]", Or = "[object Error]", Gr = "[object Function]", xl = "[object GeneratorFunction]", it = "[object Map]", $n = "[object Number]", Pf = "[object Null]", Mt = "[object Object]", Ql = "[object Promise]", jf = "[object Proxy]", er = "[object RegExp]", ot = "[object Set]", tr = "[object String]", Cr = "[object Symbol]", Xf = "[object Undefined]", nr = "[object WeakMap]", Lf = "[object WeakSet]", rr = "[object ArrayBuffer]", vn = "[object DataView]", oi = "[object Float32Array]", li = "[object Float64Array]", si = "[object Int8Array]", ci = "[object Int16Array]", ui = "[object Int32Array]", di = "[object Uint8Array]", fi = "[object Uint8ClampedArray]", hi = "[object Uint16Array]", pi = "[object Uint32Array]", _f = /\b__p \+= '';/g, qf = /\b(__p \+=) '' \+/g, Kf = /(__e\(.*?\)|\b__t\)) \+\n'';/g, Bl = /&(?:amp|lt|gt|quot|#39);/g, Dl = /[&<>"']/g, $f = RegExp(Bl.source), eh = RegExp(Dl.source), th = /<%-([\s\S]+?)%>/g, nh = /<%([\s\S]+?)%>/g, Ol = /<%=([\s\S]+?)%>/g, rh = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, ah = /^\w*$/, ih = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g, mi = /[\\^$.*+?()[\]{}|]/g, oh = RegExp(mi.source), Vi = /^\s+/, lh = /\s/, sh = /\{(?:\n\/\* \[wrapped with .+\] \*\/)?\n?/, ch = /\{\n\/\* \[wrapped with (.+)\] \*/, uh = /,? & /, dh = /[^\x00-\x2f\x3a-\x40\x5b-\x60\x7b-\x7f]+/g, fh = /[()=,{}\[\]\/\s]/, hh = /\\(\\)?/g, ph = /\$\{([^\\}]*(?:\\.[^\\}]*)*)\}/g, Gl = /\w*$/, mh = /^[-+]0x[0-9a-f]+$/i, Vh = /^0b[01]+$/i, Th = /^\[object .+?Constructor\]$/, Uh = /^0o[0-7]+$/i, Rh = /^(?:0|[1-9]\d*)$/, vh = /[\xc0-\xd6\xd8-\xf6\xf8-\xff\u0100-\u017f]/g, zr = /($^)/, Nh = /['\n\r\u2028\u2029\\]/g, Yr = "\\ud800-\\udfff", wh = "\\u0300-\\u036f", Mh = "\\ufe20-\\ufe2f", Zh = "\\u20d0-\\u20ff", Cl = wh + Mh + Zh, zl = "\\u2700-\\u27bf", Yl = "a-z\\xdf-\\xf6\\xf8-\\xff", Jh = "\\xac\\xb1\\xd7\\xf7", gh = "\\x00-\\x2f\\x3a-\\x40\\x5b-\\x60\\x7b-\\xbf", kh = "\\u2000-\\u206f", yh = " \\t\\x0b\\f\\xa0\\ufeff\\n\\r\\u2028\\u2029\\u1680\\u180e\\u2000\\u2001\\u2002\\u2003\\u2004\\u2005\\u2006\\u2007\\u2008\\u2009\\u200a\\u202f\\u205f\\u3000", Il = "A-Z\\xc0-\\xd6\\xd8-\\xde", Hl = "\\ufe0e\\ufe0f", Pl = Jh + gh + kh + yh, Ti = "['\u2019]", bh = "[" + Yr + "]", jl = "[" + Pl + "]", Ir = "[" + Cl + "]", Xl = "\\d+", Wh = "[" + zl + "]", Ll = "[" + Yl + "]", _l = "[^" + Yr + Pl + Xl + zl + Yl + Il + "]", Ui = "\\ud83c[\\udffb-\\udfff]", Eh = "(?:" + Ir + "|" + Ui + ")", ql = "[^" + Yr + "]", Ri = "(?:\\ud83c[\\udde6-\\uddff]){2}", vi = "[\\ud800-\\udbff][\\udc00-\\udfff]", Nn = "[" + Il + "]", Kl = "\\u200d", $l = "(?:" + Ll + "|" + _l + ")", Fh = "(?:" + Nn + "|" + _l + ")", es = "(?:" + Ti + "(?:d|ll|m|re|s|t|ve))?", ts = "(?:" + Ti + "(?:D|LL|M|RE|S|T|VE))?", ns = Eh + "?", rs = "[" + Hl + "]?", Sh = "(?:" + Kl + "(?:" + [ql, Ri, vi].join("|") + ")" + rs + ns + ")*", Ah = "\\d*(?:1st|2nd|3rd|(?![123])\\dth)(?=\\b|[A-Z_])", xh = "\\d*(?:1ST|2ND|3RD|(?![123])\\dTH)(?=\\b|[a-z_])", as = rs + ns + Sh, Qh = "(?:" + [Wh, Ri, vi].join("|") + ")" + as, Bh = "(?:" + [ql + Ir + "?", Ir, Ri, vi, bh].join("|") + ")", Dh = RegExp(Ti, "g"), Oh = RegExp(Ir, "g"), Ni = RegExp(Ui + "(?=" + Ui + ")|" + Bh + as, "g"), Gh = RegExp([
      Nn + "?" + Ll + "+" + es + "(?=" + [jl, Nn, "$"].join("|") + ")",
      Fh + "+" + ts + "(?=" + [jl, Nn + $l, "$"].join("|") + ")",
      Nn + "?" + $l + "+" + es,
      Nn + "+" + ts,
      xh,
      Ah,
      Xl,
      Qh
    ].join("|"), "g"), Ch = RegExp("[" + Kl + Yr + Cl + Hl + "]"), zh = /[a-z][A-Z]|[A-Z]{2}[a-z]|[0-9][a-zA-Z]|[a-zA-Z][0-9]|[^a-zA-Z0-9 ]/, Yh = [
      "Array",
      "Buffer",
      "DataView",
      "Date",
      "Error",
      "Float32Array",
      "Float64Array",
      "Function",
      "Int8Array",
      "Int16Array",
      "Int32Array",
      "Map",
      "Math",
      "Object",
      "Promise",
      "RegExp",
      "Set",
      "String",
      "Symbol",
      "TypeError",
      "Uint8Array",
      "Uint8ClampedArray",
      "Uint16Array",
      "Uint32Array",
      "WeakMap",
      "_",
      "clearTimeout",
      "isFinite",
      "parseInt",
      "setTimeout"
    ], Ih = -1, ae = {};
    ae[oi] = ae[li] = ae[si] = ae[ci] = ae[ui] = ae[di] = ae[fi] = ae[hi] = ae[pi] = !0, ae[Rn] = ae[Dr] = ae[rr] = ae[qn] = ae[vn] = ae[Kn] = ae[Or] = ae[Gr] = ae[it] = ae[$n] = ae[Mt] = ae[er] = ae[ot] = ae[tr] = ae[nr] = !1;
    var re = {};
    re[Rn] = re[Dr] = re[rr] = re[vn] = re[qn] = re[Kn] = re[oi] = re[li] = re[si] = re[ci] = re[ui] = re[it] = re[$n] = re[Mt] = re[er] = re[ot] = re[tr] = re[Cr] = re[di] = re[fi] = re[hi] = re[pi] = !0, re[Or] = re[Gr] = re[nr] = !1;
    var Hh = {
      \u00C0: "A",
      \u00C1: "A",
      \u00C2: "A",
      \u00C3: "A",
      \u00C4: "A",
      \u00C5: "A",
      \u00E0: "a",
      \u00E1: "a",
      \u00E2: "a",
      \u00E3: "a",
      \u00E4: "a",
      \u00E5: "a",
      \u00C7: "C",
      \u00E7: "c",
      \u00D0: "D",
      \u00F0: "d",
      \u00C8: "E",
      \u00C9: "E",
      \u00CA: "E",
      \u00CB: "E",
      \u00E8: "e",
      \u00E9: "e",
      \u00EA: "e",
      \u00EB: "e",
      \u00CC: "I",
      \u00CD: "I",
      \u00CE: "I",
      \u00CF: "I",
      \u00EC: "i",
      \u00ED: "i",
      \u00EE: "i",
      \u00EF: "i",
      \u00D1: "N",
      \u00F1: "n",
      \u00D2: "O",
      \u00D3: "O",
      \u00D4: "O",
      \u00D5: "O",
      \u00D6: "O",
      \u00D8: "O",
      \u00F2: "o",
      \u00F3: "o",
      \u00F4: "o",
      \u00F5: "o",
      \u00F6: "o",
      \u00F8: "o",
      \u00D9: "U",
      \u00DA: "U",
      \u00DB: "U",
      \u00DC: "U",
      \u00F9: "u",
      \u00FA: "u",
      \u00FB: "u",
      \u00FC: "u",
      \u00DD: "Y",
      \u00FD: "y",
      \u00FF: "y",
      \u00C6: "Ae",
      \u00E6: "ae",
      \u00DE: "Th",
      \u00FE: "th",
      \u00DF: "ss",
      \u0100: "A",
      \u0102: "A",
      \u0104: "A",
      \u0101: "a",
      \u0103: "a",
      \u0105: "a",
      \u0106: "C",
      \u0108: "C",
      \u010A: "C",
      \u010C: "C",
      \u0107: "c",
      \u0109: "c",
      \u010B: "c",
      \u010D: "c",
      \u010E: "D",
      \u0110: "D",
      \u010F: "d",
      \u0111: "d",
      \u0112: "E",
      \u0114: "E",
      \u0116: "E",
      \u0118: "E",
      \u011A: "E",
      \u0113: "e",
      \u0115: "e",
      \u0117: "e",
      \u0119: "e",
      \u011B: "e",
      \u011C: "G",
      \u011E: "G",
      \u0120: "G",
      \u0122: "G",
      \u011D: "g",
      \u011F: "g",
      \u0121: "g",
      \u0123: "g",
      \u0124: "H",
      \u0126: "H",
      \u0125: "h",
      \u0127: "h",
      \u0128: "I",
      \u012A: "I",
      \u012C: "I",
      \u012E: "I",
      \u0130: "I",
      \u0129: "i",
      \u012B: "i",
      \u012D: "i",
      \u012F: "i",
      \u0131: "i",
      \u0134: "J",
      \u0135: "j",
      \u0136: "K",
      \u0137: "k",
      \u0138: "k",
      \u0139: "L",
      \u013B: "L",
      \u013D: "L",
      \u013F: "L",
      \u0141: "L",
      \u013A: "l",
      \u013C: "l",
      \u013E: "l",
      \u0140: "l",
      \u0142: "l",
      \u0143: "N",
      \u0145: "N",
      \u0147: "N",
      \u014A: "N",
      \u0144: "n",
      \u0146: "n",
      \u0148: "n",
      \u014B: "n",
      \u014C: "O",
      \u014E: "O",
      \u0150: "O",
      \u014D: "o",
      \u014F: "o",
      \u0151: "o",
      \u0154: "R",
      \u0156: "R",
      \u0158: "R",
      \u0155: "r",
      \u0157: "r",
      \u0159: "r",
      \u015A: "S",
      \u015C: "S",
      \u015E: "S",
      \u0160: "S",
      \u015B: "s",
      \u015D: "s",
      \u015F: "s",
      \u0161: "s",
      \u0162: "T",
      \u0164: "T",
      \u0166: "T",
      \u0163: "t",
      \u0165: "t",
      \u0167: "t",
      \u0168: "U",
      \u016A: "U",
      \u016C: "U",
      \u016E: "U",
      \u0170: "U",
      \u0172: "U",
      \u0169: "u",
      \u016B: "u",
      \u016D: "u",
      \u016F: "u",
      \u0171: "u",
      \u0173: "u",
      \u0174: "W",
      \u0175: "w",
      \u0176: "Y",
      \u0177: "y",
      \u0178: "Y",
      \u0179: "Z",
      \u017B: "Z",
      \u017D: "Z",
      \u017A: "z",
      \u017C: "z",
      \u017E: "z",
      \u0132: "IJ",
      \u0133: "ij",
      \u0152: "Oe",
      \u0153: "oe",
      \u0149: "'n",
      \u017F: "s"
    }, Ph = {
      "&": "&amp;",
      "<": "&lt;",
      ">": "&gt;",
      '"': "&quot;",
      "'": "&#39;"
    }, jh = {
      "&amp;": "&",
      "&lt;": "<",
      "&gt;": ">",
      "&quot;": '"',
      "&#39;": "'"
    }, Xh = {
      "\\": "\\",
      "'": "'",
      "\n": "n",
      "\r": "r",
      "\u2028": "u2028",
      "\u2029": "u2029"
    }, Lh = parseFloat, _h = parseInt, is = typeof Rr == "object" && Rr && Rr.Object === Object && Rr, qh = typeof self == "object" && self && self.Object === Object && self, ve = is || qh || Function("return this")(), wi = i && !i.nodeType && i, rn = wi && !0 && n && !n.nodeType && n, os = rn && rn.exports === wi, Mi = os && is.process, Le = function() {
      try {
        var T = rn && rn.require && rn.require("util").types;
        return T || Mi && Mi.binding && Mi.binding("util");
      } catch {
      }
    }(), ls = Le && Le.isArrayBuffer, ss = Le && Le.isDate, cs = Le && Le.isMap, us = Le && Le.isRegExp, ds = Le && Le.isSet, fs = Le && Le.isTypedArray;
    function Be(T, N, R) {
      switch (R.length) {
        case 0:
          return T.call(N);
        case 1:
          return T.call(N, R[0]);
        case 2:
          return T.call(N, R[0], R[1]);
        case 3:
          return T.call(N, R[0], R[1], R[2]);
      }
      return T.apply(N, R);
    }
    function Kh(T, N, R, W) {
      for (var D = -1, L = T == null ? 0 : T.length; ++D < L; ) {
        var he = T[D];
        N(W, he, R(he), T);
      }
      return W;
    }
    function _e(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W && N(T[R], R, T) !== !1; )
        ;
      return T;
    }
    function $h(T, N) {
      for (var R = T == null ? 0 : T.length; R-- && N(T[R], R, T) !== !1; )
        ;
      return T;
    }
    function hs(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W; )
        if (!N(T[R], R, T))
          return !1;
      return !0;
    }
    function Dt(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length, D = 0, L = []; ++R < W; ) {
        var he = T[R];
        N(he, R, T) && (L[D++] = he);
      }
      return L;
    }
    function Hr(T, N) {
      var R = T == null ? 0 : T.length;
      return !!R && wn(T, N, 0) > -1;
    }
    function Zi(T, N, R) {
      for (var W = -1, D = T == null ? 0 : T.length; ++W < D; )
        if (R(N, T[W]))
          return !0;
      return !1;
    }
    function ie(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length, D = Array(W); ++R < W; )
        D[R] = N(T[R], R, T);
      return D;
    }
    function Ot(T, N) {
      for (var R = -1, W = N.length, D = T.length; ++R < W; )
        T[D + R] = N[R];
      return T;
    }
    function Ji(T, N, R, W) {
      var D = -1, L = T == null ? 0 : T.length;
      for (W && L && (R = T[++D]); ++D < L; )
        R = N(R, T[D], D, T);
      return R;
    }
    function ep(T, N, R, W) {
      var D = T == null ? 0 : T.length;
      for (W && D && (R = T[--D]); D--; )
        R = N(R, T[D], D, T);
      return R;
    }
    function gi(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W; )
        if (N(T[R], R, T))
          return !0;
      return !1;
    }
    var tp = ki("length");
    function np(T) {
      return T.split("");
    }
    function rp(T) {
      return T.match(dh) || [];
    }
    function ps(T, N, R) {
      var W;
      return R(T, function(D, L, he) {
        if (N(D, L, he))
          return W = L, !1;
      }), W;
    }
    function Pr(T, N, R, W) {
      for (var D = T.length, L = R + (W ? 1 : -1); W ? L-- : ++L < D; )
        if (N(T[L], L, T))
          return L;
      return -1;
    }
    function wn(T, N, R) {
      return N === N ? mp(T, N, R) : Pr(T, ms, R);
    }
    function ap(T, N, R, W) {
      for (var D = R - 1, L = T.length; ++D < L; )
        if (W(T[D], N))
          return D;
      return -1;
    }
    function ms(T) {
      return T !== T;
    }
    function Vs(T, N) {
      var R = T == null ? 0 : T.length;
      return R ? bi(T, N) / R : Br;
    }
    function ki(T) {
      return function(N) {
        return N == null ? a : N[T];
      };
    }
    function yi(T) {
      return function(N) {
        return T == null ? a : T[N];
      };
    }
    function Ts(T, N, R, W, D) {
      return D(T, function(L, he, te) {
        R = W ? (W = !1, L) : N(R, L, he, te);
      }), R;
    }
    function ip(T, N) {
      var R = T.length;
      for (T.sort(N); R--; )
        T[R] = T[R].value;
      return T;
    }
    function bi(T, N) {
      for (var R, W = -1, D = T.length; ++W < D; ) {
        var L = N(T[W]);
        L !== a && (R = R === a ? L : R + L);
      }
      return R;
    }
    function Wi(T, N) {
      for (var R = -1, W = Array(T); ++R < T; )
        W[R] = N(R);
      return W;
    }
    function op(T, N) {
      return ie(N, function(R) {
        return [R, T[R]];
      });
    }
    function Us(T) {
      return T && T.slice(0, ws(T) + 1).replace(Vi, "");
    }
    function De(T) {
      return function(N) {
        return T(N);
      };
    }
    function Ei(T, N) {
      return ie(N, function(R) {
        return T[R];
      });
    }
    function ar(T, N) {
      return T.has(N);
    }
    function Rs(T, N) {
      for (var R = -1, W = T.length; ++R < W && wn(N, T[R], 0) > -1; )
        ;
      return R;
    }
    function vs(T, N) {
      for (var R = T.length; R-- && wn(N, T[R], 0) > -1; )
        ;
      return R;
    }
    function lp(T, N) {
      for (var R = T.length, W = 0; R--; )
        T[R] === N && ++W;
      return W;
    }
    var sp = yi(Hh), cp = yi(Ph);
    function up(T) {
      return "\\" + Xh[T];
    }
    function dp(T, N) {
      return T == null ? a : T[N];
    }
    function Mn(T) {
      return Ch.test(T);
    }
    function fp(T) {
      return zh.test(T);
    }
    function hp(T) {
      for (var N, R = []; !(N = T.next()).done; )
        R.push(N.value);
      return R;
    }
    function Fi(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W, D) {
        R[++N] = [D, W];
      }), R;
    }
    function Ns(T, N) {
      return function(R) {
        return T(N(R));
      };
    }
    function Gt(T, N) {
      for (var R = -1, W = T.length, D = 0, L = []; ++R < W; ) {
        var he = T[R];
        (he === N || he === J) && (T[R] = J, L[D++] = R);
      }
      return L;
    }
    function jr(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W) {
        R[++N] = W;
      }), R;
    }
    function pp(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W) {
        R[++N] = [W, W];
      }), R;
    }
    function mp(T, N, R) {
      for (var W = R - 1, D = T.length; ++W < D; )
        if (T[W] === N)
          return W;
      return -1;
    }
    function Vp(T, N, R) {
      for (var W = R + 1; W--; )
        if (T[W] === N)
          return W;
      return W;
    }
    function Zn(T) {
      return Mn(T) ? Up(T) : tp(T);
    }
    function lt(T) {
      return Mn(T) ? Rp(T) : np(T);
    }
    function ws(T) {
      for (var N = T.length; N-- && lh.test(T.charAt(N)); )
        ;
      return N;
    }
    var Tp = yi(jh);
    function Up(T) {
      for (var N = Ni.lastIndex = 0; Ni.test(T); )
        ++N;
      return N;
    }
    function Rp(T) {
      return T.match(Ni) || [];
    }
    function vp(T) {
      return T.match(Gh) || [];
    }
    var Np = function T(N) {
      N = N == null ? ve : Jn.defaults(ve.Object(), N, Jn.pick(ve, Yh));
      var R = N.Array, W = N.Date, D = N.Error, L = N.Function, he = N.Math, te = N.Object, Si = N.RegExp, wp = N.String, qe = N.TypeError, Xr = R.prototype, Mp = L.prototype, gn = te.prototype, Lr = N["__core-js_shared__"], _r = Mp.toString, K = gn.hasOwnProperty, Zp = 0, Ms = function() {
        var e = /[^.]+$/.exec(Lr && Lr.keys && Lr.keys.IE_PROTO || "");
        return e ? "Symbol(src)_1." + e : "";
      }(), qr = gn.toString, Jp = _r.call(te), gp = ve._, kp = Si(
        "^" + _r.call(K).replace(mi, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"
      ), Kr = os ? N.Buffer : a, Ct = N.Symbol, $r = N.Uint8Array, Zs = Kr ? Kr.allocUnsafe : a, ea = Ns(te.getPrototypeOf, te), Js = te.create, gs = gn.propertyIsEnumerable, ta = Xr.splice, ks = Ct ? Ct.isConcatSpreadable : a, ir = Ct ? Ct.iterator : a, an = Ct ? Ct.toStringTag : a, na = function() {
        try {
          var e = un(te, "defineProperty");
          return e({}, "", {}), e;
        } catch {
        }
      }(), yp = N.clearTimeout !== ve.clearTimeout && N.clearTimeout, bp = W && W.now !== ve.Date.now && W.now, Wp = N.setTimeout !== ve.setTimeout && N.setTimeout, ra = he.ceil, aa = he.floor, Ai = te.getOwnPropertySymbols, Ep = Kr ? Kr.isBuffer : a, ys = N.isFinite, Fp = Xr.join, Sp = Ns(te.keys, te), pe = he.max, Me = he.min, Ap = W.now, xp = N.parseInt, bs = he.random, Qp = Xr.reverse, xi = un(N, "DataView"), or = un(N, "Map"), Qi = un(N, "Promise"), kn = un(N, "Set"), lr = un(N, "WeakMap"), sr = un(te, "create"), ia = lr && new lr(), yn = {}, Bp = dn(xi), Dp = dn(or), Op = dn(Qi), Gp = dn(kn), Cp = dn(lr), oa = Ct ? Ct.prototype : a, cr = oa ? oa.valueOf : a, Ws = oa ? oa.toString : a;
      function u(e) {
        if (se(e) && !C(e) && !(e instanceof j)) {
          if (e instanceof Ke)
            return e;
          if (K.call(e, "__wrapped__"))
            return Ec(e);
        }
        return new Ke(e);
      }
      var bn = function() {
        function e() {
        }
        return function(t) {
          if (!le(t))
            return {};
          if (Js)
            return Js(t);
          e.prototype = t;
          var r = new e();
          return e.prototype = a, r;
        };
      }();
      function la() {
      }
      function Ke(e, t) {
        this.__wrapped__ = e, this.__actions__ = [], this.__chain__ = !!t, this.__index__ = 0, this.__values__ = a;
      }
      u.templateSettings = {
        escape: th,
        evaluate: nh,
        interpolate: Ol,
        variable: "",
        imports: {
          _: u
        }
      }, u.prototype = la.prototype, u.prototype.constructor = u, Ke.prototype = bn(la.prototype), Ke.prototype.constructor = Ke;
      function j(e) {
        this.__wrapped__ = e, this.__actions__ = [], this.__dir__ = 1, this.__filtered__ = !1, this.__iteratees__ = [], this.__takeCount__ = Vt, this.__views__ = [];
      }
      function zp() {
        var e = new j(this.__wrapped__);
        return e.__actions__ = Fe(this.__actions__), e.__dir__ = this.__dir__, e.__filtered__ = this.__filtered__, e.__iteratees__ = Fe(this.__iteratees__), e.__takeCount__ = this.__takeCount__, e.__views__ = Fe(this.__views__), e;
      }
      function Yp() {
        if (this.__filtered__) {
          var e = new j(this);
          e.__dir__ = -1, e.__filtered__ = !0;
        } else
          e = this.clone(), e.__dir__ *= -1;
        return e;
      }
      function Ip() {
        var e = this.__wrapped__.value(), t = this.__dir__, r = C(e), o = t < 0, s = r ? e.length : 0, d = nV(0, s, this.__views__), h = d.start, m = d.end, U = m - h, w = o ? m : h - 1, M = this.__iteratees__, Z = M.length, b = 0, E = Me(U, this.__takeCount__);
        if (!r || !o && s == U && E == U)
          return ec(e, this.__actions__);
        var x = [];
        e:
          for (; U-- && b < E; ) {
            w += t;
            for (var I = -1, Q = e[w]; ++I < Z; ) {
              var P = M[I], X = P.iteratee, Ce = P.type, ye = X(Q);
              if (Ce == Re)
                Q = ye;
              else if (!ye) {
                if (Ce == je)
                  continue e;
                break e;
              }
            }
            x[b++] = Q;
          }
        return x;
      }
      j.prototype = bn(la.prototype), j.prototype.constructor = j;
      function on(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function Hp() {
        this.__data__ = sr ? sr(null) : {}, this.size = 0;
      }
      function Pp(e) {
        var t = this.has(e) && delete this.__data__[e];
        return this.size -= t ? 1 : 0, t;
      }
      function jp(e) {
        var t = this.__data__;
        if (sr) {
          var r = t[e];
          return r === v ? a : r;
        }
        return K.call(t, e) ? t[e] : a;
      }
      function Xp(e) {
        var t = this.__data__;
        return sr ? t[e] !== a : K.call(t, e);
      }
      function Lp(e, t) {
        var r = this.__data__;
        return this.size += this.has(e) ? 0 : 1, r[e] = sr && t === a ? v : t, this;
      }
      on.prototype.clear = Hp, on.prototype.delete = Pp, on.prototype.get = jp, on.prototype.has = Xp, on.prototype.set = Lp;
      function Zt(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function _p() {
        this.__data__ = [], this.size = 0;
      }
      function qp(e) {
        var t = this.__data__, r = sa(t, e);
        if (r < 0)
          return !1;
        var o = t.length - 1;
        return r == o ? t.pop() : ta.call(t, r, 1), --this.size, !0;
      }
      function Kp(e) {
        var t = this.__data__, r = sa(t, e);
        return r < 0 ? a : t[r][1];
      }
      function $p(e) {
        return sa(this.__data__, e) > -1;
      }
      function em(e, t) {
        var r = this.__data__, o = sa(r, e);
        return o < 0 ? (++this.size, r.push([e, t])) : r[o][1] = t, this;
      }
      Zt.prototype.clear = _p, Zt.prototype.delete = qp, Zt.prototype.get = Kp, Zt.prototype.has = $p, Zt.prototype.set = em;
      function Jt(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function tm() {
        this.size = 0, this.__data__ = {
          hash: new on(),
          map: new (or || Zt)(),
          string: new on()
        };
      }
      function nm(e) {
        var t = va(this, e).delete(e);
        return this.size -= t ? 1 : 0, t;
      }
      function rm(e) {
        return va(this, e).get(e);
      }
      function am(e) {
        return va(this, e).has(e);
      }
      function im(e, t) {
        var r = va(this, e), o = r.size;
        return r.set(e, t), this.size += r.size == o ? 0 : 1, this;
      }
      Jt.prototype.clear = tm, Jt.prototype.delete = nm, Jt.prototype.get = rm, Jt.prototype.has = am, Jt.prototype.set = im;
      function ln(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.__data__ = new Jt(); ++t < r; )
          this.add(e[t]);
      }
      function om(e) {
        return this.__data__.set(e, v), this;
      }
      function lm(e) {
        return this.__data__.has(e);
      }
      ln.prototype.add = ln.prototype.push = om, ln.prototype.has = lm;
      function st(e) {
        var t = this.__data__ = new Zt(e);
        this.size = t.size;
      }
      function sm() {
        this.__data__ = new Zt(), this.size = 0;
      }
      function cm(e) {
        var t = this.__data__, r = t.delete(e);
        return this.size = t.size, r;
      }
      function um(e) {
        return this.__data__.get(e);
      }
      function dm(e) {
        return this.__data__.has(e);
      }
      function fm(e, t) {
        var r = this.__data__;
        if (r instanceof Zt) {
          var o = r.__data__;
          if (!or || o.length < c - 1)
            return o.push([e, t]), this.size = ++r.size, this;
          r = this.__data__ = new Jt(o);
        }
        return r.set(e, t), this.size = r.size, this;
      }
      st.prototype.clear = sm, st.prototype.delete = cm, st.prototype.get = um, st.prototype.has = dm, st.prototype.set = fm;
      function Es(e, t) {
        var r = C(e), o = !r && fn(e), s = !r && !o && Pt(e), d = !r && !o && !s && Sn(e), h = r || o || s || d, m = h ? Wi(e.length, wp) : [], U = m.length;
        for (var w in e)
          (t || K.call(e, w)) && !(h && (w == "length" || s && (w == "offset" || w == "parent") || d && (w == "buffer" || w == "byteLength" || w == "byteOffset") || bt(w, U))) && m.push(w);
        return m;
      }
      function Fs(e) {
        var t = e.length;
        return t ? e[ji(0, t - 1)] : a;
      }
      function hm(e, t) {
        return Na(Fe(e), sn(t, 0, e.length));
      }
      function pm(e) {
        return Na(Fe(e));
      }
      function Bi(e, t, r) {
        (r !== a && !ct(e[t], r) || r === a && !(t in e)) && gt(e, t, r);
      }
      function ur(e, t, r) {
        var o = e[t];
        (!(K.call(e, t) && ct(o, r)) || r === a && !(t in e)) && gt(e, t, r);
      }
      function sa(e, t) {
        for (var r = e.length; r--; )
          if (ct(e[r][0], t))
            return r;
        return -1;
      }
      function mm(e, t, r, o) {
        return zt(e, function(s, d, h) {
          t(o, s, r(s), h);
        }), o;
      }
      function Ss(e, t) {
        return e && Ut(t, me(t), e);
      }
      function Vm(e, t) {
        return e && Ut(t, Ae(t), e);
      }
      function gt(e, t, r) {
        t == "__proto__" && na ? na(e, t, {
          configurable: !0,
          enumerable: !0,
          value: r,
          writable: !0
        }) : e[t] = r;
      }
      function Di(e, t) {
        for (var r = -1, o = t.length, s = R(o), d = e == null; ++r < o; )
          s[r] = d ? a : Ro(e, t[r]);
        return s;
      }
      function sn(e, t, r) {
        return e === e && (r !== a && (e = e <= r ? e : r), t !== a && (e = e >= t ? e : t)), e;
      }
      function $e(e, t, r, o, s, d) {
        var h, m = t & y, U = t & g, w = t & z;
        if (r && (h = s ? r(e, o, s, d) : r(e)), h !== a)
          return h;
        if (!le(e))
          return e;
        var M = C(e);
        if (M) {
          if (h = aV(e), !m)
            return Fe(e, h);
        } else {
          var Z = Ze(e), b = Z == Gr || Z == xl;
          if (Pt(e))
            return rc(e, m);
          if (Z == Mt || Z == Rn || b && !s) {
            if (h = U || b ? {} : wc(e), !m)
              return U ? jm(e, Vm(h, e)) : Pm(e, Ss(h, e));
          } else {
            if (!re[Z])
              return s ? e : {};
            h = iV(e, Z, m);
          }
        }
        d || (d = new st());
        var E = d.get(e);
        if (E)
          return E;
        d.set(e, h), qc(e) ? e.forEach(function(Q) {
          h.add($e(Q, t, r, Q, e, d));
        }) : Lc(e) && e.forEach(function(Q, P) {
          h.set(P, $e(Q, t, r, P, e, d));
        });
        var x = w ? U ? ao : ro : U ? Ae : me, I = M ? a : x(e);
        return _e(I || e, function(Q, P) {
          I && (P = Q, Q = e[P]), ur(h, P, $e(Q, t, r, P, e, d));
        }), h;
      }
      function Tm(e) {
        var t = me(e);
        return function(r) {
          return As(r, e, t);
        };
      }
      function As(e, t, r) {
        var o = r.length;
        if (e == null)
          return !o;
        for (e = te(e); o--; ) {
          var s = r[o], d = t[s], h = e[s];
          if (h === a && !(s in e) || !d(h))
            return !1;
        }
        return !0;
      }
      function xs(e, t, r) {
        if (typeof e != "function")
          throw new qe(p);
        return Tr(function() {
          e.apply(a, r);
        }, t);
      }
      function dr(e, t, r, o) {
        var s = -1, d = Hr, h = !0, m = e.length, U = [], w = t.length;
        if (!m)
          return U;
        r && (t = ie(t, De(r))), o ? (d = Zi, h = !1) : t.length >= c && (d = ar, h = !1, t = new ln(t));
        e:
          for (; ++s < m; ) {
            var M = e[s], Z = r == null ? M : r(M);
            if (M = o || M !== 0 ? M : 0, h && Z === Z) {
              for (var b = w; b--; )
                if (t[b] === Z)
                  continue e;
              U.push(M);
            } else
              d(t, Z, o) || U.push(M);
          }
        return U;
      }
      var zt = sc(Tt), Qs = sc(Gi, !0);
      function Um(e, t) {
        var r = !0;
        return zt(e, function(o, s, d) {
          return r = !!t(o, s, d), r;
        }), r;
      }
      function ca(e, t, r) {
        for (var o = -1, s = e.length; ++o < s; ) {
          var d = e[o], h = t(d);
          if (h != null && (m === a ? h === h && !Ge(h) : r(h, m)))
            var m = h, U = d;
        }
        return U;
      }
      function Rm(e, t, r, o) {
        var s = e.length;
        for (r = Y(r), r < 0 && (r = -r > s ? 0 : s + r), o = o === a || o > s ? s : Y(o), o < 0 && (o += s), o = r > o ? 0 : $c(o); r < o; )
          e[r++] = t;
        return e;
      }
      function Bs(e, t) {
        var r = [];
        return zt(e, function(o, s, d) {
          t(o, s, d) && r.push(o);
        }), r;
      }
      function Ne(e, t, r, o, s) {
        var d = -1, h = e.length;
        for (r || (r = lV), s || (s = []); ++d < h; ) {
          var m = e[d];
          t > 0 && r(m) ? t > 1 ? Ne(m, t - 1, r, o, s) : Ot(s, m) : o || (s[s.length] = m);
        }
        return s;
      }
      var Oi = cc(), Ds = cc(!0);
      function Tt(e, t) {
        return e && Oi(e, t, me);
      }
      function Gi(e, t) {
        return e && Ds(e, t, me);
      }
      function ua(e, t) {
        return Dt(t, function(r) {
          return Wt(e[r]);
        });
      }
      function cn(e, t) {
        t = It(t, e);
        for (var r = 0, o = t.length; e != null && r < o; )
          e = e[Rt(t[r++])];
        return r && r == o ? e : a;
      }
      function Os(e, t, r) {
        var o = t(e);
        return C(e) ? o : Ot(o, r(e));
      }
      function ge(e) {
        return e == null ? e === a ? Xf : Pf : an && an in te(e) ? tV(e) : pV(e);
      }
      function Ci(e, t) {
        return e > t;
      }
      function vm(e, t) {
        return e != null && K.call(e, t);
      }
      function Nm(e, t) {
        return e != null && t in te(e);
      }
      function wm(e, t, r) {
        return e >= Me(t, r) && e < pe(t, r);
      }
      function zi(e, t, r) {
        for (var o = r ? Zi : Hr, s = e[0].length, d = e.length, h = d, m = R(d), U = 1 / 0, w = []; h--; ) {
          var M = e[h];
          h && t && (M = ie(M, De(t))), U = Me(M.length, U), m[h] = !r && (t || s >= 120 && M.length >= 120) ? new ln(h && M) : a;
        }
        M = e[0];
        var Z = -1, b = m[0];
        e:
          for (; ++Z < s && w.length < U; ) {
            var E = M[Z], x = t ? t(E) : E;
            if (E = r || E !== 0 ? E : 0, !(b ? ar(b, x) : o(w, x, r))) {
              for (h = d; --h; ) {
                var I = m[h];
                if (!(I ? ar(I, x) : o(e[h], x, r)))
                  continue e;
              }
              b && b.push(x), w.push(E);
            }
          }
        return w;
      }
      function Mm(e, t, r, o) {
        return Tt(e, function(s, d, h) {
          t(o, r(s), d, h);
        }), o;
      }
      function fr(e, t, r) {
        t = It(t, e), e = gc(e, t);
        var o = e == null ? e : e[Rt(tt(t))];
        return o == null ? a : Be(o, e, r);
      }
      function Gs(e) {
        return se(e) && ge(e) == Rn;
      }
      function Zm(e) {
        return se(e) && ge(e) == rr;
      }
      function Jm(e) {
        return se(e) && ge(e) == Kn;
      }
      function hr(e, t, r, o, s) {
        return e === t ? !0 : e == null || t == null || !se(e) && !se(t) ? e !== e && t !== t : gm(e, t, r, o, hr, s);
      }
      function gm(e, t, r, o, s, d) {
        var h = C(e), m = C(t), U = h ? Dr : Ze(e), w = m ? Dr : Ze(t);
        U = U == Rn ? Mt : U, w = w == Rn ? Mt : w;
        var M = U == Mt, Z = w == Mt, b = U == w;
        if (b && Pt(e)) {
          if (!Pt(t))
            return !1;
          h = !0, M = !1;
        }
        if (b && !M)
          return d || (d = new st()), h || Sn(e) ? Rc(e, t, r, o, s, d) : $m(e, t, U, r, o, s, d);
        if (!(r & S)) {
          var E = M && K.call(e, "__wrapped__"), x = Z && K.call(t, "__wrapped__");
          if (E || x) {
            var I = E ? e.value() : e, Q = x ? t.value() : t;
            return d || (d = new st()), s(I, Q, r, o, d);
          }
        }
        return b ? (d || (d = new st()), eV(e, t, r, o, s, d)) : !1;
      }
      function km(e) {
        return se(e) && Ze(e) == it;
      }
      function Yi(e, t, r, o) {
        var s = r.length, d = s, h = !o;
        if (e == null)
          return !d;
        for (e = te(e); s--; ) {
          var m = r[s];
          if (h && m[2] ? m[1] !== e[m[0]] : !(m[0] in e))
            return !1;
        }
        for (; ++s < d; ) {
          m = r[s];
          var U = m[0], w = e[U], M = m[1];
          if (h && m[2]) {
            if (w === a && !(U in e))
              return !1;
          } else {
            var Z = new st();
            if (o)
              var b = o(w, M, U, e, t, Z);
            if (!(b === a ? hr(M, w, S | O, o, Z) : b))
              return !1;
          }
        }
        return !0;
      }
      function Cs(e) {
        if (!le(e) || cV(e))
          return !1;
        var t = Wt(e) ? kp : Th;
        return t.test(dn(e));
      }
      function ym(e) {
        return se(e) && ge(e) == er;
      }
      function bm(e) {
        return se(e) && Ze(e) == ot;
      }
      function Wm(e) {
        return se(e) && ka(e.length) && !!ae[ge(e)];
      }
      function zs(e) {
        return typeof e == "function" ? e : e == null ? xe : typeof e == "object" ? C(e) ? Hs(e[0], e[1]) : Is(e) : uu(e);
      }
      function Ii(e) {
        if (!Vr(e))
          return Sp(e);
        var t = [];
        for (var r in te(e))
          K.call(e, r) && r != "constructor" && t.push(r);
        return t;
      }
      function Em(e) {
        if (!le(e))
          return hV(e);
        var t = Vr(e), r = [];
        for (var o in e)
          o == "constructor" && (t || !K.call(e, o)) || r.push(o);
        return r;
      }
      function Hi(e, t) {
        return e < t;
      }
      function Ys(e, t) {
        var r = -1, o = Se(e) ? R(e.length) : [];
        return zt(e, function(s, d, h) {
          o[++r] = t(s, d, h);
        }), o;
      }
      function Is(e) {
        var t = oo(e);
        return t.length == 1 && t[0][2] ? Zc(t[0][0], t[0][1]) : function(r) {
          return r === e || Yi(r, e, t);
        };
      }
      function Hs(e, t) {
        return so(e) && Mc(t) ? Zc(Rt(e), t) : function(r) {
          var o = Ro(r, e);
          return o === a && o === t ? vo(r, e) : hr(t, o, S | O);
        };
      }
      function da(e, t, r, o, s) {
        e !== t && Oi(t, function(d, h) {
          if (s || (s = new st()), le(d))
            Fm(e, t, h, r, da, o, s);
          else {
            var m = o ? o(uo(e, h), d, h + "", e, t, s) : a;
            m === a && (m = d), Bi(e, h, m);
          }
        }, Ae);
      }
      function Fm(e, t, r, o, s, d, h) {
        var m = uo(e, r), U = uo(t, r), w = h.get(U);
        if (w) {
          Bi(e, r, w);
          return;
        }
        var M = d ? d(m, U, r + "", e, t, h) : a, Z = M === a;
        if (Z) {
          var b = C(U), E = !b && Pt(U), x = !b && !E && Sn(U);
          M = U, b || E || x ? C(m) ? M = m : ce(m) ? M = Fe(m) : E ? (Z = !1, M = rc(U, !0)) : x ? (Z = !1, M = ac(U, !0)) : M = [] : Ur(U) || fn(U) ? (M = m, fn(m) ? M = eu(m) : (!le(m) || Wt(m)) && (M = wc(U))) : Z = !1;
        }
        Z && (h.set(U, M), s(M, U, o, d, h), h.delete(U)), Bi(e, r, M);
      }
      function Ps(e, t) {
        var r = e.length;
        if (!!r)
          return t += t < 0 ? r : 0, bt(t, r) ? e[t] : a;
      }
      function js(e, t, r) {
        t.length ? t = ie(t, function(d) {
          return C(d) ? function(h) {
            return cn(h, d.length === 1 ? d[0] : d);
          } : d;
        }) : t = [xe];
        var o = -1;
        t = ie(t, De(A()));
        var s = Ys(e, function(d, h, m) {
          var U = ie(t, function(w) {
            return w(d);
          });
          return { criteria: U, index: ++o, value: d };
        });
        return ip(s, function(d, h) {
          return Hm(d, h, r);
        });
      }
      function Sm(e, t) {
        return Xs(e, t, function(r, o) {
          return vo(e, o);
        });
      }
      function Xs(e, t, r) {
        for (var o = -1, s = t.length, d = {}; ++o < s; ) {
          var h = t[o], m = cn(e, h);
          r(m, h) && pr(d, It(h, e), m);
        }
        return d;
      }
      function Am(e) {
        return function(t) {
          return cn(t, e);
        };
      }
      function Pi(e, t, r, o) {
        var s = o ? ap : wn, d = -1, h = t.length, m = e;
        for (e === t && (t = Fe(t)), r && (m = ie(e, De(r))); ++d < h; )
          for (var U = 0, w = t[d], M = r ? r(w) : w; (U = s(m, M, U, o)) > -1; )
            m !== e && ta.call(m, U, 1), ta.call(e, U, 1);
        return e;
      }
      function Ls(e, t) {
        for (var r = e ? t.length : 0, o = r - 1; r--; ) {
          var s = t[r];
          if (r == o || s !== d) {
            var d = s;
            bt(s) ? ta.call(e, s, 1) : _i(e, s);
          }
        }
        return e;
      }
      function ji(e, t) {
        return e + aa(bs() * (t - e + 1));
      }
      function xm(e, t, r, o) {
        for (var s = -1, d = pe(ra((t - e) / (r || 1)), 0), h = R(d); d--; )
          h[o ? d : ++s] = e, e += r;
        return h;
      }
      function Xi(e, t) {
        var r = "";
        if (!e || t < 1 || t > Bt)
          return r;
        do
          t % 2 && (r += e), t = aa(t / 2), t && (e += e);
        while (t);
        return r;
      }
      function H(e, t) {
        return fo(Jc(e, t, xe), e + "");
      }
      function Qm(e) {
        return Fs(An(e));
      }
      function Bm(e, t) {
        var r = An(e);
        return Na(r, sn(t, 0, r.length));
      }
      function pr(e, t, r, o) {
        if (!le(e))
          return e;
        t = It(t, e);
        for (var s = -1, d = t.length, h = d - 1, m = e; m != null && ++s < d; ) {
          var U = Rt(t[s]), w = r;
          if (U === "__proto__" || U === "constructor" || U === "prototype")
            return e;
          if (s != h) {
            var M = m[U];
            w = o ? o(M, U, m) : a, w === a && (w = le(M) ? M : bt(t[s + 1]) ? [] : {});
          }
          ur(m, U, w), m = m[U];
        }
        return e;
      }
      var _s = ia ? function(e, t) {
        return ia.set(e, t), e;
      } : xe, Dm = na ? function(e, t) {
        return na(e, "toString", {
          configurable: !0,
          enumerable: !1,
          value: wo(t),
          writable: !0
        });
      } : xe;
      function Om(e) {
        return Na(An(e));
      }
      function et(e, t, r) {
        var o = -1, s = e.length;
        t < 0 && (t = -t > s ? 0 : s + t), r = r > s ? s : r, r < 0 && (r += s), s = t > r ? 0 : r - t >>> 0, t >>>= 0;
        for (var d = R(s); ++o < s; )
          d[o] = e[o + t];
        return d;
      }
      function Gm(e, t) {
        var r;
        return zt(e, function(o, s, d) {
          return r = t(o, s, d), !r;
        }), !!r;
      }
      function fa(e, t, r) {
        var o = 0, s = e == null ? o : e.length;
        if (typeof t == "number" && t === t && s <= zf) {
          for (; o < s; ) {
            var d = o + s >>> 1, h = e[d];
            h !== null && !Ge(h) && (r ? h <= t : h < t) ? o = d + 1 : s = d;
          }
          return s;
        }
        return Li(e, t, xe, r);
      }
      function Li(e, t, r, o) {
        var s = 0, d = e == null ? 0 : e.length;
        if (d === 0)
          return 0;
        t = r(t);
        for (var h = t !== t, m = t === null, U = Ge(t), w = t === a; s < d; ) {
          var M = aa((s + d) / 2), Z = r(e[M]), b = Z !== a, E = Z === null, x = Z === Z, I = Ge(Z);
          if (h)
            var Q = o || x;
          else
            w ? Q = x && (o || b) : m ? Q = x && b && (o || !E) : U ? Q = x && b && !E && (o || !I) : E || I ? Q = !1 : Q = o ? Z <= t : Z < t;
          Q ? s = M + 1 : d = M;
        }
        return Me(d, Cf);
      }
      function qs(e, t) {
        for (var r = -1, o = e.length, s = 0, d = []; ++r < o; ) {
          var h = e[r], m = t ? t(h) : h;
          if (!r || !ct(m, U)) {
            var U = m;
            d[s++] = h === 0 ? 0 : h;
          }
        }
        return d;
      }
      function Ks(e) {
        return typeof e == "number" ? e : Ge(e) ? Br : +e;
      }
      function Oe(e) {
        if (typeof e == "string")
          return e;
        if (C(e))
          return ie(e, Oe) + "";
        if (Ge(e))
          return Ws ? Ws.call(e) : "";
        var t = e + "";
        return t == "0" && 1 / e == -Xe ? "-0" : t;
      }
      function Yt(e, t, r) {
        var o = -1, s = Hr, d = e.length, h = !0, m = [], U = m;
        if (r)
          h = !1, s = Zi;
        else if (d >= c) {
          var w = t ? null : qm(e);
          if (w)
            return jr(w);
          h = !1, s = ar, U = new ln();
        } else
          U = t ? [] : m;
        e:
          for (; ++o < d; ) {
            var M = e[o], Z = t ? t(M) : M;
            if (M = r || M !== 0 ? M : 0, h && Z === Z) {
              for (var b = U.length; b--; )
                if (U[b] === Z)
                  continue e;
              t && U.push(Z), m.push(M);
            } else
              s(U, Z, r) || (U !== m && U.push(Z), m.push(M));
          }
        return m;
      }
      function _i(e, t) {
        return t = It(t, e), e = gc(e, t), e == null || delete e[Rt(tt(t))];
      }
      function $s(e, t, r, o) {
        return pr(e, t, r(cn(e, t)), o);
      }
      function ha(e, t, r, o) {
        for (var s = e.length, d = o ? s : -1; (o ? d-- : ++d < s) && t(e[d], d, e); )
          ;
        return r ? et(e, o ? 0 : d, o ? d + 1 : s) : et(e, o ? d + 1 : 0, o ? s : d);
      }
      function ec(e, t) {
        var r = e;
        return r instanceof j && (r = r.value()), Ji(t, function(o, s) {
          return s.func.apply(s.thisArg, Ot([o], s.args));
        }, r);
      }
      function qi(e, t, r) {
        var o = e.length;
        if (o < 2)
          return o ? Yt(e[0]) : [];
        for (var s = -1, d = R(o); ++s < o; )
          for (var h = e[s], m = -1; ++m < o; )
            m != s && (d[s] = dr(d[s] || h, e[m], t, r));
        return Yt(Ne(d, 1), t, r);
      }
      function tc(e, t, r) {
        for (var o = -1, s = e.length, d = t.length, h = {}; ++o < s; ) {
          var m = o < d ? t[o] : a;
          r(h, e[o], m);
        }
        return h;
      }
      function Ki(e) {
        return ce(e) ? e : [];
      }
      function $i(e) {
        return typeof e == "function" ? e : xe;
      }
      function It(e, t) {
        return C(e) ? e : so(e, t) ? [e] : Wc(q(e));
      }
      var Cm = H;
      function Ht(e, t, r) {
        var o = e.length;
        return r = r === a ? o : r, !t && r >= o ? e : et(e, t, r);
      }
      var nc = yp || function(e) {
        return ve.clearTimeout(e);
      };
      function rc(e, t) {
        if (t)
          return e.slice();
        var r = e.length, o = Zs ? Zs(r) : new e.constructor(r);
        return e.copy(o), o;
      }
      function eo(e) {
        var t = new e.constructor(e.byteLength);
        return new $r(t).set(new $r(e)), t;
      }
      function zm(e, t) {
        var r = t ? eo(e.buffer) : e.buffer;
        return new e.constructor(r, e.byteOffset, e.byteLength);
      }
      function Ym(e) {
        var t = new e.constructor(e.source, Gl.exec(e));
        return t.lastIndex = e.lastIndex, t;
      }
      function Im(e) {
        return cr ? te(cr.call(e)) : {};
      }
      function ac(e, t) {
        var r = t ? eo(e.buffer) : e.buffer;
        return new e.constructor(r, e.byteOffset, e.length);
      }
      function ic(e, t) {
        if (e !== t) {
          var r = e !== a, o = e === null, s = e === e, d = Ge(e), h = t !== a, m = t === null, U = t === t, w = Ge(t);
          if (!m && !w && !d && e > t || d && h && U && !m && !w || o && h && U || !r && U || !s)
            return 1;
          if (!o && !d && !w && e < t || w && r && s && !o && !d || m && r && s || !h && s || !U)
            return -1;
        }
        return 0;
      }
      function Hm(e, t, r) {
        for (var o = -1, s = e.criteria, d = t.criteria, h = s.length, m = r.length; ++o < h; ) {
          var U = ic(s[o], d[o]);
          if (U) {
            if (o >= m)
              return U;
            var w = r[o];
            return U * (w == "desc" ? -1 : 1);
          }
        }
        return e.index - t.index;
      }
      function oc(e, t, r, o) {
        for (var s = -1, d = e.length, h = r.length, m = -1, U = t.length, w = pe(d - h, 0), M = R(U + w), Z = !o; ++m < U; )
          M[m] = t[m];
        for (; ++s < h; )
          (Z || s < d) && (M[r[s]] = e[s]);
        for (; w--; )
          M[m++] = e[s++];
        return M;
      }
      function lc(e, t, r, o) {
        for (var s = -1, d = e.length, h = -1, m = r.length, U = -1, w = t.length, M = pe(d - m, 0), Z = R(M + w), b = !o; ++s < M; )
          Z[s] = e[s];
        for (var E = s; ++U < w; )
          Z[E + U] = t[U];
        for (; ++h < m; )
          (b || s < d) && (Z[E + r[h]] = e[s++]);
        return Z;
      }
      function Fe(e, t) {
        var r = -1, o = e.length;
        for (t || (t = R(o)); ++r < o; )
          t[r] = e[r];
        return t;
      }
      function Ut(e, t, r, o) {
        var s = !r;
        r || (r = {});
        for (var d = -1, h = t.length; ++d < h; ) {
          var m = t[d], U = o ? o(r[m], e[m], m, r, e) : a;
          U === a && (U = e[m]), s ? gt(r, m, U) : ur(r, m, U);
        }
        return r;
      }
      function Pm(e, t) {
        return Ut(e, lo(e), t);
      }
      function jm(e, t) {
        return Ut(e, vc(e), t);
      }
      function pa(e, t) {
        return function(r, o) {
          var s = C(r) ? Kh : mm, d = t ? t() : {};
          return s(r, e, A(o, 2), d);
        };
      }
      function Wn(e) {
        return H(function(t, r) {
          var o = -1, s = r.length, d = s > 1 ? r[s - 1] : a, h = s > 2 ? r[2] : a;
          for (d = e.length > 3 && typeof d == "function" ? (s--, d) : a, h && ke(r[0], r[1], h) && (d = s < 3 ? a : d, s = 1), t = te(t); ++o < s; ) {
            var m = r[o];
            m && e(t, m, o, d);
          }
          return t;
        });
      }
      function sc(e, t) {
        return function(r, o) {
          if (r == null)
            return r;
          if (!Se(r))
            return e(r, o);
          for (var s = r.length, d = t ? s : -1, h = te(r); (t ? d-- : ++d < s) && o(h[d], d, h) !== !1; )
            ;
          return r;
        };
      }
      function cc(e) {
        return function(t, r, o) {
          for (var s = -1, d = te(t), h = o(t), m = h.length; m--; ) {
            var U = h[e ? m : ++s];
            if (r(d[U], U, d) === !1)
              break;
          }
          return t;
        };
      }
      function Xm(e, t, r) {
        var o = t & G, s = mr(e);
        function d() {
          var h = this && this !== ve && this instanceof d ? s : e;
          return h.apply(o ? r : this, arguments);
        }
        return d;
      }
      function uc(e) {
        return function(t) {
          t = q(t);
          var r = Mn(t) ? lt(t) : a, o = r ? r[0] : t.charAt(0), s = r ? Ht(r, 1).join("") : t.slice(1);
          return o[e]() + s;
        };
      }
      function En(e) {
        return function(t) {
          return Ji(su(lu(t).replace(Dh, "")), e, "");
        };
      }
      function mr(e) {
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return new e();
            case 1:
              return new e(t[0]);
            case 2:
              return new e(t[0], t[1]);
            case 3:
              return new e(t[0], t[1], t[2]);
            case 4:
              return new e(t[0], t[1], t[2], t[3]);
            case 5:
              return new e(t[0], t[1], t[2], t[3], t[4]);
            case 6:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5]);
            case 7:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5], t[6]);
          }
          var r = bn(e.prototype), o = e.apply(r, t);
          return le(o) ? o : r;
        };
      }
      function Lm(e, t, r) {
        var o = mr(e);
        function s() {
          for (var d = arguments.length, h = R(d), m = d, U = Fn(s); m--; )
            h[m] = arguments[m];
          var w = d < 3 && h[0] !== U && h[d - 1] !== U ? [] : Gt(h, U);
          if (d -= w.length, d < r)
            return mc(
              e,
              t,
              ma,
              s.placeholder,
              a,
              h,
              w,
              a,
              a,
              r - d
            );
          var M = this && this !== ve && this instanceof s ? o : e;
          return Be(M, this, h);
        }
        return s;
      }
      function dc(e) {
        return function(t, r, o) {
          var s = te(t);
          if (!Se(t)) {
            var d = A(r, 3);
            t = me(t), r = function(m) {
              return d(s[m], m, s);
            };
          }
          var h = e(t, r, o);
          return h > -1 ? s[d ? t[h] : h] : a;
        };
      }
      function fc(e) {
        return yt(function(t) {
          var r = t.length, o = r, s = Ke.prototype.thru;
          for (e && t.reverse(); o--; ) {
            var d = t[o];
            if (typeof d != "function")
              throw new qe(p);
            if (s && !h && Ra(d) == "wrapper")
              var h = new Ke([], !0);
          }
          for (o = h ? o : r; ++o < r; ) {
            d = t[o];
            var m = Ra(d), U = m == "wrapper" ? io(d) : a;
            U && co(U[0]) && U[1] == (fe | ne | F | ee) && !U[4].length && U[9] == 1 ? h = h[Ra(U[0])].apply(h, U[3]) : h = d.length == 1 && co(d) ? h[m]() : h.thru(d);
          }
          return function() {
            var w = arguments, M = w[0];
            if (h && w.length == 1 && C(M))
              return h.plant(M).value();
            for (var Z = 0, b = r ? t[Z].apply(this, w) : M; ++Z < r; )
              b = t[Z].call(this, b);
            return b;
          };
        });
      }
      function ma(e, t, r, o, s, d, h, m, U, w) {
        var M = t & fe, Z = t & G, b = t & B, E = t & (ne | oe), x = t & pt, I = b ? a : mr(e);
        function Q() {
          for (var P = arguments.length, X = R(P), Ce = P; Ce--; )
            X[Ce] = arguments[Ce];
          if (E)
            var ye = Fn(Q), ze = lp(X, ye);
          if (o && (X = oc(X, o, s, E)), d && (X = lc(X, d, h, E)), P -= ze, E && P < w) {
            var ue = Gt(X, ye);
            return mc(
              e,
              t,
              ma,
              Q.placeholder,
              r,
              X,
              ue,
              m,
              U,
              w - P
            );
          }
          var ut = Z ? r : this, Ft = b ? ut[e] : e;
          return P = X.length, m ? X = mV(X, m) : x && P > 1 && X.reverse(), M && U < P && (X.length = U), this && this !== ve && this instanceof Q && (Ft = I || mr(Ft)), Ft.apply(ut, X);
        }
        return Q;
      }
      function hc(e, t) {
        return function(r, o) {
          return Mm(r, e, t(o), {});
        };
      }
      function Va(e, t) {
        return function(r, o) {
          var s;
          if (r === a && o === a)
            return t;
          if (r !== a && (s = r), o !== a) {
            if (s === a)
              return o;
            typeof r == "string" || typeof o == "string" ? (r = Oe(r), o = Oe(o)) : (r = Ks(r), o = Ks(o)), s = e(r, o);
          }
          return s;
        };
      }
      function to(e) {
        return yt(function(t) {
          return t = ie(t, De(A())), H(function(r) {
            var o = this;
            return e(t, function(s) {
              return Be(s, o, r);
            });
          });
        });
      }
      function Ta(e, t) {
        t = t === a ? " " : Oe(t);
        var r = t.length;
        if (r < 2)
          return r ? Xi(t, e) : t;
        var o = Xi(t, ra(e / Zn(t)));
        return Mn(t) ? Ht(lt(o), 0, e).join("") : o.slice(0, e);
      }
      function _m(e, t, r, o) {
        var s = t & G, d = mr(e);
        function h() {
          for (var m = -1, U = arguments.length, w = -1, M = o.length, Z = R(M + U), b = this && this !== ve && this instanceof h ? d : e; ++w < M; )
            Z[w] = o[w];
          for (; U--; )
            Z[w++] = arguments[++m];
          return Be(b, s ? r : this, Z);
        }
        return h;
      }
      function pc(e) {
        return function(t, r, o) {
          return o && typeof o != "number" && ke(t, r, o) && (r = o = a), t = Et(t), r === a ? (r = t, t = 0) : r = Et(r), o = o === a ? t < r ? 1 : -1 : Et(o), xm(t, r, o, e);
        };
      }
      function Ua(e) {
        return function(t, r) {
          return typeof t == "string" && typeof r == "string" || (t = nt(t), r = nt(r)), e(t, r);
        };
      }
      function mc(e, t, r, o, s, d, h, m, U, w) {
        var M = t & ne, Z = M ? h : a, b = M ? a : h, E = M ? d : a, x = M ? a : d;
        t |= M ? F : Ue, t &= ~(M ? Ue : F), t & _ || (t &= ~(G | B));
        var I = [
          e,
          t,
          s,
          E,
          Z,
          x,
          b,
          m,
          U,
          w
        ], Q = r.apply(a, I);
        return co(e) && kc(Q, I), Q.placeholder = o, yc(Q, e, t);
      }
      function no(e) {
        var t = he[e];
        return function(r, o) {
          if (r = nt(r), o = o == null ? 0 : Me(Y(o), 292), o && ys(r)) {
            var s = (q(r) + "e").split("e"), d = t(s[0] + "e" + (+s[1] + o));
            return s = (q(d) + "e").split("e"), +(s[0] + "e" + (+s[1] - o));
          }
          return t(r);
        };
      }
      var qm = kn && 1 / jr(new kn([, -0]))[1] == Xe ? function(e) {
        return new kn(e);
      } : Jo;
      function Vc(e) {
        return function(t) {
          var r = Ze(t);
          return r == it ? Fi(t) : r == ot ? pp(t) : op(t, e(t));
        };
      }
      function kt(e, t, r, o, s, d, h, m) {
        var U = t & B;
        if (!U && typeof e != "function")
          throw new qe(p);
        var w = o ? o.length : 0;
        if (w || (t &= ~(F | Ue), o = s = a), h = h === a ? h : pe(Y(h), 0), m = m === a ? m : Y(m), w -= s ? s.length : 0, t & Ue) {
          var M = o, Z = s;
          o = s = a;
        }
        var b = U ? a : io(e), E = [
          e,
          t,
          r,
          o,
          s,
          M,
          Z,
          d,
          h,
          m
        ];
        if (b && fV(E, b), e = E[0], t = E[1], r = E[2], o = E[3], s = E[4], m = E[9] = E[9] === a ? U ? 0 : e.length : pe(E[9] - w, 0), !m && t & (ne | oe) && (t &= ~(ne | oe)), !t || t == G)
          var x = Xm(e, t, r);
        else
          t == ne || t == oe ? x = Lm(e, t, m) : (t == F || t == (G | F)) && !s.length ? x = _m(e, t, r, o) : x = ma.apply(a, E);
        var I = b ? _s : kc;
        return yc(I(x, E), e, t);
      }
      function Tc(e, t, r, o) {
        return e === a || ct(e, gn[r]) && !K.call(o, r) ? t : e;
      }
      function Uc(e, t, r, o, s, d) {
        return le(e) && le(t) && (d.set(t, e), da(e, t, a, Uc, d), d.delete(t)), e;
      }
      function Km(e) {
        return Ur(e) ? a : e;
      }
      function Rc(e, t, r, o, s, d) {
        var h = r & S, m = e.length, U = t.length;
        if (m != U && !(h && U > m))
          return !1;
        var w = d.get(e), M = d.get(t);
        if (w && M)
          return w == t && M == e;
        var Z = -1, b = !0, E = r & O ? new ln() : a;
        for (d.set(e, t), d.set(t, e); ++Z < m; ) {
          var x = e[Z], I = t[Z];
          if (o)
            var Q = h ? o(I, x, Z, t, e, d) : o(x, I, Z, e, t, d);
          if (Q !== a) {
            if (Q)
              continue;
            b = !1;
            break;
          }
          if (E) {
            if (!gi(t, function(P, X) {
              if (!ar(E, X) && (x === P || s(x, P, r, o, d)))
                return E.push(X);
            })) {
              b = !1;
              break;
            }
          } else if (!(x === I || s(x, I, r, o, d))) {
            b = !1;
            break;
          }
        }
        return d.delete(e), d.delete(t), b;
      }
      function $m(e, t, r, o, s, d, h) {
        switch (r) {
          case vn:
            if (e.byteLength != t.byteLength || e.byteOffset != t.byteOffset)
              return !1;
            e = e.buffer, t = t.buffer;
          case rr:
            return !(e.byteLength != t.byteLength || !d(new $r(e), new $r(t)));
          case qn:
          case Kn:
          case $n:
            return ct(+e, +t);
          case Or:
            return e.name == t.name && e.message == t.message;
          case er:
          case tr:
            return e == t + "";
          case it:
            var m = Fi;
          case ot:
            var U = o & S;
            if (m || (m = jr), e.size != t.size && !U)
              return !1;
            var w = h.get(e);
            if (w)
              return w == t;
            o |= O, h.set(e, t);
            var M = Rc(m(e), m(t), o, s, d, h);
            return h.delete(e), M;
          case Cr:
            if (cr)
              return cr.call(e) == cr.call(t);
        }
        return !1;
      }
      function eV(e, t, r, o, s, d) {
        var h = r & S, m = ro(e), U = m.length, w = ro(t), M = w.length;
        if (U != M && !h)
          return !1;
        for (var Z = U; Z--; ) {
          var b = m[Z];
          if (!(h ? b in t : K.call(t, b)))
            return !1;
        }
        var E = d.get(e), x = d.get(t);
        if (E && x)
          return E == t && x == e;
        var I = !0;
        d.set(e, t), d.set(t, e);
        for (var Q = h; ++Z < U; ) {
          b = m[Z];
          var P = e[b], X = t[b];
          if (o)
            var Ce = h ? o(X, P, b, t, e, d) : o(P, X, b, e, t, d);
          if (!(Ce === a ? P === X || s(P, X, r, o, d) : Ce)) {
            I = !1;
            break;
          }
          Q || (Q = b == "constructor");
        }
        if (I && !Q) {
          var ye = e.constructor, ze = t.constructor;
          ye != ze && "constructor" in e && "constructor" in t && !(typeof ye == "function" && ye instanceof ye && typeof ze == "function" && ze instanceof ze) && (I = !1);
        }
        return d.delete(e), d.delete(t), I;
      }
      function yt(e) {
        return fo(Jc(e, a, Ac), e + "");
      }
      function ro(e) {
        return Os(e, me, lo);
      }
      function ao(e) {
        return Os(e, Ae, vc);
      }
      var io = ia ? function(e) {
        return ia.get(e);
      } : Jo;
      function Ra(e) {
        for (var t = e.name + "", r = yn[t], o = K.call(yn, t) ? r.length : 0; o--; ) {
          var s = r[o], d = s.func;
          if (d == null || d == e)
            return s.name;
        }
        return t;
      }
      function Fn(e) {
        var t = K.call(u, "placeholder") ? u : e;
        return t.placeholder;
      }
      function A() {
        var e = u.iteratee || Mo;
        return e = e === Mo ? zs : e, arguments.length ? e(arguments[0], arguments[1]) : e;
      }
      function va(e, t) {
        var r = e.__data__;
        return sV(t) ? r[typeof t == "string" ? "string" : "hash"] : r.map;
      }
      function oo(e) {
        for (var t = me(e), r = t.length; r--; ) {
          var o = t[r], s = e[o];
          t[r] = [o, s, Mc(s)];
        }
        return t;
      }
      function un(e, t) {
        var r = dp(e, t);
        return Cs(r) ? r : a;
      }
      function tV(e) {
        var t = K.call(e, an), r = e[an];
        try {
          e[an] = a;
          var o = !0;
        } catch {
        }
        var s = qr.call(e);
        return o && (t ? e[an] = r : delete e[an]), s;
      }
      var lo = Ai ? function(e) {
        return e == null ? [] : (e = te(e), Dt(Ai(e), function(t) {
          return gs.call(e, t);
        }));
      } : go, vc = Ai ? function(e) {
        for (var t = []; e; )
          Ot(t, lo(e)), e = ea(e);
        return t;
      } : go, Ze = ge;
      (xi && Ze(new xi(new ArrayBuffer(1))) != vn || or && Ze(new or()) != it || Qi && Ze(Qi.resolve()) != Ql || kn && Ze(new kn()) != ot || lr && Ze(new lr()) != nr) && (Ze = function(e) {
        var t = ge(e), r = t == Mt ? e.constructor : a, o = r ? dn(r) : "";
        if (o)
          switch (o) {
            case Bp:
              return vn;
            case Dp:
              return it;
            case Op:
              return Ql;
            case Gp:
              return ot;
            case Cp:
              return nr;
          }
        return t;
      });
      function nV(e, t, r) {
        for (var o = -1, s = r.length; ++o < s; ) {
          var d = r[o], h = d.size;
          switch (d.type) {
            case "drop":
              e += h;
              break;
            case "dropRight":
              t -= h;
              break;
            case "take":
              t = Me(t, e + h);
              break;
            case "takeRight":
              e = pe(e, t - h);
              break;
          }
        }
        return { start: e, end: t };
      }
      function rV(e) {
        var t = e.match(ch);
        return t ? t[1].split(uh) : [];
      }
      function Nc(e, t, r) {
        t = It(t, e);
        for (var o = -1, s = t.length, d = !1; ++o < s; ) {
          var h = Rt(t[o]);
          if (!(d = e != null && r(e, h)))
            break;
          e = e[h];
        }
        return d || ++o != s ? d : (s = e == null ? 0 : e.length, !!s && ka(s) && bt(h, s) && (C(e) || fn(e)));
      }
      function aV(e) {
        var t = e.length, r = new e.constructor(t);
        return t && typeof e[0] == "string" && K.call(e, "index") && (r.index = e.index, r.input = e.input), r;
      }
      function wc(e) {
        return typeof e.constructor == "function" && !Vr(e) ? bn(ea(e)) : {};
      }
      function iV(e, t, r) {
        var o = e.constructor;
        switch (t) {
          case rr:
            return eo(e);
          case qn:
          case Kn:
            return new o(+e);
          case vn:
            return zm(e, r);
          case oi:
          case li:
          case si:
          case ci:
          case ui:
          case di:
          case fi:
          case hi:
          case pi:
            return ac(e, r);
          case it:
            return new o();
          case $n:
          case tr:
            return new o(e);
          case er:
            return Ym(e);
          case ot:
            return new o();
          case Cr:
            return Im(e);
        }
      }
      function oV(e, t) {
        var r = t.length;
        if (!r)
          return e;
        var o = r - 1;
        return t[o] = (r > 1 ? "& " : "") + t[o], t = t.join(r > 2 ? ", " : " "), e.replace(sh, `{
/* [wrapped with ` + t + `] */
`);
      }
      function lV(e) {
        return C(e) || fn(e) || !!(ks && e && e[ks]);
      }
      function bt(e, t) {
        var r = typeof e;
        return t = t == null ? Bt : t, !!t && (r == "number" || r != "symbol" && Rh.test(e)) && e > -1 && e % 1 == 0 && e < t;
      }
      function ke(e, t, r) {
        if (!le(r))
          return !1;
        var o = typeof t;
        return (o == "number" ? Se(r) && bt(t, r.length) : o == "string" && t in r) ? ct(r[t], e) : !1;
      }
      function so(e, t) {
        if (C(e))
          return !1;
        var r = typeof e;
        return r == "number" || r == "symbol" || r == "boolean" || e == null || Ge(e) ? !0 : ah.test(e) || !rh.test(e) || t != null && e in te(t);
      }
      function sV(e) {
        var t = typeof e;
        return t == "string" || t == "number" || t == "symbol" || t == "boolean" ? e !== "__proto__" : e === null;
      }
      function co(e) {
        var t = Ra(e), r = u[t];
        if (typeof r != "function" || !(t in j.prototype))
          return !1;
        if (e === r)
          return !0;
        var o = io(r);
        return !!o && e === o[0];
      }
      function cV(e) {
        return !!Ms && Ms in e;
      }
      var uV = Lr ? Wt : ko;
      function Vr(e) {
        var t = e && e.constructor, r = typeof t == "function" && t.prototype || gn;
        return e === r;
      }
      function Mc(e) {
        return e === e && !le(e);
      }
      function Zc(e, t) {
        return function(r) {
          return r == null ? !1 : r[e] === t && (t !== a || e in te(r));
        };
      }
      function dV(e) {
        var t = Ja(e, function(o) {
          return r.size === k && r.clear(), o;
        }), r = t.cache;
        return t;
      }
      function fV(e, t) {
        var r = e[1], o = t[1], s = r | o, d = s < (G | B | fe), h = o == fe && r == ne || o == fe && r == ee && e[7].length <= t[8] || o == (fe | ee) && t[7].length <= t[8] && r == ne;
        if (!(d || h))
          return e;
        o & G && (e[2] = t[2], s |= r & G ? 0 : _);
        var m = t[3];
        if (m) {
          var U = e[3];
          e[3] = U ? oc(U, m, t[4]) : m, e[4] = U ? Gt(e[3], J) : t[4];
        }
        return m = t[5], m && (U = e[5], e[5] = U ? lc(U, m, t[6]) : m, e[6] = U ? Gt(e[5], J) : t[6]), m = t[7], m && (e[7] = m), o & fe && (e[8] = e[8] == null ? t[8] : Me(e[8], t[8])), e[9] == null && (e[9] = t[9]), e[0] = t[0], e[1] = s, e;
      }
      function hV(e) {
        var t = [];
        if (e != null)
          for (var r in te(e))
            t.push(r);
        return t;
      }
      function pV(e) {
        return qr.call(e);
      }
      function Jc(e, t, r) {
        return t = pe(t === a ? e.length - 1 : t, 0), function() {
          for (var o = arguments, s = -1, d = pe(o.length - t, 0), h = R(d); ++s < d; )
            h[s] = o[t + s];
          s = -1;
          for (var m = R(t + 1); ++s < t; )
            m[s] = o[s];
          return m[t] = r(h), Be(e, this, m);
        };
      }
      function gc(e, t) {
        return t.length < 2 ? e : cn(e, et(t, 0, -1));
      }
      function mV(e, t) {
        for (var r = e.length, o = Me(t.length, r), s = Fe(e); o--; ) {
          var d = t[o];
          e[o] = bt(d, r) ? s[d] : a;
        }
        return e;
      }
      function uo(e, t) {
        if (!(t === "constructor" && typeof e[t] == "function") && t != "__proto__")
          return e[t];
      }
      var kc = bc(_s), Tr = Wp || function(e, t) {
        return ve.setTimeout(e, t);
      }, fo = bc(Dm);
      function yc(e, t, r) {
        var o = t + "";
        return fo(e, oV(o, VV(rV(o), r)));
      }
      function bc(e) {
        var t = 0, r = 0;
        return function() {
          var o = Ap(), s = mt - (o - r);
          if (r = o, s > 0) {
            if (++t >= Je)
              return arguments[0];
          } else
            t = 0;
          return e.apply(a, arguments);
        };
      }
      function Na(e, t) {
        var r = -1, o = e.length, s = o - 1;
        for (t = t === a ? o : t; ++r < t; ) {
          var d = ji(r, s), h = e[d];
          e[d] = e[r], e[r] = h;
        }
        return e.length = t, e;
      }
      var Wc = dV(function(e) {
        var t = [];
        return e.charCodeAt(0) === 46 && t.push(""), e.replace(ih, function(r, o, s, d) {
          t.push(s ? d.replace(hh, "$1") : o || r);
        }), t;
      });
      function Rt(e) {
        if (typeof e == "string" || Ge(e))
          return e;
        var t = e + "";
        return t == "0" && 1 / e == -Xe ? "-0" : t;
      }
      function dn(e) {
        if (e != null) {
          try {
            return _r.call(e);
          } catch {
          }
          try {
            return e + "";
          } catch {
          }
        }
        return "";
      }
      function VV(e, t) {
        return _e(Yf, function(r) {
          var o = "_." + r[0];
          t & r[1] && !Hr(e, o) && e.push(o);
        }), e.sort();
      }
      function Ec(e) {
        if (e instanceof j)
          return e.clone();
        var t = new Ke(e.__wrapped__, e.__chain__);
        return t.__actions__ = Fe(e.__actions__), t.__index__ = e.__index__, t.__values__ = e.__values__, t;
      }
      function TV(e, t, r) {
        (r ? ke(e, t, r) : t === a) ? t = 1 : t = pe(Y(t), 0);
        var o = e == null ? 0 : e.length;
        if (!o || t < 1)
          return [];
        for (var s = 0, d = 0, h = R(ra(o / t)); s < o; )
          h[d++] = et(e, s, s += t);
        return h;
      }
      function UV(e) {
        for (var t = -1, r = e == null ? 0 : e.length, o = 0, s = []; ++t < r; ) {
          var d = e[t];
          d && (s[o++] = d);
        }
        return s;
      }
      function RV() {
        var e = arguments.length;
        if (!e)
          return [];
        for (var t = R(e - 1), r = arguments[0], o = e; o--; )
          t[o - 1] = arguments[o];
        return Ot(C(r) ? Fe(r) : [r], Ne(t, 1));
      }
      var vV = H(function(e, t) {
        return ce(e) ? dr(e, Ne(t, 1, ce, !0)) : [];
      }), NV = H(function(e, t) {
        var r = tt(t);
        return ce(r) && (r = a), ce(e) ? dr(e, Ne(t, 1, ce, !0), A(r, 2)) : [];
      }), wV = H(function(e, t) {
        var r = tt(t);
        return ce(r) && (r = a), ce(e) ? dr(e, Ne(t, 1, ce, !0), a, r) : [];
      });
      function MV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : Y(t), et(e, t < 0 ? 0 : t, o)) : [];
      }
      function ZV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : Y(t), t = o - t, et(e, 0, t < 0 ? 0 : t)) : [];
      }
      function JV(e, t) {
        return e && e.length ? ha(e, A(t, 3), !0, !0) : [];
      }
      function gV(e, t) {
        return e && e.length ? ha(e, A(t, 3), !0) : [];
      }
      function kV(e, t, r, o) {
        var s = e == null ? 0 : e.length;
        return s ? (r && typeof r != "number" && ke(e, t, r) && (r = 0, o = s), Rm(e, t, r, o)) : [];
      }
      function Fc(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = r == null ? 0 : Y(r);
        return s < 0 && (s = pe(o + s, 0)), Pr(e, A(t, 3), s);
      }
      function Sc(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = o - 1;
        return r !== a && (s = Y(r), s = r < 0 ? pe(o + s, 0) : Me(s, o - 1)), Pr(e, A(t, 3), s, !0);
      }
      function Ac(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ne(e, 1) : [];
      }
      function yV(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ne(e, Xe) : [];
      }
      function bV(e, t) {
        var r = e == null ? 0 : e.length;
        return r ? (t = t === a ? 1 : Y(t), Ne(e, t)) : [];
      }
      function WV(e) {
        for (var t = -1, r = e == null ? 0 : e.length, o = {}; ++t < r; ) {
          var s = e[t];
          o[s[0]] = s[1];
        }
        return o;
      }
      function xc(e) {
        return e && e.length ? e[0] : a;
      }
      function EV(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = r == null ? 0 : Y(r);
        return s < 0 && (s = pe(o + s, 0)), wn(e, t, s);
      }
      function FV(e) {
        var t = e == null ? 0 : e.length;
        return t ? et(e, 0, -1) : [];
      }
      var SV = H(function(e) {
        var t = ie(e, Ki);
        return t.length && t[0] === e[0] ? zi(t) : [];
      }), AV = H(function(e) {
        var t = tt(e), r = ie(e, Ki);
        return t === tt(r) ? t = a : r.pop(), r.length && r[0] === e[0] ? zi(r, A(t, 2)) : [];
      }), xV = H(function(e) {
        var t = tt(e), r = ie(e, Ki);
        return t = typeof t == "function" ? t : a, t && r.pop(), r.length && r[0] === e[0] ? zi(r, a, t) : [];
      });
      function QV(e, t) {
        return e == null ? "" : Fp.call(e, t);
      }
      function tt(e) {
        var t = e == null ? 0 : e.length;
        return t ? e[t - 1] : a;
      }
      function BV(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = o;
        return r !== a && (s = Y(r), s = s < 0 ? pe(o + s, 0) : Me(s, o - 1)), t === t ? Vp(e, t, s) : Pr(e, ms, s, !0);
      }
      function DV(e, t) {
        return e && e.length ? Ps(e, Y(t)) : a;
      }
      var OV = H(Qc);
      function Qc(e, t) {
        return e && e.length && t && t.length ? Pi(e, t) : e;
      }
      function GV(e, t, r) {
        return e && e.length && t && t.length ? Pi(e, t, A(r, 2)) : e;
      }
      function CV(e, t, r) {
        return e && e.length && t && t.length ? Pi(e, t, a, r) : e;
      }
      var zV = yt(function(e, t) {
        var r = e == null ? 0 : e.length, o = Di(e, t);
        return Ls(e, ie(t, function(s) {
          return bt(s, r) ? +s : s;
        }).sort(ic)), o;
      });
      function YV(e, t) {
        var r = [];
        if (!(e && e.length))
          return r;
        var o = -1, s = [], d = e.length;
        for (t = A(t, 3); ++o < d; ) {
          var h = e[o];
          t(h, o, e) && (r.push(h), s.push(o));
        }
        return Ls(e, s), r;
      }
      function ho(e) {
        return e == null ? e : Qp.call(e);
      }
      function IV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (r && typeof r != "number" && ke(e, t, r) ? (t = 0, r = o) : (t = t == null ? 0 : Y(t), r = r === a ? o : Y(r)), et(e, t, r)) : [];
      }
      function HV(e, t) {
        return fa(e, t);
      }
      function PV(e, t, r) {
        return Li(e, t, A(r, 2));
      }
      function jV(e, t) {
        var r = e == null ? 0 : e.length;
        if (r) {
          var o = fa(e, t);
          if (o < r && ct(e[o], t))
            return o;
        }
        return -1;
      }
      function XV(e, t) {
        return fa(e, t, !0);
      }
      function LV(e, t, r) {
        return Li(e, t, A(r, 2), !0);
      }
      function _V(e, t) {
        var r = e == null ? 0 : e.length;
        if (r) {
          var o = fa(e, t, !0) - 1;
          if (ct(e[o], t))
            return o;
        }
        return -1;
      }
      function qV(e) {
        return e && e.length ? qs(e) : [];
      }
      function KV(e, t) {
        return e && e.length ? qs(e, A(t, 2)) : [];
      }
      function $V(e) {
        var t = e == null ? 0 : e.length;
        return t ? et(e, 1, t) : [];
      }
      function eT(e, t, r) {
        return e && e.length ? (t = r || t === a ? 1 : Y(t), et(e, 0, t < 0 ? 0 : t)) : [];
      }
      function tT(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : Y(t), t = o - t, et(e, t < 0 ? 0 : t, o)) : [];
      }
      function nT(e, t) {
        return e && e.length ? ha(e, A(t, 3), !1, !0) : [];
      }
      function rT(e, t) {
        return e && e.length ? ha(e, A(t, 3)) : [];
      }
      var aT = H(function(e) {
        return Yt(Ne(e, 1, ce, !0));
      }), iT = H(function(e) {
        var t = tt(e);
        return ce(t) && (t = a), Yt(Ne(e, 1, ce, !0), A(t, 2));
      }), oT = H(function(e) {
        var t = tt(e);
        return t = typeof t == "function" ? t : a, Yt(Ne(e, 1, ce, !0), a, t);
      });
      function lT(e) {
        return e && e.length ? Yt(e) : [];
      }
      function sT(e, t) {
        return e && e.length ? Yt(e, A(t, 2)) : [];
      }
      function cT(e, t) {
        return t = typeof t == "function" ? t : a, e && e.length ? Yt(e, a, t) : [];
      }
      function po(e) {
        if (!(e && e.length))
          return [];
        var t = 0;
        return e = Dt(e, function(r) {
          if (ce(r))
            return t = pe(r.length, t), !0;
        }), Wi(t, function(r) {
          return ie(e, ki(r));
        });
      }
      function Bc(e, t) {
        if (!(e && e.length))
          return [];
        var r = po(e);
        return t == null ? r : ie(r, function(o) {
          return Be(t, a, o);
        });
      }
      var uT = H(function(e, t) {
        return ce(e) ? dr(e, t) : [];
      }), dT = H(function(e) {
        return qi(Dt(e, ce));
      }), fT = H(function(e) {
        var t = tt(e);
        return ce(t) && (t = a), qi(Dt(e, ce), A(t, 2));
      }), hT = H(function(e) {
        var t = tt(e);
        return t = typeof t == "function" ? t : a, qi(Dt(e, ce), a, t);
      }), pT = H(po);
      function mT(e, t) {
        return tc(e || [], t || [], ur);
      }
      function VT(e, t) {
        return tc(e || [], t || [], pr);
      }
      var TT = H(function(e) {
        var t = e.length, r = t > 1 ? e[t - 1] : a;
        return r = typeof r == "function" ? (e.pop(), r) : a, Bc(e, r);
      });
      function Dc(e) {
        var t = u(e);
        return t.__chain__ = !0, t;
      }
      function UT(e, t) {
        return t(e), e;
      }
      function wa(e, t) {
        return t(e);
      }
      var RT = yt(function(e) {
        var t = e.length, r = t ? e[0] : 0, o = this.__wrapped__, s = function(d) {
          return Di(d, e);
        };
        return t > 1 || this.__actions__.length || !(o instanceof j) || !bt(r) ? this.thru(s) : (o = o.slice(r, +r + (t ? 1 : 0)), o.__actions__.push({
          func: wa,
          args: [s],
          thisArg: a
        }), new Ke(o, this.__chain__).thru(function(d) {
          return t && !d.length && d.push(a), d;
        }));
      });
      function vT() {
        return Dc(this);
      }
      function NT() {
        return new Ke(this.value(), this.__chain__);
      }
      function wT() {
        this.__values__ === a && (this.__values__ = Kc(this.value()));
        var e = this.__index__ >= this.__values__.length, t = e ? a : this.__values__[this.__index__++];
        return { done: e, value: t };
      }
      function MT() {
        return this;
      }
      function ZT(e) {
        for (var t, r = this; r instanceof la; ) {
          var o = Ec(r);
          o.__index__ = 0, o.__values__ = a, t ? s.__wrapped__ = o : t = o;
          var s = o;
          r = r.__wrapped__;
        }
        return s.__wrapped__ = e, t;
      }
      function JT() {
        var e = this.__wrapped__;
        if (e instanceof j) {
          var t = e;
          return this.__actions__.length && (t = new j(this)), t = t.reverse(), t.__actions__.push({
            func: wa,
            args: [ho],
            thisArg: a
          }), new Ke(t, this.__chain__);
        }
        return this.thru(ho);
      }
      function gT() {
        return ec(this.__wrapped__, this.__actions__);
      }
      var kT = pa(function(e, t, r) {
        K.call(e, r) ? ++e[r] : gt(e, r, 1);
      });
      function yT(e, t, r) {
        var o = C(e) ? hs : Um;
        return r && ke(e, t, r) && (t = a), o(e, A(t, 3));
      }
      function bT(e, t) {
        var r = C(e) ? Dt : Bs;
        return r(e, A(t, 3));
      }
      var WT = dc(Fc), ET = dc(Sc);
      function FT(e, t) {
        return Ne(Ma(e, t), 1);
      }
      function ST(e, t) {
        return Ne(Ma(e, t), Xe);
      }
      function AT(e, t, r) {
        return r = r === a ? 1 : Y(r), Ne(Ma(e, t), r);
      }
      function Oc(e, t) {
        var r = C(e) ? _e : zt;
        return r(e, A(t, 3));
      }
      function Gc(e, t) {
        var r = C(e) ? $h : Qs;
        return r(e, A(t, 3));
      }
      var xT = pa(function(e, t, r) {
        K.call(e, r) ? e[r].push(t) : gt(e, r, [t]);
      });
      function QT(e, t, r, o) {
        e = Se(e) ? e : An(e), r = r && !o ? Y(r) : 0;
        var s = e.length;
        return r < 0 && (r = pe(s + r, 0)), ya(e) ? r <= s && e.indexOf(t, r) > -1 : !!s && wn(e, t, r) > -1;
      }
      var BT = H(function(e, t, r) {
        var o = -1, s = typeof t == "function", d = Se(e) ? R(e.length) : [];
        return zt(e, function(h) {
          d[++o] = s ? Be(t, h, r) : fr(h, t, r);
        }), d;
      }), DT = pa(function(e, t, r) {
        gt(e, r, t);
      });
      function Ma(e, t) {
        var r = C(e) ? ie : Ys;
        return r(e, A(t, 3));
      }
      function OT(e, t, r, o) {
        return e == null ? [] : (C(t) || (t = t == null ? [] : [t]), r = o ? a : r, C(r) || (r = r == null ? [] : [r]), js(e, t, r));
      }
      var GT = pa(function(e, t, r) {
        e[r ? 0 : 1].push(t);
      }, function() {
        return [[], []];
      });
      function CT(e, t, r) {
        var o = C(e) ? Ji : Ts, s = arguments.length < 3;
        return o(e, A(t, 4), r, s, zt);
      }
      function zT(e, t, r) {
        var o = C(e) ? ep : Ts, s = arguments.length < 3;
        return o(e, A(t, 4), r, s, Qs);
      }
      function YT(e, t) {
        var r = C(e) ? Dt : Bs;
        return r(e, ga(A(t, 3)));
      }
      function IT(e) {
        var t = C(e) ? Fs : Qm;
        return t(e);
      }
      function HT(e, t, r) {
        (r ? ke(e, t, r) : t === a) ? t = 1 : t = Y(t);
        var o = C(e) ? hm : Bm;
        return o(e, t);
      }
      function PT(e) {
        var t = C(e) ? pm : Om;
        return t(e);
      }
      function jT(e) {
        if (e == null)
          return 0;
        if (Se(e))
          return ya(e) ? Zn(e) : e.length;
        var t = Ze(e);
        return t == it || t == ot ? e.size : Ii(e).length;
      }
      function XT(e, t, r) {
        var o = C(e) ? gi : Gm;
        return r && ke(e, t, r) && (t = a), o(e, A(t, 3));
      }
      var LT = H(function(e, t) {
        if (e == null)
          return [];
        var r = t.length;
        return r > 1 && ke(e, t[0], t[1]) ? t = [] : r > 2 && ke(t[0], t[1], t[2]) && (t = [t[0]]), js(e, Ne(t, 1), []);
      }), Za = bp || function() {
        return ve.Date.now();
      };
      function _T(e, t) {
        if (typeof t != "function")
          throw new qe(p);
        return e = Y(e), function() {
          if (--e < 1)
            return t.apply(this, arguments);
        };
      }
      function Cc(e, t, r) {
        return t = r ? a : t, t = e && t == null ? e.length : t, kt(e, fe, a, a, a, a, t);
      }
      function zc(e, t) {
        var r;
        if (typeof t != "function")
          throw new qe(p);
        return e = Y(e), function() {
          return --e > 0 && (r = t.apply(this, arguments)), e <= 1 && (t = a), r;
        };
      }
      var mo = H(function(e, t, r) {
        var o = G;
        if (r.length) {
          var s = Gt(r, Fn(mo));
          o |= F;
        }
        return kt(e, o, t, r, s);
      }), Yc = H(function(e, t, r) {
        var o = G | B;
        if (r.length) {
          var s = Gt(r, Fn(Yc));
          o |= F;
        }
        return kt(t, o, e, r, s);
      });
      function Ic(e, t, r) {
        t = r ? a : t;
        var o = kt(e, ne, a, a, a, a, a, t);
        return o.placeholder = Ic.placeholder, o;
      }
      function Hc(e, t, r) {
        t = r ? a : t;
        var o = kt(e, oe, a, a, a, a, a, t);
        return o.placeholder = Hc.placeholder, o;
      }
      function Pc(e, t, r) {
        var o, s, d, h, m, U, w = 0, M = !1, Z = !1, b = !0;
        if (typeof e != "function")
          throw new qe(p);
        t = nt(t) || 0, le(r) && (M = !!r.leading, Z = "maxWait" in r, d = Z ? pe(nt(r.maxWait) || 0, t) : d, b = "trailing" in r ? !!r.trailing : b);
        function E(ue) {
          var ut = o, Ft = s;
          return o = s = a, w = ue, h = e.apply(Ft, ut), h;
        }
        function x(ue) {
          return w = ue, m = Tr(P, t), M ? E(ue) : h;
        }
        function I(ue) {
          var ut = ue - U, Ft = ue - w, du = t - ut;
          return Z ? Me(du, d - Ft) : du;
        }
        function Q(ue) {
          var ut = ue - U, Ft = ue - w;
          return U === a || ut >= t || ut < 0 || Z && Ft >= d;
        }
        function P() {
          var ue = Za();
          if (Q(ue))
            return X(ue);
          m = Tr(P, I(ue));
        }
        function X(ue) {
          return m = a, b && o ? E(ue) : (o = s = a, h);
        }
        function Ce() {
          m !== a && nc(m), w = 0, o = U = s = m = a;
        }
        function ye() {
          return m === a ? h : X(Za());
        }
        function ze() {
          var ue = Za(), ut = Q(ue);
          if (o = arguments, s = this, U = ue, ut) {
            if (m === a)
              return x(U);
            if (Z)
              return nc(m), m = Tr(P, t), E(U);
          }
          return m === a && (m = Tr(P, t)), h;
        }
        return ze.cancel = Ce, ze.flush = ye, ze;
      }
      var qT = H(function(e, t) {
        return xs(e, 1, t);
      }), KT = H(function(e, t, r) {
        return xs(e, nt(t) || 0, r);
      });
      function $T(e) {
        return kt(e, pt);
      }
      function Ja(e, t) {
        if (typeof e != "function" || t != null && typeof t != "function")
          throw new qe(p);
        var r = function() {
          var o = arguments, s = t ? t.apply(this, o) : o[0], d = r.cache;
          if (d.has(s))
            return d.get(s);
          var h = e.apply(this, o);
          return r.cache = d.set(s, h) || d, h;
        };
        return r.cache = new (Ja.Cache || Jt)(), r;
      }
      Ja.Cache = Jt;
      function ga(e) {
        if (typeof e != "function")
          throw new qe(p);
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return !e.call(this);
            case 1:
              return !e.call(this, t[0]);
            case 2:
              return !e.call(this, t[0], t[1]);
            case 3:
              return !e.call(this, t[0], t[1], t[2]);
          }
          return !e.apply(this, t);
        };
      }
      function eU(e) {
        return zc(2, e);
      }
      var tU = Cm(function(e, t) {
        t = t.length == 1 && C(t[0]) ? ie(t[0], De(A())) : ie(Ne(t, 1), De(A()));
        var r = t.length;
        return H(function(o) {
          for (var s = -1, d = Me(o.length, r); ++s < d; )
            o[s] = t[s].call(this, o[s]);
          return Be(e, this, o);
        });
      }), Vo = H(function(e, t) {
        var r = Gt(t, Fn(Vo));
        return kt(e, F, a, t, r);
      }), jc = H(function(e, t) {
        var r = Gt(t, Fn(jc));
        return kt(e, Ue, a, t, r);
      }), nU = yt(function(e, t) {
        return kt(e, ee, a, a, a, t);
      });
      function rU(e, t) {
        if (typeof e != "function")
          throw new qe(p);
        return t = t === a ? t : Y(t), H(e, t);
      }
      function aU(e, t) {
        if (typeof e != "function")
          throw new qe(p);
        return t = t == null ? 0 : pe(Y(t), 0), H(function(r) {
          var o = r[t], s = Ht(r, 0, t);
          return o && Ot(s, o), Be(e, this, s);
        });
      }
      function iU(e, t, r) {
        var o = !0, s = !0;
        if (typeof e != "function")
          throw new qe(p);
        return le(r) && (o = "leading" in r ? !!r.leading : o, s = "trailing" in r ? !!r.trailing : s), Pc(e, t, {
          leading: o,
          maxWait: t,
          trailing: s
        });
      }
      function oU(e) {
        return Cc(e, 1);
      }
      function lU(e, t) {
        return Vo($i(t), e);
      }
      function sU() {
        if (!arguments.length)
          return [];
        var e = arguments[0];
        return C(e) ? e : [e];
      }
      function cU(e) {
        return $e(e, z);
      }
      function uU(e, t) {
        return t = typeof t == "function" ? t : a, $e(e, z, t);
      }
      function dU(e) {
        return $e(e, y | z);
      }
      function fU(e, t) {
        return t = typeof t == "function" ? t : a, $e(e, y | z, t);
      }
      function hU(e, t) {
        return t == null || As(e, t, me(t));
      }
      function ct(e, t) {
        return e === t || e !== e && t !== t;
      }
      var pU = Ua(Ci), mU = Ua(function(e, t) {
        return e >= t;
      }), fn = Gs(function() {
        return arguments;
      }()) ? Gs : function(e) {
        return se(e) && K.call(e, "callee") && !gs.call(e, "callee");
      }, C = R.isArray, VU = ls ? De(ls) : Zm;
      function Se(e) {
        return e != null && ka(e.length) && !Wt(e);
      }
      function ce(e) {
        return se(e) && Se(e);
      }
      function TU(e) {
        return e === !0 || e === !1 || se(e) && ge(e) == qn;
      }
      var Pt = Ep || ko, UU = ss ? De(ss) : Jm;
      function RU(e) {
        return se(e) && e.nodeType === 1 && !Ur(e);
      }
      function vU(e) {
        if (e == null)
          return !0;
        if (Se(e) && (C(e) || typeof e == "string" || typeof e.splice == "function" || Pt(e) || Sn(e) || fn(e)))
          return !e.length;
        var t = Ze(e);
        if (t == it || t == ot)
          return !e.size;
        if (Vr(e))
          return !Ii(e).length;
        for (var r in e)
          if (K.call(e, r))
            return !1;
        return !0;
      }
      function NU(e, t) {
        return hr(e, t);
      }
      function wU(e, t, r) {
        r = typeof r == "function" ? r : a;
        var o = r ? r(e, t) : a;
        return o === a ? hr(e, t, a, r) : !!o;
      }
      function To(e) {
        if (!se(e))
          return !1;
        var t = ge(e);
        return t == Or || t == Hf || typeof e.message == "string" && typeof e.name == "string" && !Ur(e);
      }
      function MU(e) {
        return typeof e == "number" && ys(e);
      }
      function Wt(e) {
        if (!le(e))
          return !1;
        var t = ge(e);
        return t == Gr || t == xl || t == If || t == jf;
      }
      function Xc(e) {
        return typeof e == "number" && e == Y(e);
      }
      function ka(e) {
        return typeof e == "number" && e > -1 && e % 1 == 0 && e <= Bt;
      }
      function le(e) {
        var t = typeof e;
        return e != null && (t == "object" || t == "function");
      }
      function se(e) {
        return e != null && typeof e == "object";
      }
      var Lc = cs ? De(cs) : km;
      function ZU(e, t) {
        return e === t || Yi(e, t, oo(t));
      }
      function JU(e, t, r) {
        return r = typeof r == "function" ? r : a, Yi(e, t, oo(t), r);
      }
      function gU(e) {
        return _c(e) && e != +e;
      }
      function kU(e) {
        if (uV(e))
          throw new D(f);
        return Cs(e);
      }
      function yU(e) {
        return e === null;
      }
      function bU(e) {
        return e == null;
      }
      function _c(e) {
        return typeof e == "number" || se(e) && ge(e) == $n;
      }
      function Ur(e) {
        if (!se(e) || ge(e) != Mt)
          return !1;
        var t = ea(e);
        if (t === null)
          return !0;
        var r = K.call(t, "constructor") && t.constructor;
        return typeof r == "function" && r instanceof r && _r.call(r) == Jp;
      }
      var Uo = us ? De(us) : ym;
      function WU(e) {
        return Xc(e) && e >= -Bt && e <= Bt;
      }
      var qc = ds ? De(ds) : bm;
      function ya(e) {
        return typeof e == "string" || !C(e) && se(e) && ge(e) == tr;
      }
      function Ge(e) {
        return typeof e == "symbol" || se(e) && ge(e) == Cr;
      }
      var Sn = fs ? De(fs) : Wm;
      function EU(e) {
        return e === a;
      }
      function FU(e) {
        return se(e) && Ze(e) == nr;
      }
      function SU(e) {
        return se(e) && ge(e) == Lf;
      }
      var AU = Ua(Hi), xU = Ua(function(e, t) {
        return e <= t;
      });
      function Kc(e) {
        if (!e)
          return [];
        if (Se(e))
          return ya(e) ? lt(e) : Fe(e);
        if (ir && e[ir])
          return hp(e[ir]());
        var t = Ze(e), r = t == it ? Fi : t == ot ? jr : An;
        return r(e);
      }
      function Et(e) {
        if (!e)
          return e === 0 ? e : 0;
        if (e = nt(e), e === Xe || e === -Xe) {
          var t = e < 0 ? -1 : 1;
          return t * Gf;
        }
        return e === e ? e : 0;
      }
      function Y(e) {
        var t = Et(e), r = t % 1;
        return t === t ? r ? t - r : t : 0;
      }
      function $c(e) {
        return e ? sn(Y(e), 0, Vt) : 0;
      }
      function nt(e) {
        if (typeof e == "number")
          return e;
        if (Ge(e))
          return Br;
        if (le(e)) {
          var t = typeof e.valueOf == "function" ? e.valueOf() : e;
          e = le(t) ? t + "" : t;
        }
        if (typeof e != "string")
          return e === 0 ? e : +e;
        e = Us(e);
        var r = Vh.test(e);
        return r || Uh.test(e) ? _h(e.slice(2), r ? 2 : 8) : mh.test(e) ? Br : +e;
      }
      function eu(e) {
        return Ut(e, Ae(e));
      }
      function QU(e) {
        return e ? sn(Y(e), -Bt, Bt) : e === 0 ? e : 0;
      }
      function q(e) {
        return e == null ? "" : Oe(e);
      }
      var BU = Wn(function(e, t) {
        if (Vr(t) || Se(t)) {
          Ut(t, me(t), e);
          return;
        }
        for (var r in t)
          K.call(t, r) && ur(e, r, t[r]);
      }), tu = Wn(function(e, t) {
        Ut(t, Ae(t), e);
      }), ba = Wn(function(e, t, r, o) {
        Ut(t, Ae(t), e, o);
      }), DU = Wn(function(e, t, r, o) {
        Ut(t, me(t), e, o);
      }), OU = yt(Di);
      function GU(e, t) {
        var r = bn(e);
        return t == null ? r : Ss(r, t);
      }
      var CU = H(function(e, t) {
        e = te(e);
        var r = -1, o = t.length, s = o > 2 ? t[2] : a;
        for (s && ke(t[0], t[1], s) && (o = 1); ++r < o; )
          for (var d = t[r], h = Ae(d), m = -1, U = h.length; ++m < U; ) {
            var w = h[m], M = e[w];
            (M === a || ct(M, gn[w]) && !K.call(e, w)) && (e[w] = d[w]);
          }
        return e;
      }), zU = H(function(e) {
        return e.push(a, Uc), Be(nu, a, e);
      });
      function YU(e, t) {
        return ps(e, A(t, 3), Tt);
      }
      function IU(e, t) {
        return ps(e, A(t, 3), Gi);
      }
      function HU(e, t) {
        return e == null ? e : Oi(e, A(t, 3), Ae);
      }
      function PU(e, t) {
        return e == null ? e : Ds(e, A(t, 3), Ae);
      }
      function jU(e, t) {
        return e && Tt(e, A(t, 3));
      }
      function XU(e, t) {
        return e && Gi(e, A(t, 3));
      }
      function LU(e) {
        return e == null ? [] : ua(e, me(e));
      }
      function _U(e) {
        return e == null ? [] : ua(e, Ae(e));
      }
      function Ro(e, t, r) {
        var o = e == null ? a : cn(e, t);
        return o === a ? r : o;
      }
      function qU(e, t) {
        return e != null && Nc(e, t, vm);
      }
      function vo(e, t) {
        return e != null && Nc(e, t, Nm);
      }
      var KU = hc(function(e, t, r) {
        t != null && typeof t.toString != "function" && (t = qr.call(t)), e[t] = r;
      }, wo(xe)), $U = hc(function(e, t, r) {
        t != null && typeof t.toString != "function" && (t = qr.call(t)), K.call(e, t) ? e[t].push(r) : e[t] = [r];
      }, A), e1 = H(fr);
      function me(e) {
        return Se(e) ? Es(e) : Ii(e);
      }
      function Ae(e) {
        return Se(e) ? Es(e, !0) : Em(e);
      }
      function t1(e, t) {
        var r = {};
        return t = A(t, 3), Tt(e, function(o, s, d) {
          gt(r, t(o, s, d), o);
        }), r;
      }
      function n1(e, t) {
        var r = {};
        return t = A(t, 3), Tt(e, function(o, s, d) {
          gt(r, s, t(o, s, d));
        }), r;
      }
      var r1 = Wn(function(e, t, r) {
        da(e, t, r);
      }), nu = Wn(function(e, t, r, o) {
        da(e, t, r, o);
      }), a1 = yt(function(e, t) {
        var r = {};
        if (e == null)
          return r;
        var o = !1;
        t = ie(t, function(d) {
          return d = It(d, e), o || (o = d.length > 1), d;
        }), Ut(e, ao(e), r), o && (r = $e(r, y | g | z, Km));
        for (var s = t.length; s--; )
          _i(r, t[s]);
        return r;
      });
      function i1(e, t) {
        return ru(e, ga(A(t)));
      }
      var o1 = yt(function(e, t) {
        return e == null ? {} : Sm(e, t);
      });
      function ru(e, t) {
        if (e == null)
          return {};
        var r = ie(ao(e), function(o) {
          return [o];
        });
        return t = A(t), Xs(e, r, function(o, s) {
          return t(o, s[0]);
        });
      }
      function l1(e, t, r) {
        t = It(t, e);
        var o = -1, s = t.length;
        for (s || (s = 1, e = a); ++o < s; ) {
          var d = e == null ? a : e[Rt(t[o])];
          d === a && (o = s, d = r), e = Wt(d) ? d.call(e) : d;
        }
        return e;
      }
      function s1(e, t, r) {
        return e == null ? e : pr(e, t, r);
      }
      function c1(e, t, r, o) {
        return o = typeof o == "function" ? o : a, e == null ? e : pr(e, t, r, o);
      }
      var au = Vc(me), iu = Vc(Ae);
      function u1(e, t, r) {
        var o = C(e), s = o || Pt(e) || Sn(e);
        if (t = A(t, 4), r == null) {
          var d = e && e.constructor;
          s ? r = o ? new d() : [] : le(e) ? r = Wt(d) ? bn(ea(e)) : {} : r = {};
        }
        return (s ? _e : Tt)(e, function(h, m, U) {
          return t(r, h, m, U);
        }), r;
      }
      function d1(e, t) {
        return e == null ? !0 : _i(e, t);
      }
      function f1(e, t, r) {
        return e == null ? e : $s(e, t, $i(r));
      }
      function h1(e, t, r, o) {
        return o = typeof o == "function" ? o : a, e == null ? e : $s(e, t, $i(r), o);
      }
      function An(e) {
        return e == null ? [] : Ei(e, me(e));
      }
      function p1(e) {
        return e == null ? [] : Ei(e, Ae(e));
      }
      function m1(e, t, r) {
        return r === a && (r = t, t = a), r !== a && (r = nt(r), r = r === r ? r : 0), t !== a && (t = nt(t), t = t === t ? t : 0), sn(nt(e), t, r);
      }
      function V1(e, t, r) {
        return t = Et(t), r === a ? (r = t, t = 0) : r = Et(r), e = nt(e), wm(e, t, r);
      }
      function T1(e, t, r) {
        if (r && typeof r != "boolean" && ke(e, t, r) && (t = r = a), r === a && (typeof t == "boolean" ? (r = t, t = a) : typeof e == "boolean" && (r = e, e = a)), e === a && t === a ? (e = 0, t = 1) : (e = Et(e), t === a ? (t = e, e = 0) : t = Et(t)), e > t) {
          var o = e;
          e = t, t = o;
        }
        if (r || e % 1 || t % 1) {
          var s = bs();
          return Me(e + s * (t - e + Lh("1e-" + ((s + "").length - 1))), t);
        }
        return ji(e, t);
      }
      var U1 = En(function(e, t, r) {
        return t = t.toLowerCase(), e + (r ? ou(t) : t);
      });
      function ou(e) {
        return No(q(e).toLowerCase());
      }
      function lu(e) {
        return e = q(e), e && e.replace(vh, sp).replace(Oh, "");
      }
      function R1(e, t, r) {
        e = q(e), t = Oe(t);
        var o = e.length;
        r = r === a ? o : sn(Y(r), 0, o);
        var s = r;
        return r -= t.length, r >= 0 && e.slice(r, s) == t;
      }
      function v1(e) {
        return e = q(e), e && eh.test(e) ? e.replace(Dl, cp) : e;
      }
      function N1(e) {
        return e = q(e), e && oh.test(e) ? e.replace(mi, "\\$&") : e;
      }
      var w1 = En(function(e, t, r) {
        return e + (r ? "-" : "") + t.toLowerCase();
      }), M1 = En(function(e, t, r) {
        return e + (r ? " " : "") + t.toLowerCase();
      }), Z1 = uc("toLowerCase");
      function J1(e, t, r) {
        e = q(e), t = Y(t);
        var o = t ? Zn(e) : 0;
        if (!t || o >= t)
          return e;
        var s = (t - o) / 2;
        return Ta(aa(s), r) + e + Ta(ra(s), r);
      }
      function g1(e, t, r) {
        e = q(e), t = Y(t);
        var o = t ? Zn(e) : 0;
        return t && o < t ? e + Ta(t - o, r) : e;
      }
      function k1(e, t, r) {
        e = q(e), t = Y(t);
        var o = t ? Zn(e) : 0;
        return t && o < t ? Ta(t - o, r) + e : e;
      }
      function y1(e, t, r) {
        return r || t == null ? t = 0 : t && (t = +t), xp(q(e).replace(Vi, ""), t || 0);
      }
      function b1(e, t, r) {
        return (r ? ke(e, t, r) : t === a) ? t = 1 : t = Y(t), Xi(q(e), t);
      }
      function W1() {
        var e = arguments, t = q(e[0]);
        return e.length < 3 ? t : t.replace(e[1], e[2]);
      }
      var E1 = En(function(e, t, r) {
        return e + (r ? "_" : "") + t.toLowerCase();
      });
      function F1(e, t, r) {
        return r && typeof r != "number" && ke(e, t, r) && (t = r = a), r = r === a ? Vt : r >>> 0, r ? (e = q(e), e && (typeof t == "string" || t != null && !Uo(t)) && (t = Oe(t), !t && Mn(e)) ? Ht(lt(e), 0, r) : e.split(t, r)) : [];
      }
      var S1 = En(function(e, t, r) {
        return e + (r ? " " : "") + No(t);
      });
      function A1(e, t, r) {
        return e = q(e), r = r == null ? 0 : sn(Y(r), 0, e.length), t = Oe(t), e.slice(r, r + t.length) == t;
      }
      function x1(e, t, r) {
        var o = u.templateSettings;
        r && ke(e, t, r) && (t = a), e = q(e), t = ba({}, t, o, Tc);
        var s = ba({}, t.imports, o.imports, Tc), d = me(s), h = Ei(s, d), m, U, w = 0, M = t.interpolate || zr, Z = "__p += '", b = Si(
          (t.escape || zr).source + "|" + M.source + "|" + (M === Ol ? ph : zr).source + "|" + (t.evaluate || zr).source + "|$",
          "g"
        ), E = "//# sourceURL=" + (K.call(t, "sourceURL") ? (t.sourceURL + "").replace(/\s/g, " ") : "lodash.templateSources[" + ++Ih + "]") + `
`;
        e.replace(b, function(Q, P, X, Ce, ye, ze) {
          return X || (X = Ce), Z += e.slice(w, ze).replace(Nh, up), P && (m = !0, Z += `' +
__e(` + P + `) +
'`), ye && (U = !0, Z += `';
` + ye + `;
__p += '`), X && (Z += `' +
((__t = (` + X + `)) == null ? '' : __t) +
'`), w = ze + Q.length, Q;
        }), Z += `';
`;
        var x = K.call(t, "variable") && t.variable;
        if (!x)
          Z = `with (obj) {
` + Z + `
}
`;
        else if (fh.test(x))
          throw new D(V);
        Z = (U ? Z.replace(_f, "") : Z).replace(qf, "$1").replace(Kf, "$1;"), Z = "function(" + (x || "obj") + `) {
` + (x ? "" : `obj || (obj = {});
`) + "var __t, __p = ''" + (m ? ", __e = _.escape" : "") + (U ? `, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
` : `;
`) + Z + `return __p
}`;
        var I = cu(function() {
          return L(d, E + "return " + Z).apply(a, h);
        });
        if (I.source = Z, To(I))
          throw I;
        return I;
      }
      function Q1(e) {
        return q(e).toLowerCase();
      }
      function B1(e) {
        return q(e).toUpperCase();
      }
      function D1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return Us(e);
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = lt(t), d = Rs(o, s), h = vs(o, s) + 1;
        return Ht(o, d, h).join("");
      }
      function O1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return e.slice(0, ws(e) + 1);
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = vs(o, lt(t)) + 1;
        return Ht(o, 0, s).join("");
      }
      function G1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return e.replace(Vi, "");
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = Rs(o, lt(t));
        return Ht(o, s).join("");
      }
      function C1(e, t) {
        var r = wt, o = nn;
        if (le(t)) {
          var s = "separator" in t ? t.separator : s;
          r = "length" in t ? Y(t.length) : r, o = "omission" in t ? Oe(t.omission) : o;
        }
        e = q(e);
        var d = e.length;
        if (Mn(e)) {
          var h = lt(e);
          d = h.length;
        }
        if (r >= d)
          return e;
        var m = r - Zn(o);
        if (m < 1)
          return o;
        var U = h ? Ht(h, 0, m).join("") : e.slice(0, m);
        if (s === a)
          return U + o;
        if (h && (m += U.length - m), Uo(s)) {
          if (e.slice(m).search(s)) {
            var w, M = U;
            for (s.global || (s = Si(s.source, q(Gl.exec(s)) + "g")), s.lastIndex = 0; w = s.exec(M); )
              var Z = w.index;
            U = U.slice(0, Z === a ? m : Z);
          }
        } else if (e.indexOf(Oe(s), m) != m) {
          var b = U.lastIndexOf(s);
          b > -1 && (U = U.slice(0, b));
        }
        return U + o;
      }
      function z1(e) {
        return e = q(e), e && $f.test(e) ? e.replace(Bl, Tp) : e;
      }
      var Y1 = En(function(e, t, r) {
        return e + (r ? " " : "") + t.toUpperCase();
      }), No = uc("toUpperCase");
      function su(e, t, r) {
        return e = q(e), t = r ? a : t, t === a ? fp(e) ? vp(e) : rp(e) : e.match(t) || [];
      }
      var cu = H(function(e, t) {
        try {
          return Be(e, a, t);
        } catch (r) {
          return To(r) ? r : new D(r);
        }
      }), I1 = yt(function(e, t) {
        return _e(t, function(r) {
          r = Rt(r), gt(e, r, mo(e[r], e));
        }), e;
      });
      function H1(e) {
        var t = e == null ? 0 : e.length, r = A();
        return e = t ? ie(e, function(o) {
          if (typeof o[1] != "function")
            throw new qe(p);
          return [r(o[0]), o[1]];
        }) : [], H(function(o) {
          for (var s = -1; ++s < t; ) {
            var d = e[s];
            if (Be(d[0], this, o))
              return Be(d[1], this, o);
          }
        });
      }
      function P1(e) {
        return Tm($e(e, y));
      }
      function wo(e) {
        return function() {
          return e;
        };
      }
      function j1(e, t) {
        return e == null || e !== e ? t : e;
      }
      var X1 = fc(), L1 = fc(!0);
      function xe(e) {
        return e;
      }
      function Mo(e) {
        return zs(typeof e == "function" ? e : $e(e, y));
      }
      function _1(e) {
        return Is($e(e, y));
      }
      function q1(e, t) {
        return Hs(e, $e(t, y));
      }
      var K1 = H(function(e, t) {
        return function(r) {
          return fr(r, e, t);
        };
      }), $1 = H(function(e, t) {
        return function(r) {
          return fr(e, r, t);
        };
      });
      function Zo(e, t, r) {
        var o = me(t), s = ua(t, o);
        r == null && !(le(t) && (s.length || !o.length)) && (r = t, t = e, e = this, s = ua(t, me(t)));
        var d = !(le(r) && "chain" in r) || !!r.chain, h = Wt(e);
        return _e(s, function(m) {
          var U = t[m];
          e[m] = U, h && (e.prototype[m] = function() {
            var w = this.__chain__;
            if (d || w) {
              var M = e(this.__wrapped__), Z = M.__actions__ = Fe(this.__actions__);
              return Z.push({ func: U, args: arguments, thisArg: e }), M.__chain__ = w, M;
            }
            return U.apply(e, Ot([this.value()], arguments));
          });
        }), e;
      }
      function e0() {
        return ve._ === this && (ve._ = gp), this;
      }
      function Jo() {
      }
      function t0(e) {
        return e = Y(e), H(function(t) {
          return Ps(t, e);
        });
      }
      var n0 = to(ie), r0 = to(hs), a0 = to(gi);
      function uu(e) {
        return so(e) ? ki(Rt(e)) : Am(e);
      }
      function i0(e) {
        return function(t) {
          return e == null ? a : cn(e, t);
        };
      }
      var o0 = pc(), l0 = pc(!0);
      function go() {
        return [];
      }
      function ko() {
        return !1;
      }
      function s0() {
        return {};
      }
      function c0() {
        return "";
      }
      function u0() {
        return !0;
      }
      function d0(e, t) {
        if (e = Y(e), e < 1 || e > Bt)
          return [];
        var r = Vt, o = Me(e, Vt);
        t = A(t), e -= Vt;
        for (var s = Wi(o, t); ++r < e; )
          t(r);
        return s;
      }
      function f0(e) {
        return C(e) ? ie(e, Rt) : Ge(e) ? [e] : Fe(Wc(q(e)));
      }
      function h0(e) {
        var t = ++Zp;
        return q(e) + t;
      }
      var p0 = Va(function(e, t) {
        return e + t;
      }, 0), m0 = no("ceil"), V0 = Va(function(e, t) {
        return e / t;
      }, 1), T0 = no("floor");
      function U0(e) {
        return e && e.length ? ca(e, xe, Ci) : a;
      }
      function R0(e, t) {
        return e && e.length ? ca(e, A(t, 2), Ci) : a;
      }
      function v0(e) {
        return Vs(e, xe);
      }
      function N0(e, t) {
        return Vs(e, A(t, 2));
      }
      function w0(e) {
        return e && e.length ? ca(e, xe, Hi) : a;
      }
      function M0(e, t) {
        return e && e.length ? ca(e, A(t, 2), Hi) : a;
      }
      var Z0 = Va(function(e, t) {
        return e * t;
      }, 1), J0 = no("round"), g0 = Va(function(e, t) {
        return e - t;
      }, 0);
      function k0(e) {
        return e && e.length ? bi(e, xe) : 0;
      }
      function y0(e, t) {
        return e && e.length ? bi(e, A(t, 2)) : 0;
      }
      return u.after = _T, u.ary = Cc, u.assign = BU, u.assignIn = tu, u.assignInWith = ba, u.assignWith = DU, u.at = OU, u.before = zc, u.bind = mo, u.bindAll = I1, u.bindKey = Yc, u.castArray = sU, u.chain = Dc, u.chunk = TV, u.compact = UV, u.concat = RV, u.cond = H1, u.conforms = P1, u.constant = wo, u.countBy = kT, u.create = GU, u.curry = Ic, u.curryRight = Hc, u.debounce = Pc, u.defaults = CU, u.defaultsDeep = zU, u.defer = qT, u.delay = KT, u.difference = vV, u.differenceBy = NV, u.differenceWith = wV, u.drop = MV, u.dropRight = ZV, u.dropRightWhile = JV, u.dropWhile = gV, u.fill = kV, u.filter = bT, u.flatMap = FT, u.flatMapDeep = ST, u.flatMapDepth = AT, u.flatten = Ac, u.flattenDeep = yV, u.flattenDepth = bV, u.flip = $T, u.flow = X1, u.flowRight = L1, u.fromPairs = WV, u.functions = LU, u.functionsIn = _U, u.groupBy = xT, u.initial = FV, u.intersection = SV, u.intersectionBy = AV, u.intersectionWith = xV, u.invert = KU, u.invertBy = $U, u.invokeMap = BT, u.iteratee = Mo, u.keyBy = DT, u.keys = me, u.keysIn = Ae, u.map = Ma, u.mapKeys = t1, u.mapValues = n1, u.matches = _1, u.matchesProperty = q1, u.memoize = Ja, u.merge = r1, u.mergeWith = nu, u.method = K1, u.methodOf = $1, u.mixin = Zo, u.negate = ga, u.nthArg = t0, u.omit = a1, u.omitBy = i1, u.once = eU, u.orderBy = OT, u.over = n0, u.overArgs = tU, u.overEvery = r0, u.overSome = a0, u.partial = Vo, u.partialRight = jc, u.partition = GT, u.pick = o1, u.pickBy = ru, u.property = uu, u.propertyOf = i0, u.pull = OV, u.pullAll = Qc, u.pullAllBy = GV, u.pullAllWith = CV, u.pullAt = zV, u.range = o0, u.rangeRight = l0, u.rearg = nU, u.reject = YT, u.remove = YV, u.rest = rU, u.reverse = ho, u.sampleSize = HT, u.set = s1, u.setWith = c1, u.shuffle = PT, u.slice = IV, u.sortBy = LT, u.sortedUniq = qV, u.sortedUniqBy = KV, u.split = F1, u.spread = aU, u.tail = $V, u.take = eT, u.takeRight = tT, u.takeRightWhile = nT, u.takeWhile = rT, u.tap = UT, u.throttle = iU, u.thru = wa, u.toArray = Kc, u.toPairs = au, u.toPairsIn = iu, u.toPath = f0, u.toPlainObject = eu, u.transform = u1, u.unary = oU, u.union = aT, u.unionBy = iT, u.unionWith = oT, u.uniq = lT, u.uniqBy = sT, u.uniqWith = cT, u.unset = d1, u.unzip = po, u.unzipWith = Bc, u.update = f1, u.updateWith = h1, u.values = An, u.valuesIn = p1, u.without = uT, u.words = su, u.wrap = lU, u.xor = dT, u.xorBy = fT, u.xorWith = hT, u.zip = pT, u.zipObject = mT, u.zipObjectDeep = VT, u.zipWith = TT, u.entries = au, u.entriesIn = iu, u.extend = tu, u.extendWith = ba, Zo(u, u), u.add = p0, u.attempt = cu, u.camelCase = U1, u.capitalize = ou, u.ceil = m0, u.clamp = m1, u.clone = cU, u.cloneDeep = dU, u.cloneDeepWith = fU, u.cloneWith = uU, u.conformsTo = hU, u.deburr = lu, u.defaultTo = j1, u.divide = V0, u.endsWith = R1, u.eq = ct, u.escape = v1, u.escapeRegExp = N1, u.every = yT, u.find = WT, u.findIndex = Fc, u.findKey = YU, u.findLast = ET, u.findLastIndex = Sc, u.findLastKey = IU, u.floor = T0, u.forEach = Oc, u.forEachRight = Gc, u.forIn = HU, u.forInRight = PU, u.forOwn = jU, u.forOwnRight = XU, u.get = Ro, u.gt = pU, u.gte = mU, u.has = qU, u.hasIn = vo, u.head = xc, u.identity = xe, u.includes = QT, u.indexOf = EV, u.inRange = V1, u.invoke = e1, u.isArguments = fn, u.isArray = C, u.isArrayBuffer = VU, u.isArrayLike = Se, u.isArrayLikeObject = ce, u.isBoolean = TU, u.isBuffer = Pt, u.isDate = UU, u.isElement = RU, u.isEmpty = vU, u.isEqual = NU, u.isEqualWith = wU, u.isError = To, u.isFinite = MU, u.isFunction = Wt, u.isInteger = Xc, u.isLength = ka, u.isMap = Lc, u.isMatch = ZU, u.isMatchWith = JU, u.isNaN = gU, u.isNative = kU, u.isNil = bU, u.isNull = yU, u.isNumber = _c, u.isObject = le, u.isObjectLike = se, u.isPlainObject = Ur, u.isRegExp = Uo, u.isSafeInteger = WU, u.isSet = qc, u.isString = ya, u.isSymbol = Ge, u.isTypedArray = Sn, u.isUndefined = EU, u.isWeakMap = FU, u.isWeakSet = SU, u.join = QV, u.kebabCase = w1, u.last = tt, u.lastIndexOf = BV, u.lowerCase = M1, u.lowerFirst = Z1, u.lt = AU, u.lte = xU, u.max = U0, u.maxBy = R0, u.mean = v0, u.meanBy = N0, u.min = w0, u.minBy = M0, u.stubArray = go, u.stubFalse = ko, u.stubObject = s0, u.stubString = c0, u.stubTrue = u0, u.multiply = Z0, u.nth = DV, u.noConflict = e0, u.noop = Jo, u.now = Za, u.pad = J1, u.padEnd = g1, u.padStart = k1, u.parseInt = y1, u.random = T1, u.reduce = CT, u.reduceRight = zT, u.repeat = b1, u.replace = W1, u.result = l1, u.round = J0, u.runInContext = T, u.sample = IT, u.size = jT, u.snakeCase = E1, u.some = XT, u.sortedIndex = HV, u.sortedIndexBy = PV, u.sortedIndexOf = jV, u.sortedLastIndex = XV, u.sortedLastIndexBy = LV, u.sortedLastIndexOf = _V, u.startCase = S1, u.startsWith = A1, u.subtract = g0, u.sum = k0, u.sumBy = y0, u.template = x1, u.times = d0, u.toFinite = Et, u.toInteger = Y, u.toLength = $c, u.toLower = Q1, u.toNumber = nt, u.toSafeInteger = QU, u.toString = q, u.toUpper = B1, u.trim = D1, u.trimEnd = O1, u.trimStart = G1, u.truncate = C1, u.unescape = z1, u.uniqueId = h0, u.upperCase = Y1, u.upperFirst = No, u.each = Oc, u.eachRight = Gc, u.first = xc, Zo(u, function() {
        var e = {};
        return Tt(u, function(t, r) {
          K.call(u.prototype, r) || (e[r] = t);
        }), e;
      }(), { chain: !1 }), u.VERSION = l, _e(["bind", "bindKey", "curry", "curryRight", "partial", "partialRight"], function(e) {
        u[e].placeholder = u;
      }), _e(["drop", "take"], function(e, t) {
        j.prototype[e] = function(r) {
          r = r === a ? 1 : pe(Y(r), 0);
          var o = this.__filtered__ && !t ? new j(this) : this.clone();
          return o.__filtered__ ? o.__takeCount__ = Me(r, o.__takeCount__) : o.__views__.push({
            size: Me(r, Vt),
            type: e + (o.__dir__ < 0 ? "Right" : "")
          }), o;
        }, j.prototype[e + "Right"] = function(r) {
          return this.reverse()[e](r).reverse();
        };
      }), _e(["filter", "map", "takeWhile"], function(e, t) {
        var r = t + 1, o = r == je || r == Ee;
        j.prototype[e] = function(s) {
          var d = this.clone();
          return d.__iteratees__.push({
            iteratee: A(s, 3),
            type: r
          }), d.__filtered__ = d.__filtered__ || o, d;
        };
      }), _e(["head", "last"], function(e, t) {
        var r = "take" + (t ? "Right" : "");
        j.prototype[e] = function() {
          return this[r](1).value()[0];
        };
      }), _e(["initial", "tail"], function(e, t) {
        var r = "drop" + (t ? "" : "Right");
        j.prototype[e] = function() {
          return this.__filtered__ ? new j(this) : this[r](1);
        };
      }), j.prototype.compact = function() {
        return this.filter(xe);
      }, j.prototype.find = function(e) {
        return this.filter(e).head();
      }, j.prototype.findLast = function(e) {
        return this.reverse().find(e);
      }, j.prototype.invokeMap = H(function(e, t) {
        return typeof e == "function" ? new j(this) : this.map(function(r) {
          return fr(r, e, t);
        });
      }), j.prototype.reject = function(e) {
        return this.filter(ga(A(e)));
      }, j.prototype.slice = function(e, t) {
        e = Y(e);
        var r = this;
        return r.__filtered__ && (e > 0 || t < 0) ? new j(r) : (e < 0 ? r = r.takeRight(-e) : e && (r = r.drop(e)), t !== a && (t = Y(t), r = t < 0 ? r.dropRight(-t) : r.take(t - e)), r);
      }, j.prototype.takeRightWhile = function(e) {
        return this.reverse().takeWhile(e).reverse();
      }, j.prototype.toArray = function() {
        return this.take(Vt);
      }, Tt(j.prototype, function(e, t) {
        var r = /^(?:filter|find|map|reject)|While$/.test(t), o = /^(?:head|last)$/.test(t), s = u[o ? "take" + (t == "last" ? "Right" : "") : t], d = o || /^find/.test(t);
        !s || (u.prototype[t] = function() {
          var h = this.__wrapped__, m = o ? [1] : arguments, U = h instanceof j, w = m[0], M = U || C(h), Z = function(P) {
            var X = s.apply(u, Ot([P], m));
            return o && b ? X[0] : X;
          };
          M && r && typeof w == "function" && w.length != 1 && (U = M = !1);
          var b = this.__chain__, E = !!this.__actions__.length, x = d && !b, I = U && !E;
          if (!d && M) {
            h = I ? h : new j(this);
            var Q = e.apply(h, m);
            return Q.__actions__.push({ func: wa, args: [Z], thisArg: a }), new Ke(Q, b);
          }
          return x && I ? e.apply(this, m) : (Q = this.thru(Z), x ? o ? Q.value()[0] : Q.value() : Q);
        });
      }), _e(["pop", "push", "shift", "sort", "splice", "unshift"], function(e) {
        var t = Xr[e], r = /^(?:push|sort|unshift)$/.test(e) ? "tap" : "thru", o = /^(?:pop|shift)$/.test(e);
        u.prototype[e] = function() {
          var s = arguments;
          if (o && !this.__chain__) {
            var d = this.value();
            return t.apply(C(d) ? d : [], s);
          }
          return this[r](function(h) {
            return t.apply(C(h) ? h : [], s);
          });
        };
      }), Tt(j.prototype, function(e, t) {
        var r = u[t];
        if (r) {
          var o = r.name + "";
          K.call(yn, o) || (yn[o] = []), yn[o].push({ name: t, func: r });
        }
      }), yn[ma(a, B).name] = [{
        name: "wrapper",
        func: a
      }], j.prototype.clone = zp, j.prototype.reverse = Yp, j.prototype.value = Ip, u.prototype.at = RT, u.prototype.chain = vT, u.prototype.commit = NT, u.prototype.next = wT, u.prototype.plant = ZT, u.prototype.reverse = JT, u.prototype.toJSON = u.prototype.valueOf = u.prototype.value = gT, u.prototype.first = u.prototype.head, ir && (u.prototype[ir] = MT), u;
    }, Jn = Np();
    rn ? ((rn.exports = Jn)._ = Jn, wi._ = Jn) : ve._ = Jn;
  }).call(Rr);
})($t, $t.exports);
const xu = "/alarms", XR = "/alarms?_s=isSituation==true", LR = async () => {
  try {
    const n = await sl.get(XR);
    return n.status === 204 ? { alarm: [], totalCount: 0, count: 0, offset: 0 } : n.data;
  } catch {
    return !1;
  }
}, _R = async () => {
  try {
    const i = (await sl.get(`${xu}/count`)).data, a = [];
    for (let f = 0; f < i; f += 10) {
      const p = await sl.get(`${xu}?offset=${f}`);
      a.push(p);
    }
    const c = (await Promise.all(a)).map((f) => f.data.alarm);
    return $t.exports.flattenDeep(c);
  } catch {
    return !1;
  }
}, Cn = {
  ENGINE_DBSCAN: "dbscan",
  ENGINE_DEEP_LEARNING: "cluster",
  HELLINGER_OPTION: "hellinger",
  SPACE_DISTANCE_OPTION: "alarminspaceandtimedistance"
}, qR = "/alec", KR = "/alec/engine/configuration", rf = "/alec/agreement/configuration", $R = "/alec/situation/statusList", af = async (n) => {
  try {
    return (await Ln.post(`${rf}`, {
      agreed: n
    })).status === 201;
  } catch {
    return !1;
  }
}, ev = async () => {
  try {
    const n = await Ln.get(`${rf}`);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, tv = async (n, i) => {
  try {
    return (await Ln.post(KR, {
      distanceMeasureName: i ? Cn.HELLINGER_OPTION : Cn.SPACE_DISTANCE_OPTION,
      engineName: n,
      alpha: 144.47117699,
      beta: 0.55257784,
      epsilon: 100
    })).status === 200;
  } catch {
    return !1;
  }
}, nv = async (n, i) => {
  try {
    return (await Ln.post(`${qR}/situation/${i}/${n}`, {
      headers: {
        "Access-Control-Allow-Origin": "*"
      }
    })).status === 200;
  } catch {
    return !1;
  }
}, rv = async () => {
  try {
    const n = await Ln.get($R);
    return n.status === 200, n.data;
  } catch {
    return !1;
  }
}, av = window.Pinia.defineStore, Zl = av("situationsStore", {
  state: () => ({
    situations: [],
    alarms: {}
  }),
  actions: {
    async getSituations() {
      const n = await LR(), i = await _R(), a = await rv();
      if (n && i) {
        const l = n.alarm, c = i;
        this.alarms = $t.exports.mapKeys(c, (f) => f.id), l.forEach((f) => {
          if (f.relatedAlarms.forEach((p) => {
            var V;
            p.count = (V = this.alarms[p.id]) == null ? void 0 : V.count;
          }), a.find((p) => p.id == f.id)) {
            const p = $t.exports.first(
              a.filter((V) => V.id == f.id)
            );
            f.status = p == null ? void 0 : p.status;
          }
        }), this.situations = l.filter((f) => f.status !== "REJECTED");
      }
    }
  }
}), iv = window.Vue.defineComponent, ov = window.Vue.unref, lv = window.Vue.renderList, sv = window.Vue.Fragment, Yo = window.Vue.openBlock, Io = window.Vue.createElementBlock, cv = window.Vue.toDisplayString, uv = window.Vue.normalizeClass;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const dv = { class: "alarms-list" }, fv = /* @__PURE__ */ iv({
  __name: "AlarmsCountBySeverity",
  props: {
    relatedAlarms: null,
    size: null
  },
  setup(n) {
    const i = n;
    return (a, l) => (Yo(), Io("div", dv, [
      (Yo(!0), Io(sv, null, lv(ov($t.exports.groupBy)(i == null ? void 0 : i.relatedAlarms, "severity"), (c, f) => (Yo(), Io("div", {
        class: uv(["alarm-count", [`${f.toString().toLowerCase()}-color`, i.size]]),
        key: f
      }, cv(c.length), 3))), 128))
    ]));
  }
});
const of = /* @__PURE__ */ Pe(fv, [["__scopeId", "data-v-b8c8b147"]]), hv = window.Vue.openBlock, pv = window.Vue.createElementBlock, mv = window.Vue.createElementVNode;
var Vv = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Tv = {}, Uv = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Rv = /* @__PURE__ */ mv("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM15.36,8.64l-4.95,4.95-2.3-2.3a1,1,0,0,0-1.41,0h0a1,1,0,0,0,0,1.41l3,3a1,1,0,0,0,1.41,0l5.66-5.66a1,1,0,0,0,0-1.41h0A1,1,0,0,0,15.36,8.64Z" }, null, -1), vv = [
  Rv
];
function Nv(n, i) {
  return hv(), pv("svg", Uv, vv);
}
var lf = /* @__PURE__ */ Vv(Tv, [["render", Nv]]);
var wv = Object.defineProperty, Qu = Object.getOwnPropertySymbols, Mv = Object.prototype.hasOwnProperty, Zv = Object.prototype.propertyIsEnumerable, Bu = (n, i, a) => i in n ? wv(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Du = (n, i) => {
  for (var a in i || (i = {}))
    Mv.call(i, a) && Bu(n, a, i[a]);
  if (Qu)
    for (var a of Qu(i))
      Zv.call(i, a) && Bu(n, a, i[a]);
  return n;
};
const Jv = window.Vue.defineComponent, gv = window.Vue.toRaw, Ho = window.Vue.h;
var kv = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const yv = {
  icon: {
    type: Object,
    required: !1
  },
  flex: {
    type: Boolean,
    default: !1
  },
  title: {
    type: String,
    required: !1
  }
}, bv = Jv({
  props: yv,
  render() {
    const n = this.$attrs, i = n.class ? n.class.split(" ").reduce((c, f) => (c[f] = !0, c), {}) : {}, a = {};
    i["feather-icon"] = !0, this.flex && (i["feather-icon-flex"] = !0), this.title ? (a["aria-label"] = this.title, a["aria-hidden"] = "false") : a["aria-hidden"] = "true", a.focusable = "false", a.role = "img";
    let l = gv(this.icon);
    return this.$slots.default ? Ho("span", { class: "feather-icon-container" }, [
      Ho(this.$slots.default()[0], Du({
        class: i
      }, a))
    ]) : Ho(l, Du({
      class: i
    }, a));
  }
});
var ft = /* @__PURE__ */ kv(bv, [["__scopeId", "data-v-52cbf270"]]);
const Wv = window.Vue.defineComponent, Ou = window.Vue.normalizeClass, Wa = window.Vue.createElementVNode, Ev = window.Vue.toDisplayString, Gu = window.Vue.unref, Cu = window.Vue.createVNode, zu = window.Vue.openBlock, Yu = window.Vue.createElementBlock, Fv = window.Vue.createCommentVNode;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const Sv = { class: "content" }, Av = { class: "title-row" }, xv = { class: "title" }, Qv = {
  key: 0,
  class: "accepted"
}, Bv = /* @__PURE__ */ Wv({
  __name: "SituationCard",
  props: {
    alarmInfo: null,
    selected: { type: Boolean }
  },
  emits: ["situation-selected"],
  setup(n, { emit: i }) {
    const a = n, l = () => {
      var c;
      i("situation-selected", (c = a.alarmInfo) == null ? void 0 : c.id);
    };
    return (c, f) => {
      var p, V, v, k;
      return zu(), Yu("div", {
        onClick: l,
        class: Ou(["card", { selected: a.selected }])
      }, [
        Wa("div", {
          class: Ou(["severity-line", [`${(V = (p = a.alarmInfo) == null ? void 0 : p.severity) == null ? void 0 : V.toLowerCase()}-bg dark`]])
        }, null, 2),
        Wa("div", Sv, [
          Wa("div", Av, [
            Wa("div", xv, "[ " + Ev((v = a.alarmInfo) == null ? void 0 : v.id) + " ]", 1),
            a.alarmInfo.status == "ACCEPTED" ? (zu(), Yu("div", Qv, [
              Cu(Gu(ft), {
                icon: Gu(lf),
                "aria-hidden": "true",
                class: "icon accepted"
              }, null, 8, ["icon"])
            ])) : Fv("", !0)
          ]),
          Cu(of, {
            relatedAlarms: (k = a.alarmInfo) == null ? void 0 : k.relatedAlarms,
            size: "normal"
          }, null, 8, ["relatedAlarms"])
        ])
      ], 2);
    };
  }
});
const Dv = /* @__PURE__ */ Pe(Bv, [["__scopeId", "data-v-f87e0485"]]);
const Ea = window.Vue.ref, Ov = window.Vue.inject, Gv = window.Vue.computed, Cv = window.Vue.onMounted, zv = {
  id: {
    type: String
  },
  controls: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, Yv = (n) => {
  const i = Ea(!1), a = Ea(), l = Ea(n.controls), c = Ea(n.id), f = () => {
    a.value && a.value.focus();
  }, p = Ov("registerTab");
  Cv(() => {
    if (a.value && p) {
      const v = a.value.parentElement, k = v && v.parentElement ? v.parentElement : void 0, J = Array.from(k ? k.children : []).filter((g) => g.querySelectorAll("[role=tab]").length), y = v ? J.indexOf(v) : -1;
      p({
        el: a.value,
        focus: f,
        disabled: n.disabled,
        selected: i,
        id: c,
        controls: l,
        index: y
      });
    }
  });
  const V = Gv(() => ({
    role: "tab",
    ref: "tab",
    tabindex: i.value ? 0 : -1,
    id: c.value,
    "aria-selected": i.value,
    "aria-controls": l.value,
    "aria-disabled": n.disabled,
    "data-ref-id": "feather-tab"
  }));
  return {
    selected: i,
    attrs: V,
    tab: a
  };
};
const Iv = function(n, i) {
  return window ? window.setTimeout(n, i) : setTimeout(n, i);
}, Hv = function(n) {
  return window ? window.clearTimeout(n) : clearTimeout(n);
};
var Pv = Object.defineProperty, jv = Object.defineProperties, Xv = Object.getOwnPropertyDescriptors, Iu = Object.getOwnPropertySymbols, Lv = Object.prototype.hasOwnProperty, _v = Object.prototype.propertyIsEnumerable, Hu = (n, i, a) => i in n ? Pv(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Pu = (n, i) => {
  for (var a in i || (i = {}))
    Lv.call(i, a) && Hu(n, a, i[a]);
  if (Iu)
    for (var a of Iu(i))
      _v.call(i, a) && Hu(n, a, i[a]);
  return n;
}, qv = (n, i) => jv(n, Xv(i));
const Kv = window.Vue.defineComponent, $v = window.Vue.h;
var eN = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const tN = {
  center: {
    type: Boolean,
    default: !1
  }
}, nN = Kv({
  props: tN,
  data() {
    return {
      pressed: !1,
      active: !1,
      styles: {},
      failsafe: -1
    };
  },
  computed: {
    parent() {
      return this.$el.parentNode;
    }
  },
  methods: {
    onClick(n) {
      this.pressed = !1, this.active = !1, requestAnimationFrame(() => {
        const { clientWidth: i, clientHeight: a } = this.parent, l = Math.round(Math.max(i, a));
        let c = {
          top: "0px",
          left: "0px"
        };
        if (!this.center) {
          const f = this.parent.getBoundingClientRect(), p = n.pageY, V = n.pageX;
          c = {
            top: `${p - f.top - l / 2 - document.documentElement.scrollTop}px`,
            left: `${V - f.left - l / 2 - document.documentElement.scrollLeft}px`
          };
        }
        this.styles = qv(Pu({}, c), {
          height: `${l}px`,
          width: `${l}px`
        }), this.pressed = !0, requestAnimationFrame(() => {
          this.active = !0, Hv(this.failsafe), this.failsafe = Iv(() => {
            this.pressed = !1, this.active = !1;
          }, 380 + 100 + 20);
        });
      });
    }
  },
  render() {
    if (this.pressed !== !1)
      return $v("div", {
        style: Pu({}, this.styles),
        class: ["ripple", { active: this.active, center: this.center }],
        onTransitionEnd: () => {
          this.pressed = !1, this.active = !1;
        },
        onTransitionCancel: () => {
          this.pressed = !1, this.active = !1;
        }
      });
  },
  mounted() {
    this.parent.addEventListener("click", this.onClick);
    const n = window.getComputedStyle(this.parent);
    this.parent.style.overflow = "hidden", !(n.position === "relative" || n.position === "absolute" || n.position === "fixed") && (this.parent.style.position = "relative");
  },
  unmounted() {
    this.parent.removeEventListener("click", this.onClick);
  }
});
var Pn = /* @__PURE__ */ eN(nN, [["__scopeId", "data-v-18e2a5db"]]);
const Ie = function(n) {
  n = n || "feather";
  const i = Math.floor(Math.random() * 1e9);
  return [n.replace(/\s+/g, "-"), Date.now(), i].join("-");
}, xt = {
  PAGEUP: 33,
  PAGEDOWN: 34,
  UP: 38,
  DOWN: 40,
  LEFT: 37,
  RIGHT: 39,
  HOME: 36,
  END: 35,
  ENTER: 13,
  SPACE: 32,
  ESCAPE: 27,
  BACKSPACE: 8,
  DELETE: 46
}, ju = window.Vue.ref, rN = window.Vue.toRef, aN = window.Vue.watch, Xu = window.Vue.provide, iN = {
  prop: "modelValue",
  event: "update:modelValue"
}, oN = {
  "update:modelValue": (n) => !0
}, lN = {
  modelValue: {
    type: Number,
    default: 0
  },
  vertical: {
    type: Boolean,
    default: !0
  }
}, sN = (n, i) => {
  const a = rN(n, "modelValue"), l = ju(n.modelValue), c = ju([]);
  aN(a, (S) => {
    v(S);
  });
  const f = (S) => {
    S.preventDefault(), c.value.some((O, G) => O.tab && O.tab.el.contains(S.target) ? (V(G), v(G), !0) : !1);
  }, p = (S) => {
    if (((ee) => ee.shiftKey || ee.ctrlKey || ee.metaKey || ee.altKey)(S))
      return;
    const G = S.keyCode, B = (ee) => {
      ee.stopPropagation(), ee.preventDefault();
    }, _ = c.value.filter((ee) => ee.tab && !ee.tab.disabled), ne = c.value.findIndex((ee) => ee.tab && ee.tab.el.contains(document.activeElement));
    let oe = ne !== -1 ? ne : l.value;
    const F = [xt.RIGHT], Ue = [xt.LEFT], fe = [xt.ENTER, xt.SPACE];
    n.vertical && (F.push(xt.DOWN), Ue.push(xt.UP)), F.indexOf(G) > -1 ? (oe++, oe >= _.length && (oe = 0), B(S), V(c.value.indexOf(_[oe]))) : Ue.indexOf(G) > -1 && (oe--, oe < 0 && (oe = _.length - 1), B(S), V(c.value.indexOf(_[oe]))), fe.indexOf(G) > -1 && v(oe);
  }, V = (S) => {
    c.value.forEach(function(O, G) {
      S === G && O.tab && O.tab.focus();
    });
  }, v = (S) => {
    const O = c.value[S];
    !O || O.tab && O.tab.disabled || (c.value.forEach((G, B) => {
      G.tab && (G.tab.selected = S === B), G.panel && (G.panel.selected = S === B);
    }), l.value = S, i.emit("update:modelValue", S));
  };
  Xu("registerTab", (S) => {
    const O = S.index;
    O > -1 && (c.value[O] = { ...c.value[O], tab: S }, c.value = [...c.value], y());
  }), Xu("registerPanel", (S) => {
    const O = S.index;
    O > -1 && (c.value[O] = {
      ...c.value[O],
      panel: S
    }, c.value = [...c.value], y());
  });
  const y = () => {
    c.value.forEach(({ tab: S, panel: O }, G) => {
      if (O && S) {
        const B = S.id || Ie("tab"), _ = S.controls || Ie("panel");
        S.controls = _, S.id = B, O.tab = B, O.id = _;
      }
      G === l.value && (O && (O.selected = !0), S && (S.selected = !0));
    });
  };
  return {
    listeners: {
      click: f,
      keydown: p
    },
    attrs: {
      role: "tablist"
    },
    selected: l,
    pairs: c
  };
}, Fa = window.Vue.ref, cN = window.Vue.inject, uN = window.Vue.computed, dN = window.Vue.onMounted, fN = {
  id: {
    type: String
  },
  tab: {
    type: String
  }
}, hN = (n) => {
  const i = Fa(!1), a = Fa(), l = Fa(n.tab), c = Fa(n.id), f = cN("registerPanel");
  dN(() => {
    if (f) {
      const V = a.value, v = V && V.parentElement ? V.parentElement : void 0, k = V ? Array.from(v ? v.children : []).indexOf(V) : -1;
      f({
        selected: i,
        id: c,
        tab: l,
        el: a.value,
        index: k
      });
    }
  });
  const p = uN(() => ({
    role: "tabpanel",
    id: c.value,
    ref: "panel",
    tabindex: "0",
    "aria-expanded": i.value,
    "aria-labelledby": l.value,
    "data-ref-id": "feather-tab-panel"
  }));
  return {
    selected: i,
    attrs: p,
    panel: a
  };
}, Jl = window.Vue.defineComponent, pN = window.Vue.resolveComponent, gl = window.Vue.openBlock, kl = window.Vue.createElementBlock, yr = window.Vue.createElementVNode, sf = window.Vue.mergeProps, Pa = window.Vue.renderSlot, mN = window.Vue.createVNode, VN = window.Vue.normalizeStyle, TN = window.Vue.toHandlers, UN = window.Vue.withDirectives, RN = window.Vue.normalizeProps, vN = window.Vue.guardReactiveProps, NN = window.Vue.vShow;
var yl = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const wN = zv, MN = Jl({
  props: wN,
  setup(n) {
    return Yv(n);
  },
  components: {
    FeatherRipple: Pn
  }
}), ZN = { role: "presentation" }, JN = { class: "tab-text" };
function gN(n, i, a, l, c, f) {
  const p = pN("FeatherRipple");
  return gl(), kl("li", ZN, [
    yr("button", sf(n.attrs, {
      class: ["tab hover focus", { disabled: n.disabled, selected: n.selected }]
    }), [
      yr("span", JN, [
        Pa(n.$slots, "default", {}, void 0, !0)
      ]),
      mN(p)
    ], 16)
  ]);
}
var kN = /* @__PURE__ */ yl(MN, [["render", gN], ["__scopeId", "data-v-e6bb52b6"]]);
const yN = lN, bN = oN, WN = Jl({
  model: iN,
  emits: bN,
  props: yN,
  setup(n, i) {
    return sN(n, i);
  },
  data: () => ({
    transform: "",
    durationNumber: 250,
    width: "1px",
    ro: void 0
  }),
  watch: {
    selected() {
      this.updateSlider();
    },
    pairs: {
      handler(n) {
        n && n.length && this.ro && n.forEach((i) => {
          i.tab && this.ro.observe(i.tab.el);
        });
      },
      immediate: !0
    }
  },
  computed: {
    duration() {
      return `${this.durationNumber}ms`;
    }
  },
  methods: {
    updateSlider() {
      this.$refs.slider.getBoundingClientRect().width < 5 ? this.durationNumber = 0 : this.durationNumber = 250, this.$nextTick(() => {
        const n = this.$el.getBoundingClientRect(), i = this.$el.querySelector("[aria-selected='true']").getBoundingClientRect(), a = i.left - n.left, l = i.height - 2;
        this.width = `${i.width}px`, this.transform = `translateX(${a}px) translateY(${l}px)`;
      });
    }
  },
  mounted() {
    this.updateSlider(), this.ro = new ResizeObserver(() => {
      this.updateSlider();
    });
  },
  unmounted() {
    this.ro.disconnect();
  }
}), EN = { class: "feather-tab-container" }, FN = { class: "tab-panels" };
function SN(n, i, a, l, c, f) {
  return gl(), kl("div", EN, [
    yr("div", {
      ref: "slider",
      class: "feather-tab-slider",
      style: VN({
        transform: n.transform,
        "transition-duration": n.duration,
        width: n.width
      })
    }, null, 4),
    yr("ul", sf(n.attrs, TN(n.listeners)), [
      Pa(n.$slots, "tabs", {}, void 0, !0)
    ], 16),
    yr("div", FN, [
      Pa(n.$slots, "default", {}, void 0, !0)
    ])
  ]);
}
var AN = /* @__PURE__ */ yl(WN, [["render", SN], ["__scopeId", "data-v-27adffb9"]]);
const xN = fN, QN = Jl({
  props: xN,
  setup(n) {
    return hN(n);
  }
});
function BN(n, i, a, l, c, f) {
  return UN((gl(), kl("div", RN(vN(n.attrs)), [
    Pa(n.$slots, "default")
  ], 16)), [
    [NN, n.selected]
  ]);
}
var DN = /* @__PURE__ */ yl(QN, [["render", BN]]);
const ON = window.Vue.defineComponent, GN = window.Vue.toDisplayString, CN = window.Vue.normalizeClass, zN = window.Vue.openBlock, YN = window.Vue.createElementBlock, IN = window.Vue.createCommentVNode, HN = /* @__PURE__ */ ON({
  __name: "SeverityStatus",
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const i = n;
    return (a, l) => i != null && i.severity ? (zN(), YN("span", {
      key: 0,
      class: CN(["severity-status", [`${i.severity.toLowerCase()}-color`]])
    }, GN(i.severity), 3)) : IN("", !0);
  }
});
const cf = /* @__PURE__ */ Pe(HN, [["__scopeId", "data-v-83c2cdce"]]);
function Vn(n) {
  if (n === null || n === !0 || n === !1)
    return NaN;
  var i = Number(n);
  return isNaN(i) ? i : i < 0 ? Math.ceil(i) : Math.floor(i);
}
function We(n, i) {
  if (i.length < n)
    throw new TypeError(n + " argument" + (n > 1 ? "s" : "") + " required, but only " + i.length + " present");
}
function Nt(n) {
  We(1, arguments);
  var i = Object.prototype.toString.call(n);
  return n instanceof Date || typeof n == "object" && i === "[object Date]" ? new Date(n.getTime()) : typeof n == "number" || i === "[object Number]" ? new Date(n) : ((typeof n == "string" || i === "[object String]") && typeof console < "u" && (console.warn("Starting with v2.0.0-beta.1 date-fns doesn't accept strings as date arguments. Please use `parseISO` to parse strings. See: https://github.com/date-fns/date-fns/blob/master/docs/upgradeGuide.md#string-arguments"), console.warn(new Error().stack)), new Date(NaN));
}
function PN(n, i) {
  We(2, arguments);
  var a = Nt(n).getTime(), l = Vn(i);
  return new Date(a + l);
}
var jN = {};
function ri() {
  return jN;
}
function XN(n) {
  var i = new Date(Date.UTC(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds(), n.getMilliseconds()));
  return i.setUTCFullYear(n.getFullYear()), n.getTime() - i.getTime();
}
function LN(n) {
  return We(1, arguments), n instanceof Date || typeof n == "object" && Object.prototype.toString.call(n) === "[object Date]";
}
function _N(n) {
  if (We(1, arguments), !LN(n) && typeof n != "number")
    return !1;
  var i = Nt(n);
  return !isNaN(Number(i));
}
function qN(n, i) {
  We(2, arguments);
  var a = Vn(i);
  return PN(n, -a);
}
var KN = 864e5;
function $N(n) {
  We(1, arguments);
  var i = Nt(n), a = i.getTime();
  i.setUTCMonth(0, 1), i.setUTCHours(0, 0, 0, 0);
  var l = i.getTime(), c = a - l;
  return Math.floor(c / KN) + 1;
}
function ja(n) {
  We(1, arguments);
  var i = 1, a = Nt(n), l = a.getUTCDay(), c = (l < i ? 7 : 0) + l - i;
  return a.setUTCDate(a.getUTCDate() - c), a.setUTCHours(0, 0, 0, 0), a;
}
function uf(n) {
  We(1, arguments);
  var i = Nt(n), a = i.getUTCFullYear(), l = new Date(0);
  l.setUTCFullYear(a + 1, 0, 4), l.setUTCHours(0, 0, 0, 0);
  var c = ja(l), f = new Date(0);
  f.setUTCFullYear(a, 0, 4), f.setUTCHours(0, 0, 0, 0);
  var p = ja(f);
  return i.getTime() >= c.getTime() ? a + 1 : i.getTime() >= p.getTime() ? a : a - 1;
}
function ew(n) {
  We(1, arguments);
  var i = uf(n), a = new Date(0);
  a.setUTCFullYear(i, 0, 4), a.setUTCHours(0, 0, 0, 0);
  var l = ja(a);
  return l;
}
var tw = 6048e5;
function nw(n) {
  We(1, arguments);
  var i = Nt(n), a = ja(i).getTime() - ew(i).getTime();
  return Math.round(a / tw) + 1;
}
function Xa(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = ri(), y = Vn((a = (l = (c = (f = i == null ? void 0 : i.weekStartsOn) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.weekStartsOn) !== null && c !== void 0 ? c : J.weekStartsOn) !== null && l !== void 0 ? l : (v = J.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.weekStartsOn) !== null && a !== void 0 ? a : 0);
  if (!(y >= 0 && y <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  var g = Nt(n), z = g.getUTCDay(), S = (z < y ? 7 : 0) + z - y;
  return g.setUTCDate(g.getUTCDate() - S), g.setUTCHours(0, 0, 0, 0), g;
}
function df(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = Nt(n), y = J.getUTCFullYear(), g = ri(), z = Vn((a = (l = (c = (f = i == null ? void 0 : i.firstWeekContainsDate) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.firstWeekContainsDate) !== null && c !== void 0 ? c : g.firstWeekContainsDate) !== null && l !== void 0 ? l : (v = g.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.firstWeekContainsDate) !== null && a !== void 0 ? a : 1);
  if (!(z >= 1 && z <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var S = new Date(0);
  S.setUTCFullYear(y + 1, 0, z), S.setUTCHours(0, 0, 0, 0);
  var O = Xa(S, i), G = new Date(0);
  G.setUTCFullYear(y, 0, z), G.setUTCHours(0, 0, 0, 0);
  var B = Xa(G, i);
  return J.getTime() >= O.getTime() ? y + 1 : J.getTime() >= B.getTime() ? y : y - 1;
}
function rw(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = ri(), y = Vn((a = (l = (c = (f = i == null ? void 0 : i.firstWeekContainsDate) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.firstWeekContainsDate) !== null && c !== void 0 ? c : J.firstWeekContainsDate) !== null && l !== void 0 ? l : (v = J.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.firstWeekContainsDate) !== null && a !== void 0 ? a : 1), g = df(n, i), z = new Date(0);
  z.setUTCFullYear(g, 0, y), z.setUTCHours(0, 0, 0, 0);
  var S = Xa(z, i);
  return S;
}
var aw = 6048e5;
function iw(n, i) {
  We(1, arguments);
  var a = Nt(n), l = Xa(a, i).getTime() - rw(a, i).getTime();
  return Math.round(l / aw) + 1;
}
function $(n, i) {
  for (var a = n < 0 ? "-" : "", l = Math.abs(n).toString(); l.length < i; )
    l = "0" + l;
  return a + l;
}
var ow = {
  y: function(n, i) {
    var a = n.getUTCFullYear(), l = a > 0 ? a : 1 - a;
    return $(i === "yy" ? l % 100 : l, i.length);
  },
  M: function(n, i) {
    var a = n.getUTCMonth();
    return i === "M" ? String(a + 1) : $(a + 1, 2);
  },
  d: function(n, i) {
    return $(n.getUTCDate(), i.length);
  },
  a: function(n, i) {
    var a = n.getUTCHours() / 12 >= 1 ? "pm" : "am";
    switch (i) {
      case "a":
      case "aa":
        return a.toUpperCase();
      case "aaa":
        return a;
      case "aaaaa":
        return a[0];
      case "aaaa":
      default:
        return a === "am" ? "a.m." : "p.m.";
    }
  },
  h: function(n, i) {
    return $(n.getUTCHours() % 12 || 12, i.length);
  },
  H: function(n, i) {
    return $(n.getUTCHours(), i.length);
  },
  m: function(n, i) {
    return $(n.getUTCMinutes(), i.length);
  },
  s: function(n, i) {
    return $(n.getUTCSeconds(), i.length);
  },
  S: function(n, i) {
    var a = i.length, l = n.getUTCMilliseconds(), c = Math.floor(l * Math.pow(10, a - 3));
    return $(c, i.length);
  }
};
const jt = ow;
var Bn = {
  am: "am",
  pm: "pm",
  midnight: "midnight",
  noon: "noon",
  morning: "morning",
  afternoon: "afternoon",
  evening: "evening",
  night: "night"
}, lw = {
  G: function(n, i, a) {
    var l = n.getUTCFullYear() > 0 ? 1 : 0;
    switch (i) {
      case "G":
      case "GG":
      case "GGG":
        return a.era(l, {
          width: "abbreviated"
        });
      case "GGGGG":
        return a.era(l, {
          width: "narrow"
        });
      case "GGGG":
      default:
        return a.era(l, {
          width: "wide"
        });
    }
  },
  y: function(n, i, a) {
    if (i === "yo") {
      var l = n.getUTCFullYear(), c = l > 0 ? l : 1 - l;
      return a.ordinalNumber(c, {
        unit: "year"
      });
    }
    return jt.y(n, i);
  },
  Y: function(n, i, a, l) {
    var c = df(n, l), f = c > 0 ? c : 1 - c;
    if (i === "YY") {
      var p = f % 100;
      return $(p, 2);
    }
    return i === "Yo" ? a.ordinalNumber(f, {
      unit: "year"
    }) : $(f, i.length);
  },
  R: function(n, i) {
    var a = uf(n);
    return $(a, i.length);
  },
  u: function(n, i) {
    var a = n.getUTCFullYear();
    return $(a, i.length);
  },
  Q: function(n, i, a) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (i) {
      case "Q":
        return String(l);
      case "QQ":
        return $(l, 2);
      case "Qo":
        return a.ordinalNumber(l, {
          unit: "quarter"
        });
      case "QQQ":
        return a.quarter(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "QQQQQ":
        return a.quarter(l, {
          width: "narrow",
          context: "formatting"
        });
      case "QQQQ":
      default:
        return a.quarter(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  q: function(n, i, a) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (i) {
      case "q":
        return String(l);
      case "qq":
        return $(l, 2);
      case "qo":
        return a.ordinalNumber(l, {
          unit: "quarter"
        });
      case "qqq":
        return a.quarter(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "qqqqq":
        return a.quarter(l, {
          width: "narrow",
          context: "standalone"
        });
      case "qqqq":
      default:
        return a.quarter(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  M: function(n, i, a) {
    var l = n.getUTCMonth();
    switch (i) {
      case "M":
      case "MM":
        return jt.M(n, i);
      case "Mo":
        return a.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "MMM":
        return a.month(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "MMMMM":
        return a.month(l, {
          width: "narrow",
          context: "formatting"
        });
      case "MMMM":
      default:
        return a.month(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  L: function(n, i, a) {
    var l = n.getUTCMonth();
    switch (i) {
      case "L":
        return String(l + 1);
      case "LL":
        return $(l + 1, 2);
      case "Lo":
        return a.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "LLL":
        return a.month(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "LLLLL":
        return a.month(l, {
          width: "narrow",
          context: "standalone"
        });
      case "LLLL":
      default:
        return a.month(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  w: function(n, i, a, l) {
    var c = iw(n, l);
    return i === "wo" ? a.ordinalNumber(c, {
      unit: "week"
    }) : $(c, i.length);
  },
  I: function(n, i, a) {
    var l = nw(n);
    return i === "Io" ? a.ordinalNumber(l, {
      unit: "week"
    }) : $(l, i.length);
  },
  d: function(n, i, a) {
    return i === "do" ? a.ordinalNumber(n.getUTCDate(), {
      unit: "date"
    }) : jt.d(n, i);
  },
  D: function(n, i, a) {
    var l = $N(n);
    return i === "Do" ? a.ordinalNumber(l, {
      unit: "dayOfYear"
    }) : $(l, i.length);
  },
  E: function(n, i, a) {
    var l = n.getUTCDay();
    switch (i) {
      case "E":
      case "EE":
      case "EEE":
        return a.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "EEEEE":
        return a.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "EEEEEE":
        return a.day(l, {
          width: "short",
          context: "formatting"
        });
      case "EEEE":
      default:
        return a.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  e: function(n, i, a, l) {
    var c = n.getUTCDay(), f = (c - l.weekStartsOn + 8) % 7 || 7;
    switch (i) {
      case "e":
        return String(f);
      case "ee":
        return $(f, 2);
      case "eo":
        return a.ordinalNumber(f, {
          unit: "day"
        });
      case "eee":
        return a.day(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "eeeee":
        return a.day(c, {
          width: "narrow",
          context: "formatting"
        });
      case "eeeeee":
        return a.day(c, {
          width: "short",
          context: "formatting"
        });
      case "eeee":
      default:
        return a.day(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  c: function(n, i, a, l) {
    var c = n.getUTCDay(), f = (c - l.weekStartsOn + 8) % 7 || 7;
    switch (i) {
      case "c":
        return String(f);
      case "cc":
        return $(f, i.length);
      case "co":
        return a.ordinalNumber(f, {
          unit: "day"
        });
      case "ccc":
        return a.day(c, {
          width: "abbreviated",
          context: "standalone"
        });
      case "ccccc":
        return a.day(c, {
          width: "narrow",
          context: "standalone"
        });
      case "cccccc":
        return a.day(c, {
          width: "short",
          context: "standalone"
        });
      case "cccc":
      default:
        return a.day(c, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  i: function(n, i, a) {
    var l = n.getUTCDay(), c = l === 0 ? 7 : l;
    switch (i) {
      case "i":
        return String(c);
      case "ii":
        return $(c, i.length);
      case "io":
        return a.ordinalNumber(c, {
          unit: "day"
        });
      case "iii":
        return a.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "iiiii":
        return a.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "iiiiii":
        return a.day(l, {
          width: "short",
          context: "formatting"
        });
      case "iiii":
      default:
        return a.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  a: function(n, i, a) {
    var l = n.getUTCHours(), c = l / 12 >= 1 ? "pm" : "am";
    switch (i) {
      case "a":
      case "aa":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "aaa":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "aaaaa":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "aaaa":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  b: function(n, i, a) {
    var l = n.getUTCHours(), c;
    switch (l === 12 ? c = Bn.noon : l === 0 ? c = Bn.midnight : c = l / 12 >= 1 ? "pm" : "am", i) {
      case "b":
      case "bb":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "bbb":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "bbbbb":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "bbbb":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  B: function(n, i, a) {
    var l = n.getUTCHours(), c;
    switch (l >= 17 ? c = Bn.evening : l >= 12 ? c = Bn.afternoon : l >= 4 ? c = Bn.morning : c = Bn.night, i) {
      case "B":
      case "BB":
      case "BBB":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "BBBBB":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "BBBB":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  h: function(n, i, a) {
    if (i === "ho") {
      var l = n.getUTCHours() % 12;
      return l === 0 && (l = 12), a.ordinalNumber(l, {
        unit: "hour"
      });
    }
    return jt.h(n, i);
  },
  H: function(n, i, a) {
    return i === "Ho" ? a.ordinalNumber(n.getUTCHours(), {
      unit: "hour"
    }) : jt.H(n, i);
  },
  K: function(n, i, a) {
    var l = n.getUTCHours() % 12;
    return i === "Ko" ? a.ordinalNumber(l, {
      unit: "hour"
    }) : $(l, i.length);
  },
  k: function(n, i, a) {
    var l = n.getUTCHours();
    return l === 0 && (l = 24), i === "ko" ? a.ordinalNumber(l, {
      unit: "hour"
    }) : $(l, i.length);
  },
  m: function(n, i, a) {
    return i === "mo" ? a.ordinalNumber(n.getUTCMinutes(), {
      unit: "minute"
    }) : jt.m(n, i);
  },
  s: function(n, i, a) {
    return i === "so" ? a.ordinalNumber(n.getUTCSeconds(), {
      unit: "second"
    }) : jt.s(n, i);
  },
  S: function(n, i) {
    return jt.S(n, i);
  },
  X: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    if (f === 0)
      return "Z";
    switch (i) {
      case "X":
        return _u(f);
      case "XXXX":
      case "XX":
        return pn(f);
      case "XXXXX":
      case "XXX":
      default:
        return pn(f, ":");
    }
  },
  x: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "x":
        return _u(f);
      case "xxxx":
      case "xx":
        return pn(f);
      case "xxxxx":
      case "xxx":
      default:
        return pn(f, ":");
    }
  },
  O: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "O":
      case "OO":
      case "OOO":
        return "GMT" + Lu(f, ":");
      case "OOOO":
      default:
        return "GMT" + pn(f, ":");
    }
  },
  z: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "z":
      case "zz":
      case "zzz":
        return "GMT" + Lu(f, ":");
      case "zzzz":
      default:
        return "GMT" + pn(f, ":");
    }
  },
  t: function(n, i, a, l) {
    var c = l._originalDate || n, f = Math.floor(c.getTime() / 1e3);
    return $(f, i.length);
  },
  T: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTime();
    return $(f, i.length);
  }
};
function Lu(n, i) {
  var a = n > 0 ? "-" : "+", l = Math.abs(n), c = Math.floor(l / 60), f = l % 60;
  if (f === 0)
    return a + String(c);
  var p = i || "";
  return a + String(c) + p + $(f, 2);
}
function _u(n, i) {
  if (n % 60 === 0) {
    var a = n > 0 ? "-" : "+";
    return a + $(Math.abs(n) / 60, 2);
  }
  return pn(n, i);
}
function pn(n, i) {
  var a = i || "", l = n > 0 ? "-" : "+", c = Math.abs(n), f = $(Math.floor(c / 60), 2), p = $(c % 60, 2);
  return l + f + a + p;
}
const sw = lw;
var qu = function(n, i) {
  switch (n) {
    case "P":
      return i.date({
        width: "short"
      });
    case "PP":
      return i.date({
        width: "medium"
      });
    case "PPP":
      return i.date({
        width: "long"
      });
    case "PPPP":
    default:
      return i.date({
        width: "full"
      });
  }
}, ff = function(n, i) {
  switch (n) {
    case "p":
      return i.time({
        width: "short"
      });
    case "pp":
      return i.time({
        width: "medium"
      });
    case "ppp":
      return i.time({
        width: "long"
      });
    case "pppp":
    default:
      return i.time({
        width: "full"
      });
  }
}, cw = function(n, i) {
  var a = n.match(/(P+)(p+)?/) || [], l = a[1], c = a[2];
  if (!c)
    return qu(n, i);
  var f;
  switch (l) {
    case "P":
      f = i.dateTime({
        width: "short"
      });
      break;
    case "PP":
      f = i.dateTime({
        width: "medium"
      });
      break;
    case "PPP":
      f = i.dateTime({
        width: "long"
      });
      break;
    case "PPPP":
    default:
      f = i.dateTime({
        width: "full"
      });
      break;
  }
  return f.replace("{{date}}", qu(l, i)).replace("{{time}}", ff(c, i));
}, uw = {
  p: ff,
  P: cw
};
const dw = uw;
var fw = ["D", "DD"], hw = ["YY", "YYYY"];
function pw(n) {
  return fw.indexOf(n) !== -1;
}
function mw(n) {
  return hw.indexOf(n) !== -1;
}
function Ku(n, i, a) {
  if (n === "YYYY")
    throw new RangeError("Use `yyyy` instead of `YYYY` (in `".concat(i, "`) for formatting years to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "YY")
    throw new RangeError("Use `yy` instead of `YY` (in `".concat(i, "`) for formatting years to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "D")
    throw new RangeError("Use `d` instead of `D` (in `".concat(i, "`) for formatting days of the month to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "DD")
    throw new RangeError("Use `dd` instead of `DD` (in `".concat(i, "`) for formatting days of the month to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
}
var Vw = {
  lessThanXSeconds: {
    one: "less than a second",
    other: "less than {{count}} seconds"
  },
  xSeconds: {
    one: "1 second",
    other: "{{count}} seconds"
  },
  halfAMinute: "half a minute",
  lessThanXMinutes: {
    one: "less than a minute",
    other: "less than {{count}} minutes"
  },
  xMinutes: {
    one: "1 minute",
    other: "{{count}} minutes"
  },
  aboutXHours: {
    one: "about 1 hour",
    other: "about {{count}} hours"
  },
  xHours: {
    one: "1 hour",
    other: "{{count}} hours"
  },
  xDays: {
    one: "1 day",
    other: "{{count}} days"
  },
  aboutXWeeks: {
    one: "about 1 week",
    other: "about {{count}} weeks"
  },
  xWeeks: {
    one: "1 week",
    other: "{{count}} weeks"
  },
  aboutXMonths: {
    one: "about 1 month",
    other: "about {{count}} months"
  },
  xMonths: {
    one: "1 month",
    other: "{{count}} months"
  },
  aboutXYears: {
    one: "about 1 year",
    other: "about {{count}} years"
  },
  xYears: {
    one: "1 year",
    other: "{{count}} years"
  },
  overXYears: {
    one: "over 1 year",
    other: "over {{count}} years"
  },
  almostXYears: {
    one: "almost 1 year",
    other: "almost {{count}} years"
  }
}, Tw = function(n, i, a) {
  var l, c = Vw[n];
  return typeof c == "string" ? l = c : i === 1 ? l = c.one : l = c.other.replace("{{count}}", i.toString()), a != null && a.addSuffix ? a.comparison && a.comparison > 0 ? "in " + l : l + " ago" : l;
};
const Uw = Tw;
function Po(n) {
  return function() {
    var i = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {}, a = i.width ? String(i.width) : n.defaultWidth, l = n.formats[a] || n.formats[n.defaultWidth];
    return l;
  };
}
var Rw = {
  full: "EEEE, MMMM do, y",
  long: "MMMM do, y",
  medium: "MMM d, y",
  short: "MM/dd/yyyy"
}, vw = {
  full: "h:mm:ss a zzzz",
  long: "h:mm:ss a z",
  medium: "h:mm:ss a",
  short: "h:mm a"
}, Nw = {
  full: "{{date}} 'at' {{time}}",
  long: "{{date}} 'at' {{time}}",
  medium: "{{date}}, {{time}}",
  short: "{{date}}, {{time}}"
}, ww = {
  date: Po({
    formats: Rw,
    defaultWidth: "full"
  }),
  time: Po({
    formats: vw,
    defaultWidth: "full"
  }),
  dateTime: Po({
    formats: Nw,
    defaultWidth: "full"
  })
};
const Mw = ww;
var Zw = {
  lastWeek: "'last' eeee 'at' p",
  yesterday: "'yesterday at' p",
  today: "'today at' p",
  tomorrow: "'tomorrow at' p",
  nextWeek: "eeee 'at' p",
  other: "P"
}, Jw = function(n, i, a, l) {
  return Zw[n];
};
const gw = Jw;
function vr(n) {
  return function(i, a) {
    var l = a != null && a.context ? String(a.context) : "standalone", c;
    if (l === "formatting" && n.formattingValues) {
      var f = n.defaultFormattingWidth || n.defaultWidth, p = a != null && a.width ? String(a.width) : f;
      c = n.formattingValues[p] || n.formattingValues[f];
    } else {
      var V = n.defaultWidth, v = a != null && a.width ? String(a.width) : n.defaultWidth;
      c = n.values[v] || n.values[V];
    }
    var k = n.argumentCallback ? n.argumentCallback(i) : i;
    return c[k];
  };
}
var kw = {
  narrow: ["B", "A"],
  abbreviated: ["BC", "AD"],
  wide: ["Before Christ", "Anno Domini"]
}, yw = {
  narrow: ["1", "2", "3", "4"],
  abbreviated: ["Q1", "Q2", "Q3", "Q4"],
  wide: ["1st quarter", "2nd quarter", "3rd quarter", "4th quarter"]
}, bw = {
  narrow: ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"],
  abbreviated: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  wide: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
}, Ww = {
  narrow: ["S", "M", "T", "W", "T", "F", "S"],
  short: ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"],
  abbreviated: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
  wide: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
}, Ew = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  }
}, Fw = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  }
}, Sw = function(n, i) {
  var a = Number(n), l = a % 100;
  if (l > 20 || l < 10)
    switch (l % 10) {
      case 1:
        return a + "st";
      case 2:
        return a + "nd";
      case 3:
        return a + "rd";
    }
  return a + "th";
}, Aw = {
  ordinalNumber: Sw,
  era: vr({
    values: kw,
    defaultWidth: "wide"
  }),
  quarter: vr({
    values: yw,
    defaultWidth: "wide",
    argumentCallback: function(n) {
      return n - 1;
    }
  }),
  month: vr({
    values: bw,
    defaultWidth: "wide"
  }),
  day: vr({
    values: Ww,
    defaultWidth: "wide"
  }),
  dayPeriod: vr({
    values: Ew,
    defaultWidth: "wide",
    formattingValues: Fw,
    defaultFormattingWidth: "wide"
  })
};
const xw = Aw;
function Nr(n) {
  return function(i) {
    var a = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = a.width, c = l && n.matchPatterns[l] || n.matchPatterns[n.defaultMatchWidth], f = i.match(c);
    if (!f)
      return null;
    var p = f[0], V = l && n.parsePatterns[l] || n.parsePatterns[n.defaultParseWidth], v = Array.isArray(V) ? Bw(V, function(y) {
      return y.test(p);
    }) : Qw(V, function(y) {
      return y.test(p);
    }), k;
    k = n.valueCallback ? n.valueCallback(v) : v, k = a.valueCallback ? a.valueCallback(k) : k;
    var J = i.slice(p.length);
    return {
      value: k,
      rest: J
    };
  };
}
function Qw(n, i) {
  for (var a in n)
    if (n.hasOwnProperty(a) && i(n[a]))
      return a;
}
function Bw(n, i) {
  for (var a = 0; a < n.length; a++)
    if (i(n[a]))
      return a;
}
function Dw(n) {
  return function(i) {
    var a = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = i.match(n.matchPattern);
    if (!l)
      return null;
    var c = l[0], f = i.match(n.parsePattern);
    if (!f)
      return null;
    var p = n.valueCallback ? n.valueCallback(f[0]) : f[0];
    p = a.valueCallback ? a.valueCallback(p) : p;
    var V = i.slice(c.length);
    return {
      value: p,
      rest: V
    };
  };
}
var Ow = /^(\d+)(th|st|nd|rd)?/i, Gw = /\d+/i, Cw = {
  narrow: /^(b|a)/i,
  abbreviated: /^(b\.?\s?c\.?|b\.?\s?c\.?\s?e\.?|a\.?\s?d\.?|c\.?\s?e\.?)/i,
  wide: /^(before christ|before common era|anno domini|common era)/i
}, zw = {
  any: [/^b/i, /^(a|c)/i]
}, Yw = {
  narrow: /^[1234]/i,
  abbreviated: /^q[1234]/i,
  wide: /^[1234](th|st|nd|rd)? quarter/i
}, Iw = {
  any: [/1/i, /2/i, /3/i, /4/i]
}, Hw = {
  narrow: /^[jfmasond]/i,
  abbreviated: /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i,
  wide: /^(january|february|march|april|may|june|july|august|september|october|november|december)/i
}, Pw = {
  narrow: [/^j/i, /^f/i, /^m/i, /^a/i, /^m/i, /^j/i, /^j/i, /^a/i, /^s/i, /^o/i, /^n/i, /^d/i],
  any: [/^ja/i, /^f/i, /^mar/i, /^ap/i, /^may/i, /^jun/i, /^jul/i, /^au/i, /^s/i, /^o/i, /^n/i, /^d/i]
}, jw = {
  narrow: /^[smtwf]/i,
  short: /^(su|mo|tu|we|th|fr|sa)/i,
  abbreviated: /^(sun|mon|tue|wed|thu|fri|sat)/i,
  wide: /^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)/i
}, Xw = {
  narrow: [/^s/i, /^m/i, /^t/i, /^w/i, /^t/i, /^f/i, /^s/i],
  any: [/^su/i, /^m/i, /^tu/i, /^w/i, /^th/i, /^f/i, /^sa/i]
}, Lw = {
  narrow: /^(a|p|mi|n|(in the|at) (morning|afternoon|evening|night))/i,
  any: /^([ap]\.?\s?m\.?|midnight|noon|(in the|at) (morning|afternoon|evening|night))/i
}, _w = {
  any: {
    am: /^a/i,
    pm: /^p/i,
    midnight: /^mi/i,
    noon: /^no/i,
    morning: /morning/i,
    afternoon: /afternoon/i,
    evening: /evening/i,
    night: /night/i
  }
}, qw = {
  ordinalNumber: Dw({
    matchPattern: Ow,
    parsePattern: Gw,
    valueCallback: function(n) {
      return parseInt(n, 10);
    }
  }),
  era: Nr({
    matchPatterns: Cw,
    defaultMatchWidth: "wide",
    parsePatterns: zw,
    defaultParseWidth: "any"
  }),
  quarter: Nr({
    matchPatterns: Yw,
    defaultMatchWidth: "wide",
    parsePatterns: Iw,
    defaultParseWidth: "any",
    valueCallback: function(n) {
      return n + 1;
    }
  }),
  month: Nr({
    matchPatterns: Hw,
    defaultMatchWidth: "wide",
    parsePatterns: Pw,
    defaultParseWidth: "any"
  }),
  day: Nr({
    matchPatterns: jw,
    defaultMatchWidth: "wide",
    parsePatterns: Xw,
    defaultParseWidth: "any"
  }),
  dayPeriod: Nr({
    matchPatterns: Lw,
    defaultMatchWidth: "any",
    parsePatterns: _w,
    defaultParseWidth: "any"
  })
};
const Kw = qw;
var $w = {
  code: "en-US",
  formatDistance: Uw,
  formatLong: Mw,
  formatRelative: gw,
  localize: xw,
  match: Kw,
  options: {
    weekStartsOn: 0,
    firstWeekContainsDate: 1
  }
};
const eM = $w;
var tM = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, nM = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, rM = /^'([^]*?)'?$/, aM = /''/g, iM = /[a-zA-Z]/;
function oM(n, i, a) {
  var l, c, f, p, V, v, k, J, y, g, z, S, O, G, B, _, ne, oe;
  We(2, arguments);
  var F = String(i), Ue = ri(), fe = (l = (c = a == null ? void 0 : a.locale) !== null && c !== void 0 ? c : Ue.locale) !== null && l !== void 0 ? l : eM, ee = Vn((f = (p = (V = (v = a == null ? void 0 : a.firstWeekContainsDate) !== null && v !== void 0 ? v : a == null || (k = a.locale) === null || k === void 0 || (J = k.options) === null || J === void 0 ? void 0 : J.firstWeekContainsDate) !== null && V !== void 0 ? V : Ue.firstWeekContainsDate) !== null && p !== void 0 ? p : (y = Ue.locale) === null || y === void 0 || (g = y.options) === null || g === void 0 ? void 0 : g.firstWeekContainsDate) !== null && f !== void 0 ? f : 1);
  if (!(ee >= 1 && ee <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var pt = Vn((z = (S = (O = (G = a == null ? void 0 : a.weekStartsOn) !== null && G !== void 0 ? G : a == null || (B = a.locale) === null || B === void 0 || (_ = B.options) === null || _ === void 0 ? void 0 : _.weekStartsOn) !== null && O !== void 0 ? O : Ue.weekStartsOn) !== null && S !== void 0 ? S : (ne = Ue.locale) === null || ne === void 0 || (oe = ne.options) === null || oe === void 0 ? void 0 : oe.weekStartsOn) !== null && z !== void 0 ? z : 0);
  if (!(pt >= 0 && pt <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  if (!fe.localize)
    throw new RangeError("locale must contain localize property");
  if (!fe.formatLong)
    throw new RangeError("locale must contain formatLong property");
  var wt = Nt(n);
  if (!_N(wt))
    throw new RangeError("Invalid time value");
  var nn = XN(wt), Je = qN(wt, nn), mt = {
    firstWeekContainsDate: ee,
    weekStartsOn: pt,
    locale: fe,
    _originalDate: wt
  }, je = F.match(nM).map(function(Re) {
    var Ee = Re[0];
    if (Ee === "p" || Ee === "P") {
      var Xe = dw[Ee];
      return Xe(Re, fe.formatLong);
    }
    return Re;
  }).join("").match(tM).map(function(Re) {
    if (Re === "''")
      return "'";
    var Ee = Re[0];
    if (Ee === "'")
      return lM(Re);
    var Xe = sw[Ee];
    if (Xe)
      return !(a != null && a.useAdditionalWeekYearTokens) && mw(Re) && Ku(Re, i, String(n)), !(a != null && a.useAdditionalDayOfYearTokens) && pw(Re) && Ku(Re, i, String(n)), Xe(Je, Re, fe.localize, mt);
    if (Ee.match(iM))
      throw new RangeError("Format string contains an unescaped latin alphabet character `" + Ee + "`");
    return Re;
  }).join("");
  return je;
}
function lM(n) {
  var i = n.match(rM);
  return i ? i[1].replace(aM, "'") : n;
}
const sM = window.Vue.defineComponent, $u = window.Vue.toDisplayString, ed = window.Vue.createElementVNode, cM = window.Vue.unref, uM = window.Vue.openBlock, dM = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const fM = { class: "box" }, hM = { class: "label" }, pM = { class: "date" }, mM = /* @__PURE__ */ sM({
  __name: "DateBox",
  props: {
    label: null,
    date: null
  },
  setup(n) {
    const i = n;
    return (a, l) => (uM(), dM("div", fM, [
      ed("div", hM, $u(i.label), 1),
      ed("div", pM, $u(cM(oM)(new Date(i.date), "d/M HH:mm:ss")), 1)
    ]));
  }
});
const td = /* @__PURE__ */ Pe(mM, [["__scopeId", "data-v-4f3d2a9d"]]), VM = window.Vue.openBlock, TM = window.Vue.createElementBlock, UM = window.Vue.createElementVNode;
var RM = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const vM = {}, NM = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, wM = /* @__PURE__ */ UM("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM14.59,8,12,10.59,9.41,8A1,1,0,0,0,8,8H8A1,1,0,0,0,8,9.41L10.59,12,8,14.59A1,1,0,0,0,8,16H8a1,1,0,0,0,1.41,0L12,13.41,14.59,16A1,1,0,0,0,16,16h0a1,1,0,0,0,0-1.41L13.41,12,16,9.41A1,1,0,0,0,16,8h0A1,1,0,0,0,14.59,8Z" }, null, -1), MM = [
  wM
];
function ZM(n, i) {
  return VM(), TM("svg", NM, MM);
}
var JM = /* @__PURE__ */ RM(vM, [["render", ZM]]);
const gM = window.Vue.computed, kM = (n, i) => {
  const a = {};
  return Object.keys(i).forEach((l) => {
    a[`${l}Label`] = gM(() => n.value[l] ? n.value[l] : i[l]);
  }), a;
}, yM = window.Vue.openBlock, bM = window.Vue.createElementBlock, WM = window.Vue.createElementVNode;
var EM = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const FM = {}, SM = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, AM = /* @__PURE__ */ WM("path", { d: "M19,17.55,13.43,12,19,6.4A1,1,0,1,0,17.58,5L12,10.58,6.42,5A1,1,0,0,0,5,6.42L10.6,12,5,17.6A1,1,0,0,0,5,19a1,1,0,0,0,.71.29,1,1,0,0,0,.71-.3L12,13.41,17.6,19a1,1,0,0,0,.71.29A1,1,0,0,0,19,19,1,1,0,0,0,19,17.55Z" }, null, -1), xM = [
  AM
];
function QM(n, i) {
  return yM(), bM("svg", SM, xM);
}
var hf = /* @__PURE__ */ EM(FM, [["render", QM]]);
const nd = window.Vue.computed, BM = (n, i, a) => {
  const l = nd(() => i.value.filter((f) => !f.disabled)), c = nd(() => n.value ? l.value.indexOf(n.value) : -1);
  return {
    selectPrevious() {
      n.value && n.value.disabled || (c.value === 0 ? a(l.value[l.value.length - 1]) : a(l.value[c.value - 1]));
    },
    selectNext() {
      n.value && n.value.disabled || (c.value === l.value.length - 1 ? a(l.value[0]) : a(l.value[c.value + 1]));
    }
  };
};
const DM = window.Vue.openBlock, OM = window.Vue.createElementBlock, bl = window.Vue.createElementVNode;
var GM = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const CM = {}, zM = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, YM = /* @__PURE__ */ bl("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" }, null, -1), IM = /* @__PURE__ */ bl("rect", {
  x: "11",
  y: "7",
  width: "2",
  height: "7",
  rx: "1"
}, null, -1), HM = /* @__PURE__ */ bl("rect", {
  x: "11",
  y: "15",
  width: "2",
  height: "2",
  rx: "0.65"
}, null, -1), PM = [
  YM,
  IM,
  HM
];
function jM(n, i) {
  return DM(), OM("svg", zM, PM);
}
var XM = /* @__PURE__ */ GM(CM, [["render", jM]]), LM = Object.defineProperty, _M = Object.defineProperties, qM = Object.getOwnPropertyDescriptors, rd = Object.getOwnPropertySymbols, KM = Object.prototype.hasOwnProperty, $M = Object.prototype.propertyIsEnumerable, ad = (n, i, a) => i in n ? LM(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, pf = (n, i) => {
  for (var a in i || (i = {}))
    KM.call(i, a) && ad(n, a, i[a]);
  if (rd)
    for (var a of rd(i))
      $M.call(i, a) && ad(n, a, i[a]);
  return n;
}, mf = (n, i) => _M(n, qM(i));
const _n = window.Vue.defineComponent, Wl = window.Vue.resolveComponent, mn = window.Vue.openBlock, kr = window.Vue.createElementBlock, eZ = window.Vue.createVNode, Vf = window.Vue.createBlock, tZ = window.Vue.withModifiers, jn = window.Vue.inject, Fr = window.Vue.computed;
window.Vue.normalizeClass;
window.Vue.createElementVNode;
const id = window.Vue.toDisplayString, nZ = window.Vue.renderSlot, jo = window.Vue.createCommentVNode, rZ = window.Vue.withDirectives, aZ = window.Vue.vShow, cl = window.Vue.ref, od = window.Vue.toRef, ld = window.Vue.nextTick, ul = window.Vue.watch;
window.Vue.createTextVNode;
window.Vue.Fragment;
window.Vue.renderList;
const Tf = window.Vue.provide, sd = window.Vue.isRef, iZ = window.Vue.onBeforeUnmount;
var ai = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const oZ = {
  title: {
    type: String,
    default: ""
  },
  icon: {
    type: Object,
    required: !0
  }
}, lZ = _n({
  props: oZ,
  components: {
    FeatherIcon: ft
  }
}), sZ = ["title"];
function cZ(n, i, a, l, c, f) {
  const p = Wl("FeatherIcon");
  return mn(), kr("a", {
    title: n.title,
    class: "action-icon hide-when-disabled",
    href: "#"
  }, [
    eZ(p, { icon: n.icon }, null, 8, ["icon"])
  ], 8, sZ);
}
var uZ = /* @__PURE__ */ ai(lZ, [["render", cZ], ["__scopeId", "data-v-4265058e"]]);
const dZ = _n({
  emits: ["clear"],
  props: {
    clear: {
      type: String,
      default: ""
    }
  },
  computed: {
    clearIcon() {
      return hf;
    }
  },
  components: {
    ActionIcon: uZ
  }
});
function fZ(n, i, a, l, c, f) {
  const p = Wl("ActionIcon");
  return mn(), Vf(p, {
    onClick: i[0] || (i[0] = tZ((V) => n.$emit("clear"), ["stop", "prevent"])),
    title: n.clear,
    icon: n.clearIcon,
    "data-ref-id": "feather-form-element-clear"
  }, null, 8, ["title", "icon"]);
}
var hZ = /* @__PURE__ */ ai(dZ, [["render", fZ]]);
const pZ = _n({
  computed: {
    errorIcon() {
      return XM;
    }
  },
  components: {
    FeatherIcon: ft
  }
});
function mZ(n, i, a, l, c, f) {
  const p = Wl("FeatherIcon");
  return mn(), Vf(p, {
    icon: n.errorIcon,
    class: "error-icon hide-when-disabled",
    "data-ref-id": "error-icon"
  }, null, 8, ["icon"]);
}
var VZ = /* @__PURE__ */ ai(pZ, [["render", mZ], ["__scopeId", "data-v-0b8faef3"]]);
const TZ = {
  for: {
    type: String,
    required: !0
  },
  focused: {
    type: Boolean,
    default: !1
  },
  showClear: {
    type: Boolean,
    default: !1
  },
  clearText: {
    type: String
  },
  raised: {
    type: Boolean,
    default: !1
  }
}, UZ = {
  clear: () => !0,
  "wrapper-click": (n) => !0
};
_n({
  emits: UZ,
  props: TZ,
  data() {
    return {
      prefixWidth: 0,
      prefixObserver: void 0
    };
  },
  setup() {
    const n = jn("wrapperOptions", {}), i = jn("validationErrorMessage", !1), a = Fr(() => n.error ? n.error : i && i.value ? i.value : !1);
    return mf(pf({}, n), { error: a });
  },
  computed: {
    computedClearText() {
      return this.clearText ? this.clearText : this.clear ? this.clear : "";
    },
    inputId() {
      return this.for;
    },
    hasPre() {
      return this.$slots.pre && this.$slots.pre().findIndex((i) => i.children && i.children.length !== 0) !== -1;
    },
    containerCls() {
      const n = [];
      return this.hideLabel && n.push("hide-label"), this.raised && n.push("raised"), this.focused && n.push("focused"), this.error && n.push("error"), this.background && n.push("background"), this.disabled && n.push("disabled"), this.inline && n.push("inline"), this.hasPre && n.push("has-prefix"), n;
    }
  },
  methods: {
    handleWrapperClick(n) {
      this.disabled || this.$emit("wrapper-click", n);
    }
  },
  mounted() {
    const n = this.$el.querySelector(".prefix");
    if (n) {
      const i = { childList: !0, subtree: !0 }, a = () => {
        const l = this.$el.querySelector(".prefix");
        this.prefixWidth = l ? l.offsetWidth : 0;
      };
      this.prefixObserver = new MutationObserver(a), this.prefixObserver.observe(n, i), a();
    }
  },
  unmounted() {
    this.prefixObserver && this.prefixObserver.disconnect();
  },
  components: {
    ClearIcon: hZ,
    ErrorIcon: VZ
  }
});
const RZ = _n({
  setup() {
    const n = jn("subTextOptions", {}), i = jn("validationErrorMessage", !1), a = Fr(() => n.error ? n.error : i && i.value ? i.value : "");
    return mf(pf({}, n), { error: a });
  }
}), vZ = { class: "feather-input-sub-text" }, NZ = {
  key: 0,
  class: "feather-input-spacer"
}, wZ = {
  key: 1,
  class: "feather-input-hint",
  "data-ref-id": "feather-form-element-hint"
}, MZ = {
  key: 2,
  class: "feather-input-error",
  "data-ref-id": "feather-form-element-error",
  "aria-live": "assertive"
};
function ZZ(n, i, a, l, c, f) {
  return rZ((mn(), kr("div", vZ, [
    !n.hint && !n.error.length ? (mn(), kr("div", NZ, "\xA0")) : jo("", !0),
    n.hint && !n.error.length ? (mn(), kr("div", wZ, id(n.hint), 1)) : jo("", !0),
    n.error.length > 0 ? (mn(), kr("div", MZ, id(n.error), 1)) : jo("", !0),
    nZ(n.$slots, "right", {}, void 0, !0)
  ], 512)), [
    [aZ, !n.inline || n.hint || n.error.length]
  ]);
}
var Uf = /* @__PURE__ */ ai(RZ, [["render", ZZ], ["__scopeId", "data-v-8e0ac99e"]]);
const JZ = {
  headingText: {
    type: Function,
    default: (n) => n.length && n.length === 1 ? "1 error" : `${n.length} errors`
  },
  errorList: {
    type: Array,
    default: () => []
  },
  generalError: {
    type: String,
    default: ""
  }
};
_n({
  props: JZ,
  setup(n) {
    const i = jn("featherFormErrors", cl([])), a = od(n, "errorList"), l = Fr(() => {
      var J;
      return (J = a.value) != null && J.length ? a.value : i.value;
    }), c = od(n, "generalError"), f = (J) => {
      document.getElementById(J).focus();
    }, p = (J) => J.replace(/ \*$/, ""), V = cl(), v = (J) => `${p(J.label)} - ${J.message}`, k = Fr(() => (l.value.length && ld(() => V.value.focus()), n.headingText(l.value)));
    return ul(c, (J) => {
      J.length && ld(() => V.value.focus());
    }), {
      errors: l,
      errorsHeading: k,
      heading: V,
      focusElement: f,
      mainError: c,
      getFullMessage: v
    };
  }
});
const Rf = (n, i, a, l, c) => {
  const f = jn("featherForm", !1);
  if (l && f && n.value) {
    const p = cl("");
    Tf("validationErrorMessage", p);
    const V = () => {
      if (c && sd(c) && c.value)
        return p.value = c.value, {
          success: !1,
          message: c.value,
          inputId: n.value,
          label: a
        };
      try {
        return l.validateSync(i.value), p.value = "", { success: !0 };
      } catch (J) {
        const y = J;
        return p.value = y.errors[0], {
          success: !1,
          message: y.errors[0],
          inputId: n.value,
          label: a
        };
      }
    }, k = {
      clear: () => {
        p.value = "";
      },
      validate: V
    };
    return c && sd(c) && ul(c, () => {
      f.runValidation();
    }), ul(n, (J, y) => {
      J && f && f.register(J, k), y && f && f.deregister(y);
    }, { immediate: !0 }), iZ(() => {
      f.deregister(n.value, !0);
    }), { validate: V };
  }
  return { validate: () => !0 };
}, vf = (n) => ({
  inherittedAttrs: Fr(() => ({
    class: n.class,
    "data-ref-id": n["data-ref-id"]
  }))
}), Nf = {
  hint: {
    type: String
  },
  error: {
    type: String
  },
  inline: {
    type: Boolean,
    default: !1
  }
}, wf = (n) => {
  Tf("subTextOptions", n);
}, Sa = window.Vue.ref, gZ = window.Vue.watch, kZ = window.Vue.watchEffect, cd = window.Vue.computed, Xo = window.Vue.provide, Mf = (n, i, a, l, c) => {
  const f = Sa([]), p = Sa(), V = Sa(), v = Sa();
  kZ(() => {
    if (!f.value.length)
      return;
    const B = f.value.map((_) => _.value);
    if (n.value !== void 0 && n.value !== null && (p.value = f.value[B.indexOf(n.value)]), !p.value && f.value.length) {
      let _ = f.value.filter((ne) => !ne.disabled);
      _ = _.length ? _ : f.value, V.value = _[0], V.value.first = !0;
    }
  }), gZ(p, (B, _) => {
    _ && (_.checked = !1), B && (B.checked = !0);
  });
  const k = (B) => {
    B && B.disabled || (V.value && (V.value.first = !1), p.value !== B && (i("update:modelValue", B.value), p.value = B, B.focus()));
  }, J = cd(() => p.value || V.value), y = BM(J, f, k), g = cd(() => Ie("feather-radio-group"));
  v.value = g.value;
  const { validate: z } = Rf(v, n, a, l, c);
  return Xo("register", (B) => {
    f.value = [...f.value, B], v.value === g.value && (v.value = B.id);
  }), Xo("select", k), Xo("blur", (B) => {
    i("blur", B);
  }), {
    keydown: (B) => {
      switch (B.keyCode) {
        case 13:
        case 32:
          p.value ? k(p.value) : V.value && k(V.value);
          break;
        case 40:
        case 39:
          y.selectNext();
          break;
        case 37:
        case 38:
          y.selectPrevious();
          break;
      }
    },
    ...y,
    focus: () => {
      p.value && p.value.focus();
    },
    validate: z,
    firstElementId: v,
    groupId: g
  };
};
var yZ = Object.defineProperty, bZ = Object.defineProperties, WZ = Object.getOwnPropertyDescriptors, ud = Object.getOwnPropertySymbols, EZ = Object.prototype.hasOwnProperty, FZ = Object.prototype.propertyIsEnumerable, dd = (n, i, a) => i in n ? yZ(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Yn = (n, i) => {
  for (var a in i || (i = {}))
    EZ.call(i, a) && dd(n, a, i[a]);
  if (ud)
    for (var a of ud(i))
      FZ.call(i, a) && dd(n, a, i[a]);
  return n;
}, Zf = (n, i) => bZ(n, WZ(i));
const en = window.Vue.defineComponent, br = window.Vue.inject, La = window.Vue.computed, Wr = window.Vue.ref, at = window.Vue.resolveComponent, He = window.Vue.openBlock, xr = window.Vue.createElementBlock, Jf = window.Vue.normalizeClass, vt = window.Vue.renderSlot, qt = window.Vue.createBlock, Sr = window.Vue.createCommentVNode, _a = window.Vue.createElementVNode, SZ = window.Vue.withModifiers, ii = window.Vue.createVNode, gf = window.Vue.toRef, dl = window.Vue.mergeProps, Qt = window.Vue.withCtx, AZ = window.Vue.h, xZ = window.Vue.provide;
var tn = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const QZ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  clickable: {
    type: Boolean,
    default: !1
  }
}, BZ = en({
  props: QZ,
  setup(n) {
    const i = br("isCondensed", null), a = La(() => i || n.condensed), l = Wr(!1);
    return {
      focused: l,
      handleFocus: () => {
        l.value = !0;
      },
      handleBlur: () => {
        l.value = !1;
      },
      isCondensed: a
    };
  },
  components: {
    FeatherRipple: Pn
  }
}), DZ = ["aria-disabled"];
function OZ(n, i, a, l, c, f) {
  const p = at("FeatherRipple");
  return He(), xr("div", {
    class: Jf(["chip", {
      condensed: n.isCondensed,
      disabled: n.disabled,
      focused: n.focused
    }]),
    onFocusin: i[0] || (i[0] = (V) => n.clickable ? n.handleFocus : null),
    onFocusout: i[1] || (i[1] = (V) => n.clickable ? n.handleBlur : null),
    "aria-disabled": n.disabled
  }, [
    vt(n.$slots, "default", {}, void 0, !0),
    n.clickable ? (He(), qt(p, { key: 0 })) : Sr("", !0)
  ], 42, DZ);
}
var El = /* @__PURE__ */ tn(BZ, [["render", OZ], ["__scopeId", "data-v-44d413dc"]]);
const GZ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  textId: {
    type: String,
    required: !0
  },
  label: {
    type: String,
    required: !0
  }
}, CZ = en({
  emits: ["delete"],
  props: GZ,
  setup(n, i) {
    return {
      handleDelete: () => {
        n.disabled || i.emit("delete");
      },
      icon: hf
    };
  },
  components: {
    FeatherIcon: ft
  }
}), zZ = { class: "chip-delete" }, YZ = ["aria-label", "aria-describedby"];
function IZ(n, i, a, l, c, f) {
  const p = at("FeatherIcon");
  return He(), xr("span", zZ, [
    _a("a", {
      href: "#",
      class: "delete-icon",
      role: "button",
      onClick: i[0] || (i[0] = SZ((...V) => n.handleDelete && n.handleDelete(...V), ["stop", "prevent"])),
      "aria-label": n.label,
      "aria-describedby": n.textId
    }, [
      ii(p, {
        icon: n.icon,
        flex: ""
      }, null, 8, ["icon"])
    ], 8, YZ)
  ]);
}
var HZ = /* @__PURE__ */ tn(CZ, [["render", IZ], ["__scopeId", "data-v-4bae6cb4"]]);
const PZ = en({
  data() {
    return {
      titleText: void 0
    };
  },
  methods: {
    ellipsisActive() {
      return this.$refs.container.offsetWidth < this.$refs.container.scrollWidth;
    },
    updateTitle() {
      this.titleText = this.ellipsisActive() ? this.$refs.container.textContent : void 0;
    }
  },
  mounted() {
    this.updateTitle();
  },
  beforeUpdate() {
    this.updateTitle();
  }
}), jZ = ["title"];
function XZ(n, i, a, l, c, f) {
  return He(), xr("span", {
    class: "label",
    title: n.titleText,
    ref: "container"
  }, [
    vt(n.$slots, "default", {}, void 0, !0)
  ], 8, jZ);
}
var Fl = /* @__PURE__ */ tn(PZ, [["render", XZ], ["__scopeId", "data-v-1a0445b2"]]);
const LZ = {}, _Z = {
  class: "chip-icon",
  role: "presentation"
};
function qZ(n, i) {
  return He(), xr("span", _Z, [
    vt(n.$slots, "default", {}, void 0, !0)
  ]);
}
var Sl = /* @__PURE__ */ tn(LZ, [["render", qZ], ["__scopeId", "data-v-2230176f"]]);
const fd = {
  delete: "Remove"
}, KZ = en({
  inheritAttrs: !1,
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    labels: {
      type: Object,
      default: () => fd
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n, i) {
    const a = kM(gf(n, "labels"), fd), l = La(() => Ie("chip-text")), c = () => {
      n.disabled || i.emit("click");
    }, f = Yn({}, i.attrs);
    return n.disabled && delete f.onClick, Zf(Yn({}, a), {
      chipTextId: l,
      handleClick: c,
      attrs: f
    });
  },
  computed: {
    chipTextAttrs() {
      return this.canClick ? {
        role: "button",
        tabindex: "0"
      } : {};
    },
    canDelete() {
      return !!this.$attrs.onDelete && !this.disabled;
    },
    canClick() {
      return !!this.$attrs.onClick && !this.disabled;
    },
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    DeleteIcon: HZ,
    Label: Fl,
    PreIcon: Sl
  }
}), $Z = ["aria-disabled"];
function eJ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("DeleteIcon"), k = at("Chip");
  return He(), qt(k, dl(n.attrs, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: { hover: n.canClick, focus: n.canClick || n.canDelete },
    role: "row",
    clickable: n.canClick
  }), {
    default: Qt(() => [
      _a("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        _a("span", dl(n.chipTextAttrs, { class: "chip-label-button" }), [
          n.hasIcon ? (He(), qt(p, { key: 0 }, {
            default: Qt(() => [
              vt(n.$slots, "icon", {}, void 0, !0)
            ]),
            _: 3
          })) : Sr("", !0),
          ii(V, { id: n.chipTextId }, {
            default: Qt(() => [
              vt(n.$slots, "default", {}, void 0, !0)
            ]),
            _: 3
          }, 8, ["id"])
        ], 16)
      ], 8, $Z),
      n.canDelete ? (He(), qt(v, {
        key: 0,
        disabled: n.disabled,
        "text-id": n.chipTextId,
        label: n.deleteLabel,
        role: "gridcell",
        onDelete: i[0] || (i[0] = (J) => n.$emit("delete"))
      }, null, 8, ["disabled", "text-id", "label"])) : Sr("", !0)
    ]),
    _: 3
  }, 16, ["disabled", "condensed", "class", "clickable"]);
}
var tJ = /* @__PURE__ */ tn(KZ, [["render", eJ], ["__scopeId", "data-v-48b2704a"]]);
const nJ = en({
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    Label: Fl,
    PreIcon: Sl
  }
}), rJ = ["aria-disabled"];
function aJ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("Chip");
  return He(), qt(v, {
    role: "row",
    disabled: n.disabled,
    condensed: n.condensed,
    clickable: !1
  }, {
    default: Qt(() => [
      _a("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        n.hasIcon ? (He(), qt(p, { key: 0 }, {
          default: Qt(() => [
            vt(n.$slots, "icon", {}, void 0, !0)
          ]),
          _: 3
        })) : Sr("", !0),
        ii(V, null, {
          default: Qt(() => [
            vt(n.$slots, "default", {}, void 0, !0)
          ]),
          _: 3
        })
      ], 8, rJ)
    ]),
    _: 3
  }, 8, ["disabled", "condensed"]);
}
var iJ = /* @__PURE__ */ tn(nJ, [["render", aJ], ["__scopeId", "data-v-3e0c4eba"]]);
const oJ = en({
  props: {
    value: {
      type: [String, Object, Number, Array],
      required: !0
    },
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n) {
    const i = Wr(!1), a = Wr(!1), l = La(() => Ie("chip-label-id")), c = La(() => i.value || a.value ? 0 : -1), f = Wr(), p = () => {
      f.value.$el.focus();
    }, V = br("register", (g) => {
    }), v = br("blur", (g) => {
    }), k = br("select", (g) => {
    }), J = {
      first: i,
      focus: p,
      disabled: n.disabled,
      value: n.value,
      checked: a
    };
    return V(J), {
      labelId: l,
      tabindex: c,
      first: i,
      blur: v,
      click: () => {
        k(J);
      },
      input: f,
      checked: a
    };
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    Label: Fl,
    PreIcon: Sl
  }
});
function lJ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("Chip");
  return He(), qt(v, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: Jf(["focus hover", { selected: n.checked }]),
    role: "radio",
    ref: "input",
    "aria-checked": n.checked ? "true" : "false",
    "aria-disabled": n.disabled,
    "aria-labelledby": n.labelId,
    tabindex: n.tabindex,
    onClick: n.click,
    "allow-click": !n.disabled,
    onBlur: n.blur
  }, {
    default: Qt(() => [
      n.hasIcon ? (He(), qt(p, { key: 0 }, {
        default: Qt(() => [
          vt(n.$slots, "icon", {}, void 0, !0)
        ]),
        _: 3
      })) : Sr("", !0),
      ii(V, { id: n.labelId }, {
        default: Qt(() => [
          vt(n.$slots, "default", {}, void 0, !0)
        ]),
        _: 3
      }, 8, ["id"])
    ]),
    _: 3
  }, 8, ["disabled", "condensed", "class", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex", "onClick", "allow-click", "onBlur"]);
}
var sJ = /* @__PURE__ */ tn(oJ, [["render", lJ], ["__scopeId", "data-v-bbcc2f70"]]);
const cJ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  labels: {
    type: Object
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  value: {
    type: [String, Object, Number, Array, Boolean]
  }
}, hd = en({
  props: cJ,
  setup() {
    return { format: br("chipListFormat", "") };
  },
  render() {
    const n = (i) => AZ(i, Yn(Yn({}, this.$props), this.$attrs), Yn({
      default: this.$slots.default
    }, this.$slots.icon && { icon: this.$slots.icon }));
    return this.format === "grid" ? n(tJ) : this.format === "radio" ? n(sJ) : n(iJ);
  }
}), uJ = {
  label: {
    type: String,
    required: !0
  },
  mode: {
    type: String,
    default: "list",
    validator: (n) => ["list", "radio", "single"].indexOf(n) !== -1
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  condensed: {
    type: Boolean,
    default: !1
  }
}, dJ = en({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: uJ,
  setup(n, i) {
    const a = n.mode === "list" ? "grid" : n.mode;
    xZ("chipListFormat", a);
    const l = a === "single";
    if (a === "radio") {
      const p = gf(n, "modelValue");
      return Zf(Yn({
        attrs: {
          role: "radiogroup"
        }
      }, Mf(p, i.emit, n.label, {}, Wr(""))), {
        single: l
      });
    }
    return { attrs: {
      role: "grid"
    }, keydown: () => {
    }, single: l };
  }
}), fJ = ["aria-label"];
function hJ(n, i, a, l, c, f) {
  return He(), xr("div", dl(n.attrs, {
    "data-ref-id": "feather-chip-list",
    "aria-label": n.label,
    class: ["chip-list", { condensed: n.condensed, single: n.single }],
    onKeydown: i[0] || (i[0] = (...p) => n.keydown && n.keydown(...p))
  }), [
    vt(n.$slots, "default", {}, void 0, !0)
  ], 16, fJ);
}
var pJ = /* @__PURE__ */ tn(dJ, [["render", hJ], ["__scopeId", "data-v-1e06f41d"]]);
const pd = (n) => {
  const i = new Date(n);
  return `${i.getMonth() + 1}/${i.getDate()} ${i.getUTCHours()}:${i.getMinutes()}:${i.getSeconds()}`;
}, mJ = window.Vue.defineComponent, Lo = window.Vue.toDisplayString, _t = window.Vue.createElementVNode, md = window.Vue.unref, Vd = window.Vue.createTextVNode, VJ = window.Vue.createVNode, TJ = window.Vue.openBlock, UJ = window.Vue.createElementBlock, RJ = window.Vue.createCommentVNode, vJ = window.Vue.pushScopeId, NJ = window.Vue.popScopeId, kf = (n) => (vJ("data-v-c8e172b5"), n = n(), NJ(), n), wJ = {
  key: 0,
  class: "card"
}, MJ = { class: "row" }, ZJ = { class: "title" }, JJ = /* @__PURE__ */ kf(() => /* @__PURE__ */ _t("strong", null, "First Event", -1)), gJ = /* @__PURE__ */ kf(() => /* @__PURE__ */ _t("strong", null, "Last Event", -1)), kJ = ["innerHTML"], yJ = window.Vue.ref, bJ = window.Vue.watch, WJ = /* @__PURE__ */ mJ({
  __name: "AlarmDetail",
  props: {
    id: null
  },
  setup(n) {
    const i = n, a = Zl(), l = yJ(a.alarms[i.id]);
    return bJ(i, () => {
      l.value = a.alarms[i.id];
    }), (c, f) => {
      var p;
      return l.value ? (TJ(), UJ("div", wJ, [
        _t("div", MJ, [
          _t("div", ZJ, "[" + Lo(l.value.id) + "]", 1),
          _t("div", null, [
            JJ,
            Vd(" - " + Lo(md(pd)(l.value.firstEventTime)), 1)
          ]),
          _t("div", null, [
            gJ,
            Vd(" - " + Lo(md(pd)(l.value.lastEvent.createTime)), 1)
          ]),
          _t("div", null, [
            VJ(cf, {
              severity: (p = l.value) == null ? void 0 : p.severity
            }, null, 8, ["severity"])
          ])
        ]),
        _t("div", {
          innerHTML: l.value.description
        }, null, 8, kJ)
      ])) : RJ("", !0);
    };
  }
});
const EJ = /* @__PURE__ */ Pe(WJ, [["__scopeId", "data-v-c8e172b5"]]), FJ = window.Vue.defineComponent, SJ = window.Vue.normalizeClass, AJ = window.Vue.openBlock, xJ = window.Vue.createElementBlock, QJ = window.Vue.createCommentVNode, BJ = /* @__PURE__ */ FJ({
  __name: "StatusColor",
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const i = n;
    return (a, l) => i != null && i.severity ? (AJ(), xJ("span", {
      key: 0,
      class: SJ(["circle", [`${i.severity.toLowerCase()}-bg dark`]])
    }, null, 2)) : QJ("", !0);
  }
});
const DJ = /* @__PURE__ */ Pe(BJ, [["__scopeId", "data-v-e08880d6"]]), OJ = window.Vue.defineComponent, Ya = window.Vue.createElementVNode, yf = window.Vue.createTextVNode, Aa = window.Vue.unref, Td = window.Vue.normalizeClass, _o = window.Vue.withCtx, qo = window.Vue.createVNode, Ud = window.Vue.renderList, Ko = window.Vue.Fragment, Dn = window.Vue.openBlock, xa = window.Vue.createElementBlock, GJ = window.Vue.toDisplayString, Rd = window.Vue.createBlock, CJ = window.Vue.pushScopeId, zJ = window.Vue.popScopeId, YJ = (n) => (CJ("data-v-4d8a6eca"), n = n(), zJ(), n), IJ = /* @__PURE__ */ YJ(() => /* @__PURE__ */ Ya("div", { class: "title" }, "Alarms", -1)), HJ = { class: "alarm-filters-container" }, PJ = /* @__PURE__ */ yf(" ALL "), jJ = { class: "section" }, XJ = { class: "alarm-list" }, vd = window.Vue.ref, LJ = window.Vue.watch, _J = window.Vue.computed, qJ = /* @__PURE__ */ OJ({
  __name: "AlarmFilters",
  props: {
    relatedAlarms: null
  },
  setup(n) {
    const i = n, a = _J(
      () => $t.exports.keys($t.exports.groupBy(i.relatedAlarms, "severity"))
    ), l = vd(["all"]), c = vd(i.relatedAlarms), f = (p) => {
      l.value = l.value.filter((V) => V !== "all"), l.value.includes(p) ? l.value = l.value.filter((V) => V !== p) : l.value.push(p), p === "all" || l.value.length === 0 ? (l.value = ["all"], c.value = i.relatedAlarms) : c.value = i.relatedAlarms.filter(
        (V) => l.value.includes(V.severity)
      );
    };
    return LJ(i, () => {
      l.value = ["all"], c.value = i.relatedAlarms;
    }), (p, V) => (Dn(), xa(Ko, null, [
      IJ,
      Ya("div", HJ, [
        (Dn(), Rd(Aa(pJ), {
          key: l.value.toString(),
          condensed: "",
          class: "alarm-filters",
          label: "Random list for condensed visual testing"
        }, {
          default: _o(() => [
            qo(Aa(hd), {
              class: Td({ clicked: l.value.includes("all") }),
              onClick: V[0] || (V[0] = (v) => f("all"))
            }, {
              default: _o(() => [
                PJ
              ]),
              _: 1
            }, 8, ["class"]),
            (Dn(!0), xa(Ko, null, Ud(Aa(a), (v) => (Dn(), Rd(Aa(hd), {
              class: Td({ clicked: l.value.includes(v) }),
              key: v,
              onClick: (k) => f(v)
            }, {
              default: _o(() => [
                qo(DJ, { severity: v }, null, 8, ["severity"]),
                yf(GJ(v), 1)
              ]),
              _: 2
            }, 1032, ["class", "onClick"]))), 128))
          ]),
          _: 1
        })),
        Ya("div", jJ, [
          Ya("div", XJ, [
            (Dn(!0), xa(Ko, null, Ud(c.value, (v) => (Dn(), xa("div", {
              key: v.id
            }, [
              qo(EJ, {
                id: v.id
              }, null, 8, ["id"])
            ]))), 128))
          ])
        ])
      ])
    ], 64));
  }
});
const KJ = /* @__PURE__ */ Pe(qJ, [["__scopeId", "data-v-4d8a6eca"]]);
var $J = Object.defineProperty, eg = Object.defineProperties, tg = Object.getOwnPropertyDescriptors, Nd = Object.getOwnPropertySymbols, ng = Object.prototype.hasOwnProperty, rg = Object.prototype.propertyIsEnumerable, wd = (n, i, a) => i in n ? $J(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, wr = (n, i) => {
  for (var a in i || (i = {}))
    ng.call(i, a) && wd(n, a, i[a]);
  if (Nd)
    for (var a of Nd(i))
      rg.call(i, a) && wd(n, a, i[a]);
  return n;
}, Md = (n, i) => eg(n, tg(i));
const ag = window.Vue.defineComponent, ig = window.Vue.inject, Mr = window.Vue.h;
var og = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const lg = {
  primary: {
    type: Boolean,
    default: !1
  },
  text: {
    type: Boolean,
    default: !1
  },
  secondary: {
    type: Boolean,
    default: !1
  },
  icon: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  },
  asAnchor: {
    type: Boolean,
    default: !1
  },
  onColor: {
    type: Boolean,
    default: !1
  }
}, sg = ag({
  inheritAttrs: !1,
  props: lg,
  setup() {
    return { hasTooltip: ig("feather-has-tooltip", !1) };
  },
  render() {
    const n = () => {
      let V = "";
      this.primary && (V = "btn-primary"), this.secondary && (V = "btn-secondary"), (this.text || this.icon) && (V = "btn-text");
      const v = ["btn", "hover", "focus", V];
      return this.icon && (v.push("btn-icon"), v.push("btn-icon-table")), this.onColor && v.push("on-color"), v;
    }, i = this.asAnchor ? "a" : "button", a = {}, l = wr({}, this.$attrs);
    a.attrs = l || {}, this.asAnchor ? a.attrs.role = "button" : a.attrs.type = a.attrs.type || "button", this.disabled && (a.attrs["aria-disabled"] = "true"), a.on = {
      onClick: (V) => {
        this.disabled ? (this.asAnchor && V.preventDefault(), this.$emit("disabled-click", V)) : this.$emit("click", V);
      }
    };
    const c = n();
    a.class = [this.$attrs.class].concat(c), this.$slots.icon && a.class.push("has-icon");
    let f = Mr(Pn);
    if (this.disabled && (f = void 0), this.icon && this.$slots.default) {
      const V = this.icon;
      return a.attrs["aria-label"] = V, this.hasTooltip || (a.attrs.title = V), Mr(i, Md(wr(wr({}, a.attrs), a.on), { class: a.class }), [
        this.$slots.default(),
        this.disabled ? void 0 : Mr(Pn, { center: !0 })
      ]);
    }
    const p = Mr("span", { class: ["btn-content"] }, [
      this.$slots.default ? this.$slots.default() : ""
    ]);
    return Mr(i, Md(wr(wr({}, a.attrs), a.on), { class: a.class }), [
      this.$slots.icon ? this.$slots.icon() : void 0,
      p,
      f
    ]);
  }
});
var qa = /* @__PURE__ */ og(sg, [["__scopeId", "data-v-702d1074"]]);
const cg = "/whoami", ug = async () => {
  try {
    const n = await Ln.get(cg);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, dg = window.Pinia.defineStore, Qr = dg("userStore", {
  state: () => ({
    isAdmin: !1,
    userId: null,
    firstTime: !0,
    allowSave: !0
  }),
  actions: {
    async getUserRole() {
      const n = await ug();
      n && (this.isAdmin = n.roles.includes("ROLE_ADMIN"), this.userId = n.id);
    },
    async getAlecInfo() {
      const n = await ev();
      n && (this.firstTime = !1, this.allowSave = n.agreed);
    },
    async savePermission(n) {
      if (this.allowSave = n, !n) {
        const i = await af(n);
        this.allowSave = i;
      }
    }
  }
}), fg = window.Vue.defineComponent, hn = window.Vue.unref, Zr = window.Vue.normalizeClass, St = window.Vue.createVNode, Qa = window.Vue.toDisplayString, On = window.Vue.openBlock, Gn = window.Vue.createElementBlock, hg = window.Vue.createCommentVNode, Zd = window.Vue.withCtx, rt = window.Vue.createElementVNode, pg = window.Vue.createTextVNode, mg = window.Vue.Fragment, Vg = window.Vue.pushScopeId, Tg = window.Vue.popScopeId, Ug = (n) => (Vg("data-v-14e0faf2"), n = n(), Tg(), n), Rg = { class: "section" }, vg = {
  key: 0,
  class: "btn-row"
}, Ng = { key: 0 }, wg = { key: 1 }, Mg = { key: 0 }, Zg = { key: 1 }, Jg = { class: "situation-detail" }, gg = { class: "situation-info" }, kg = { class: "id" }, yg = ["innerHTML"], bg = /* @__PURE__ */ Ug(() => /* @__PURE__ */ rt("strong", null, "Reduction key:", -1)), Wg = { class: "boxes" }, Eg = { class: "parameters" }, Fg = { class: "section" }, Sg = window.Vue.ref, Ag = window.Vue.watch, xg = /* @__PURE__ */ fg({
  __name: "SituationDetailTab",
  props: {
    alarmInfo: null
  },
  setup(n) {
    const i = n, a = "ACCEPTED", l = "REJECTED", c = Zl(), f = Qr(), p = Sg(i.alarmInfo.status), V = (v) => {
      var k;
      nv((k = i.alarmInfo) == null ? void 0 : k.id, v.toLowerCase()), p.value = v, c.getSituations();
    };
    return Ag(i, () => {
      p.value = i.alarmInfo.status || "";
    }), (v, k) => {
      var J, y, g, z, S;
      return On(), Gn(mg, null, [
        rt("div", Rg, [
          hn(f).allowSave ? (On(), Gn("div", vg, [
            St(hn(qa), {
              class: Zr(["btn", { accepted: p.value == a }]),
              onClick: k[0] || (k[0] = () => V(a))
            }, {
              default: Zd(() => [
                St(hn(ft), {
                  icon: hn(lf),
                  "aria-hidden": "true",
                  class: Zr(["icon accept", { accepted: p.value == a }])
                }, null, 8, ["icon", "class"]),
                p.value == a ? (On(), Gn("span", Ng, Qa(a))) : (On(), Gn("span", wg, " ACCEPT"))
              ]),
              _: 1
            }, 8, ["class"]),
            St(hn(qa), {
              class: Zr(["btn", { rejected: p.value == l }]),
              onClick: k[1] || (k[1] = () => V(l))
            }, {
              default: Zd(() => [
                St(hn(ft), {
                  icon: hn(JM),
                  "aria-hidden": "true",
                  class: Zr(["icon reject", { rejected: p.value == l }])
                }, null, 8, ["icon", "class"]),
                p.value == l ? (On(), Gn("span", Mg, Qa(l))) : (On(), Gn("span", Zg, " REJECT"))
              ]),
              _: 1
            }, 8, ["class"])
          ])) : hg("", !0),
          rt("div", Jg, [
            rt("div", {
              class: Zr(["severity-line", [`${(y = (J = i.alarmInfo) == null ? void 0 : J.severity) == null ? void 0 : y.toLowerCase()}-bg dark`]])
            }, null, 2),
            rt("div", gg, [
              rt("div", kg, [
                rt("div", null, "Situation " + Qa((g = i.alarmInfo) == null ? void 0 : g.id), 1),
                St(cf, {
                  severity: (z = i.alarmInfo) == null ? void 0 : z.severity
                }, null, 8, ["severity"])
              ]),
              rt("span", {
                innerHTML: i.alarmInfo.description
              }, null, 8, yg),
              rt("p", null, [
                bg,
                pg(" " + Qa(i.alarmInfo.reductionKey), 1)
              ]),
              rt("div", Wg, [
                St(td, {
                  label: "First Event",
                  date: i.alarmInfo.firstEventTime
                }, null, 8, ["date"]),
                St(td, {
                  label: "Last Event",
                  date: i.alarmInfo.lastEvent.time
                }, null, 8, ["date"])
              ])
            ]),
            rt("div", Eg, [
              St(of, {
                relatedAlarms: (S = i.alarmInfo) == null ? void 0 : S.relatedAlarms,
                size: "large"
              }, null, 8, ["relatedAlarms"])
            ])
          ])
        ]),
        rt("div", Fg, [
          St(KJ, {
            "related-alarms": i.alarmInfo.relatedAlarms
          }, null, 8, ["related-alarms"])
        ])
      ], 64);
    };
  }
});
const Qg = /* @__PURE__ */ Pe(xg, [["__scopeId", "data-v-14e0faf2"]]), Bg = window.Vue.defineComponent, Dg = window.Vue.createTextVNode, $o = window.Vue.unref, Ba = window.Vue.withCtx, Da = window.Vue.createVNode, Og = window.Vue.openBlock, Gg = window.Vue.createElementBlock, Cg = window.Vue.createCommentVNode;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const zg = {
  key: 0,
  class: "detail"
}, Yg = /* @__PURE__ */ Dg("Details"), Ig = /* @__PURE__ */ Bg({
  __name: "SituationDetail",
  props: {
    alarmInfo: null
  },
  setup(n) {
    const i = n;
    return (a, l) => i.alarmInfo ? (Og(), Gg("div", zg, [
      Da($o(AN), null, {
        tabs: Ba(() => [
          Da($o(kN), null, {
            default: Ba(() => [
              Yg
            ]),
            _: 1
          })
        ]),
        default: Ba(() => [
          Da($o(DN), { class: "panel" }, {
            default: Ba(() => [
              Da(Qg, {
                "alarm-info": i.alarmInfo
              }, null, 8, ["alarm-info"])
            ]),
            _: 1
          })
        ]),
        _: 1
      })
    ])) : Cg("", !0);
  }
});
const Hg = /* @__PURE__ */ Pe(Ig, [["__scopeId", "data-v-85513d07"]]), Pg = window.Vue.defineComponent, fl = window.Vue.createElementVNode, Jd = window.Vue.unref, jg = window.Vue.renderList, Xg = window.Vue.Fragment, el = window.Vue.openBlock, tl = window.Vue.createElementBlock, gd = window.Vue.createVNode, Lg = window.Vue.pushScopeId, _g = window.Vue.popScopeId, qg = (n) => (Lg("data-v-0e46f90e"), n = n(), _g(), n), Kg = { class: "list-main" }, $g = /* @__PURE__ */ qg(() => /* @__PURE__ */ fl("h2", null, "Situation List", -1)), ek = { class: "container" }, tk = { class: "situation-list" }, nk = window.Vue.reactive, rk = /* @__PURE__ */ Pg({
  __name: "SituationList",
  setup(n) {
    const i = Zl();
    i.getSituations();
    const a = nk({
      selectedSituationIndex: 0,
      situationSelected: ""
    }), l = (c) => {
      a.situationSelected = c, a.selectedSituationIndex = i.situations.findIndex(
        (f) => f.id === c
      );
    };
    return i.$subscribe((c, f) => {
      var p;
      a.situationSelected = (p = f.situations[0]) == null ? void 0 : p.id;
    }), (c, f) => (el(), tl("div", Kg, [
      $g,
      fl("div", ek, [
        fl("div", tk, [
          (el(!0), tl(Xg, null, jg(Jd(i).situations, (p) => (el(), tl("div", {
            key: p.id
          }, [
            gd(Dv, {
              "alarm-info": p,
              onSituationSelected: l,
              selected: a.situationSelected == p.id
            }, null, 8, ["alarm-info", "selected"])
          ]))), 128))
        ]),
        gd(Hg, {
          "alarm-info": Jd(i).situations[a.selectedSituationIndex]
        }, null, 8, ["alarm-info"])
      ])
    ]));
  }
});
const ak = /* @__PURE__ */ Pe(rk, [["__scopeId", "data-v-0e46f90e"]]);
var ik = Object.defineProperty, ok = Object.defineProperties, lk = Object.getOwnPropertyDescriptors, kd = Object.getOwnPropertySymbols, sk = Object.prototype.hasOwnProperty, ck = Object.prototype.propertyIsEnumerable, yd = (n, i, a) => i in n ? ik(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Ia = (n, i) => {
  for (var a in i || (i = {}))
    sk.call(i, a) && yd(n, a, i[a]);
  if (kd)
    for (var a of kd(i))
      ck.call(i, a) && yd(n, a, i[a]);
  return n;
}, bf = (n, i) => ok(n, lk(i));
const Wf = window.Vue.defineComponent, uk = window.Vue.ref, Er = window.Vue.computed, dk = window.Vue.reactive, bd = window.Vue.watch, nl = window.Vue.inject, Ef = window.Vue.resolveComponent, hl = window.Vue.openBlock, Ff = window.Vue.createElementBlock, Kt = window.Vue.createElementVNode, fk = window.Vue.createBlock, hk = window.Vue.createCommentVNode, Sf = window.Vue.renderSlot, pk = window.Vue.pushScopeId, mk = window.Vue.popScopeId, rl = window.Vue.toRef, Wd = window.Vue.mergeProps, Vk = window.Vue.toDisplayString, Tk = window.Vue.createVNode;
var Af = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Uk = {
  value: {
    type: [String, Number, Boolean, Array, Object, Date, Function],
    required: !0
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, Rk = Wf({
  props: Uk,
  setup(n) {
    const i = uk(), a = () => {
      i.value.focus();
    }, l = Er(() => Ie("feather-radio-button")), c = dk({
      first: !1,
      focus: a,
      disabled: n.disabled,
      value: n.value,
      checked: !1,
      id: l.value
    }), f = Er(() => Ie("radio-label-id")), p = Er(() => c.first || c.checked ? 0 : -1);
    bd(() => n.disabled, (y) => {
      c.disabled = y;
    }, { immediate: !0 }), bd(() => n.value, (y) => {
      c.value = y;
    }, { immediate: !0 });
    const V = nl("register", (y) => {
    }), v = nl("blur", (y) => {
    }), k = nl("select", (y) => {
    });
    return V(c), {
      labelId: f,
      tabindex: p,
      vm: c,
      blur: v,
      click: () => {
        k(c);
      },
      input: i,
      id: l
    };
  },
  components: {
    FeatherRipple: Pn
  }
}), vk = (n) => (pk("data-v-24790cf0"), n = n(), mk(), n), Nk = { class: "layout-container" }, wk = ["id", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex"], Mk = { class: "radio hover focus" }, Zk = /* @__PURE__ */ vk(() => /* @__PURE__ */ Kt("div", {
  class: "icon",
  "aria-hidden": "true"
}, [
  /* @__PURE__ */ Kt("svg", { class: "dot" }, [
    /* @__PURE__ */ Kt("circle", {
      cx: "50%",
      cy: "50%",
      r: "0.3125rem",
      stroke: "black",
      "stroke-width": "0"
    })
  ])
], -1)), Jk = ["id"];
function gk(n, i, a, l, c, f) {
  const p = Ef("feather-ripple");
  return hl(), Ff("div", Nk, [
    Kt("div", {
      class: "feather-radio",
      role: "radio",
      ref: "input",
      id: n.id,
      "aria-checked": n.vm.checked ? "true" : "false",
      "aria-disabled": n.vm.disabled ? "true" : "false",
      "aria-labelledby": n.labelId,
      tabindex: n.tabindex,
      onClick: i[0] || (i[0] = (...V) => n.click && n.click(...V)),
      onBlur: i[1] || (i[1] = (...V) => n.blur && n.blur(...V)),
      "data-ref-id": "feather-radio"
    }, [
      Kt("div", Mk, [
        Zk,
        n.vm.disabled ? hk("", !0) : (hl(), fk(p, {
          key: 0,
          center: ""
        }))
      ]),
      Kt("span", {
        class: "label",
        "data-ref-id": "feather-radio-label",
        id: n.labelId
      }, [
        Sf(n.$slots, "default", {}, void 0, !0)
      ], 8, Jk)
    ], 40, wk)
  ]);
}
var Ka = /* @__PURE__ */ Af(Rk, [["render", gk], ["__scopeId", "data-v-24790cf0"]]);
const kk = bf(Ia({}, Nf), {
  label: {
    type: String,
    required: !0
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
}), yk = {
  "update:modelValue": (n) => !0,
  blur: (n) => !0
}, bk = Wf({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: kk,
  emits: yk,
  setup(n, i) {
    wf(n);
    const a = rl(n, "error"), l = rl(n, "modelValue"), c = Er(() => Ie("feather-input-description")), f = Er(() => {
      const p = bf(Ia({}, i.attrs), {
        class: "",
        "aria-describedby": c.value
      });
      return p["aria-invalid"] || (p["aria-invalid"] = !!a.value), p;
    });
    return Ia(Ia({
      descriptionId: c,
      attrs: f
    }, Mf(l, i.emit, n.label, n.schema, rl(n, "error"))), vf(i.attrs));
  },
  components: {
    InputSubText: Uf
  }
}), Wk = ["for"], Ek = ["id"];
function Fk(n, i, a, l, c, f) {
  const p = Ef("InputSubText");
  return hl(), Ff("div", Wd(n.inherittedAttrs, {
    class: ["feather-radio-group-container", { vertical: n.vertical }]
  }), [
    Kt("label", {
      for: n.groupId,
      class: "group-label"
    }, Vk(n.label), 9, Wk),
    Kt("div", Wd(n.attrs, {
      class: "feather-radio-group",
      "data-ref-id": "feather-radio-group",
      role: "radiogroup",
      id: n.groupId,
      onKeydown: i[0] || (i[0] = (...V) => n.keydown && n.keydown(...V))
    }), [
      Sf(n.$slots, "default", {}, void 0, !0)
    ], 16, Ek),
    Tk(p, { id: n.descriptionId }, null, 8, ["id"])
  ], 16);
}
var xf = /* @__PURE__ */ Af(bk, [["render", Fk], ["__scopeId", "data-v-6775aeb9"]]);
const Sk = window.Vue.openBlock, Ak = window.Vue.createElementBlock, xk = window.Vue.createStaticVNode;
var Qk = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Bk = {}, Dk = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Ok = /* @__PURE__ */ xk('<path d="M4,13H14V11H4a1,1,0,0,0,0,2Z"></path><path d="M21,6a1,1,0,0,0-1-1H14V7h6A1,1,0,0,0,21,6Z"></path><path d="M20,17H11v2h9a1,1,0,0,0,0-2Z"></path><path d="M11.5,3h-1a.5.5,0,0,0-.5.5V5H4A1,1,0,0,0,4,7h6V8.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,11.5,3Z"></path><path d="M20,11H18V9.5a.5.5,0,0,0-.5-.5h-1a.5.5,0,0,0-.5.5v5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5V13h2a1,1,0,0,0,0-2Z"></path><path d="M8.5,15h-1a.5.5,0,0,0-.5.5V17H4a1,1,0,0,0,0,2H7v1.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,8.5,15Z"></path>', 6), Gk = [
  Ok
];
function Ck(n, i) {
  return Sk(), Ak("svg", Dk, Gk);
}
var zk = /* @__PURE__ */ Qk(Bk, [["render", Ck]]);
const Yk = window.Vue.openBlock, Ik = window.Vue.createElementBlock, Hk = window.Vue.createStaticVNode;
var Pk = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const jk = {}, Xk = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Lk = /* @__PURE__ */ Hk('<rect x="8" y="7" width="8" height="2" rx="1"></rect><rect x="8" y="11" width="8" height="2" rx="1"></rect><rect x="8" y="15" width="8" height="2" rx="1"></rect><path d="M21,21l-3-3v2H6V9L4,7V20a2,2,0,0,0,2,2H18v2Z"></path><path d="M6,4H18V15l2,2V4a2,2,0,0,0-2-2H6V0L3,3,6,6Z"></path>', 5), _k = [
  Lk
];
function qk(n, i) {
  return Yk(), Ik("svg", Xk, _k);
}
var Kk = /* @__PURE__ */ Pk(jk, [["render", qk]]);
const $k = window.Vue.openBlock, ey = window.Vue.createElementBlock, ty = window.Vue.createElementVNode;
var ny = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const ry = {}, ay = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, iy = /* @__PURE__ */ ty("path", { d: "M17,10h5a1,1,0,0,0,1-1V3a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1V5.59L10.59,11H8V9A1,1,0,0,0,7,8H2A1,1,0,0,0,1,9v6a1,1,0,0,0,1,1H7a1,1,0,0,0,1-1V13h2.58L16,18.49V21a1,1,0,0,0,1,1h5a1,1,0,0,0,1-1V15a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1v.64L12.41,12,16,8.42V9A1,1,0,0,0,17,10Zm1,6h3v4H18ZM18,4h3V8H18Z" }, null, -1), oy = [
  iy
];
function ly(n, i) {
  return $k(), ey("svg", ay, oy);
}
var sy = /* @__PURE__ */ ny(ry, [["render", ly]]);
const cy = window.Vue.openBlock, uy = window.Vue.createElementBlock, dy = window.Vue.createStaticVNode;
var fy = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const hy = {}, py = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, my = /* @__PURE__ */ dy('<rect x="3" y="3" width="4" height="4"></rect><rect x="10" y="3" width="4" height="4"></rect><rect x="17" y="3" width="4" height="4"></rect><rect x="3" y="10" width="4" height="4"></rect><rect x="10" y="10" width="4" height="4"></rect><rect x="17" y="10" width="4" height="4"></rect><rect x="3" y="17" width="4" height="4"></rect><rect x="10" y="17" width="4" height="4"></rect><rect x="17" y="17" width="4" height="4"></rect>', 9), Vy = [
  my
];
function Ty(n, i) {
  return cy(), uy("svg", py, Vy);
}
var Uy = /* @__PURE__ */ fy(hy, [["render", Ty]]);
const Ry = window.Vue.openBlock, vy = window.Vue.createElementBlock, Ny = window.Vue.createElementVNode;
var wy = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const My = {}, Zy = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Jy = /* @__PURE__ */ Ny("path", { d: "M18,6a4,4,0,0,0-1,7.86V18a2,2,0,0,1-2,2H10a2,2,0,0,1-2-2V12.9a5.5,5.5,0,0,0,4.5-5.4V5a2,2,0,0,0-2-2h-1a1,1,0,0,0,0,2h1V7.5a3.5,3.5,0,0,1-7,0V5h1a1,1,0,0,0,0-2h-1a2,2,0,0,0-2,2V7.5A5.5,5.5,0,0,0,6,12.9V18a4,4,0,0,0,4,4h5a4,4,0,0,0,4-4V13.86A4,4,0,0,0,18,6Zm0,6a2,2,0,1,1,2-2A2,2,0,0,1,18,12Z" }, null, -1), gy = [
  Jy
];
function ky(n, i) {
  return Ry(), vy("svg", Zy, gy);
}
var yy = /* @__PURE__ */ wy(My, [["render", ky]]);
const Qf = () => window.VRouter || Al, by = window.Vue.defineComponent, de = window.Vue.createElementVNode, Ar = window.Vue.createTextVNode, be = window.Vue.unref, At = window.Vue.createVNode, Oa = window.Vue.withCtx, Ed = window.Vue.openBlock, Fd = window.Vue.createElementBlock, Wy = window.Vue.createCommentVNode, Ey = window.Vue.pushScopeId, Fy = window.Vue.popScopeId, ht = (n) => (Ey("data-v-0fd6b0cd"), n = n(), Fy(), n), Sy = { class: "container" }, Ay = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("div", { class: "title" }, "ALEC", -1)), xy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("div", { class: "description" }, [
  /* @__PURE__ */ Ar(" The Architecture for Learning Enabled Correlation, or ALEC, provides a machine learning powered solution for alarm correlation. Complex networks produce significant, and potentially overwhelming, amount of events and alarms. "),
  /* @__PURE__ */ de("br"),
  /* @__PURE__ */ de("strong", null, "Our goal"),
  /* @__PURE__ */ Ar(" with ALEC to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. ")
], -1)), Qy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("h3", null, "Key Benefits:", -1)), By = { class: "list" }, Dy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("span", null, " Alleviate alarm load by clustering them into actionable situations. ", -1)), Oy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("span", null, "Assist in root cause analysis.", -1)), Gy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("span", null, " Avoid potential issues flagged by alarms with low visibility. ", -1)), Cy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("span", null, "Easy configuration.", -1)), zy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("span", null, " Continuous improvement. Provided anonymized data helps us improve ALEC. ", -1)), Yy = {
  key: 0,
  class: "optin"
}, Iy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("h3", null, "Enable ALEC for more precise results:", -1)), Hy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("div", null, " Our goal with ALEC, is to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. The information provided is anonymized and will be used for internal purposes only. ", -1)), Py = { class: "choices" }, jy = /* @__PURE__ */ ht(() => /* @__PURE__ */ de("div", { class: "radio-text" }, " By choosing \u201CYes\u201D you accept that OpenNMS can store the information that you provide, for educational and research purposes. ", -1)), Xy = /* @__PURE__ */ Ar("Yes"), Ly = /* @__PURE__ */ Ar("No"), _y = /* @__PURE__ */ Ar(" Continue "), qy = window.Vue.ref, Ky = /* @__PURE__ */ by({
  __name: "WelcomePage",
  setup(n) {
    const i = Qr(), a = Qf(), l = qy(!0), c = () => {
      const f = Boolean(l.value);
      i.savePermission(f), f ? a.push({ name: "configuration" }) : a.push({ name: "situations" });
    };
    return (f, p) => (Ed(), Fd("div", Sy, [
      Ay,
      xy,
      de("div", null, [
        Qy,
        de("div", By, [
          de("div", null, [
            At(be(ft), {
              icon: be(Uy),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Dy
          ]),
          de("div", null, [
            At(be(ft), {
              icon: be(sy),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Oy
          ]),
          de("div", null, [
            At(be(ft), {
              icon: be(yy),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Gy
          ]),
          de("div", null, [
            At(be(ft), {
              icon: be(zk),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Cy
          ]),
          de("div", null, [
            At(be(ft), {
              icon: be(Kk),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            zy
          ])
        ])
      ]),
      be(i).isAdmin ? (Ed(), Fd("div", Yy, [
        Iy,
        Hy,
        de("div", Py, [
          jy,
          At(be(xf), {
            horizontal: "",
            modelValue: l.value,
            "onUpdate:modelValue": p[0] || (p[0] = (V) => l.value = V),
            label: ""
          }, {
            default: Oa(() => [
              At(be(Ka), { value: !0 }, {
                default: Oa(() => [
                  Xy
                ]),
                _: 1
              }),
              At(be(Ka), { value: !1 }, {
                default: Oa(() => [
                  Ly
                ]),
                _: 1
              })
            ]),
            _: 1
          }, 8, ["modelValue"])
        ])
      ])) : Wy("", !0),
      At(be(qa), {
        "data-test": "continue-btn",
        primary: "",
        class: "continue-btn",
        onClick: c
      }, {
        default: Oa(() => [
          _y
        ]),
        _: 1
      })
    ]));
  }
});
const $y = /* @__PURE__ */ Pe(Ky, [["__scopeId", "data-v-0fd6b0cd"]]), e2 = "data:image/png;base64,", t2 = "data:image/png;base64,";
var n2 = Object.defineProperty, r2 = Object.defineProperties, a2 = Object.getOwnPropertyDescriptors, Sd = Object.getOwnPropertySymbols, i2 = Object.prototype.hasOwnProperty, o2 = Object.prototype.propertyIsEnumerable, Ad = (n, i, a) => i in n ? n2(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, pl = (n, i) => {
  for (var a in i || (i = {}))
    i2.call(i, a) && Ad(n, a, i[a]);
  if (Sd)
    for (var a of Sd(i))
      o2.call(i, a) && Ad(n, a, i[a]);
  return n;
}, Bf = (n, i) => r2(n, a2(i));
const Df = window.Vue.defineComponent, l2 = window.Vue.inject, s2 = window.Vue.resolveComponent, al = window.Vue.openBlock, xd = window.Vue.createElementBlock, zn = window.Vue.createElementVNode, c2 = window.Vue.createBlock, Qd = window.Vue.createCommentVNode, u2 = window.Vue.renderSlot, d2 = window.Vue.pushScopeId, f2 = window.Vue.popScopeId, il = window.Vue.toRef, Ga = window.Vue.computed, h2 = window.Vue.ref;
window.Vue.mergeProps;
window.Vue.toDisplayString;
window.Vue.createVNode;
var p2 = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const m2 = {
  disabled: {
    type: Boolean,
    default: !1
  },
  modelValue: {
    type: Boolean,
    default: !1
  },
  indeterminate: {
    type: Boolean,
    default: !1
  },
  label: {
    type: String
  },
  noFocus: {
    type: Boolean,
    default: !1
  }
}, V2 = {
  "update:modelValue": (n) => !0,
  click: (n) => !0,
  indeterminate: (n) => !0
}, T2 = Df({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  emits: V2,
  props: m2,
  mounted() {
    this.registerCheckbox && this.registerCheckbox(this.inputId);
  },
  setup() {
    return {
      registerCheckbox: l2("registerCheckbox", (i) => {
      })
    };
  },
  computed: {
    inputId() {
      return this.label && this.label.length ? void 0 : Ie("checkbox");
    },
    labelId() {
      return this.label && this.label.length ? void 0 : Ie("checkbox-label");
    }
  },
  watch: {
    indeterminate: {
      handler(n) {
        this.$emit("update:modelValue", void 0), this.$emit("indeterminate", n);
      }
    }
  },
  methods: {
    focus() {
      this.$refs.input.focus();
    },
    updateValue() {
      this.disabled || (this.indeterminate ? (this.$emit("update:modelValue", !0), this.$emit("indeterminate", !1)) : (this.$emit("update:modelValue", !this.modelValue), this.$emit("indeterminate", !1)));
    },
    click(n) {
      this.focus(), this.updateValue(), this.$emit("click", n);
    },
    keydown(n) {
      (n.keyCode === xt.SPACE || n.keyCode === xt.ENTER) && this.updateValue(), n.keyCode === xt.SPACE && n.preventDefault();
    }
  },
  components: {
    FeatherRipple: Pn
  }
}), U2 = (n) => (d2("data-v-a7af27e2"), n = n(), f2(), n), R2 = { class: "layout-container" }, v2 = ["aria-checked", "aria-disabled", "aria-labelledby", "aria-label", "id", "tabindex"], N2 = { class: "checkbox hover focus" }, w2 = /* @__PURE__ */ U2(() => /* @__PURE__ */ zn("div", { class: "box" }, [
  /* @__PURE__ */ zn("svg", {
    class: "checkmark",
    role: "presentation",
    focusable: "false",
    viewBox: "0 0 24 24"
  }, [
    /* @__PURE__ */ zn("path", {
      class: "path",
      fill: "none",
      d: "M1.73,12.91 8.1,19.28 22.79,4.59"
    })
  ]),
  /* @__PURE__ */ zn("div", { class: "indeterminate" })
], -1)), M2 = ["id", "for"];
function Z2(n, i, a, l, c, f) {
  const p = s2("feather-ripple");
  return al(), xd("div", R2, [
    zn("div", {
      class: "feather-checkbox feather-form-input feather-checkbox-table",
      onClick: i[0] || (i[0] = (...V) => n.click && n.click(...V)),
      onKeydown: i[1] || (i[1] = (...V) => n.keydown && n.keydown(...V)),
      "aria-checked": n.modelValue ? "true" : n.indeterminate ? "mixed" : "false",
      "aria-disabled": n.disabled,
      "aria-labelledby": n.labelId,
      "aria-label": n.label,
      id: n.inputId,
      tabindex: n.noFocus ? -1 : 0,
      ref: "input",
      role: "checkbox",
      "data-ref-id": "feather-checkbox"
    }, [
      zn("div", N2, [
        w2,
        n.disabled ? Qd("", !0) : (al(), c2(p, {
          key: 0,
          center: ""
        }))
      ]),
      n.label ? Qd("", !0) : (al(), xd("label", {
        key: 0,
        "data-ref-id": "feather-checkbox-label",
        id: n.labelId,
        for: n.inputId
      }, [
        u2(n.$slots, "default", {}, void 0, !0)
      ], 8, M2))
    ], 40, v2)
  ]);
}
var J2 = /* @__PURE__ */ p2(T2, [["render", Z2], ["__scopeId", "data-v-a7af27e2"]]);
const g2 = Bf(pl({}, Nf), {
  modelValue: {
    type: [Array, Object],
    required: !1
  },
  label: {
    type: String,
    required: !0
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
});
Df({
  props: g2,
  provide() {
    return {
      registerCheckbox: this.registerCheckbox
    };
  },
  setup(n, i) {
    wf(n);
    const a = il(n, "error"), l = Ga(() => Ie("feather-checkbox-group")), c = Ga(() => Ie("feather-input-description")), f = Ga(() => Ie("feather-input-label")), p = Ga(() => {
      const J = JSON.parse(JSON.stringify(i.attrs));
      return J["aria-invalid"] || (J["aria-invalid"] = !!a.value), Bf(pl({}, J), {
        class: "",
        "aria-describedby": c.value
      });
    }), V = h2(l.value), { validate: v } = Rf(V, il(n, "modelValue"), n.label, n.schema, il(n, "error"));
    return pl({
      groupId: l,
      inputId: V,
      descriptionId: c,
      labelId: f,
      attrs: p,
      validate: v,
      registerCheckbox: (J) => {
        J && V.value === l.value && (V.value = J);
      }
    }, vf(i.attrs));
  },
  components: {
    InputSubText: Uf
  }
});
const k2 = window.Vue.defineComponent, Te = window.Vue.createElementVNode, ml = window.Vue.createTextVNode, Xt = window.Vue.unref, Jr = window.Vue.withCtx, gr = window.Vue.createVNode, y2 = window.Vue.openBlock, b2 = window.Vue.createElementBlock, W2 = window.Vue.pushScopeId, E2 = window.Vue.popScopeId, Un = (n) => (W2("data-v-9a5b5f4e"), n = n(), E2(), n), F2 = { class: "container" }, S2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("h2", null, "Configuration", -1)), A2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("p", null, [
  /* @__PURE__ */ ml(" ALEC relies on correlation engines to identify related alarm groupings (situations). These engines are powered by machine learning techniques that leverage alarms data to make informed decisions. "),
  /* @__PURE__ */ Te("br"),
  /* @__PURE__ */ ml(" For detail information about proposed engines you can read "),
  /* @__PURE__ */ Te("strong", null, [
    /* @__PURE__ */ Te("a", {
      target: "_blank",
      href: "https://docs.opennms.com/alec/3.0.0-SNAPSHOT/engines/cluster.html"
    }, " here ")
  ])
], -1)), x2 = { class: "radio-content" }, Q2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("strong", { class: "title" }, "Clustering", -1)), B2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("div", null, " Groups data points (alarms) based on a distance measure. We calculate alarms difference in time and add it to their distance within their network topology ", -1)), D2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("img", {
  class: "img",
  src: e2
}, null, -1)), O2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("div", { class: "hellinger" }, [
  /* @__PURE__ */ Te("strong", null, "With hellinger distance"),
  /* @__PURE__ */ Te("br"),
  /* @__PURE__ */ Te("span", { class: "description" }, " (Uses the Hellinger Distance between alarms as a scaling variable. It pushes alarms further apart if its value is high and vice versa.) ")
], -1)), G2 = /* @__PURE__ */ Un(() => /* @__PURE__ */ Te("div", { class: "radio-content" }, [
  /* @__PURE__ */ Te("strong", { class: "title" }, "Deep Learning"),
  /* @__PURE__ */ Te("div", null, " A Neural Network network is consulted to assess if alarms are related. Based on its evaluation, situations are built by association. "),
  /* @__PURE__ */ Te("img", {
    class: "img img2",
    src: t2
  })
], -1)), C2 = /* @__PURE__ */ ml(" Continue "), Bd = window.Vue.ref, z2 = /* @__PURE__ */ k2({
  __name: "ConfigurationPage",
  setup(n) {
    const i = Qr(), a = Qf(), l = Bd(!1), c = Bd(Cn.ENGINE_DBSCAN), f = () => {
      af(i.allowSave), tv(c.value, l.value), a.push({ name: "situations" });
    };
    return (p, V) => (y2(), b2("div", F2, [
      S2,
      A2,
      Te("div", null, [
        gr(Xt(xf), {
          class: "radio-group",
          label: "Currently, ALEC provides two clustering based engines, please, select one (can be changed later):",
          modelValue: c.value,
          "onUpdate:modelValue": V[1] || (V[1] = (v) => c.value = v)
        }, {
          default: Jr(() => [
            gr(Xt(Ka), {
              class: "radio",
              value: Xt(Cn).ENGINE_DBSCAN
            }, {
              default: Jr(() => [
                Te("div", x2, [
                  Q2,
                  B2,
                  D2,
                  gr(Xt(J2), {
                    modelValue: l.value,
                    "onUpdate:modelValue": V[0] || (V[0] = (v) => l.value = v),
                    disabled: c.value !== Xt(Cn).ENGINE_DBSCAN,
                    class: "checkbox"
                  }, {
                    default: Jr(() => [
                      O2
                    ]),
                    _: 1
                  }, 8, ["modelValue", "disabled"])
                ])
              ]),
              _: 1
            }, 8, ["value"]),
            gr(Xt(Ka), {
              class: "radio",
              value: Xt(Cn).ENGINE_DEEP_LEARNING
            }, {
              default: Jr(() => [
                G2
              ]),
              _: 1
            }, 8, ["value"])
          ]),
          _: 1
        }, 8, ["label", "modelValue"])
      ]),
      gr(Xt(qa), {
        primary: "",
        class: "btn",
        onClick: V[2] || (V[2] = () => f())
      }, {
        default: Jr(() => [
          C2
        ]),
        _: 1
      })
    ]));
  }
});
const Y2 = /* @__PURE__ */ Pe(z2, [["__scopeId", "data-v-9a5b5f4e"]]), I2 = window.VueRouter.createRouter, H2 = window.VueRouter.createWebHistory, ol = async () => {
  const n = window.VRouter || Al, i = Qr();
  i.userId || (await i.getUserRole(), await i.getAlecInfo(), n.push({ name: "home" }));
}, Of = [
  {
    path: "/",
    name: "home",
    beforeEnter: async (n) => {
      const i = window.VRouter || Al, a = Qr();
      await a.getUserRole(), await a.getAlecInfo(), a.firstTime ? i.push({ name: "welcome", params: n.params }) : i.push({ name: "situations", params: n.params });
    },
    component: {}
  },
  {
    path: "/welcome",
    name: "welcome",
    beforeEnter: () => ol(),
    component: $y
  },
  {
    path: "/setup",
    name: "configuration",
    beforeEnter: () => ol(),
    component: Y2
  },
  {
    path: "/situations",
    name: "situations",
    beforeEnter: () => ol(),
    component: ak
  }
], Dd = window.VRouter;
if (Dd)
  for (const n of Of) {
    const { path: i, name: a, component: l, beforeEnter: c } = n;
    Dd.addRoute("Plugin", {
      path: i.slice(1),
      name: a,
      component: l,
      beforeEnter: c
    });
  }
const Al = I2({
  history: H2(),
  routes: Of
});
window.Vue.createApp;
window.Pinia.createPinia;
window.uiextension = G0;
