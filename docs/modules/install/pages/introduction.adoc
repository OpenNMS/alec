...
A xref:distributed_install.adoc[distributed deployment], which installs ALEC externally on an OpenNMS Sentinel server(s), requires a more involved setup that we recommend for a production environment where you have load, scaling, and resiliency requirements.
