...
public class DefaultLlmUsageMetricsTest {
    ...
    public void sumsRoundsIntoBucketsAndPerConsumerTotals() {
        ...
        metrics.recordRound(ToolConsumer.RCA, new TokenUsage(100, 20, 30, 0));
        metrics.recordRound(ToolConsumer.RCA, new TokenUsage(50, 10, 0, 5));
        metrics.recordRound(ToolConsumer.CLUSTERING, new TokenUsage(200, 40, 0, 0));
        metrics.recordRound(ToolConsumer.VALIDATION, new TokenUsage(10, 1, 0, 0));
        metrics.recordRound(ToolConsumer.RCA, null); // ignored
    ...
    public void countsCallsAndFailuresPerConsumer() {
        DefaultLlmUsageMetrics metrics = new DefaultLlmUsageMetrics();
        metrics.recordCall(ToolConsumer.RCA, true);
        metrics.recordCall(ToolConsumer.RCA, false);
        metrics.recordCall(ToolConsumer.CLUSTERING, true);
        metrics.recordCall(ToolConsumer.VALIDATION, false);
        Map<String, Metric> m = metrics.getMetrics();
