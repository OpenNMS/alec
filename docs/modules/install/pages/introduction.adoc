...
The xref:basic_install.adoc[basic deployment], which installs ALEC in the OpenNMS JVM as an additional service, is easy to set up and lets you evaluate or test ALEC with your existing environment.

The xref:distributed_install.adoc[distributed deployment], which installs ALEC externally on the Sentinel JVM, requires a more involved setup that we recommend for a production environment where you have load, scaling, and resiliency requirements.
