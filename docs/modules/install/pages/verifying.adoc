...
Now, let's the the `ifDescr` for an interface on the node we just provisioned.
```
export IFDESCR="eth0"
```
...
If both of these alarms are triggered together in a small enough window of time (within 1 minute for the purposes of this example), a situation should be automaticall created:

image::verify_situation_example.png[Situation,600]

Congragulations you have correlated your first two alarms!
