...
| | `features/mcp` | The embedded MCP server: a registry of read-only tools over OpenNMS data, the shared OpenAI-style tool loop used by root cause analysis and LLM clustering, a JSON-RPC endpoint for external MCP clients, and tool-call metrics. |
...
```
 ...
 |                     (4)  |     | (3)                      |   +--------------------+
 |                          |     v                          |
 |                   MCP tool registry  <--- JSON-RPC -------|<-- external MCP client
 |                    /              \       (admin login)   |    (LM Studio chat,
...
3. The registry dispatches it: in-process tools read the OpenNMS Integration API; events, metrics and device configuration go through the OpenNMS REST API with the configured login.
...
The same tools are served over MCP (Streamable HTTP, stateless) at `POST /opennms/rest/alec/mcp` for external clients, behind an OpenNMS admin login. Tool-call counts and LLM token usage (total, per bucket, per consumer, and as per-second rates) are Dropwizard gauges the driver publishes over JMX next to the engine's own, so OpenNMS charts them as "ALEC Graph Stats" with no extra configuration.
...
To use the tools from an MCP client instead, point it at `http://<host>:8980/opennms/rest/alec/mcp` with HTTP Basic credentials of an OpenNMS admin, for example in LM Studio's `mcp.json`:

```json
{ "mcpServers": { "opennms-alec": {
