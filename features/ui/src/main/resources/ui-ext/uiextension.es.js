const Pe = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
}, k0 = {}, y0 = window.Vue.resolveComponent, b0 = window.Vue.createVNode, W0 = window.Vue.openBlock, E0 = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const F0 = { class: "main" };
function S0(n, i) {
  const a = y0("router-view");
  return W0(), E0("div", F0, [
    b0(a)
  ]);
}
const A0 = /* @__PURE__ */ Pe(k0, [["render", S0], ["__scopeId", "data-v-5d32d140"]]), x0 = window.Vue.defineComponent, Q0 = window.Vue.openBlock, B0 = window.Vue.createBlock, D0 = /* @__PURE__ */ x0({
  __name: "App",
  setup(n) {
    return (i, a) => (Q0(), B0(A0));
  }
});
var Nr = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function O0(n) {
  return n && n.__esModule && Object.prototype.hasOwnProperty.call(n, "default") ? n.default : n;
}
var Bd = { exports: {} }, Vl = { exports: {} }, Dd = function(i, a) {
  return function() {
    for (var c = new Array(arguments.length), f = 0; f < c.length; f++)
      c[f] = arguments[f];
    return i.apply(a, c);
  };
}, G0 = Dd, Tl = Object.prototype.toString, Ul = function(n) {
  return function(i) {
    var a = Tl.call(i);
    return n[a] || (n[a] = a.slice(8, -1).toLowerCase());
  };
}(/* @__PURE__ */ Object.create(null));
function Un(n) {
  return n = n.toLowerCase(), function(a) {
    return Ul(a) === n;
  };
}
function Rl(n) {
  return Array.isArray(n);
}
function Pa(n) {
  return typeof n > "u";
}
function C0(n) {
  return n !== null && !Pa(n) && n.constructor !== null && !Pa(n.constructor) && typeof n.constructor.isBuffer == "function" && n.constructor.isBuffer(n);
}
var Od = Un("ArrayBuffer");
function z0(n) {
  var i;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? i = ArrayBuffer.isView(n) : i = n && n.buffer && Od(n.buffer), i;
}
function Y0(n) {
  return typeof n == "string";
}
function I0(n) {
  return typeof n == "number";
}
function Gd(n) {
  return n !== null && typeof n == "object";
}
function za(n) {
  if (Ul(n) !== "object")
    return !1;
  var i = Object.getPrototypeOf(n);
  return i === null || i === Object.prototype;
}
var H0 = Un("Date"), P0 = Un("File"), j0 = Un("Blob"), X0 = Un("FileList");
function vl(n) {
  return Tl.call(n) === "[object Function]";
}
function L0(n) {
  return Gd(n) && vl(n.pipe);
}
function _0(n) {
  var i = "[object FormData]";
  return n && (typeof FormData == "function" && n instanceof FormData || Tl.call(n) === i || vl(n.toString) && n.toString() === i);
}
var q0 = Un("URLSearchParams");
function K0(n) {
  return n.trim ? n.trim() : n.replace(/^\s+|\s+$/g, "");
}
function $0() {
  return typeof navigator < "u" && (navigator.product === "ReactNative" || navigator.product === "NativeScript" || navigator.product === "NS") ? !1 : typeof window < "u" && typeof document < "u";
}
function Nl(n, i) {
  if (!(n === null || typeof n > "u"))
    if (typeof n != "object" && (n = [n]), Rl(n))
      for (var a = 0, l = n.length; a < l; a++)
        i.call(null, n[a], a, n);
    else
      for (var c in n)
        Object.prototype.hasOwnProperty.call(n, c) && i.call(null, n[c], c, n);
}
function ll() {
  var n = {};
  function i(c, f) {
    za(n[f]) && za(c) ? n[f] = ll(n[f], c) : za(c) ? n[f] = ll({}, c) : Rl(c) ? n[f] = c.slice() : n[f] = c;
  }
  for (var a = 0, l = arguments.length; a < l; a++)
    Nl(arguments[a], i);
  return n;
}
function eR(n, i, a) {
  return Nl(i, function(c, f) {
    a && typeof c == "function" ? n[f] = G0(c, a) : n[f] = c;
  }), n;
}
function tR(n) {
  return n.charCodeAt(0) === 65279 && (n = n.slice(1)), n;
}
function nR(n, i, a, l) {
  n.prototype = Object.create(i.prototype, l), n.prototype.constructor = n, a && Object.assign(n.prototype, a);
}
function rR(n, i, a) {
  var l, c, f, p = {};
  i = i || {};
  do {
    for (l = Object.getOwnPropertyNames(n), c = l.length; c-- > 0; )
      f = l[c], p[f] || (i[f] = n[f], p[f] = !0);
    n = Object.getPrototypeOf(n);
  } while (n && (!a || a(n, i)) && n !== Object.prototype);
  return i;
}
function aR(n, i, a) {
  n = String(n), (a === void 0 || a > n.length) && (a = n.length), a -= i.length;
  var l = n.indexOf(i, a);
  return l !== -1 && l === a;
}
function iR(n) {
  if (!n)
    return null;
  var i = n.length;
  if (Pa(i))
    return null;
  for (var a = new Array(i); i-- > 0; )
    a[i] = n[i];
  return a;
}
var oR = function(n) {
  return function(i) {
    return n && i instanceof n;
  };
}(typeof Uint8Array < "u" && Object.getPrototypeOf(Uint8Array)), Me = {
  isArray: Rl,
  isArrayBuffer: Od,
  isBuffer: C0,
  isFormData: _0,
  isArrayBufferView: z0,
  isString: Y0,
  isNumber: I0,
  isObject: Gd,
  isPlainObject: za,
  isUndefined: Pa,
  isDate: H0,
  isFile: P0,
  isBlob: j0,
  isFunction: vl,
  isStream: L0,
  isURLSearchParams: q0,
  isStandardBrowserEnv: $0,
  forEach: Nl,
  merge: ll,
  extend: eR,
  trim: K0,
  stripBOM: tR,
  inherits: nR,
  toFlatObject: rR,
  kindOf: Ul,
  kindOfTest: Un,
  endsWith: aR,
  toArray: iR,
  isTypedArray: oR,
  isFileList: X0
}, Qn = Me;
function fu(n) {
  return encodeURIComponent(n).replace(/%3A/gi, ":").replace(/%24/g, "$").replace(/%2C/gi, ",").replace(/%20/g, "+").replace(/%5B/gi, "[").replace(/%5D/gi, "]");
}
var Cd = function(i, a, l) {
  if (!a)
    return i;
  var c;
  if (l)
    c = l(a);
  else if (Qn.isURLSearchParams(a))
    c = a.toString();
  else {
    var f = [];
    Qn.forEach(a, function(v, k) {
      v === null || typeof v > "u" || (Qn.isArray(v) ? k = k + "[]" : v = [v], Qn.forEach(v, function(y) {
        Qn.isDate(y) ? y = y.toISOString() : Qn.isObject(y) && (y = JSON.stringify(y)), f.push(fu(k) + "=" + fu(y));
      }));
    }), c = f.join("&");
  }
  if (c) {
    var p = i.indexOf("#");
    p !== -1 && (i = i.slice(0, p)), i += (i.indexOf("?") === -1 ? "?" : "&") + c;
  }
  return i;
}, lR = Me;
function ei() {
  this.handlers = [];
}
ei.prototype.use = function(i, a, l) {
  return this.handlers.push({
    fulfilled: i,
    rejected: a,
    synchronous: l ? l.synchronous : !1,
    runWhen: l ? l.runWhen : null
  }), this.handlers.length - 1;
};
ei.prototype.eject = function(i) {
  this.handlers[i] && (this.handlers[i] = null);
};
ei.prototype.forEach = function(i) {
  lR.forEach(this.handlers, function(l) {
    l !== null && i(l);
  });
};
var sR = ei, cR = Me, uR = function(i, a) {
  cR.forEach(i, function(c, f) {
    f !== a && f.toUpperCase() === a.toUpperCase() && (i[a] = c, delete i[f]);
  });
}, zd = Me;
function Pn(n, i, a, l, c) {
  Error.call(this), this.message = n, this.name = "AxiosError", i && (this.code = i), a && (this.config = a), l && (this.request = l), c && (this.response = c);
}
zd.inherits(Pn, Error, {
  toJSON: function() {
    return {
      message: this.message,
      name: this.name,
      description: this.description,
      number: this.number,
      fileName: this.fileName,
      lineNumber: this.lineNumber,
      columnNumber: this.columnNumber,
      stack: this.stack,
      config: this.config,
      code: this.code,
      status: this.response && this.response.status ? this.response.status : null
    };
  }
});
var Yd = Pn.prototype, Id = {};
[
  "ERR_BAD_OPTION_VALUE",
  "ERR_BAD_OPTION",
  "ECONNABORTED",
  "ETIMEDOUT",
  "ERR_NETWORK",
  "ERR_FR_TOO_MANY_REDIRECTS",
  "ERR_DEPRECATED",
  "ERR_BAD_RESPONSE",
  "ERR_BAD_REQUEST",
  "ERR_CANCELED"
].forEach(function(n) {
  Id[n] = { value: n };
});
Object.defineProperties(Pn, Id);
Object.defineProperty(Yd, "isAxiosError", { value: !0 });
Pn.from = function(n, i, a, l, c, f) {
  var p = Object.create(Yd);
  return zd.toFlatObject(n, p, function(v) {
    return v !== Error.prototype;
  }), Pn.call(p, n.message, i, a, l, c), p.name = n.name, f && Object.assign(p, f), p;
};
var qn = Pn, Hd = {
  silentJSONParsing: !0,
  forcedJSONParsing: !0,
  clarifyTimeoutError: !1
}, dt = Me;
function dR(n, i) {
  i = i || new FormData();
  var a = [];
  function l(f) {
    return f === null ? "" : dt.isDate(f) ? f.toISOString() : dt.isArrayBuffer(f) || dt.isTypedArray(f) ? typeof Blob == "function" ? new Blob([f]) : Buffer.from(f) : f;
  }
  function c(f, p) {
    if (dt.isPlainObject(f) || dt.isArray(f)) {
      if (a.indexOf(f) !== -1)
        throw Error("Circular reference detected in " + p);
      a.push(f), dt.forEach(f, function(v, k) {
        if (!dt.isUndefined(v)) {
          var J = p ? p + "." + k : k, y;
          if (v && !p && typeof v == "object") {
            if (dt.endsWith(k, "{}"))
              v = JSON.stringify(v);
            else if (dt.endsWith(k, "[]") && (y = dt.toArray(v))) {
              y.forEach(function(g) {
                !dt.isUndefined(g) && i.append(J, l(g));
              });
              return;
            }
          }
          c(v, J);
        }
      }), a.pop();
    } else
      i.append(p, l(f));
  }
  return c(n), i;
}
var Pd = dR, bo, hu;
function fR() {
  if (hu)
    return bo;
  hu = 1;
  var n = qn;
  return bo = function(a, l, c) {
    var f = c.config.validateStatus;
    !c.status || !f || f(c.status) ? a(c) : l(new n(
      "Request failed with status code " + c.status,
      [n.ERR_BAD_REQUEST, n.ERR_BAD_RESPONSE][Math.floor(c.status / 100) - 4],
      c.config,
      c.request,
      c
    ));
  }, bo;
}
var Wo, pu;
function hR() {
  if (pu)
    return Wo;
  pu = 1;
  var n = Me;
  return Wo = n.isStandardBrowserEnv() ? function() {
    return {
      write: function(l, c, f, p, V, v) {
        var k = [];
        k.push(l + "=" + encodeURIComponent(c)), n.isNumber(f) && k.push("expires=" + new Date(f).toGMTString()), n.isString(p) && k.push("path=" + p), n.isString(V) && k.push("domain=" + V), v === !0 && k.push("secure"), document.cookie = k.join("; ");
      },
      read: function(l) {
        var c = document.cookie.match(new RegExp("(^|;\\s*)(" + l + ")=([^;]*)"));
        return c ? decodeURIComponent(c[3]) : null;
      },
      remove: function(l) {
        this.write(l, "", Date.now() - 864e5);
      }
    };
  }() : function() {
    return {
      write: function() {
      },
      read: function() {
        return null;
      },
      remove: function() {
      }
    };
  }(), Wo;
}
var pR = function(i) {
  return /^([a-z][a-z\d+\-.]*:)?\/\//i.test(i);
}, mR = function(i, a) {
  return a ? i.replace(/\/+$/, "") + "/" + a.replace(/^\/+/, "") : i;
}, VR = pR, TR = mR, jd = function(i, a) {
  return i && !VR(a) ? TR(i, a) : a;
}, Eo, mu;
function UR() {
  if (mu)
    return Eo;
  mu = 1;
  var n = Me, i = [
    "age",
    "authorization",
    "content-length",
    "content-type",
    "etag",
    "expires",
    "from",
    "host",
    "if-modified-since",
    "if-unmodified-since",
    "last-modified",
    "location",
    "max-forwards",
    "proxy-authorization",
    "referer",
    "retry-after",
    "user-agent"
  ];
  return Eo = function(l) {
    var c = {}, f, p, V;
    return l && n.forEach(l.split(`
`), function(k) {
      if (V = k.indexOf(":"), f = n.trim(k.substr(0, V)).toLowerCase(), p = n.trim(k.substr(V + 1)), f) {
        if (c[f] && i.indexOf(f) >= 0)
          return;
        f === "set-cookie" ? c[f] = (c[f] ? c[f] : []).concat([p]) : c[f] = c[f] ? c[f] + ", " + p : p;
      }
    }), c;
  }, Eo;
}
var Fo, Vu;
function RR() {
  if (Vu)
    return Fo;
  Vu = 1;
  var n = Me;
  return Fo = n.isStandardBrowserEnv() ? function() {
    var a = /(msie|trident)/i.test(navigator.userAgent), l = document.createElement("a"), c;
    function f(p) {
      var V = p;
      return a && (l.setAttribute("href", V), V = l.href), l.setAttribute("href", V), {
        href: l.href,
        protocol: l.protocol ? l.protocol.replace(/:$/, "") : "",
        host: l.host,
        search: l.search ? l.search.replace(/^\?/, "") : "",
        hash: l.hash ? l.hash.replace(/^#/, "") : "",
        hostname: l.hostname,
        port: l.port,
        pathname: l.pathname.charAt(0) === "/" ? l.pathname : "/" + l.pathname
      };
    }
    return c = f(window.location.href), function(V) {
      var v = n.isString(V) ? f(V) : V;
      return v.protocol === c.protocol && v.host === c.host;
    };
  }() : function() {
    return function() {
      return !0;
    };
  }(), Fo;
}
var So, Tu;
function ti() {
  if (Tu)
    return So;
  Tu = 1;
  var n = qn, i = Me;
  function a(l) {
    n.call(this, l == null ? "canceled" : l, n.ERR_CANCELED), this.name = "CanceledError";
  }
  return i.inherits(a, n, {
    __CANCEL__: !0
  }), So = a, So;
}
var Ao, Uu;
function vR() {
  return Uu || (Uu = 1, Ao = function(i) {
    var a = /^([-+\w]{1,25})(:?\/\/|:)/.exec(i);
    return a && a[1] || "";
  }), Ao;
}
var xo, Ru;
function vu() {
  if (Ru)
    return xo;
  Ru = 1;
  var n = Me, i = fR(), a = hR(), l = Cd, c = jd, f = UR(), p = RR(), V = Hd, v = qn, k = ti(), J = vR();
  return xo = function(g) {
    return new Promise(function(S, O) {
      var G = g.data, B = g.headers, _ = g.responseType, ne;
      function oe() {
        g.cancelToken && g.cancelToken.unsubscribe(ne), g.signal && g.signal.removeEventListener("abort", ne);
      }
      n.isFormData(G) && n.isStandardBrowserEnv() && delete B["Content-Type"];
      var F = new XMLHttpRequest();
      if (g.auth) {
        var Ue = g.auth.username || "", fe = g.auth.password ? unescape(encodeURIComponent(g.auth.password)) : "";
        B.Authorization = "Basic " + btoa(Ue + ":" + fe);
      }
      var ee = c(g.baseURL, g.url);
      F.open(g.method.toUpperCase(), l(ee, g.params, g.paramsSerializer), !0), F.timeout = g.timeout;
      function ht() {
        if (!!F) {
          var Je = "getAllResponseHeaders" in F ? f(F.getAllResponseHeaders()) : null, pt = !_ || _ === "text" || _ === "json" ? F.responseText : F.response, je = {
            data: pt,
            status: F.status,
            statusText: F.statusText,
            headers: Je,
            config: g,
            request: F
          };
          i(function(Ee) {
            S(Ee), oe();
          }, function(Ee) {
            O(Ee), oe();
          }, je), F = null;
        }
      }
      if ("onloadend" in F ? F.onloadend = ht : F.onreadystatechange = function() {
        !F || F.readyState !== 4 || F.status === 0 && !(F.responseURL && F.responseURL.indexOf("file:") === 0) || setTimeout(ht);
      }, F.onabort = function() {
        !F || (O(new v("Request aborted", v.ECONNABORTED, g, F)), F = null);
      }, F.onerror = function() {
        O(new v("Network Error", v.ERR_NETWORK, g, F, F)), F = null;
      }, F.ontimeout = function() {
        var pt = g.timeout ? "timeout of " + g.timeout + "ms exceeded" : "timeout exceeded", je = g.transitional || V;
        g.timeoutErrorMessage && (pt = g.timeoutErrorMessage), O(new v(
          pt,
          je.clarifyTimeoutError ? v.ETIMEDOUT : v.ECONNABORTED,
          g,
          F
        )), F = null;
      }, n.isStandardBrowserEnv()) {
        var Mt = (g.withCredentials || p(ee)) && g.xsrfCookieName ? a.read(g.xsrfCookieName) : void 0;
        Mt && (B[g.xsrfHeaderName] = Mt);
      }
      "setRequestHeader" in F && n.forEach(B, function(pt, je) {
        typeof G > "u" && je.toLowerCase() === "content-type" ? delete B[je] : F.setRequestHeader(je, pt);
      }), n.isUndefined(g.withCredentials) || (F.withCredentials = !!g.withCredentials), _ && _ !== "json" && (F.responseType = g.responseType), typeof g.onDownloadProgress == "function" && F.addEventListener("progress", g.onDownloadProgress), typeof g.onUploadProgress == "function" && F.upload && F.upload.addEventListener("progress", g.onUploadProgress), (g.cancelToken || g.signal) && (ne = function(Je) {
        !F || (O(!Je || Je && Je.type ? new k() : Je), F.abort(), F = null);
      }, g.cancelToken && g.cancelToken.subscribe(ne), g.signal && (g.signal.aborted ? ne() : g.signal.addEventListener("abort", ne))), G || (G = null);
      var rn = J(ee);
      if (rn && ["http", "https", "file"].indexOf(rn) === -1) {
        O(new v("Unsupported protocol " + rn + ":", v.ERR_BAD_REQUEST, g));
        return;
      }
      F.send(G);
    });
  }, xo;
}
var Qo, Nu;
function NR() {
  return Nu || (Nu = 1, Qo = null), Qo;
}
var Ve = Me, Mu = uR, wu = qn, MR = Hd, wR = Pd, ZR = {
  "Content-Type": "application/x-www-form-urlencoded"
};
function Zu(n, i) {
  !Ve.isUndefined(n) && Ve.isUndefined(n["Content-Type"]) && (n["Content-Type"] = i);
}
function JR() {
  var n;
  return (typeof XMLHttpRequest < "u" || typeof process < "u" && Object.prototype.toString.call(process) === "[object process]") && (n = vu()), n;
}
function gR(n, i, a) {
  if (Ve.isString(n))
    try {
      return (i || JSON.parse)(n), Ve.trim(n);
    } catch (l) {
      if (l.name !== "SyntaxError")
        throw l;
    }
  return (a || JSON.stringify)(n);
}
var ni = {
  transitional: MR,
  adapter: JR(),
  transformRequest: [function(i, a) {
    if (Mu(a, "Accept"), Mu(a, "Content-Type"), Ve.isFormData(i) || Ve.isArrayBuffer(i) || Ve.isBuffer(i) || Ve.isStream(i) || Ve.isFile(i) || Ve.isBlob(i))
      return i;
    if (Ve.isArrayBufferView(i))
      return i.buffer;
    if (Ve.isURLSearchParams(i))
      return Zu(a, "application/x-www-form-urlencoded;charset=utf-8"), i.toString();
    var l = Ve.isObject(i), c = a && a["Content-Type"], f;
    if ((f = Ve.isFileList(i)) || l && c === "multipart/form-data") {
      var p = this.env && this.env.FormData;
      return wR(f ? { "files[]": i } : i, p && new p());
    } else if (l || c === "application/json")
      return Zu(a, "application/json"), gR(i);
    return i;
  }],
  transformResponse: [function(i) {
    var a = this.transitional || ni.transitional, l = a && a.silentJSONParsing, c = a && a.forcedJSONParsing, f = !l && this.responseType === "json";
    if (f || c && Ve.isString(i) && i.length)
      try {
        return JSON.parse(i);
      } catch (p) {
        if (f)
          throw p.name === "SyntaxError" ? wu.from(p, wu.ERR_BAD_RESPONSE, this, null, this.response) : p;
      }
    return i;
  }],
  timeout: 0,
  xsrfCookieName: "XSRF-TOKEN",
  xsrfHeaderName: "X-XSRF-TOKEN",
  maxContentLength: -1,
  maxBodyLength: -1,
  env: {
    FormData: NR()
  },
  validateStatus: function(i) {
    return i >= 200 && i < 300;
  },
  headers: {
    common: {
      Accept: "application/json, text/plain, */*"
    }
  }
};
Ve.forEach(["delete", "get", "head"], function(i) {
  ni.headers[i] = {};
});
Ve.forEach(["post", "put", "patch"], function(i) {
  ni.headers[i] = Ve.merge(ZR);
});
var Ml = ni, kR = Me, yR = Ml, bR = function(i, a, l) {
  var c = this || yR;
  return kR.forEach(l, function(p) {
    i = p.call(c, i, a);
  }), i;
}, Bo, Ju;
function Xd() {
  return Ju || (Ju = 1, Bo = function(i) {
    return !!(i && i.__CANCEL__);
  }), Bo;
}
var gu = Me, Do = bR, WR = Xd(), ER = Ml, FR = ti();
function Oo(n) {
  if (n.cancelToken && n.cancelToken.throwIfRequested(), n.signal && n.signal.aborted)
    throw new FR();
}
var SR = function(i) {
  Oo(i), i.headers = i.headers || {}, i.data = Do.call(
    i,
    i.data,
    i.headers,
    i.transformRequest
  ), i.headers = gu.merge(
    i.headers.common || {},
    i.headers[i.method] || {},
    i.headers
  ), gu.forEach(
    ["delete", "get", "head", "post", "put", "patch", "common"],
    function(c) {
      delete i.headers[c];
    }
  );
  var a = i.adapter || ER.adapter;
  return a(i).then(function(c) {
    return Oo(i), c.data = Do.call(
      i,
      c.data,
      c.headers,
      i.transformResponse
    ), c;
  }, function(c) {
    return WR(c) || (Oo(i), c && c.response && (c.response.data = Do.call(
      i,
      c.response.data,
      c.response.headers,
      i.transformResponse
    ))), Promise.reject(c);
  });
}, Ye = Me, Ld = function(i, a) {
  a = a || {};
  var l = {};
  function c(J, y) {
    return Ye.isPlainObject(J) && Ye.isPlainObject(y) ? Ye.merge(J, y) : Ye.isPlainObject(y) ? Ye.merge({}, y) : Ye.isArray(y) ? y.slice() : y;
  }
  function f(J) {
    if (Ye.isUndefined(a[J])) {
      if (!Ye.isUndefined(i[J]))
        return c(void 0, i[J]);
    } else
      return c(i[J], a[J]);
  }
  function p(J) {
    if (!Ye.isUndefined(a[J]))
      return c(void 0, a[J]);
  }
  function V(J) {
    if (Ye.isUndefined(a[J])) {
      if (!Ye.isUndefined(i[J]))
        return c(void 0, i[J]);
    } else
      return c(void 0, a[J]);
  }
  function v(J) {
    if (J in a)
      return c(i[J], a[J]);
    if (J in i)
      return c(void 0, i[J]);
  }
  var k = {
    url: p,
    method: p,
    data: p,
    baseURL: V,
    transformRequest: V,
    transformResponse: V,
    paramsSerializer: V,
    timeout: V,
    timeoutMessage: V,
    withCredentials: V,
    adapter: V,
    responseType: V,
    xsrfCookieName: V,
    xsrfHeaderName: V,
    onUploadProgress: V,
    onDownloadProgress: V,
    decompress: V,
    maxContentLength: V,
    maxBodyLength: V,
    beforeRedirect: V,
    transport: V,
    httpAgent: V,
    httpsAgent: V,
    cancelToken: V,
    socketPath: V,
    responseEncoding: V,
    validateStatus: v
  };
  return Ye.forEach(Object.keys(i).concat(Object.keys(a)), function(y) {
    var g = k[y] || f, H = g(y);
    Ye.isUndefined(H) && g !== v || (l[y] = H);
  }), l;
}, Go, ku;
function _d() {
  return ku || (ku = 1, Go = {
    version: "0.27.2"
  }), Go;
}
var AR = _d().version, qt = qn, wl = {};
["object", "boolean", "number", "function", "string", "symbol"].forEach(function(n, i) {
  wl[n] = function(l) {
    return typeof l === n || "a" + (i < 1 ? "n " : " ") + n;
  };
});
var yu = {};
wl.transitional = function(i, a, l) {
  function c(f, p) {
    return "[Axios v" + AR + "] Transitional option '" + f + "'" + p + (l ? ". " + l : "");
  }
  return function(f, p, V) {
    if (i === !1)
      throw new qt(
        c(p, " has been removed" + (a ? " in " + a : "")),
        qt.ERR_DEPRECATED
      );
    return a && !yu[p] && (yu[p] = !0, console.warn(
      c(
        p,
        " has been deprecated since v" + a + " and will be removed in the near future"
      )
    )), i ? i(f, p, V) : !0;
  };
};
function xR(n, i, a) {
  if (typeof n != "object")
    throw new qt("options must be an object", qt.ERR_BAD_OPTION_VALUE);
  for (var l = Object.keys(n), c = l.length; c-- > 0; ) {
    var f = l[c], p = i[f];
    if (p) {
      var V = n[f], v = V === void 0 || p(V, f, n);
      if (v !== !0)
        throw new qt("option " + f + " must be " + v, qt.ERR_BAD_OPTION_VALUE);
      continue;
    }
    if (a !== !0)
      throw new qt("Unknown option " + f, qt.ERR_BAD_OPTION);
  }
}
var QR = {
  assertOptions: xR,
  validators: wl
}, qd = Me, BR = Cd, bu = sR, Wu = SR, ri = Ld, DR = jd, Kd = QR, Bn = Kd.validators;
function jn(n) {
  this.defaults = n, this.interceptors = {
    request: new bu(),
    response: new bu()
  };
}
jn.prototype.request = function(i, a) {
  typeof i == "string" ? (a = a || {}, a.url = i) : a = i || {}, a = ri(this.defaults, a), a.method ? a.method = a.method.toLowerCase() : this.defaults.method ? a.method = this.defaults.method.toLowerCase() : a.method = "get";
  var l = a.transitional;
  l !== void 0 && Kd.assertOptions(l, {
    silentJSONParsing: Bn.transitional(Bn.boolean),
    forcedJSONParsing: Bn.transitional(Bn.boolean),
    clarifyTimeoutError: Bn.transitional(Bn.boolean)
  }, !1);
  var c = [], f = !0;
  this.interceptors.request.forEach(function(H) {
    typeof H.runWhen == "function" && H.runWhen(a) === !1 || (f = f && H.synchronous, c.unshift(H.fulfilled, H.rejected));
  });
  var p = [];
  this.interceptors.response.forEach(function(H) {
    p.push(H.fulfilled, H.rejected);
  });
  var V;
  if (!f) {
    var v = [Wu, void 0];
    for (Array.prototype.unshift.apply(v, c), v = v.concat(p), V = Promise.resolve(a); v.length; )
      V = V.then(v.shift(), v.shift());
    return V;
  }
  for (var k = a; c.length; ) {
    var J = c.shift(), y = c.shift();
    try {
      k = J(k);
    } catch (g) {
      y(g);
      break;
    }
  }
  try {
    V = Wu(k);
  } catch (g) {
    return Promise.reject(g);
  }
  for (; p.length; )
    V = V.then(p.shift(), p.shift());
  return V;
};
jn.prototype.getUri = function(i) {
  i = ri(this.defaults, i);
  var a = DR(i.baseURL, i.url);
  return BR(a, i.params, i.paramsSerializer);
};
qd.forEach(["delete", "get", "head", "options"], function(i) {
  jn.prototype[i] = function(a, l) {
    return this.request(ri(l || {}, {
      method: i,
      url: a,
      data: (l || {}).data
    }));
  };
});
qd.forEach(["post", "put", "patch"], function(i) {
  function a(l) {
    return function(f, p, V) {
      return this.request(ri(V || {}, {
        method: i,
        headers: l ? {
          "Content-Type": "multipart/form-data"
        } : {},
        url: f,
        data: p
      }));
    };
  }
  jn.prototype[i] = a(), jn.prototype[i + "Form"] = a(!0);
});
var OR = jn, Co, Eu;
function GR() {
  if (Eu)
    return Co;
  Eu = 1;
  var n = ti();
  function i(a) {
    if (typeof a != "function")
      throw new TypeError("executor must be a function.");
    var l;
    this.promise = new Promise(function(p) {
      l = p;
    });
    var c = this;
    this.promise.then(function(f) {
      if (!!c._listeners) {
        var p, V = c._listeners.length;
        for (p = 0; p < V; p++)
          c._listeners[p](f);
        c._listeners = null;
      }
    }), this.promise.then = function(f) {
      var p, V = new Promise(function(v) {
        c.subscribe(v), p = v;
      }).then(f);
      return V.cancel = function() {
        c.unsubscribe(p);
      }, V;
    }, a(function(p) {
      c.reason || (c.reason = new n(p), l(c.reason));
    });
  }
  return i.prototype.throwIfRequested = function() {
    if (this.reason)
      throw this.reason;
  }, i.prototype.subscribe = function(l) {
    if (this.reason) {
      l(this.reason);
      return;
    }
    this._listeners ? this._listeners.push(l) : this._listeners = [l];
  }, i.prototype.unsubscribe = function(l) {
    if (!!this._listeners) {
      var c = this._listeners.indexOf(l);
      c !== -1 && this._listeners.splice(c, 1);
    }
  }, i.source = function() {
    var l, c = new i(function(p) {
      l = p;
    });
    return {
      token: c,
      cancel: l
    };
  }, Co = i, Co;
}
var zo, Fu;
function CR() {
  return Fu || (Fu = 1, zo = function(i) {
    return function(l) {
      return i.apply(null, l);
    };
  }), zo;
}
var Yo, Su;
function zR() {
  if (Su)
    return Yo;
  Su = 1;
  var n = Me;
  return Yo = function(a) {
    return n.isObject(a) && a.isAxiosError === !0;
  }, Yo;
}
var Au = Me, YR = Dd, Ya = OR, IR = Ld, HR = Ml;
function $d(n) {
  var i = new Ya(n), a = YR(Ya.prototype.request, i);
  return Au.extend(a, Ya.prototype, i), Au.extend(a, i), a.create = function(c) {
    return $d(IR(n, c));
  }, a;
}
var Qe = $d(HR);
Qe.Axios = Ya;
Qe.CanceledError = ti();
Qe.CancelToken = GR();
Qe.isCancel = Xd();
Qe.VERSION = _d().version;
Qe.toFormData = Pd;
Qe.AxiosError = qn;
Qe.Cancel = Qe.CanceledError;
Qe.all = function(i) {
  return Promise.all(i);
};
Qe.spread = CR();
Qe.isAxiosError = zR();
Vl.exports = Qe;
Vl.exports.default = Qe;
(function(n) {
  n.exports = Vl.exports;
})(Bd);
const ef = /* @__PURE__ */ O0(Bd.exports), PR = ef.create({
  baseURL: "/opennms/api/v2".toString() || "/opennms/api/v2",
  withCredentials: !0
}), Dr = ef.create({
  baseURL: "/opennms/rest".toString() || "/opennms/rest",
  withCredentials: !0
}), jR = "/alarms", XR = async () => {
  try {
    const n = await PR.get(jR);
    return n.status === 204 ? { alarm: [], totalCount: 0, count: 0, offset: 0 } : n.data;
  } catch {
    return !1;
  }
};
var Xn = { exports: {} };
/**
 * @license
 * Lodash <https://lodash.com/>
 * Copyright OpenJS Foundation and other contributors <https://openjsf.org/>
 * Released under MIT license <https://lodash.com/license>
 * Based on Underscore.js 1.8.3 <http://underscorejs.org/LICENSE>
 * Copyright Jeremy Ashkenas, DocumentCloud and Investigative Reporters & Editors
 */
(function(n, i) {
  (function() {
    var a, l = "4.17.21", c = 200, f = "Unsupported core-js use. Try https://npms.io/search?q=ponyfill.", p = "Expected a function", V = "Invalid `variable` option passed into `_.template`", v = "__lodash_hash_undefined__", k = 500, J = "__lodash_placeholder__", y = 1, g = 2, H = 4, S = 1, O = 2, G = 1, B = 2, _ = 4, ne = 8, oe = 16, F = 32, Ue = 64, fe = 128, ee = 256, ht = 512, Mt = 30, rn = "...", Je = 800, pt = 16, je = 1, Re = 2, Ee = 3, Xe = 1 / 0, Dt = 9007199254740991, Df = 17976931348623157e292, Cr = 0 / 0, mt = 4294967295, Of = mt - 1, Gf = mt >>> 1, Cf = [
      ["ary", fe],
      ["bind", G],
      ["bindKey", B],
      ["curry", ne],
      ["curryRight", oe],
      ["flip", ht],
      ["partial", F],
      ["partialRight", Ue],
      ["rearg", ee]
    ], vn = "[object Arguments]", zr = "[object Array]", zf = "[object AsyncFunction]", $n = "[object Boolean]", er = "[object Date]", Yf = "[object DOMException]", Yr = "[object Error]", Ir = "[object Function]", xl = "[object GeneratorFunction]", it = "[object Map]", tr = "[object Number]", If = "[object Null]", wt = "[object Object]", Ql = "[object Promise]", Hf = "[object Proxy]", nr = "[object RegExp]", ot = "[object Set]", rr = "[object String]", Hr = "[object Symbol]", Pf = "[object Undefined]", ar = "[object WeakMap]", jf = "[object WeakSet]", ir = "[object ArrayBuffer]", Nn = "[object DataView]", li = "[object Float32Array]", si = "[object Float64Array]", ci = "[object Int8Array]", ui = "[object Int16Array]", di = "[object Int32Array]", fi = "[object Uint8Array]", hi = "[object Uint8ClampedArray]", pi = "[object Uint16Array]", mi = "[object Uint32Array]", Xf = /\b__p \+= '';/g, Lf = /\b(__p \+=) '' \+/g, _f = /(__e\(.*?\)|\b__t\)) \+\n'';/g, Bl = /&(?:amp|lt|gt|quot|#39);/g, Dl = /[&<>"']/g, qf = RegExp(Bl.source), Kf = RegExp(Dl.source), $f = /<%-([\s\S]+?)%>/g, eh = /<%([\s\S]+?)%>/g, Ol = /<%=([\s\S]+?)%>/g, th = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, nh = /^\w*$/, rh = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g, Vi = /[\\^$.*+?()[\]{}|]/g, ah = RegExp(Vi.source), Ti = /^\s+/, ih = /\s/, oh = /\{(?:\n\/\* \[wrapped with .+\] \*\/)?\n?/, lh = /\{\n\/\* \[wrapped with (.+)\] \*/, sh = /,? & /, ch = /[^\x00-\x2f\x3a-\x40\x5b-\x60\x7b-\x7f]+/g, uh = /[()=,{}\[\]\/\s]/, dh = /\\(\\)?/g, fh = /\$\{([^\\}]*(?:\\.[^\\}]*)*)\}/g, Gl = /\w*$/, hh = /^[-+]0x[0-9a-f]+$/i, ph = /^0b[01]+$/i, mh = /^\[object .+?Constructor\]$/, Vh = /^0o[0-7]+$/i, Th = /^(?:0|[1-9]\d*)$/, Uh = /[\xc0-\xd6\xd8-\xf6\xf8-\xff\u0100-\u017f]/g, Pr = /($^)/, Rh = /['\n\r\u2028\u2029\\]/g, jr = "\\ud800-\\udfff", vh = "\\u0300-\\u036f", Nh = "\\ufe20-\\ufe2f", Mh = "\\u20d0-\\u20ff", Cl = vh + Nh + Mh, zl = "\\u2700-\\u27bf", Yl = "a-z\\xdf-\\xf6\\xf8-\\xff", wh = "\\xac\\xb1\\xd7\\xf7", Zh = "\\x00-\\x2f\\x3a-\\x40\\x5b-\\x60\\x7b-\\xbf", Jh = "\\u2000-\\u206f", gh = " \\t\\x0b\\f\\xa0\\ufeff\\n\\r\\u2028\\u2029\\u1680\\u180e\\u2000\\u2001\\u2002\\u2003\\u2004\\u2005\\u2006\\u2007\\u2008\\u2009\\u200a\\u202f\\u205f\\u3000", Il = "A-Z\\xc0-\\xd6\\xd8-\\xde", Hl = "\\ufe0e\\ufe0f", Pl = wh + Zh + Jh + gh, Ui = "['\u2019]", kh = "[" + jr + "]", jl = "[" + Pl + "]", Xr = "[" + Cl + "]", Xl = "\\d+", yh = "[" + zl + "]", Ll = "[" + Yl + "]", _l = "[^" + jr + Pl + Xl + zl + Yl + Il + "]", Ri = "\\ud83c[\\udffb-\\udfff]", bh = "(?:" + Xr + "|" + Ri + ")", ql = "[^" + jr + "]", vi = "(?:\\ud83c[\\udde6-\\uddff]){2}", Ni = "[\\ud800-\\udbff][\\udc00-\\udfff]", Mn = "[" + Il + "]", Kl = "\\u200d", $l = "(?:" + Ll + "|" + _l + ")", Wh = "(?:" + Mn + "|" + _l + ")", es = "(?:" + Ui + "(?:d|ll|m|re|s|t|ve))?", ts = "(?:" + Ui + "(?:D|LL|M|RE|S|T|VE))?", ns = bh + "?", rs = "[" + Hl + "]?", Eh = "(?:" + Kl + "(?:" + [ql, vi, Ni].join("|") + ")" + rs + ns + ")*", Fh = "\\d*(?:1st|2nd|3rd|(?![123])\\dth)(?=\\b|[A-Z_])", Sh = "\\d*(?:1ST|2ND|3RD|(?![123])\\dTH)(?=\\b|[a-z_])", as = rs + ns + Eh, Ah = "(?:" + [yh, vi, Ni].join("|") + ")" + as, xh = "(?:" + [ql + Xr + "?", Xr, vi, Ni, kh].join("|") + ")", Qh = RegExp(Ui, "g"), Bh = RegExp(Xr, "g"), Mi = RegExp(Ri + "(?=" + Ri + ")|" + xh + as, "g"), Dh = RegExp([
      Mn + "?" + Ll + "+" + es + "(?=" + [jl, Mn, "$"].join("|") + ")",
      Wh + "+" + ts + "(?=" + [jl, Mn + $l, "$"].join("|") + ")",
      Mn + "?" + $l + "+" + es,
      Mn + "+" + ts,
      Sh,
      Fh,
      Xl,
      Ah
    ].join("|"), "g"), Oh = RegExp("[" + Kl + jr + Cl + Hl + "]"), Gh = /[a-z][A-Z]|[A-Z]{2}[a-z]|[0-9][a-zA-Z]|[a-zA-Z][0-9]|[^a-zA-Z0-9 ]/, Ch = [
      "Array",
      "Buffer",
      "DataView",
      "Date",
      "Error",
      "Float32Array",
      "Float64Array",
      "Function",
      "Int8Array",
      "Int16Array",
      "Int32Array",
      "Map",
      "Math",
      "Object",
      "Promise",
      "RegExp",
      "Set",
      "String",
      "Symbol",
      "TypeError",
      "Uint8Array",
      "Uint8ClampedArray",
      "Uint16Array",
      "Uint32Array",
      "WeakMap",
      "_",
      "clearTimeout",
      "isFinite",
      "parseInt",
      "setTimeout"
    ], zh = -1, ae = {};
    ae[li] = ae[si] = ae[ci] = ae[ui] = ae[di] = ae[fi] = ae[hi] = ae[pi] = ae[mi] = !0, ae[vn] = ae[zr] = ae[ir] = ae[$n] = ae[Nn] = ae[er] = ae[Yr] = ae[Ir] = ae[it] = ae[tr] = ae[wt] = ae[nr] = ae[ot] = ae[rr] = ae[ar] = !1;
    var re = {};
    re[vn] = re[zr] = re[ir] = re[Nn] = re[$n] = re[er] = re[li] = re[si] = re[ci] = re[ui] = re[di] = re[it] = re[tr] = re[wt] = re[nr] = re[ot] = re[rr] = re[Hr] = re[fi] = re[hi] = re[pi] = re[mi] = !0, re[Yr] = re[Ir] = re[ar] = !1;
    var Yh = {
      \u00C0: "A",
      \u00C1: "A",
      \u00C2: "A",
      \u00C3: "A",
      \u00C4: "A",
      \u00C5: "A",
      \u00E0: "a",
      \u00E1: "a",
      \u00E2: "a",
      \u00E3: "a",
      \u00E4: "a",
      \u00E5: "a",
      \u00C7: "C",
      \u00E7: "c",
      \u00D0: "D",
      \u00F0: "d",
      \u00C8: "E",
      \u00C9: "E",
      \u00CA: "E",
      \u00CB: "E",
      \u00E8: "e",
      \u00E9: "e",
      \u00EA: "e",
      \u00EB: "e",
      \u00CC: "I",
      \u00CD: "I",
      \u00CE: "I",
      \u00CF: "I",
      \u00EC: "i",
      \u00ED: "i",
      \u00EE: "i",
      \u00EF: "i",
      \u00D1: "N",
      \u00F1: "n",
      \u00D2: "O",
      \u00D3: "O",
      \u00D4: "O",
      \u00D5: "O",
      \u00D6: "O",
      \u00D8: "O",
      \u00F2: "o",
      \u00F3: "o",
      \u00F4: "o",
      \u00F5: "o",
      \u00F6: "o",
      \u00F8: "o",
      \u00D9: "U",
      \u00DA: "U",
      \u00DB: "U",
      \u00DC: "U",
      \u00F9: "u",
      \u00FA: "u",
      \u00FB: "u",
      \u00FC: "u",
      \u00DD: "Y",
      \u00FD: "y",
      \u00FF: "y",
      \u00C6: "Ae",
      \u00E6: "ae",
      \u00DE: "Th",
      \u00FE: "th",
      \u00DF: "ss",
      \u0100: "A",
      \u0102: "A",
      \u0104: "A",
      \u0101: "a",
      \u0103: "a",
      \u0105: "a",
      \u0106: "C",
      \u0108: "C",
      \u010A: "C",
      \u010C: "C",
      \u0107: "c",
      \u0109: "c",
      \u010B: "c",
      \u010D: "c",
      \u010E: "D",
      \u0110: "D",
      \u010F: "d",
      \u0111: "d",
      \u0112: "E",
      \u0114: "E",
      \u0116: "E",
      \u0118: "E",
      \u011A: "E",
      \u0113: "e",
      \u0115: "e",
      \u0117: "e",
      \u0119: "e",
      \u011B: "e",
      \u011C: "G",
      \u011E: "G",
      \u0120: "G",
      \u0122: "G",
      \u011D: "g",
      \u011F: "g",
      \u0121: "g",
      \u0123: "g",
      \u0124: "H",
      \u0126: "H",
      \u0125: "h",
      \u0127: "h",
      \u0128: "I",
      \u012A: "I",
      \u012C: "I",
      \u012E: "I",
      \u0130: "I",
      \u0129: "i",
      \u012B: "i",
      \u012D: "i",
      \u012F: "i",
      \u0131: "i",
      \u0134: "J",
      \u0135: "j",
      \u0136: "K",
      \u0137: "k",
      \u0138: "k",
      \u0139: "L",
      \u013B: "L",
      \u013D: "L",
      \u013F: "L",
      \u0141: "L",
      \u013A: "l",
      \u013C: "l",
      \u013E: "l",
      \u0140: "l",
      \u0142: "l",
      \u0143: "N",
      \u0145: "N",
      \u0147: "N",
      \u014A: "N",
      \u0144: "n",
      \u0146: "n",
      \u0148: "n",
      \u014B: "n",
      \u014C: "O",
      \u014E: "O",
      \u0150: "O",
      \u014D: "o",
      \u014F: "o",
      \u0151: "o",
      \u0154: "R",
      \u0156: "R",
      \u0158: "R",
      \u0155: "r",
      \u0157: "r",
      \u0159: "r",
      \u015A: "S",
      \u015C: "S",
      \u015E: "S",
      \u0160: "S",
      \u015B: "s",
      \u015D: "s",
      \u015F: "s",
      \u0161: "s",
      \u0162: "T",
      \u0164: "T",
      \u0166: "T",
      \u0163: "t",
      \u0165: "t",
      \u0167: "t",
      \u0168: "U",
      \u016A: "U",
      \u016C: "U",
      \u016E: "U",
      \u0170: "U",
      \u0172: "U",
      \u0169: "u",
      \u016B: "u",
      \u016D: "u",
      \u016F: "u",
      \u0171: "u",
      \u0173: "u",
      \u0174: "W",
      \u0175: "w",
      \u0176: "Y",
      \u0177: "y",
      \u0178: "Y",
      \u0179: "Z",
      \u017B: "Z",
      \u017D: "Z",
      \u017A: "z",
      \u017C: "z",
      \u017E: "z",
      \u0132: "IJ",
      \u0133: "ij",
      \u0152: "Oe",
      \u0153: "oe",
      \u0149: "'n",
      \u017F: "s"
    }, Ih = {
      "&": "&amp;",
      "<": "&lt;",
      ">": "&gt;",
      '"': "&quot;",
      "'": "&#39;"
    }, Hh = {
      "&amp;": "&",
      "&lt;": "<",
      "&gt;": ">",
      "&quot;": '"',
      "&#39;": "'"
    }, Ph = {
      "\\": "\\",
      "'": "'",
      "\n": "n",
      "\r": "r",
      "\u2028": "u2028",
      "\u2029": "u2029"
    }, jh = parseFloat, Xh = parseInt, is = typeof Nr == "object" && Nr && Nr.Object === Object && Nr, Lh = typeof self == "object" && self && self.Object === Object && self, ve = is || Lh || Function("return this")(), wi = i && !i.nodeType && i, an = wi && !0 && n && !n.nodeType && n, os = an && an.exports === wi, Zi = os && is.process, Le = function() {
      try {
        var T = an && an.require && an.require("util").types;
        return T || Zi && Zi.binding && Zi.binding("util");
      } catch {
      }
    }(), ls = Le && Le.isArrayBuffer, ss = Le && Le.isDate, cs = Le && Le.isMap, us = Le && Le.isRegExp, ds = Le && Le.isSet, fs = Le && Le.isTypedArray;
    function Be(T, N, R) {
      switch (R.length) {
        case 0:
          return T.call(N);
        case 1:
          return T.call(N, R[0]);
        case 2:
          return T.call(N, R[0], R[1]);
        case 3:
          return T.call(N, R[0], R[1], R[2]);
      }
      return T.apply(N, R);
    }
    function _h(T, N, R, W) {
      for (var D = -1, L = T == null ? 0 : T.length; ++D < L; ) {
        var he = T[D];
        N(W, he, R(he), T);
      }
      return W;
    }
    function _e(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W && N(T[R], R, T) !== !1; )
        ;
      return T;
    }
    function qh(T, N) {
      for (var R = T == null ? 0 : T.length; R-- && N(T[R], R, T) !== !1; )
        ;
      return T;
    }
    function hs(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W; )
        if (!N(T[R], R, T))
          return !1;
      return !0;
    }
    function Ot(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length, D = 0, L = []; ++R < W; ) {
        var he = T[R];
        N(he, R, T) && (L[D++] = he);
      }
      return L;
    }
    function Lr(T, N) {
      var R = T == null ? 0 : T.length;
      return !!R && wn(T, N, 0) > -1;
    }
    function Ji(T, N, R) {
      for (var W = -1, D = T == null ? 0 : T.length; ++W < D; )
        if (R(N, T[W]))
          return !0;
      return !1;
    }
    function ie(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length, D = Array(W); ++R < W; )
        D[R] = N(T[R], R, T);
      return D;
    }
    function Gt(T, N) {
      for (var R = -1, W = N.length, D = T.length; ++R < W; )
        T[D + R] = N[R];
      return T;
    }
    function gi(T, N, R, W) {
      var D = -1, L = T == null ? 0 : T.length;
      for (W && L && (R = T[++D]); ++D < L; )
        R = N(R, T[D], D, T);
      return R;
    }
    function Kh(T, N, R, W) {
      var D = T == null ? 0 : T.length;
      for (W && D && (R = T[--D]); D--; )
        R = N(R, T[D], D, T);
      return R;
    }
    function ki(T, N) {
      for (var R = -1, W = T == null ? 0 : T.length; ++R < W; )
        if (N(T[R], R, T))
          return !0;
      return !1;
    }
    var $h = yi("length");
    function ep(T) {
      return T.split("");
    }
    function tp(T) {
      return T.match(ch) || [];
    }
    function ps(T, N, R) {
      var W;
      return R(T, function(D, L, he) {
        if (N(D, L, he))
          return W = L, !1;
      }), W;
    }
    function _r(T, N, R, W) {
      for (var D = T.length, L = R + (W ? 1 : -1); W ? L-- : ++L < D; )
        if (N(T[L], L, T))
          return L;
      return -1;
    }
    function wn(T, N, R) {
      return N === N ? hp(T, N, R) : _r(T, ms, R);
    }
    function np(T, N, R, W) {
      for (var D = R - 1, L = T.length; ++D < L; )
        if (W(T[D], N))
          return D;
      return -1;
    }
    function ms(T) {
      return T !== T;
    }
    function Vs(T, N) {
      var R = T == null ? 0 : T.length;
      return R ? Wi(T, N) / R : Cr;
    }
    function yi(T) {
      return function(N) {
        return N == null ? a : N[T];
      };
    }
    function bi(T) {
      return function(N) {
        return T == null ? a : T[N];
      };
    }
    function Ts(T, N, R, W, D) {
      return D(T, function(L, he, te) {
        R = W ? (W = !1, L) : N(R, L, he, te);
      }), R;
    }
    function rp(T, N) {
      var R = T.length;
      for (T.sort(N); R--; )
        T[R] = T[R].value;
      return T;
    }
    function Wi(T, N) {
      for (var R, W = -1, D = T.length; ++W < D; ) {
        var L = N(T[W]);
        L !== a && (R = R === a ? L : R + L);
      }
      return R;
    }
    function Ei(T, N) {
      for (var R = -1, W = Array(T); ++R < T; )
        W[R] = N(R);
      return W;
    }
    function ap(T, N) {
      return ie(N, function(R) {
        return [R, T[R]];
      });
    }
    function Us(T) {
      return T && T.slice(0, Ms(T) + 1).replace(Ti, "");
    }
    function De(T) {
      return function(N) {
        return T(N);
      };
    }
    function Fi(T, N) {
      return ie(N, function(R) {
        return T[R];
      });
    }
    function or(T, N) {
      return T.has(N);
    }
    function Rs(T, N) {
      for (var R = -1, W = T.length; ++R < W && wn(N, T[R], 0) > -1; )
        ;
      return R;
    }
    function vs(T, N) {
      for (var R = T.length; R-- && wn(N, T[R], 0) > -1; )
        ;
      return R;
    }
    function ip(T, N) {
      for (var R = T.length, W = 0; R--; )
        T[R] === N && ++W;
      return W;
    }
    var op = bi(Yh), lp = bi(Ih);
    function sp(T) {
      return "\\" + Ph[T];
    }
    function cp(T, N) {
      return T == null ? a : T[N];
    }
    function Zn(T) {
      return Oh.test(T);
    }
    function up(T) {
      return Gh.test(T);
    }
    function dp(T) {
      for (var N, R = []; !(N = T.next()).done; )
        R.push(N.value);
      return R;
    }
    function Si(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W, D) {
        R[++N] = [D, W];
      }), R;
    }
    function Ns(T, N) {
      return function(R) {
        return T(N(R));
      };
    }
    function Ct(T, N) {
      for (var R = -1, W = T.length, D = 0, L = []; ++R < W; ) {
        var he = T[R];
        (he === N || he === J) && (T[R] = J, L[D++] = R);
      }
      return L;
    }
    function qr(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W) {
        R[++N] = W;
      }), R;
    }
    function fp(T) {
      var N = -1, R = Array(T.size);
      return T.forEach(function(W) {
        R[++N] = [W, W];
      }), R;
    }
    function hp(T, N, R) {
      for (var W = R - 1, D = T.length; ++W < D; )
        if (T[W] === N)
          return W;
      return -1;
    }
    function pp(T, N, R) {
      for (var W = R + 1; W--; )
        if (T[W] === N)
          return W;
      return W;
    }
    function Jn(T) {
      return Zn(T) ? Vp(T) : $h(T);
    }
    function lt(T) {
      return Zn(T) ? Tp(T) : ep(T);
    }
    function Ms(T) {
      for (var N = T.length; N-- && ih.test(T.charAt(N)); )
        ;
      return N;
    }
    var mp = bi(Hh);
    function Vp(T) {
      for (var N = Mi.lastIndex = 0; Mi.test(T); )
        ++N;
      return N;
    }
    function Tp(T) {
      return T.match(Mi) || [];
    }
    function Up(T) {
      return T.match(Dh) || [];
    }
    var Rp = function T(N) {
      N = N == null ? ve : gn.defaults(ve.Object(), N, gn.pick(ve, Ch));
      var R = N.Array, W = N.Date, D = N.Error, L = N.Function, he = N.Math, te = N.Object, Ai = N.RegExp, vp = N.String, qe = N.TypeError, Kr = R.prototype, Np = L.prototype, kn = te.prototype, $r = N["__core-js_shared__"], ea = Np.toString, K = kn.hasOwnProperty, Mp = 0, ws = function() {
        var e = /[^.]+$/.exec($r && $r.keys && $r.keys.IE_PROTO || "");
        return e ? "Symbol(src)_1." + e : "";
      }(), ta = kn.toString, wp = ea.call(te), Zp = ve._, Jp = Ai(
        "^" + ea.call(K).replace(Vi, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"
      ), na = os ? N.Buffer : a, zt = N.Symbol, ra = N.Uint8Array, Zs = na ? na.allocUnsafe : a, aa = Ns(te.getPrototypeOf, te), Js = te.create, gs = kn.propertyIsEnumerable, ia = Kr.splice, ks = zt ? zt.isConcatSpreadable : a, lr = zt ? zt.iterator : a, on = zt ? zt.toStringTag : a, oa = function() {
        try {
          var e = dn(te, "defineProperty");
          return e({}, "", {}), e;
        } catch {
        }
      }(), gp = N.clearTimeout !== ve.clearTimeout && N.clearTimeout, kp = W && W.now !== ve.Date.now && W.now, yp = N.setTimeout !== ve.setTimeout && N.setTimeout, la = he.ceil, sa = he.floor, xi = te.getOwnPropertySymbols, bp = na ? na.isBuffer : a, ys = N.isFinite, Wp = Kr.join, Ep = Ns(te.keys, te), pe = he.max, we = he.min, Fp = W.now, Sp = N.parseInt, bs = he.random, Ap = Kr.reverse, Qi = dn(N, "DataView"), sr = dn(N, "Map"), Bi = dn(N, "Promise"), yn = dn(N, "Set"), cr = dn(N, "WeakMap"), ur = dn(te, "create"), ca = cr && new cr(), bn = {}, xp = fn(Qi), Qp = fn(sr), Bp = fn(Bi), Dp = fn(yn), Op = fn(cr), ua = zt ? zt.prototype : a, dr = ua ? ua.valueOf : a, Ws = ua ? ua.toString : a;
      function u(e) {
        if (se(e) && !C(e) && !(e instanceof j)) {
          if (e instanceof Ke)
            return e;
          if (K.call(e, "__wrapped__"))
            return Ec(e);
        }
        return new Ke(e);
      }
      var Wn = function() {
        function e() {
        }
        return function(t) {
          if (!le(t))
            return {};
          if (Js)
            return Js(t);
          e.prototype = t;
          var r = new e();
          return e.prototype = a, r;
        };
      }();
      function da() {
      }
      function Ke(e, t) {
        this.__wrapped__ = e, this.__actions__ = [], this.__chain__ = !!t, this.__index__ = 0, this.__values__ = a;
      }
      u.templateSettings = {
        escape: $f,
        evaluate: eh,
        interpolate: Ol,
        variable: "",
        imports: {
          _: u
        }
      }, u.prototype = da.prototype, u.prototype.constructor = u, Ke.prototype = Wn(da.prototype), Ke.prototype.constructor = Ke;
      function j(e) {
        this.__wrapped__ = e, this.__actions__ = [], this.__dir__ = 1, this.__filtered__ = !1, this.__iteratees__ = [], this.__takeCount__ = mt, this.__views__ = [];
      }
      function Gp() {
        var e = new j(this.__wrapped__);
        return e.__actions__ = Fe(this.__actions__), e.__dir__ = this.__dir__, e.__filtered__ = this.__filtered__, e.__iteratees__ = Fe(this.__iteratees__), e.__takeCount__ = this.__takeCount__, e.__views__ = Fe(this.__views__), e;
      }
      function Cp() {
        if (this.__filtered__) {
          var e = new j(this);
          e.__dir__ = -1, e.__filtered__ = !0;
        } else
          e = this.clone(), e.__dir__ *= -1;
        return e;
      }
      function zp() {
        var e = this.__wrapped__.value(), t = this.__dir__, r = C(e), o = t < 0, s = r ? e.length : 0, d = eV(0, s, this.__views__), h = d.start, m = d.end, U = m - h, M = o ? m : h - 1, w = this.__iteratees__, Z = w.length, b = 0, E = we(U, this.__takeCount__);
        if (!r || !o && s == U && E == U)
          return ec(e, this.__actions__);
        var x = [];
        e:
          for (; U-- && b < E; ) {
            M += t;
            for (var Y = -1, Q = e[M]; ++Y < Z; ) {
              var P = w[Y], X = P.iteratee, Ce = P.type, ye = X(Q);
              if (Ce == Re)
                Q = ye;
              else if (!ye) {
                if (Ce == je)
                  continue e;
                break e;
              }
            }
            x[b++] = Q;
          }
        return x;
      }
      j.prototype = Wn(da.prototype), j.prototype.constructor = j;
      function ln(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function Yp() {
        this.__data__ = ur ? ur(null) : {}, this.size = 0;
      }
      function Ip(e) {
        var t = this.has(e) && delete this.__data__[e];
        return this.size -= t ? 1 : 0, t;
      }
      function Hp(e) {
        var t = this.__data__;
        if (ur) {
          var r = t[e];
          return r === v ? a : r;
        }
        return K.call(t, e) ? t[e] : a;
      }
      function Pp(e) {
        var t = this.__data__;
        return ur ? t[e] !== a : K.call(t, e);
      }
      function jp(e, t) {
        var r = this.__data__;
        return this.size += this.has(e) ? 0 : 1, r[e] = ur && t === a ? v : t, this;
      }
      ln.prototype.clear = Yp, ln.prototype.delete = Ip, ln.prototype.get = Hp, ln.prototype.has = Pp, ln.prototype.set = jp;
      function Zt(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function Xp() {
        this.__data__ = [], this.size = 0;
      }
      function Lp(e) {
        var t = this.__data__, r = fa(t, e);
        if (r < 0)
          return !1;
        var o = t.length - 1;
        return r == o ? t.pop() : ia.call(t, r, 1), --this.size, !0;
      }
      function _p(e) {
        var t = this.__data__, r = fa(t, e);
        return r < 0 ? a : t[r][1];
      }
      function qp(e) {
        return fa(this.__data__, e) > -1;
      }
      function Kp(e, t) {
        var r = this.__data__, o = fa(r, e);
        return o < 0 ? (++this.size, r.push([e, t])) : r[o][1] = t, this;
      }
      Zt.prototype.clear = Xp, Zt.prototype.delete = Lp, Zt.prototype.get = _p, Zt.prototype.has = qp, Zt.prototype.set = Kp;
      function Jt(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.clear(); ++t < r; ) {
          var o = e[t];
          this.set(o[0], o[1]);
        }
      }
      function $p() {
        this.size = 0, this.__data__ = {
          hash: new ln(),
          map: new (sr || Zt)(),
          string: new ln()
        };
      }
      function em(e) {
        var t = Za(this, e).delete(e);
        return this.size -= t ? 1 : 0, t;
      }
      function tm(e) {
        return Za(this, e).get(e);
      }
      function nm(e) {
        return Za(this, e).has(e);
      }
      function rm(e, t) {
        var r = Za(this, e), o = r.size;
        return r.set(e, t), this.size += r.size == o ? 0 : 1, this;
      }
      Jt.prototype.clear = $p, Jt.prototype.delete = em, Jt.prototype.get = tm, Jt.prototype.has = nm, Jt.prototype.set = rm;
      function sn(e) {
        var t = -1, r = e == null ? 0 : e.length;
        for (this.__data__ = new Jt(); ++t < r; )
          this.add(e[t]);
      }
      function am(e) {
        return this.__data__.set(e, v), this;
      }
      function im(e) {
        return this.__data__.has(e);
      }
      sn.prototype.add = sn.prototype.push = am, sn.prototype.has = im;
      function st(e) {
        var t = this.__data__ = new Zt(e);
        this.size = t.size;
      }
      function om() {
        this.__data__ = new Zt(), this.size = 0;
      }
      function lm(e) {
        var t = this.__data__, r = t.delete(e);
        return this.size = t.size, r;
      }
      function sm(e) {
        return this.__data__.get(e);
      }
      function cm(e) {
        return this.__data__.has(e);
      }
      function um(e, t) {
        var r = this.__data__;
        if (r instanceof Zt) {
          var o = r.__data__;
          if (!sr || o.length < c - 1)
            return o.push([e, t]), this.size = ++r.size, this;
          r = this.__data__ = new Jt(o);
        }
        return r.set(e, t), this.size = r.size, this;
      }
      st.prototype.clear = om, st.prototype.delete = lm, st.prototype.get = sm, st.prototype.has = cm, st.prototype.set = um;
      function Es(e, t) {
        var r = C(e), o = !r && hn(e), s = !r && !o && jt(e), d = !r && !o && !s && An(e), h = r || o || s || d, m = h ? Ei(e.length, vp) : [], U = m.length;
        for (var M in e)
          (t || K.call(e, M)) && !(h && (M == "length" || s && (M == "offset" || M == "parent") || d && (M == "buffer" || M == "byteLength" || M == "byteOffset") || bt(M, U))) && m.push(M);
        return m;
      }
      function Fs(e) {
        var t = e.length;
        return t ? e[Xi(0, t - 1)] : a;
      }
      function dm(e, t) {
        return Ja(Fe(e), cn(t, 0, e.length));
      }
      function fm(e) {
        return Ja(Fe(e));
      }
      function Di(e, t, r) {
        (r !== a && !ct(e[t], r) || r === a && !(t in e)) && gt(e, t, r);
      }
      function fr(e, t, r) {
        var o = e[t];
        (!(K.call(e, t) && ct(o, r)) || r === a && !(t in e)) && gt(e, t, r);
      }
      function fa(e, t) {
        for (var r = e.length; r--; )
          if (ct(e[r][0], t))
            return r;
        return -1;
      }
      function hm(e, t, r, o) {
        return Yt(e, function(s, d, h) {
          t(o, s, r(s), h);
        }), o;
      }
      function Ss(e, t) {
        return e && Tt(t, me(t), e);
      }
      function pm(e, t) {
        return e && Tt(t, Ae(t), e);
      }
      function gt(e, t, r) {
        t == "__proto__" && oa ? oa(e, t, {
          configurable: !0,
          enumerable: !0,
          value: r,
          writable: !0
        }) : e[t] = r;
      }
      function Oi(e, t) {
        for (var r = -1, o = t.length, s = R(o), d = e == null; ++r < o; )
          s[r] = d ? a : vo(e, t[r]);
        return s;
      }
      function cn(e, t, r) {
        return e === e && (r !== a && (e = e <= r ? e : r), t !== a && (e = e >= t ? e : t)), e;
      }
      function $e(e, t, r, o, s, d) {
        var h, m = t & y, U = t & g, M = t & H;
        if (r && (h = s ? r(e, o, s, d) : r(e)), h !== a)
          return h;
        if (!le(e))
          return e;
        var w = C(e);
        if (w) {
          if (h = nV(e), !m)
            return Fe(e, h);
        } else {
          var Z = Ze(e), b = Z == Ir || Z == xl;
          if (jt(e))
            return rc(e, m);
          if (Z == wt || Z == vn || b && !s) {
            if (h = U || b ? {} : Mc(e), !m)
              return U ? Hm(e, pm(h, e)) : Im(e, Ss(h, e));
          } else {
            if (!re[Z])
              return s ? e : {};
            h = rV(e, Z, m);
          }
        }
        d || (d = new st());
        var E = d.get(e);
        if (E)
          return E;
        d.set(e, h), qc(e) ? e.forEach(function(Q) {
          h.add($e(Q, t, r, Q, e, d));
        }) : Lc(e) && e.forEach(function(Q, P) {
          h.set(P, $e(Q, t, r, P, e, d));
        });
        var x = M ? U ? io : ao : U ? Ae : me, Y = w ? a : x(e);
        return _e(Y || e, function(Q, P) {
          Y && (P = Q, Q = e[P]), fr(h, P, $e(Q, t, r, P, e, d));
        }), h;
      }
      function mm(e) {
        var t = me(e);
        return function(r) {
          return As(r, e, t);
        };
      }
      function As(e, t, r) {
        var o = r.length;
        if (e == null)
          return !o;
        for (e = te(e); o--; ) {
          var s = r[o], d = t[s], h = e[s];
          if (h === a && !(s in e) || !d(h))
            return !1;
        }
        return !0;
      }
      function xs(e, t, r) {
        if (typeof e != "function")
          throw new qe(p);
        return Rr(function() {
          e.apply(a, r);
        }, t);
      }
      function hr(e, t, r, o) {
        var s = -1, d = Lr, h = !0, m = e.length, U = [], M = t.length;
        if (!m)
          return U;
        r && (t = ie(t, De(r))), o ? (d = Ji, h = !1) : t.length >= c && (d = or, h = !1, t = new sn(t));
        e:
          for (; ++s < m; ) {
            var w = e[s], Z = r == null ? w : r(w);
            if (w = o || w !== 0 ? w : 0, h && Z === Z) {
              for (var b = M; b--; )
                if (t[b] === Z)
                  continue e;
              U.push(w);
            } else
              d(t, Z, o) || U.push(w);
          }
        return U;
      }
      var Yt = sc(Vt), Qs = sc(Ci, !0);
      function Vm(e, t) {
        var r = !0;
        return Yt(e, function(o, s, d) {
          return r = !!t(o, s, d), r;
        }), r;
      }
      function ha(e, t, r) {
        for (var o = -1, s = e.length; ++o < s; ) {
          var d = e[o], h = t(d);
          if (h != null && (m === a ? h === h && !Ge(h) : r(h, m)))
            var m = h, U = d;
        }
        return U;
      }
      function Tm(e, t, r, o) {
        var s = e.length;
        for (r = z(r), r < 0 && (r = -r > s ? 0 : s + r), o = o === a || o > s ? s : z(o), o < 0 && (o += s), o = r > o ? 0 : $c(o); r < o; )
          e[r++] = t;
        return e;
      }
      function Bs(e, t) {
        var r = [];
        return Yt(e, function(o, s, d) {
          t(o, s, d) && r.push(o);
        }), r;
      }
      function Ne(e, t, r, o, s) {
        var d = -1, h = e.length;
        for (r || (r = iV), s || (s = []); ++d < h; ) {
          var m = e[d];
          t > 0 && r(m) ? t > 1 ? Ne(m, t - 1, r, o, s) : Gt(s, m) : o || (s[s.length] = m);
        }
        return s;
      }
      var Gi = cc(), Ds = cc(!0);
      function Vt(e, t) {
        return e && Gi(e, t, me);
      }
      function Ci(e, t) {
        return e && Ds(e, t, me);
      }
      function pa(e, t) {
        return Ot(t, function(r) {
          return Wt(e[r]);
        });
      }
      function un(e, t) {
        t = Ht(t, e);
        for (var r = 0, o = t.length; e != null && r < o; )
          e = e[Ut(t[r++])];
        return r && r == o ? e : a;
      }
      function Os(e, t, r) {
        var o = t(e);
        return C(e) ? o : Gt(o, r(e));
      }
      function ge(e) {
        return e == null ? e === a ? Pf : If : on && on in te(e) ? $m(e) : fV(e);
      }
      function zi(e, t) {
        return e > t;
      }
      function Um(e, t) {
        return e != null && K.call(e, t);
      }
      function Rm(e, t) {
        return e != null && t in te(e);
      }
      function vm(e, t, r) {
        return e >= we(t, r) && e < pe(t, r);
      }
      function Yi(e, t, r) {
        for (var o = r ? Ji : Lr, s = e[0].length, d = e.length, h = d, m = R(d), U = 1 / 0, M = []; h--; ) {
          var w = e[h];
          h && t && (w = ie(w, De(t))), U = we(w.length, U), m[h] = !r && (t || s >= 120 && w.length >= 120) ? new sn(h && w) : a;
        }
        w = e[0];
        var Z = -1, b = m[0];
        e:
          for (; ++Z < s && M.length < U; ) {
            var E = w[Z], x = t ? t(E) : E;
            if (E = r || E !== 0 ? E : 0, !(b ? or(b, x) : o(M, x, r))) {
              for (h = d; --h; ) {
                var Y = m[h];
                if (!(Y ? or(Y, x) : o(e[h], x, r)))
                  continue e;
              }
              b && b.push(x), M.push(E);
            }
          }
        return M;
      }
      function Nm(e, t, r, o) {
        return Vt(e, function(s, d, h) {
          t(o, r(s), d, h);
        }), o;
      }
      function pr(e, t, r) {
        t = Ht(t, e), e = gc(e, t);
        var o = e == null ? e : e[Ut(tt(t))];
        return o == null ? a : Be(o, e, r);
      }
      function Gs(e) {
        return se(e) && ge(e) == vn;
      }
      function Mm(e) {
        return se(e) && ge(e) == ir;
      }
      function wm(e) {
        return se(e) && ge(e) == er;
      }
      function mr(e, t, r, o, s) {
        return e === t ? !0 : e == null || t == null || !se(e) && !se(t) ? e !== e && t !== t : Zm(e, t, r, o, mr, s);
      }
      function Zm(e, t, r, o, s, d) {
        var h = C(e), m = C(t), U = h ? zr : Ze(e), M = m ? zr : Ze(t);
        U = U == vn ? wt : U, M = M == vn ? wt : M;
        var w = U == wt, Z = M == wt, b = U == M;
        if (b && jt(e)) {
          if (!jt(t))
            return !1;
          h = !0, w = !1;
        }
        if (b && !w)
          return d || (d = new st()), h || An(e) ? Rc(e, t, r, o, s, d) : qm(e, t, U, r, o, s, d);
        if (!(r & S)) {
          var E = w && K.call(e, "__wrapped__"), x = Z && K.call(t, "__wrapped__");
          if (E || x) {
            var Y = E ? e.value() : e, Q = x ? t.value() : t;
            return d || (d = new st()), s(Y, Q, r, o, d);
          }
        }
        return b ? (d || (d = new st()), Km(e, t, r, o, s, d)) : !1;
      }
      function Jm(e) {
        return se(e) && Ze(e) == it;
      }
      function Ii(e, t, r, o) {
        var s = r.length, d = s, h = !o;
        if (e == null)
          return !d;
        for (e = te(e); s--; ) {
          var m = r[s];
          if (h && m[2] ? m[1] !== e[m[0]] : !(m[0] in e))
            return !1;
        }
        for (; ++s < d; ) {
          m = r[s];
          var U = m[0], M = e[U], w = m[1];
          if (h && m[2]) {
            if (M === a && !(U in e))
              return !1;
          } else {
            var Z = new st();
            if (o)
              var b = o(M, w, U, e, t, Z);
            if (!(b === a ? mr(w, M, S | O, o, Z) : b))
              return !1;
          }
        }
        return !0;
      }
      function Cs(e) {
        if (!le(e) || lV(e))
          return !1;
        var t = Wt(e) ? Jp : mh;
        return t.test(fn(e));
      }
      function gm(e) {
        return se(e) && ge(e) == nr;
      }
      function km(e) {
        return se(e) && Ze(e) == ot;
      }
      function ym(e) {
        return se(e) && Ea(e.length) && !!ae[ge(e)];
      }
      function zs(e) {
        return typeof e == "function" ? e : e == null ? xe : typeof e == "object" ? C(e) ? Hs(e[0], e[1]) : Is(e) : uu(e);
      }
      function Hi(e) {
        if (!Ur(e))
          return Ep(e);
        var t = [];
        for (var r in te(e))
          K.call(e, r) && r != "constructor" && t.push(r);
        return t;
      }
      function bm(e) {
        if (!le(e))
          return dV(e);
        var t = Ur(e), r = [];
        for (var o in e)
          o == "constructor" && (t || !K.call(e, o)) || r.push(o);
        return r;
      }
      function Pi(e, t) {
        return e < t;
      }
      function Ys(e, t) {
        var r = -1, o = Se(e) ? R(e.length) : [];
        return Yt(e, function(s, d, h) {
          o[++r] = t(s, d, h);
        }), o;
      }
      function Is(e) {
        var t = lo(e);
        return t.length == 1 && t[0][2] ? Zc(t[0][0], t[0][1]) : function(r) {
          return r === e || Ii(r, e, t);
        };
      }
      function Hs(e, t) {
        return co(e) && wc(t) ? Zc(Ut(e), t) : function(r) {
          var o = vo(r, e);
          return o === a && o === t ? No(r, e) : mr(t, o, S | O);
        };
      }
      function ma(e, t, r, o, s) {
        e !== t && Gi(t, function(d, h) {
          if (s || (s = new st()), le(d))
            Wm(e, t, h, r, ma, o, s);
          else {
            var m = o ? o(fo(e, h), d, h + "", e, t, s) : a;
            m === a && (m = d), Di(e, h, m);
          }
        }, Ae);
      }
      function Wm(e, t, r, o, s, d, h) {
        var m = fo(e, r), U = fo(t, r), M = h.get(U);
        if (M) {
          Di(e, r, M);
          return;
        }
        var w = d ? d(m, U, r + "", e, t, h) : a, Z = w === a;
        if (Z) {
          var b = C(U), E = !b && jt(U), x = !b && !E && An(U);
          w = U, b || E || x ? C(m) ? w = m : ce(m) ? w = Fe(m) : E ? (Z = !1, w = rc(U, !0)) : x ? (Z = !1, w = ac(U, !0)) : w = [] : vr(U) || hn(U) ? (w = m, hn(m) ? w = eu(m) : (!le(m) || Wt(m)) && (w = Mc(U))) : Z = !1;
        }
        Z && (h.set(U, w), s(w, U, o, d, h), h.delete(U)), Di(e, r, w);
      }
      function Ps(e, t) {
        var r = e.length;
        if (!!r)
          return t += t < 0 ? r : 0, bt(t, r) ? e[t] : a;
      }
      function js(e, t, r) {
        t.length ? t = ie(t, function(d) {
          return C(d) ? function(h) {
            return un(h, d.length === 1 ? d[0] : d);
          } : d;
        }) : t = [xe];
        var o = -1;
        t = ie(t, De(A()));
        var s = Ys(e, function(d, h, m) {
          var U = ie(t, function(M) {
            return M(d);
          });
          return { criteria: U, index: ++o, value: d };
        });
        return rp(s, function(d, h) {
          return Ym(d, h, r);
        });
      }
      function Em(e, t) {
        return Xs(e, t, function(r, o) {
          return No(e, o);
        });
      }
      function Xs(e, t, r) {
        for (var o = -1, s = t.length, d = {}; ++o < s; ) {
          var h = t[o], m = un(e, h);
          r(m, h) && Vr(d, Ht(h, e), m);
        }
        return d;
      }
      function Fm(e) {
        return function(t) {
          return un(t, e);
        };
      }
      function ji(e, t, r, o) {
        var s = o ? np : wn, d = -1, h = t.length, m = e;
        for (e === t && (t = Fe(t)), r && (m = ie(e, De(r))); ++d < h; )
          for (var U = 0, M = t[d], w = r ? r(M) : M; (U = s(m, w, U, o)) > -1; )
            m !== e && ia.call(m, U, 1), ia.call(e, U, 1);
        return e;
      }
      function Ls(e, t) {
        for (var r = e ? t.length : 0, o = r - 1; r--; ) {
          var s = t[r];
          if (r == o || s !== d) {
            var d = s;
            bt(s) ? ia.call(e, s, 1) : qi(e, s);
          }
        }
        return e;
      }
      function Xi(e, t) {
        return e + sa(bs() * (t - e + 1));
      }
      function Sm(e, t, r, o) {
        for (var s = -1, d = pe(la((t - e) / (r || 1)), 0), h = R(d); d--; )
          h[o ? d : ++s] = e, e += r;
        return h;
      }
      function Li(e, t) {
        var r = "";
        if (!e || t < 1 || t > Dt)
          return r;
        do
          t % 2 && (r += e), t = sa(t / 2), t && (e += e);
        while (t);
        return r;
      }
      function I(e, t) {
        return ho(Jc(e, t, xe), e + "");
      }
      function Am(e) {
        return Fs(xn(e));
      }
      function xm(e, t) {
        var r = xn(e);
        return Ja(r, cn(t, 0, r.length));
      }
      function Vr(e, t, r, o) {
        if (!le(e))
          return e;
        t = Ht(t, e);
        for (var s = -1, d = t.length, h = d - 1, m = e; m != null && ++s < d; ) {
          var U = Ut(t[s]), M = r;
          if (U === "__proto__" || U === "constructor" || U === "prototype")
            return e;
          if (s != h) {
            var w = m[U];
            M = o ? o(w, U, m) : a, M === a && (M = le(w) ? w : bt(t[s + 1]) ? [] : {});
          }
          fr(m, U, M), m = m[U];
        }
        return e;
      }
      var _s = ca ? function(e, t) {
        return ca.set(e, t), e;
      } : xe, Qm = oa ? function(e, t) {
        return oa(e, "toString", {
          configurable: !0,
          enumerable: !1,
          value: wo(t),
          writable: !0
        });
      } : xe;
      function Bm(e) {
        return Ja(xn(e));
      }
      function et(e, t, r) {
        var o = -1, s = e.length;
        t < 0 && (t = -t > s ? 0 : s + t), r = r > s ? s : r, r < 0 && (r += s), s = t > r ? 0 : r - t >>> 0, t >>>= 0;
        for (var d = R(s); ++o < s; )
          d[o] = e[o + t];
        return d;
      }
      function Dm(e, t) {
        var r;
        return Yt(e, function(o, s, d) {
          return r = t(o, s, d), !r;
        }), !!r;
      }
      function Va(e, t, r) {
        var o = 0, s = e == null ? o : e.length;
        if (typeof t == "number" && t === t && s <= Gf) {
          for (; o < s; ) {
            var d = o + s >>> 1, h = e[d];
            h !== null && !Ge(h) && (r ? h <= t : h < t) ? o = d + 1 : s = d;
          }
          return s;
        }
        return _i(e, t, xe, r);
      }
      function _i(e, t, r, o) {
        var s = 0, d = e == null ? 0 : e.length;
        if (d === 0)
          return 0;
        t = r(t);
        for (var h = t !== t, m = t === null, U = Ge(t), M = t === a; s < d; ) {
          var w = sa((s + d) / 2), Z = r(e[w]), b = Z !== a, E = Z === null, x = Z === Z, Y = Ge(Z);
          if (h)
            var Q = o || x;
          else
            M ? Q = x && (o || b) : m ? Q = x && b && (o || !E) : U ? Q = x && b && !E && (o || !Y) : E || Y ? Q = !1 : Q = o ? Z <= t : Z < t;
          Q ? s = w + 1 : d = w;
        }
        return we(d, Of);
      }
      function qs(e, t) {
        for (var r = -1, o = e.length, s = 0, d = []; ++r < o; ) {
          var h = e[r], m = t ? t(h) : h;
          if (!r || !ct(m, U)) {
            var U = m;
            d[s++] = h === 0 ? 0 : h;
          }
        }
        return d;
      }
      function Ks(e) {
        return typeof e == "number" ? e : Ge(e) ? Cr : +e;
      }
      function Oe(e) {
        if (typeof e == "string")
          return e;
        if (C(e))
          return ie(e, Oe) + "";
        if (Ge(e))
          return Ws ? Ws.call(e) : "";
        var t = e + "";
        return t == "0" && 1 / e == -Xe ? "-0" : t;
      }
      function It(e, t, r) {
        var o = -1, s = Lr, d = e.length, h = !0, m = [], U = m;
        if (r)
          h = !1, s = Ji;
        else if (d >= c) {
          var M = t ? null : Lm(e);
          if (M)
            return qr(M);
          h = !1, s = or, U = new sn();
        } else
          U = t ? [] : m;
        e:
          for (; ++o < d; ) {
            var w = e[o], Z = t ? t(w) : w;
            if (w = r || w !== 0 ? w : 0, h && Z === Z) {
              for (var b = U.length; b--; )
                if (U[b] === Z)
                  continue e;
              t && U.push(Z), m.push(w);
            } else
              s(U, Z, r) || (U !== m && U.push(Z), m.push(w));
          }
        return m;
      }
      function qi(e, t) {
        return t = Ht(t, e), e = gc(e, t), e == null || delete e[Ut(tt(t))];
      }
      function $s(e, t, r, o) {
        return Vr(e, t, r(un(e, t)), o);
      }
      function Ta(e, t, r, o) {
        for (var s = e.length, d = o ? s : -1; (o ? d-- : ++d < s) && t(e[d], d, e); )
          ;
        return r ? et(e, o ? 0 : d, o ? d + 1 : s) : et(e, o ? d + 1 : 0, o ? s : d);
      }
      function ec(e, t) {
        var r = e;
        return r instanceof j && (r = r.value()), gi(t, function(o, s) {
          return s.func.apply(s.thisArg, Gt([o], s.args));
        }, r);
      }
      function Ki(e, t, r) {
        var o = e.length;
        if (o < 2)
          return o ? It(e[0]) : [];
        for (var s = -1, d = R(o); ++s < o; )
          for (var h = e[s], m = -1; ++m < o; )
            m != s && (d[s] = hr(d[s] || h, e[m], t, r));
        return It(Ne(d, 1), t, r);
      }
      function tc(e, t, r) {
        for (var o = -1, s = e.length, d = t.length, h = {}; ++o < s; ) {
          var m = o < d ? t[o] : a;
          r(h, e[o], m);
        }
        return h;
      }
      function $i(e) {
        return ce(e) ? e : [];
      }
      function eo(e) {
        return typeof e == "function" ? e : xe;
      }
      function Ht(e, t) {
        return C(e) ? e : co(e, t) ? [e] : Wc(q(e));
      }
      var Om = I;
      function Pt(e, t, r) {
        var o = e.length;
        return r = r === a ? o : r, !t && r >= o ? e : et(e, t, r);
      }
      var nc = gp || function(e) {
        return ve.clearTimeout(e);
      };
      function rc(e, t) {
        if (t)
          return e.slice();
        var r = e.length, o = Zs ? Zs(r) : new e.constructor(r);
        return e.copy(o), o;
      }
      function to(e) {
        var t = new e.constructor(e.byteLength);
        return new ra(t).set(new ra(e)), t;
      }
      function Gm(e, t) {
        var r = t ? to(e.buffer) : e.buffer;
        return new e.constructor(r, e.byteOffset, e.byteLength);
      }
      function Cm(e) {
        var t = new e.constructor(e.source, Gl.exec(e));
        return t.lastIndex = e.lastIndex, t;
      }
      function zm(e) {
        return dr ? te(dr.call(e)) : {};
      }
      function ac(e, t) {
        var r = t ? to(e.buffer) : e.buffer;
        return new e.constructor(r, e.byteOffset, e.length);
      }
      function ic(e, t) {
        if (e !== t) {
          var r = e !== a, o = e === null, s = e === e, d = Ge(e), h = t !== a, m = t === null, U = t === t, M = Ge(t);
          if (!m && !M && !d && e > t || d && h && U && !m && !M || o && h && U || !r && U || !s)
            return 1;
          if (!o && !d && !M && e < t || M && r && s && !o && !d || m && r && s || !h && s || !U)
            return -1;
        }
        return 0;
      }
      function Ym(e, t, r) {
        for (var o = -1, s = e.criteria, d = t.criteria, h = s.length, m = r.length; ++o < h; ) {
          var U = ic(s[o], d[o]);
          if (U) {
            if (o >= m)
              return U;
            var M = r[o];
            return U * (M == "desc" ? -1 : 1);
          }
        }
        return e.index - t.index;
      }
      function oc(e, t, r, o) {
        for (var s = -1, d = e.length, h = r.length, m = -1, U = t.length, M = pe(d - h, 0), w = R(U + M), Z = !o; ++m < U; )
          w[m] = t[m];
        for (; ++s < h; )
          (Z || s < d) && (w[r[s]] = e[s]);
        for (; M--; )
          w[m++] = e[s++];
        return w;
      }
      function lc(e, t, r, o) {
        for (var s = -1, d = e.length, h = -1, m = r.length, U = -1, M = t.length, w = pe(d - m, 0), Z = R(w + M), b = !o; ++s < w; )
          Z[s] = e[s];
        for (var E = s; ++U < M; )
          Z[E + U] = t[U];
        for (; ++h < m; )
          (b || s < d) && (Z[E + r[h]] = e[s++]);
        return Z;
      }
      function Fe(e, t) {
        var r = -1, o = e.length;
        for (t || (t = R(o)); ++r < o; )
          t[r] = e[r];
        return t;
      }
      function Tt(e, t, r, o) {
        var s = !r;
        r || (r = {});
        for (var d = -1, h = t.length; ++d < h; ) {
          var m = t[d], U = o ? o(r[m], e[m], m, r, e) : a;
          U === a && (U = e[m]), s ? gt(r, m, U) : fr(r, m, U);
        }
        return r;
      }
      function Im(e, t) {
        return Tt(e, so(e), t);
      }
      function Hm(e, t) {
        return Tt(e, vc(e), t);
      }
      function Ua(e, t) {
        return function(r, o) {
          var s = C(r) ? _h : hm, d = t ? t() : {};
          return s(r, e, A(o, 2), d);
        };
      }
      function En(e) {
        return I(function(t, r) {
          var o = -1, s = r.length, d = s > 1 ? r[s - 1] : a, h = s > 2 ? r[2] : a;
          for (d = e.length > 3 && typeof d == "function" ? (s--, d) : a, h && ke(r[0], r[1], h) && (d = s < 3 ? a : d, s = 1), t = te(t); ++o < s; ) {
            var m = r[o];
            m && e(t, m, o, d);
          }
          return t;
        });
      }
      function sc(e, t) {
        return function(r, o) {
          if (r == null)
            return r;
          if (!Se(r))
            return e(r, o);
          for (var s = r.length, d = t ? s : -1, h = te(r); (t ? d-- : ++d < s) && o(h[d], d, h) !== !1; )
            ;
          return r;
        };
      }
      function cc(e) {
        return function(t, r, o) {
          for (var s = -1, d = te(t), h = o(t), m = h.length; m--; ) {
            var U = h[e ? m : ++s];
            if (r(d[U], U, d) === !1)
              break;
          }
          return t;
        };
      }
      function Pm(e, t, r) {
        var o = t & G, s = Tr(e);
        function d() {
          var h = this && this !== ve && this instanceof d ? s : e;
          return h.apply(o ? r : this, arguments);
        }
        return d;
      }
      function uc(e) {
        return function(t) {
          t = q(t);
          var r = Zn(t) ? lt(t) : a, o = r ? r[0] : t.charAt(0), s = r ? Pt(r, 1).join("") : t.slice(1);
          return o[e]() + s;
        };
      }
      function Fn(e) {
        return function(t) {
          return gi(su(lu(t).replace(Qh, "")), e, "");
        };
      }
      function Tr(e) {
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return new e();
            case 1:
              return new e(t[0]);
            case 2:
              return new e(t[0], t[1]);
            case 3:
              return new e(t[0], t[1], t[2]);
            case 4:
              return new e(t[0], t[1], t[2], t[3]);
            case 5:
              return new e(t[0], t[1], t[2], t[3], t[4]);
            case 6:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5]);
            case 7:
              return new e(t[0], t[1], t[2], t[3], t[4], t[5], t[6]);
          }
          var r = Wn(e.prototype), o = e.apply(r, t);
          return le(o) ? o : r;
        };
      }
      function jm(e, t, r) {
        var o = Tr(e);
        function s() {
          for (var d = arguments.length, h = R(d), m = d, U = Sn(s); m--; )
            h[m] = arguments[m];
          var M = d < 3 && h[0] !== U && h[d - 1] !== U ? [] : Ct(h, U);
          if (d -= M.length, d < r)
            return mc(
              e,
              t,
              Ra,
              s.placeholder,
              a,
              h,
              M,
              a,
              a,
              r - d
            );
          var w = this && this !== ve && this instanceof s ? o : e;
          return Be(w, this, h);
        }
        return s;
      }
      function dc(e) {
        return function(t, r, o) {
          var s = te(t);
          if (!Se(t)) {
            var d = A(r, 3);
            t = me(t), r = function(m) {
              return d(s[m], m, s);
            };
          }
          var h = e(t, r, o);
          return h > -1 ? s[d ? t[h] : h] : a;
        };
      }
      function fc(e) {
        return yt(function(t) {
          var r = t.length, o = r, s = Ke.prototype.thru;
          for (e && t.reverse(); o--; ) {
            var d = t[o];
            if (typeof d != "function")
              throw new qe(p);
            if (s && !h && wa(d) == "wrapper")
              var h = new Ke([], !0);
          }
          for (o = h ? o : r; ++o < r; ) {
            d = t[o];
            var m = wa(d), U = m == "wrapper" ? oo(d) : a;
            U && uo(U[0]) && U[1] == (fe | ne | F | ee) && !U[4].length && U[9] == 1 ? h = h[wa(U[0])].apply(h, U[3]) : h = d.length == 1 && uo(d) ? h[m]() : h.thru(d);
          }
          return function() {
            var M = arguments, w = M[0];
            if (h && M.length == 1 && C(w))
              return h.plant(w).value();
            for (var Z = 0, b = r ? t[Z].apply(this, M) : w; ++Z < r; )
              b = t[Z].call(this, b);
            return b;
          };
        });
      }
      function Ra(e, t, r, o, s, d, h, m, U, M) {
        var w = t & fe, Z = t & G, b = t & B, E = t & (ne | oe), x = t & ht, Y = b ? a : Tr(e);
        function Q() {
          for (var P = arguments.length, X = R(P), Ce = P; Ce--; )
            X[Ce] = arguments[Ce];
          if (E)
            var ye = Sn(Q), ze = ip(X, ye);
          if (o && (X = oc(X, o, s, E)), d && (X = lc(X, d, h, E)), P -= ze, E && P < M) {
            var ue = Ct(X, ye);
            return mc(
              e,
              t,
              Ra,
              Q.placeholder,
              r,
              X,
              ue,
              m,
              U,
              M - P
            );
          }
          var ut = Z ? r : this, Ft = b ? ut[e] : e;
          return P = X.length, m ? X = hV(X, m) : x && P > 1 && X.reverse(), w && U < P && (X.length = U), this && this !== ve && this instanceof Q && (Ft = Y || Tr(Ft)), Ft.apply(ut, X);
        }
        return Q;
      }
      function hc(e, t) {
        return function(r, o) {
          return Nm(r, e, t(o), {});
        };
      }
      function va(e, t) {
        return function(r, o) {
          var s;
          if (r === a && o === a)
            return t;
          if (r !== a && (s = r), o !== a) {
            if (s === a)
              return o;
            typeof r == "string" || typeof o == "string" ? (r = Oe(r), o = Oe(o)) : (r = Ks(r), o = Ks(o)), s = e(r, o);
          }
          return s;
        };
      }
      function no(e) {
        return yt(function(t) {
          return t = ie(t, De(A())), I(function(r) {
            var o = this;
            return e(t, function(s) {
              return Be(s, o, r);
            });
          });
        });
      }
      function Na(e, t) {
        t = t === a ? " " : Oe(t);
        var r = t.length;
        if (r < 2)
          return r ? Li(t, e) : t;
        var o = Li(t, la(e / Jn(t)));
        return Zn(t) ? Pt(lt(o), 0, e).join("") : o.slice(0, e);
      }
      function Xm(e, t, r, o) {
        var s = t & G, d = Tr(e);
        function h() {
          for (var m = -1, U = arguments.length, M = -1, w = o.length, Z = R(w + U), b = this && this !== ve && this instanceof h ? d : e; ++M < w; )
            Z[M] = o[M];
          for (; U--; )
            Z[M++] = arguments[++m];
          return Be(b, s ? r : this, Z);
        }
        return h;
      }
      function pc(e) {
        return function(t, r, o) {
          return o && typeof o != "number" && ke(t, r, o) && (r = o = a), t = Et(t), r === a ? (r = t, t = 0) : r = Et(r), o = o === a ? t < r ? 1 : -1 : Et(o), Sm(t, r, o, e);
        };
      }
      function Ma(e) {
        return function(t, r) {
          return typeof t == "string" && typeof r == "string" || (t = nt(t), r = nt(r)), e(t, r);
        };
      }
      function mc(e, t, r, o, s, d, h, m, U, M) {
        var w = t & ne, Z = w ? h : a, b = w ? a : h, E = w ? d : a, x = w ? a : d;
        t |= w ? F : Ue, t &= ~(w ? Ue : F), t & _ || (t &= ~(G | B));
        var Y = [
          e,
          t,
          s,
          E,
          Z,
          x,
          b,
          m,
          U,
          M
        ], Q = r.apply(a, Y);
        return uo(e) && kc(Q, Y), Q.placeholder = o, yc(Q, e, t);
      }
      function ro(e) {
        var t = he[e];
        return function(r, o) {
          if (r = nt(r), o = o == null ? 0 : we(z(o), 292), o && ys(r)) {
            var s = (q(r) + "e").split("e"), d = t(s[0] + "e" + (+s[1] + o));
            return s = (q(d) + "e").split("e"), +(s[0] + "e" + (+s[1] - o));
          }
          return t(r);
        };
      }
      var Lm = yn && 1 / qr(new yn([, -0]))[1] == Xe ? function(e) {
        return new yn(e);
      } : go;
      function Vc(e) {
        return function(t) {
          var r = Ze(t);
          return r == it ? Si(t) : r == ot ? fp(t) : ap(t, e(t));
        };
      }
      function kt(e, t, r, o, s, d, h, m) {
        var U = t & B;
        if (!U && typeof e != "function")
          throw new qe(p);
        var M = o ? o.length : 0;
        if (M || (t &= ~(F | Ue), o = s = a), h = h === a ? h : pe(z(h), 0), m = m === a ? m : z(m), M -= s ? s.length : 0, t & Ue) {
          var w = o, Z = s;
          o = s = a;
        }
        var b = U ? a : oo(e), E = [
          e,
          t,
          r,
          o,
          s,
          w,
          Z,
          d,
          h,
          m
        ];
        if (b && uV(E, b), e = E[0], t = E[1], r = E[2], o = E[3], s = E[4], m = E[9] = E[9] === a ? U ? 0 : e.length : pe(E[9] - M, 0), !m && t & (ne | oe) && (t &= ~(ne | oe)), !t || t == G)
          var x = Pm(e, t, r);
        else
          t == ne || t == oe ? x = jm(e, t, m) : (t == F || t == (G | F)) && !s.length ? x = Xm(e, t, r, o) : x = Ra.apply(a, E);
        var Y = b ? _s : kc;
        return yc(Y(x, E), e, t);
      }
      function Tc(e, t, r, o) {
        return e === a || ct(e, kn[r]) && !K.call(o, r) ? t : e;
      }
      function Uc(e, t, r, o, s, d) {
        return le(e) && le(t) && (d.set(t, e), ma(e, t, a, Uc, d), d.delete(t)), e;
      }
      function _m(e) {
        return vr(e) ? a : e;
      }
      function Rc(e, t, r, o, s, d) {
        var h = r & S, m = e.length, U = t.length;
        if (m != U && !(h && U > m))
          return !1;
        var M = d.get(e), w = d.get(t);
        if (M && w)
          return M == t && w == e;
        var Z = -1, b = !0, E = r & O ? new sn() : a;
        for (d.set(e, t), d.set(t, e); ++Z < m; ) {
          var x = e[Z], Y = t[Z];
          if (o)
            var Q = h ? o(Y, x, Z, t, e, d) : o(x, Y, Z, e, t, d);
          if (Q !== a) {
            if (Q)
              continue;
            b = !1;
            break;
          }
          if (E) {
            if (!ki(t, function(P, X) {
              if (!or(E, X) && (x === P || s(x, P, r, o, d)))
                return E.push(X);
            })) {
              b = !1;
              break;
            }
          } else if (!(x === Y || s(x, Y, r, o, d))) {
            b = !1;
            break;
          }
        }
        return d.delete(e), d.delete(t), b;
      }
      function qm(e, t, r, o, s, d, h) {
        switch (r) {
          case Nn:
            if (e.byteLength != t.byteLength || e.byteOffset != t.byteOffset)
              return !1;
            e = e.buffer, t = t.buffer;
          case ir:
            return !(e.byteLength != t.byteLength || !d(new ra(e), new ra(t)));
          case $n:
          case er:
          case tr:
            return ct(+e, +t);
          case Yr:
            return e.name == t.name && e.message == t.message;
          case nr:
          case rr:
            return e == t + "";
          case it:
            var m = Si;
          case ot:
            var U = o & S;
            if (m || (m = qr), e.size != t.size && !U)
              return !1;
            var M = h.get(e);
            if (M)
              return M == t;
            o |= O, h.set(e, t);
            var w = Rc(m(e), m(t), o, s, d, h);
            return h.delete(e), w;
          case Hr:
            if (dr)
              return dr.call(e) == dr.call(t);
        }
        return !1;
      }
      function Km(e, t, r, o, s, d) {
        var h = r & S, m = ao(e), U = m.length, M = ao(t), w = M.length;
        if (U != w && !h)
          return !1;
        for (var Z = U; Z--; ) {
          var b = m[Z];
          if (!(h ? b in t : K.call(t, b)))
            return !1;
        }
        var E = d.get(e), x = d.get(t);
        if (E && x)
          return E == t && x == e;
        var Y = !0;
        d.set(e, t), d.set(t, e);
        for (var Q = h; ++Z < U; ) {
          b = m[Z];
          var P = e[b], X = t[b];
          if (o)
            var Ce = h ? o(X, P, b, t, e, d) : o(P, X, b, e, t, d);
          if (!(Ce === a ? P === X || s(P, X, r, o, d) : Ce)) {
            Y = !1;
            break;
          }
          Q || (Q = b == "constructor");
        }
        if (Y && !Q) {
          var ye = e.constructor, ze = t.constructor;
          ye != ze && "constructor" in e && "constructor" in t && !(typeof ye == "function" && ye instanceof ye && typeof ze == "function" && ze instanceof ze) && (Y = !1);
        }
        return d.delete(e), d.delete(t), Y;
      }
      function yt(e) {
        return ho(Jc(e, a, Ac), e + "");
      }
      function ao(e) {
        return Os(e, me, so);
      }
      function io(e) {
        return Os(e, Ae, vc);
      }
      var oo = ca ? function(e) {
        return ca.get(e);
      } : go;
      function wa(e) {
        for (var t = e.name + "", r = bn[t], o = K.call(bn, t) ? r.length : 0; o--; ) {
          var s = r[o], d = s.func;
          if (d == null || d == e)
            return s.name;
        }
        return t;
      }
      function Sn(e) {
        var t = K.call(u, "placeholder") ? u : e;
        return t.placeholder;
      }
      function A() {
        var e = u.iteratee || Zo;
        return e = e === Zo ? zs : e, arguments.length ? e(arguments[0], arguments[1]) : e;
      }
      function Za(e, t) {
        var r = e.__data__;
        return oV(t) ? r[typeof t == "string" ? "string" : "hash"] : r.map;
      }
      function lo(e) {
        for (var t = me(e), r = t.length; r--; ) {
          var o = t[r], s = e[o];
          t[r] = [o, s, wc(s)];
        }
        return t;
      }
      function dn(e, t) {
        var r = cp(e, t);
        return Cs(r) ? r : a;
      }
      function $m(e) {
        var t = K.call(e, on), r = e[on];
        try {
          e[on] = a;
          var o = !0;
        } catch {
        }
        var s = ta.call(e);
        return o && (t ? e[on] = r : delete e[on]), s;
      }
      var so = xi ? function(e) {
        return e == null ? [] : (e = te(e), Ot(xi(e), function(t) {
          return gs.call(e, t);
        }));
      } : ko, vc = xi ? function(e) {
        for (var t = []; e; )
          Gt(t, so(e)), e = aa(e);
        return t;
      } : ko, Ze = ge;
      (Qi && Ze(new Qi(new ArrayBuffer(1))) != Nn || sr && Ze(new sr()) != it || Bi && Ze(Bi.resolve()) != Ql || yn && Ze(new yn()) != ot || cr && Ze(new cr()) != ar) && (Ze = function(e) {
        var t = ge(e), r = t == wt ? e.constructor : a, o = r ? fn(r) : "";
        if (o)
          switch (o) {
            case xp:
              return Nn;
            case Qp:
              return it;
            case Bp:
              return Ql;
            case Dp:
              return ot;
            case Op:
              return ar;
          }
        return t;
      });
      function eV(e, t, r) {
        for (var o = -1, s = r.length; ++o < s; ) {
          var d = r[o], h = d.size;
          switch (d.type) {
            case "drop":
              e += h;
              break;
            case "dropRight":
              t -= h;
              break;
            case "take":
              t = we(t, e + h);
              break;
            case "takeRight":
              e = pe(e, t - h);
              break;
          }
        }
        return { start: e, end: t };
      }
      function tV(e) {
        var t = e.match(lh);
        return t ? t[1].split(sh) : [];
      }
      function Nc(e, t, r) {
        t = Ht(t, e);
        for (var o = -1, s = t.length, d = !1; ++o < s; ) {
          var h = Ut(t[o]);
          if (!(d = e != null && r(e, h)))
            break;
          e = e[h];
        }
        return d || ++o != s ? d : (s = e == null ? 0 : e.length, !!s && Ea(s) && bt(h, s) && (C(e) || hn(e)));
      }
      function nV(e) {
        var t = e.length, r = new e.constructor(t);
        return t && typeof e[0] == "string" && K.call(e, "index") && (r.index = e.index, r.input = e.input), r;
      }
      function Mc(e) {
        return typeof e.constructor == "function" && !Ur(e) ? Wn(aa(e)) : {};
      }
      function rV(e, t, r) {
        var o = e.constructor;
        switch (t) {
          case ir:
            return to(e);
          case $n:
          case er:
            return new o(+e);
          case Nn:
            return Gm(e, r);
          case li:
          case si:
          case ci:
          case ui:
          case di:
          case fi:
          case hi:
          case pi:
          case mi:
            return ac(e, r);
          case it:
            return new o();
          case tr:
          case rr:
            return new o(e);
          case nr:
            return Cm(e);
          case ot:
            return new o();
          case Hr:
            return zm(e);
        }
      }
      function aV(e, t) {
        var r = t.length;
        if (!r)
          return e;
        var o = r - 1;
        return t[o] = (r > 1 ? "& " : "") + t[o], t = t.join(r > 2 ? ", " : " "), e.replace(oh, `{
/* [wrapped with ` + t + `] */
`);
      }
      function iV(e) {
        return C(e) || hn(e) || !!(ks && e && e[ks]);
      }
      function bt(e, t) {
        var r = typeof e;
        return t = t == null ? Dt : t, !!t && (r == "number" || r != "symbol" && Th.test(e)) && e > -1 && e % 1 == 0 && e < t;
      }
      function ke(e, t, r) {
        if (!le(r))
          return !1;
        var o = typeof t;
        return (o == "number" ? Se(r) && bt(t, r.length) : o == "string" && t in r) ? ct(r[t], e) : !1;
      }
      function co(e, t) {
        if (C(e))
          return !1;
        var r = typeof e;
        return r == "number" || r == "symbol" || r == "boolean" || e == null || Ge(e) ? !0 : nh.test(e) || !th.test(e) || t != null && e in te(t);
      }
      function oV(e) {
        var t = typeof e;
        return t == "string" || t == "number" || t == "symbol" || t == "boolean" ? e !== "__proto__" : e === null;
      }
      function uo(e) {
        var t = wa(e), r = u[t];
        if (typeof r != "function" || !(t in j.prototype))
          return !1;
        if (e === r)
          return !0;
        var o = oo(r);
        return !!o && e === o[0];
      }
      function lV(e) {
        return !!ws && ws in e;
      }
      var sV = $r ? Wt : yo;
      function Ur(e) {
        var t = e && e.constructor, r = typeof t == "function" && t.prototype || kn;
        return e === r;
      }
      function wc(e) {
        return e === e && !le(e);
      }
      function Zc(e, t) {
        return function(r) {
          return r == null ? !1 : r[e] === t && (t !== a || e in te(r));
        };
      }
      function cV(e) {
        var t = ba(e, function(o) {
          return r.size === k && r.clear(), o;
        }), r = t.cache;
        return t;
      }
      function uV(e, t) {
        var r = e[1], o = t[1], s = r | o, d = s < (G | B | fe), h = o == fe && r == ne || o == fe && r == ee && e[7].length <= t[8] || o == (fe | ee) && t[7].length <= t[8] && r == ne;
        if (!(d || h))
          return e;
        o & G && (e[2] = t[2], s |= r & G ? 0 : _);
        var m = t[3];
        if (m) {
          var U = e[3];
          e[3] = U ? oc(U, m, t[4]) : m, e[4] = U ? Ct(e[3], J) : t[4];
        }
        return m = t[5], m && (U = e[5], e[5] = U ? lc(U, m, t[6]) : m, e[6] = U ? Ct(e[5], J) : t[6]), m = t[7], m && (e[7] = m), o & fe && (e[8] = e[8] == null ? t[8] : we(e[8], t[8])), e[9] == null && (e[9] = t[9]), e[0] = t[0], e[1] = s, e;
      }
      function dV(e) {
        var t = [];
        if (e != null)
          for (var r in te(e))
            t.push(r);
        return t;
      }
      function fV(e) {
        return ta.call(e);
      }
      function Jc(e, t, r) {
        return t = pe(t === a ? e.length - 1 : t, 0), function() {
          for (var o = arguments, s = -1, d = pe(o.length - t, 0), h = R(d); ++s < d; )
            h[s] = o[t + s];
          s = -1;
          for (var m = R(t + 1); ++s < t; )
            m[s] = o[s];
          return m[t] = r(h), Be(e, this, m);
        };
      }
      function gc(e, t) {
        return t.length < 2 ? e : un(e, et(t, 0, -1));
      }
      function hV(e, t) {
        for (var r = e.length, o = we(t.length, r), s = Fe(e); o--; ) {
          var d = t[o];
          e[o] = bt(d, r) ? s[d] : a;
        }
        return e;
      }
      function fo(e, t) {
        if (!(t === "constructor" && typeof e[t] == "function") && t != "__proto__")
          return e[t];
      }
      var kc = bc(_s), Rr = yp || function(e, t) {
        return ve.setTimeout(e, t);
      }, ho = bc(Qm);
      function yc(e, t, r) {
        var o = t + "";
        return ho(e, aV(o, pV(tV(o), r)));
      }
      function bc(e) {
        var t = 0, r = 0;
        return function() {
          var o = Fp(), s = pt - (o - r);
          if (r = o, s > 0) {
            if (++t >= Je)
              return arguments[0];
          } else
            t = 0;
          return e.apply(a, arguments);
        };
      }
      function Ja(e, t) {
        var r = -1, o = e.length, s = o - 1;
        for (t = t === a ? o : t; ++r < t; ) {
          var d = Xi(r, s), h = e[d];
          e[d] = e[r], e[r] = h;
        }
        return e.length = t, e;
      }
      var Wc = cV(function(e) {
        var t = [];
        return e.charCodeAt(0) === 46 && t.push(""), e.replace(rh, function(r, o, s, d) {
          t.push(s ? d.replace(dh, "$1") : o || r);
        }), t;
      });
      function Ut(e) {
        if (typeof e == "string" || Ge(e))
          return e;
        var t = e + "";
        return t == "0" && 1 / e == -Xe ? "-0" : t;
      }
      function fn(e) {
        if (e != null) {
          try {
            return ea.call(e);
          } catch {
          }
          try {
            return e + "";
          } catch {
          }
        }
        return "";
      }
      function pV(e, t) {
        return _e(Cf, function(r) {
          var o = "_." + r[0];
          t & r[1] && !Lr(e, o) && e.push(o);
        }), e.sort();
      }
      function Ec(e) {
        if (e instanceof j)
          return e.clone();
        var t = new Ke(e.__wrapped__, e.__chain__);
        return t.__actions__ = Fe(e.__actions__), t.__index__ = e.__index__, t.__values__ = e.__values__, t;
      }
      function mV(e, t, r) {
        (r ? ke(e, t, r) : t === a) ? t = 1 : t = pe(z(t), 0);
        var o = e == null ? 0 : e.length;
        if (!o || t < 1)
          return [];
        for (var s = 0, d = 0, h = R(la(o / t)); s < o; )
          h[d++] = et(e, s, s += t);
        return h;
      }
      function VV(e) {
        for (var t = -1, r = e == null ? 0 : e.length, o = 0, s = []; ++t < r; ) {
          var d = e[t];
          d && (s[o++] = d);
        }
        return s;
      }
      function TV() {
        var e = arguments.length;
        if (!e)
          return [];
        for (var t = R(e - 1), r = arguments[0], o = e; o--; )
          t[o - 1] = arguments[o];
        return Gt(C(r) ? Fe(r) : [r], Ne(t, 1));
      }
      var UV = I(function(e, t) {
        return ce(e) ? hr(e, Ne(t, 1, ce, !0)) : [];
      }), RV = I(function(e, t) {
        var r = tt(t);
        return ce(r) && (r = a), ce(e) ? hr(e, Ne(t, 1, ce, !0), A(r, 2)) : [];
      }), vV = I(function(e, t) {
        var r = tt(t);
        return ce(r) && (r = a), ce(e) ? hr(e, Ne(t, 1, ce, !0), a, r) : [];
      });
      function NV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : z(t), et(e, t < 0 ? 0 : t, o)) : [];
      }
      function MV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : z(t), t = o - t, et(e, 0, t < 0 ? 0 : t)) : [];
      }
      function wV(e, t) {
        return e && e.length ? Ta(e, A(t, 3), !0, !0) : [];
      }
      function ZV(e, t) {
        return e && e.length ? Ta(e, A(t, 3), !0) : [];
      }
      function JV(e, t, r, o) {
        var s = e == null ? 0 : e.length;
        return s ? (r && typeof r != "number" && ke(e, t, r) && (r = 0, o = s), Tm(e, t, r, o)) : [];
      }
      function Fc(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = r == null ? 0 : z(r);
        return s < 0 && (s = pe(o + s, 0)), _r(e, A(t, 3), s);
      }
      function Sc(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = o - 1;
        return r !== a && (s = z(r), s = r < 0 ? pe(o + s, 0) : we(s, o - 1)), _r(e, A(t, 3), s, !0);
      }
      function Ac(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ne(e, 1) : [];
      }
      function gV(e) {
        var t = e == null ? 0 : e.length;
        return t ? Ne(e, Xe) : [];
      }
      function kV(e, t) {
        var r = e == null ? 0 : e.length;
        return r ? (t = t === a ? 1 : z(t), Ne(e, t)) : [];
      }
      function yV(e) {
        for (var t = -1, r = e == null ? 0 : e.length, o = {}; ++t < r; ) {
          var s = e[t];
          o[s[0]] = s[1];
        }
        return o;
      }
      function xc(e) {
        return e && e.length ? e[0] : a;
      }
      function bV(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = r == null ? 0 : z(r);
        return s < 0 && (s = pe(o + s, 0)), wn(e, t, s);
      }
      function WV(e) {
        var t = e == null ? 0 : e.length;
        return t ? et(e, 0, -1) : [];
      }
      var EV = I(function(e) {
        var t = ie(e, $i);
        return t.length && t[0] === e[0] ? Yi(t) : [];
      }), FV = I(function(e) {
        var t = tt(e), r = ie(e, $i);
        return t === tt(r) ? t = a : r.pop(), r.length && r[0] === e[0] ? Yi(r, A(t, 2)) : [];
      }), SV = I(function(e) {
        var t = tt(e), r = ie(e, $i);
        return t = typeof t == "function" ? t : a, t && r.pop(), r.length && r[0] === e[0] ? Yi(r, a, t) : [];
      });
      function AV(e, t) {
        return e == null ? "" : Wp.call(e, t);
      }
      function tt(e) {
        var t = e == null ? 0 : e.length;
        return t ? e[t - 1] : a;
      }
      function xV(e, t, r) {
        var o = e == null ? 0 : e.length;
        if (!o)
          return -1;
        var s = o;
        return r !== a && (s = z(r), s = s < 0 ? pe(o + s, 0) : we(s, o - 1)), t === t ? pp(e, t, s) : _r(e, ms, s, !0);
      }
      function QV(e, t) {
        return e && e.length ? Ps(e, z(t)) : a;
      }
      var BV = I(Qc);
      function Qc(e, t) {
        return e && e.length && t && t.length ? ji(e, t) : e;
      }
      function DV(e, t, r) {
        return e && e.length && t && t.length ? ji(e, t, A(r, 2)) : e;
      }
      function OV(e, t, r) {
        return e && e.length && t && t.length ? ji(e, t, a, r) : e;
      }
      var GV = yt(function(e, t) {
        var r = e == null ? 0 : e.length, o = Oi(e, t);
        return Ls(e, ie(t, function(s) {
          return bt(s, r) ? +s : s;
        }).sort(ic)), o;
      });
      function CV(e, t) {
        var r = [];
        if (!(e && e.length))
          return r;
        var o = -1, s = [], d = e.length;
        for (t = A(t, 3); ++o < d; ) {
          var h = e[o];
          t(h, o, e) && (r.push(h), s.push(o));
        }
        return Ls(e, s), r;
      }
      function po(e) {
        return e == null ? e : Ap.call(e);
      }
      function zV(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (r && typeof r != "number" && ke(e, t, r) ? (t = 0, r = o) : (t = t == null ? 0 : z(t), r = r === a ? o : z(r)), et(e, t, r)) : [];
      }
      function YV(e, t) {
        return Va(e, t);
      }
      function IV(e, t, r) {
        return _i(e, t, A(r, 2));
      }
      function HV(e, t) {
        var r = e == null ? 0 : e.length;
        if (r) {
          var o = Va(e, t);
          if (o < r && ct(e[o], t))
            return o;
        }
        return -1;
      }
      function PV(e, t) {
        return Va(e, t, !0);
      }
      function jV(e, t, r) {
        return _i(e, t, A(r, 2), !0);
      }
      function XV(e, t) {
        var r = e == null ? 0 : e.length;
        if (r) {
          var o = Va(e, t, !0) - 1;
          if (ct(e[o], t))
            return o;
        }
        return -1;
      }
      function LV(e) {
        return e && e.length ? qs(e) : [];
      }
      function _V(e, t) {
        return e && e.length ? qs(e, A(t, 2)) : [];
      }
      function qV(e) {
        var t = e == null ? 0 : e.length;
        return t ? et(e, 1, t) : [];
      }
      function KV(e, t, r) {
        return e && e.length ? (t = r || t === a ? 1 : z(t), et(e, 0, t < 0 ? 0 : t)) : [];
      }
      function $V(e, t, r) {
        var o = e == null ? 0 : e.length;
        return o ? (t = r || t === a ? 1 : z(t), t = o - t, et(e, t < 0 ? 0 : t, o)) : [];
      }
      function eT(e, t) {
        return e && e.length ? Ta(e, A(t, 3), !1, !0) : [];
      }
      function tT(e, t) {
        return e && e.length ? Ta(e, A(t, 3)) : [];
      }
      var nT = I(function(e) {
        return It(Ne(e, 1, ce, !0));
      }), rT = I(function(e) {
        var t = tt(e);
        return ce(t) && (t = a), It(Ne(e, 1, ce, !0), A(t, 2));
      }), aT = I(function(e) {
        var t = tt(e);
        return t = typeof t == "function" ? t : a, It(Ne(e, 1, ce, !0), a, t);
      });
      function iT(e) {
        return e && e.length ? It(e) : [];
      }
      function oT(e, t) {
        return e && e.length ? It(e, A(t, 2)) : [];
      }
      function lT(e, t) {
        return t = typeof t == "function" ? t : a, e && e.length ? It(e, a, t) : [];
      }
      function mo(e) {
        if (!(e && e.length))
          return [];
        var t = 0;
        return e = Ot(e, function(r) {
          if (ce(r))
            return t = pe(r.length, t), !0;
        }), Ei(t, function(r) {
          return ie(e, yi(r));
        });
      }
      function Bc(e, t) {
        if (!(e && e.length))
          return [];
        var r = mo(e);
        return t == null ? r : ie(r, function(o) {
          return Be(t, a, o);
        });
      }
      var sT = I(function(e, t) {
        return ce(e) ? hr(e, t) : [];
      }), cT = I(function(e) {
        return Ki(Ot(e, ce));
      }), uT = I(function(e) {
        var t = tt(e);
        return ce(t) && (t = a), Ki(Ot(e, ce), A(t, 2));
      }), dT = I(function(e) {
        var t = tt(e);
        return t = typeof t == "function" ? t : a, Ki(Ot(e, ce), a, t);
      }), fT = I(mo);
      function hT(e, t) {
        return tc(e || [], t || [], fr);
      }
      function pT(e, t) {
        return tc(e || [], t || [], Vr);
      }
      var mT = I(function(e) {
        var t = e.length, r = t > 1 ? e[t - 1] : a;
        return r = typeof r == "function" ? (e.pop(), r) : a, Bc(e, r);
      });
      function Dc(e) {
        var t = u(e);
        return t.__chain__ = !0, t;
      }
      function VT(e, t) {
        return t(e), e;
      }
      function ga(e, t) {
        return t(e);
      }
      var TT = yt(function(e) {
        var t = e.length, r = t ? e[0] : 0, o = this.__wrapped__, s = function(d) {
          return Oi(d, e);
        };
        return t > 1 || this.__actions__.length || !(o instanceof j) || !bt(r) ? this.thru(s) : (o = o.slice(r, +r + (t ? 1 : 0)), o.__actions__.push({
          func: ga,
          args: [s],
          thisArg: a
        }), new Ke(o, this.__chain__).thru(function(d) {
          return t && !d.length && d.push(a), d;
        }));
      });
      function UT() {
        return Dc(this);
      }
      function RT() {
        return new Ke(this.value(), this.__chain__);
      }
      function vT() {
        this.__values__ === a && (this.__values__ = Kc(this.value()));
        var e = this.__index__ >= this.__values__.length, t = e ? a : this.__values__[this.__index__++];
        return { done: e, value: t };
      }
      function NT() {
        return this;
      }
      function MT(e) {
        for (var t, r = this; r instanceof da; ) {
          var o = Ec(r);
          o.__index__ = 0, o.__values__ = a, t ? s.__wrapped__ = o : t = o;
          var s = o;
          r = r.__wrapped__;
        }
        return s.__wrapped__ = e, t;
      }
      function wT() {
        var e = this.__wrapped__;
        if (e instanceof j) {
          var t = e;
          return this.__actions__.length && (t = new j(this)), t = t.reverse(), t.__actions__.push({
            func: ga,
            args: [po],
            thisArg: a
          }), new Ke(t, this.__chain__);
        }
        return this.thru(po);
      }
      function ZT() {
        return ec(this.__wrapped__, this.__actions__);
      }
      var JT = Ua(function(e, t, r) {
        K.call(e, r) ? ++e[r] : gt(e, r, 1);
      });
      function gT(e, t, r) {
        var o = C(e) ? hs : Vm;
        return r && ke(e, t, r) && (t = a), o(e, A(t, 3));
      }
      function kT(e, t) {
        var r = C(e) ? Ot : Bs;
        return r(e, A(t, 3));
      }
      var yT = dc(Fc), bT = dc(Sc);
      function WT(e, t) {
        return Ne(ka(e, t), 1);
      }
      function ET(e, t) {
        return Ne(ka(e, t), Xe);
      }
      function FT(e, t, r) {
        return r = r === a ? 1 : z(r), Ne(ka(e, t), r);
      }
      function Oc(e, t) {
        var r = C(e) ? _e : Yt;
        return r(e, A(t, 3));
      }
      function Gc(e, t) {
        var r = C(e) ? qh : Qs;
        return r(e, A(t, 3));
      }
      var ST = Ua(function(e, t, r) {
        K.call(e, r) ? e[r].push(t) : gt(e, r, [t]);
      });
      function AT(e, t, r, o) {
        e = Se(e) ? e : xn(e), r = r && !o ? z(r) : 0;
        var s = e.length;
        return r < 0 && (r = pe(s + r, 0)), Fa(e) ? r <= s && e.indexOf(t, r) > -1 : !!s && wn(e, t, r) > -1;
      }
      var xT = I(function(e, t, r) {
        var o = -1, s = typeof t == "function", d = Se(e) ? R(e.length) : [];
        return Yt(e, function(h) {
          d[++o] = s ? Be(t, h, r) : pr(h, t, r);
        }), d;
      }), QT = Ua(function(e, t, r) {
        gt(e, r, t);
      });
      function ka(e, t) {
        var r = C(e) ? ie : Ys;
        return r(e, A(t, 3));
      }
      function BT(e, t, r, o) {
        return e == null ? [] : (C(t) || (t = t == null ? [] : [t]), r = o ? a : r, C(r) || (r = r == null ? [] : [r]), js(e, t, r));
      }
      var DT = Ua(function(e, t, r) {
        e[r ? 0 : 1].push(t);
      }, function() {
        return [[], []];
      });
      function OT(e, t, r) {
        var o = C(e) ? gi : Ts, s = arguments.length < 3;
        return o(e, A(t, 4), r, s, Yt);
      }
      function GT(e, t, r) {
        var o = C(e) ? Kh : Ts, s = arguments.length < 3;
        return o(e, A(t, 4), r, s, Qs);
      }
      function CT(e, t) {
        var r = C(e) ? Ot : Bs;
        return r(e, Wa(A(t, 3)));
      }
      function zT(e) {
        var t = C(e) ? Fs : Am;
        return t(e);
      }
      function YT(e, t, r) {
        (r ? ke(e, t, r) : t === a) ? t = 1 : t = z(t);
        var o = C(e) ? dm : xm;
        return o(e, t);
      }
      function IT(e) {
        var t = C(e) ? fm : Bm;
        return t(e);
      }
      function HT(e) {
        if (e == null)
          return 0;
        if (Se(e))
          return Fa(e) ? Jn(e) : e.length;
        var t = Ze(e);
        return t == it || t == ot ? e.size : Hi(e).length;
      }
      function PT(e, t, r) {
        var o = C(e) ? ki : Dm;
        return r && ke(e, t, r) && (t = a), o(e, A(t, 3));
      }
      var jT = I(function(e, t) {
        if (e == null)
          return [];
        var r = t.length;
        return r > 1 && ke(e, t[0], t[1]) ? t = [] : r > 2 && ke(t[0], t[1], t[2]) && (t = [t[0]]), js(e, Ne(t, 1), []);
      }), ya = kp || function() {
        return ve.Date.now();
      };
      function XT(e, t) {
        if (typeof t != "function")
          throw new qe(p);
        return e = z(e), function() {
          if (--e < 1)
            return t.apply(this, arguments);
        };
      }
      function Cc(e, t, r) {
        return t = r ? a : t, t = e && t == null ? e.length : t, kt(e, fe, a, a, a, a, t);
      }
      function zc(e, t) {
        var r;
        if (typeof t != "function")
          throw new qe(p);
        return e = z(e), function() {
          return --e > 0 && (r = t.apply(this, arguments)), e <= 1 && (t = a), r;
        };
      }
      var Vo = I(function(e, t, r) {
        var o = G;
        if (r.length) {
          var s = Ct(r, Sn(Vo));
          o |= F;
        }
        return kt(e, o, t, r, s);
      }), Yc = I(function(e, t, r) {
        var o = G | B;
        if (r.length) {
          var s = Ct(r, Sn(Yc));
          o |= F;
        }
        return kt(t, o, e, r, s);
      });
      function Ic(e, t, r) {
        t = r ? a : t;
        var o = kt(e, ne, a, a, a, a, a, t);
        return o.placeholder = Ic.placeholder, o;
      }
      function Hc(e, t, r) {
        t = r ? a : t;
        var o = kt(e, oe, a, a, a, a, a, t);
        return o.placeholder = Hc.placeholder, o;
      }
      function Pc(e, t, r) {
        var o, s, d, h, m, U, M = 0, w = !1, Z = !1, b = !0;
        if (typeof e != "function")
          throw new qe(p);
        t = nt(t) || 0, le(r) && (w = !!r.leading, Z = "maxWait" in r, d = Z ? pe(nt(r.maxWait) || 0, t) : d, b = "trailing" in r ? !!r.trailing : b);
        function E(ue) {
          var ut = o, Ft = s;
          return o = s = a, M = ue, h = e.apply(Ft, ut), h;
        }
        function x(ue) {
          return M = ue, m = Rr(P, t), w ? E(ue) : h;
        }
        function Y(ue) {
          var ut = ue - U, Ft = ue - M, du = t - ut;
          return Z ? we(du, d - Ft) : du;
        }
        function Q(ue) {
          var ut = ue - U, Ft = ue - M;
          return U === a || ut >= t || ut < 0 || Z && Ft >= d;
        }
        function P() {
          var ue = ya();
          if (Q(ue))
            return X(ue);
          m = Rr(P, Y(ue));
        }
        function X(ue) {
          return m = a, b && o ? E(ue) : (o = s = a, h);
        }
        function Ce() {
          m !== a && nc(m), M = 0, o = U = s = m = a;
        }
        function ye() {
          return m === a ? h : X(ya());
        }
        function ze() {
          var ue = ya(), ut = Q(ue);
          if (o = arguments, s = this, U = ue, ut) {
            if (m === a)
              return x(U);
            if (Z)
              return nc(m), m = Rr(P, t), E(U);
          }
          return m === a && (m = Rr(P, t)), h;
        }
        return ze.cancel = Ce, ze.flush = ye, ze;
      }
      var LT = I(function(e, t) {
        return xs(e, 1, t);
      }), _T = I(function(e, t, r) {
        return xs(e, nt(t) || 0, r);
      });
      function qT(e) {
        return kt(e, ht);
      }
      function ba(e, t) {
        if (typeof e != "function" || t != null && typeof t != "function")
          throw new qe(p);
        var r = function() {
          var o = arguments, s = t ? t.apply(this, o) : o[0], d = r.cache;
          if (d.has(s))
            return d.get(s);
          var h = e.apply(this, o);
          return r.cache = d.set(s, h) || d, h;
        };
        return r.cache = new (ba.Cache || Jt)(), r;
      }
      ba.Cache = Jt;
      function Wa(e) {
        if (typeof e != "function")
          throw new qe(p);
        return function() {
          var t = arguments;
          switch (t.length) {
            case 0:
              return !e.call(this);
            case 1:
              return !e.call(this, t[0]);
            case 2:
              return !e.call(this, t[0], t[1]);
            case 3:
              return !e.call(this, t[0], t[1], t[2]);
          }
          return !e.apply(this, t);
        };
      }
      function KT(e) {
        return zc(2, e);
      }
      var $T = Om(function(e, t) {
        t = t.length == 1 && C(t[0]) ? ie(t[0], De(A())) : ie(Ne(t, 1), De(A()));
        var r = t.length;
        return I(function(o) {
          for (var s = -1, d = we(o.length, r); ++s < d; )
            o[s] = t[s].call(this, o[s]);
          return Be(e, this, o);
        });
      }), To = I(function(e, t) {
        var r = Ct(t, Sn(To));
        return kt(e, F, a, t, r);
      }), jc = I(function(e, t) {
        var r = Ct(t, Sn(jc));
        return kt(e, Ue, a, t, r);
      }), eU = yt(function(e, t) {
        return kt(e, ee, a, a, a, t);
      });
      function tU(e, t) {
        if (typeof e != "function")
          throw new qe(p);
        return t = t === a ? t : z(t), I(e, t);
      }
      function nU(e, t) {
        if (typeof e != "function")
          throw new qe(p);
        return t = t == null ? 0 : pe(z(t), 0), I(function(r) {
          var o = r[t], s = Pt(r, 0, t);
          return o && Gt(s, o), Be(e, this, s);
        });
      }
      function rU(e, t, r) {
        var o = !0, s = !0;
        if (typeof e != "function")
          throw new qe(p);
        return le(r) && (o = "leading" in r ? !!r.leading : o, s = "trailing" in r ? !!r.trailing : s), Pc(e, t, {
          leading: o,
          maxWait: t,
          trailing: s
        });
      }
      function aU(e) {
        return Cc(e, 1);
      }
      function iU(e, t) {
        return To(eo(t), e);
      }
      function oU() {
        if (!arguments.length)
          return [];
        var e = arguments[0];
        return C(e) ? e : [e];
      }
      function lU(e) {
        return $e(e, H);
      }
      function sU(e, t) {
        return t = typeof t == "function" ? t : a, $e(e, H, t);
      }
      function cU(e) {
        return $e(e, y | H);
      }
      function uU(e, t) {
        return t = typeof t == "function" ? t : a, $e(e, y | H, t);
      }
      function dU(e, t) {
        return t == null || As(e, t, me(t));
      }
      function ct(e, t) {
        return e === t || e !== e && t !== t;
      }
      var fU = Ma(zi), hU = Ma(function(e, t) {
        return e >= t;
      }), hn = Gs(function() {
        return arguments;
      }()) ? Gs : function(e) {
        return se(e) && K.call(e, "callee") && !gs.call(e, "callee");
      }, C = R.isArray, pU = ls ? De(ls) : Mm;
      function Se(e) {
        return e != null && Ea(e.length) && !Wt(e);
      }
      function ce(e) {
        return se(e) && Se(e);
      }
      function mU(e) {
        return e === !0 || e === !1 || se(e) && ge(e) == $n;
      }
      var jt = bp || yo, VU = ss ? De(ss) : wm;
      function TU(e) {
        return se(e) && e.nodeType === 1 && !vr(e);
      }
      function UU(e) {
        if (e == null)
          return !0;
        if (Se(e) && (C(e) || typeof e == "string" || typeof e.splice == "function" || jt(e) || An(e) || hn(e)))
          return !e.length;
        var t = Ze(e);
        if (t == it || t == ot)
          return !e.size;
        if (Ur(e))
          return !Hi(e).length;
        for (var r in e)
          if (K.call(e, r))
            return !1;
        return !0;
      }
      function RU(e, t) {
        return mr(e, t);
      }
      function vU(e, t, r) {
        r = typeof r == "function" ? r : a;
        var o = r ? r(e, t) : a;
        return o === a ? mr(e, t, a, r) : !!o;
      }
      function Uo(e) {
        if (!se(e))
          return !1;
        var t = ge(e);
        return t == Yr || t == Yf || typeof e.message == "string" && typeof e.name == "string" && !vr(e);
      }
      function NU(e) {
        return typeof e == "number" && ys(e);
      }
      function Wt(e) {
        if (!le(e))
          return !1;
        var t = ge(e);
        return t == Ir || t == xl || t == zf || t == Hf;
      }
      function Xc(e) {
        return typeof e == "number" && e == z(e);
      }
      function Ea(e) {
        return typeof e == "number" && e > -1 && e % 1 == 0 && e <= Dt;
      }
      function le(e) {
        var t = typeof e;
        return e != null && (t == "object" || t == "function");
      }
      function se(e) {
        return e != null && typeof e == "object";
      }
      var Lc = cs ? De(cs) : Jm;
      function MU(e, t) {
        return e === t || Ii(e, t, lo(t));
      }
      function wU(e, t, r) {
        return r = typeof r == "function" ? r : a, Ii(e, t, lo(t), r);
      }
      function ZU(e) {
        return _c(e) && e != +e;
      }
      function JU(e) {
        if (sV(e))
          throw new D(f);
        return Cs(e);
      }
      function gU(e) {
        return e === null;
      }
      function kU(e) {
        return e == null;
      }
      function _c(e) {
        return typeof e == "number" || se(e) && ge(e) == tr;
      }
      function vr(e) {
        if (!se(e) || ge(e) != wt)
          return !1;
        var t = aa(e);
        if (t === null)
          return !0;
        var r = K.call(t, "constructor") && t.constructor;
        return typeof r == "function" && r instanceof r && ea.call(r) == wp;
      }
      var Ro = us ? De(us) : gm;
      function yU(e) {
        return Xc(e) && e >= -Dt && e <= Dt;
      }
      var qc = ds ? De(ds) : km;
      function Fa(e) {
        return typeof e == "string" || !C(e) && se(e) && ge(e) == rr;
      }
      function Ge(e) {
        return typeof e == "symbol" || se(e) && ge(e) == Hr;
      }
      var An = fs ? De(fs) : ym;
      function bU(e) {
        return e === a;
      }
      function WU(e) {
        return se(e) && Ze(e) == ar;
      }
      function EU(e) {
        return se(e) && ge(e) == jf;
      }
      var FU = Ma(Pi), SU = Ma(function(e, t) {
        return e <= t;
      });
      function Kc(e) {
        if (!e)
          return [];
        if (Se(e))
          return Fa(e) ? lt(e) : Fe(e);
        if (lr && e[lr])
          return dp(e[lr]());
        var t = Ze(e), r = t == it ? Si : t == ot ? qr : xn;
        return r(e);
      }
      function Et(e) {
        if (!e)
          return e === 0 ? e : 0;
        if (e = nt(e), e === Xe || e === -Xe) {
          var t = e < 0 ? -1 : 1;
          return t * Df;
        }
        return e === e ? e : 0;
      }
      function z(e) {
        var t = Et(e), r = t % 1;
        return t === t ? r ? t - r : t : 0;
      }
      function $c(e) {
        return e ? cn(z(e), 0, mt) : 0;
      }
      function nt(e) {
        if (typeof e == "number")
          return e;
        if (Ge(e))
          return Cr;
        if (le(e)) {
          var t = typeof e.valueOf == "function" ? e.valueOf() : e;
          e = le(t) ? t + "" : t;
        }
        if (typeof e != "string")
          return e === 0 ? e : +e;
        e = Us(e);
        var r = ph.test(e);
        return r || Vh.test(e) ? Xh(e.slice(2), r ? 2 : 8) : hh.test(e) ? Cr : +e;
      }
      function eu(e) {
        return Tt(e, Ae(e));
      }
      function AU(e) {
        return e ? cn(z(e), -Dt, Dt) : e === 0 ? e : 0;
      }
      function q(e) {
        return e == null ? "" : Oe(e);
      }
      var xU = En(function(e, t) {
        if (Ur(t) || Se(t)) {
          Tt(t, me(t), e);
          return;
        }
        for (var r in t)
          K.call(t, r) && fr(e, r, t[r]);
      }), tu = En(function(e, t) {
        Tt(t, Ae(t), e);
      }), Sa = En(function(e, t, r, o) {
        Tt(t, Ae(t), e, o);
      }), QU = En(function(e, t, r, o) {
        Tt(t, me(t), e, o);
      }), BU = yt(Oi);
      function DU(e, t) {
        var r = Wn(e);
        return t == null ? r : Ss(r, t);
      }
      var OU = I(function(e, t) {
        e = te(e);
        var r = -1, o = t.length, s = o > 2 ? t[2] : a;
        for (s && ke(t[0], t[1], s) && (o = 1); ++r < o; )
          for (var d = t[r], h = Ae(d), m = -1, U = h.length; ++m < U; ) {
            var M = h[m], w = e[M];
            (w === a || ct(w, kn[M]) && !K.call(e, M)) && (e[M] = d[M]);
          }
        return e;
      }), GU = I(function(e) {
        return e.push(a, Uc), Be(nu, a, e);
      });
      function CU(e, t) {
        return ps(e, A(t, 3), Vt);
      }
      function zU(e, t) {
        return ps(e, A(t, 3), Ci);
      }
      function YU(e, t) {
        return e == null ? e : Gi(e, A(t, 3), Ae);
      }
      function IU(e, t) {
        return e == null ? e : Ds(e, A(t, 3), Ae);
      }
      function HU(e, t) {
        return e && Vt(e, A(t, 3));
      }
      function PU(e, t) {
        return e && Ci(e, A(t, 3));
      }
      function jU(e) {
        return e == null ? [] : pa(e, me(e));
      }
      function XU(e) {
        return e == null ? [] : pa(e, Ae(e));
      }
      function vo(e, t, r) {
        var o = e == null ? a : un(e, t);
        return o === a ? r : o;
      }
      function LU(e, t) {
        return e != null && Nc(e, t, Um);
      }
      function No(e, t) {
        return e != null && Nc(e, t, Rm);
      }
      var _U = hc(function(e, t, r) {
        t != null && typeof t.toString != "function" && (t = ta.call(t)), e[t] = r;
      }, wo(xe)), qU = hc(function(e, t, r) {
        t != null && typeof t.toString != "function" && (t = ta.call(t)), K.call(e, t) ? e[t].push(r) : e[t] = [r];
      }, A), KU = I(pr);
      function me(e) {
        return Se(e) ? Es(e) : Hi(e);
      }
      function Ae(e) {
        return Se(e) ? Es(e, !0) : bm(e);
      }
      function $U(e, t) {
        var r = {};
        return t = A(t, 3), Vt(e, function(o, s, d) {
          gt(r, t(o, s, d), o);
        }), r;
      }
      function e1(e, t) {
        var r = {};
        return t = A(t, 3), Vt(e, function(o, s, d) {
          gt(r, s, t(o, s, d));
        }), r;
      }
      var t1 = En(function(e, t, r) {
        ma(e, t, r);
      }), nu = En(function(e, t, r, o) {
        ma(e, t, r, o);
      }), n1 = yt(function(e, t) {
        var r = {};
        if (e == null)
          return r;
        var o = !1;
        t = ie(t, function(d) {
          return d = Ht(d, e), o || (o = d.length > 1), d;
        }), Tt(e, io(e), r), o && (r = $e(r, y | g | H, _m));
        for (var s = t.length; s--; )
          qi(r, t[s]);
        return r;
      });
      function r1(e, t) {
        return ru(e, Wa(A(t)));
      }
      var a1 = yt(function(e, t) {
        return e == null ? {} : Em(e, t);
      });
      function ru(e, t) {
        if (e == null)
          return {};
        var r = ie(io(e), function(o) {
          return [o];
        });
        return t = A(t), Xs(e, r, function(o, s) {
          return t(o, s[0]);
        });
      }
      function i1(e, t, r) {
        t = Ht(t, e);
        var o = -1, s = t.length;
        for (s || (s = 1, e = a); ++o < s; ) {
          var d = e == null ? a : e[Ut(t[o])];
          d === a && (o = s, d = r), e = Wt(d) ? d.call(e) : d;
        }
        return e;
      }
      function o1(e, t, r) {
        return e == null ? e : Vr(e, t, r);
      }
      function l1(e, t, r, o) {
        return o = typeof o == "function" ? o : a, e == null ? e : Vr(e, t, r, o);
      }
      var au = Vc(me), iu = Vc(Ae);
      function s1(e, t, r) {
        var o = C(e), s = o || jt(e) || An(e);
        if (t = A(t, 4), r == null) {
          var d = e && e.constructor;
          s ? r = o ? new d() : [] : le(e) ? r = Wt(d) ? Wn(aa(e)) : {} : r = {};
        }
        return (s ? _e : Vt)(e, function(h, m, U) {
          return t(r, h, m, U);
        }), r;
      }
      function c1(e, t) {
        return e == null ? !0 : qi(e, t);
      }
      function u1(e, t, r) {
        return e == null ? e : $s(e, t, eo(r));
      }
      function d1(e, t, r, o) {
        return o = typeof o == "function" ? o : a, e == null ? e : $s(e, t, eo(r), o);
      }
      function xn(e) {
        return e == null ? [] : Fi(e, me(e));
      }
      function f1(e) {
        return e == null ? [] : Fi(e, Ae(e));
      }
      function h1(e, t, r) {
        return r === a && (r = t, t = a), r !== a && (r = nt(r), r = r === r ? r : 0), t !== a && (t = nt(t), t = t === t ? t : 0), cn(nt(e), t, r);
      }
      function p1(e, t, r) {
        return t = Et(t), r === a ? (r = t, t = 0) : r = Et(r), e = nt(e), vm(e, t, r);
      }
      function m1(e, t, r) {
        if (r && typeof r != "boolean" && ke(e, t, r) && (t = r = a), r === a && (typeof t == "boolean" ? (r = t, t = a) : typeof e == "boolean" && (r = e, e = a)), e === a && t === a ? (e = 0, t = 1) : (e = Et(e), t === a ? (t = e, e = 0) : t = Et(t)), e > t) {
          var o = e;
          e = t, t = o;
        }
        if (r || e % 1 || t % 1) {
          var s = bs();
          return we(e + s * (t - e + jh("1e-" + ((s + "").length - 1))), t);
        }
        return Xi(e, t);
      }
      var V1 = Fn(function(e, t, r) {
        return t = t.toLowerCase(), e + (r ? ou(t) : t);
      });
      function ou(e) {
        return Mo(q(e).toLowerCase());
      }
      function lu(e) {
        return e = q(e), e && e.replace(Uh, op).replace(Bh, "");
      }
      function T1(e, t, r) {
        e = q(e), t = Oe(t);
        var o = e.length;
        r = r === a ? o : cn(z(r), 0, o);
        var s = r;
        return r -= t.length, r >= 0 && e.slice(r, s) == t;
      }
      function U1(e) {
        return e = q(e), e && Kf.test(e) ? e.replace(Dl, lp) : e;
      }
      function R1(e) {
        return e = q(e), e && ah.test(e) ? e.replace(Vi, "\\$&") : e;
      }
      var v1 = Fn(function(e, t, r) {
        return e + (r ? "-" : "") + t.toLowerCase();
      }), N1 = Fn(function(e, t, r) {
        return e + (r ? " " : "") + t.toLowerCase();
      }), M1 = uc("toLowerCase");
      function w1(e, t, r) {
        e = q(e), t = z(t);
        var o = t ? Jn(e) : 0;
        if (!t || o >= t)
          return e;
        var s = (t - o) / 2;
        return Na(sa(s), r) + e + Na(la(s), r);
      }
      function Z1(e, t, r) {
        e = q(e), t = z(t);
        var o = t ? Jn(e) : 0;
        return t && o < t ? e + Na(t - o, r) : e;
      }
      function J1(e, t, r) {
        e = q(e), t = z(t);
        var o = t ? Jn(e) : 0;
        return t && o < t ? Na(t - o, r) + e : e;
      }
      function g1(e, t, r) {
        return r || t == null ? t = 0 : t && (t = +t), Sp(q(e).replace(Ti, ""), t || 0);
      }
      function k1(e, t, r) {
        return (r ? ke(e, t, r) : t === a) ? t = 1 : t = z(t), Li(q(e), t);
      }
      function y1() {
        var e = arguments, t = q(e[0]);
        return e.length < 3 ? t : t.replace(e[1], e[2]);
      }
      var b1 = Fn(function(e, t, r) {
        return e + (r ? "_" : "") + t.toLowerCase();
      });
      function W1(e, t, r) {
        return r && typeof r != "number" && ke(e, t, r) && (t = r = a), r = r === a ? mt : r >>> 0, r ? (e = q(e), e && (typeof t == "string" || t != null && !Ro(t)) && (t = Oe(t), !t && Zn(e)) ? Pt(lt(e), 0, r) : e.split(t, r)) : [];
      }
      var E1 = Fn(function(e, t, r) {
        return e + (r ? " " : "") + Mo(t);
      });
      function F1(e, t, r) {
        return e = q(e), r = r == null ? 0 : cn(z(r), 0, e.length), t = Oe(t), e.slice(r, r + t.length) == t;
      }
      function S1(e, t, r) {
        var o = u.templateSettings;
        r && ke(e, t, r) && (t = a), e = q(e), t = Sa({}, t, o, Tc);
        var s = Sa({}, t.imports, o.imports, Tc), d = me(s), h = Fi(s, d), m, U, M = 0, w = t.interpolate || Pr, Z = "__p += '", b = Ai(
          (t.escape || Pr).source + "|" + w.source + "|" + (w === Ol ? fh : Pr).source + "|" + (t.evaluate || Pr).source + "|$",
          "g"
        ), E = "//# sourceURL=" + (K.call(t, "sourceURL") ? (t.sourceURL + "").replace(/\s/g, " ") : "lodash.templateSources[" + ++zh + "]") + `
`;
        e.replace(b, function(Q, P, X, Ce, ye, ze) {
          return X || (X = Ce), Z += e.slice(M, ze).replace(Rh, sp), P && (m = !0, Z += `' +
__e(` + P + `) +
'`), ye && (U = !0, Z += `';
` + ye + `;
__p += '`), X && (Z += `' +
((__t = (` + X + `)) == null ? '' : __t) +
'`), M = ze + Q.length, Q;
        }), Z += `';
`;
        var x = K.call(t, "variable") && t.variable;
        if (!x)
          Z = `with (obj) {
` + Z + `
}
`;
        else if (uh.test(x))
          throw new D(V);
        Z = (U ? Z.replace(Xf, "") : Z).replace(Lf, "$1").replace(_f, "$1;"), Z = "function(" + (x || "obj") + `) {
` + (x ? "" : `obj || (obj = {});
`) + "var __t, __p = ''" + (m ? ", __e = _.escape" : "") + (U ? `, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
` : `;
`) + Z + `return __p
}`;
        var Y = cu(function() {
          return L(d, E + "return " + Z).apply(a, h);
        });
        if (Y.source = Z, Uo(Y))
          throw Y;
        return Y;
      }
      function A1(e) {
        return q(e).toLowerCase();
      }
      function x1(e) {
        return q(e).toUpperCase();
      }
      function Q1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return Us(e);
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = lt(t), d = Rs(o, s), h = vs(o, s) + 1;
        return Pt(o, d, h).join("");
      }
      function B1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return e.slice(0, Ms(e) + 1);
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = vs(o, lt(t)) + 1;
        return Pt(o, 0, s).join("");
      }
      function D1(e, t, r) {
        if (e = q(e), e && (r || t === a))
          return e.replace(Ti, "");
        if (!e || !(t = Oe(t)))
          return e;
        var o = lt(e), s = Rs(o, lt(t));
        return Pt(o, s).join("");
      }
      function O1(e, t) {
        var r = Mt, o = rn;
        if (le(t)) {
          var s = "separator" in t ? t.separator : s;
          r = "length" in t ? z(t.length) : r, o = "omission" in t ? Oe(t.omission) : o;
        }
        e = q(e);
        var d = e.length;
        if (Zn(e)) {
          var h = lt(e);
          d = h.length;
        }
        if (r >= d)
          return e;
        var m = r - Jn(o);
        if (m < 1)
          return o;
        var U = h ? Pt(h, 0, m).join("") : e.slice(0, m);
        if (s === a)
          return U + o;
        if (h && (m += U.length - m), Ro(s)) {
          if (e.slice(m).search(s)) {
            var M, w = U;
            for (s.global || (s = Ai(s.source, q(Gl.exec(s)) + "g")), s.lastIndex = 0; M = s.exec(w); )
              var Z = M.index;
            U = U.slice(0, Z === a ? m : Z);
          }
        } else if (e.indexOf(Oe(s), m) != m) {
          var b = U.lastIndexOf(s);
          b > -1 && (U = U.slice(0, b));
        }
        return U + o;
      }
      function G1(e) {
        return e = q(e), e && qf.test(e) ? e.replace(Bl, mp) : e;
      }
      var C1 = Fn(function(e, t, r) {
        return e + (r ? " " : "") + t.toUpperCase();
      }), Mo = uc("toUpperCase");
      function su(e, t, r) {
        return e = q(e), t = r ? a : t, t === a ? up(e) ? Up(e) : tp(e) : e.match(t) || [];
      }
      var cu = I(function(e, t) {
        try {
          return Be(e, a, t);
        } catch (r) {
          return Uo(r) ? r : new D(r);
        }
      }), z1 = yt(function(e, t) {
        return _e(t, function(r) {
          r = Ut(r), gt(e, r, Vo(e[r], e));
        }), e;
      });
      function Y1(e) {
        var t = e == null ? 0 : e.length, r = A();
        return e = t ? ie(e, function(o) {
          if (typeof o[1] != "function")
            throw new qe(p);
          return [r(o[0]), o[1]];
        }) : [], I(function(o) {
          for (var s = -1; ++s < t; ) {
            var d = e[s];
            if (Be(d[0], this, o))
              return Be(d[1], this, o);
          }
        });
      }
      function I1(e) {
        return mm($e(e, y));
      }
      function wo(e) {
        return function() {
          return e;
        };
      }
      function H1(e, t) {
        return e == null || e !== e ? t : e;
      }
      var P1 = fc(), j1 = fc(!0);
      function xe(e) {
        return e;
      }
      function Zo(e) {
        return zs(typeof e == "function" ? e : $e(e, y));
      }
      function X1(e) {
        return Is($e(e, y));
      }
      function L1(e, t) {
        return Hs(e, $e(t, y));
      }
      var _1 = I(function(e, t) {
        return function(r) {
          return pr(r, e, t);
        };
      }), q1 = I(function(e, t) {
        return function(r) {
          return pr(e, r, t);
        };
      });
      function Jo(e, t, r) {
        var o = me(t), s = pa(t, o);
        r == null && !(le(t) && (s.length || !o.length)) && (r = t, t = e, e = this, s = pa(t, me(t)));
        var d = !(le(r) && "chain" in r) || !!r.chain, h = Wt(e);
        return _e(s, function(m) {
          var U = t[m];
          e[m] = U, h && (e.prototype[m] = function() {
            var M = this.__chain__;
            if (d || M) {
              var w = e(this.__wrapped__), Z = w.__actions__ = Fe(this.__actions__);
              return Z.push({ func: U, args: arguments, thisArg: e }), w.__chain__ = M, w;
            }
            return U.apply(e, Gt([this.value()], arguments));
          });
        }), e;
      }
      function K1() {
        return ve._ === this && (ve._ = Zp), this;
      }
      function go() {
      }
      function $1(e) {
        return e = z(e), I(function(t) {
          return Ps(t, e);
        });
      }
      var e0 = no(ie), t0 = no(hs), n0 = no(ki);
      function uu(e) {
        return co(e) ? yi(Ut(e)) : Fm(e);
      }
      function r0(e) {
        return function(t) {
          return e == null ? a : un(e, t);
        };
      }
      var a0 = pc(), i0 = pc(!0);
      function ko() {
        return [];
      }
      function yo() {
        return !1;
      }
      function o0() {
        return {};
      }
      function l0() {
        return "";
      }
      function s0() {
        return !0;
      }
      function c0(e, t) {
        if (e = z(e), e < 1 || e > Dt)
          return [];
        var r = mt, o = we(e, mt);
        t = A(t), e -= mt;
        for (var s = Ei(o, t); ++r < e; )
          t(r);
        return s;
      }
      function u0(e) {
        return C(e) ? ie(e, Ut) : Ge(e) ? [e] : Fe(Wc(q(e)));
      }
      function d0(e) {
        var t = ++Mp;
        return q(e) + t;
      }
      var f0 = va(function(e, t) {
        return e + t;
      }, 0), h0 = ro("ceil"), p0 = va(function(e, t) {
        return e / t;
      }, 1), m0 = ro("floor");
      function V0(e) {
        return e && e.length ? ha(e, xe, zi) : a;
      }
      function T0(e, t) {
        return e && e.length ? ha(e, A(t, 2), zi) : a;
      }
      function U0(e) {
        return Vs(e, xe);
      }
      function R0(e, t) {
        return Vs(e, A(t, 2));
      }
      function v0(e) {
        return e && e.length ? ha(e, xe, Pi) : a;
      }
      function N0(e, t) {
        return e && e.length ? ha(e, A(t, 2), Pi) : a;
      }
      var M0 = va(function(e, t) {
        return e * t;
      }, 1), w0 = ro("round"), Z0 = va(function(e, t) {
        return e - t;
      }, 0);
      function J0(e) {
        return e && e.length ? Wi(e, xe) : 0;
      }
      function g0(e, t) {
        return e && e.length ? Wi(e, A(t, 2)) : 0;
      }
      return u.after = XT, u.ary = Cc, u.assign = xU, u.assignIn = tu, u.assignInWith = Sa, u.assignWith = QU, u.at = BU, u.before = zc, u.bind = Vo, u.bindAll = z1, u.bindKey = Yc, u.castArray = oU, u.chain = Dc, u.chunk = mV, u.compact = VV, u.concat = TV, u.cond = Y1, u.conforms = I1, u.constant = wo, u.countBy = JT, u.create = DU, u.curry = Ic, u.curryRight = Hc, u.debounce = Pc, u.defaults = OU, u.defaultsDeep = GU, u.defer = LT, u.delay = _T, u.difference = UV, u.differenceBy = RV, u.differenceWith = vV, u.drop = NV, u.dropRight = MV, u.dropRightWhile = wV, u.dropWhile = ZV, u.fill = JV, u.filter = kT, u.flatMap = WT, u.flatMapDeep = ET, u.flatMapDepth = FT, u.flatten = Ac, u.flattenDeep = gV, u.flattenDepth = kV, u.flip = qT, u.flow = P1, u.flowRight = j1, u.fromPairs = yV, u.functions = jU, u.functionsIn = XU, u.groupBy = ST, u.initial = WV, u.intersection = EV, u.intersectionBy = FV, u.intersectionWith = SV, u.invert = _U, u.invertBy = qU, u.invokeMap = xT, u.iteratee = Zo, u.keyBy = QT, u.keys = me, u.keysIn = Ae, u.map = ka, u.mapKeys = $U, u.mapValues = e1, u.matches = X1, u.matchesProperty = L1, u.memoize = ba, u.merge = t1, u.mergeWith = nu, u.method = _1, u.methodOf = q1, u.mixin = Jo, u.negate = Wa, u.nthArg = $1, u.omit = n1, u.omitBy = r1, u.once = KT, u.orderBy = BT, u.over = e0, u.overArgs = $T, u.overEvery = t0, u.overSome = n0, u.partial = To, u.partialRight = jc, u.partition = DT, u.pick = a1, u.pickBy = ru, u.property = uu, u.propertyOf = r0, u.pull = BV, u.pullAll = Qc, u.pullAllBy = DV, u.pullAllWith = OV, u.pullAt = GV, u.range = a0, u.rangeRight = i0, u.rearg = eU, u.reject = CT, u.remove = CV, u.rest = tU, u.reverse = po, u.sampleSize = YT, u.set = o1, u.setWith = l1, u.shuffle = IT, u.slice = zV, u.sortBy = jT, u.sortedUniq = LV, u.sortedUniqBy = _V, u.split = W1, u.spread = nU, u.tail = qV, u.take = KV, u.takeRight = $V, u.takeRightWhile = eT, u.takeWhile = tT, u.tap = VT, u.throttle = rU, u.thru = ga, u.toArray = Kc, u.toPairs = au, u.toPairsIn = iu, u.toPath = u0, u.toPlainObject = eu, u.transform = s1, u.unary = aU, u.union = nT, u.unionBy = rT, u.unionWith = aT, u.uniq = iT, u.uniqBy = oT, u.uniqWith = lT, u.unset = c1, u.unzip = mo, u.unzipWith = Bc, u.update = u1, u.updateWith = d1, u.values = xn, u.valuesIn = f1, u.without = sT, u.words = su, u.wrap = iU, u.xor = cT, u.xorBy = uT, u.xorWith = dT, u.zip = fT, u.zipObject = hT, u.zipObjectDeep = pT, u.zipWith = mT, u.entries = au, u.entriesIn = iu, u.extend = tu, u.extendWith = Sa, Jo(u, u), u.add = f0, u.attempt = cu, u.camelCase = V1, u.capitalize = ou, u.ceil = h0, u.clamp = h1, u.clone = lU, u.cloneDeep = cU, u.cloneDeepWith = uU, u.cloneWith = sU, u.conformsTo = dU, u.deburr = lu, u.defaultTo = H1, u.divide = p0, u.endsWith = T1, u.eq = ct, u.escape = U1, u.escapeRegExp = R1, u.every = gT, u.find = yT, u.findIndex = Fc, u.findKey = CU, u.findLast = bT, u.findLastIndex = Sc, u.findLastKey = zU, u.floor = m0, u.forEach = Oc, u.forEachRight = Gc, u.forIn = YU, u.forInRight = IU, u.forOwn = HU, u.forOwnRight = PU, u.get = vo, u.gt = fU, u.gte = hU, u.has = LU, u.hasIn = No, u.head = xc, u.identity = xe, u.includes = AT, u.indexOf = bV, u.inRange = p1, u.invoke = KU, u.isArguments = hn, u.isArray = C, u.isArrayBuffer = pU, u.isArrayLike = Se, u.isArrayLikeObject = ce, u.isBoolean = mU, u.isBuffer = jt, u.isDate = VU, u.isElement = TU, u.isEmpty = UU, u.isEqual = RU, u.isEqualWith = vU, u.isError = Uo, u.isFinite = NU, u.isFunction = Wt, u.isInteger = Xc, u.isLength = Ea, u.isMap = Lc, u.isMatch = MU, u.isMatchWith = wU, u.isNaN = ZU, u.isNative = JU, u.isNil = kU, u.isNull = gU, u.isNumber = _c, u.isObject = le, u.isObjectLike = se, u.isPlainObject = vr, u.isRegExp = Ro, u.isSafeInteger = yU, u.isSet = qc, u.isString = Fa, u.isSymbol = Ge, u.isTypedArray = An, u.isUndefined = bU, u.isWeakMap = WU, u.isWeakSet = EU, u.join = AV, u.kebabCase = v1, u.last = tt, u.lastIndexOf = xV, u.lowerCase = N1, u.lowerFirst = M1, u.lt = FU, u.lte = SU, u.max = V0, u.maxBy = T0, u.mean = U0, u.meanBy = R0, u.min = v0, u.minBy = N0, u.stubArray = ko, u.stubFalse = yo, u.stubObject = o0, u.stubString = l0, u.stubTrue = s0, u.multiply = M0, u.nth = QV, u.noConflict = K1, u.noop = go, u.now = ya, u.pad = w1, u.padEnd = Z1, u.padStart = J1, u.parseInt = g1, u.random = m1, u.reduce = OT, u.reduceRight = GT, u.repeat = k1, u.replace = y1, u.result = i1, u.round = w0, u.runInContext = T, u.sample = zT, u.size = HT, u.snakeCase = b1, u.some = PT, u.sortedIndex = YV, u.sortedIndexBy = IV, u.sortedIndexOf = HV, u.sortedLastIndex = PV, u.sortedLastIndexBy = jV, u.sortedLastIndexOf = XV, u.startCase = E1, u.startsWith = F1, u.subtract = Z0, u.sum = J0, u.sumBy = g0, u.template = S1, u.times = c0, u.toFinite = Et, u.toInteger = z, u.toLength = $c, u.toLower = A1, u.toNumber = nt, u.toSafeInteger = AU, u.toString = q, u.toUpper = x1, u.trim = Q1, u.trimEnd = B1, u.trimStart = D1, u.truncate = O1, u.unescape = G1, u.uniqueId = d0, u.upperCase = C1, u.upperFirst = Mo, u.each = Oc, u.eachRight = Gc, u.first = xc, Jo(u, function() {
        var e = {};
        return Vt(u, function(t, r) {
          K.call(u.prototype, r) || (e[r] = t);
        }), e;
      }(), { chain: !1 }), u.VERSION = l, _e(["bind", "bindKey", "curry", "curryRight", "partial", "partialRight"], function(e) {
        u[e].placeholder = u;
      }), _e(["drop", "take"], function(e, t) {
        j.prototype[e] = function(r) {
          r = r === a ? 1 : pe(z(r), 0);
          var o = this.__filtered__ && !t ? new j(this) : this.clone();
          return o.__filtered__ ? o.__takeCount__ = we(r, o.__takeCount__) : o.__views__.push({
            size: we(r, mt),
            type: e + (o.__dir__ < 0 ? "Right" : "")
          }), o;
        }, j.prototype[e + "Right"] = function(r) {
          return this.reverse()[e](r).reverse();
        };
      }), _e(["filter", "map", "takeWhile"], function(e, t) {
        var r = t + 1, o = r == je || r == Ee;
        j.prototype[e] = function(s) {
          var d = this.clone();
          return d.__iteratees__.push({
            iteratee: A(s, 3),
            type: r
          }), d.__filtered__ = d.__filtered__ || o, d;
        };
      }), _e(["head", "last"], function(e, t) {
        var r = "take" + (t ? "Right" : "");
        j.prototype[e] = function() {
          return this[r](1).value()[0];
        };
      }), _e(["initial", "tail"], function(e, t) {
        var r = "drop" + (t ? "" : "Right");
        j.prototype[e] = function() {
          return this.__filtered__ ? new j(this) : this[r](1);
        };
      }), j.prototype.compact = function() {
        return this.filter(xe);
      }, j.prototype.find = function(e) {
        return this.filter(e).head();
      }, j.prototype.findLast = function(e) {
        return this.reverse().find(e);
      }, j.prototype.invokeMap = I(function(e, t) {
        return typeof e == "function" ? new j(this) : this.map(function(r) {
          return pr(r, e, t);
        });
      }), j.prototype.reject = function(e) {
        return this.filter(Wa(A(e)));
      }, j.prototype.slice = function(e, t) {
        e = z(e);
        var r = this;
        return r.__filtered__ && (e > 0 || t < 0) ? new j(r) : (e < 0 ? r = r.takeRight(-e) : e && (r = r.drop(e)), t !== a && (t = z(t), r = t < 0 ? r.dropRight(-t) : r.take(t - e)), r);
      }, j.prototype.takeRightWhile = function(e) {
        return this.reverse().takeWhile(e).reverse();
      }, j.prototype.toArray = function() {
        return this.take(mt);
      }, Vt(j.prototype, function(e, t) {
        var r = /^(?:filter|find|map|reject)|While$/.test(t), o = /^(?:head|last)$/.test(t), s = u[o ? "take" + (t == "last" ? "Right" : "") : t], d = o || /^find/.test(t);
        !s || (u.prototype[t] = function() {
          var h = this.__wrapped__, m = o ? [1] : arguments, U = h instanceof j, M = m[0], w = U || C(h), Z = function(P) {
            var X = s.apply(u, Gt([P], m));
            return o && b ? X[0] : X;
          };
          w && r && typeof M == "function" && M.length != 1 && (U = w = !1);
          var b = this.__chain__, E = !!this.__actions__.length, x = d && !b, Y = U && !E;
          if (!d && w) {
            h = Y ? h : new j(this);
            var Q = e.apply(h, m);
            return Q.__actions__.push({ func: ga, args: [Z], thisArg: a }), new Ke(Q, b);
          }
          return x && Y ? e.apply(this, m) : (Q = this.thru(Z), x ? o ? Q.value()[0] : Q.value() : Q);
        });
      }), _e(["pop", "push", "shift", "sort", "splice", "unshift"], function(e) {
        var t = Kr[e], r = /^(?:push|sort|unshift)$/.test(e) ? "tap" : "thru", o = /^(?:pop|shift)$/.test(e);
        u.prototype[e] = function() {
          var s = arguments;
          if (o && !this.__chain__) {
            var d = this.value();
            return t.apply(C(d) ? d : [], s);
          }
          return this[r](function(h) {
            return t.apply(C(h) ? h : [], s);
          });
        };
      }), Vt(j.prototype, function(e, t) {
        var r = u[t];
        if (r) {
          var o = r.name + "";
          K.call(bn, o) || (bn[o] = []), bn[o].push({ name: t, func: r });
        }
      }), bn[Ra(a, B).name] = [{
        name: "wrapper",
        func: a
      }], j.prototype.clone = Gp, j.prototype.reverse = Cp, j.prototype.value = zp, u.prototype.at = TT, u.prototype.chain = UT, u.prototype.commit = RT, u.prototype.next = vT, u.prototype.plant = MT, u.prototype.reverse = wT, u.prototype.toJSON = u.prototype.valueOf = u.prototype.value = ZT, u.prototype.first = u.prototype.head, lr && (u.prototype[lr] = NT), u;
    }, gn = Rp();
    an ? ((an.exports = gn)._ = gn, wi._ = gn) : ve._ = gn;
  }).call(Nr);
})(Xn, Xn.exports);
const LR = window.Pinia.defineStore, tf = LR("situationsStore", {
  state: () => ({
    situations: [],
    alarms: {}
  }),
  actions: {
    async getSituations() {
      const n = await XR();
      if (n) {
        this.alarms = Xn.exports.mapKeys(n.alarm, (a) => a.id);
        const i = n.alarm.filter(
          (a) => a.relatedAlarms && a.relatedAlarms.length > 0
        );
        i.forEach((a) => {
          a.relatedAlarms.forEach((l) => {
            var c, f, p;
            l.count = (c = this.alarms[l.id]) == null ? void 0 : c.count, l.firstEventTime = (f = this.alarms[l.id]) == null ? void 0 : f.firstEventTime, l.lastEventTime = (p = this.alarms[l.id]) == null ? void 0 : p.lastEventTime;
          });
        }), this.situations = i;
      }
    }
  }
}), _R = window.Vue.defineComponent, qR = window.Vue.toDisplayString, KR = window.Vue.normalizeClass, $R = window.Vue.openBlock, ev = window.Vue.createElementBlock, tv = window.Vue.createCommentVNode, nv = /* @__PURE__ */ _R({
  __name: "SeverityStatus",
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const i = n;
    return (a, l) => i != null && i.severity ? ($R(), ev("span", {
      key: 0,
      class: KR(["severity-status", [`${i.severity.toLowerCase()}-color`]])
    }, qR(i.severity), 3)) : tv("", !0);
  }
});
const Zl = /* @__PURE__ */ Pe(nv, [["__scopeId", "data-v-83c2cdce"]]), rv = window.Vue.defineComponent, av = window.Vue.unref, iv = window.Vue.renderList, ov = window.Vue.Fragment, Io = window.Vue.openBlock, Ho = window.Vue.createElementBlock, lv = window.Vue.toDisplayString, sv = window.Vue.normalizeClass;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const cv = { class: "alarms-list" }, uv = /* @__PURE__ */ rv({
  __name: "AlarmsCountBySeverity",
  props: {
    relatedAlarms: null,
    size: null
  },
  setup(n) {
    const i = n;
    return (a, l) => (Io(), Ho("div", cv, [
      (Io(!0), Ho(ov, null, iv(av(Xn.exports.groupBy)(i == null ? void 0 : i.relatedAlarms, "severity"), (c, f) => (Io(), Ho("div", {
        class: sv(["alarm-count", [`${f.toString().toLowerCase()}-color`, i.size]]),
        key: f
      }, lv(c.length), 3))), 128))
    ]));
  }
});
const nf = /* @__PURE__ */ Pe(uv, [["__scopeId", "data-v-b8c8b147"]]), dv = window.Vue.defineComponent, xu = window.Vue.normalizeClass, Aa = window.Vue.createElementVNode, fv = window.Vue.toDisplayString, Qu = window.Vue.createVNode, hv = window.Vue.openBlock, pv = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const mv = { class: "content" }, Vv = { class: "title-row" }, Tv = { class: "title" }, Uv = /* @__PURE__ */ dv({
  __name: "SituationCard",
  props: {
    alarmInfo: null,
    selected: { type: Boolean }
  },
  emits: ["situation-selected"],
  setup(n, { emit: i }) {
    const a = n, l = () => {
      var c;
      i("situation-selected", (c = a.alarmInfo) == null ? void 0 : c.id);
    };
    return (c, f) => {
      var p, V, v, k, J;
      return hv(), pv("div", {
        onClick: l,
        class: xu(["card", { selected: a.selected }])
      }, [
        Aa("div", {
          class: xu(["severity-line", [`${(V = (p = a.alarmInfo) == null ? void 0 : p.severity) == null ? void 0 : V.toLowerCase()}-bg dark`]])
        }, null, 2),
        Aa("div", mv, [
          Aa("div", Vv, [
            Aa("div", Tv, "[ " + fv((v = a.alarmInfo) == null ? void 0 : v.id) + " ]", 1),
            Qu(Zl, {
              severity: (k = a.alarmInfo) == null ? void 0 : k.severity
            }, null, 8, ["severity"])
          ]),
          Qu(nf, {
            relatedAlarms: (J = a.alarmInfo) == null ? void 0 : J.relatedAlarms,
            size: "normal"
          }, null, 8, ["relatedAlarms"])
        ])
      ], 2);
    };
  }
});
const Rv = /* @__PURE__ */ Pe(Uv, [["__scopeId", "data-v-34bf806c"]]);
const xa = window.Vue.ref, vv = window.Vue.inject, Nv = window.Vue.computed, Mv = window.Vue.onMounted, wv = {
  id: {
    type: String
  },
  controls: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, Zv = (n) => {
  const i = xa(!1), a = xa(), l = xa(n.controls), c = xa(n.id), f = () => {
    a.value && a.value.focus();
  }, p = vv("registerTab");
  Mv(() => {
    if (a.value && p) {
      const v = a.value.parentElement, k = v && v.parentElement ? v.parentElement : void 0, J = Array.from(k ? k.children : []).filter((g) => g.querySelectorAll("[role=tab]").length), y = v ? J.indexOf(v) : -1;
      p({
        el: a.value,
        focus: f,
        disabled: n.disabled,
        selected: i,
        id: c,
        controls: l,
        index: y
      });
    }
  });
  const V = Nv(() => ({
    role: "tab",
    ref: "tab",
    tabindex: i.value ? 0 : -1,
    id: c.value,
    "aria-selected": i.value,
    "aria-controls": l.value,
    "aria-disabled": n.disabled,
    "data-ref-id": "feather-tab"
  }));
  return {
    selected: i,
    attrs: V,
    tab: a
  };
};
const Jv = function(n, i) {
  return window ? window.setTimeout(n, i) : setTimeout(n, i);
}, gv = function(n) {
  return window ? window.clearTimeout(n) : clearTimeout(n);
};
var kv = Object.defineProperty, yv = Object.defineProperties, bv = Object.getOwnPropertyDescriptors, Bu = Object.getOwnPropertySymbols, Wv = Object.prototype.hasOwnProperty, Ev = Object.prototype.propertyIsEnumerable, Du = (n, i, a) => i in n ? kv(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Ou = (n, i) => {
  for (var a in i || (i = {}))
    Wv.call(i, a) && Du(n, a, i[a]);
  if (Bu)
    for (var a of Bu(i))
      Ev.call(i, a) && Du(n, a, i[a]);
  return n;
}, Fv = (n, i) => yv(n, bv(i));
const Sv = window.Vue.defineComponent, Av = window.Vue.h;
var xv = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Qv = {
  center: {
    type: Boolean,
    default: !1
  }
}, Bv = Sv({
  props: Qv,
  data() {
    return {
      pressed: !1,
      active: !1,
      styles: {},
      failsafe: -1
    };
  },
  computed: {
    parent() {
      return this.$el.parentNode;
    }
  },
  methods: {
    onClick(n) {
      this.pressed = !1, this.active = !1, requestAnimationFrame(() => {
        const { clientWidth: i, clientHeight: a } = this.parent, l = Math.round(Math.max(i, a));
        let c = {
          top: "0px",
          left: "0px"
        };
        if (!this.center) {
          const f = this.parent.getBoundingClientRect(), p = n.pageY, V = n.pageX;
          c = {
            top: `${p - f.top - l / 2 - document.documentElement.scrollTop}px`,
            left: `${V - f.left - l / 2 - document.documentElement.scrollLeft}px`
          };
        }
        this.styles = Fv(Ou({}, c), {
          height: `${l}px`,
          width: `${l}px`
        }), this.pressed = !0, requestAnimationFrame(() => {
          this.active = !0, gv(this.failsafe), this.failsafe = Jv(() => {
            this.pressed = !1, this.active = !1;
          }, 380 + 100 + 20);
        });
      });
    }
  },
  render() {
    if (this.pressed !== !1)
      return Av("div", {
        style: Ou({}, this.styles),
        class: ["ripple", { active: this.active, center: this.center }],
        onTransitionEnd: () => {
          this.pressed = !1, this.active = !1;
        },
        onTransitionCancel: () => {
          this.pressed = !1, this.active = !1;
        }
      });
  },
  mounted() {
    this.parent.addEventListener("click", this.onClick);
    const n = window.getComputedStyle(this.parent);
    this.parent.style.overflow = "hidden", !(n.position === "relative" || n.position === "absolute" || n.position === "fixed") && (this.parent.style.position = "relative");
  },
  unmounted() {
    this.parent.removeEventListener("click", this.onClick);
  }
});
var Ln = /* @__PURE__ */ xv(Bv, [["__scopeId", "data-v-18e2a5db"]]);
const Ie = function(n) {
  n = n || "feather";
  const i = Math.floor(Math.random() * 1e9);
  return [n.replace(/\s+/g, "-"), Date.now(), i].join("-");
}, Qt = {
  PAGEUP: 33,
  PAGEDOWN: 34,
  UP: 38,
  DOWN: 40,
  LEFT: 37,
  RIGHT: 39,
  HOME: 36,
  END: 35,
  ENTER: 13,
  SPACE: 32,
  ESCAPE: 27,
  BACKSPACE: 8,
  DELETE: 46
}, Gu = window.Vue.ref, Dv = window.Vue.toRef, Ov = window.Vue.watch, Cu = window.Vue.provide, Gv = {
  prop: "modelValue",
  event: "update:modelValue"
}, Cv = {
  "update:modelValue": (n) => !0
}, zv = {
  modelValue: {
    type: Number,
    default: 0
  },
  vertical: {
    type: Boolean,
    default: !0
  }
}, Yv = (n, i) => {
  const a = Dv(n, "modelValue"), l = Gu(n.modelValue), c = Gu([]);
  Ov(a, (S) => {
    v(S);
  });
  const f = (S) => {
    S.preventDefault(), c.value.some((O, G) => O.tab && O.tab.el.contains(S.target) ? (V(G), v(G), !0) : !1);
  }, p = (S) => {
    if (((ee) => ee.shiftKey || ee.ctrlKey || ee.metaKey || ee.altKey)(S))
      return;
    const G = S.keyCode, B = (ee) => {
      ee.stopPropagation(), ee.preventDefault();
    }, _ = c.value.filter((ee) => ee.tab && !ee.tab.disabled), ne = c.value.findIndex((ee) => ee.tab && ee.tab.el.contains(document.activeElement));
    let oe = ne !== -1 ? ne : l.value;
    const F = [Qt.RIGHT], Ue = [Qt.LEFT], fe = [Qt.ENTER, Qt.SPACE];
    n.vertical && (F.push(Qt.DOWN), Ue.push(Qt.UP)), F.indexOf(G) > -1 ? (oe++, oe >= _.length && (oe = 0), B(S), V(c.value.indexOf(_[oe]))) : Ue.indexOf(G) > -1 && (oe--, oe < 0 && (oe = _.length - 1), B(S), V(c.value.indexOf(_[oe]))), fe.indexOf(G) > -1 && v(oe);
  }, V = (S) => {
    c.value.forEach(function(O, G) {
      S === G && O.tab && O.tab.focus();
    });
  }, v = (S) => {
    const O = c.value[S];
    !O || O.tab && O.tab.disabled || (c.value.forEach((G, B) => {
      G.tab && (G.tab.selected = S === B), G.panel && (G.panel.selected = S === B);
    }), l.value = S, i.emit("update:modelValue", S));
  };
  Cu("registerTab", (S) => {
    const O = S.index;
    O > -1 && (c.value[O] = { ...c.value[O], tab: S }, c.value = [...c.value], y());
  }), Cu("registerPanel", (S) => {
    const O = S.index;
    O > -1 && (c.value[O] = {
      ...c.value[O],
      panel: S
    }, c.value = [...c.value], y());
  });
  const y = () => {
    c.value.forEach(({ tab: S, panel: O }, G) => {
      if (O && S) {
        const B = S.id || Ie("tab"), _ = S.controls || Ie("panel");
        S.controls = _, S.id = B, O.tab = B, O.id = _;
      }
      G === l.value && (O && (O.selected = !0), S && (S.selected = !0));
    });
  };
  return {
    listeners: {
      click: f,
      keydown: p
    },
    attrs: {
      role: "tablist"
    },
    selected: l,
    pairs: c
  };
}, Qa = window.Vue.ref, Iv = window.Vue.inject, Hv = window.Vue.computed, Pv = window.Vue.onMounted, jv = {
  id: {
    type: String
  },
  tab: {
    type: String
  }
}, Xv = (n) => {
  const i = Qa(!1), a = Qa(), l = Qa(n.tab), c = Qa(n.id), f = Iv("registerPanel");
  Pv(() => {
    if (f) {
      const V = a.value, v = V && V.parentElement ? V.parentElement : void 0, k = V ? Array.from(v ? v.children : []).indexOf(V) : -1;
      f({
        selected: i,
        id: c,
        tab: l,
        el: a.value,
        index: k
      });
    }
  });
  const p = Hv(() => ({
    role: "tabpanel",
    id: c.value,
    ref: "panel",
    tabindex: "0",
    "aria-expanded": i.value,
    "aria-labelledby": l.value,
    "data-ref-id": "feather-tab-panel"
  }));
  return {
    selected: i,
    attrs: p,
    panel: a
  };
}, Jl = window.Vue.defineComponent, Lv = window.Vue.resolveComponent, gl = window.Vue.openBlock, kl = window.Vue.createElementBlock, Er = window.Vue.createElementVNode, rf = window.Vue.mergeProps, ja = window.Vue.renderSlot, _v = window.Vue.createVNode, qv = window.Vue.normalizeStyle, Kv = window.Vue.toHandlers, $v = window.Vue.withDirectives, eN = window.Vue.normalizeProps, tN = window.Vue.guardReactiveProps, nN = window.Vue.vShow;
var yl = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const rN = wv, aN = Jl({
  props: rN,
  setup(n) {
    return Zv(n);
  },
  components: {
    FeatherRipple: Ln
  }
}), iN = { role: "presentation" }, oN = { class: "tab-text" };
function lN(n, i, a, l, c, f) {
  const p = Lv("FeatherRipple");
  return gl(), kl("li", iN, [
    Er("button", rf(n.attrs, {
      class: ["tab hover focus", { disabled: n.disabled, selected: n.selected }]
    }), [
      Er("span", oN, [
        ja(n.$slots, "default", {}, void 0, !0)
      ]),
      _v(p)
    ], 16)
  ]);
}
var zu = /* @__PURE__ */ yl(aN, [["render", lN], ["__scopeId", "data-v-e6bb52b6"]]);
const sN = zv, cN = Cv, uN = Jl({
  model: Gv,
  emits: cN,
  props: sN,
  setup(n, i) {
    return Yv(n, i);
  },
  data: () => ({
    transform: "",
    durationNumber: 250,
    width: "1px",
    ro: void 0
  }),
  watch: {
    selected() {
      this.updateSlider();
    },
    pairs: {
      handler(n) {
        n && n.length && this.ro && n.forEach((i) => {
          i.tab && this.ro.observe(i.tab.el);
        });
      },
      immediate: !0
    }
  },
  computed: {
    duration() {
      return `${this.durationNumber}ms`;
    }
  },
  methods: {
    updateSlider() {
      this.$refs.slider.getBoundingClientRect().width < 5 ? this.durationNumber = 0 : this.durationNumber = 250, this.$nextTick(() => {
        const n = this.$el.getBoundingClientRect(), i = this.$el.querySelector("[aria-selected='true']").getBoundingClientRect(), a = i.left - n.left, l = i.height - 2;
        this.width = `${i.width}px`, this.transform = `translateX(${a}px) translateY(${l}px)`;
      });
    }
  },
  mounted() {
    this.updateSlider(), this.ro = new ResizeObserver(() => {
      this.updateSlider();
    });
  },
  unmounted() {
    this.ro.disconnect();
  }
}), dN = { class: "feather-tab-container" }, fN = { class: "tab-panels" };
function hN(n, i, a, l, c, f) {
  return gl(), kl("div", dN, [
    Er("div", {
      ref: "slider",
      class: "feather-tab-slider",
      style: qv({
        transform: n.transform,
        "transition-duration": n.duration,
        width: n.width
      })
    }, null, 4),
    Er("ul", rf(n.attrs, Kv(n.listeners)), [
      ja(n.$slots, "tabs", {}, void 0, !0)
    ], 16),
    Er("div", fN, [
      ja(n.$slots, "default", {}, void 0, !0)
    ])
  ]);
}
var pN = /* @__PURE__ */ yl(uN, [["render", hN], ["__scopeId", "data-v-27adffb9"]]);
const mN = jv, VN = Jl({
  props: mN,
  setup(n) {
    return Xv(n);
  }
});
function TN(n, i, a, l, c, f) {
  return $v((gl(), kl("div", eN(tN(n.attrs)), [
    ja(n.$slots, "default")
  ], 16)), [
    [nN, n.selected]
  ]);
}
var Yu = /* @__PURE__ */ yl(VN, [["render", TN]]);
function Tn(n) {
  if (n === null || n === !0 || n === !1)
    return NaN;
  var i = Number(n);
  return isNaN(i) ? i : i < 0 ? Math.ceil(i) : Math.floor(i);
}
function We(n, i) {
  if (i.length < n)
    throw new TypeError(n + " argument" + (n > 1 ? "s" : "") + " required, but only " + i.length + " present");
}
function Nt(n) {
  We(1, arguments);
  var i = Object.prototype.toString.call(n);
  return n instanceof Date || typeof n == "object" && i === "[object Date]" ? new Date(n.getTime()) : typeof n == "number" || i === "[object Number]" ? new Date(n) : ((typeof n == "string" || i === "[object String]") && typeof console < "u" && (console.warn("Starting with v2.0.0-beta.1 date-fns doesn't accept strings as date arguments. Please use `parseISO` to parse strings. See: https://github.com/date-fns/date-fns/blob/master/docs/upgradeGuide.md#string-arguments"), console.warn(new Error().stack)), new Date(NaN));
}
function UN(n, i) {
  We(2, arguments);
  var a = Nt(n).getTime(), l = Tn(i);
  return new Date(a + l);
}
var RN = {};
function ai() {
  return RN;
}
function vN(n) {
  var i = new Date(Date.UTC(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds(), n.getMilliseconds()));
  return i.setUTCFullYear(n.getFullYear()), n.getTime() - i.getTime();
}
function NN(n) {
  return We(1, arguments), n instanceof Date || typeof n == "object" && Object.prototype.toString.call(n) === "[object Date]";
}
function MN(n) {
  if (We(1, arguments), !NN(n) && typeof n != "number")
    return !1;
  var i = Nt(n);
  return !isNaN(Number(i));
}
function wN(n, i) {
  We(2, arguments);
  var a = Tn(i);
  return UN(n, -a);
}
var ZN = 864e5;
function JN(n) {
  We(1, arguments);
  var i = Nt(n), a = i.getTime();
  i.setUTCMonth(0, 1), i.setUTCHours(0, 0, 0, 0);
  var l = i.getTime(), c = a - l;
  return Math.floor(c / ZN) + 1;
}
function Xa(n) {
  We(1, arguments);
  var i = 1, a = Nt(n), l = a.getUTCDay(), c = (l < i ? 7 : 0) + l - i;
  return a.setUTCDate(a.getUTCDate() - c), a.setUTCHours(0, 0, 0, 0), a;
}
function af(n) {
  We(1, arguments);
  var i = Nt(n), a = i.getUTCFullYear(), l = new Date(0);
  l.setUTCFullYear(a + 1, 0, 4), l.setUTCHours(0, 0, 0, 0);
  var c = Xa(l), f = new Date(0);
  f.setUTCFullYear(a, 0, 4), f.setUTCHours(0, 0, 0, 0);
  var p = Xa(f);
  return i.getTime() >= c.getTime() ? a + 1 : i.getTime() >= p.getTime() ? a : a - 1;
}
function gN(n) {
  We(1, arguments);
  var i = af(n), a = new Date(0);
  a.setUTCFullYear(i, 0, 4), a.setUTCHours(0, 0, 0, 0);
  var l = Xa(a);
  return l;
}
var kN = 6048e5;
function yN(n) {
  We(1, arguments);
  var i = Nt(n), a = Xa(i).getTime() - gN(i).getTime();
  return Math.round(a / kN) + 1;
}
function La(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = ai(), y = Tn((a = (l = (c = (f = i == null ? void 0 : i.weekStartsOn) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.weekStartsOn) !== null && c !== void 0 ? c : J.weekStartsOn) !== null && l !== void 0 ? l : (v = J.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.weekStartsOn) !== null && a !== void 0 ? a : 0);
  if (!(y >= 0 && y <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  var g = Nt(n), H = g.getUTCDay(), S = (H < y ? 7 : 0) + H - y;
  return g.setUTCDate(g.getUTCDate() - S), g.setUTCHours(0, 0, 0, 0), g;
}
function of(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = Nt(n), y = J.getUTCFullYear(), g = ai(), H = Tn((a = (l = (c = (f = i == null ? void 0 : i.firstWeekContainsDate) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.firstWeekContainsDate) !== null && c !== void 0 ? c : g.firstWeekContainsDate) !== null && l !== void 0 ? l : (v = g.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.firstWeekContainsDate) !== null && a !== void 0 ? a : 1);
  if (!(H >= 1 && H <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var S = new Date(0);
  S.setUTCFullYear(y + 1, 0, H), S.setUTCHours(0, 0, 0, 0);
  var O = La(S, i), G = new Date(0);
  G.setUTCFullYear(y, 0, H), G.setUTCHours(0, 0, 0, 0);
  var B = La(G, i);
  return J.getTime() >= O.getTime() ? y + 1 : J.getTime() >= B.getTime() ? y : y - 1;
}
function bN(n, i) {
  var a, l, c, f, p, V, v, k;
  We(1, arguments);
  var J = ai(), y = Tn((a = (l = (c = (f = i == null ? void 0 : i.firstWeekContainsDate) !== null && f !== void 0 ? f : i == null || (p = i.locale) === null || p === void 0 || (V = p.options) === null || V === void 0 ? void 0 : V.firstWeekContainsDate) !== null && c !== void 0 ? c : J.firstWeekContainsDate) !== null && l !== void 0 ? l : (v = J.locale) === null || v === void 0 || (k = v.options) === null || k === void 0 ? void 0 : k.firstWeekContainsDate) !== null && a !== void 0 ? a : 1), g = of(n, i), H = new Date(0);
  H.setUTCFullYear(g, 0, y), H.setUTCHours(0, 0, 0, 0);
  var S = La(H, i);
  return S;
}
var WN = 6048e5;
function EN(n, i) {
  We(1, arguments);
  var a = Nt(n), l = La(a, i).getTime() - bN(a, i).getTime();
  return Math.round(l / WN) + 1;
}
function $(n, i) {
  for (var a = n < 0 ? "-" : "", l = Math.abs(n).toString(); l.length < i; )
    l = "0" + l;
  return a + l;
}
var FN = {
  y: function(n, i) {
    var a = n.getUTCFullYear(), l = a > 0 ? a : 1 - a;
    return $(i === "yy" ? l % 100 : l, i.length);
  },
  M: function(n, i) {
    var a = n.getUTCMonth();
    return i === "M" ? String(a + 1) : $(a + 1, 2);
  },
  d: function(n, i) {
    return $(n.getUTCDate(), i.length);
  },
  a: function(n, i) {
    var a = n.getUTCHours() / 12 >= 1 ? "pm" : "am";
    switch (i) {
      case "a":
      case "aa":
        return a.toUpperCase();
      case "aaa":
        return a;
      case "aaaaa":
        return a[0];
      case "aaaa":
      default:
        return a === "am" ? "a.m." : "p.m.";
    }
  },
  h: function(n, i) {
    return $(n.getUTCHours() % 12 || 12, i.length);
  },
  H: function(n, i) {
    return $(n.getUTCHours(), i.length);
  },
  m: function(n, i) {
    return $(n.getUTCMinutes(), i.length);
  },
  s: function(n, i) {
    return $(n.getUTCSeconds(), i.length);
  },
  S: function(n, i) {
    var a = i.length, l = n.getUTCMilliseconds(), c = Math.floor(l * Math.pow(10, a - 3));
    return $(c, i.length);
  }
};
const Xt = FN;
var Dn = {
  am: "am",
  pm: "pm",
  midnight: "midnight",
  noon: "noon",
  morning: "morning",
  afternoon: "afternoon",
  evening: "evening",
  night: "night"
}, SN = {
  G: function(n, i, a) {
    var l = n.getUTCFullYear() > 0 ? 1 : 0;
    switch (i) {
      case "G":
      case "GG":
      case "GGG":
        return a.era(l, {
          width: "abbreviated"
        });
      case "GGGGG":
        return a.era(l, {
          width: "narrow"
        });
      case "GGGG":
      default:
        return a.era(l, {
          width: "wide"
        });
    }
  },
  y: function(n, i, a) {
    if (i === "yo") {
      var l = n.getUTCFullYear(), c = l > 0 ? l : 1 - l;
      return a.ordinalNumber(c, {
        unit: "year"
      });
    }
    return Xt.y(n, i);
  },
  Y: function(n, i, a, l) {
    var c = of(n, l), f = c > 0 ? c : 1 - c;
    if (i === "YY") {
      var p = f % 100;
      return $(p, 2);
    }
    return i === "Yo" ? a.ordinalNumber(f, {
      unit: "year"
    }) : $(f, i.length);
  },
  R: function(n, i) {
    var a = af(n);
    return $(a, i.length);
  },
  u: function(n, i) {
    var a = n.getUTCFullYear();
    return $(a, i.length);
  },
  Q: function(n, i, a) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (i) {
      case "Q":
        return String(l);
      case "QQ":
        return $(l, 2);
      case "Qo":
        return a.ordinalNumber(l, {
          unit: "quarter"
        });
      case "QQQ":
        return a.quarter(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "QQQQQ":
        return a.quarter(l, {
          width: "narrow",
          context: "formatting"
        });
      case "QQQQ":
      default:
        return a.quarter(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  q: function(n, i, a) {
    var l = Math.ceil((n.getUTCMonth() + 1) / 3);
    switch (i) {
      case "q":
        return String(l);
      case "qq":
        return $(l, 2);
      case "qo":
        return a.ordinalNumber(l, {
          unit: "quarter"
        });
      case "qqq":
        return a.quarter(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "qqqqq":
        return a.quarter(l, {
          width: "narrow",
          context: "standalone"
        });
      case "qqqq":
      default:
        return a.quarter(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  M: function(n, i, a) {
    var l = n.getUTCMonth();
    switch (i) {
      case "M":
      case "MM":
        return Xt.M(n, i);
      case "Mo":
        return a.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "MMM":
        return a.month(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "MMMMM":
        return a.month(l, {
          width: "narrow",
          context: "formatting"
        });
      case "MMMM":
      default:
        return a.month(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  L: function(n, i, a) {
    var l = n.getUTCMonth();
    switch (i) {
      case "L":
        return String(l + 1);
      case "LL":
        return $(l + 1, 2);
      case "Lo":
        return a.ordinalNumber(l + 1, {
          unit: "month"
        });
      case "LLL":
        return a.month(l, {
          width: "abbreviated",
          context: "standalone"
        });
      case "LLLLL":
        return a.month(l, {
          width: "narrow",
          context: "standalone"
        });
      case "LLLL":
      default:
        return a.month(l, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  w: function(n, i, a, l) {
    var c = EN(n, l);
    return i === "wo" ? a.ordinalNumber(c, {
      unit: "week"
    }) : $(c, i.length);
  },
  I: function(n, i, a) {
    var l = yN(n);
    return i === "Io" ? a.ordinalNumber(l, {
      unit: "week"
    }) : $(l, i.length);
  },
  d: function(n, i, a) {
    return i === "do" ? a.ordinalNumber(n.getUTCDate(), {
      unit: "date"
    }) : Xt.d(n, i);
  },
  D: function(n, i, a) {
    var l = JN(n);
    return i === "Do" ? a.ordinalNumber(l, {
      unit: "dayOfYear"
    }) : $(l, i.length);
  },
  E: function(n, i, a) {
    var l = n.getUTCDay();
    switch (i) {
      case "E":
      case "EE":
      case "EEE":
        return a.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "EEEEE":
        return a.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "EEEEEE":
        return a.day(l, {
          width: "short",
          context: "formatting"
        });
      case "EEEE":
      default:
        return a.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  e: function(n, i, a, l) {
    var c = n.getUTCDay(), f = (c - l.weekStartsOn + 8) % 7 || 7;
    switch (i) {
      case "e":
        return String(f);
      case "ee":
        return $(f, 2);
      case "eo":
        return a.ordinalNumber(f, {
          unit: "day"
        });
      case "eee":
        return a.day(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "eeeee":
        return a.day(c, {
          width: "narrow",
          context: "formatting"
        });
      case "eeeeee":
        return a.day(c, {
          width: "short",
          context: "formatting"
        });
      case "eeee":
      default:
        return a.day(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  c: function(n, i, a, l) {
    var c = n.getUTCDay(), f = (c - l.weekStartsOn + 8) % 7 || 7;
    switch (i) {
      case "c":
        return String(f);
      case "cc":
        return $(f, i.length);
      case "co":
        return a.ordinalNumber(f, {
          unit: "day"
        });
      case "ccc":
        return a.day(c, {
          width: "abbreviated",
          context: "standalone"
        });
      case "ccccc":
        return a.day(c, {
          width: "narrow",
          context: "standalone"
        });
      case "cccccc":
        return a.day(c, {
          width: "short",
          context: "standalone"
        });
      case "cccc":
      default:
        return a.day(c, {
          width: "wide",
          context: "standalone"
        });
    }
  },
  i: function(n, i, a) {
    var l = n.getUTCDay(), c = l === 0 ? 7 : l;
    switch (i) {
      case "i":
        return String(c);
      case "ii":
        return $(c, i.length);
      case "io":
        return a.ordinalNumber(c, {
          unit: "day"
        });
      case "iii":
        return a.day(l, {
          width: "abbreviated",
          context: "formatting"
        });
      case "iiiii":
        return a.day(l, {
          width: "narrow",
          context: "formatting"
        });
      case "iiiiii":
        return a.day(l, {
          width: "short",
          context: "formatting"
        });
      case "iiii":
      default:
        return a.day(l, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  a: function(n, i, a) {
    var l = n.getUTCHours(), c = l / 12 >= 1 ? "pm" : "am";
    switch (i) {
      case "a":
      case "aa":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "aaa":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "aaaaa":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "aaaa":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  b: function(n, i, a) {
    var l = n.getUTCHours(), c;
    switch (l === 12 ? c = Dn.noon : l === 0 ? c = Dn.midnight : c = l / 12 >= 1 ? "pm" : "am", i) {
      case "b":
      case "bb":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "bbb":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        }).toLowerCase();
      case "bbbbb":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "bbbb":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  B: function(n, i, a) {
    var l = n.getUTCHours(), c;
    switch (l >= 17 ? c = Dn.evening : l >= 12 ? c = Dn.afternoon : l >= 4 ? c = Dn.morning : c = Dn.night, i) {
      case "B":
      case "BB":
      case "BBB":
        return a.dayPeriod(c, {
          width: "abbreviated",
          context: "formatting"
        });
      case "BBBBB":
        return a.dayPeriod(c, {
          width: "narrow",
          context: "formatting"
        });
      case "BBBB":
      default:
        return a.dayPeriod(c, {
          width: "wide",
          context: "formatting"
        });
    }
  },
  h: function(n, i, a) {
    if (i === "ho") {
      var l = n.getUTCHours() % 12;
      return l === 0 && (l = 12), a.ordinalNumber(l, {
        unit: "hour"
      });
    }
    return Xt.h(n, i);
  },
  H: function(n, i, a) {
    return i === "Ho" ? a.ordinalNumber(n.getUTCHours(), {
      unit: "hour"
    }) : Xt.H(n, i);
  },
  K: function(n, i, a) {
    var l = n.getUTCHours() % 12;
    return i === "Ko" ? a.ordinalNumber(l, {
      unit: "hour"
    }) : $(l, i.length);
  },
  k: function(n, i, a) {
    var l = n.getUTCHours();
    return l === 0 && (l = 24), i === "ko" ? a.ordinalNumber(l, {
      unit: "hour"
    }) : $(l, i.length);
  },
  m: function(n, i, a) {
    return i === "mo" ? a.ordinalNumber(n.getUTCMinutes(), {
      unit: "minute"
    }) : Xt.m(n, i);
  },
  s: function(n, i, a) {
    return i === "so" ? a.ordinalNumber(n.getUTCSeconds(), {
      unit: "second"
    }) : Xt.s(n, i);
  },
  S: function(n, i) {
    return Xt.S(n, i);
  },
  X: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    if (f === 0)
      return "Z";
    switch (i) {
      case "X":
        return Hu(f);
      case "XXXX":
      case "XX":
        return mn(f);
      case "XXXXX":
      case "XXX":
      default:
        return mn(f, ":");
    }
  },
  x: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "x":
        return Hu(f);
      case "xxxx":
      case "xx":
        return mn(f);
      case "xxxxx":
      case "xxx":
      default:
        return mn(f, ":");
    }
  },
  O: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "O":
      case "OO":
      case "OOO":
        return "GMT" + Iu(f, ":");
      case "OOOO":
      default:
        return "GMT" + mn(f, ":");
    }
  },
  z: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTimezoneOffset();
    switch (i) {
      case "z":
      case "zz":
      case "zzz":
        return "GMT" + Iu(f, ":");
      case "zzzz":
      default:
        return "GMT" + mn(f, ":");
    }
  },
  t: function(n, i, a, l) {
    var c = l._originalDate || n, f = Math.floor(c.getTime() / 1e3);
    return $(f, i.length);
  },
  T: function(n, i, a, l) {
    var c = l._originalDate || n, f = c.getTime();
    return $(f, i.length);
  }
};
function Iu(n, i) {
  var a = n > 0 ? "-" : "+", l = Math.abs(n), c = Math.floor(l / 60), f = l % 60;
  if (f === 0)
    return a + String(c);
  var p = i || "";
  return a + String(c) + p + $(f, 2);
}
function Hu(n, i) {
  if (n % 60 === 0) {
    var a = n > 0 ? "-" : "+";
    return a + $(Math.abs(n) / 60, 2);
  }
  return mn(n, i);
}
function mn(n, i) {
  var a = i || "", l = n > 0 ? "-" : "+", c = Math.abs(n), f = $(Math.floor(c / 60), 2), p = $(c % 60, 2);
  return l + f + a + p;
}
const AN = SN;
var Pu = function(n, i) {
  switch (n) {
    case "P":
      return i.date({
        width: "short"
      });
    case "PP":
      return i.date({
        width: "medium"
      });
    case "PPP":
      return i.date({
        width: "long"
      });
    case "PPPP":
    default:
      return i.date({
        width: "full"
      });
  }
}, lf = function(n, i) {
  switch (n) {
    case "p":
      return i.time({
        width: "short"
      });
    case "pp":
      return i.time({
        width: "medium"
      });
    case "ppp":
      return i.time({
        width: "long"
      });
    case "pppp":
    default:
      return i.time({
        width: "full"
      });
  }
}, xN = function(n, i) {
  var a = n.match(/(P+)(p+)?/) || [], l = a[1], c = a[2];
  if (!c)
    return Pu(n, i);
  var f;
  switch (l) {
    case "P":
      f = i.dateTime({
        width: "short"
      });
      break;
    case "PP":
      f = i.dateTime({
        width: "medium"
      });
      break;
    case "PPP":
      f = i.dateTime({
        width: "long"
      });
      break;
    case "PPPP":
    default:
      f = i.dateTime({
        width: "full"
      });
      break;
  }
  return f.replace("{{date}}", Pu(l, i)).replace("{{time}}", lf(c, i));
}, QN = {
  p: lf,
  P: xN
};
const BN = QN;
var DN = ["D", "DD"], ON = ["YY", "YYYY"];
function GN(n) {
  return DN.indexOf(n) !== -1;
}
function CN(n) {
  return ON.indexOf(n) !== -1;
}
function ju(n, i, a) {
  if (n === "YYYY")
    throw new RangeError("Use `yyyy` instead of `YYYY` (in `".concat(i, "`) for formatting years to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "YY")
    throw new RangeError("Use `yy` instead of `YY` (in `".concat(i, "`) for formatting years to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "D")
    throw new RangeError("Use `d` instead of `D` (in `".concat(i, "`) for formatting days of the month to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
  if (n === "DD")
    throw new RangeError("Use `dd` instead of `DD` (in `".concat(i, "`) for formatting days of the month to the input `").concat(a, "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"));
}
var zN = {
  lessThanXSeconds: {
    one: "less than a second",
    other: "less than {{count}} seconds"
  },
  xSeconds: {
    one: "1 second",
    other: "{{count}} seconds"
  },
  halfAMinute: "half a minute",
  lessThanXMinutes: {
    one: "less than a minute",
    other: "less than {{count}} minutes"
  },
  xMinutes: {
    one: "1 minute",
    other: "{{count}} minutes"
  },
  aboutXHours: {
    one: "about 1 hour",
    other: "about {{count}} hours"
  },
  xHours: {
    one: "1 hour",
    other: "{{count}} hours"
  },
  xDays: {
    one: "1 day",
    other: "{{count}} days"
  },
  aboutXWeeks: {
    one: "about 1 week",
    other: "about {{count}} weeks"
  },
  xWeeks: {
    one: "1 week",
    other: "{{count}} weeks"
  },
  aboutXMonths: {
    one: "about 1 month",
    other: "about {{count}} months"
  },
  xMonths: {
    one: "1 month",
    other: "{{count}} months"
  },
  aboutXYears: {
    one: "about 1 year",
    other: "about {{count}} years"
  },
  xYears: {
    one: "1 year",
    other: "{{count}} years"
  },
  overXYears: {
    one: "over 1 year",
    other: "over {{count}} years"
  },
  almostXYears: {
    one: "almost 1 year",
    other: "almost {{count}} years"
  }
}, YN = function(n, i, a) {
  var l, c = zN[n];
  return typeof c == "string" ? l = c : i === 1 ? l = c.one : l = c.other.replace("{{count}}", i.toString()), a != null && a.addSuffix ? a.comparison && a.comparison > 0 ? "in " + l : l + " ago" : l;
};
const IN = YN;
function Po(n) {
  return function() {
    var i = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {}, a = i.width ? String(i.width) : n.defaultWidth, l = n.formats[a] || n.formats[n.defaultWidth];
    return l;
  };
}
var HN = {
  full: "EEEE, MMMM do, y",
  long: "MMMM do, y",
  medium: "MMM d, y",
  short: "MM/dd/yyyy"
}, PN = {
  full: "h:mm:ss a zzzz",
  long: "h:mm:ss a z",
  medium: "h:mm:ss a",
  short: "h:mm a"
}, jN = {
  full: "{{date}} 'at' {{time}}",
  long: "{{date}} 'at' {{time}}",
  medium: "{{date}}, {{time}}",
  short: "{{date}}, {{time}}"
}, XN = {
  date: Po({
    formats: HN,
    defaultWidth: "full"
  }),
  time: Po({
    formats: PN,
    defaultWidth: "full"
  }),
  dateTime: Po({
    formats: jN,
    defaultWidth: "full"
  })
};
const LN = XN;
var _N = {
  lastWeek: "'last' eeee 'at' p",
  yesterday: "'yesterday at' p",
  today: "'today at' p",
  tomorrow: "'tomorrow at' p",
  nextWeek: "eeee 'at' p",
  other: "P"
}, qN = function(n, i, a, l) {
  return _N[n];
};
const KN = qN;
function Mr(n) {
  return function(i, a) {
    var l = a != null && a.context ? String(a.context) : "standalone", c;
    if (l === "formatting" && n.formattingValues) {
      var f = n.defaultFormattingWidth || n.defaultWidth, p = a != null && a.width ? String(a.width) : f;
      c = n.formattingValues[p] || n.formattingValues[f];
    } else {
      var V = n.defaultWidth, v = a != null && a.width ? String(a.width) : n.defaultWidth;
      c = n.values[v] || n.values[V];
    }
    var k = n.argumentCallback ? n.argumentCallback(i) : i;
    return c[k];
  };
}
var $N = {
  narrow: ["B", "A"],
  abbreviated: ["BC", "AD"],
  wide: ["Before Christ", "Anno Domini"]
}, eM = {
  narrow: ["1", "2", "3", "4"],
  abbreviated: ["Q1", "Q2", "Q3", "Q4"],
  wide: ["1st quarter", "2nd quarter", "3rd quarter", "4th quarter"]
}, tM = {
  narrow: ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"],
  abbreviated: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  wide: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
}, nM = {
  narrow: ["S", "M", "T", "W", "T", "F", "S"],
  short: ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"],
  abbreviated: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
  wide: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
}, rM = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night"
  }
}, aM = {
  narrow: {
    am: "a",
    pm: "p",
    midnight: "mi",
    noon: "n",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  abbreviated: {
    am: "AM",
    pm: "PM",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  },
  wide: {
    am: "a.m.",
    pm: "p.m.",
    midnight: "midnight",
    noon: "noon",
    morning: "in the morning",
    afternoon: "in the afternoon",
    evening: "in the evening",
    night: "at night"
  }
}, iM = function(n, i) {
  var a = Number(n), l = a % 100;
  if (l > 20 || l < 10)
    switch (l % 10) {
      case 1:
        return a + "st";
      case 2:
        return a + "nd";
      case 3:
        return a + "rd";
    }
  return a + "th";
}, oM = {
  ordinalNumber: iM,
  era: Mr({
    values: $N,
    defaultWidth: "wide"
  }),
  quarter: Mr({
    values: eM,
    defaultWidth: "wide",
    argumentCallback: function(n) {
      return n - 1;
    }
  }),
  month: Mr({
    values: tM,
    defaultWidth: "wide"
  }),
  day: Mr({
    values: nM,
    defaultWidth: "wide"
  }),
  dayPeriod: Mr({
    values: rM,
    defaultWidth: "wide",
    formattingValues: aM,
    defaultFormattingWidth: "wide"
  })
};
const lM = oM;
function wr(n) {
  return function(i) {
    var a = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = a.width, c = l && n.matchPatterns[l] || n.matchPatterns[n.defaultMatchWidth], f = i.match(c);
    if (!f)
      return null;
    var p = f[0], V = l && n.parsePatterns[l] || n.parsePatterns[n.defaultParseWidth], v = Array.isArray(V) ? cM(V, function(y) {
      return y.test(p);
    }) : sM(V, function(y) {
      return y.test(p);
    }), k;
    k = n.valueCallback ? n.valueCallback(v) : v, k = a.valueCallback ? a.valueCallback(k) : k;
    var J = i.slice(p.length);
    return {
      value: k,
      rest: J
    };
  };
}
function sM(n, i) {
  for (var a in n)
    if (n.hasOwnProperty(a) && i(n[a]))
      return a;
}
function cM(n, i) {
  for (var a = 0; a < n.length; a++)
    if (i(n[a]))
      return a;
}
function uM(n) {
  return function(i) {
    var a = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, l = i.match(n.matchPattern);
    if (!l)
      return null;
    var c = l[0], f = i.match(n.parsePattern);
    if (!f)
      return null;
    var p = n.valueCallback ? n.valueCallback(f[0]) : f[0];
    p = a.valueCallback ? a.valueCallback(p) : p;
    var V = i.slice(c.length);
    return {
      value: p,
      rest: V
    };
  };
}
var dM = /^(\d+)(th|st|nd|rd)?/i, fM = /\d+/i, hM = {
  narrow: /^(b|a)/i,
  abbreviated: /^(b\.?\s?c\.?|b\.?\s?c\.?\s?e\.?|a\.?\s?d\.?|c\.?\s?e\.?)/i,
  wide: /^(before christ|before common era|anno domini|common era)/i
}, pM = {
  any: [/^b/i, /^(a|c)/i]
}, mM = {
  narrow: /^[1234]/i,
  abbreviated: /^q[1234]/i,
  wide: /^[1234](th|st|nd|rd)? quarter/i
}, VM = {
  any: [/1/i, /2/i, /3/i, /4/i]
}, TM = {
  narrow: /^[jfmasond]/i,
  abbreviated: /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i,
  wide: /^(january|february|march|april|may|june|july|august|september|october|november|december)/i
}, UM = {
  narrow: [/^j/i, /^f/i, /^m/i, /^a/i, /^m/i, /^j/i, /^j/i, /^a/i, /^s/i, /^o/i, /^n/i, /^d/i],
  any: [/^ja/i, /^f/i, /^mar/i, /^ap/i, /^may/i, /^jun/i, /^jul/i, /^au/i, /^s/i, /^o/i, /^n/i, /^d/i]
}, RM = {
  narrow: /^[smtwf]/i,
  short: /^(su|mo|tu|we|th|fr|sa)/i,
  abbreviated: /^(sun|mon|tue|wed|thu|fri|sat)/i,
  wide: /^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)/i
}, vM = {
  narrow: [/^s/i, /^m/i, /^t/i, /^w/i, /^t/i, /^f/i, /^s/i],
  any: [/^su/i, /^m/i, /^tu/i, /^w/i, /^th/i, /^f/i, /^sa/i]
}, NM = {
  narrow: /^(a|p|mi|n|(in the|at) (morning|afternoon|evening|night))/i,
  any: /^([ap]\.?\s?m\.?|midnight|noon|(in the|at) (morning|afternoon|evening|night))/i
}, MM = {
  any: {
    am: /^a/i,
    pm: /^p/i,
    midnight: /^mi/i,
    noon: /^no/i,
    morning: /morning/i,
    afternoon: /afternoon/i,
    evening: /evening/i,
    night: /night/i
  }
}, wM = {
  ordinalNumber: uM({
    matchPattern: dM,
    parsePattern: fM,
    valueCallback: function(n) {
      return parseInt(n, 10);
    }
  }),
  era: wr({
    matchPatterns: hM,
    defaultMatchWidth: "wide",
    parsePatterns: pM,
    defaultParseWidth: "any"
  }),
  quarter: wr({
    matchPatterns: mM,
    defaultMatchWidth: "wide",
    parsePatterns: VM,
    defaultParseWidth: "any",
    valueCallback: function(n) {
      return n + 1;
    }
  }),
  month: wr({
    matchPatterns: TM,
    defaultMatchWidth: "wide",
    parsePatterns: UM,
    defaultParseWidth: "any"
  }),
  day: wr({
    matchPatterns: RM,
    defaultMatchWidth: "wide",
    parsePatterns: vM,
    defaultParseWidth: "any"
  }),
  dayPeriod: wr({
    matchPatterns: NM,
    defaultMatchWidth: "any",
    parsePatterns: MM,
    defaultParseWidth: "any"
  })
};
const ZM = wM;
var JM = {
  code: "en-US",
  formatDistance: IN,
  formatLong: LN,
  formatRelative: KN,
  localize: lM,
  match: ZM,
  options: {
    weekStartsOn: 0,
    firstWeekContainsDate: 1
  }
};
const gM = JM;
var kM = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, yM = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, bM = /^'([^]*?)'?$/, WM = /''/g, EM = /[a-zA-Z]/;
function sl(n, i, a) {
  var l, c, f, p, V, v, k, J, y, g, H, S, O, G, B, _, ne, oe;
  We(2, arguments);
  var F = String(i), Ue = ai(), fe = (l = (c = a == null ? void 0 : a.locale) !== null && c !== void 0 ? c : Ue.locale) !== null && l !== void 0 ? l : gM, ee = Tn((f = (p = (V = (v = a == null ? void 0 : a.firstWeekContainsDate) !== null && v !== void 0 ? v : a == null || (k = a.locale) === null || k === void 0 || (J = k.options) === null || J === void 0 ? void 0 : J.firstWeekContainsDate) !== null && V !== void 0 ? V : Ue.firstWeekContainsDate) !== null && p !== void 0 ? p : (y = Ue.locale) === null || y === void 0 || (g = y.options) === null || g === void 0 ? void 0 : g.firstWeekContainsDate) !== null && f !== void 0 ? f : 1);
  if (!(ee >= 1 && ee <= 7))
    throw new RangeError("firstWeekContainsDate must be between 1 and 7 inclusively");
  var ht = Tn((H = (S = (O = (G = a == null ? void 0 : a.weekStartsOn) !== null && G !== void 0 ? G : a == null || (B = a.locale) === null || B === void 0 || (_ = B.options) === null || _ === void 0 ? void 0 : _.weekStartsOn) !== null && O !== void 0 ? O : Ue.weekStartsOn) !== null && S !== void 0 ? S : (ne = Ue.locale) === null || ne === void 0 || (oe = ne.options) === null || oe === void 0 ? void 0 : oe.weekStartsOn) !== null && H !== void 0 ? H : 0);
  if (!(ht >= 0 && ht <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  if (!fe.localize)
    throw new RangeError("locale must contain localize property");
  if (!fe.formatLong)
    throw new RangeError("locale must contain formatLong property");
  var Mt = Nt(n);
  if (!MN(Mt))
    throw new RangeError("Invalid time value");
  var rn = vN(Mt), Je = wN(Mt, rn), pt = {
    firstWeekContainsDate: ee,
    weekStartsOn: ht,
    locale: fe,
    _originalDate: Mt
  }, je = F.match(yM).map(function(Re) {
    var Ee = Re[0];
    if (Ee === "p" || Ee === "P") {
      var Xe = BN[Ee];
      return Xe(Re, fe.formatLong);
    }
    return Re;
  }).join("").match(kM).map(function(Re) {
    if (Re === "''")
      return "'";
    var Ee = Re[0];
    if (Ee === "'")
      return FM(Re);
    var Xe = AN[Ee];
    if (Xe)
      return !(a != null && a.useAdditionalWeekYearTokens) && CN(Re) && ju(Re, i, String(n)), !(a != null && a.useAdditionalDayOfYearTokens) && GN(Re) && ju(Re, i, String(n)), Xe(Je, Re, fe.localize, pt);
    if (Ee.match(EM))
      throw new RangeError("Format string contains an unescaped latin alphabet character `" + Ee + "`");
    return Re;
  }).join("");
  return je;
}
function FM(n) {
  var i = n.match(bM);
  return i ? i[1].replace(WM, "'") : n;
}
const SM = window.Vue.defineComponent, Xu = window.Vue.toDisplayString, Lu = window.Vue.createElementVNode, AM = window.Vue.unref, xM = window.Vue.openBlock, QM = window.Vue.createElementBlock;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const BM = { class: "box" }, DM = { class: "label" }, OM = { class: "date" }, GM = /* @__PURE__ */ SM({
  __name: "DateBox",
  props: {
    label: null,
    date: null
  },
  setup(n) {
    const i = n;
    return (a, l) => (xM(), QM("div", BM, [
      Lu("div", DM, Xu(i.label), 1),
      Lu("div", OM, Xu(AM(sl)(new Date(i.date), "d/M HH:mm:ss")), 1)
    ]));
  }
});
const _u = /* @__PURE__ */ Pe(GM, [["__scopeId", "data-v-150a6a2c"]]);
var CM = Object.defineProperty, qu = Object.getOwnPropertySymbols, zM = Object.prototype.hasOwnProperty, YM = Object.prototype.propertyIsEnumerable, Ku = (n, i, a) => i in n ? CM(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, $u = (n, i) => {
  for (var a in i || (i = {}))
    zM.call(i, a) && Ku(n, a, i[a]);
  if (qu)
    for (var a of qu(i))
      YM.call(i, a) && Ku(n, a, i[a]);
  return n;
};
const IM = window.Vue.defineComponent, HM = window.Vue.toRaw, jo = window.Vue.h;
var PM = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const jM = {
  icon: {
    type: Object,
    required: !1
  },
  flex: {
    type: Boolean,
    default: !1
  },
  title: {
    type: String,
    required: !1
  }
}, XM = IM({
  props: jM,
  render() {
    const n = this.$attrs, i = n.class ? n.class.split(" ").reduce((c, f) => (c[f] = !0, c), {}) : {}, a = {};
    i["feather-icon"] = !0, this.flex && (i["feather-icon-flex"] = !0), this.title ? (a["aria-label"] = this.title, a["aria-hidden"] = "false") : a["aria-hidden"] = "true", a.focusable = "false", a.role = "img";
    let l = HM(this.icon);
    return this.$slots.default ? jo("span", { class: "feather-icon-container" }, [
      jo(this.$slots.default()[0], $u({
        class: i
      }, a))
    ]) : jo(l, $u({
      class: i
    }, a));
  }
});
var Rt = /* @__PURE__ */ PM(XM, [["__scopeId", "data-v-52cbf270"]]);
const LM = window.Vue.openBlock, _M = window.Vue.createElementBlock, qM = window.Vue.createElementVNode;
var KM = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const $M = {}, ew = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, tw = /* @__PURE__ */ qM("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM15.36,8.64l-4.95,4.95-2.3-2.3a1,1,0,0,0-1.41,0h0a1,1,0,0,0,0,1.41l3,3a1,1,0,0,0,1.41,0l5.66-5.66a1,1,0,0,0,0-1.41h0A1,1,0,0,0,15.36,8.64Z" }, null, -1), nw = [
  tw
];
function rw(n, i) {
  return LM(), _M("svg", ew, nw);
}
var aw = /* @__PURE__ */ KM($M, [["render", rw]]);
const iw = window.Vue.openBlock, ow = window.Vue.createElementBlock, lw = window.Vue.createElementVNode;
var sw = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const cw = {}, uw = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, dw = /* @__PURE__ */ lw("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20ZM14.59,8,12,10.59,9.41,8A1,1,0,0,0,8,8H8A1,1,0,0,0,8,9.41L10.59,12,8,14.59A1,1,0,0,0,8,16H8a1,1,0,0,0,1.41,0L12,13.41,14.59,16A1,1,0,0,0,16,16h0a1,1,0,0,0,0-1.41L13.41,12,16,9.41A1,1,0,0,0,16,8h0A1,1,0,0,0,14.59,8Z" }, null, -1), fw = [
  dw
];
function hw(n, i) {
  return iw(), ow("svg", uw, fw);
}
var pw = /* @__PURE__ */ sw(cw, [["render", hw]]);
const Yn = {
  ENGINE_DBSCAN: "dbscan",
  ENGINE_DEEP_LEARNING: "cluster",
  HELLINGER_OPTION: "hellinger",
  SPACE_DISTANCE_OPTION: "alarminspaceandtimedistance"
}, mw = "/alec", Vw = "/alec/engine/configuration", sf = "/alec/agreement/configuration", cf = async (n) => {
  try {
    return (await Dr.post(`${sf}`, {
      agreed: n
    })).status === 201;
  } catch {
    return !1;
  }
}, Tw = async () => {
  try {
    const n = await Dr.get(`${sf}`);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, Uw = async (n, i) => {
  try {
    return (await Dr.post(Vw, {
      distanceMeasureName: i ? Yn.HELLINGER_OPTION : Yn.SPACE_DISTANCE_OPTION,
      engineName: n,
      alpha: 144.47117699,
      beta: 0.55257784,
      epsilon: 100
    })).status === 200;
  } catch {
    return !1;
  }
}, Rw = async (n, i) => {
  try {
    return (await Dr.post(`${mw}/situation/${i}/${n}`, {
      headers: {
        "Access-Control-Allow-Origin": "*"
      }
    })).status === 200;
  } catch {
    return !1;
  }
};
const vw = window.Vue.computed, Nw = (n, i) => {
  const a = {};
  return Object.keys(i).forEach((l) => {
    a[`${l}Label`] = vw(() => n.value[l] ? n.value[l] : i[l]);
  }), a;
}, Mw = window.Vue.openBlock, ww = window.Vue.createElementBlock, Zw = window.Vue.createElementVNode;
var Jw = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const gw = {}, kw = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, yw = /* @__PURE__ */ Zw("path", { d: "M19,17.55,13.43,12,19,6.4A1,1,0,1,0,17.58,5L12,10.58,6.42,5A1,1,0,0,0,5,6.42L10.6,12,5,17.6A1,1,0,0,0,5,19a1,1,0,0,0,.71.29,1,1,0,0,0,.71-.3L12,13.41,17.6,19a1,1,0,0,0,.71.29A1,1,0,0,0,19,19,1,1,0,0,0,19,17.55Z" }, null, -1), bw = [
  yw
];
function Ww(n, i) {
  return Mw(), ww("svg", kw, bw);
}
var uf = /* @__PURE__ */ Jw(gw, [["render", Ww]]);
const ed = window.Vue.computed, Ew = (n, i, a) => {
  const l = ed(() => i.value.filter((f) => !f.disabled)), c = ed(() => n.value ? l.value.indexOf(n.value) : -1);
  return {
    selectPrevious() {
      n.value && n.value.disabled || (c.value === 0 ? a(l.value[l.value.length - 1]) : a(l.value[c.value - 1]));
    },
    selectNext() {
      n.value && n.value.disabled || (c.value === l.value.length - 1 ? a(l.value[0]) : a(l.value[c.value + 1]));
    }
  };
};
const Fw = window.Vue.openBlock, Sw = window.Vue.createElementBlock, bl = window.Vue.createElementVNode;
var Aw = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const xw = {}, Qw = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Bw = /* @__PURE__ */ bl("path", { d: "M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" }, null, -1), Dw = /* @__PURE__ */ bl("rect", {
  x: "11",
  y: "7",
  width: "2",
  height: "7",
  rx: "1"
}, null, -1), Ow = /* @__PURE__ */ bl("rect", {
  x: "11",
  y: "15",
  width: "2",
  height: "2",
  rx: "0.65"
}, null, -1), Gw = [
  Bw,
  Dw,
  Ow
];
function Cw(n, i) {
  return Fw(), Sw("svg", Qw, Gw);
}
var zw = /* @__PURE__ */ Aw(xw, [["render", Cw]]), Yw = Object.defineProperty, Iw = Object.defineProperties, Hw = Object.getOwnPropertyDescriptors, td = Object.getOwnPropertySymbols, Pw = Object.prototype.hasOwnProperty, jw = Object.prototype.propertyIsEnumerable, nd = (n, i, a) => i in n ? Yw(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, df = (n, i) => {
  for (var a in i || (i = {}))
    Pw.call(i, a) && nd(n, a, i[a]);
  if (td)
    for (var a of td(i))
      jw.call(i, a) && nd(n, a, i[a]);
  return n;
}, ff = (n, i) => Iw(n, Hw(i));
const Kn = window.Vue.defineComponent, Wl = window.Vue.resolveComponent, Vn = window.Vue.openBlock, Wr = window.Vue.createElementBlock, Xw = window.Vue.createVNode, hf = window.Vue.createBlock, Lw = window.Vue.withModifiers, _n = window.Vue.inject, xr = window.Vue.computed;
window.Vue.normalizeClass;
window.Vue.createElementVNode;
const rd = window.Vue.toDisplayString, _w = window.Vue.renderSlot, Xo = window.Vue.createCommentVNode, qw = window.Vue.withDirectives, Kw = window.Vue.vShow, cl = window.Vue.ref, ad = window.Vue.toRef, id = window.Vue.nextTick, ul = window.Vue.watch;
window.Vue.createTextVNode;
window.Vue.Fragment;
window.Vue.renderList;
const pf = window.Vue.provide, od = window.Vue.isRef, $w = window.Vue.onBeforeUnmount;
var ii = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const eZ = {
  title: {
    type: String,
    default: ""
  },
  icon: {
    type: Object,
    required: !0
  }
}, tZ = Kn({
  props: eZ,
  components: {
    FeatherIcon: Rt
  }
}), nZ = ["title"];
function rZ(n, i, a, l, c, f) {
  const p = Wl("FeatherIcon");
  return Vn(), Wr("a", {
    title: n.title,
    class: "action-icon hide-when-disabled",
    href: "#"
  }, [
    Xw(p, { icon: n.icon }, null, 8, ["icon"])
  ], 8, nZ);
}
var aZ = /* @__PURE__ */ ii(tZ, [["render", rZ], ["__scopeId", "data-v-4265058e"]]);
const iZ = Kn({
  emits: ["clear"],
  props: {
    clear: {
      type: String,
      default: ""
    }
  },
  computed: {
    clearIcon() {
      return uf;
    }
  },
  components: {
    ActionIcon: aZ
  }
});
function oZ(n, i, a, l, c, f) {
  const p = Wl("ActionIcon");
  return Vn(), hf(p, {
    onClick: i[0] || (i[0] = Lw((V) => n.$emit("clear"), ["stop", "prevent"])),
    title: n.clear,
    icon: n.clearIcon,
    "data-ref-id": "feather-form-element-clear"
  }, null, 8, ["title", "icon"]);
}
var lZ = /* @__PURE__ */ ii(iZ, [["render", oZ]]);
const sZ = Kn({
  computed: {
    errorIcon() {
      return zw;
    }
  },
  components: {
    FeatherIcon: Rt
  }
});
function cZ(n, i, a, l, c, f) {
  const p = Wl("FeatherIcon");
  return Vn(), hf(p, {
    icon: n.errorIcon,
    class: "error-icon hide-when-disabled",
    "data-ref-id": "error-icon"
  }, null, 8, ["icon"]);
}
var uZ = /* @__PURE__ */ ii(sZ, [["render", cZ], ["__scopeId", "data-v-0b8faef3"]]);
const dZ = {
  for: {
    type: String,
    required: !0
  },
  focused: {
    type: Boolean,
    default: !1
  },
  showClear: {
    type: Boolean,
    default: !1
  },
  clearText: {
    type: String
  },
  raised: {
    type: Boolean,
    default: !1
  }
}, fZ = {
  clear: () => !0,
  "wrapper-click": (n) => !0
};
Kn({
  emits: fZ,
  props: dZ,
  data() {
    return {
      prefixWidth: 0,
      prefixObserver: void 0
    };
  },
  setup() {
    const n = _n("wrapperOptions", {}), i = _n("validationErrorMessage", !1), a = xr(() => n.error ? n.error : i && i.value ? i.value : !1);
    return ff(df({}, n), { error: a });
  },
  computed: {
    computedClearText() {
      return this.clearText ? this.clearText : this.clear ? this.clear : "";
    },
    inputId() {
      return this.for;
    },
    hasPre() {
      return this.$slots.pre && this.$slots.pre().findIndex((i) => i.children && i.children.length !== 0) !== -1;
    },
    containerCls() {
      const n = [];
      return this.hideLabel && n.push("hide-label"), this.raised && n.push("raised"), this.focused && n.push("focused"), this.error && n.push("error"), this.background && n.push("background"), this.disabled && n.push("disabled"), this.inline && n.push("inline"), this.hasPre && n.push("has-prefix"), n;
    }
  },
  methods: {
    handleWrapperClick(n) {
      this.disabled || this.$emit("wrapper-click", n);
    }
  },
  mounted() {
    const n = this.$el.querySelector(".prefix");
    if (n) {
      const i = { childList: !0, subtree: !0 }, a = () => {
        const l = this.$el.querySelector(".prefix");
        this.prefixWidth = l ? l.offsetWidth : 0;
      };
      this.prefixObserver = new MutationObserver(a), this.prefixObserver.observe(n, i), a();
    }
  },
  unmounted() {
    this.prefixObserver && this.prefixObserver.disconnect();
  },
  components: {
    ClearIcon: lZ,
    ErrorIcon: uZ
  }
});
const hZ = Kn({
  setup() {
    const n = _n("subTextOptions", {}), i = _n("validationErrorMessage", !1), a = xr(() => n.error ? n.error : i && i.value ? i.value : "");
    return ff(df({}, n), { error: a });
  }
}), pZ = { class: "feather-input-sub-text" }, mZ = {
  key: 0,
  class: "feather-input-spacer"
}, VZ = {
  key: 1,
  class: "feather-input-hint",
  "data-ref-id": "feather-form-element-hint"
}, TZ = {
  key: 2,
  class: "feather-input-error",
  "data-ref-id": "feather-form-element-error",
  "aria-live": "assertive"
};
function UZ(n, i, a, l, c, f) {
  return qw((Vn(), Wr("div", pZ, [
    !n.hint && !n.error.length ? (Vn(), Wr("div", mZ, "\xA0")) : Xo("", !0),
    n.hint && !n.error.length ? (Vn(), Wr("div", VZ, rd(n.hint), 1)) : Xo("", !0),
    n.error.length > 0 ? (Vn(), Wr("div", TZ, rd(n.error), 1)) : Xo("", !0),
    _w(n.$slots, "right", {}, void 0, !0)
  ], 512)), [
    [Kw, !n.inline || n.hint || n.error.length]
  ]);
}
var mf = /* @__PURE__ */ ii(hZ, [["render", UZ], ["__scopeId", "data-v-8e0ac99e"]]);
const RZ = {
  headingText: {
    type: Function,
    default: (n) => n.length && n.length === 1 ? "1 error" : `${n.length} errors`
  },
  errorList: {
    type: Array,
    default: () => []
  },
  generalError: {
    type: String,
    default: ""
  }
};
Kn({
  props: RZ,
  setup(n) {
    const i = _n("featherFormErrors", cl([])), a = ad(n, "errorList"), l = xr(() => {
      var J;
      return (J = a.value) != null && J.length ? a.value : i.value;
    }), c = ad(n, "generalError"), f = (J) => {
      document.getElementById(J).focus();
    }, p = (J) => J.replace(/ \*$/, ""), V = cl(), v = (J) => `${p(J.label)} - ${J.message}`, k = xr(() => (l.value.length && id(() => V.value.focus()), n.headingText(l.value)));
    return ul(c, (J) => {
      J.length && id(() => V.value.focus());
    }), {
      errors: l,
      errorsHeading: k,
      heading: V,
      focusElement: f,
      mainError: c,
      getFullMessage: v
    };
  }
});
const Vf = (n, i, a, l, c) => {
  const f = _n("featherForm", !1);
  if (l && f && n.value) {
    const p = cl("");
    pf("validationErrorMessage", p);
    const V = () => {
      if (c && od(c) && c.value)
        return p.value = c.value, {
          success: !1,
          message: c.value,
          inputId: n.value,
          label: a
        };
      try {
        return l.validateSync(i.value), p.value = "", { success: !0 };
      } catch (J) {
        const y = J;
        return p.value = y.errors[0], {
          success: !1,
          message: y.errors[0],
          inputId: n.value,
          label: a
        };
      }
    }, k = {
      clear: () => {
        p.value = "";
      },
      validate: V
    };
    return c && od(c) && ul(c, () => {
      f.runValidation();
    }), ul(n, (J, y) => {
      J && f && f.register(J, k), y && f && f.deregister(y);
    }, { immediate: !0 }), $w(() => {
      f.deregister(n.value, !0);
    }), { validate: V };
  }
  return { validate: () => !0 };
}, Tf = (n) => ({
  inherittedAttrs: xr(() => ({
    class: n.class,
    "data-ref-id": n["data-ref-id"]
  }))
}), Uf = {
  hint: {
    type: String
  },
  error: {
    type: String
  },
  inline: {
    type: Boolean,
    default: !1
  }
}, Rf = (n) => {
  pf("subTextOptions", n);
}, Ba = window.Vue.ref, vZ = window.Vue.watch, NZ = window.Vue.watchEffect, ld = window.Vue.computed, Lo = window.Vue.provide, vf = (n, i, a, l, c) => {
  const f = Ba([]), p = Ba(), V = Ba(), v = Ba();
  NZ(() => {
    if (!f.value.length)
      return;
    const B = f.value.map((_) => _.value);
    if (n.value !== void 0 && n.value !== null && (p.value = f.value[B.indexOf(n.value)]), !p.value && f.value.length) {
      let _ = f.value.filter((ne) => !ne.disabled);
      _ = _.length ? _ : f.value, V.value = _[0], V.value.first = !0;
    }
  }), vZ(p, (B, _) => {
    _ && (_.checked = !1), B && (B.checked = !0);
  });
  const k = (B) => {
    B && B.disabled || (V.value && (V.value.first = !1), p.value !== B && (i("update:modelValue", B.value), p.value = B, B.focus()));
  }, J = ld(() => p.value || V.value), y = Ew(J, f, k), g = ld(() => Ie("feather-radio-group"));
  v.value = g.value;
  const { validate: H } = Vf(v, n, a, l, c);
  return Lo("register", (B) => {
    f.value = [...f.value, B], v.value === g.value && (v.value = B.id);
  }), Lo("select", k), Lo("blur", (B) => {
    i("blur", B);
  }), {
    keydown: (B) => {
      switch (B.keyCode) {
        case 13:
        case 32:
          p.value ? k(p.value) : V.value && k(V.value);
          break;
        case 40:
        case 39:
          y.selectNext();
          break;
        case 37:
        case 38:
          y.selectPrevious();
          break;
      }
    },
    ...y,
    focus: () => {
      p.value && p.value.focus();
    },
    validate: H,
    firstElementId: v,
    groupId: g
  };
};
var MZ = Object.defineProperty, wZ = Object.defineProperties, ZZ = Object.getOwnPropertyDescriptors, sd = Object.getOwnPropertySymbols, JZ = Object.prototype.hasOwnProperty, gZ = Object.prototype.propertyIsEnumerable, cd = (n, i, a) => i in n ? MZ(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Hn = (n, i) => {
  for (var a in i || (i = {}))
    JZ.call(i, a) && cd(n, a, i[a]);
  if (sd)
    for (var a of sd(i))
      gZ.call(i, a) && cd(n, a, i[a]);
  return n;
}, Nf = (n, i) => wZ(n, ZZ(i));
const tn = window.Vue.defineComponent, Fr = window.Vue.inject, _a = window.Vue.computed, Sr = window.Vue.ref, at = window.Vue.resolveComponent, He = window.Vue.openBlock, Or = window.Vue.createElementBlock, Mf = window.Vue.normalizeClass, vt = window.Vue.renderSlot, $t = window.Vue.createBlock, Qr = window.Vue.createCommentVNode, qa = window.Vue.createElementVNode, kZ = window.Vue.withModifiers, oi = window.Vue.createVNode, wf = window.Vue.toRef, dl = window.Vue.mergeProps, Bt = window.Vue.withCtx, yZ = window.Vue.h, bZ = window.Vue.provide;
var nn = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const WZ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  clickable: {
    type: Boolean,
    default: !1
  }
}, EZ = tn({
  props: WZ,
  setup(n) {
    const i = Fr("isCondensed", null), a = _a(() => i || n.condensed), l = Sr(!1);
    return {
      focused: l,
      handleFocus: () => {
        l.value = !0;
      },
      handleBlur: () => {
        l.value = !1;
      },
      isCondensed: a
    };
  },
  components: {
    FeatherRipple: Ln
  }
}), FZ = ["aria-disabled"];
function SZ(n, i, a, l, c, f) {
  const p = at("FeatherRipple");
  return He(), Or("div", {
    class: Mf(["chip", {
      condensed: n.isCondensed,
      disabled: n.disabled,
      focused: n.focused
    }]),
    onFocusin: i[0] || (i[0] = (V) => n.clickable ? n.handleFocus : null),
    onFocusout: i[1] || (i[1] = (V) => n.clickable ? n.handleBlur : null),
    "aria-disabled": n.disabled
  }, [
    vt(n.$slots, "default", {}, void 0, !0),
    n.clickable ? (He(), $t(p, { key: 0 })) : Qr("", !0)
  ], 42, FZ);
}
var El = /* @__PURE__ */ nn(EZ, [["render", SZ], ["__scopeId", "data-v-44d413dc"]]);
const AZ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  textId: {
    type: String,
    required: !0
  },
  label: {
    type: String,
    required: !0
  }
}, xZ = tn({
  emits: ["delete"],
  props: AZ,
  setup(n, i) {
    return {
      handleDelete: () => {
        n.disabled || i.emit("delete");
      },
      icon: uf
    };
  },
  components: {
    FeatherIcon: Rt
  }
}), QZ = { class: "chip-delete" }, BZ = ["aria-label", "aria-describedby"];
function DZ(n, i, a, l, c, f) {
  const p = at("FeatherIcon");
  return He(), Or("span", QZ, [
    qa("a", {
      href: "#",
      class: "delete-icon",
      role: "button",
      onClick: i[0] || (i[0] = kZ((...V) => n.handleDelete && n.handleDelete(...V), ["stop", "prevent"])),
      "aria-label": n.label,
      "aria-describedby": n.textId
    }, [
      oi(p, {
        icon: n.icon,
        flex: ""
      }, null, 8, ["icon"])
    ], 8, BZ)
  ]);
}
var OZ = /* @__PURE__ */ nn(xZ, [["render", DZ], ["__scopeId", "data-v-4bae6cb4"]]);
const GZ = tn({
  data() {
    return {
      titleText: void 0
    };
  },
  methods: {
    ellipsisActive() {
      return this.$refs.container.offsetWidth < this.$refs.container.scrollWidth;
    },
    updateTitle() {
      this.titleText = this.ellipsisActive() ? this.$refs.container.textContent : void 0;
    }
  },
  mounted() {
    this.updateTitle();
  },
  beforeUpdate() {
    this.updateTitle();
  }
}), CZ = ["title"];
function zZ(n, i, a, l, c, f) {
  return He(), Or("span", {
    class: "label",
    title: n.titleText,
    ref: "container"
  }, [
    vt(n.$slots, "default", {}, void 0, !0)
  ], 8, CZ);
}
var Fl = /* @__PURE__ */ nn(GZ, [["render", zZ], ["__scopeId", "data-v-1a0445b2"]]);
const YZ = {}, IZ = {
  class: "chip-icon",
  role: "presentation"
};
function HZ(n, i) {
  return He(), Or("span", IZ, [
    vt(n.$slots, "default", {}, void 0, !0)
  ]);
}
var Sl = /* @__PURE__ */ nn(YZ, [["render", HZ], ["__scopeId", "data-v-2230176f"]]);
const ud = {
  delete: "Remove"
}, PZ = tn({
  inheritAttrs: !1,
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    labels: {
      type: Object,
      default: () => ud
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n, i) {
    const a = Nw(wf(n, "labels"), ud), l = _a(() => Ie("chip-text")), c = () => {
      n.disabled || i.emit("click");
    }, f = Hn({}, i.attrs);
    return n.disabled && delete f.onClick, Nf(Hn({}, a), {
      chipTextId: l,
      handleClick: c,
      attrs: f
    });
  },
  computed: {
    chipTextAttrs() {
      return this.canClick ? {
        role: "button",
        tabindex: "0"
      } : {};
    },
    canDelete() {
      return !!this.$attrs.onDelete && !this.disabled;
    },
    canClick() {
      return !!this.$attrs.onClick && !this.disabled;
    },
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    DeleteIcon: OZ,
    Label: Fl,
    PreIcon: Sl
  }
}), jZ = ["aria-disabled"];
function XZ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("DeleteIcon"), k = at("Chip");
  return He(), $t(k, dl(n.attrs, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: { hover: n.canClick, focus: n.canClick || n.canDelete },
    role: "row",
    clickable: n.canClick
  }), {
    default: Bt(() => [
      qa("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        qa("span", dl(n.chipTextAttrs, { class: "chip-label-button" }), [
          n.hasIcon ? (He(), $t(p, { key: 0 }, {
            default: Bt(() => [
              vt(n.$slots, "icon", {}, void 0, !0)
            ]),
            _: 3
          })) : Qr("", !0),
          oi(V, { id: n.chipTextId }, {
            default: Bt(() => [
              vt(n.$slots, "default", {}, void 0, !0)
            ]),
            _: 3
          }, 8, ["id"])
        ], 16)
      ], 8, jZ),
      n.canDelete ? (He(), $t(v, {
        key: 0,
        disabled: n.disabled,
        "text-id": n.chipTextId,
        label: n.deleteLabel,
        role: "gridcell",
        onDelete: i[0] || (i[0] = (J) => n.$emit("delete"))
      }, null, 8, ["disabled", "text-id", "label"])) : Qr("", !0)
    ]),
    _: 3
  }, 16, ["disabled", "condensed", "class", "clickable"]);
}
var LZ = /* @__PURE__ */ nn(PZ, [["render", XZ], ["__scopeId", "data-v-48b2704a"]]);
const _Z = tn({
  props: {
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    Label: Fl,
    PreIcon: Sl
  }
}), qZ = ["aria-disabled"];
function KZ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("Chip");
  return He(), $t(v, {
    role: "row",
    disabled: n.disabled,
    condensed: n.condensed,
    clickable: !1
  }, {
    default: Bt(() => [
      qa("span", {
        role: "gridcell",
        "aria-disabled": n.disabled
      }, [
        n.hasIcon ? (He(), $t(p, { key: 0 }, {
          default: Bt(() => [
            vt(n.$slots, "icon", {}, void 0, !0)
          ]),
          _: 3
        })) : Qr("", !0),
        oi(V, null, {
          default: Bt(() => [
            vt(n.$slots, "default", {}, void 0, !0)
          ]),
          _: 3
        })
      ], 8, qZ)
    ]),
    _: 3
  }, 8, ["disabled", "condensed"]);
}
var $Z = /* @__PURE__ */ nn(_Z, [["render", KZ], ["__scopeId", "data-v-3e0c4eba"]]);
const eJ = tn({
  props: {
    value: {
      type: [String, Object, Number, Array],
      required: !0
    },
    disabled: {
      type: Boolean,
      default: !1
    },
    condensed: {
      type: Boolean,
      default: !1
    }
  },
  setup(n) {
    const i = Sr(!1), a = Sr(!1), l = _a(() => Ie("chip-label-id")), c = _a(() => i.value || a.value ? 0 : -1), f = Sr(), p = () => {
      f.value.$el.focus();
    }, V = Fr("register", (g) => {
    }), v = Fr("blur", (g) => {
    }), k = Fr("select", (g) => {
    }), J = {
      first: i,
      focus: p,
      disabled: n.disabled,
      value: n.value,
      checked: a
    };
    return V(J), {
      labelId: l,
      tabindex: c,
      first: i,
      blur: v,
      click: () => {
        k(J);
      },
      input: f,
      checked: a
    };
  },
  computed: {
    hasIcon() {
      return this.$slots.icon && this.$slots.icon().findIndex((i) => i.children && i.children.length !== 0 || typeof i.type == "object") !== -1;
    }
  },
  components: {
    Chip: El,
    Label: Fl,
    PreIcon: Sl
  }
});
function tJ(n, i, a, l, c, f) {
  const p = at("PreIcon"), V = at("Label"), v = at("Chip");
  return He(), $t(v, {
    disabled: n.disabled,
    condensed: n.condensed,
    class: Mf(["focus hover", { selected: n.checked }]),
    role: "radio",
    ref: "input",
    "aria-checked": n.checked ? "true" : "false",
    "aria-disabled": n.disabled,
    "aria-labelledby": n.labelId,
    tabindex: n.tabindex,
    onClick: n.click,
    "allow-click": !n.disabled,
    onBlur: n.blur
  }, {
    default: Bt(() => [
      n.hasIcon ? (He(), $t(p, { key: 0 }, {
        default: Bt(() => [
          vt(n.$slots, "icon", {}, void 0, !0)
        ]),
        _: 3
      })) : Qr("", !0),
      oi(V, { id: n.labelId }, {
        default: Bt(() => [
          vt(n.$slots, "default", {}, void 0, !0)
        ]),
        _: 3
      }, 8, ["id"])
    ]),
    _: 3
  }, 8, ["disabled", "condensed", "class", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex", "onClick", "allow-click", "onBlur"]);
}
var nJ = /* @__PURE__ */ nn(eJ, [["render", tJ], ["__scopeId", "data-v-bbcc2f70"]]);
const rJ = {
  disabled: {
    type: Boolean,
    default: !1
  },
  labels: {
    type: Object
  },
  condensed: {
    type: Boolean,
    default: !1
  },
  value: {
    type: [String, Object, Number, Array, Boolean]
  }
}, dd = tn({
  props: rJ,
  setup() {
    return { format: Fr("chipListFormat", "") };
  },
  render() {
    const n = (i) => yZ(i, Hn(Hn({}, this.$props), this.$attrs), Hn({
      default: this.$slots.default
    }, this.$slots.icon && { icon: this.$slots.icon }));
    return this.format === "grid" ? n(LZ) : this.format === "radio" ? n(nJ) : n($Z);
  }
}), aJ = {
  label: {
    type: String,
    required: !0
  },
  mode: {
    type: String,
    default: "list",
    validator: (n) => ["list", "radio", "single"].indexOf(n) !== -1
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  condensed: {
    type: Boolean,
    default: !1
  }
}, iJ = tn({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: aJ,
  setup(n, i) {
    const a = n.mode === "list" ? "grid" : n.mode;
    bZ("chipListFormat", a);
    const l = a === "single";
    if (a === "radio") {
      const p = wf(n, "modelValue");
      return Nf(Hn({
        attrs: {
          role: "radiogroup"
        }
      }, vf(p, i.emit, n.label, {}, Sr(""))), {
        single: l
      });
    }
    return { attrs: {
      role: "grid"
    }, keydown: () => {
    }, single: l };
  }
}), oJ = ["aria-label"];
function lJ(n, i, a, l, c, f) {
  return He(), Or("div", dl(n.attrs, {
    "data-ref-id": "feather-chip-list",
    "aria-label": n.label,
    class: ["chip-list", { condensed: n.condensed, single: n.single }],
    onKeydown: i[0] || (i[0] = (...p) => n.keydown && n.keydown(...p))
  }), [
    vt(n.$slots, "default", {}, void 0, !0)
  ], 16, oJ);
}
var sJ = /* @__PURE__ */ nn(iJ, [["render", lJ], ["__scopeId", "data-v-1e06f41d"]]);
const fd = (n) => {
  const i = new Date(n);
  return `${i.getMonth() + 1}/${i.getDate()} ${i.getUTCHours()}:${i.getMinutes()}:${i.getSeconds()}`;
}, cJ = window.Vue.defineComponent, Lt = window.Vue.unref, _o = window.Vue.toDisplayString, Kt = window.Vue.createElementVNode, hd = window.Vue.createTextVNode, uJ = window.Vue.createVNode, dJ = window.Vue.openBlock, fJ = window.Vue.createElementBlock, hJ = window.Vue.createCommentVNode, pJ = window.Vue.pushScopeId, mJ = window.Vue.popScopeId, Zf = (n) => (pJ("data-v-efb670fd"), n = n(), mJ(), n), VJ = {
  key: 0,
  class: "card"
}, TJ = { class: "row" }, UJ = { class: "title" }, RJ = /* @__PURE__ */ Zf(() => /* @__PURE__ */ Kt("strong", null, "First Event", -1)), vJ = /* @__PURE__ */ Zf(() => /* @__PURE__ */ Kt("strong", null, "Last Event", -1)), NJ = ["innerHTML"], MJ = /* @__PURE__ */ cJ({
  __name: "AlarmDetail",
  props: {
    id: null
  },
  setup(n) {
    const i = n, l = tf().alarms[i.id];
    return (c, f) => {
      var p;
      return Lt(l) ? (dJ(), fJ("div", VJ, [
        Kt("div", TJ, [
          Kt("div", UJ, "[" + _o(Lt(l).id) + "]", 1),
          Kt("div", null, [
            RJ,
            hd(" - " + _o(Lt(fd)(Lt(l).firstEventTime)), 1)
          ]),
          Kt("div", null, [
            vJ,
            hd(" - " + _o(Lt(fd)(Lt(l).lastEvent.createTime)), 1)
          ]),
          Kt("div", null, [
            uJ(Zl, {
              severity: (p = Lt(l)) == null ? void 0 : p.severity
            }, null, 8, ["severity"])
          ])
        ]),
        Kt("div", {
          innerHTML: Lt(l).description
        }, null, 8, NJ)
      ])) : hJ("", !0);
    };
  }
});
const wJ = /* @__PURE__ */ Pe(MJ, [["__scopeId", "data-v-efb670fd"]]), ZJ = window.Vue.defineComponent, JJ = window.Vue.normalizeClass, gJ = window.Vue.openBlock, kJ = window.Vue.createElementBlock, yJ = window.Vue.createCommentVNode, bJ = /* @__PURE__ */ ZJ({
  __name: "StatusColor",
  props: {
    severity: {
      required: !1,
      type: String
    }
  },
  setup(n) {
    const i = n;
    return (a, l) => i != null && i.severity ? (gJ(), kJ("span", {
      key: 0,
      class: JJ(["circle", [`${i.severity.toLowerCase()}-bg dark`]])
    }, null, 2)) : yJ("", !0);
  }
});
const WJ = /* @__PURE__ */ Pe(bJ, [["__scopeId", "data-v-e08880d6"]]), EJ = window.Vue.defineComponent, Ia = window.Vue.createElementVNode, Jf = window.Vue.createTextVNode, Da = window.Vue.unref, pd = window.Vue.normalizeClass, qo = window.Vue.withCtx, Ko = window.Vue.createVNode, md = window.Vue.renderList, $o = window.Vue.Fragment, On = window.Vue.openBlock, Oa = window.Vue.createElementBlock, FJ = window.Vue.toDisplayString, Vd = window.Vue.createBlock, SJ = window.Vue.pushScopeId, AJ = window.Vue.popScopeId, xJ = (n) => (SJ("data-v-4d8a6eca"), n = n(), AJ(), n), QJ = /* @__PURE__ */ xJ(() => /* @__PURE__ */ Ia("div", { class: "title" }, "Alarms", -1)), BJ = { class: "alarm-filters-container" }, DJ = /* @__PURE__ */ Jf(" ALL "), OJ = { class: "section" }, GJ = { class: "alarm-list" }, Td = window.Vue.ref, CJ = window.Vue.watch, zJ = window.Vue.computed, YJ = /* @__PURE__ */ EJ({
  __name: "AlarmFilters",
  props: {
    relatedAlarms: null
  },
  setup(n) {
    const i = n, a = zJ(
      () => Xn.exports.keys(Xn.exports.groupBy(i.relatedAlarms, "severity"))
    ), l = Td(["all"]), c = Td(i.relatedAlarms), f = (p) => {
      l.value = l.value.filter((V) => V !== "all"), l.value.includes(p) ? l.value = l.value.filter((V) => V !== p) : l.value.push(p), p === "all" || l.value.length === 0 ? (l.value = ["all"], c.value = i.relatedAlarms) : c.value = i.relatedAlarms.filter(
        (V) => l.value.includes(V.severity)
      );
    };
    return CJ(i, () => {
      l.value = ["all"], c.value = i.relatedAlarms;
    }), (p, V) => (On(), Oa($o, null, [
      QJ,
      Ia("div", BJ, [
        (On(), Vd(Da(sJ), {
          key: l.value.toString(),
          condensed: "",
          class: "alarm-filters",
          label: "Random list for condensed visual testing"
        }, {
          default: qo(() => [
            Ko(Da(dd), {
              class: pd({ clicked: l.value.includes("all") }),
              onClick: V[0] || (V[0] = (v) => f("all"))
            }, {
              default: qo(() => [
                DJ
              ]),
              _: 1
            }, 8, ["class"]),
            (On(!0), Oa($o, null, md(Da(a), (v) => (On(), Vd(Da(dd), {
              class: pd({ clicked: l.value.includes(v) }),
              key: v,
              onClick: (k) => f(v)
            }, {
              default: qo(() => [
                Ko(WJ, { severity: v }, null, 8, ["severity"]),
                Jf(FJ(v), 1)
              ]),
              _: 2
            }, 1032, ["class", "onClick"]))), 128))
          ]),
          _: 1
        })),
        Ia("div", OJ, [
          Ia("div", GJ, [
            (On(!0), Oa($o, null, md(c.value, (v) => (On(), Oa("div", {
              key: v.id
            }, [
              Ko(wJ, {
                id: v.id
              }, null, 8, ["id"])
            ]))), 128))
          ])
        ])
      ])
    ], 64));
  }
});
const IJ = /* @__PURE__ */ Pe(YJ, [["__scopeId", "data-v-4d8a6eca"]]);
var HJ = Object.defineProperty, PJ = Object.defineProperties, jJ = Object.getOwnPropertyDescriptors, Ud = Object.getOwnPropertySymbols, XJ = Object.prototype.hasOwnProperty, LJ = Object.prototype.propertyIsEnumerable, Rd = (n, i, a) => i in n ? HJ(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Zr = (n, i) => {
  for (var a in i || (i = {}))
    XJ.call(i, a) && Rd(n, a, i[a]);
  if (Ud)
    for (var a of Ud(i))
      LJ.call(i, a) && Rd(n, a, i[a]);
  return n;
}, vd = (n, i) => PJ(n, jJ(i));
const _J = window.Vue.defineComponent, qJ = window.Vue.inject, Jr = window.Vue.h;
var KJ = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const $J = {
  primary: {
    type: Boolean,
    default: !1
  },
  text: {
    type: Boolean,
    default: !1
  },
  secondary: {
    type: Boolean,
    default: !1
  },
  icon: {
    type: String
  },
  disabled: {
    type: Boolean,
    default: !1
  },
  asAnchor: {
    type: Boolean,
    default: !1
  },
  onColor: {
    type: Boolean,
    default: !1
  }
}, eg = _J({
  inheritAttrs: !1,
  props: $J,
  setup() {
    return { hasTooltip: qJ("feather-has-tooltip", !1) };
  },
  render() {
    const n = () => {
      let V = "";
      this.primary && (V = "btn-primary"), this.secondary && (V = "btn-secondary"), (this.text || this.icon) && (V = "btn-text");
      const v = ["btn", "hover", "focus", V];
      return this.icon && (v.push("btn-icon"), v.push("btn-icon-table")), this.onColor && v.push("on-color"), v;
    }, i = this.asAnchor ? "a" : "button", a = {}, l = Zr({}, this.$attrs);
    a.attrs = l || {}, this.asAnchor ? a.attrs.role = "button" : a.attrs.type = a.attrs.type || "button", this.disabled && (a.attrs["aria-disabled"] = "true"), a.on = {
      onClick: (V) => {
        this.disabled ? (this.asAnchor && V.preventDefault(), this.$emit("disabled-click", V)) : this.$emit("click", V);
      }
    };
    const c = n();
    a.class = [this.$attrs.class].concat(c), this.$slots.icon && a.class.push("has-icon");
    let f = Jr(Ln);
    if (this.disabled && (f = void 0), this.icon && this.$slots.default) {
      const V = this.icon;
      return a.attrs["aria-label"] = V, this.hasTooltip || (a.attrs.title = V), Jr(i, vd(Zr(Zr({}, a.attrs), a.on), { class: a.class }), [
        this.$slots.default(),
        this.disabled ? void 0 : Jr(Ln, { center: !0 })
      ]);
    }
    const p = Jr("span", { class: ["btn-content"] }, [
      this.$slots.default ? this.$slots.default() : ""
    ]);
    return Jr(i, vd(Zr(Zr({}, a.attrs), a.on), { class: a.class }), [
      this.$slots.icon ? this.$slots.icon() : void 0,
      p,
      f
    ]);
  }
});
var Ka = /* @__PURE__ */ KJ(eg, [["__scopeId", "data-v-702d1074"]]);
const tg = "/whoami", ng = async () => {
  try {
    const n = await Dr.get(tg);
    return n.status === 200 ? n.data : !1;
  } catch {
    return !1;
  }
}, rg = window.Pinia.defineStore, Gr = rg("userStore", {
  state: () => ({
    isAdmin: !1,
    userId: null,
    firstTime: !0,
    allowSave: !0
  }),
  actions: {
    async getUserRole() {
      const n = await ng();
      n && (this.isAdmin = n.roles.includes("ROLE_ADMIN"), this.userId = n.id);
    },
    async getAlecInfo() {
      const n = await Tw();
      n && (this.firstTime = !1, this.allowSave = n.agreed);
    },
    async savePermission(n) {
      if (this.allowSave = n, !n) {
        const i = await cf(n);
        this.allowSave = i;
      }
    }
  }
}), ag = window.Vue.defineComponent, St = window.Vue.unref, gr = window.Vue.normalizeClass, At = window.Vue.createVNode, Gn = window.Vue.openBlock, Cn = window.Vue.createElementBlock, ig = window.Vue.createCommentVNode, Nd = window.Vue.withCtx, rt = window.Vue.createElementVNode, Md = window.Vue.toDisplayString, og = window.Vue.createTextVNode, lg = window.Vue.Fragment, sg = window.Vue.pushScopeId, cg = window.Vue.popScopeId, ug = (n) => (sg("data-v-4e3fbcb0"), n = n(), cg(), n), dg = { class: "section" }, fg = {
  key: 0,
  class: "btn-row"
}, hg = { key: 0 }, pg = { key: 1 }, mg = { key: 0 }, Vg = { key: 1 }, Tg = { class: "situation-detail" }, Ug = { class: "situation-info" }, Rg = { class: "id" }, vg = ["innerHTML"], Ng = /* @__PURE__ */ ug(() => /* @__PURE__ */ rt("strong", null, "Reduction key:", -1)), Mg = { class: "boxes" }, wg = { class: "parameters" }, Zg = { class: "section" }, Jg = window.Vue.ref, gg = window.Vue.watch, kg = /* @__PURE__ */ ag({
  __name: "SituationDetailTab",
  props: {
    alarmInfo: null
  },
  setup(n) {
    const i = n, a = Gr(), l = Jg(""), c = (f) => {
      var p;
      Rw((p = i.alarmInfo) == null ? void 0 : p.id, f), l.value = f;
    };
    return gg(i, () => {
      l.value = "";
    }), (f, p) => {
      var V, v, k, J, y;
      return Gn(), Cn(lg, null, [
        rt("div", dg, [
          St(a).allowSave ? (Gn(), Cn("div", fg, [
            At(St(Ka), {
              class: gr(["btn", { accepted: l.value == "accepted" }]),
              onClick: p[0] || (p[0] = () => c("accepted"))
            }, {
              default: Nd(() => [
                At(St(Rt), {
                  icon: St(aw),
                  "aria-hidden": "true",
                  class: gr(["icon accept", { accepted: l.value == "accepted" }])
                }, null, 8, ["icon", "class"]),
                l.value == "accepted" ? (Gn(), Cn("span", hg, " ACCEPTED")) : (Gn(), Cn("span", pg, " ACCEPT"))
              ]),
              _: 1
            }, 8, ["class"]),
            At(St(Ka), {
              class: gr(["btn", { rejected: l.value == "refused" }]),
              onClick: p[1] || (p[1] = () => c("refused"))
            }, {
              default: Nd(() => [
                At(St(Rt), {
                  icon: St(pw),
                  "aria-hidden": "true",
                  class: gr(["icon reject", { rejected: l.value == "refused" }])
                }, null, 8, ["icon", "class"]),
                l.value == "refused" ? (Gn(), Cn("span", mg, " REJECTED")) : (Gn(), Cn("span", Vg, " REJECT"))
              ]),
              _: 1
            }, 8, ["class"])
          ])) : ig("", !0),
          rt("div", Tg, [
            rt("div", {
              class: gr(["severity-line", [`${(v = (V = i.alarmInfo) == null ? void 0 : V.severity) == null ? void 0 : v.toLowerCase()}-bg dark`]])
            }, null, 2),
            rt("div", Ug, [
              rt("div", Rg, [
                rt("div", null, "Situation " + Md((k = i.alarmInfo) == null ? void 0 : k.id), 1),
                At(Zl, {
                  severity: (J = i.alarmInfo) == null ? void 0 : J.severity
                }, null, 8, ["severity"])
              ]),
              rt("span", {
                innerHTML: i.alarmInfo.description
              }, null, 8, vg),
              rt("p", null, [
                Ng,
                og(" " + Md(i.alarmInfo.reductionKey), 1)
              ]),
              rt("div", Mg, [
                At(_u, {
                  label: "First Event",
                  date: St(sl)(new Date(i.alarmInfo.firstEventTime), "d/M HH:mm:ss")
                }, null, 8, ["date"]),
                At(_u, {
                  label: "Last Event",
                  date: St(sl)(new Date(i.alarmInfo.lastEvent.time), "d/M HH:mm:ss")
                }, null, 8, ["date"])
              ])
            ]),
            rt("div", wg, [
              At(nf, {
                relatedAlarms: (y = i.alarmInfo) == null ? void 0 : y.relatedAlarms,
                size: "large"
              }, null, 8, ["relatedAlarms"])
            ])
          ])
        ]),
        rt("div", Zg, [
          At(IJ, {
            "related-alarms": i.alarmInfo.relatedAlarms
          }, null, 8, ["related-alarms"])
        ])
      ], 64);
    };
  }
});
const yg = /* @__PURE__ */ Pe(kg, [["__scopeId", "data-v-4e3fbcb0"]]), bg = window.Vue.defineComponent, gf = window.Vue.createTextVNode, kr = window.Vue.unref, zn = window.Vue.withCtx, pn = window.Vue.createVNode, Wg = window.Vue.resolveComponent, Eg = window.Vue.openBlock, Fg = window.Vue.createElementBlock, Sg = window.Vue.createCommentVNode;
window.Vue.pushScopeId;
window.Vue.popScopeId;
const Ag = {
  key: 0,
  class: "detail"
}, xg = /* @__PURE__ */ gf("Details"), Qg = /* @__PURE__ */ gf("Metrics"), Bg = /* @__PURE__ */ bg({
  __name: "SituationDetail",
  props: {
    alarmInfo: null
  },
  setup(n) {
    const i = n;
    return (a, l) => {
      const c = Wg("SituationMetrics");
      return i.alarmInfo ? (Eg(), Fg("div", Ag, [
        pn(kr(pN), null, {
          tabs: zn(() => [
            pn(kr(zu), null, {
              default: zn(() => [
                xg
              ]),
              _: 1
            }),
            pn(kr(zu), null, {
              default: zn(() => [
                Qg
              ]),
              _: 1
            })
          ]),
          default: zn(() => [
            pn(kr(Yu), { class: "panel" }, {
              default: zn(() => [
                pn(yg, {
                  "alarm-info": i.alarmInfo
                }, null, 8, ["alarm-info"])
              ]),
              _: 1
            }),
            pn(kr(Yu), { class: "panel" }, {
              default: zn(() => [
                pn(c, {
                  situation: i == null ? void 0 : i.alarmInfo
                }, null, 8, ["situation"])
              ]),
              _: 1
            })
          ]),
          _: 1
        })
      ])) : Sg("", !0);
    };
  }
});
const Dg = /* @__PURE__ */ Pe(Bg, [["__scopeId", "data-v-6e09dae4"]]), Og = window.Vue.defineComponent, fl = window.Vue.createElementVNode, wd = window.Vue.unref, Gg = window.Vue.renderList, Cg = window.Vue.Fragment, el = window.Vue.openBlock, tl = window.Vue.createElementBlock, Zd = window.Vue.createVNode, zg = window.Vue.pushScopeId, Yg = window.Vue.popScopeId, Ig = (n) => (zg("data-v-c673a70c"), n = n(), Yg(), n), Hg = { class: "list-main" }, Pg = /* @__PURE__ */ Ig(() => /* @__PURE__ */ fl("h2", null, "Situation List", -1)), jg = { class: "container" }, Xg = { class: "situation-list" }, Lg = window.Vue.reactive, _g = /* @__PURE__ */ Og({
  __name: "SituationList",
  setup(n) {
    const i = tf();
    i.getSituations();
    const a = Lg({
      selectedSituationIndex: 0,
      situationSelected: ""
    }), l = (c) => {
      a.situationSelected = c, a.selectedSituationIndex = i.situations.findIndex(
        (f) => f.id === c
      );
    };
    return i.$subscribe((c, f) => {
      a.situationSelected = f.situations[0].id;
    }), (c, f) => (el(), tl("div", Hg, [
      Pg,
      fl("div", jg, [
        fl("div", Xg, [
          (el(!0), tl(Cg, null, Gg(wd(i).situations, (p) => (el(), tl("div", {
            key: p.id
          }, [
            Zd(Rv, {
              "alarm-info": p,
              onSituationSelected: l,
              selected: a.situationSelected == p.id
            }, null, 8, ["alarm-info", "selected"])
          ]))), 128))
        ]),
        Zd(Dg, {
          "alarm-info": wd(i).situations[a.selectedSituationIndex]
        }, null, 8, ["alarm-info"])
      ])
    ]));
  }
});
const qg = /* @__PURE__ */ Pe(_g, [["__scopeId", "data-v-c673a70c"]]);
var Kg = Object.defineProperty, $g = Object.defineProperties, ek = Object.getOwnPropertyDescriptors, Jd = Object.getOwnPropertySymbols, tk = Object.prototype.hasOwnProperty, nk = Object.prototype.propertyIsEnumerable, gd = (n, i, a) => i in n ? Kg(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, Ha = (n, i) => {
  for (var a in i || (i = {}))
    tk.call(i, a) && gd(n, a, i[a]);
  if (Jd)
    for (var a of Jd(i))
      nk.call(i, a) && gd(n, a, i[a]);
  return n;
}, kf = (n, i) => $g(n, ek(i));
const yf = window.Vue.defineComponent, rk = window.Vue.ref, Ar = window.Vue.computed, ak = window.Vue.reactive, kd = window.Vue.watch, nl = window.Vue.inject, bf = window.Vue.resolveComponent, hl = window.Vue.openBlock, Wf = window.Vue.createElementBlock, en = window.Vue.createElementVNode, ik = window.Vue.createBlock, ok = window.Vue.createCommentVNode, Ef = window.Vue.renderSlot, lk = window.Vue.pushScopeId, sk = window.Vue.popScopeId, rl = window.Vue.toRef, yd = window.Vue.mergeProps, ck = window.Vue.toDisplayString, uk = window.Vue.createVNode;
var Ff = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const dk = {
  value: {
    type: [String, Number, Boolean, Array, Object, Date, Function],
    required: !0
  },
  disabled: {
    type: Boolean,
    default: !1
  }
}, fk = yf({
  props: dk,
  setup(n) {
    const i = rk(), a = () => {
      i.value.focus();
    }, l = Ar(() => Ie("feather-radio-button")), c = ak({
      first: !1,
      focus: a,
      disabled: n.disabled,
      value: n.value,
      checked: !1,
      id: l.value
    }), f = Ar(() => Ie("radio-label-id")), p = Ar(() => c.first || c.checked ? 0 : -1);
    kd(() => n.disabled, (y) => {
      c.disabled = y;
    }, { immediate: !0 }), kd(() => n.value, (y) => {
      c.value = y;
    }, { immediate: !0 });
    const V = nl("register", (y) => {
    }), v = nl("blur", (y) => {
    }), k = nl("select", (y) => {
    });
    return V(c), {
      labelId: f,
      tabindex: p,
      vm: c,
      blur: v,
      click: () => {
        k(c);
      },
      input: i,
      id: l
    };
  },
  components: {
    FeatherRipple: Ln
  }
}), hk = (n) => (lk("data-v-24790cf0"), n = n(), sk(), n), pk = { class: "layout-container" }, mk = ["id", "aria-checked", "aria-disabled", "aria-labelledby", "tabindex"], Vk = { class: "radio hover focus" }, Tk = /* @__PURE__ */ hk(() => /* @__PURE__ */ en("div", {
  class: "icon",
  "aria-hidden": "true"
}, [
  /* @__PURE__ */ en("svg", { class: "dot" }, [
    /* @__PURE__ */ en("circle", {
      cx: "50%",
      cy: "50%",
      r: "0.3125rem",
      stroke: "black",
      "stroke-width": "0"
    })
  ])
], -1)), Uk = ["id"];
function Rk(n, i, a, l, c, f) {
  const p = bf("feather-ripple");
  return hl(), Wf("div", pk, [
    en("div", {
      class: "feather-radio",
      role: "radio",
      ref: "input",
      id: n.id,
      "aria-checked": n.vm.checked ? "true" : "false",
      "aria-disabled": n.vm.disabled ? "true" : "false",
      "aria-labelledby": n.labelId,
      tabindex: n.tabindex,
      onClick: i[0] || (i[0] = (...V) => n.click && n.click(...V)),
      onBlur: i[1] || (i[1] = (...V) => n.blur && n.blur(...V)),
      "data-ref-id": "feather-radio"
    }, [
      en("div", Vk, [
        Tk,
        n.vm.disabled ? ok("", !0) : (hl(), ik(p, {
          key: 0,
          center: ""
        }))
      ]),
      en("span", {
        class: "label",
        "data-ref-id": "feather-radio-label",
        id: n.labelId
      }, [
        Ef(n.$slots, "default", {}, void 0, !0)
      ], 8, Uk)
    ], 40, mk)
  ]);
}
var $a = /* @__PURE__ */ Ff(fk, [["render", Rk], ["__scopeId", "data-v-24790cf0"]]);
const vk = kf(Ha({}, Uf), {
  label: {
    type: String,
    required: !0
  },
  modelValue: {
    type: [String, Number, Boolean, Array, Object, Date, Function]
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
}), Nk = {
  "update:modelValue": (n) => !0,
  blur: (n) => !0
}, Mk = yf({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  props: vk,
  emits: Nk,
  setup(n, i) {
    Rf(n);
    const a = rl(n, "error"), l = rl(n, "modelValue"), c = Ar(() => Ie("feather-input-description")), f = Ar(() => {
      const p = kf(Ha({}, i.attrs), {
        class: "",
        "aria-describedby": c.value
      });
      return p["aria-invalid"] || (p["aria-invalid"] = !!a.value), p;
    });
    return Ha(Ha({
      descriptionId: c,
      attrs: f
    }, vf(l, i.emit, n.label, n.schema, rl(n, "error"))), Tf(i.attrs));
  },
  components: {
    InputSubText: mf
  }
}), wk = ["for"], Zk = ["id"];
function Jk(n, i, a, l, c, f) {
  const p = bf("InputSubText");
  return hl(), Wf("div", yd(n.inherittedAttrs, {
    class: ["feather-radio-group-container", { vertical: n.vertical }]
  }), [
    en("label", {
      for: n.groupId,
      class: "group-label"
    }, ck(n.label), 9, wk),
    en("div", yd(n.attrs, {
      class: "feather-radio-group",
      "data-ref-id": "feather-radio-group",
      role: "radiogroup",
      id: n.groupId,
      onKeydown: i[0] || (i[0] = (...V) => n.keydown && n.keydown(...V))
    }), [
      Ef(n.$slots, "default", {}, void 0, !0)
    ], 16, Zk),
    uk(p, { id: n.descriptionId }, null, 8, ["id"])
  ], 16);
}
var Sf = /* @__PURE__ */ Ff(Mk, [["render", Jk], ["__scopeId", "data-v-6775aeb9"]]);
const gk = window.Vue.openBlock, kk = window.Vue.createElementBlock, yk = window.Vue.createStaticVNode;
var bk = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Wk = {}, Ek = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Fk = /* @__PURE__ */ yk('<path d="M4,13H14V11H4a1,1,0,0,0,0,2Z"></path><path d="M21,6a1,1,0,0,0-1-1H14V7h6A1,1,0,0,0,21,6Z"></path><path d="M20,17H11v2h9a1,1,0,0,0,0-2Z"></path><path d="M11.5,3h-1a.5.5,0,0,0-.5.5V5H4A1,1,0,0,0,4,7h6V8.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,11.5,3Z"></path><path d="M20,11H18V9.5a.5.5,0,0,0-.5-.5h-1a.5.5,0,0,0-.5.5v5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5V13h2a1,1,0,0,0,0-2Z"></path><path d="M8.5,15h-1a.5.5,0,0,0-.5.5V17H4a1,1,0,0,0,0,2H7v1.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,8.5,15Z"></path>', 6), Sk = [
  Fk
];
function Ak(n, i) {
  return gk(), kk("svg", Ek, Sk);
}
var xk = /* @__PURE__ */ bk(Wk, [["render", Ak]]);
const Qk = window.Vue.openBlock, Bk = window.Vue.createElementBlock, Dk = window.Vue.createStaticVNode;
var Ok = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Gk = {}, Ck = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, zk = /* @__PURE__ */ Dk('<rect x="8" y="7" width="8" height="2" rx="1"></rect><rect x="8" y="11" width="8" height="2" rx="1"></rect><rect x="8" y="15" width="8" height="2" rx="1"></rect><path d="M21,21l-3-3v2H6V9L4,7V20a2,2,0,0,0,2,2H18v2Z"></path><path d="M6,4H18V15l2,2V4a2,2,0,0,0-2-2H6V0L3,3,6,6Z"></path>', 5), Yk = [
  zk
];
function Ik(n, i) {
  return Qk(), Bk("svg", Ck, Yk);
}
var Hk = /* @__PURE__ */ Ok(Gk, [["render", Ik]]);
const Pk = window.Vue.openBlock, jk = window.Vue.createElementBlock, Xk = window.Vue.createElementVNode;
var Lk = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const _k = {}, qk = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Kk = /* @__PURE__ */ Xk("path", { d: "M17,10h5a1,1,0,0,0,1-1V3a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1V5.59L10.59,11H8V9A1,1,0,0,0,7,8H2A1,1,0,0,0,1,9v6a1,1,0,0,0,1,1H7a1,1,0,0,0,1-1V13h2.58L16,18.49V21a1,1,0,0,0,1,1h5a1,1,0,0,0,1-1V15a1,1,0,0,0-1-1H17a1,1,0,0,0-1,1v.64L12.41,12,16,8.42V9A1,1,0,0,0,17,10Zm1,6h3v4H18ZM18,4h3V8H18Z" }, null, -1), $k = [
  Kk
];
function ey(n, i) {
  return Pk(), jk("svg", qk, $k);
}
var ty = /* @__PURE__ */ Lk(_k, [["render", ey]]);
const ny = window.Vue.openBlock, ry = window.Vue.createElementBlock, ay = window.Vue.createStaticVNode;
var iy = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const oy = {}, ly = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, sy = /* @__PURE__ */ ay('<rect x="3" y="3" width="4" height="4"></rect><rect x="10" y="3" width="4" height="4"></rect><rect x="17" y="3" width="4" height="4"></rect><rect x="3" y="10" width="4" height="4"></rect><rect x="10" y="10" width="4" height="4"></rect><rect x="17" y="10" width="4" height="4"></rect><rect x="3" y="17" width="4" height="4"></rect><rect x="10" y="17" width="4" height="4"></rect><rect x="17" y="17" width="4" height="4"></rect>', 9), cy = [
  sy
];
function uy(n, i) {
  return ny(), ry("svg", ly, cy);
}
var dy = /* @__PURE__ */ iy(oy, [["render", uy]]);
const fy = window.Vue.openBlock, hy = window.Vue.createElementBlock, py = window.Vue.createElementVNode;
var my = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const Vy = {}, Ty = {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Uy = /* @__PURE__ */ py("path", { d: "M18,6a4,4,0,0,0-1,7.86V18a2,2,0,0,1-2,2H10a2,2,0,0,1-2-2V12.9a5.5,5.5,0,0,0,4.5-5.4V5a2,2,0,0,0-2-2h-1a1,1,0,0,0,0,2h1V7.5a3.5,3.5,0,0,1-7,0V5h1a1,1,0,0,0,0-2h-1a2,2,0,0,0-2,2V7.5A5.5,5.5,0,0,0,6,12.9V18a4,4,0,0,0,4,4h5a4,4,0,0,0,4-4V13.86A4,4,0,0,0,18,6Zm0,6a2,2,0,1,1,2-2A2,2,0,0,1,18,12Z" }, null, -1), Ry = [
  Uy
];
function vy(n, i) {
  return fy(), hy("svg", Ty, Ry);
}
var Ny = /* @__PURE__ */ my(Vy, [["render", vy]]);
const Af = () => window.VRouter || Al, My = window.Vue.defineComponent, de = window.Vue.createElementVNode, Br = window.Vue.createTextVNode, be = window.Vue.unref, xt = window.Vue.createVNode, Ga = window.Vue.withCtx, bd = window.Vue.openBlock, Wd = window.Vue.createElementBlock, wy = window.Vue.createCommentVNode, Zy = window.Vue.pushScopeId, Jy = window.Vue.popScopeId, ft = (n) => (Zy("data-v-0fd6b0cd"), n = n(), Jy(), n), gy = { class: "container" }, ky = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("div", { class: "title" }, "ALEC", -1)), yy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("div", { class: "description" }, [
  /* @__PURE__ */ Br(" The Architecture for Learning Enabled Correlation, or ALEC, provides a machine learning powered solution for alarm correlation. Complex networks produce significant, and potentially overwhelming, amount of events and alarms. "),
  /* @__PURE__ */ de("br"),
  /* @__PURE__ */ de("strong", null, "Our goal"),
  /* @__PURE__ */ Br(" with ALEC to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. ")
], -1)), by = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("h3", null, "Key Benefits:", -1)), Wy = { class: "list" }, Ey = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("span", null, " Alleviate alarm load by clustering them into actionable situations. ", -1)), Fy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("span", null, "Assist in root cause analysis.", -1)), Sy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("span", null, " Avoid potential issues flagged by alarms with low visibility. ", -1)), Ay = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("span", null, "Easy configuration.", -1)), xy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("span", null, " Continuous improvement. Provided anonymized data helps us improve ALEC. ", -1)), Qy = {
  key: 0,
  class: "optin"
}, By = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("h3", null, "Enable ALEC for more precise results:", -1)), Dy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("div", null, " Our goal with ALEC, is to aid IT Operators to tackle these issues effectively identifying alarm groups that are easier to manage. The information provided is anonymized and will be used for internal purposes only. ", -1)), Oy = { class: "choices" }, Gy = /* @__PURE__ */ ft(() => /* @__PURE__ */ de("div", { class: "radio-text" }, " By choosing \u201CYes\u201D you accept that OpenNMS can store the information that you provide, for educational and research purposes. ", -1)), Cy = /* @__PURE__ */ Br("Yes"), zy = /* @__PURE__ */ Br("No"), Yy = /* @__PURE__ */ Br(" Continue "), Iy = window.Vue.ref, Hy = /* @__PURE__ */ My({
  __name: "WelcomePage",
  setup(n) {
    const i = Gr(), a = Af(), l = Iy(!0), c = () => {
      const f = Boolean(l.value);
      i.savePermission(f), f ? a.push({ name: "configuration" }) : a.push({ name: "situations" });
    };
    return (f, p) => (bd(), Wd("div", gy, [
      ky,
      yy,
      de("div", null, [
        by,
        de("div", Wy, [
          de("div", null, [
            xt(be(Rt), {
              icon: be(dy),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Ey
          ]),
          de("div", null, [
            xt(be(Rt), {
              icon: be(ty),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Fy
          ]),
          de("div", null, [
            xt(be(Rt), {
              icon: be(Ny),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Sy
          ]),
          de("div", null, [
            xt(be(Rt), {
              icon: be(xk),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            Ay
          ]),
          de("div", null, [
            xt(be(Rt), {
              icon: be(Hk),
              "aria-hidden": "true",
              class: "icon"
            }, null, 8, ["icon"]),
            xy
          ])
        ])
      ]),
      be(i).isAdmin ? (bd(), Wd("div", Qy, [
        By,
        Dy,
        de("div", Oy, [
          Gy,
          xt(be(Sf), {
            horizontal: "",
            modelValue: l.value,
            "onUpdate:modelValue": p[0] || (p[0] = (V) => l.value = V),
            label: ""
          }, {
            default: Ga(() => [
              xt(be($a), { value: !0 }, {
                default: Ga(() => [
                  Cy
                ]),
                _: 1
              }),
              xt(be($a), { value: !1 }, {
                default: Ga(() => [
                  zy
                ]),
                _: 1
              })
            ]),
            _: 1
          }, 8, ["modelValue"])
        ])
      ])) : wy("", !0),
      xt(be(Ka), {
        "data-test": "continue-btn",
        primary: "",
        class: "continue-btn",
        onClick: c
      }, {
        default: Ga(() => [
          Yy
        ]),
        _: 1
      })
    ]));
  }
});
const Py = /* @__PURE__ */ Pe(Hy, [["__scopeId", "data-v-0fd6b0cd"]]), jy = "data:image/png;base64,", Xy = "data:image/png;base64,";
var Ly = Object.defineProperty, _y = Object.defineProperties, qy = Object.getOwnPropertyDescriptors, Ed = Object.getOwnPropertySymbols, Ky = Object.prototype.hasOwnProperty, $y = Object.prototype.propertyIsEnumerable, Fd = (n, i, a) => i in n ? Ly(n, i, { enumerable: !0, configurable: !0, writable: !0, value: a }) : n[i] = a, pl = (n, i) => {
  for (var a in i || (i = {}))
    Ky.call(i, a) && Fd(n, a, i[a]);
  if (Ed)
    for (var a of Ed(i))
      $y.call(i, a) && Fd(n, a, i[a]);
  return n;
}, xf = (n, i) => _y(n, qy(i));
const Qf = window.Vue.defineComponent, eb = window.Vue.inject, tb = window.Vue.resolveComponent, al = window.Vue.openBlock, Sd = window.Vue.createElementBlock, In = window.Vue.createElementVNode, nb = window.Vue.createBlock, Ad = window.Vue.createCommentVNode, rb = window.Vue.renderSlot, ab = window.Vue.pushScopeId, ib = window.Vue.popScopeId, il = window.Vue.toRef, Ca = window.Vue.computed, ob = window.Vue.ref;
window.Vue.mergeProps;
window.Vue.toDisplayString;
window.Vue.createVNode;
var lb = (n, i) => {
  const a = n.__vccOpts || n;
  for (const [l, c] of i)
    a[l] = c;
  return a;
};
const sb = {
  disabled: {
    type: Boolean,
    default: !1
  },
  modelValue: {
    type: Boolean,
    default: !1
  },
  indeterminate: {
    type: Boolean,
    default: !1
  },
  label: {
    type: String
  },
  noFocus: {
    type: Boolean,
    default: !1
  }
}, cb = {
  "update:modelValue": (n) => !0,
  click: (n) => !0,
  indeterminate: (n) => !0
}, ub = Qf({
  model: {
    prop: "modelValue",
    event: "update:modelValue"
  },
  emits: cb,
  props: sb,
  mounted() {
    this.registerCheckbox && this.registerCheckbox(this.inputId);
  },
  setup() {
    return {
      registerCheckbox: eb("registerCheckbox", (i) => {
      })
    };
  },
  computed: {
    inputId() {
      return this.label && this.label.length ? void 0 : Ie("checkbox");
    },
    labelId() {
      return this.label && this.label.length ? void 0 : Ie("checkbox-label");
    }
  },
  watch: {
    indeterminate: {
      handler(n) {
        this.$emit("update:modelValue", void 0), this.$emit("indeterminate", n);
      }
    }
  },
  methods: {
    focus() {
      this.$refs.input.focus();
    },
    updateValue() {
      this.disabled || (this.indeterminate ? (this.$emit("update:modelValue", !0), this.$emit("indeterminate", !1)) : (this.$emit("update:modelValue", !this.modelValue), this.$emit("indeterminate", !1)));
    },
    click(n) {
      this.focus(), this.updateValue(), this.$emit("click", n);
    },
    keydown(n) {
      (n.keyCode === Qt.SPACE || n.keyCode === Qt.ENTER) && this.updateValue(), n.keyCode === Qt.SPACE && n.preventDefault();
    }
  },
  components: {
    FeatherRipple: Ln
  }
}), db = (n) => (ab("data-v-a7af27e2"), n = n(), ib(), n), fb = { class: "layout-container" }, hb = ["aria-checked", "aria-disabled", "aria-labelledby", "aria-label", "id", "tabindex"], pb = { class: "checkbox hover focus" }, mb = /* @__PURE__ */ db(() => /* @__PURE__ */ In("div", { class: "box" }, [
  /* @__PURE__ */ In("svg", {
    class: "checkmark",
    role: "presentation",
    focusable: "false",
    viewBox: "0 0 24 24"
  }, [
    /* @__PURE__ */ In("path", {
      class: "path",
      fill: "none",
      d: "M1.73,12.91 8.1,19.28 22.79,4.59"
    })
  ]),
  /* @__PURE__ */ In("div", { class: "indeterminate" })
], -1)), Vb = ["id", "for"];
function Tb(n, i, a, l, c, f) {
  const p = tb("feather-ripple");
  return al(), Sd("div", fb, [
    In("div", {
      class: "feather-checkbox feather-form-input feather-checkbox-table",
      onClick: i[0] || (i[0] = (...V) => n.click && n.click(...V)),
      onKeydown: i[1] || (i[1] = (...V) => n.keydown && n.keydown(...V)),
      "aria-checked": n.modelValue ? "true" : n.indeterminate ? "mixed" : "false",
      "aria-disabled": n.disabled,
      "aria-labelledby": n.labelId,
      "aria-label": n.label,
      id: n.inputId,
      tabindex: n.noFocus ? -1 : 0,
      ref: "input",
      role: "checkbox",
      "data-ref-id": "feather-checkbox"
    }, [
      In("div", pb, [
        mb,
        n.disabled ? Ad("", !0) : (al(), nb(p, {
          key: 0,
          center: ""
        }))
      ]),
      n.label ? Ad("", !0) : (al(), Sd("label", {
        key: 0,
        "data-ref-id": "feather-checkbox-label",
        id: n.labelId,
        for: n.inputId
      }, [
        rb(n.$slots, "default", {}, void 0, !0)
      ], 8, Vb))
    ], 40, hb)
  ]);
}
var Ub = /* @__PURE__ */ lb(ub, [["render", Tb], ["__scopeId", "data-v-a7af27e2"]]);
const Rb = xf(pl({}, Uf), {
  modelValue: {
    type: [Array, Object],
    required: !1
  },
  label: {
    type: String,
    required: !0
  },
  vertical: {
    type: Boolean,
    default: !1
  },
  schema: {
    type: Object,
    required: !1
  }
});
Qf({
  props: Rb,
  provide() {
    return {
      registerCheckbox: this.registerCheckbox
    };
  },
  setup(n, i) {
    Rf(n);
    const a = il(n, "error"), l = Ca(() => Ie("feather-checkbox-group")), c = Ca(() => Ie("feather-input-description")), f = Ca(() => Ie("feather-input-label")), p = Ca(() => {
      const J = JSON.parse(JSON.stringify(i.attrs));
      return J["aria-invalid"] || (J["aria-invalid"] = !!a.value), xf(pl({}, J), {
        class: "",
        "aria-describedby": c.value
      });
    }), V = ob(l.value), { validate: v } = Vf(V, il(n, "modelValue"), n.label, n.schema, il(n, "error"));
    return pl({
      groupId: l,
      inputId: V,
      descriptionId: c,
      labelId: f,
      attrs: p,
      validate: v,
      registerCheckbox: (J) => {
        J && V.value === l.value && (V.value = J);
      }
    }, Tf(i.attrs));
  },
  components: {
    InputSubText: mf
  }
});
const vb = window.Vue.defineComponent, Te = window.Vue.createElementVNode, ml = window.Vue.createTextVNode, _t = window.Vue.unref, yr = window.Vue.withCtx, br = window.Vue.createVNode, Nb = window.Vue.openBlock, Mb = window.Vue.createElementBlock, wb = window.Vue.pushScopeId, Zb = window.Vue.popScopeId, Rn = (n) => (wb("data-v-9a5b5f4e"), n = n(), Zb(), n), Jb = { class: "container" }, gb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("h2", null, "Configuration", -1)), kb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("p", null, [
  /* @__PURE__ */ ml(" ALEC relies on correlation engines to identify related alarm groupings (situations). These engines are powered by machine learning techniques that leverage alarms data to make informed decisions. "),
  /* @__PURE__ */ Te("br"),
  /* @__PURE__ */ ml(" For detail information about proposed engines you can read "),
  /* @__PURE__ */ Te("strong", null, [
    /* @__PURE__ */ Te("a", {
      target: "_blank",
      href: "https://docs.opennms.com/alec/3.0.0-SNAPSHOT/engines/cluster.html"
    }, " here ")
  ])
], -1)), yb = { class: "radio-content" }, bb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("strong", { class: "title" }, "Clustering", -1)), Wb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("div", null, " Groups data points (alarms) based on a distance measure. We calculate alarms difference in time and add it to their distance within their network topology ", -1)), Eb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("img", {
  class: "img",
  src: jy
}, null, -1)), Fb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("div", { class: "hellinger" }, [
  /* @__PURE__ */ Te("strong", null, "With hellinger distance"),
  /* @__PURE__ */ Te("br"),
  /* @__PURE__ */ Te("span", { class: "description" }, " (Uses the Hellinger Distance between alarms as a scaling variable. It pushes alarms further apart if its value is high and vice versa.) ")
], -1)), Sb = /* @__PURE__ */ Rn(() => /* @__PURE__ */ Te("div", { class: "radio-content" }, [
  /* @__PURE__ */ Te("strong", { class: "title" }, "Deep Learning"),
  /* @__PURE__ */ Te("div", null, " A Neural Network network is consulted to assess if alarms are related. Based on its evaluation, situations are built by association. "),
  /* @__PURE__ */ Te("img", {
    class: "img img2",
    src: Xy
  })
], -1)), Ab = /* @__PURE__ */ ml(" Continue "), xd = window.Vue.ref, xb = /* @__PURE__ */ vb({
  __name: "ConfigurationPage",
  setup(n) {
    const i = Gr(), a = Af(), l = xd(!1), c = xd(Yn.ENGINE_DBSCAN), f = () => {
      cf(i.allowSave), Uw(c.value, l.value), a.push({ name: "situations" });
    };
    return (p, V) => (Nb(), Mb("div", Jb, [
      gb,
      kb,
      Te("div", null, [
        br(_t(Sf), {
          class: "radio-group",
          label: "Currently, ALEC provides two clustering based engines, please, select one (can be changed later):",
          modelValue: c.value,
          "onUpdate:modelValue": V[1] || (V[1] = (v) => c.value = v)
        }, {
          default: yr(() => [
            br(_t($a), {
              class: "radio",
              value: _t(Yn).ENGINE_DBSCAN
            }, {
              default: yr(() => [
                Te("div", yb, [
                  bb,
                  Wb,
                  Eb,
                  br(_t(Ub), {
                    modelValue: l.value,
                    "onUpdate:modelValue": V[0] || (V[0] = (v) => l.value = v),
                    disabled: c.value !== _t(Yn).ENGINE_DBSCAN,
                    class: "checkbox"
                  }, {
                    default: yr(() => [
                      Fb
                    ]),
                    _: 1
                  }, 8, ["modelValue", "disabled"])
                ])
              ]),
              _: 1
            }, 8, ["value"]),
            br(_t($a), {
              class: "radio",
              value: _t(Yn).ENGINE_DEEP_LEARNING
            }, {
              default: yr(() => [
                Sb
              ]),
              _: 1
            }, 8, ["value"])
          ]),
          _: 1
        }, 8, ["label", "modelValue"])
      ]),
      br(_t(Ka), {
        primary: "",
        class: "btn",
        onClick: V[2] || (V[2] = () => f())
      }, {
        default: yr(() => [
          Ab
        ]),
        _: 1
      })
    ]));
  }
});
const Qb = /* @__PURE__ */ Pe(xb, [["__scopeId", "data-v-9a5b5f4e"]]), Bb = window.VueRouter.createRouter, Db = window.VueRouter.createWebHistory, ol = async () => {
  const n = window.VRouter || Al, i = Gr();
  i.userId || (await i.getUserRole(), await i.getAlecInfo(), n.push({ name: "home" }));
}, Bf = [
  {
    path: "/",
    name: "home",
    beforeEnter: async (n) => {
      const i = window.VRouter || Al, a = Gr();
      await a.getUserRole(), await a.getAlecInfo(), a.firstTime ? i.push({ name: "welcome", params: n.params }) : i.push({ name: "situations", params: n.params });
    },
    component: {}
  },
  {
    path: "/welcome",
    name: "welcome",
    beforeEnter: () => ol(),
    component: Py
  },
  {
    path: "/setup",
    name: "configuration",
    beforeEnter: () => ol(),
    component: Qb
  },
  {
    path: "/situations",
    name: "situations",
    beforeEnter: () => ol(),
    component: qg
  }
], Qd = window.VRouter;
if (Qd)
  for (const n of Bf) {
    const { path: i, name: a, component: l, beforeEnter: c } = n;
    Qd.addRoute("Plugin", {
      path: i.slice(1),
      name: a,
      component: l,
      beforeEnter: c
    });
  }
const Al = Bb({
  history: Db(),
  routes: Bf
});
window.Vue.createApp;
window.Pinia.createPinia;
window.uiextension = D0;
